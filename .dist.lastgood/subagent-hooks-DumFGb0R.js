...
async function handleMatrixSubagentSpawning(api, event) {
  ...
  try {
    return {
      ...
      deliveryOrigin: resolveMatrixBindingDeliveryOrigin(
...
async function handleMatrixSubagentEnded(event) {
  ...
  const matching = (accountId ? listBindingsForAccount(accountId) : listAllBindings()).filter(
  ...
  if (event.sendFarewell) {
    ...
    for (const binding of matching) {
      ...
      if (
        ...
        ).some((entry) => entry.bindingId === bindingId)
      )
...
function handleMatrixSubagentDeliveryTarget(event) {
  ...
  const requesterThreadId =
    event.requesterOrigin?.threadId != null && event.requesterOrigin.threadId !== ""
      ...
      : "";
  const bindings = (
    requesterAccountId ? listBindingsForAccount(requesterAccountId) : listAllBindings()
  ).filter(
...
//#endregion
export {
  handleMatrixSubagentEnded as n,
  handleMatrixSubagentSpawning as r,
  handleMatrixSubagentDeliveryTarget as t,
};
