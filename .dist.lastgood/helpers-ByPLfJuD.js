...
function detectCronDenialToken(text) {
  ...
  for (const token of CRON_DENIAL_CASE_INSENSITIVE_TOKENS)
    if (lowerText.includes(token)) return token;
}
...
function resolveCronPayloadOutcome(params) {
  ...
  const hasPendingPresentationWarning =
    ...
    (normalizedFinalAssistantVisibleText !== void 0 || hasSuccessfulPayloadBeforeLastError);
  ...
  const shouldUseFinalAssistantVisibleText =
    ...
    normalizedFinalAssistantVisibleText !== void 0 &&
...
//#endregion
export {
  resolveHeartbeatAckMaxChars as a,
  resolveCronPayloadOutcome as i,
  pickLastNonEmptyTextFromPayloads as n,
  pickSummaryFromOutput as r,
  isHeartbeatOnlyResponse as t,
};
