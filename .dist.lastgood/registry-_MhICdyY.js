...
import { t as loadBundledCapabilityRuntimeRegistry } from "./bundled-capability-runtime-CVwneU9i.js";
import {
  b as getPackageManifestMetadata,
  y as PLUGIN_MANIFEST_FILENAME,
} from "./discovery-bnS95tO3.js";
import { u as tryReadJsonSync } from "./json-files-DifBk3kt.js";
import { t as loadPluginManifestRegistry } from "./manifest-registry-DJ43wWua.js";
import {
  a as resolveBundledPluginScanDir,
  i as normalizeBundledPluginStringList,
} from "./module-export-CpPaSiil.js";
import { a as resolveManifestContractPluginIds } from "./plugin-registry-CWiNK0Xv.js";
...
import { l as resolveLoaderPackageRoot } from "./sdk-alias-B6hytWos.js";
import { a as normalizeLowercaseStringOrEmpty } from "./string-coerce-BdEutqX5.js";
import { i as resolveBundledExplicitWebSearchProvidersFromPublicArtifacts } from "./web-provider-public-artifacts.explicit-CjQ-BUNt.js";
...
function readBundledCapabilityManifest(pluginDir) {
  if (
    normalizeBundledPluginStringList(
      getPackageManifestMetadata(readJsonRecord(path.join(pluginDir, "package.json")))?.extensions,
    ).length === 0
  )
    return;
...
const BUNDLED_PLUGIN_CONTRACT_SNAPSHOTS = BUNDLED_CAPABILITY_MANIFESTS.map(
...
function loadVitestCapabilityContractEntries(params) {
  ...
  if (new Set(bulkEntries.map((entry) => entry.pluginId)).size === pluginIds.length)
    return bulkEntries;
...
function loadProviderContractRegistry() {
  try {
    providerContractLoadError = void 0;
    const publicArtifactEntries = resolveBundledProviderContractPluginIds().flatMap(
      (pluginId) =>
...
function loadWebSearchProviderContractRegistry() {
  const publicArtifactEntries = resolveBundledManifestContractPluginIds(
    "webSearchProviders",
  ).flatMap((pluginId) =>
    (
      resolveBundledExplicitWebSearchProvidersFromPublicArtifacts({ onlyPluginIds: [pluginId] }) ??
      []
    ).map((provider) => ({
...
const pluginRegistrationContractRegistry = createLazyArrayView(
  loadPluginRegistrationContractRegistry,
);
//#endregion
export {
  resolveWebFetchProviderContractEntriesForPluginId as a,
  resolveProviderContractProvidersForPluginIds as i,
  providerContractLoadError as n,
  resolveWebSearchProviderContractEntriesForPluginId as o,
  resolveProviderContractPluginIdsForProviderAlias as r,
  resolveBundledExplicitProviderContractsFromPublicArtifacts as s,
  pluginRegistrationContractRegistry as t,
};
