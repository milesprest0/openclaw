...
import {
  a as collectStateDeepFilesystemFindings,
  i as collectSandboxBrowserHashLabelFindings,
  o as readConfigSnapshotForAudit,
  s as shouldIgnoreInstalledPluginDirName,
  t as collectIncludeFilePermFindings,
} from "./audit-extra.async-Dywl5vOO.js";
...
import { r as resolveNativeSkillsEnabled } from "./commands-DMXWyZVb.js";
import "./scan-paths-BLf-vGUd.js";
import "./auth-D_VLzYQI.js";
import { i as resolveSandboxConfigForAgent } from "./config-aLdl9MiY.js";
import { s as normalizePluginsConfig } from "./config-state-BF1r9Z0v.js";
import { t as hasConfiguredInternalHooks } from "./configured-CykWRQiU.js";
import { r as DEFAULT_PROVIDER } from "./defaults-4m7JJmD2.js";
import { t as resolveAllowedAgentIds } from "./hooks-policy-BFK3vbP8.js";
import { s as loadManifestMetadataSnapshot } from "./manifest-contract-eligibility-Bpx8eP31.js";
import {
...
import { t as inferParamBFromIdOrName } from "./model-param-b-DfM8glvH.js";
import {
  _ as modelKey,
  h as resolveModelRefFromString,
  i as buildModelAliasIndex,
} from "./model-selection-shared-D9oQINZ2.js";
import { n as isDangerousNetworkMode, r as normalizeNetworkMode } from "./network-mode-CAACLKz5.js";
import {
  i as listDangerousPluginNodeCommands,
  o as resolveNodeCommandAllowlist,
  t as DEFAULT_DANGEROUS_NODE_COMMANDS,
} from "./node-command-policy-y9_MR0c9.js";
import { i as readInstalledPackageVersion } from "./package-update-utils-XrztudBX.js";
import { i as isPathInside } from "./path-1liOXr_N.js";
import {
  g as createPluginRegistryIdNormalizer,
  p as loadPluginRegistrySnapshot,
} from "./plugin-registry-CWiNK0Xv.js";
import { t as inspectReadOnlyChannelAccount } from "./read-only-account-inspect-BHBr3X3O.js";
import { n as listReadOnlyChannelPluginsForConfig } from "./read-only-DzM-sROf.js";
...
import { r as resolveSandboxToolPolicyForAgent } from "./tool-policy-DpDV9E2l.js";
...
import { t as getBlockedBindReason } from "./validate-sandbox-security-B91sDgd8.js";
...
async function loadPluginTrustPolicyDeps() {
  pluginTrustPolicyDepsPromise ??= Promise.all([
    import("./config-BEvRNU70.js"),
    import("./tool-policy-BWGUOJTj.js"),
    import("./tool-policy-match-Dn0woysQ.js"),
    import("./tool-policy-CX5HxzLW.js"),
    import("./audit-tool-policy-Bhy3hQ84.js"),
...
async function isChannelPluginConfigured(cfg, plugin) {
  ...
  for (const accountId of candidates) {
    ...
    const inspectedRecord =
      inspected && typeof inspected === "object" && !Array.isArray(inspected) ? inspected : null;
...
async function listInstalledPluginDirs(params) {
  ...
  if (
    !(
      await fs.stat(extensionsDir).catch((err) => {
  ...
  return {
    extensionsDir,
    pluginDirs: (
      await fs.readdir(extensionsDir, { withFileTypes: true }).catch((err) => {
        params.onReadError?.(err);
        return [];
      })
    )
...
function resolveToolPolicies$2(params) {
  ...
  if (params.sandboxMode === "all")
    policies.push(
      params.deps.resolveSandboxToolPolicyForAgent(params.cfg, params.agentId ?? void 0),
    );
  return policies;
}
  ...
  if (npmPluginInstalls.length > 0) {
    ...
    for (const [pluginId, record] of npmPluginInstalls) {
      ...
      const installedVersion = await readInstalledPackageVersion(
        record.installPath ?? path.join(params.stateDir, "extensions", pluginId),
      );
...
function hasConfiguredWebSearchCredential(params) {
  return (
    hasConfiguredSearchCredentialCandidate(
      params.searchConfig ?? params.config.tools?.web?.search,
    ) ||
  ...
  for (const agent of list) {
    ...
    if (typeof model === "string") add(model, `agents.list.${id}.model`);
      ...
      if (Array.isArray(fallbacks))
        for (const fallback of fallbacks) add(fallback, `agents.list.${id}.model.fallbacks`);
    }
  }
  return out;
}
...
function resolveToolPolicies$1(params) {
  const policies = [];
  const profilePolicy = resolveToolProfilePolicy(
    params.agentTools?.profile ?? params.cfg.tools?.profile,
  );
  ...
  if (params.sandboxMode === "all")
    policies.push(resolveSandboxToolPolicyForAgent(params.cfg, params.agentId ?? void 0));
  return policies;
}
...
function isGatewayRemotelyExposed(cfg) {
  if ((typeof cfg.gateway?.bind === "string" ? cfg.gateway.bind : "loopback") !== "loopback")
    return true;
  ...
  for (const [channelId, value] of Object.entries(channels)) {
    ...
    if (accounts && typeof accounts === "object")
      for (const [accountId, accountVal] of Object.entries(accounts)) {
        ...
        if (accountVal.groupPolicy === "open")
...
function listPotentialMultiUserSignals(cfg) {
  ...
  const inspectSection = (section, basePath) => {
    ...
    if ((typeof section.dmPolicy === "string" ? section.dmPolicy : null) === "open")
      out.add(`${basePath}.dmPolicy="open"`);
    if (
      (Array.isArray(section.allowFrom) ? section.allowFrom : []).some((entry) =>
        isWildcardEntry(entry),
      )
    )
      out.add(`${basePath}.allowFrom includes "*"`);
    if (
      (Array.isArray(section.groupAllowFrom) ? section.groupAllowFrom : []).some((entry) =>
        isWildcardEntry(entry),
      )
    )
    ...
    if (dm && typeof dm === "object") {
      const dmSection = dm;
      if ((typeof dmSection.policy === "string" ? dmSection.policy : null) === "open")
        out.add(`${basePath}.dm.policy="open"`);
      if (
        (Array.isArray(dmSection.allowFrom) ? dmSection.allowFrom : []).some((entry) =>
          isWildcardEntry(entry),
        )
      )
...
function collectHooksHardeningFindings(cfg, env = process.env) {
  ...
  if ((normalizeOptionalString(cfg.hooks?.path) ?? "") === "/")
    findings.push({
  ...
  for (const { source, docker } of configs) {
    ...
    if (
      normalizeOptionalLowercaseString(
        typeof docker.seccompProfile === "string" ? docker.seccompProfile : void 0,
      ) === "unconfined"
    )
      findings.push({
    ...
    if (
      normalizeOptionalLowercaseString(
        typeof docker.apparmorProfile === "string" ? docker.apparmorProfile : void 0,
      ) === "unconfined"
    )
      findings.push({
...
function collectNodeDangerousAllowCommandFindings(cfg) {
  ...
  findings.push({
    ...
    remediation:
      "Remove these entries from gateway.nodes.allowCommands (recommended). If you keep them, treat gateway auth as full operator access and keep gateway exposure local/tailnet-only.",
  });
  return findings;
}
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    if (truncated)
      findings.push({
        ...
        remediation:
          "Flatten or simplify the skills/ directory hierarchy to stay within the scan budget, or move deeply-nested skill collections to a managed skill location.",
      });
