...
import { t as ensureMatrixCryptoRuntime } from "./deps-CSz6o0Ol.js";
import { t as formatMatrixErrorMessage } from "./errors-DQ-r4gaL.js";
import { a as withFileLock } from "./file-lock-C5nUdr1w.js";
import { n as LogService, r as noop } from "./logger-DwhV1i8m.js";
import {
  i as MATRIX_IDB_SNAPSHOT_LOCK_OPTIONS,
  n as isRepairableSecretStorageAccessError,
} from "./recovery-key-store-B5hhZBrh.js";
...
const extractKey = (keyPath, value) => {
  ...
  while (remainingKeyPath !== null) {
    ...
    if (
      !(
        (identifier === "length" && (typeof object === "string" || Array.isArray(object))) ||
      ...
      (typeof object !== "object" || object === null || !Object.hasOwn(object, identifier))
    )
    ...
    if (this.direction === "next") {
      ...
      for (const record of records.values(range)) {
        ...
        const cmpResultPosition =
          this._position !== void 0 ? cmp(record.key, this._position) : void 0;
        if (key !== void 0) {
...
var FakeEventTarget = class {
  ...
  dispatchEvent(event) {
    ...
    for (const obj of event.eventPath)
      if (!event.propagationStopped) invokeEventListeners(event, obj);
...
const enforceRange = (num, type) => {
  if (isNaN(num) || num < 0 || num > (type === "unsigned long" ? 4294967295 : 9007199254740991))
    throw new TypeError();
...
var BinarySearchTree = class {
  ...
  _put(node, record, noOverwrite) {
    ...
    if (comparison < 0)
      if (node.left) return this._put(node.left, record, noOverwrite);
      else {
        node.left = {
          record,
          left: void 0,
      ...
      if (node.right) return this._put(node.right, record, noOverwrite);
      else {
        node.right = {
          record,
          left: void 0,
  ...
  storeRecord(newRecord) {
    ...
    if (!this.multiEntry || !Array.isArray(indexKey)) {
      ...
      for (const individualIndexKey of indexKey)
        if (this.records.get(individualIndexKey)) throw new ConstraintError();
    }
    if (!this.multiEntry || !Array.isArray(indexKey))
      this.records.put({
        key: indexKey,
...
var FDBObjectStore = class {
  ...
  createIndex(name, keyPath, optionalParameters = {}) {
    if (arguments.length < 2) throw new TypeError();
    const multiEntry =
      optionalParameters.multiEntry !== void 0 ? optionalParameters.multiEntry : false;
  ...
  deleteIndex(name) {
    ...
    this.transaction._execRequestAsync({
      operation: () => {
        if (rawIndex === this._rawObjectStore.rawIndexes.get(name))
...
var FDBTransaction = class extends FakeEventTarget {
  ...
  _abort(errName) {
    ...
    queueTask(() => {
      ...
      if (isUpgradeTransaction)
        this.db._rawDatabase.connections = this.db._rawDatabase.connections.filter(
          (connection) => !connection._rawDatabase.transactions.includes(this),
        );
  ...
  _execRequestAsync(obj) {
    ...
    if (!request)
      if (!source) request = new FDBRequest();
      else {
...
var ObjectStore = class {
  ...
  storeRecord(newRecord, noOverwrite, rollbackLog) {
    ...
    try {
      for (const rawIndex of this.rawIndexes.values())
        if (rawIndex.initialized) rawIndex.storeRecord(newRecord);
    } catch (err) {
      if (err.name === "ConstraintError")
        for (const rollback of rollbackLogForThisOperation) rollback();
      throw err;
    }
...
const closeConnection = (connection, forced = false) => {
  ...
  if (
    connection._rawDatabase.transactions.every((transaction) => {
...
var FDBDatabase = class extends FakeEventTarget {
  ...
  createObjectStore(name, options = {}) {
    ...
    const autoIncrement =
      options !== null && options.autoIncrement !== void 0 ? options.autoIncrement : false;
  ...
  transaction(storeNames, mode, options) {
    ...
    if (
      this._rawDatabase.transactions.some((transaction) => {
        return (
          transaction._state === "active" &&
  ...
  processTransactions() {
    queueTask(() => {
      const running = this.transactions.filter(
        (transaction) => transaction._started && transaction._state !== "finished",
      );
      ...
      const next = waiting.find((transaction, i) => {
        if (
          running.some(
            (other) =>
              !(transaction.mode === "readonly" && other.mode === "readonly") &&
        ...
        return !waiting
          .slice(0, i)
          .some((other) => intersection(other._scope, transaction._scope).size > 0);
      });
      if (next) {
...
function validateRequiredArguments(numArguments, expectedNumArguments, methodName) {
  if (numArguments < expectedNumArguments)
    throw new TypeError(
      `${methodName}: At least ${expectedNumArguments} ${expectedNumArguments === 1 ? "argument" : "arguments"} required, but only ${arguments.length} passed`,
    );
}
//#endregion
...
const waitForOthersClosedDelete = (databases, name, openDatabases, cb) => {
  if (
    openDatabases.some((openDatabase2) => {
  ...
  const deleteDBTask = () => {
    return new Promise((resolve) => {
      ...
      try {
        ...
        queueTask(() => {
          if (
            openConnections.some((openDatabase3) => {
...
const runVersionchangeTransaction = (connection, version, request, cb) => {
  ...
  queueTask(() => {
    ...
    const waitForOthersClosed = () => {
      if (
        openConnections.some((openDatabase2) => {
          return !openDatabase2._closed && !openDatabase2._closePending;
        })
      ) {
  ...
  databases() {
    return Promise.resolve(
      Array.from(this._databases.entries(), ([name, database]) => {
        ...
        return {
          name,
          version: activeVersionChangeConnection
...
var MatrixCryptoBootstrapper = class {
  ...
  async trustFreshOwnIdentity(crypto) {
    const ownIdentity =
      typeof crypto.getOwnIdentity === "function"
        ? await crypto.getOwnIdentity().catch(() => void 0)
        : void 0;
  ...
  async ensureOwnDeviceTrust(crypto, options) {
    ...
    if (typeof crypto.crossSignDevice === "function") {
      if (
        typeof crypto.isCrossSigningReady === "function" ? await crypto.isCrossSigningReady() : true
      )
        await crypto.crossSignDevice(deviceId);
    }
    const verified = isMatrixDeviceOwnerVerified(
      typeof crypto.getDeviceVerificationStatus === "function"
        ? await crypto.getDeviceVerificationStatus(userId, deviceId).catch(() => null)
        : null,
    );
...
var MatrixDecryptBridge = class {
  ...
  async drainPendingDecryptions(reason) {
    for (let attempts = 0; attempts < MATRIX_DECRYPT_RETRY_MAX_ATTEMPTS; attempts += 1) {
      ...
      if (!Array.from(this.decryptRetries.values()).some((state) => state.timer || state.inFlight))
        return;
    }
  }
  handleEncryptedEventDecrypted(params) {
    ...
    if (isDecryptionFailure(params.decryptedEvent)) {
      this.emitFailedDecryptionOnce(
        ...
        decryptedRoomId,
        decryptedRaw,
        /* @__PURE__ */ new Error("Matrix event failed to decrypt"),
      );
      if (shouldRetryDecryptionFailure(params.decryptedEvent))
        this.scheduleDecryptRetry({
          event: params.encryptedEvent,
  ...
  pruneDecryptedMessageDedupe(now) {
    ...
    for (const [key, createdAt] of this.decryptedMessageDedupe)
      if (now - createdAt > ttlMs) this.decryptedMessageDedupe.delete(key);
...
async function restoreIdbFromDisk(snapshotPath) {
  ...
  for (const resolvedPath of candidatePaths) {
    ...
    try {
      if (
        await withFileLock(resolvedPath, MATRIX_IDB_SNAPSHOT_LOCK_OPTIONS, async () => {
...
var MatrixVerificationManager = class {
  ...
  pruneVerificationSessions(nowMs) {
    for (const [id, session] of this.verificationSessions) {
      ...
      if (
        ...
        nowMs - session.updatedAtMs > TERMINAL_SESSION_RETENTION_MS
      )
  ...
  findVerificationSession(id) {
    ...
    const transactionMatches = Array.from(this.verificationSessions.values()).filter((session) => {
      return (
        this.readRequestValue(session.request, () => session.request.transactionId?.trim(), "") ===
        id
      );
    });
  ...
  maybeAutoStartInboundSas(session) {
    ...
    if (!(methods.includes(VerificationMethod.Sas) || chosenMethod === VerificationMethod.Sas))
      return;
  ...
  maybeAutoConfirmSas(session) {
    ...
    session.sasAutoConfirmTimer = setTimeout(() => {
      session.sasAutoConfirmTimer = void 0;
      if (
        this.readVerificationPhase(session.request, VerificationPhase.Requested) >=
        VerificationPhase.Cancelled
      )
        return;
      session.sasAutoConfirmStarted = true;
      this.confirmSasForSession(session, callbacks, { trustOwnDevice: true })
        .then(() => {
          this.touchVerificationSession(session);
        })
        .catch((err) => {
  ...
  ensureVerificationStarted(session) {
    ...
    session.verifyPromise = session.activeVerifier
      .verify()
      .then(() => {
        this.touchVerificationSession(session);
      })
  ...
  listVerifications() {
    ...
    return Array.from(this.verificationSessions.values())
      .map((session) => this.buildVerificationSummary(session))
