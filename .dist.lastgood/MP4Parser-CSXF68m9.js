import { i as FourCcToken } from "./APEv2Parser-B7oX-NFw.js";
import {
  D as Uint8ArrayType,
  E as UINT8,
  O as lib_exports,
  ...
  g as INT8,
  k as textDecode,
  s as makeUnexpectedFileContentError,
  t as BasicParser,
  u as INT16_BE,
  v as UINT16_BE,
  w as UINT64_BE,
...
import { t as Genres } from "./ID3v1Parser-DFzvMrAH.js";
import { t as require_src } from "./src-Dce2BfHT.js";
import { n as TrackType } from "./types-FndFYD-a.js";
import { a as getBit, d as uint8ArrayToHex } from "./Util-DouHaPQs.js";
...
var Atom = class Atom {
  ...
  getPayloadLength(remaining) {
    return (
      (this.header.length === 0n ? remaining : Number(this.header.length)) - this.getHeaderLength()
    );
  }
...
var MP4Parser = class MP4Parser extends BasicParser {
  ...
  async parse() {
    ...
    while (!this.tokenizer.fileInfo.size || remainingFileSize > 0) {
      ...
      remainingFileSize -=
        rootAtom.header.length === BigInt(0) ? remainingFileSize : Number(rootAtom.header.length);
    }
    const formatList = [];
    ...
    for (const audioTrack of audioTracks) {
        ...
        if (audioTrack.fragments.length > 0) {
          ...
          for (const fragment of audioTrack.fragments)
            for (const sample of fragment.trackRun.samples) {
              ...
              if (dur === 0)
                throw new Error(
                  "Missing sampleDuration and no defaultSampleDuration in track fragment header",
                );
              if (size === 0)
                throw new Error(
                  "Missing sampleSize and no defaultSampleSize in track fragment header",
                );
              totalTimeUnits += dur;
      ...
      if (ssd.description && audioTrack.media.header) {
        ...
        if (audioTrack.media.header.timeScale === 0 && audioTrack.timeToSampleTable.length > 0)
          audioTrack.duration =
            audioTrack.timeToSampleTable
              .map((ttstEntry) => ttstEntry.count * ttstEntry.duration)
              .reduce((total, sampleSize) => total + sampleSize) / ssd.description.sampleRate;
      }
      const encoderInfo = encoderDict[ssd.dataFormat];
      ...
      if (firstAudioTrack.duration) {
        ...
        if (firstAudioTrack.sizeInBytes)
          this.metadata.setFormat(
            "bitrate",
            (8 * firstAudioTrack.sizeInBytes) / firstAudioTrack.duration,
          );
      }
    }
  ...
  async parseTrackBox(trakBox) {
    ...
    await trakBox.readAtoms(
      ...
      async (child, remaining) => {
        ...
        switch (child.header.name) {
          case "chap":
            track.chapterList = await this.tokenizer.readToken(
          ...
          case "hdlr":
            ...
            track.isAudio = () =>
              track.handler.handlerType === "audi" || track.handler.handlerType === "soun";
          ...
          case "stco":
            track.chunkOffsetTable = (
              await this.tokenizer.readToken(new StcoAtom(payLoadLength))
            ).entries;
            break;
          case "stsc":
            track.sampleToChunkTable = (
              await this.tokenizer.readToken(new StscAtom(payLoadLength))
            ).entries;
            break;
          case "stsd":
            track.soundSampleDescription = (
              await this.tokenizer.readToken(new StsdAtom(payLoadLength))
            ).table.map((dfEntry) => this.parseSoundSampleDescription(dfEntry));
            break;
          case "stts":
            track.timeToSampleTable = (
              await this.tokenizer.readToken(new SttsAtom(payLoadLength))
            ).entries;
            break;
          case "stsz": {
      ...
      async (child, remaining) => {
        ...
        switch (child.header.name) {
          ...
          case "trun": {
            ...
            if (tfhd)
              this.tracks.get(tfhd.trackId)?.fragments.push({
                header: tfhd,
                trackRun: trun,
              });
            break;
  ...
  async parseChapterTrack(chapterTrack, track, len) {
    ...
    for (let i = 0; i < chapterTrack.chunkOffsetTable.length && len > 0; ++i) {
      ...
      const sampleSize =
        chapterTrack.sampleSize > 0 ? chapterTrack.sampleSize : chapterTrack.sampleSizeTable[i];
