...
const FAL_SUPPORTED_SIZES = ["1024x1024", "1024x1536", "1536x1024", "1024x1792", "1792x1024"];
const FAL_SUPPORTED_ASPECT_RATIOS = ["1:1", "4:3", "3:4", "16:9", "9:16"];
...
function resolveFalNetworkPolicy(params) {
  ...
  const trustedHostPolicy = mergeSsrFPolicies(
    buildHostnameAllowlistPolicyFromSuffixAllowlist([hostSuffix]),
...
async function fetchImageBuffer(url, networkPolicy) {
  const downloadPolicy = (() => {
    ...
    try {
      return matchesTrustedHostSuffix(new URL(url).hostname, trustedSuffix)
        ? trustedPolicy
        : void 0;
    } catch {
      return;
