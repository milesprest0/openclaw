...
import {
  i as resolveAgentModelTimeoutMsValue,
  n as resolveAgentModelFallbackValues,
  r as resolveAgentModelPrimaryValue,
} from "./model-input-C-vWoAxX.js";
import { f as resolveConfiguredModelRef } from "./model-selection-shared-D9oQINZ2.js";
...
function resolveProviderlessConfiguredImageModelRef(params) {
  ...
  throw new Error(
    `Ambiguous image model "${ref}". Configure a provider-prefixed ref such as ${matches.map((match) => `"${match}"`).join(" or ")}.`,
  );
}
function resolveConfiguredImageModelRefs(params) {
  ...
  return {
    ...(params.imageModelConfig.primary !== void 0
      ? {
          primary: primary
            ? resolveProviderlessConfiguredImageModelRef({
                cfg: params.cfg,
    ...
    ...(params.imageModelConfig.timeoutMs !== void 0
...
function resolveProviderVisionModelFromConfig(params) {
  const id = (
    (
      findNormalizedProviderValue(params.cfg?.models?.providers, params.provider)?.models ?? []
    ).find((m) => Boolean((m?.id ?? "").trim()) && m.input?.includes("image"))?.id ?? ""
  ).trim();
  if (!id) return null;
...
async function minimaxUnderstandImage(params) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `MiniMax VLM request failed (${res.status} ${res.statusText}).${trace}${body ? ` Body: ${body.slice(0, 400)}` : ""}`,
    );
  }
...
//#endregion
export {
  coerceImageModelConfig as a,
  resolveConfiguredImageModelRefs as c,
  coerceToolModelConfig as d,
  hasAuthForProvider as f,
  coerceImageAssistantText as i,
  resolveProviderVisionModelFromConfig as l,
  resolveDefaultModelRef as m,
  isMinimaxVlmProvider as n,
  decodeDataUrl as o,
  hasToolModelConfig as p,
  minimaxUnderstandImage as r,
  hasImageReasoningOnlyResponse as s,
  isMinimaxVlmModel as t,
  buildToolModelConfigFromCandidates as u,
};
