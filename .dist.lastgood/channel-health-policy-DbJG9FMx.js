...
function evaluateChannelHealth(snapshot, policy) {
    ...
    if (!busyStateInitializedForLifecycle) {
      ...
      if (
        (lastRunActivityAt == null
          ...
          : Math.max(0, policy.now - lastRunActivityAt)) < BUSY_ACTIVITY_STALE_THRESHOLD_MS
      )
        return {
          healthy: true,
          reason: "busy",
  ...
  if (snapshot.lastStartAt != null) {
    if (policy.now - snapshot.lastStartAt < policy.channelConnectGraceMs)
      return {
        healthy: true,
  ...
  if (snapshot.connected === true && lastTransportActivityAt != null) {
    if (lastStartAt != null && lastTransportActivityAt < lastStartAt) {
      if (Math.max(0, policy.now - lastStartAt) <= policy.staleEventThresholdMs)
        return {
          healthy: true,
          reason: "healthy",
        };
    ...
    if (policy.now - lastTransportActivityAt > policy.staleEventThresholdMs)
      return {
        healthy: false,
        reason: "stale-socket",
      };
...
//#endregion
export {
  resolveChannelRestartReason as i,
  DEFAULT_CHANNEL_STALE_EVENT_THRESHOLD_MS as n,
  evaluateChannelHealth as r,
  DEFAULT_CHANNEL_CONNECT_GRACE_MS as t,
};
