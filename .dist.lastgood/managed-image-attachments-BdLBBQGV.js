...
import { C as safeFileURLToPath } from "./fs-safe-CgBWiL92.js";
import {
  l as resolveOpenAiCompatibleHttpOperatorScopes,
  t as authorizeGatewayHttpRequestOrReply,
  u as resolveOpenAiCompatibleHttpSenderIsOwner,
} from "./http-auth-utils-QDjCKoNp.js";
import { a as sendMethodNotAllowed, i as sendJson } from "./http-common-CyzMuYsM.js";
import {
  a as getImageMetadata,
  l as resizeToJpeg,
  o as hasAlphaChannel,
  u as resizeToPng,
...
import { n as assertLocalMediaAllowed } from "./local-media-access-gxEsNSj9.js";
import { t as isPassThroughRemoteMediaSource } from "./media-source-url-nm3hHe4P.js";
...
function computeManagedImageResizeTarget(metadata, limits) {
  ...
  while (width > limits.maxWidth || height > limits.maxHeight || width * height > limits.maxPixels)
...
async function writeManagedImageRecord(record, stateDir = resolveStateDir()) {
  await writeJson(resolveOutgoingRecordPath(record.attachmentId, stateDir), record, {
...
function parseManagedOutgoingRoute(value) {
  try {
    const match = new URL(value, "http://localhost").pathname.match(
      /^\/api\/chat\/media\/outgoing\/([^/]+)\/([^/]+)\/full$/,
    );
...
async function recordMatchesTranscriptMessage(record, cache) {
  ...
  return (
    (await getSessionManagedOutgoingAttachmentIndex(record.sessionKey, cache))?.has(
...
//#endregion
export {
  handleManagedOutgoingImageHttpRequest as a,
  createManagedOutgoingImageBlocks as i,
  attachManagedOutgoingImagesToMessage as n,
  resolveManagedImageAttachmentLimits as o,
  cleanupManagedOutgoingImageRecords as r,
  DEFAULT_MANAGED_IMAGE_ATTACHMENT_LIMITS as t,
};
