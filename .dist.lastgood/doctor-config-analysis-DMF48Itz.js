...
function noteOpencodeProviderOverrides(cfg) {
  ...
  const lines = overrides.flatMap((id) => {
    ...
    const api =
      isRecord(providerEntry) && typeof providerEntry.api === "string" ? providerEntry.api : void 0;
    return [
...
function noteIncludeConfinementWarning(snapshot) {
  const includeIssue = (snapshot.issues ?? []).find(
    (issue) =>
...
//#endregion
export {
  stripUnknownConfigKeys as a,
  resolveConfigPathTarget as i,
  noteIncludeConfinementWarning as n,
  noteOpencodeProviderOverrides as r,
  formatConfigPath as t,
};
