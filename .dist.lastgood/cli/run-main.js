...
import { h as getSubCliEntries, v as getCoreCliCommandNames } from "../argv-DQxa_OA3.js";
import { t as resolveCliArgvInvocation } from "../argv-invocation-Bc25Z788.js";
import {
  a as consumeGatewayFastPathRootOptionToken,
  n as resolveCliNetworkProxyPolicy,
  o as consumeGatewayRunOptionToken,
  t as resolveCliCommandPathPolicy,
} from "../command-path-policy-CFCJblDB.js";
import {
  a as shouldSkipPluginCommandRegistration,
  r as shouldRegisterPrimaryCommandOnly,
  t as isReservedNonPluginCommandRoot,
} from "../command-registration-policy-DxSkwxV7.js";
import { i as normalizeEnv, t as isTruthyEnvValue } from "../env-GyTZ5xFK.js";
...
import {
  i as parseCliContainerArgs,
  n as parseCliProfileArgs,
  r as maybeRunCliInContainer,
  t as applyCliProfileEnv,
} from "../profile-mG6GlPPm.js";
import { t as assertSupportedRuntime } from "../runtime-guard-vJnP437q.js";
import {
...
import { t as normalizeWindowsArgv } from "../windows-argv-gsojWZI9.js";
...
function resolveMissingPluginCommandMessage$1(pluginId, config, options) {
  ...
  if (parentPluginId) {
    ...
    if (config?.plugins?.entries?.[parentPluginId]?.enabled === false)
      return `The \`openclaw ${normalizedPluginId}\` command is unavailable because \`plugins.entries.${parentPluginId}.enabled=false\`. Re-enable that entry if you want the bundled plugin command surface.`;
    if (
      commandAlias.kind !== "runtime-slash" &&
      commandAlias.enabledByDefault !== true &&
      config?.plugins?.entries?.[parentPluginId]?.enabled !== true
    )
      return `The \`openclaw ${normalizedPluginId}\` command is provided by the "${parentPluginId}" plugin, but that bundled plugin is disabled by default. Run \`openclaw plugins enable ${parentPluginId}\` to enable that CLI surface.`;
    if (commandAlias.kind === "runtime-slash")
      return `"${normalizedPluginId}" is a runtime slash command (/${normalizedPluginId}), not a CLI command. It is provided by the "${parentPluginId}" plugin. ${commandAlias.cliCommand ? `Use \`openclaw ${commandAlias.cliCommand}\` for related CLI operations, or ` : "Use "}\`/${normalizedPluginId}\` in a chat session.`;
  }
  ...
  if (config?.plugins?.entries?.[normalizedPluginId]?.enabled === false)
    return `The \`openclaw ${normalizedPluginId}\` command is unavailable because \`plugins.entries.${normalizedPluginId}.enabled=false\`. Re-enable that entry if you want the bundled plugin CLI surface.`;
  return null;
}
//#endregion
...
function createGatewayCliMainStartupTrace(argv) {
  const enabled =
    isTruthyEnvValue(process$1.env.OPENCLAW_GATEWAY_STARTUP_TRACE) &&
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
  ...
  const [
    ...
    Promise.all([
      import("commander"),
      import("../run-BLcnu-gU.js"),
      import("../version-CPHvR5h9.js"),
      import("../banner-CW1D3pPK.js"),
      import("../command-startup-policy-BUPGJHRn.js"),
    ]),
  );
  if (
    !resolveCliStartupPolicy({
...
async function closeCliMemoryManagers() {
  try {
    ...
    const { closeActiveMemorySearchManagers } = await import("../memory-runtime-BE6HW5N2.js");
...
async function disposeCliAgentHarnesses() {
  try {
    const { listAgentHarnessIds, disposeRegisteredAgentHarnesses } =
      await import("../registry-B_ynF5__.js");
...
function resolveMissingPluginCommandMessage(pluginId, config, options) {
  return resolveMissingPluginCommandMessage$1(
    pluginId,
    config,
...
async function ensureCliEnvProxyDispatcher() {
  try {
    const { hasEnvHttpProxyAgentConfigured } = await import("../proxy-env-C6nySVJ4.js");
    if (!hasEnvHttpProxyAgentConfigured()) return;
    const { ensureGlobalUndiciEnvProxyDispatcher } =
      await import("../undici-global-dispatcher-BR6C_HGa.js");
...
async function isPluginCliRoot(params) {
  try {
    const { resolvePluginCliRootOwnerIds } = await import("../cli-registry-loader-DEYxZboV.js");
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
  const [
    ...
    Promise.all([import("../runtime-BbneBknh.js"), import("../coverage-BGhtZZm9.js")]),
  );
...
async function runCli(argv = process$1.argv) {
  ...
  if (
    (parsedContainer.container ??
      normalizeOptionalString(process$1.env.OPENCLAW_CONTAINER) ??
      null) &&
  ...
  if (shouldLoadCliDotEnv())
    await startupTrace.measure("dotenv", async () => {
      const { loadCliDotEnv } = await import("../dotenv-MsKJNlm5.js");
  ...
  const readBestEffortCliConfig = async () => {
    if (!bestEffortConfigPromise)
      bestEffortConfigPromise = import("../io-CHfe4hv8.js").then(({ readBestEffortConfig }) =>
  ...
  const stopStartedProxy = async () => {
    ...
    proxyHandle = null;
    if (handle) {
      const { stopProxy } = await import("../proxy-lifecycle-DfJMrpEk.js");
  ...
  if (shouldStartProxyForCli(normalizedArgv)) {
    ...
    const { startProxy } = await import("../proxy-lifecycle-DfJMrpEk.js");
  ...
  try {
    if (shouldUseRootHelpFastPath(normalizedArgv)) {
      const { outputPrecomputedRootHelpText } = await import("../root-help-metadata-DHiEWDay.js");
      if (!outputPrecomputedRootHelpText()) {
        const { outputRootHelp } = await import("../root-help-D4TTqli8.js");
    ...
    if (shouldUseBrowserHelpFastPath(normalizedArgv)) {
      const { outputPrecomputedBrowserHelpText } =
        await import("../root-help-metadata-DHiEWDay.js");
    ...
    if (shouldRunBareRootCrestodian) {
      ...
      const { createCliProgress } = await import("../progress-ghRJ4KUf.js");
    ...
    try {
      const { enableConsoleCapture } = await import("../logging-ByyCV6Zg.js");
      ...
      const [
        ...
        Promise.all([
          import("../program-SfP4eCiN.js"),
          import("../infra/errors.js"),
          import("../fatal-error-hooks-CasAm3tu.js"),
          import("../unhandled-rejections-ClnPgowZ.js"),
          import("../restore-BsToLA9W.js"),
        ]),
      );
      ...
      if (primary && shouldRegisterPrimaryCommandOnly(parseArgv))
        await startupTrace.measure("register-primary", async () => {
          const { getProgramContext } = await import("../program-context-BpVY-fBN.js");
          const ctx = getProgramContext(program);
          if (ctx) {
            const { registerCoreCliByName } = await import("../command-registry-COjN_tcz.js");
            await registerCoreCliByName(program, ctx, primary, parseArgv);
          }
          const { registerSubCliByName } = await import("../register.subclis-Cu24DPFm.js");
        ...
        if (config) {
          if (
            ...
            const { resolveManifestCommandAliasOwner } =
              await import("../manifest-command-aliases.runtime-DZSzdUkb.js");
            const missingPluginCommandMessage = resolveMissingPluginCommandMessage$1(
              primary,
              config,
