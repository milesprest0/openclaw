...
function createOpenClawModelRegistry(authStorage, modelsJsonPath, agentDir, options) {
  ...
  const matchesProviderFilter = (entry) =>
  ...
  registry.getAll = () => {
    ...
    return shouldNormalize
      ? entries.map((entry) => normalizeDiscoveredPiModel(entry, agentDir))
      : entries;
  };
...
function createAuthStorage(AuthStorageLike, path, creds) {
  ...
  const withRuntimeOverride =
    typeof withFactory.create === "function" ? withFactory.create(path) : new AuthStorageLike(path);
...
//#endregion
export {
  normalizeDiscoveredPiModel as a,
  scrubLegacyStaticAuthJsonEntriesForDiscovery as c,
  discoverModels as i,
  PiModelRegistryClass as n,
  resolvePiCredentialsForDiscovery as o,
  discoverAuthStorage as r,
  addEnvBackedPiCredentials as s,
  PiAuthStorageClass as t,
};
