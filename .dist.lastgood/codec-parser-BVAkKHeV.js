...
for (let i = 0; i < 15; i++) {
  ...
  for (let j = 0; j <= 255; j++) {
    flacCrc16Table[i + 1][j] =
      flacCrc16Table[0][flacCrc16Table[i][j] >>> 8] ^ (flacCrc16Table[i][j] << 8);
...
const crc32Function = (data) => {
  ...
  while (i <= crcChunkSize)
    crc =
      crc32Table[15][(data[i++] ^ crc) & 255] ^
      crc32Table[14][(data[i++] ^ (crc >>> 8)) & 255] ^
      crc32Table[13][(data[i++] ^ (crc >>> 16)) & 255] ^
...
var CodecFrame = class extends Frame {
  static *[getFrame](Header, Frame, codecParser, headerCache, readOffset) {
    ...
    if (headerValue) {
      ...
      return new Frame(
        headerValue,
        (yield* codecParser[readRawData](frameLengthValue, readOffset))[subarray](
          0,
          frameLengthValue,
        ),
        samplesValue,
...
var MPEGHeader = class MPEGHeader extends CodecHeader {
  static *[getHeader](codecParser, headerCache, readOffset) {
    ...
    header[frameLength] = Math.floor(
      (125 * header[bitrate] * header[samples]) / header[sampleRate] + header[framePadding],
    );
...
var AACHeader = class AACHeader extends CodecHeader {
  static *[getHeader](codecParser, headerCache, readOffset) {
    ...
    const key = bytesToString([data[0], data[1], data[2], (data[3] & 252) | (data[6] & 3)]);
    const cachedHeader = headerCache[getHeader](key);
    if (!cachedHeader) {
    ...
    do {
      ...
      if (header) {
        ...
        while (nextHeaderOffset <= MAX_FLAC_FRAME_SIZE) {
          if (
            this._codecParser._flushing ||
            (yield* FLACHeader[getHeader](this._codecParser, this._headerCache, nextHeaderOffset))
          ) {
...
var OggPage = class OggPage extends Frame {
  static *[getFrame](codecParser, headerCache, readOffset) {
    ...
    if (header) {
      ...
      const rawDataValue = (yield* codecParser[readRawData](totalLength, 0))[subarray](
        0,
        totalLength,
      );
...
var OpusHeader = class OpusHeader extends CodecHeader {
  static [getHeaderFromUint8Array](dataValue, packetData, headerCache) {
    ...
    if (header["channelMappingFamily"] in channelMappingFamilies) {
      header[channelMode] =
        channelMappingFamilies[header[channelMappingFamily]][header[channels$1] - 1];
...
var VorbisParser = class extends Parser {
      ...
      if (oggPageSegment[0] === 1) {
        ...
        if (header)
          oggPage[codecFrames$1].push(
            new VorbisFrame(oggPageSegment, header, this._getSamples(oggPageSegment, header)),
          );
        else
  ...
  _getSamples(segment, header) {
    const currentBlockSize = this._mode.blockFlags[(segment[0] >> 1) & this._mode.mask]
      ? header[blocksize1]
      : header[blocksize0];
    const samplesValue =
      this._prevBlockSize === null ? 0 : (this._prevBlockSize + currentBlockSize) / 4;
  ...
  _parseSetupHeader(setup) {
    ...
    while (mode.count < 64 && bitReader.position > 0) {
      ...
      if (currentByte === 4) {
        ...
        if (((reverse(modeBits) & 126) >> 1) + 1 !== mode.count)
          this._codecParser[logWarning](
  ...
  _parsePage(oggPage) {
    ...
    if (this._previousAbsoluteGranulePosition !== null)
      oggPage[samples] = Number(
        oggPage[absoluteGranulePosition$1] - this._previousAbsoluteGranulePosition,
      );
...
var CodecParser = class {
  constructor(
    mimeType,
    { onCodec, onCodecHeader, onCodecUpdate, enableLogging = false, enableFrameCRC32 = true } = {},
  ) {
  ...
  *[readRawData](minSize = 0, readOffset = 0) {
  ...
  [mapCodecFrameStats](frame) {
    ...
    frame[header$1][bitrate] =
      frame["duration"] > 0 ? Math.round(frame[data$1][length] / frame[duration]) * 8 : 0;
      ...
      if (frame["isLastPage"]) {
        ...
        frame[codecFrames$1].forEach((codecFrame) => {
          ...
          if (absoluteGranulePositionSamples < untrimmedCodecSamples) {
            codecFrame[samples] =
              absoluteGranulePositionSamples > 0 ? absoluteGranulePositionSamples : 0;
...
//#endregion
export {
  codec_parser_default as a,
  header as c,
  streamCount as d,
  totalSamples as f,
  codecFrames as i,
  isLastPage as l,
  channelMappingTable as n,
  coupledStreamCount as o,
  vorbisSetup as p,
  channels as r,
  data as s,
  absoluteGranulePosition as t,
  preSkip as u,
};
