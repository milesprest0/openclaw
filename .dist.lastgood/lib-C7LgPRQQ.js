...
import {
  i as require_coerce,
  s as require_gte,
  t as require_satisfies,
} from "./satisfies-6eAKDsP0.js";
  ...
  function _createInputDescriptor(input, inputOptions, containerOptions) {
    ...
    if (is.object(inputOptions)) {
      ...
      if (is.defined(inputOptions.autoOrient))
        if (is.bool(inputOptions.autoOrient)) inputDescriptor.autoOrient = inputOptions.autoOrient;
      ...
      if (is.defined(inputOptions.ignoreIcc))
        if (is.bool(inputOptions.ignoreIcc)) inputDescriptor.ignoreIcc = inputOptions.ignoreIcc;
      ...
      if (is.defined(inputOptions.unlimited))
        if (is.bool(inputOptions.unlimited)) inputDescriptor.unlimited = inputOptions.unlimited;
      ...
      if (is.defined(inputOptions.animated))
        if (is.bool(inputOptions.animated)) inputDescriptor.pages = inputOptions.animated ? -1 : 1;
          ...
          if (is.defined(inputOptions.create.noise)) {
            ...
            if (is.defined(inputOptions.create.noise.mean))
              if (
                ...
                is.inRange(inputOptions.create.noise.mean, 0, 1e4)
              )
            ...
            if (is.defined(inputOptions.create.noise.sigma))
              if (
                ...
                is.inRange(inputOptions.create.noise.sigma, 0, 1e4)
              )
      ...
      if (is.defined(inputOptions.text))
        if (is.object(inputOptions.text) && is.string(inputOptions.text.text)) {
          ...
          if (is.defined(inputOptions.text.rgba))
            if (is.bool(inputOptions.text.rgba)) inputDescriptor.textRgba = inputOptions.text.rgba;
            else throw is.invalidParameterError("text.rgba", "bool", inputOptions.text.rgba);
          if (is.defined(inputOptions.text.spacing))
            if (
              is.integer(inputOptions.text.spacing) &&
              is.inRange(inputOptions.text.spacing, -1e6, 1e6)
            )
        ...
        if (is.defined(this.options.join)) {
          ...
          if (is.defined(inputOptions.join.across))
            if (
              ...
              is.inRange(inputOptions.join.across, 1, 1e6)
            )
  ...
  function resize(widthOrOptions, height, options) {
    ...
    if (is.object(options)) {
      ...
      if (is.defined(options.kernel))
        if (is.string(kernel[options.kernel])) this.options.kernel = kernel[options.kernel];
  ...
  function extend(extend) {
    if (is.integer(extend) && extend > 0) {
      ...
      if (is.defined(extend.top))
        if (is.integer(extend.top) && extend.top >= 0) this.options.extendTop = extend.top;
      ...
      if (is.defined(extend.left))
        if (is.integer(extend.left) && extend.left >= 0) this.options.extendLeft = extend.left;
        else throw is.invalidParameterError("left", "positive integer", extend.left);
      if (is.defined(extend.right))
        if (is.integer(extend.right) && extend.right >= 0) this.options.extendRight = extend.right;
  ...
  function composite(images) {
    ...
    this.options.composite = images.map((image) => {
      ...
      if (is.defined(image.blend))
        if (is.string(blend[image.blend])) composite.blend = blend[image.blend];
        else throw is.invalidParameterError("blend", "valid blend name", image.blend);
      if (is.defined(image.tile))
        if (is.bool(image.tile)) composite.tile = image.tile;
        else throw is.invalidParameterError("tile", "boolean", image.tile);
      if (is.defined(image.left))
        if (is.integer(image.left)) composite.left = image.left;
        else throw is.invalidParameterError("left", "integer", image.left);
      if (is.defined(image.top))
        if (is.integer(image.top)) composite.top = image.top;
      ...
      if (is.defined(image.premultiplied))
        if (is.bool(image.premultiplied)) composite.premultiplied = image.premultiplied;
...
var require_operation = /* @__PURE__ */ __commonJSMin((exports, module) => {
    ...
    if (is.defined(options))
      if (is.object(options)) {
        ...
        if (is.defined(options.idx))
          if (is.number(options.idx)) this.options.affineIdx = options.idx;
          else throw is.invalidParameterError("options.idx", "number", options.idx);
        if (is.defined(options.idy))
          if (is.number(options.idy)) this.options.affineIdy = options.idy;
          else throw is.invalidParameterError("options.idy", "number", options.idy);
        if (is.defined(options.odx))
          if (is.number(options.odx)) this.options.affineOdx = options.odx;
          else throw is.invalidParameterError("options.odx", "number", options.odx);
        if (is.defined(options.ody))
          if (is.number(options.ody)) this.options.affineOdy = options.ody;
  ...
  function sharpen(options, flat, jagged) {
    if (!is.defined(options)) this.options.sharpenSigma = -1;
      ...
      if (is.defined(flat))
        if (is.number(flat) && is.inRange(flat, 0, 1e4)) this.options.sharpenM1 = flat;
        else throw is.invalidParameterError("flat", "number between 0 and 10000", flat);
      if (is.defined(jagged))
        if (is.number(jagged) && is.inRange(jagged, 0, 1e4)) this.options.sharpenM2 = jagged;
  ...
  function blur(options) {
    ...
    else if (is.number(sigma) && is.inRange(sigma, 0.3, 1e3)) this.options.blurSigma = sigma;
  ...
  function modulate(options) {
    ...
    if ("hue" in options)
      if (is.integer(options.hue)) this.options.hue = options.hue % 360;
      else throw is.invalidParameterError("hue", "number", options.hue);
    if ("lightness" in options)
      if (is.number(options.lightness)) this.options.lightness = options.lightness;
...
var require_channel = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  function ensureAlpha(alpha) {
    if (is.defined(alpha))
      if (is.number(alpha) && is.inRange(alpha, 0, 1)) this.options.ensureAlpha = alpha;
...
var require_output = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  const bitdepthFromColourCount = (colours) =>
  ...
  function toFile(fileOut, callback) {
    ...
    if (!is.string(fileOut)) err = /* @__PURE__ */ new Error("Missing output file path");
      ...
      err = errJp2Save();
    if (err)
      if (is.fn(callback)) callback(err);
      ...
      for (const [ifd, entries] of Object.entries(exif))
        if (is.object(entries))
          for (const [k, v] of Object.entries(entries))
            if (is.string(v)) this.options.withExif[`exif-${ifd.toLowerCase()}-${k}`] = v;
    ...
    if (is.object(options)) {
      ...
      if (is.defined(options.preset))
        if (
          ...
          is.inArray(options.preset, ["default", "photo", "picture", "drawing", "icon", "text"])
        )
  ...
  function jp2(options) {
    ...
    if (is.object(options)) {
      ...
      if (is.defined(options.lossless))
        if (is.bool(options.lossless)) this.options.jp2Lossless = options.lossless;
  ...
  function trySetAnimationOptions(source, target) {
    if (is.object(source) && is.defined(source.loop))
      if (is.integer(source.loop) && is.inRange(source.loop, 0, 65535)) target.loop = source.loop;
  ...
  function tiff(options) {
    if (is.object(options)) {
      ...
      if (is.defined(options.xres))
        if (is.number(options.xres) && options.xres > 0) this.options.tiffXres = options.xres;
        else throw is.invalidParameterError("xres", "number greater than zero", options.xres);
      if (is.defined(options.yres))
        if (is.number(options.yres) && options.yres > 0) this.options.tiffYres = options.yres;
    ...
    if (is.object(options)) {
      ...
      if (is.defined(options.quality))
        if (is.integer(options.quality) && is.inRange(options.quality, 1, 100))
          ...
          throw is.invalidParameterError("quality", "integer between 1 and 100", options.quality);
      if (is.defined(options.lossless))
        if (is.bool(options.lossless)) this.options.heifLossless = options.lossless;
        else throw is.invalidParameterError("lossless", "boolean", options.lossless);
      if (is.defined(options.effort))
  ...
  function jxl(options) {
    if (is.object(options)) {
      ...
      if (is.defined(options.lossless))
        if (is.bool(options.lossless)) this.options.jxlLossless = options.lossless;
        else throw is.invalidParameterError("lossless", "boolean", options.lossless);
      if (is.defined(options.effort))
  ...
  function tile(options) {
    if (is.object(options)) {
      ...
      if (is.defined(options.id))
        if (is.string(options.id)) this.options.tileId = options.id;
        else throw is.invalidParameterError("id", "string", options.id);
      if (is.defined(options.basename))
        if (is.string(options.basename)) this.options.tileBasename = options.basename;
...
var require_utility = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  function cache(options) {
    if (is.bool(options))
      if (options) return sharp.cache(50, 20, 100);
