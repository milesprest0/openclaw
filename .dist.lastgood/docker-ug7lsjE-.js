...
import {
  S as SANDBOX_REGISTRY_PATH,
  _ as SANDBOX_AGENT_WORKSPACE_MOUNT,
  v as SANDBOX_BROWSERS_DIR,
  x as SANDBOX_CONTAINERS_DIR,
  y as SANDBOX_BROWSER_REGISTRY_PATH,
} from "./constants-DcbmEUTo.js";
import { d as writeJson } from "./json-files-DifBk3kt.js";
import { n as markOpenClawExecEnv } from "./openclaw-exec-env-C8wgFL16.js";
...
import { r as acquireSessionWriteLock } from "./session-write-lock-DjdS_616.js";
...
import { r as validateSandboxSecurity } from "./validate-sandbox-security-B91sDgd8.js";
import {
...
async function readRegistry() {
  return {
    entries: (await readShardedEntries(SANDBOX_CONTAINERS_DIR)).map((entry) =>
  ...
  return await withRegistryLock(registryPath, async () => {
    ...
    for (const entry of registry.entries)
      await withEntryLock(shardedDir, entry.containerName, async () => {
        if (!(await readShardedEntry(shardedDir, entry.containerName)))
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (exitCode !== 0 && !opts?.allowFailure) {
        ...
        reject(
          Object.assign(new Error(message || `docker ${args.join(" ")} failed`), {
            code: exitCode,
            stdout,
            stderr,
          }),
    ...
    if (stdin)
      if (opts?.input !== void 0) stdin.end(opts.input);
      else stdin.end();
  });
}
...
async function readDockerPort(containerName, port) {
  const result = await execDocker(["port", containerName, `${port}/tcp`], { allowFailure: true });
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params) {
  ...
  if (params.scope === "agent")
    return formatCliCommand(
      `openclaw sandbox recreate --agent ${resolveSandboxAgentId(params.sessionKey) ?? "main"}`,
    );
...
export {
  readRegistry as C,
  updateRegistry as D,
  updateBrowserRegistry as E,
  computeSandboxBrowserConfigHash as O,
  readBrowserRegistry as S,
  removeRegistryEntry as T,
  resolveSandboxScopeKey as _,
  execDocker as a,
  inspectLegacySandboxRegistryFiles as b,
  isDockerDaemonUnavailable as c,
  readDockerNetworkDriver as d,
  readDockerNetworkGateway as f,
  resolveSandboxAgentId as g,
  appendWorkspaceMountArgs as h,
  ensureSandboxContainer as i,
  readDockerContainerEnvVar as l,
  resolveDockerSpawnInvocation as m,
  dockerContainerState as n,
  execDockerRaw as o,
  readDockerPort as p,
  ensureDockerImage as r,
  formatDockerDaemonUnavailableError as s,
  buildSandboxCreateArgs as t,
  readDockerContainerLabel as u,
  resolveSandboxWorkspaceDir as v,
  removeBrowserRegistryEntry as w,
  migrateLegacySandboxRegistryFiles as x,
  slugifySessionKey as y,
};
