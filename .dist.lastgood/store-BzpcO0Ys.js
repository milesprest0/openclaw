...
function expireStaleCommitmentsInStore(store, nowMs) {
  ...
  store.commitments = store.commitments.map((commitment) => {
    if (!isActiveStatus(commitment.status) || commitment.dueWindow.latestMs + staleAfterMs >= nowMs)
      return commitment;
    changed = true;
    return {
      ...commitment,
...
async function listCommitments(params) {
  return (await loadCommitmentStoreWithExpiredMarked(Date.now())).commitments
    .filter(
      (commitment) =>
...
//#endregion
export {
  loadCommitmentStore as a,
  resolveCommitmentStorePath as c,
  resolveCommitmentTimezone as d,
  resolveCommitmentsConfig as f,
  listPendingCommitmentsForScope as i,
  saveCommitmentStore as l,
  listDueCommitmentSessionKeys as n,
  markCommitmentsAttempted as o,
  listDueCommitmentsForSession as r,
  markCommitmentsStatus as s,
  listCommitments as t,
  upsertInferredCommitments as u,
};
