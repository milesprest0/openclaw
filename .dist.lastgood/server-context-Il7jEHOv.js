...
import {
  C as PROFILE_ATTACH_RETRY_TIMEOUT_MS,
  T as usesFastLoopbackCdpProbeClass,
  a as fetchOk,
  b as CHROME_MCP_ATTACH_READY_WINDOW_MS,
  f as redactCdpUrl,
  i as fetchJson,
  j as DEFAULT_BROWSER_LOCAL_CDP_READY_TIMEOUT_MS,
  l as normalizeCdpHttpBaseForJsonEndpoints,
  m as withAllowedHostname,
  n as assertCdpEndpointAllowed,
  t as appendCdpPath,
  w as resolveCdpReachabilityTimeouts,
} from "./cdp.helpers-DnNpBUOj.js";
import {
  E as withBrowserNavigationPolicy,
  S as assertBrowserNavigationAllowed,
  T as requiresInspectableBrowserNavigationRedirectsForUrl,
  a as resolveOpenClawUserDataDir,
  b as resolveBrowserNavigationProxyMode,
  c as formatChromeCdpDiagnostic,
  d as createTargetViaCdp,
  i as launchOpenClawChrome,
  n as isChromeCdpReady,
  o as stopOpenClawChrome,
  p as normalizeCdpWsUrl,
  r as isChromeReachable,
  s as diagnoseChromeCdp,
  w as assertBrowserNavigationResultAllowed,
  x as InvalidBrowserNavigationUrlError,
} from "./chrome-B-sV0ork.js";
import { g as listChromeMcpTabs } from "./chrome-mcp-M0WGdDqO.js";
import "./text-runtime-lKuAtsoz.js";
import { i as resolveProfile, n as resolveBrowserConfig } from "./config-DhdywqBF.js";
import {
  a as BrowserProfileUnavailableError,
  c as BrowserTabNotFoundError,
  d as toBrowserErrorResponse,
  i as BrowserProfileNotFoundError,
  l as BrowserTargetAmbiguousError,
  o as BrowserResetUnsupportedError,
} from "./errors-BxBS-H8t.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import "./config-CE62Wyxe.js";
import { s as getRuntimeConfigSourceSnapshot } from "./runtime-snapshot-B0aZX-OG.js";
import { d as isPrivateNetworkAllowedByPolicy } from "./ssrf-DO8eIXaD.js";
import { c as normalizeOptionalString } from "./string-coerce-BdEutqX5.js";
import {
  n as getPwAiModule,
  r as getBrowserProfileCapabilities,
  t as resolveTargetIdFromTabs,
} from "./target-id-Bq4VUN4D.js";
...
async function getChromeMcpModule() {
  return await import("./chrome-mcp-B4Glo31r.js");
}
//#endregion
...
async function closePlaywrightBrowserConnectionForProfile(cdpUrl) {
  try {
    await (
      await getPwAiModule({ mode: "soft" })
    )?.closePlaywrightBrowserConnection(cdpUrl ? { cdpUrl } : void 0);
  } catch {}
}
//#endregion
...
function assertManagedLaunchNotCoolingDown(profileName, profileState) {
  ...
  throw new BrowserProfileUnavailableError(
    `Browser launch for profile "${profileName}" is cooling down after ${failure.consecutiveFailures} consecutive managed Chrome launch failures. Retry in ${retrySeconds}s after fixing Chrome startup, or set browser.enabled=false if the browser tool is not needed. Last error: ${failure.lastError}`,
  );
}
function createProfileAvailability({ opts, profile, state, getProfileState, setProfileRunning }) {
  ...
  const capabilities = getBrowserProfileCapabilities(profile);
  const resolveTimeouts = (timeoutMs) =>
  ...
  const describeCdpFailure = async (timeoutMs) => {
    const { httpTimeoutMs, wsTimeoutMs } = resolveTimeouts(timeoutMs);
    return formatChromeCdpDiagnostic(
      await diagnoseChromeCdp(
        profile.cdpUrl,
        httpTimeoutMs,
        wsTimeoutMs,
        getCdpReachabilityPolicy(),
  ...
  const formatChromeMcpAttachFailure = (lastError) => {
    ...
    if (message.includes("DevToolsActivePort") || message.includes("Could not connect to Chrome"))
      return (
        `Chrome MCP existing-session attach for profile "${profile.name}" could not connect to Chrome. Enable remote debugging in the browser inspect page, keep the browser open, approve the attach prompt, and retry. If you do not need your signed-in browser session, use the managed "openclaw" profile instead.` +
        detail
      );
  ...
  const waitForCdpReadyAfterLaunch = async () => {
    ...
    throw new Error(
      `Chrome CDP websocket for profile "${profile.name}" is not reachable after start. ${await describeCdpFailure(250)}`,
    );
  };
  ...
  const ensureBrowserAvailableOnce = async (options) => {
    ...
    if (!profileState.running) {
      const detail = await describeCdpFailure(PROFILE_ATTACH_RETRY_TIMEOUT_MS);
      throw new BrowserProfileUnavailableError(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by openclaw. ${formatLocalPortOwnershipHint(profile)} ${detail}`,
      );
    }
    ...
    if (!(await isReachable(600))) {
      const err = /* @__PURE__ */ new Error(
        `Chrome CDP websocket for profile "${profile.name}" is not reachable after restart. ${await describeCdpFailure(600)}`,
      );
      recordManagedLaunchFailure(profileState, err);
      throw err;
    }
...
function createProfileResetOps({
  profile,
  getProfileState,
  stopRunningBrowser,
  isHttpReachable,
  ...
  const resetProfile = async () => {
    ...
    profileState.managedLaunchFailure = void 0;
    profileState.ensureBrowserAvailable = null;
    if ((await isHttpReachable(300)) && !profileState.running)
      await closePlaywrightBrowserConnectionForProfile(profile.cdpUrl);
...
function createProfileSelectionOps({
  profile,
  getProfileState,
  getCdpControlPolicy,
  ensureBrowserAvailable,
  ...
  const focusTab = async (targetId) => {
    ...
    if (capabilities.usesPersistentPlaywright) {
      const focusPageByTargetIdViaPlaywright = (await getPwAiModule({ mode: "strict" }))
  ...
  const closeTab = async (targetId) => {
    ...
    if (capabilities.usesPersistentPlaywright) {
      const closePageByTargetIdViaPlaywright = (await getPwAiModule({ mode: "strict" }))
...
function assignTabAlias(params) {
  ...
  if (params.label) {
    ...
    for (const [targetId, current] of Object.entries(aliases.byTargetId))
      if (targetId !== params.tab.targetId && current.label === label) delete current.label;
    entry.label = label;
  }
...
function assignTabAliases(profileState, tabs) {
  ...
  for (const targetId of Object.keys(aliases.byTargetId))
    if (!liveTargetIds.has(targetId)) delete aliases.byTargetId[targetId];
  return tabs.map((tab) =>
    assignTabAlias({
      profileState,
      tab,
...
function createProfileTabOps({ profile, state, getProfileState }) {
  ...
  const readTabs = async () => {
    ...
    if (capabilities.usesPersistentPlaywright) {
      const listPagesViaPlaywright = (await getPwAiModule({ mode: "strict" }))
  ...
  const openTab = async (url, opts) => {
    ...
    if (capabilities.usesPersistentPlaywright) {
      const createPageViaPlaywright = (await getPwAiModule({ mode: "strict" }))
    ...
    const created = await fetchJson(
      endpoint,
      cdpActionTimeouts?.httpTimeoutMs ?? 1500,
      { method: "PUT" },
      getCdpControlPolicy(),
    ).catch(async (err) => {
  ...
  const forProfile = (profileName) => {
    ...
    if (!profile)
      throw new BrowserProfileNotFoundError(
        `Profile "${name}" not found. Available profiles: ${Object.keys(current.resolved.profiles).join(", ") || "(none)"}`,
      );
    ...
    for (const name of listKnownProfileNames(current)) {
      ...
      if (capabilities.usesChromeMcp)
        try {
          ...
          if (running)
            tabCount = (
              await listChromeMcpTabs(profile.name, profile, { ephemeral: true }).catch(() => [])
        ...
        try {
          ...
          if (
            ...
            running = true;
            tabCount = (await profileCtx.listTabs().catch(() => [])).filter(
              (t) => t.type === "page",
            ).length;
          }
        } catch {}
...
//#endregion
export {
  listKnownProfileNames as n,
  loadBrowserConfigForRuntimeRefresh as r,
  createBrowserRouteContext as t,
};
