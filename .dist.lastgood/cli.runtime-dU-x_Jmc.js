...
import { r as withManager } from "./cli-utils-Ce4lxCq8.js";
import { t as resolveCommandSecretRefsViaGateway } from "./command-secret-gateway-CNlzw0VK.js";
import { X as resolveMemoryRemDreamingConfig } from "./dreaming-BDr8qIGr.js";
import { n as resolveShortTermPromotionDreamingConfig } from "./dreaming-C4YEOnNj.js";
import {
  i as seedHistoricalDailyMemorySignals,
  l as writeBackfillDiaryEntries,
  s as removeBackfillDiaryEntries,
} from "./dreaming-phases-D2W5WvGC.js";
import {
  n as repairDreamingArtifacts,
  t as auditDreamingArtifacts,
} from "./dreaming-repair-ekTuQXFS.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import "./temp-path-DRiyUI5S.js";
import { c as normalizeExtraMemoryPaths, s as listMemoryFiles } from "./internal-BUtLJk16.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import "./routing-BccEMOrJ.js";
import { C as setVerbose } from "./logger-BU4ooQvN.js";
import { n as getMemorySearchManager } from "./memory-C87SiNGh.js";
import { l as resolveSessionTranscriptsDirForAgent } from "./paths-CfeECf6Z.js";
import "./memory-core-host-status-CodVEHuG.js";
import "./memory-core-host-runtime-cli-DVR6IrLX.js";
...
import { i as withProgressTotals, r as withProgress } from "./progress-DRZHxXMU.js";
import { n as asNullableRecord } from "./record-coerce-BuYFJfzm.js";
import { n as previewGroundedRemMarkdown, t as previewRemHarness } from "./rem-harness-CVzoRsKa.js";
import { t as buildAgentSessionKey } from "./resolve-route-mwkm9MN4.js";
import { n as defaultRuntime } from "./runtime-kqN0Yohi.js";
import {
  a as rankShortTermPromotionCandidates,
  c as recordGroundedShortTermCandidates,
  d as repairShortTermPromotionArtifacts,
  f as resolveShortTermRecallLockPath,
  l as recordShortTermRecalls,
  n as applyShortTermPromotions,
  p as resolveShortTermRecallStorePath,
  r as auditShortTermPromotionArtifacts,
  u as removeGroundedShortTermCandidates,
} from "./short-term-promotion-C3R0Rj1g.js";
import { n as isRich, r as theme, t as colorize } from "./theme-CiH_wF8x.js";
import { n as resolvePreferredOpenClawTmpDir } from "./tmp-openclaw-dir-B4r8YQhH.js";
import { g as shortenHomePath, h as shortenHomeInString } from "./utils-BGRcpLKt.js";
...
async function listHistoricalDailyFiles(inputPath) {
  ...
  return (await fs$1.readdir(resolvedPath, { withFileTypes: true }))
...
async function scanSessionFiles(agentId) {
  ...
  try {
    return {
      source: "sessions",
      totalFiles: (await fs$1.readdir(sessionsDir, { withFileTypes: true })).filter(
...
async function scanMemorySources(params) {
  ...
  return {
    sources: scans,
    totalFiles: totals.some((total) => total === null)
      ? null
...
async function runMemoryStatus(opts) {
      ...
      run: async (manager) => {
        ...
        if (deep) {
          const hasVectorStoreProbe =
            manager.status().backend === "builtin" &&
            ...
            qmd:
              status.backend === "qmd"
                ? {
                    ...
                    collections:
                      typeof customQmd?.collections === "number" ? customQmd.collections : void 0,
                  }
                : void 0,
          });
        }
  ...
  for (const result of allResults) {
    ...
    if (status.vector) {
      const formatVectorState = (available) =>
      ...
      if (status.backend === "builtin") {
        ...
        if (status.vector.semanticAvailable !== void 0)
          formatVectorLine("Semantic vectors", formatVectorState(status.vector.semanticAvailable));
      } else
        formatVectorLine(
          "Vector",
          formatVectorState(status.vector.semanticAvailable ?? status.vector.available),
        );
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      for (const result of results) {
        lines.push(
          `${colorize(rich, theme.success, result.score.toFixed(3))} ${colorize(rich, theme.accent, `${shortenHomePath(result.path)}:${result.startLine}-${result.endLine}`)}`,
        );
    ...
    run: async (manager) => {
      ...
      if (candidates.length === 0) {
        ...
        if (audit.issues.length > 0)
          for (const issue of audit.issues) defaultRuntime.log(issue.message);
        return;
      }
      const rich = isRich();
      const lines = [];
      lines.push(
        `${colorize(rich, theme.heading, "Short-Term Promotion Candidates")} ${colorize(rich, theme.muted, `(${agentId})`)}`,
      );
      ...
      for (const candidate of candidates) {
        lines.push(
          `${colorize(rich, theme.success, candidate.score.toFixed(3))} ${colorize(rich, theme.accent, `${shortenHomePath(candidate.path)}:${candidate.startLine}-${candidate.endLine}`)}`,
        );
        lines.push(
          colorize(
            rich,
    ...
    run: async (manager) => {
      ...
      try {
        ...
        if (cleanupWorkspaceDir)
          await fs$1.rm(cleanupWorkspaceDir, {
            recursive: true,
            force: true,
          });
      }
...
async function runMemoryRemBackfill(opts) {
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs$1.mkdtemp(
      ...
      try {
        ...
        const sourcePathByDay = new Map(
          sourceFiles
            .map((sourcePath) => [extractIsoDayFromPath(sourcePath), sourcePath])
        ...
        if (opts.stageShortTerm) {
          replacedShortTermEntries = (await removeGroundedShortTermCandidates({ workspaceDir }))
            .removed;
