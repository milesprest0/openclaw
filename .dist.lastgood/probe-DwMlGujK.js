...
import { t as resolveTelegramAllowedUpdates } from "./allowed-updates-YLJOPZwV.js";
import { t as CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "./approval-handler-adapter-runtime-CEmfb1ag.js";
import {
  a as listTokenSourcedAccounts,
  i as createUnionActionGate,
  r as resolveReactionMessageId,
} from "./channel-actions-Ds3BuQCc.js";
...
import { o as buildExecApprovalPendingReplyPayload } from "./exec-approval-reply-BPlmHS3G.js";
import {
  a as isTelegramExecApprovalHandlerConfigured,
  i as isTelegramExecApprovalClientEnabled,
} from "./exec-approvals-Jy7fhfmT.js";
import { D as resolveExecApprovalRequestAllowedDecisions } from "./exec-approvals-VkyGhwX9.js";
...
import {
  ...
  u as isTelegramPollingNetworkError,
} from "./format-xrIurvHN.js";
import { t as isTelegramInlineButtonsEnabled } from "./inline-buttons-CG9H1O6u.js";
...
async function loadTelegramActionRuntime() {
  telegramActionRuntimePromise ??= import("./action-runtime-Dbfa8k19.js");
...
function resolveTelegramActionDiscovery(cfg) {
  ...
  return {
    ...
    buttonsEnabled: accounts.some((account) =>
...
function shouldSuppressTelegramExecApprovalForwardingFallback(params) {
  if ((normalizeMessageChannel(params.target.channel) ?? params.target.channel) !== "telegram")
    return false;
  if (normalizeMessageChannel(params.request.request.turnSourceChannel ?? "") !== "telegram")
    return false;
  const accountId =
...
function createLease(params) {
  ...
  return {
    ...
    release: () => {
      if (released) return;
      released = true;
      if (params.registry.get(params.tokenFingerprint)?.owner === owner)
...
async function monitorTelegramProvider(opts = {}) {
  ...
  try {
    ...
    const proxyFetch =
      opts.proxyFetch ?? (account.config.proxy ? makeProxyFetch(account.config.proxy) : void 0);
    if (opts.useWebhook) {
    ...
    try {
      ...
      const createTelegramTransportForPolling = () =>
        resolveTelegramTransport(proxyFetch, { network: account.config.network });
...
function resolveProbeFetcher(token, options) {
  ...
  const resolved = resolveTelegramFetch(proxyUrl ? makeProxyFetch(proxyUrl) : void 0, {
...
async function probeTelegram(token, timeoutMs, proxyOrOptions) {
  ...
  try {
    ...
    if (!meRes)
      throw fetchError ?? /* @__PURE__ */ new Error(`probe timed out after ${timeoutBudgetMs}ms`);
...
//#endregion
export {
  shouldSuppressTelegramExecApprovalForwardingFallback as a,
  buildTelegramExecApprovalPendingPayload as i,
  resetTelegramProbeFetcherCacheForTests as n,
  telegramMessageActions as o,
  monitorTelegramProvider as r,
  probeTelegram as t,
};
