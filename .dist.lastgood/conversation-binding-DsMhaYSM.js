...
import "./agent-harness-runtime-CRXkNxrN.js";
...
import { f as CODEX_CONTROL_METHODS, i as formatCodexDisplayText } from "./request-BC8expe9.js";
import {
  i as readCodexAppServerBinding,
  n as isCodexAppServerNativeAuthProfile,
  o as writeCodexAppServerBinding,
  r as normalizeCodexAppServerBindingModelProvider,
  t as clearCodexAppServerBinding,
} from "./session-binding-o-m5DVaT.js";
import {
  a as getSharedCodexAppServerClient,
...
function resolveCodexDefaultWorkspaceDir(pluginConfig) {
  return (
    readString$1(readRecord$1(readRecord$1(pluginConfig)?.appServer), "defaultWorkspaceDir") ??
    process.cwd()
  );
}
...
function trackCodexConversationActiveTurn(active) {
  ...
  return () => {
    if (activeTurns.get(active.sessionFile)?.turnId === active.turnId)
...
function createCodexConversationTurnCollector(threadId) {
  ...
  const handleNotification = (notification) => {
    ...
    if (notification.method === "turn/completed") {
      ...
      if (readString(turn, "status") === "failed")
        failedError =
...
async function handleCodexConversationInboundClaim(event, ctx, options = {}) {
  ...
  try {
    return {
      handled: true,
      reply: (
        await enqueueBoundTurn(data.sessionFile, () =>
...
async function attachExistingThread(params) {
  ...
  const runtimeApprovalPolicy =
    typeof runtime.approvalPolicy === "string" ? runtime.approvalPolicy : void 0;
    ...
    return {
      reply: {
        text:
          (
            await collector
              .wait({ timeoutMs: params.timeoutMs ?? DEFAULT_BOUND_TURN_TIMEOUT_MS })
              .finally(activeCleanup)
          ).replyText.trim() || "Codex completed without a text reply.",
      },
    };
  } finally {
    notificationCleanup();
...
//#endregion
export {
  parseCodexFastModeArg as a,
  setCodexConversationFastMode as c,
  steerCodexConversationTurn as d,
  stopCodexConversationTurn as f,
  formatPermissionsMode as i,
  setCodexConversationModel as l,
  resolveCodexDefaultWorkspaceDir as m,
  handleCodexConversationInboundClaim as n,
  parseCodexPermissionsModeArg as o,
  readCodexConversationBindingData as p,
  startCodexConversationThread as r,
  readCodexConversationActiveTurn as s,
  handleCodexConversationBindingResolved as t,
  setCodexConversationPermissions as u,
};
