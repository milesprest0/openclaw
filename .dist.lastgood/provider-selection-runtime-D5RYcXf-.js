...
function selectFirstAutoProvider(providers) {
  let selected;
  for (const provider of providers)
    if (!selected || compareProviderAutoSelectOrder(provider, selected) < 0) selected = provider;
  return selected;
}
...
//#endregion
export {
  resolveProviderRawConfig as n,
  selectConfiguredOrAutoProvider as r,
  resolveConfiguredCapabilityProvider as t,
};
