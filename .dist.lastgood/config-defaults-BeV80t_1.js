...
function resolveAnthropicDefaultAuthMode(config, env) {
  ...
  const anthropicProfiles = Object.entries(profiles).filter(
    ([, profile]) => profile?.provider === "anthropic" || profile?.provider === "claude-cli",
  );
  const order = [
    ...(config.auth?.order?.anthropic ?? []),
    ...(config.auth?.order?.["claude-cli"] ?? []),
  ];
  ...
  const hasOauth = anthropicProfiles.some(
    ([, profile]) =>
      profile?.provider === "claude-cli" || profile?.mode === "oauth" || profile?.mode === "token",
  );
...
function usesClaudeCliModelSelection(config) {
  ...
  if ((primary ? parseProviderModelRef(primary, "anthropic") : null)?.provider === "claude-cli")
    return true;
...
function toCanonicalAnthropicModelRef(ref) {
  return ref.startsWith(`claude-cli/`)
