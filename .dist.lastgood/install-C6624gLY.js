...
import {
  g as unscopedPackageName,
  v as resolveSafeInstallDir,
} from "./channel-catalog-registry-uu7_tUkL.js";
import { t as parseFrontmatter } from "./frontmatter-D_UmYjUx.js";
...
async function loadHookInstallRuntime() {
  hookInstallRuntimePromise ??= import("./install.runtime-BeDMWrM-.js");
...
async function validateHookDir(hookDir) {
  ...
  if (
    !(await Promise.all(
      ["handler.ts", "handler.js", "index.ts", "index.js"].map(async (candidate) =>
        runtime.fileExists(path.join(hookDir, candidate)),
      ),
...
async function installHookPackageFromDir(params) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInsideWithRealpath(params.packageDir, hookDir, { requireRealpath: true }))
      return {
        ok: false,
