...
import { t as buildQuotedMessageOptions } from "./quoted-message-BvLCUGwM.js";
...
function hasInboundUserContent(rawMessage) {
  ...
  for (const candidate of buildMessageChain(rawMessage))
    if (hasInteractiveResponseContent(candidate)) return true;
  return false;
}
//#endregion
...
function createWebSendApi(params) {
  const resolveOutboundJid = (recipient) =>
    params.authDir
      ? toWhatsappJidWithLid(recipient, { authDir: params.authDir })
      : toWhatsappJid(recipient);
  const resolveMentions = async (jid, text) =>
    params.resolveOutboundMentions
      ? await params.resolveOutboundMentions({
          jid,
          text,
        })
      : {
...
//#endregion
export {
  mayContainWhatsAppOutboundMention as a,
  extractContactContext as c,
  extractMediaPlaceholder as d,
  extractMentionedJids as f,
  addWhatsAppOutboundMentionsToContent as i,
  extractContextInfo as l,
  hasInboundUserContent as m,
  listWhatsAppSendResultMessageIds as n,
  resolveWhatsAppOutboundMentions as o,
  extractText as p,
  normalizeWhatsAppSendResult as r,
  describeReplyContext as s,
  createWebSendApi as t,
  extractLocationData as u,
};
