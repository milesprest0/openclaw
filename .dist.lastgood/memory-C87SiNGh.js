...
function getMemorySearchManagerCacheStore() {
  ...
  if (
    ...
    resolved !== null &&
    resolved.qmdManagerCache instanceof Map &&
    resolved.pendingQmdManagerCreates instanceof Map
  ) {
    const cacheStore = resolved;
    if (!(cacheStore.qmdManagerOpenFailures instanceof Map))
      cacheStore.qmdManagerOpenFailures = /* @__PURE__ */ new Map();
    return cacheStore;
  }
...
function loadQmdManagerModule() {
  qmdManagerModulePromise ??= import("./qmd-manager-DsWx64jV.js");
...
function clearQmdManagerOpenFailure(scopeKey, identityKey) {
  if (QMD_MANAGER_OPEN_FAILURES.get(scopeKey)?.identityKey === identityKey)
    QMD_MANAGER_OPEN_FAILURES.delete(scopeKey);
}
  ...
  if (resolved.backend === "qmd" && resolved.qmd) {
    ...
    const pendingCreate = {
      ...
      promise: (async () => {
        ...
        if (PENDING_QMD_MANAGER_CREATES.get(scopeKey) === pendingCreate)
