...
import {
  l as loadDeviceAuthToken,
  n as GatewayClient,
  r as GatewayClientRequestError,
...
import { i as READ_SCOPE } from "./operator-scopes-Codapd9g.js";
...
async function probeGateway(opts) {
  ...
  const deviceIdentity = await (async () => {
    try {
      ...
      const { loadDeviceIdentityIfPresent } = await import("./device-identity-WRrX-UIa.js");
      ...
      onHelloOk: async (hello) => {
        ...
        auth = resolveProbeAuthSummary({
          ...
          scopes: Array.isArray(hello?.auth?.scopes)
            ? hello.auth.scopes.filter((scope) => typeof scope === "string")
            : [],
          authMetadataPresent,
        });
        if (detailLevel === "none") {
...
export {
  probeGateway as a,
  isPairingPendingProbeFailure as i,
  MIN_PROBE_TIMEOUT_MS as n,
  resolveGatewayProbeCapability as o,
