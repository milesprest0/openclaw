...
import {
  n as resolveBundledPluginsDir,
  r as resolveSourceCheckoutDependencyDiagnostic,
} from "./bundled-dir-CSFHC_7h.js";
...
import { n as MANIFEST_KEY } from "./legacy-names-CY6qJpf3.js";
import { t as normalizeManifestCommandAliases } from "./manifest-command-aliases-Ymsnd92A.js";
import "./boundary-file-read-CkGowAmx.js";
import "./path-safety-eKsvzHks.js";
import {
  i as normalizeModelCatalogProviderId,
  n as normalizeModelCatalogProviderRows,
  r as buildModelCatalogMergeKey,
  t as normalizeModelCatalog,
} from "./normalize-C7P1Ynu1.js";
import { t as parseJsonWithJson5Fallback } from "./parse-json-compat-ViYJmwDA.js";
...
import {
  r as createPluginCacheKey,
  t as PluginLruCache,
} from "./plugin-cache-primitives-BzJqZ6Iq.js";
import { t as isBlockedObjectKey } from "./prototype-keys-D58M68EP.js";
import {
  i as openRootFileSync,
  n as matchRootFileOpenFailure,
  r as openRootFile,
} from "./root-file-B8t_jFqF.js";
import { i as resolveRootPathSync, r as resolveRootPath } from "./root-path-BwPOxXJq.js";
import "./boundary-path-CjEHOBDl.js";
import {
...
import { c as isRecord, d as resolveConfigDir, p as resolveUserPath } from "./utils-BGRcpLKt.js";
...
function parseOpenClawReleaseVersion(value) {
  ...
  const alphaNumber =
    candidate.channel === "alpha"
      ...
      : void 0;
  const betaNumber =
    candidate.channel === "beta" ? Number.parseInt(candidate.match.groups.beta ?? "", 10) : void 0;
  return {
...
function normalizePreviewCatalog(params) {
  const provider = normalizeModelCatalog(
...
function loadOpenClawProviderIndex(source = OPENCLAW_PROVIDER_INDEX) {
  return (
    normalizeOpenClawProviderIndex(source) ?? {
      version: 1,
      providers: {},
    }
  );
...
function normalizeManifestProviderRequestProvider(value) {
  ...
  const compatibilityFamily =
    normalizeOptionalString(value.compatibilityFamily) === "moonshot" ? "moonshot" : void 0;
  ...
  const openAICompletions =
    typeof supportsStreamingUsage === "boolean" ? { supportsStreamingUsage } : void 0;
...
function normalizeManifestDefaultPlatforms(value) {
  return normalizeTrimmedStringList(value).filter((platform) =>
    MANIFEST_DEFAULT_ENABLEMENT_PLATFORMS.has(platform),
  );
}
...
function normalizeManifestSetup(value) {
  ...
  const requiresRuntime =
    typeof value.requiresRuntime === "boolean" ? value.requiresRuntime : void 0;
  const setup = {
    ...(providers ? { providers } : {}),
...
function normalizeManifestChannelCommandDefaults(value) {
  ...
  const nativeCommandsAutoEnabled =
    typeof value.nativeCommandsAutoEnabled === "boolean" ? value.nativeCommandsAutoEnabled : void 0;
  const nativeSkillsAutoEnabled =
    typeof value.nativeSkillsAutoEnabled === "boolean" ? value.nativeSkillsAutoEnabled : void 0;
...
function slugifyPluginId(raw, rootDir) {
  ...
  return (
    (normalizeLowercaseStringOrEmpty(raw) || normalizeLowercaseStringOrEmpty(fallback))
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/-+/g, "-")
...
function isBundledSourceOverlayPath(params) {
  ...
  return (
    (params.mountPoints ?? readLinuxMountPoints()).has(resolved) || isFilesystemMountPoint(resolved)
  );
}
...
function listBuiltRuntimeEntryCandidates(entryPath) {
  ...
  const distWithoutExtension = normalizedRelative.startsWith("src/")
    ? `./dist/${normalizedRelative.slice(4).replace(/\.[^.]+$/u, "")}`
...
function resolvePackageEntrySource(params) {
  ...
  const openCandidate = (absolutePath) => {
    const opened = openRootFileSync({
      absolutePath,
      rootPath: params.packageDir,
      ...(params.packageRootRealPath !== void 0
...
function deriveIdHint(params) {
  ...
  const normalizedPackageId =
    unscoped.endsWith("-provider") && unscoped.length > 9 ? unscoped.slice(0, -9) : unscoped;
...
//#endregion
export {
  isPrereleaseResolutionAllowed as A,
  planProviderIndexModelCatalogRows as C,
  compareOpenClawReleaseVersions as D,
  loadOpenClawProviderIndex as E,
  parseRegistryNpmSpec as M,
  validateRegistryNpmSpec as N,
  formatPrereleaseResolutionError as O,
  shouldRejectHardlinkedPluginFiles as P,
  resolvePackageExtensionEntries as S,
  planManifestModelCatalogSuppressions as T,
  normalizeBundlePathList as _,
  resolvePluginSourceRoots as a,
  getPackageManifestMetadata as b,
  validatePackageExtensionEntriesForInstall as c,
  CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH as d,
  CODEX_BUNDLE_MANIFEST_RELATIVE_PATH as f,
  mergeBundlePathLists as g,
  loadBundleManifest as h,
  resolvePluginCacheInputs as i,
  isPrereleaseSemverVersion as j,
  isExactSemverVersion as k,
  buildBundledPluginLoadPathAliases as l,
  detectBundleManifestFormat as m,
  buildPluginDependencyStatus as n,
  tracePluginLifecyclePhase as o,
  CURSOR_BUNDLE_MANIFEST_RELATIVE_PATH as p,
  normalizePluginDependencySpecs as r,
  tracePluginLifecyclePhaseAsync as s,
  discoverOpenClawPlugins as t,
  normalizeBundledLookupPath as u,
  DEFAULT_PLUGIN_ENTRY_CANDIDATES as v,
  planManifestModelCatalogRows as w,
  loadPluginManifest as x,
  PLUGIN_MANIFEST_FILENAME as y,
};
