...
import { t as mergeActivationSectionsIntoRuntimeConfig } from "./plugin-activation-runtime-config-eemqNhzO.js";
...
import { n as listGatewayMethods } from "./server-methods-list-C0RQ1k8x.js";
import { c as initSubagentRegistry } from "./subagent-registry-DESvhcq6.js";
...
async function prepareGatewayPluginBootstrap(params) {
  ...
  if (!params.minimalTestGateway || startupMaintenanceConfig.channels !== void 0) {
    const { runChannelPluginStartupMaintenance } = await import("./lifecycle-startup-DL1s1Bu2.js");
    const startupTasks = [
    ...
    if (!params.minimalTestGateway) {
      const { runStartupSessionMigration } =
        await import("./server-startup-session-migration-BPZ4BQTA.js");
      startupTasks.push(
...
async function loadGatewayStartupPluginRuntime(params) {
  const { loadGatewayStartupPlugins } = await import("./server-plugin-bootstrap-CLEjU-xr.js");
