...
import { t as resolveMatrixRoomKeyBackupIssue } from "./backup-health-1pC5l6ya.js";
import { t as withResolvedActionClient } from "./client-zsajgNip.js";
import { r as resolveMatrixAuthContext } from "./config-HHHAstzN.js";
import { n as resolveMatrixConfigPath } from "./config-paths-CNjwUFIB.js";
import { t as updateMatrixAccountConfig } from "./config-update-CcUth9IH.js";
import {
  n as summarizeMatrixDeviceHealth,
  t as isOpenClawManagedMatrixDevice,
} from "./device-health-rVVIlN7T.js";
...
import { i as setMatrixSdkLogMode, r as setMatrixSdkConsoleLogging } from "./logging-OuOcffLy.js";
import {
  n as updateMatrixOwnProfile,
  t as applyMatrixProfileUpdate,
} from "./profile-update-DrbYVi0n.js";
import "./client-BZL_DUpm.js";
import "./runtime-api-BxUd6s1_.js";
import { t as getMatrixRuntime } from "./runtime-Brjuvs-E.js";
import { n as matrixSetupAdapter } from "./setup-core-BtIr1_RT.js";
import {
  a as confirmMatrixVerificationSas,
  c as getMatrixRoomKeyBackupStatus,
  d as listMatrixVerifications,
  f as mismatchMatrixVerificationSas,
  g as runMatrixSelfVerification,
  h as restoreMatrixRoomKeyBackup,
  l as getMatrixVerificationSas,
  m as resetMatrixRoomKeyBackup,
  n as bootstrapMatrixVerification,
  p as requestMatrixVerification,
  r as cancelMatrixVerification,
  t as acceptMatrixVerification,
  u as getMatrixVerificationStatus,
  v as startMatrixVerification,
  y as verifyMatrixRecoveryKey,
} from "./verification-PclgQYmw.js";
...
async function pruneMatrixStaleGatewayDevices(opts = {}) {
  return await withResolvedActionClient(opts, async (client) => {
    ...
    const staleGatewayDeviceIds = summarizeMatrixDeviceHealth(devices).staleOpenClawDevices.map(
...
function loadMatrixActionClientModule() {
  matrixActionClientModulePromise ??= import("./client-aJK0oSUX.js");
  return matrixActionClientModulePromise;
}
function loadMatrixDirectManagementModule() {
  matrixDirectManagementModulePromise ??= import("./direct-management-BwmtqISh.js");
...
async function addMatrixAccount(params) {
  ...
  if (accountConfig.encryption === true) {
    const { maybeBootstrapNewEncryptedMatrixAccount } =
      await import("./setup-bootstrap-dWH-MT5W.js");
  ...
  if (desiredDisplayName || desiredAvatarUrl)
    try {
      ...
      if (synced.convertedAvatarFromHttp && synced.resolvedAvatarUrl) {
        const withAvatar = updateMatrixAccountConfig(runtime.config.current(), accountId, {
...
function printDirectRoomCandidate(room) {
  ...
  console.log(
    `- ${formatMatrixCliText(room.roomId)} [${room.source}] strict=${room.strict ? "yes" : "no"} joined=${members}`,
  );
}
function printDirectRoomInspection(result) {
  ...
  console.log(
    `Mapped rooms: ${result.mappedRoomIds.length ? result.mappedRoomIds.map((roomId) => formatMatrixCliText(roomId)).join(", ") : "none"}`,
  );
  console.log(
    `Discovered strict rooms: ${result.discoveredStrictRoomIds.length ? result.discoveredStrictRoomIds.map((roomId) => formatMatrixCliText(roomId)).join(", ") : "none"}`,
  );
...
async function setupMatrixEncryption(params) {
  ...
  if (
    ...
    throw new Error(
      `Matrix account "${accountId}" is not configured; run ${formatMatrixCliCommand("account add", accountId)} first.`,
    );
...
function sanitizeMatrixCliText(value) {
  ...
  for (const character of withoutAnsi)
    if (!isUnsafeMatrixCliTerminalCode(character.charCodeAt(0))) sanitized += character;
  return sanitized;
}
...
function printMatrixVerificationSummary(summary) {
  ...
  console.log(
    `Methods: ${summary.methods.length ? summary.methods.map(sanitizeMatrixCliText).join(", ") : "none"}`,
  );
...
function printVerificationStatus(status, verbose = false, accountId) {
  ...
  if (verbose) {
    ...
    if (status.serverDeviceKnown !== void 0)
...
function printMatrixEncryptionSetupResult(result, verbose = false) {
  printAccountLabel(result.accountId);
  console.log(
    `Encryption config: ${result.encryptionChanged ? "enabled" : "already enabled"} at ${formatMatrixCliText(result.configPath)}`,
  );
...
function registerMatrixCli(params) {
  ...
  root
    .command("account")
    .description("Manage matrix channel accounts")
    .command("add")
  ...
  root
    .command("profile")
    .description("Manage Matrix bot profile")
    .command("set")
      ...
      await runMatrixCliCommand({
        ...
        onText: (result) => {
          ...
          if (result.profile.convertedAvatarFromHttp && result.avatarUrl)
            console.log(`Avatar converted and saved as: ${formatMatrixCliText(result.avatarUrl)}`);
        },
  ...
  root
    .command("encryption")
    .description("Set up Matrix end-to-end encryption")
    .command("setup")
  ...
  verify
    ...
    .action(async (options) => {
      ...
      await runMatrixCliCommand({
        ...
        json: options.json === true,
        run: async () => {
          if (options.ownUser === true && (options.userId || options.deviceId || options.roomId))
            throw new Error(
  ...
  backup
    ...
    .action(async (options) => {
      ...
      await runMatrixCliCommand({
        ...
        json: options.json === true,
        run: async () =>
          await getMatrixRoomKeyBackupStatus({
            accountId,
            cfg,
          }),
        onText: (status, verbose) => {
  ...
  devices
    ...
    .action(async (options) => {
      ...
      await runMatrixCliCommand({
        ...
        json: options.json === true,
        run: async () =>
          await pruneMatrixStaleGatewayDevices({
            accountId,
            cfg,
          }),
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale OpenClaw devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.map((deviceId) => formatMatrixCliText(deviceId)).join(", ") : "none"}`,
          );
