...
import {
  a as resolveAllowFromAccountId,
  c as safeChannelKey,
  i as readAllowFromFileWithExists,
  l as setAllowFromFileReadCache,
  n as dedupePreserveOrder,
  o as resolveAllowFromFilePath,
  r as readAllowFromFileSyncWithExists,
  s as resolvePairingCredentialsDir,
  t as clearAllowFromFileReadCacheForNamespace,
  u as shouldIncludeLegacyAllowFromEntries,
} from "./allow-from-store-file-OPFnpxrV.js";
...
function normalizeAllowEntry(channel, entry) {
  ...
  return (
    normalizeOptionalString(
      adapter?.normalizeAllowEntry ? adapter.normalizeAllowEntry(trimmed) : trimmed,
    ) ?? ""
  );
}
function normalizeAllowFromList(channel, store) {
  return dedupePreserveOrder(
    (Array.isArray(store.allowFrom) ? store.allowFrom : [])
      .map((v) => normalizeAllowEntry(channel, v))
      .filter(Boolean),
  );
}
...
async function readChannelAllowFromStore(channel, env = process.env, accountId) {
  ...
  const scopedEntries = await readAllowFromStateForPath(
    channel,
    resolveAllowFromFilePath(channel, env, resolvedAccountId),
  );
  const legacyEntries = await readAllowFromStateForPath(
    channel,
...
function readChannelAllowFromStoreSync(channel, env = process.env, accountId) {
  ...
  const scopedEntries = readAllowFromStateForPathSync(
    channel,
    resolveAllowFromFilePath(channel, env, resolvedAccountId),
  );
  const legacyEntries = readAllowFromStateForPathSync(
    channel,
    resolveAllowFromFilePath(channel, env),
  );
  return dedupePreserveOrder([...scopedEntries, ...legacyEntries]);
  ...
  return await withFileLock(
    ...
    async () => {
      ...
      if (existingIdx >= 0) {
        ...
        const code =
          (normalizeOptionalString(existing?.code) ?? "") || generateUniqueCode(existingCodes);
        const next = {
          id,
          code,
...
export {
  ...
  readLegacyChannelAllowFromStoreSync as c,
  upsertChannelPairingRequest as d,
  getPairingAdapter as f,
  listChannelPairingRequests as i,
  removeChannelAllowFromStoreEntry as l,
  notifyPairingApproved as m,
  approveChannelPairingCode as n,
  readChannelAllowFromStoreSync as o,
  listPairingChannels as p,
  clearPairingAllowFromReadCacheForTest as r,
  readLegacyChannelAllowFromStore as s,
  addChannelAllowFromStoreEntry as t,
  resolveChannelAllowFromPath as u,
};
