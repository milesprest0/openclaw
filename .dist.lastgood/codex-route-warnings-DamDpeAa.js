...
import { t as AGENT_MODEL_CONFIG_KEYS } from "./model-refs-42YDfhLg.js";
...
function collectCodexRouteWarnings(params) {
  ...
  return [
    [
      ...
      ...hits.map(
        (hit) =>
          `- ${hit.path}: ${hit.model} should become ${hit.canonicalModel}${hit.runtime ? `; current runtime is "${hit.runtime}"` : ""}.`,
      ),
...
function maybeRepairCodexRoutes(params) {
  if (collectConfigModelRefs(params.cfg, params.env).length === 0)
    return {
      cfg: params.cfg,
      warnings: [],
      changes: [],
...
function rewriteSessionModelPair(params) {
  ...
  const model =
    typeof params.entry[params.modelKey] === "string" ? params.entry[params.modelKey] : void 0;
...
function scanCodexSessionStoreRoutes(store) {
  return Object.entries(store).flatMap(([sessionKey, entry]) => {
    if (!entry) return [];
    return normalizeString(entry.modelProvider) === "openai-codex" ||
...
async function maybeRepairCodexSessionRoutes(params) {
  ...
  if (!params.shouldRepair) {
    const stale = targets.flatMap((target) => {
      return scanCodexSessionStoreRoutes(loadSessionStore(target.storePath)).map(
        (sessionKey) => `${target.agentId}:${sessionKey}`,
      );
    });
    return {
  ...
  return {
    ...
    changes:
      repairedSessions > 0
        ? [
            `Repaired Codex session routes: moved ${repairedSessions} session${repairedSessions === 1 ? "" : "s"} across ${repairedStores} store${repairedStores === 1 ? "" : "s"} to openai/* while preserving runtime policy.`,
          ]
        : [],
  };
}
//#endregion
export {
  repairCodexSessionStoreRoutes as i,
  maybeRepairCodexRoutes as n,
  maybeRepairCodexSessionRoutes as r,
  collectCodexRouteWarnings as t,
};
