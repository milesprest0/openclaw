...
function isMp3(buf) {
  ...
  return (
    ...
    (buf[0] === 255 && (buf[1] & 224) === 224 && (buf[1] & 6) !== 0) ||
...
var AudioBuffer$1 = class {
  ...
  constructor(options) {
    ...
    if (options.sampleRate < 3e3 || options.sampleRate > 768e3)
...
const decoders = {
  async oga(buf) {
    ...
    if (!decoder) {
      let { OggVorbisDecoder } = await import("./ogg-vorbis-BFHu14ND.js");
  ...
  async mp3(buf) {
    ...
    if (!decoder) {
      const { MPEGDecoder } = await import("./mpg123-decoder-BGLt-Afw.js");
  ...
  async flac(buf) {
    ...
    if (!decoder) {
      const { FLACDecoder } = await import("./flac-wkVvhu-k.js");
  ...
  async opus(buf) {
    ...
    if (!decoder) {
      const { OggOpusDecoder } = await import("./ogg-opus-decoder-BEyUwciB.js");
  ...
  async wav(buf) {
    ...
    if (!decode) {
      let module = await import("./node-wav-Bk_H2VBD.js").then((m) =>
        /* @__PURE__ */ __toESM(m.default, 1),
      );
  ...
  async qoa(buf) {
    ...
    if (!decode) decoders.qoa.decode = decode = (await import("./qoa-format-BZBPK1Kk.js")).decode;
    return buf && createBuffer(await decode(buf));
  },
};
