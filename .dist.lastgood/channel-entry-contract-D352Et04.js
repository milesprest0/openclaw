...
import { o as emptyChannelConfigSchema } from "./config-schema-BPiFZhPG.js";
import {
  n as formatPluginLoadProfileLine,
  r as shouldProfilePluginLoader,
  t as createProfiler,
} from "./plugin-load-profile-ByUHrmZ9.js";
import {
  r as getCachedPluginSourceModuleLoader,
...
function addBundledEntryCandidates(candidates, basePath, boundaryRoot) {
  for (const candidate of resolveSpecifierCandidates(basePath)) {
    if (candidates.some((entry) => entry.path === candidate && entry.boundaryRoot === boundaryRoot))
      continue;
    candidates.push({
      path: candidate,
      boundaryRoot,
      ...
      formatBundledEntryModuleOpenFailure({
        ...
        failure: {
          ...
          error: /* @__PURE__ */ new Error(
            `ENOENT: no such file or directory, lstat '${fallbackCandidate.path}'`,
          ),
        },
      }),
    );
...
function defineBundledChannelEntry({
  id,
  name,
  description,
  importMetaUrl,
  ...
  const loadChannelOutbound = outbound
    ? (options) => loadBundledEntryExportSync(importMetaUrl, outbound, options)
    : void 0;
  const loadChannelSecrets = secrets
    ? (options) => loadBundledEntryExportSync(importMetaUrl, secrets, options)
    : void 0;
  const loadChannelAccountInspector = accountInspect
    ? (options) => loadBundledEntryExportSync(importMetaUrl, accountInspect, options)
    : void 0;
...
function defineBundledChannelSetupEntry({
  ...
  const loadLegacyStateMigrationDetector = legacyStateMigrations
    ? (options) => loadBundledEntryExportSync(importMetaUrl, legacyStateMigrations, options)
    : void 0;
  const loadLegacySessionSurface = legacySessionSurface
    ? (options) => loadBundledEntryExportSync(importMetaUrl, legacySessionSurface, options)
    : void 0;
  return {
    ...
    ...(secrets
      ? {
          loadSetupSecrets: (options) =>
            loadBundledEntryExportSync(importMetaUrl, secrets, options),
        }
      : {}),
...
//#endregion
export {
  defineBundledChannelSetupEntry as n,
  loadBundledEntryExportSync as r,
  defineBundledChannelEntry as t,
};
