...
function resolveLineAccount(params) {
  ...
  const enabled =
    accountConfig?.enabled ?? (accountId === "default" ? (lineConfig?.enabled ?? true) : false);
  return {
    accountId,
...
function resolveDefaultLineAccountId(cfg) {
  ...
  if (
    preferred &&
    listLineAccountIds(cfg).some((accountId) => normalizeAccountId$1(accountId) === preferred)
  )
    return preferred;
...
//#endregion
export {
  resolveLineAccount as i,
  normalizeAccountId as n,
  resolveDefaultLineAccountId as r,
  listLineAccountIds as t,
};
