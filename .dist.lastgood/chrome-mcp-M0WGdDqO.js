...
import "./record-shared-DTpcMPFX.js";
    ...
    try {
      await withChromeMcpHandshakeTimeout(
        (async () => {
          ...
          if (!(await client.listTools()).tools.some((tool) => tool.name === "list_pages"))
      ...
      throw new BrowserProfileUnavailableError(
        `Chrome MCP existing-session attach failed for profile "${profileName}". Make sure ${options.browserUrl ? `the configured Chrome endpoint (${options.browserUrl})` : options.userDataDir ? `the configured Chromium user data dir (${options.userDataDir})` : "Google Chrome's default profile"} is running locally with remote debugging enabled. Details: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  })();
  ready.catch(() => {});
...
async function findPageById(profileName, pageId, profileOptions) {
  const page = (await listChromeMcpPages(profileName, profileOptions)).find(
...
function getChromeMcpPid(profileName) {
  for (const [key, session] of sessions.entries())
    if (cacheKeyMatchesProfileName(key, profileName)) return session.transport.pid ?? null;
  return null;
}
...
async function listChromeMcpPages(profileName, profileOptions, options = {}) {
  return extractStructuredPages(
    await callTool(profileName, profileOptions, "list_pages", {}, options),
  );
}
...
async function openChromeMcpTab(profileName, url, profileOptions) {
  ...
  const pages = extractStructuredPages(
    await callTool(profileName, profileOptions, "new_page", {
      url: "about:blank",
...
async function takeChromeMcpSnapshot(params) {
  return extractSnapshot(
    await callTool(params.profileName, chromeMcpProfileOptionsFromParams(params), "take_snapshot", {
      pageId: parsePageId(params.targetId),
    }),
  );
}
...
//#endregion
export {
  stopAllChromeMcpSessions as C,
  waitForChromeMcpText as D,
  uploadChromeMcpFile as E,
  setChromeMcpSessionFactoryForTest as S,
  takeChromeMcpSnapshot as T,
  navigateChromeMcpPage as _,
  closeChromeMcpTab as a,
  resetChromeMcpSessionsForTest as b,
  evaluateChromeMcpScript as c,
  focusChromeMcpTab as d,
  getChromeMcpPid as f,
  listChromeMcpTabs as g,
  listChromeMcpPages as h,
  closeChromeMcpSession as i,
  fillChromeMcpElement as l,
  hoverChromeMcpElement as m,
  clickChromeMcpCoords as n,
  dragChromeMcpElement as o,
  handleChromeMcpDialog as p,
  clickChromeMcpElement as r,
  ensureChromeMcpAvailable as s,
  buildChromeMcpArgs as t,
  fillChromeMcpForm as u,
  openChromeMcpTab as v,
  takeChromeMcpScreenshot as w,
  resizeChromeMcpPage as x,
  pressChromeMcpKey as y,
};
