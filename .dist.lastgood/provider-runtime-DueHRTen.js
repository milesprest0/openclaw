import {
  n as getLoadedRuntimePluginRegistry,
  t as getActiveRuntimePluginRegistry,
} from "./active-runtime-registry-B331FvbS.js";
import { t as sanitizeForLog } from "./ansi-BiBWu5cu.js";
import { l as resolveGpt5SystemPromptContribution } from "./gpt5-prompt-overlay-DpzD1H_Q.js";
import { t as normalizeProviderModelIdWithManifest } from "./manifest-model-id-normalization-1hQVtYU8.js";
import { i as resolveConfigScopedRuntimeCacheValue } from "./plugin-cache-primitives-BzJqZ6Iq.js";
import { a as resolvePluginControlPlaneFingerprint } from "./plugin-metadata-snapshot-C2hpt3LY.js";
import {
  n as mergePluginTextTransforms,
  t as applyPluginTextReplacements,
...
import {
  c as resolveExternalAuthProfileProviderPluginIds,
  d as resolveOwningPluginIdsForProvider,
  r as resolveCatalogHookProviderPluginIds,
  s as resolveExternalAuthProfileCompatFallbackPluginIds,
} from "./providers-CWWim-ts.js";
import {
  n as resolvePluginProviders,
  r as resolveProviderConfigApiOwnerHint,
  t as isPluginProvidersLoadInFlight,
} from "./providers.runtime-MFDKvuvZ.js";
import { n as getActivePluginRegistryWorkspaceDirFromState } from "./runtime-state-CQLzHhNR.js";
import {
...
function prepareProviderExtraParams(params) {
  return (
    ensureProviderRuntimePluginHandle(params).plugin?.prepareExtraParams?.(params.context) ?? void 0
  );
}
...
function resolveRuntimeTextTransforms() {
  ...
  return mergePluginTextTransforms(
    ...(Array.isArray(registry?.textTransforms)
...
function resolveProviderSystemPromptContribution(params) {
  ...
  return mergeProviderSystemPromptContributions(
    ...
    plugin?.resolveSystemPromptContribution?.(params.context) ?? void 0,
  );
}
...
function transformProviderSystemPrompt(params) {
  ...
  return applyPluginTextReplacements(
...
function normalizeProviderModelIdWithPlugin(params) {
  return (
    normalizeOptionalString(
      resolveProviderHookPlugin(params)?.normalizeModelId?.(params.context),
    ) ?? normalizeProviderModelIdWithManifest(params)
  );
}
function normalizeProviderTransportWithPlugin(params) {
  const hasTransportChange = (normalized) =>
...
function inspectProviderToolSchemasWithPlugin(params) {
  return (
    ensureProviderRuntimePluginHandle(params).plugin?.inspectToolSchemas?.(params.context) ?? void 0
  );
}
...
function resolveProviderTransportTurnStateWithPlugin(params) {
  return (
    resolveProviderRuntimePlugin(params)?.resolveTransportTurnState?.(params.context) ?? void 0
  );
}
function resolveProviderWebSocketSessionPolicyWithPlugin(params) {
  return (
    resolveProviderRuntimePlugin(params)?.resolveWebSocketSessionPolicy?.(params.context) ?? void 0
  );
}
...
function matchesProviderContextOverflowWithPlugin(params) {
  ...
  for (const plugin of plugins)
    if (plugin.matchesContextOverflowError?.(params.context)) return true;
  return false;
}
...
function resolveProviderThinkingProfile(params) {
  ...
  if (bundledSurface?.resolveThinkingProfile)
    return bundledSurface.resolveThinkingProfile(params.context) ?? void 0;
...
function applyProviderConfigDefaultsWithPlugin(params) {
  ...
  if (bundledSurface?.applyConfigDefaults)
    return bundledSurface.applyConfigDefaults(params.context) ?? void 0;
...
function resolveProviderSyntheticAuthWithPlugin(params) {
  ...
  if (typeof discoveryProvider?.resolveSyntheticAuth === "function")
    return discoveryProvider.resolveSyntheticAuth(params.context) ?? void 0;
...
//#endregion
export {
  resolveProviderRuntimePlugin as $,
  resolveProviderReasoningOutputModeWithPlugin as A,
  resolveProviderWebSocketSessionPolicyWithPlugin as B,
  resolveExternalAuthProfilesWithPlugins as C,
  resolveProviderConfigApiKeyWithPlugin as D,
  resolveProviderCacheTtlEligibility as E,
  resolveProviderTextTransforms as F,
  shouldPreferProviderRuntimeResolvedModel as G,
  runProviderDynamicModel as H,
  resolveProviderThinkingProfile as I,
  resolveRuntimeTextTransforms as J,
  transformProviderSystemPrompt as K,
  resolveProviderTransportTurnStateWithPlugin as L,
  resolveProviderStreamFn as M,
  resolveProviderSyntheticAuthWithPlugin as N,
  resolveProviderDefaultThinkingLevel as O,
  resolveProviderSystemPromptContribution as P,
  resolveProviderFollowupFallbackRoute as Q,
  resolveProviderUsageAuthWithPlugin as R,
  refreshProviderOAuthCredentialWithPlugin as S,
  resolveProviderBinaryThinking as T,
  sanitizeProviderReplayHistoryWithPlugin as U,
  resolveProviderXHighThinking as V,
  shouldDeferProviderSyntheticProfileAuthWithPlugin as W,
  resolveProviderAuthProfileId as X,
  prepareProviderExtraParams as Y,
  resolveProviderExtraParamsForTransport as Z,
  normalizeProviderResolvedModelWithPlugin as _,
  applyProviderResolvedTransportWithPlugin as a,
  prepareProviderDynamicModel as b,
  buildProviderMissingAuthMessageWithPlugin as c,
  createProviderEmbeddingProvider as d,
  resolveProviderRuntimePluginHandle as et,
  formatProviderAuthProfileApiKeyWithPlugin as f,
  normalizeProviderModelIdWithPlugin as g,
  normalizeProviderConfigWithPlugin as h,
  applyProviderResolvedModelCompatWithPlugins as i,
  resolveProviderReplayPolicyWithPlugin as j,
  resolveProviderModernModelRef as k,
  buildProviderUnknownModelHintWithPlugin as l,
  matchesProviderContextOverflowWithPlugin as m,
  applyProviderConfigDefaultsWithPlugin as n,
  augmentModelCatalogWithProviderPlugins as o,
  inspectProviderToolSchemasWithPlugin as p,
  validateProviderReplayTurnsWithPlugin as q,
  applyProviderNativeStreamingUsageCompatWithPlugin as r,
  buildProviderAuthDoctorHintWithPlugin as s,
  __testing as t,
  wrapProviderStreamFn as tt,
  classifyProviderFailoverReasonWithPlugin as u,
  normalizeProviderToolSchemasWithPlugin as v,
  resolveExternalOAuthProfilesWithPlugins as w,
  prepareProviderRuntimeAuth as x,
  normalizeProviderTransportWithPlugin as y,
  resolveProviderUsageSnapshotWithPlugin as z,
};
