...
import {
  c as matchesExpectedPluginId,
  d as resolveDefaultPluginNpmDir,
  h as packageNameMatchesId,
  l as resolveDefaultPluginExtensionsDir,
  m as validatePluginId,
  p as safePluginInstallFileName,
  s as encodePluginInstallDirName,
...
import {
  ...
  c as validatePackageExtensionEntriesForInstall,
  j as isPrereleaseSemverVersion,
  k as isExactSemverVersion,
} from "./discovery-bnS95tO3.js";
import { r as runCommandWithTimeout } from "./exec-Csn_09g3.js";
import {
  a as resolveNpmSpecMetadata,
  i as resolveNpmPackArchiveMetadata,
} from "./install-source-utils-DrH7L6Si.js";
import { d as writeJson, i as readJsonIfExists, r as readJson } from "./json-files-DifBk3kt.js";
...
import {
  n as relinkOpenClawPeerDependenciesInManagedNpmRoot,
  t as linkOpenClawPeerDependencies,
} from "./plugin-peer-link-lg5CcSDT.js";
import {
  n as createSafeNpmInstallEnv,
  t as createSafeNpmInstallArgs,
} from "./safe-package-install-BQGUJgWF.js";
import {
...
function readDependencyRecord(value) {
  ...
  for (const [key, raw] of Object.entries(value))
    if (typeof raw === "string") dependencies[key] = raw;
  return dependencies;
}
...
async function repairManagedNpmRootOpenClawPeer(params) {
  ...
  const hasManifestDependency =
    "openclaw" in
    readDependencyRecord(
      (await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"))).dependencies,
    );
...
async function scrubManagedNpmRootOpenClawPeer(params) {
  ...
  if (await pathExists(openclawPackageDir))
    await fs.rm(openclawPackageDir, {
      recursive: true,
      force: true,
    });
...
async function resolveTrustedOfficialPrereleaseResolution(params) {
  ...
  const semverVersions = (Array.isArray(parsed) ? parsed : [parsed]).filter(
    (value) => typeof value === "string" && isExactSemverVersion(value),
  );
...
async function installPluginFromSourceDir(params) {
  if (await detectNativePackageInstallSource(params.sourceDir))
    ...
    scan: async () =>
      await params.runtime.scanInstalledPackageDependencyTree({
        allowManagedNpmRootPackagePeerSymlinks:
          params.dependencyScanRootDir !== void 0 &&
...
async function installPluginFromFile(params) {
  ...
  try {
    await (
      await runtime.root(extensionsDir)
    ).copyIn(path.basename(preparedTarget.targetPath), filePath);
  } catch (err) {
    return {
      ok: false,
      error: String(err),
...
async function installPluginFromNpmPackArchive(params) {
  const { logger, timeoutMs, mode, dryRun } = (
    await loadPluginInstallRuntime()
  ).resolveTimedInstallModeOptions(params, defaultLogger);
...
//#endregion
export {
  installPluginFromNpmSpec as a,
  installPluginFromNpmPackArchive as i,
  installPluginFromArchive as n,
  installPluginFromPath as o,
  installPluginFromInstalledPackageDir as r,
  PLUGIN_INSTALL_ERROR_CODE as t,
};
