...
import {
  d as isRootHelpInvocation,
  f as isRootVersionInvocation,
  r as getCommandPositionalsWithRootOptions,
...
import { t as resolveNodeStartupTlsEnvironment } from "./node-startup-env-Dd-tFr9y.js";
import { t as ensureOpenClawExecMarkerOnProcess } from "./openclaw-exec-env-C8wgFL16.js";
import {
  a as resolveCliContainerTarget,
...
import { t as installProcessWarningFilter } from "./warning-filter-DGMT4JYy.js";
...
function runOpenClawCompileCacheRespawnPlan(
  ...
  child.once("error", (error) => {
    clearSignalExitTimer();
    runtime.writeError(
      `[openclaw] Failed to respawn CLI without compile cache: ${error instanceof Error ? (error.stack ?? error.message) : String(error)}\n`,
    );
    runtime.exit(1);
  });
  return child;
...
function resolveCliRespawnCommand(params) {
  const basename = pathModuleForPlatform(params.platform ?? process.platform)
    .basename(params.execPath)
    .toLowerCase();
...
function hasExperimentalWarningSuppressed(params = {}) {
  ...
  return execArgv.some(
    (arg) => arg === "--disable-warning=ExperimentalWarning" || arg === "--no-warnings",
  );
}
function buildCliRespawnPlan(params = {}) {
  ...
  if (shouldSkipStartupEnvironmentRespawnForArgv(argv) || isTruthyEnvValue(env.OPENCLAW_NO_RESPAWN))
    return null;
...
if (
  ...
  if (
    ...
    normalizeEnv();
    if (!isRootHelpInvocation(process$1.argv) && !isRootVersionInvocation(process$1.argv))
      assertNotRoot();
    enableOpenClawCompileCache({ installRoot });
    gatewayEntryStartupTrace.mark("bootstrap");
    if (shouldForceReadOnlyAuthStore(process$1.argv))
      process$1.env.OPENCLAW_AUTH_STORE_READONLY = "1";
  ...
  try {
    ...
    if (
      !(
        ...
        (await import("./root-help-metadata-DHiEWDay.js")).outputPrecomputedRootHelpText
      )()
    ) {
