...
import { n as hasActiveCronJobs } from "./active-jobs-qoCzunMt.js";
import {
...
import { c as getQueueSize, s as getCommandLaneSnapshots } from "./command-queue-BtommWHg.js";
import "./agent-scope-9AmhTwki.js";
import { t as appendCronStyleCurrentTimeLine } from "./current-time-DseN6ll4.js";
import { i as resolveUserTimezone } from "./date-time-BBf0a32z.js";
import { t as deliverOutboundPayloads } from "./deliver-Dz9Dpt9U.js";
...
import { a as formatErrorMessage, s as hasErrnoCode } from "./errors-DZMrVkYL.js";
import {
  a as isHeartbeatContentEffectivelyEmpty,
  c as resolveHeartbeatPrompt$1,
  l as resolveHeartbeatPromptForResponseTool,
  o as isTaskDue,
  s as parseHeartbeatTasks,
  u as stripHeartbeatToken,
} from "./heartbeat-Cn4100FJ.js";
import { a as resolveIndicatorType, t as emitHeartbeatEvent } from "./heartbeat-events-Dux0y1fS.js";
import {
  a as isRelayableExecCompletionEvent,
  i as isExecCompletionEvent,
  n as buildExecEventPrompt,
  r as isCronSystemEvent,
  t as buildCronEventPrompt,
} from "./heartbeat-events-filter-DfMXYGL3.js";
...
import {
  a as getHeartbeatToolNotificationText,
  s as resolveHeartbeatToolResponseFromReplyResult,
...
import {
  a as isRetryableHeartbeatBusySkipReason,
  i as areHeartbeatsEnabled,
  n as HEARTBEAT_SKIP_LANES_BUSY,
  o as requestHeartbeat,
  r as HEARTBEAT_SKIP_REQUESTS_IN_FLIGHT,
  s as setHeartbeatWakeHandler,
  t as HEARTBEAT_SKIP_CRON_IN_PROGRESS,
} from "./heartbeat-wake-2F-mloyP.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import { n as isNestedAgentLane } from "./lanes-CEV2BIp0.js";
import { t as resolveEmbeddedSessionLane } from "./lanes-CY07HApd.js";
import {
...
import { t as getChannelPlugin } from "./registry-D3zb3mnd.js";
import {
...
import { n as resolveResponsePrefixTemplate } from "./response-prefix-template-CgiuGPjz.js";
import { t as getActivePluginChannelRegistry } from "./runtime-ChXyIFtl.js";
import { n as defaultRuntime } from "./runtime-kqN0Yohi.js";
import { t as resolveCronSession } from "./session-BTvoPxV7.js";
...
import {
  ...
  t as archiveRemovedSessionTranscripts,
} from "./store-BpWdoYPF.js";
import {
  n as listDueCommitmentSessionKeys,
  o as markCommitmentsAttempted,
  r as listDueCommitmentsForSession,
  s as markCommitmentsStatus,
} from "./store-BzpcO0Ys.js";
...
import {
  n as resolveHeartbeatSenderContext,
  t as resolveHeartbeatDeliveryTarget,
} from "./targets-BqvQ_FF9.js";
import {
  n as resolveSafeTimeoutDelayMs,
  t as MAX_SAFE_TIMEOUT_DELAY_MS,
} from "./timer-delay-DZlUjkX3.js";
import { t as HEARTBEAT_TOKEN } from "./tokens-DWz8lWRf.js";
import { t as createTypingCallbacks } from "./typing-Cye5G_6W.js";
import { b as escapeRegExp } from "./utils-BGRcpLKt.js";
import { r as DEFAULT_HEARTBEAT_FILENAME } from "./workspace-DvzHmVcJ.js";
...
function resolveHeartbeatPhaseMs(params) {
  ...
  return (
    createHash("sha256")
      ...
      .digest()
      .readUInt32BE(0) % intervalMs
  );
}
function computeNextHeartbeatPhaseDueMs(params) {
  const intervalMs = Math.max(1, Math.floor(params.intervalMs));
  const nowMs = Math.floor(params.nowMs);
  let deltaMs = normalizeModulo(
    normalizeModulo(Math.floor(params.phaseMs), intervalMs) - normalizeModulo(nowMs, intervalMs),
    intervalMs,
  );
...
function createHeartbeatTypingCallbacks(params) {
  ...
  const keepaliveIntervalMs =
    typeof params.typingIntervalSeconds === "number" && params.typingIntervalSeconds > 0
      ? params.typingIntervalSeconds * 1e3
      : DEFAULT_HEARTBEAT_TYPING_INTERVAL_SECONDS * 1e3;
  const target = {
    cfg: params.cfg,
    to,
...
function resolveHeartbeatSession(cfg, agentId, heartbeat, forcedSessionKey) {
  ...
  if (canonical !== "global" && !isSubagentSessionKey(canonical)) {
    if (resolveAgentIdFromSessionKey(canonical) === normalizeAgentId(resolvedAgentId))
      return {
...
function normalizeHeartbeatReply(payload, responsePrefix, ackMaxChars) {
  const stripped = stripHeartbeatToken(
    stripLeadingHeartbeatResponsePrefix(
...
function resolveDueHeartbeatTasks(preflight, startedAt) {
  ...
  return tasks.filter((task) =>
    isTaskDue(preflight.session.entry?.heartbeatTaskState?.[task.name], task.interval, startedAt),
  );
}
...
async function runHeartbeatOnce(opts) {
  ...
  const forcedSessionAgentId =
    explicitAgentId.length > 0 ? void 0 : parseAgentSessionKey(opts.sessionKey)?.agentId;
  ...
  if (!resolveHeartbeatIntervalMs(cfg, void 0, heartbeat))
    return {
      status: "skipped",
      reason: "disabled",
    };
  ...
  if (
    ...
    startedAt - recentSessionEntry.updatedAt < 3e4
  )
    return {
      status: "skipped",
      reason: HEARTBEAT_SKIP_REQUESTS_IN_FLIGHT,
  ...
  const ctx = {
    ...
    To: sender,
    OriginatingChannel:
      !suppressOriginatingContext && delivery.channel !== "none" ? delivery.channel : void 0,
  ...
  const heartbeatChannelPlugin =
    delivery.channel !== "none" ? resolveHeartbeatChannelPlugin(delivery.channel) : void 0;
  const heartbeatTyping =
    ...
    isHeartbeatTypingEnabled({
      ...
      ? createHeartbeatTypingCallbacks({
          ...
          ...(heartbeatTypingIntervalSeconds !== void 0
  ...
  const maybeSendHeartbeatOk = async () => {
    ...
    if (heartbeatPlugin?.heartbeat?.checkReady) {
      if (
        !(
          await heartbeatPlugin.heartbeat.checkReady({
            cfg,
            accountId: delivery.accountId,
            deps: opts.deps,
          })
  ...
  try {
    ...
    const timeoutOverrideSeconds =
      typeof heartbeat?.timeoutSeconds === "number" ? heartbeat.timeoutSeconds : void 0;
    ...
    const prevHeartbeatAt =
      typeof entry?.lastHeartbeatSentAt === "number" ? entry.lastHeartbeatSentAt : void 0;
    if (
      !shouldSkipMain &&
      !mediaUrls.length &&
      ...
      startedAt - prevHeartbeatAt < 1440 * 60 * 1e3
    ) {
      await restoreHeartbeatUpdatedAt({
        storePath,
        sessionKey,
...
function startHeartbeatRunner(opts) {
  ...
  const resolveNextDue = (now, intervalMs, phaseMs, prevState) =>
  ...
  const advanceAgentSchedule = (agent, now, reason) => {
    agent.nextDueMs = seekActiveSlotForAgent(
      agent,
      reason === "interval"
  ...
  const run = async (params) => {
    ...
    const resolveRequestedHeartbeat = (heartbeat) =>
      requestedHeartbeat
        ? {
            ...heartbeat,
            ...requestedHeartbeat,
