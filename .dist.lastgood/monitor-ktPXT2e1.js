...
import {
  a as resolveMatrixAccountConfig,
  i as resolveMatrixAccountAllowlistConfig,
} from "./account-config-dncftWpr.js";
import { n as resolveConfiguredMatrixBotUserIds } from "./accounts-Fm6UOeFA.js";
import { n as setActiveMatrixClient } from "./active-client-Dm6TyGz4.js";
import {
...
import { r as mergeDmAllowFromSources } from "./allow-from-REahseRU.js";
import {
  n as normalizeMatrixUserId,
  r as resolveMatrixAllowListMatch,
  t as normalizeMatrixAllowList,
} from "./allowlist-BMsQK3S3.js";
import { n as formatAllowlistMatchMeta } from "./allowlist-match-BzbxuWAy.js";
import { t as CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "./approval-handler-adapter-runtime-CEmfb1ag.js";
import { t as createAsyncLock } from "./async-lock-CzABRu7R.js";
import "./text-runtime-lKuAtsoz.js";
import { a as waitUntilAbort } from "./channel-lifecycle.core-8qluSzTD.js";
...
import {
  o as MATRIX_OPENCLAW_FINALIZED_PREVIEW_KEY,
  r as promoteMatrixDirectRoomCandidate,
} from "./direct-management-FqagZ_db.js";
import {
  o as readJoinedMatrixMembers,
  r as isStrictDirectMembership,
  t as hasDirectMatrixMemberFlag,
} from "./direct-room-DNGwJuvi.js";
import { n as hasFinalChannelTurnDispatch } from "./dispatch-result-Cyoa53q0.js";
import { l as resolvePinnedMainDmOwnerFromAllowlist } from "./dm-policy-shared-COpbJNwP.js";
import { t as formatMatrixEncryptedEventDisabledWarning } from "./encryption-guidance-DZjNpv05.js";
import { r as isMatrixNotFoundError, t as formatMatrixErrorMessage } from "./errors-DQ-r4gaL.js";
import {
...
import {
  i as isMatrixMediaSizeLimitError,
  r as MatrixMediaSizeLimitError,
} from "./http-client-BMhj43-k.js";
...
import "./string-coerce-runtime-Xx8DhtgQ.js";
import {
...
import { n as LogService } from "./logger-DwhV1i8m.js";
import { n as logInboundDrop, r as logTypingFailure } from "./logging-ChWKeerl.js";
import {
  a as resolveMatrixMessageBody,
  i as resolveMatrixMessageAttachment,
  n as formatMatrixMediaUnavailableText,
  r as formatMatrixMessageText,
  s as fetchMatrixPollSnapshot,
  t as formatMatrixMediaTooLargeText,
} from "./media-text-DmHp7WUL.js";
import { n as createReplyPrefixOptions } from "./reply-prefix-B_htGII3.js";
import { t as resolveMatrixTargets } from "./resolve-targets-Dj8dFSel.js";
import {
  a as EventType,
  i as resolveMatrixThreadRouting,
  n as resolveMatrixReplyToEventId,
  o as RelationType,
  r as resolveMatrixThreadRootId,
  t as resolveMatrixInboundRoute,
} from "./route-CnYyc2RW.js";
import { t as isBunRuntime } from "./runtime-BKy1ze1L.js";
import { t as getMatrixRuntime } from "./runtime-Brjuvs-E.js";
import {
...
import {
  a as sendMessageMatrix,
  g as isPollStartType,
  h as isPollEventType,
  p as formatPollAsText,
  t as chunkMatrixText,
  v as parsePollStartContent,
} from "./send-8C77gzLp.js";
import {
  n as resolveMatrixRoomConfig,
  t as resolveMatrixStoredSessionMeta,
} from "./session-store-metadata-B76AF4kL.js";
import {
  i as resolveSharedMatrixClient,
  n as releaseSharedClientInstance,
} from "./shared-Dm7O4FiT.js";
import {
  i as throwIfMatrixStartupAborted,
  r as isMatrixStartupAbortError,
} from "./startup-abort-gz1WDgpo.js";
import { i as resolveMatrixStateFilePath } from "./storage-B9keKclB.js";
...
import {
  n as isMatrixReadySyncState,
  r as isMatrixTerminalSyncState,
  t as isMatrixDisconnectedSyncState,
} from "./sync-state-irE7fl8K.js";
import { t as isMatrixQualifiedUserId } from "./target-ids-t9MhLKbB.js";
import "./client-BZL_DUpm.js";
import "./runtime-api-BxUd6s1_.js";
...
import { b as escapeRegExp } from "./utils-BGRcpLKt.js";
...
async function resolveMatrixMonitorLiveUserAllowlist(params) {
  ...
  const startupByInput = new Map(
    (params.startupResolvedEntries ?? []).map((entry) => [entry.input, entry.id]),
  );
  const pending = [];
  ...
  const canonicalized = canonicalizeAllowlistWithResolvedIds({
    existing: pending,
    resolvedMap: (
      await resolveMatrixMonitorUserEntries({
        cfg: params.cfg,
        accountId: params.accountId,
        entries: pending,
        runtime: params.runtime,
...
async function resolveMatrixMonitorRoomsConfig(params) {
  ...
  return sanitizeMatrixRoomUserAllowlists(
...
function isMatrixVerificationRoomMessage(content) {
  return (
    ...
    (trimMaybeString$1(content.msgtype) === "m.notice" &&
...
async function loadMatrixDirectRoomDeps() {
  matrixDirectRoomDepsPromise ??= Promise.all([
    import("./direct-management-BwmtqISh.js"),
    import("./direct-room-DrNHzPDH.js"),
...
function formatVerificationSasNotice(summary) {
  ...
  const emojiLine =
    Array.isArray(sas.emoji) && sas.emoji.length > 0
      ? `SAS emoji: ${sas.emoji.map(([emoji, name]) => `${trimMaybeString(emoji) ?? "?"} ${trimMaybeString(name) ?? "?"}`).join(" | ")}`
      : null;
  const decimalLine =
...
async function resolveVerificationSummaryForSignal(client, params) {
  ...
  const activeRoomId = trimMaybeString(
    (
      await inspectMatrixDirectRooms({
        client,
...
function createMatrixVerificationEventRouter(params) {
  ...
  async function routeVerificationSummary(summary) {
    ...
    if (
      !trackBounded(
        routedVerificationSasFingerprints,
        `${summary.id}:${JSON.stringify(summary.sas)}`,
      )
    )
      return;
  ...
  function routeVerificationEvent(roomId, event) {
    ...
    const routeTask = async () => {
      ...
      const sourceFingerprint =
        trimMaybeString(event?.event_id) ?? `${senderId}:${event.type}:${flowId ?? "none"}`;
      if (
        !(await (
          await loadMatrixDirectRoomDeps()
        ).isStrictDirectRoom({
      ...
      if (stageNotice) {
        if (
          trackBounded(
            routedVerificationStageNotices,
            `${roomId}:${senderId}:${flowId ?? sourceFingerprint}:${signal.stage}`,
          )
        )
...
function createMatrixPostHealthySyncDecryptFailureTracker(params) {
  ...
  return {
    recordFailure(roomId, event, error) {
...
function registerMatrixMonitorEvents(params) {
  ...
  client.on("room.encrypted_event", (roomId, event) => {
    ...
    logVerboseMessage(
      `matrix: encrypted event room=${roomId} type=${event?.type ?? "unknown"} id=${eventId}`,
    );
  });
  ...
  client.on("room.event", (roomId, event) => {
    ...
    if (eventType === EventType.RoomMember) {
      ...
      logVerboseMessage(
        `matrix: member event room=${roomId} stateKey=${event.state_key ?? ""} membership=${membership ?? "unknown"}`,
      );
    }
...
async function fetchMatrixMediaBuffer(params) {
  try {
    return {
      buffer: await params.client.downloadContent(params.mxcUrl, {
...
function stripNativeMatrixMentionPrefix(text, candidate) {
  return stripMatchedMatrixMentionPrefix(
    text,
    new RegExp(`^\\s*${escapeRegExp(candidate)}(?:\\s*[:,])?(?:\\s+|$)`, "i"),
  );
}
function stripRegexMatrixMentionPrefix(text, pattern) {
  ...
  return stripMatchedMatrixMentionPrefix(
    text,
    new RegExp(`^\\s*(?:${pattern.source})(?:\\s*[:,])?(?:\\s+|$)`, flags),
  );
}
...
function checkFormattedBodyMention(params) {
  ...
  for (const match of params.formattedBody.matchAll(
    ...
    try {
      ...
      if (decodeURIComponent(parsed.hash.replace(/^#\/?/, "").trim()) !== params.userId.trim())
        continue;
      if (
...
function createMatrixReplyContextResolver(params) {
  ...
  return async (input) => {
    ...
    return remember(cacheKey, {
      ...
      replyToSender:
        (senderId &&
          (await params.getMemberDisplayName(input.roomId, senderId).catch(() => void 0))) ??
        senderId,
...
function createRoomHistoryTrackerInternal(
  ...
  function clearRoomWatermarks(roomId) {
    ...
    for (const key of agentWatermarks.keys())
      if (key.endsWith(roomSuffix)) agentWatermarks.delete(key);
  }
...
function createRoomHistoryTracker(
  maxQueueSize = DEFAULT_MAX_QUEUE_SIZE,
  maxRoomQueues = DEFAULT_MAX_ROOM_QUEUES,
  maxWatermarkEntries = MAX_WATERMARK_ENTRIES,
  maxPreparedTriggerEntries = MAX_PREPARED_TRIGGER_ENTRIES,
...
function createMatrixThreadContextResolver(params) {
  ...
  return async (input) => {
    ...
    const senderId = trimMatrixMaybeString(rawEvent.sender);
    const senderName =
      senderId && (await params.getMemberDisplayName(input.roomId, senderId).catch(() => void 0));
...
function loadMatrixSendModule() {
  matrixSendModulePromise ??= import("./send-WjVc4kCy.js");
...
function loadMatrixReactionEvents() {
  matrixReactionEventsPromise ??= import("./reaction-events-CfWyCoCv.js");
  return matrixReactionEventsPromise;
}
function loadMatrixDraftStream() {
  matrixDraftStreamPromise ??= import("./draft-stream-0x7hMdSF.js");
...
function resolveMatrixPendingHistoryText(params) {
  ...
  const filename =
    typeof params.content.filename === "string" ? params.content.filename.trim() : void 0;
    ...
    try {
      ...
      const continueIngress = async (params) => {
        ...
        if (isRoom && roomUserMatch && !roomUserMatch.allowed) {
          logVerboseMessage(
            `matrix: blocked sender ${senderId} (room users allowlist, ${roomMatchMeta}, ${formatAllowlistMatchMeta(roomUserMatch)})`,
          );
          await commitInboundEventIfClaimed();
          return;
        }
        if (
          ...
          logVerboseMessage(
            `matrix: blocked sender ${senderId} (groupAllowFrom, ${roomMatchMeta}, ${formatAllowlistMatchMeta(groupAllowMatch)})`,
          );
          await commitInboundEventIfClaimed();
          return;
        }
        ...
        const contentUrl =
          "url" in content && typeof content.url === "string" ? content.url : void 0;
        const contentFile =
        ...
        const selfDisplayName = content.formatted_body
          ? await getMemberDisplayName(roomId, selfUserId).catch(() => void 0)
          : void 0;
      ...
      const shouldIncludeRoomContextSender = (kind, contextSenderId) =>
      ...
      logVerboseMessage(
        `matrix inbound: room=${roomId} from=${senderId} preview="${bodyText.slice(0, 200).replace(/\n/g, "\\n")}"`,
      );
      ...
      const pushPreviewToolProgress = async (line, options) => {
        ...
        if (shouldStreamPreviewToolProgress && !previewToolProgressSuppressed && normalized) {
          if (previewToolProgressLines.at(-1) !== normalized)
            previewToolProgressLines = [...previewToolProgressLines, normalized].slice(
              -resolveChannelProgressDraftMaxLines(progressConfigEntry),
            );
        }
      ...
      const queueDraftBlockBoundary = (payload, context) => {
        ...
        const nextDraftBoundaryOffset =
          (latestQueuedDraftBoundaryOffsets.get(messageGeneration) ?? 0) + payloadTextLength;
  ...
  const getRoomAliases = async (roomId) => {
    ...
    try {
      ...
      if (Array.isArray(rawAliases))
        altAliases = rawAliases.filter((entry) => typeof entry === "string");
    } catch (err) {
...
async function loadMatrixStartupMaintenanceDeps() {
  matrixStartupMaintenanceDepsPromise ??= Promise.all([
    import("./config-update-CNZS5JNh.js"),
    import("./device-health-2TDkifq8.js"),
    import("./profile-Cy3xmVxQ.js"),
    import("./legacy-crypto-restore-C9YiMYHp.js"),
    import("./startup-verification-jDXxXO3Y.js"),
  ]).then(
    ([
      configUpdateModule,
      deviceHealthModule,
...
function resolveMatrixStreamingMode(streaming) {
  ...
  if (isMatrixStreamingConfig(streaming)) {
    if (streaming.mode === "partial" || streaming.mode === "quiet" || streaming.mode === "progress")
