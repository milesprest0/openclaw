...
import { n as normalizeDeviceAuthScopes } from "./device-auth-BTSFehem.js";
import {
  a as resolveBootstrapProfileScopesForRoles,
  i as resolveBootstrapProfileScopesForRole,
} from "./device-bootstrap-profile-BPv-hU36.js";
import { d as writeJson, i as readJsonIfExists } from "./json-files-DifBk3kt.js";
import { t as rejectPendingPairingRequest } from "./pairing-pending-CkavCB86.js";
import {
  a as reconcilePendingPairingRequests,
  c as resolveScopeOutsideRequestedRoles,
  i as pruneExpiredPending,
  l as roleScopesAllow,
  n as verifyPairingToken,
  o as resolvePairingPaths,
  r as coercePairingStateRecord,
  s as resolveMissingRequestedScope,
  t as generatePairingToken,
} from "./pairing-token-BUAmIw7n.js";
...
async function approveDevicePairing(requestId, optionsOrBaseDir, maybeBaseDir) {
  const options =
    typeof optionsOrBaseDir === "string" || optionsOrBaseDir === void 0 ? void 0 : optionsOrBaseDir;
...
async function approveBootstrapDevicePairing(requestId, bootstrapProfile, baseDir) {
  ...
  return await withLock(async () => {
    ...
    const pending = state.pendingById[requestId];
    if (!pending) return null;
    const missingRole = resolveRequestedRoles(pending).find(
    ...
    const missingScope = resolveMissingRequestedScope({
      role: OPERATOR_ROLE,
      requestedScopes: normalizeDeviceAuthScopes(pending.scopes).filter((scope) =>
    ...
    const sanitizedApprovedScopes = resolveBootstrapProfileScopesForRoles(
      approvedRoles,
      mergeScopes(existing?.approvedScopes ?? existing?.scopes, pending.scopes, approvedScopes) ??
        [],
    );
    const tokens = existing?.tokens ? { ...existing.tokens } : {};
...
async function removePairedDevice(deviceId, baseDir) {
  return await withLock(async () => {
    ...
    for (const [requestId, pending] of Object.entries(state.pendingById))
      if (pending.deviceId === normalized) delete state.pendingById[requestId];
    await persistState(state, baseDir, "both");
...
//#endregion
export {
  updatePairedDeviceMetadata as _,
  getPairedDevice as a,
  listApprovedPairedDeviceRoles as c,
  rejectDevicePairing as d,
  removePairedDevice as f,
  summarizeDeviceTokens as g,
  rotateDeviceToken as h,
  formatDevicePairingForbiddenMessage as i,
  listDevicePairing as l,
  revokeDeviceToken as m,
  approveDevicePairing as n,
  getPendingDevicePairing as o,
  requestDevicePairing as p,
  ensureDeviceToken as r,
  hasEffectivePairedDeviceRole as s,
  approveBootstrapDevicePairing as t,
  listEffectivePairedDeviceRoles as u,
  verifyDeviceToken as v,
};
