import {
  n as isGemma4ModelId,
  t as sanitizeGoogleAssistantFirstOrdering,
} from "./google-turn-ordering-Dg_Stek2.js";
import { t as createLazyImportLoader } from "./lazy-promise-B1nDYoKn.js";
import "./gpt5-prompt-overlay-DpzD1H_Q.js";
import "./provider-attribution-SP5uub3O.js";
import { a as normalizeModelCompat } from "./provider-model-compat-DIuSGYN-.js";
import {
...
function shouldPreserveThinkingBlocks(modelId) {
  ...
  if (/(?:opus|sonnet|haiku)-(?:[5-9]|\d{2})(?![0-9])/.test(id)) return true;
  if (/(?:opus|sonnet|haiku)-latest/.test(id)) return true;
...
function buildAnthropicReplayPolicyForModel(modelId) {
  return buildStrictAnthropicReplayPolicy({
    dropThinkingBlocks:
      normalizeLowercaseStringOrEmpty(modelId).includes("claude") &&
      !shouldPreserveThinkingBlocks(modelId),
  });
}
...
function createMoonshotThinkingWrapper(baseStreamFn, thinkingType, thinkingKeep) {
  return async (model, context, options) => {
    return streamWithPayloadPatch(
      baseStreamFn ?? (await loadDefaultStreamFn()),
      model,
      context,
      options,
      (payloadObj) => {
...
const CLAUDE_MAX_THINKING_DEFAULT_MODEL_PREFIXES = ["claude-sonnet-5"];
...
function isClaudeMaxThinkingDefaultModelId(modelId) {
...
function buildProviderReplayFamilyHooks(options) {
  switch (options.family) {
    ...
    case "anthropic-by-model":
      return { buildReplayPolicy: ({ modelId }) => buildAnthropicReplayPolicyForModel(modelId) };
...
//#endregion
export {
  buildPassthroughGeminiSanitizingReplayPolicy as C,
  shouldPreserveThinkingBlocks as D,
  sanitizeGoogleGeminiReplayHistory as E,
  buildOpenAICompatibleReplayPolicy as S,
  resolveTaggedReasoningOutputMode as T,
  streamWithPayloadPatch as _,
  buildProviderReplayFamilyHooks as a,
  buildHybridAnthropicOrOpenAIReplayPolicy as b,
  isClaudeMaxThinkingDefaultModelId as c,
  resolveClaudeThinkingProfile as d,
  cloneFirstTemplateModel as f,
  resolveMoonshotThinkingType as g,
  resolveMoonshotThinkingKeep as h,
  PASSTHROUGH_GEMINI_REPLAY_HOOKS as i,
  isClaudeOpus47ModelId as l,
  createMoonshotThinkingWrapper as m,
  NATIVE_ANTHROPIC_REPLAY_HOOKS as n,
  getModelProviderHint as o,
  matchesExactOrPrefix as p,
  OPENAI_COMPATIBLE_REPLAY_HOOKS as r,
  isClaudeAdaptiveThinkingDefaultModelId as s,
  ANTHROPIC_BY_MODEL_REPLAY_HOOKS as t,
  isProxyReasoningUnsupportedModelHint as u,
  buildAnthropicReplayPolicyForModel as v,
  buildStrictAnthropicReplayPolicy as w,
  buildNativeAnthropicReplayPolicyForModel as x,
  buildGoogleGeminiReplayPolicy as y,
};
