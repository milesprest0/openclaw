...
import { v as resolveSafeInstallDir } from "./channel-catalog-registry-uu7_tUkL.js";
import {
  h as searchClawHubSkills,
  r as downloadClawHubSkillArchive,
  s as fetchClawHubSkillDetail,
  u as resolveClawHubBaseUrl,
...
async function resolveRequestedUpdateSlug(params) {
  ...
  if (
    (await readClawHubSkillOrigin(resolveSkillInstallDir(params.workspaceDir, trackedSlug))) ||
    params.lock.skills[trackedSlug]
  )
    return trackedSlug;
...
async function ensureSkillRoot(rootDir) {
  for (const candidate of ["SKILL.md", "skill.md", "skills.md", "SKILL.MD"])
    if (await pathExists(path.join(rootDir, candidate))) return;
...
async function writeClawHubSkillOrigin(skillDir, origin) {
  await writeJson(path.join(skillDir, SKILL_ORIGIN_RELATIVE_PATH), origin, {
    trailingNewline: true,
  });
}
...
async function resolveTrackedUpdateTarget(params) {
  const origin =
    (await readClawHubSkillOrigin(resolveSkillInstallDir(params.workspaceDir, params.slug))) ??
    null;
...
//#endregion
export {
  updateSkillsFromClawHub as i,
  readTrackedClawHubSkillSlugs as n,
  searchSkillsFromClawHub as r,
  installSkillFromClawHub as t,
};
