...
import { t as resolveChannelInboundAttachmentRoots } from "./channel-inbound-roots-Drc2Rpkx.js";
import { n as normalizeMediaProviderId } from "./config-provider-models-DWlpS7W-.js";
import {
  d as getMediaUnderstandingProvider,
  u as buildMediaUnderstandingRegistry,
} from "./defaults.constants-u25YkDfl.js";
import { i as runExec } from "./exec-Csn_09g3.js";
import { n as MediaFetchError, r as fetchRemoteMedia } from "./fetch-CIuHGNdE.js";
...
import {
  r as mergeInboundPathRoots,
  t as isInboundPathAllowed,
} from "./inbound-path-policy-UV1fIghZ.js";
import "./local-file-access-KUGx5XDy.js";
import { a as getDefaultMediaLocalRoots } from "./local-roots-CCSkqO59.js";
import { a as logWarn } from "./logger-BWdMCYqK.js";
import {
  c as kindFromMime,
  i as getFileExtension,
  n as detectMime,
  o as isAudioFileName,
...
import { n as findNormalizedProviderValue } from "./provider-id-CG9pXYPs.js";
import { t as providerSupportsCapability } from "./provider-supports-5F7_jCPP.js";
import { i as resolveModelEntries, o as resolveScopeDecision } from "./resolve-DB4enOPl.js";
import "./temp-path-DRiyUI5S.js";
import {
  _ as fileExists,
  a as runCliEntry,
  g as extractGeminiResponse,
  o as runProviderEntry,
  r as formatDecisionSummary,
  t as buildModelDecision,
  v as MediaUnderstandingSkipError,
  y as isMediaUnderstandingSkipError,
} from "./runner.entries-BWDlqt3w.js";
import "./shared-DqfWqGL5.js";
...
import { t as buildRandomTempFilePath } from "./temp-download-CgMpGD8s.js";
...
function isAttachmentRecord(value) {
  ...
  if (entry.alreadyTranscribed !== void 0 && typeof entry.alreadyTranscribed !== "boolean")
    return false;
  return true;
}
function selectAttachments(params) {
  const { capability, attachments, policy } = params;
  const matches = (Array.isArray(attachments) ? attachments.filter(isAttachmentRecord) : []).filter(
    (item) => {
  ...
  async getBuffer(params) {
    ...
    if (entry.resolvedPath)
      try {
        ...
        if (entry.resolvedPath) {
          if (size !== void 0 && size > params.maxBytes)
            throw new MediaUnderstandingSkipError(
              "maxBytes",
              `Attachment ${params.attachmentIndex + 1} exceeds maxBytes ${params.maxBytes}`,
            );
...
async function loadModelCatalogApi() {
  cachedModelCatalogApi ??= await import("./model-catalog-iNiq3Mxw.js");
...
async function hasProviderAuthAvailable(params) {
  ...
  cachedHasAvailableAuthForProvider ??= (await import("./model-auth-w3SveVAp.js"))
...
function resolveConfiguredImageModel(params) {
  return findNormalizedProviderValue(params.cfg.models?.providers, params.providerId)?.models?.find(
    (entry) => {
...
function resolveCatalogImageModelId(params) {
  ...
  return normalizeOptionalString(
    (matches.find((entry) => normalizeLowercaseStringOrEmpty(entry.id) === "auto") ?? matches[0])
      ?.id,
  );
}
function resolveDefaultMediaModelFromRegistry(params) {
  return normalizeOptionalString(
    params.providerRegistry.get(normalizeMediaProviderId(params.providerId))?.defaultModels?.[
      params.capability
    ],
  );
}
...
async function resolveAutoImageModelId(params) {
  ...
  if (explicit) {
    if (
      (await explicitImageModelVisionStatus({
        cfg: params.cfg,
  ...
  const { resolveDefaultMediaModel } = await import("./defaults-DaV-V90B.js");
...
function resolveMediaAttachmentLocalRoots(params) {
  ...
  return mergeInboundPathRoots(
    ...
    workspaceDir ? [path.resolve(workspaceDir)] : void 0,
...
async function resolveLocalWhisperCppEntry() {
  ...
  const modelPath =
    envModel && (await fileExists(envModel))
      ? envModel
...
async function resolveAutoImageModel(params) {
  ...
  const resolvedActive = toActive(
    await resolveActiveModelEntry({
      cfg: params.cfg,
...
export {
  runCapability as a,
  MediaAttachmentCache as c,
  resolveAttachmentKind as d,
  resolveMediaAttachmentLocalRoots as i,
  isAudioAttachment as l,
  clearMediaUnderstandingBinaryCacheForTests as n,
  createMediaAttachmentCache as o,
  resolveAutoImageModel as r,
  normalizeMediaAttachments as s,
  buildProviderRegistry as t,
  normalizeAttachments as u,
};
