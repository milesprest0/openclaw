...
import { t as formatResolvedUnresolvedNote } from "./setup-9P7OQJwo.js";
import { r as writeQrDataUrlToTempFile } from "./setup-core-BKAKPD2K.js";
import { l as patchScopedAccountConfig } from "./setup-helpers-BWYI0iQf.js";
import {
...
import {
  ...
  d as resolveZaloGroupsByEntries,
  l as resolveZaloAllowFromEntries,
  y as startZaloQrLogin,
} from "./zalo-js-DeQ75Hia.js";
...
async function promptZalouserAllowFrom(params) {
  ...
  while (true) {
    const parts = parseZalouserEntries(
      await prompter.text({
    ...
    const unique = mergeAllowFromEntries(
      existingAllowFrom,
      resolvedEntries.filter((item) => item.resolved && item.id).map((item) => item.id),
    );
...
const zalouserDmPolicy = {
  ...
  resolveConfigKeys: (cfg, accountId) =>
    (accountId ?? resolveDefaultZalouserAccountId(cfg)) !== "default"
      ? {
...
const zalouserSetupWizard = {
  ...
  status: {
    ...
    resolveStatusLines: async ({ cfg, accountId, configured }) => {
      return [
        `${accountId && accountId !== "default" ? `Zalo Personal (${accountId})` : "Zalo Personal"}: ${configured ? "logged in" : "needs QR login"}`,
      ];
    },
  },
  prepare: async ({ cfg, accountId, prompter, options }) => {
    ...
    if (!(await checkZcaAuthenticated(account.profile))) {
      ...
      if (start.qrDataUrl) {
        const qrPath = await writeQrDataUrlToTempFile(start.qrDataUrl, account.profile);
        await prompter.note(
          [start.message, qrPath ? `QR image saved to: ${qrPath}` : void 0]
            .filter(Boolean)
            .join("\n"),
          "QR Login",
        );
  ...
  groupAccess: {
    ...
    applyAllowlist: ({ cfg, accountId, resolved }) =>
      setZalouserGroupAllowlist(cfg, accountId, resolved),
  },
