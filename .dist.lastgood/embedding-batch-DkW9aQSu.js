...
import {
  ...
  T as formatUnavailableBatchError,
  _ as buildBatchHeaders,
  b as applyEmbeddingBatchOutputLine,
  d as uploadBatchJsonlFile,
  f as resolveBatchCompletionFromStatus,
  g as runEmbeddingBatchGroups,
  h as buildEmbeddingBatchGroupOptions,
  m as throwIfBatchTerminalFailure,
  p as resolveCompletedBatchResult,
  v as normalizeBatchBaseUrl,
  w as extractBatchErrorMessage,
  x as postJsonWithRetry,
  y as EMBEDDING_BATCH_ENDPOINT,
...
function resolveVoyageBatchDeps(overrides) {
  return {
    ...
    sleep:
      overrides?.sleep ?? (async (ms) => await new Promise((resolve) => setTimeout(resolve, ms))),
    ...
    return await params.deps.withRemoteHttpResponse(
      buildVoyageBatchRequest({
        ...
        onResponse: async (res) => {
          ...
          const text = await res.text();
          if (!text.trim()) return;
          return extractBatchErrorMessage(
            text
              .split("\n")
              .map((line) => line.trim())
              .filter(Boolean)
