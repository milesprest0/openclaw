...
import { t as resolveBrewExecutable } from "./brew-okgWb_rF.js";
import {
  _ as assertCanonicalPathWithinBase,
  y as safePathSegmentHashed,
} from "./channel-catalog-registry-uu7_tUkL.js";
import { n as hasBinary } from "./config-eval-p3cuOZWd.js";
import "./sandbox-paths-BVIaOwJU.js";
import { l as resolveSkillKey } from "./config-kFPSWjDy.js";
...
import { a as scanSkillInstallSource } from "./install-security-scan-BAybFOI9.js";
import { t as createLazyImportLoader } from "./lazy-promise-B1nDYoKn.js";
import { s as isWithinDir } from "./path-1liOXr_N.js";
import { o as root } from "./secure-temp-dir-CCj3cY2B.js";
import { t as resolveSkillsInstallPreferences } from "./skills-DYy402QP.js";
import { t as resolveSkillSource } from "./source-B-A_EHXC.js";
import { s as normalizeOptionalLowercaseString } from "./string-coerce-BdEutqX5.js";
import { d as resolveConfigDir, p as resolveUserPath, s as ensureDir } from "./utils-BGRcpLKt.js";
...
const extractModuleLoader = createLazyImportLoader(
  () => import("./skills-install-extract-C3AB_kp4.js"),
);
...
async function installDownloadSpec(params) {
  ...
  if (!(spec.extract ?? Boolean(archiveType)))
    return {
      ok: true,
...
function resolveBrewMissingFailure(spec) {
  ...
  return createInstallFailure({
    message: `brew not installed — ${process.platform === "linux" ? `Homebrew is not installed. Install it from https://brew.sh or install "${formula}" manually using your system package manager (e.g. apt, dnf, pacman).` : "Homebrew is not installed. Install it from https://brew.sh"}`,
  });
}
...
async function installGoViaApt(timeoutMs) {
  ...
  const sudoCheck = await runCommandSafely(["sudo", "-n", "true"], { timeoutMs: 5e3 });
  if (sudoCheck.code !== 0)
    return createInstallFailure({
      message:
...
async function installSkill(params) {
  ...
  if (!new Set(["openclaw-bundled", "openclaw-managed", "openclaw-extra"]).has(skillSource))
    warnings.push(
