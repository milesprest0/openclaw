...
import {
  ...
  t as getLoadedChannelPluginById,
...
function resolveProviderFromContext(ctx, cfg) {
  ...
  if (!explicitMessageChannel && explicitMessageChannels.includes("webchat"))
    return {
      providerId: void 0,
      hadResolutionError: false,
    };
  const direct =
    normalizeAnyChannelId(explicitMessageChannel ?? void 0) ??
    explicitMessageChannel ??
  ...
  for (const candidate of candidates) {
    ...
    if (normalizedCandidateChannel === "webchat")
      return {
        providerId: void 0,
        hadResolutionError: false,
      };
    const normalized =
      normalizeAnyChannelId(normalizedCandidateChannel ?? void 0) ??
      normalizedCandidateChannel ??
...
function probeInferredProviders(ctx, cfg) {
  ...
  return {
    candidates: listLoadedChannelPlugins()
      .map((plugin) => {
