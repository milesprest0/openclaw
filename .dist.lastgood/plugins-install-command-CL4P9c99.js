...
import { t as findBundledPluginSource } from "./bundled-sources-BqA2rgRL.js";
import { l as resolveDefaultPluginExtensionsDir } from "./channel-catalog-registry-uu7_tUkL.js";
import { o as collectChannelDoctorStaleConfigMutations } from "./channel-doctor-Cz_i63YQ.js";
...
import { s as tracePluginLifecyclePhaseAsync } from "./discovery-bnS95tO3.js";
...
import { n as installHooksFromPath, t as installHooksFromNpmSpec } from "./install-C6624gLY.js";
import "./config-CMOdcWpc.js";
import {
  a as installPluginFromNpmSpec,
  i as installPluginFromNpmPackArchive,
  o as installPluginFromPath,
  t as PLUGIN_INSTALL_ERROR_CODE,
} from "./install-Djc3uDTp.js";
import { u as readConfigFileSnapshot } from "./io-CEQSCTGy.js";
import {
  r as resolveMarketplaceInstallShortcut,
  t as installPluginFromMarketplace,
} from "./marketplace-Bq_M0rSR.js";
...
import {
  r as resolvePluginInstallRequestContext,
  t as resolvePluginInstallInvalidConfigPolicy,
} from "./plugin-install-config-policy-D-Web6qy.js";
import {
  a as resolveOfficialExternalNpmPackageTrust,
  i as resolveOfficialExternalInstallPlanBeforeNpm,
  r as resolveBundledInstallPlanForNpmFailure,
  t as resolveBundledInstallPlanBeforeNpm,
} from "./plugin-install-plan-CM7tdiQ8.js";
import {
  a as formatPluginInstallWithHookFallbackError,
  c as parseNpmPackPrefixPath,
  l as parseNpmPrefixSpec,
  n as createHookPackInstallLogger,
  r as createPluginInstallLogger,
} from "./plugins-command-helpers-D4actSTy.js";
import {
  i as resolvePinnedNpmInstallRecordForCli,
  n as persistPluginInstall,
  t as persistHookPackInstall,
...
async function tryInstallHookPackFromLocalPath(params) {
  if (params.link) {
    if (!fs.statSync(params.resolvedPath).isDirectory())
      return {
        ok: false,
...
async function loadConfigFromSnapshotForInstall(request, snapshot) {
  ...
  if (
    ...
    throw buildInvalidPluginInstallConfigError(
      `Config invalid outside the plugin recovery path for ${request.bundledPluginId ?? "the requested plugin"}; run \`openclaw doctor --fix\` before reinstalling it.`,
    );
...
async function runPluginInstallCommand(params) {
  ...
  if (npmPrefixSpec !== null) {
    ...
    if (
      !(
        await tryInstallPluginOrHookPackFromNpmSpec({
          snapshot,
          installMode,
          spec: npmPrefixSpec,
          pin: opts.pin,
  ...
  if (npmPackPath !== null) {
    ...
    if (
      !(
        await tryInstallPluginFromNpmPackArchive({
          snapshot,
          installMode,
  ...
  if (gitSpec) {
    if (
      !(
        await tryInstallPluginFromGitSpec({
          snapshot,
          installMode,
          spec: raw,
          safetyOverrides,
