...
import { t as PLUGIN_REGISTRY_STATE } from "./runtime-state-CQLzHhNR.js";
...
function clearPluginRunContext(params) {
  const normalizedNamespace =
    params.namespace !== void 0 ? normalizeNamespace(params.namespace) : void 0;
  const namespaceFilter =
    normalizedNamespace !== void 0 && normalizedNamespace !== "" ? normalizedNamespace : void 0;
  const state = getPluginHostRuntimeState();
  ...
  if (params.runId && !params.pluginId && namespaceFilter === void 0)
...
function deletePluginSessionSchedulerJob(params) {
  ...
  if (params.expectedGeneration !== void 0 && record.generation !== params.expectedGeneration)
    return;
...
function hasPluginSessionSchedulerJob(params) {
  const record = getPluginHostRuntimeState()
    .schedulerJobsByPlugin.get(params.pluginId)
    ?.get(params.jobId);
...
async function cleanupPreviousPluginHostRegistry(params) {
  const [{ getRuntimeConfig }, { cleanupReplacedPluginHostRegistry }] = await Promise.all([
    ...
    import("./host-hook-cleanup-C9uma_4c.js"),
  ]);
...
function setActivePluginRegistry(
  registry,
  cacheKey,
  runtimeSubagentMode = "default",
  workspaceDir,
...
function collectLoadedPluginIds(registry, ids) {
  ...
  for (const plugin of registry.plugins)
    if (plugin.status === "loaded" && plugin.format !== "bundle") ids.add(plugin.id);
}
/**
...
//#endregion
export {
  buildPluginAgentTurnPrepareContext as A,
  cleanupPluginSessionSchedulerJobs as C,
  makePluginSessionSchedulerJobKey as D,
  getPluginSessionSchedulerJobGeneration as E,
  isPluginJsonValue as M,
  withPluginHostCleanupTimeout as N,
  registerPluginSessionSchedulerJob as O,
  createEmptyPluginRegistry as P,
  isPluginRegistryRetired as S,
  getPluginRunContext as T,
  requireActivePluginHttpRouteRegistry as _,
  getActivePluginRegistry as a,
  resolveActivePluginHttpRouteRegistry as b,
  getActivePluginRegistryWorkspaceDir as c,
  pinActivePluginChannelRegistry as d,
  pinActivePluginHttpRouteRegistry as f,
  requireActivePluginChannelRegistry as g,
  releasePinnedPluginHttpRouteRegistry as h,
  getActivePluginHttpRouteRegistryVersion as i,
  normalizePluginHostHookId as j,
  setPluginRunContext as k,
  getActivePluginRuntimeSubagentMode as l,
  releasePinnedPluginChannelRegistry as m,
  getActivePluginChannelRegistryVersion as n,
  getActivePluginRegistryKey as o,
  recordImportedPluginId as p,
  getActivePluginHttpRouteRegistry as r,
  getActivePluginRegistryVersion as s,
  getActivePluginChannelRegistry as t,
  listImportedRuntimePluginIds as u,
  requireActivePluginRegistry as v,
  clearPluginRunContext as w,
  setActivePluginRegistry as x,
  resetPluginRuntimeStateForTest as y,
};
