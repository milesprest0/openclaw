...
import { t as readBooleanParam$1 } from "./boolean-param-BpYIf-x6.js";
import { t as resolveFirstBoundAccountId } from "./bound-account-read-CptA7I-q.js";
import { r as resolveOutboundChannelPlugin$1 } from "./channel-resolution-PPtZ0iMr.js";
import {
  n as resolveMessageChannelSelection,
  t as listConfiguredMessageChannels,
} from "./channel-selection-uDv4DTk3.js";
import {
  a as actionHasTarget,
  o as actionRequiresTarget,
  r as applyTargetToParams,
  s as hasPotentialPluginActionParam,
} from "./channel-target-BNd4ALAB.js";
...
import {
  ...
  x as resolveSnakeCaseParamKey,
} from "./common-BPZLgNoA.js";
import { t as resolveChannelAccountMediaMaxMb } from "./configured-max-bytes-B8j5nYjM.js";
import { f as throwIfAborted, p as parseReplyDirectives } from "./deliver-Dz9Dpt9U.js";
import { t as isTruthyEnvValue } from "./env-GyTZ5xFK.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import { b as basenameFromMediaSource } from "./fs-safe-CgBWiL92.js";
...
} from "./load-options-6LTmiW9v.js";
import { r as getAgentScopedMediaLocalRoots } from "./local-roots-CCSkqO59.js";
import { a as resolveChannelMessageToolMediaSourceParamKeys } from "./message-action-discovery-jVpVmmf8.js";
import {
  s as isDeliverableMessageChannel,
  u as normalizeMessageChannel,
} from "./message-channel-GRZIhFYD.js";
import { n as sendPoll, t as sendMessage } from "./message-Drg-ekN8.js";
import { r as extensionForMime } from "./mime-CSQ-Gv-M.js";
import { u as resolveStorePath } from "./paths-CfeECf6Z.js";
import {
  a as hasReplyPayloadContent,
...
import { t as resolveAgentScopedOutboundMediaAccess } from "./read-capability-BlYckV7v.js";
...
import { r as recordSessionMetaFromInbound } from "./store-BpWdoYPF.js";
import {
...
import {
  i as resolveChannelTarget,
  n as lookupDirectoryDisplay,
  t as formatTargetDisplay,
} from "./target-resolver-_EpoEgO9.js";
import { t as extractToolPayload } from "./tool-payload-CFuJ-o-I.js";
import { t as appendAssistantMessageToSessionTranscript } from "./transcript-B5N3gLUE.js";
import "./inbound.runtime-DhjY4jgl.js";
...
function hasPollCreationParams(params) {
  ...
  for (const [key, value] of Object.entries(params))
    if (isChannelPollCreationParamName(key) && hasExplicitUnknownPollValue(key, value)) return true;
  return false;
}
//#endregion
...
function isSlackThreadBindGuardEnabled(env = process.env) {
  const raw = env[SLACK_THREAD_BIND_GUARD_ENV];
  if (typeof raw !== "string" || !raw.trim()) return true;
  return isTruthyEnvValue(raw);
}
function resolveInboundTurnThreadId(toolContext) {
  const turnThreadContext = toolContext?.turnThreadContext;
  if (!turnThreadContext?.isInboundThreadedTurn) return {};
...
function isTopLevelOverride(raw) {
...
function resolveAndApplyOutboundThreadId(actionParams, context) {
  ...
  if (Object.hasOwn(actionParams, "topLevel")) delete actionParams.topLevel;
  if (threadId) return threadId;
  if (topLevel) return;
  const guardEnabled =
    context.action === "send" &&
    normalizeLowercaseStringOrEmpty(context.channel) === "slack" &&
    isSlackThreadBindGuardEnabled();
...
function applyCrossContextDecoration(params) {
  if (params.preferPresentation && params.decoration.presentationBuilder)
    return {
  ...
  const pluginHandled = await tryHandleWithPluginAction({
    ...
    onHandled: async () => {
      ...
      const mirrorMediaUrls =
        ...
        params.mediaUrls ??
        (params.mediaUrl ? [params.mediaUrl] : void 0);
...
function inferPeerKindForAccountBinding(channel, target) {
  ...
  const candidates = [target, normalizeTargetForAccountBinding(channel, target)].filter((value) =>
    Boolean(value),
  );
  ...
  if (channelIdRaw) {
    const resolved = await resolveResolvedTargetOrThrow({
      ...
      validateResolvedTarget: (target) =>
        target.kind === "user" ? `Channel id "${channelIdRaw}" resolved to a user target.` : void 0,
    });
...
async function runGatewayPluginMessageActionOrNull(params) {
  ...
  if (!plugin?.actions?.handleAction) return null;
  if ((plugin.actions.resolveExecutionMode?.({ action: params.action }) ?? "local") !== "gateway")
    return null;
...
async function handleBroadcastAction(input, params) {
  ...
  if (!(input.cfg.tools?.message?.broadcast?.enabled !== false))
...
async function handleSendAction(ctx) {
  ...
  const mirrorMediaUrls =
    mergedMediaUrls.length > 0 ? mergedMediaUrls : mediaUrl ? [mediaUrl] : void 0;
...
async function handlePluginAction(ctx) {
  ...
  if (
    !resolveOutboundChannelPlugin$1({
      channel,
      cfg,
...
//#endregion
export {
  POLL_CREATION_PARAM_DEFS as a,
  resolveOutboundSessionRoute as i,
  runMessageAction as n,
  SHARED_POLL_CREATION_PARAM_NAMES as o,
  ensureOutboundSessionEntry as r,
  dispatchChannelMessageAction as s,
  getToolResult as t,
};
