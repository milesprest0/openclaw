...
import { n as info, o as success } from "./globals-BdfwDi2E.js";
import { c as resolveComparableIdentity } from "./identity-CG3p7vp1.js";
...
import "./text-runtime-BMQLJDUy.js";
...
async function stringifyCreds(creds) {
  const { BufferJSON } = await import("./session.runtime-kSSNrIzT.js");
...
function enqueueCredsSave(authDir, saveCreds, onError) {
  const next = (credsSaveQueues.get(authDir) ?? Promise.resolve())
    .then(() => saveCreds())
    .catch((error) => {
      onError(error);
    })
...
async function shouldClearOnLogout(authDir, isLegacyAuthDir) {
  try {
    ...
    if (isLegacyAuthDir)
      return (await fs$1.readdir(authDir, { withFileTypes: true })).some((entry) => {
    ...
    return (
      (await fs$1.lstat(resolveWebCredsBackupPath(authDir)).catch(() => null))?.isFile() === true
    );
  } catch (error) {
...
async function classifyWebAuthDirOwnership(authDir) {
  ...
  const [baseRealPath, authDirRealPath] = await Promise.all([
    fs$1.realpath(whatsappAuthBase).catch(() => null),
    fs$1.realpath(resolvedAuthDir).catch(() => null),
  ]);
...
async function logoutWeb(params) {
  ...
  if ((await waitForWebAuthBarrier(resolvedAuthDir, "logoutWeb")) === "timed_out")
    runtime.log(
  ...
  if (params.isLegacyAuthDir) {
    ...
    if (ownership.kind === "owned")
      await fs$1.rm(ownership.authDir, {
        recursive: true,
        force: true,
      });
...
async function readWebSelfIdentityForDecision(authDir = resolveDefaultWebAuthDir(), fallback) {
  const resolvedAuthDir = resolveUserPath(authDir);
  if (
    (await waitForWebAuthBarrier(resolvedAuthDir, "readWebSelfIdentityForDecision")) === "timed_out"
  )
...
//#endregion
export {
  waitForCredsSaveQueueWithTimeout as C,
  waitForCredsSaveQueue as S,
  readWebSelfIdentityForDecision as _,
  getWebAuthAgeMs as a,
  webAuthExists as b,
  pickWebChannel as c,
  readWebAuthExistsForDecision as d,
  readWebAuthSnapshot as f,
  readWebSelfIdentity as g,
  readWebSelfId as h,
  formatWhatsAppWebAuthStatusState as i,
  readCredsJsonRaw as l,
  readWebAuthState as m,
  WHATSAPP_AUTH_UNSTABLE_CODE as n,
  logWebSelfId as o,
  readWebAuthSnapshotBestEffort as p,
  WhatsAppAuthUnstableError as r,
  logoutWeb as s,
  WA_WEB_AUTH_DIR as t,
  readWebAuthExistsBestEffort as u,
  resolveDefaultWebAuthDir as v,
  writeCredsJsonAtomically as w,
  enqueueCredsSave as x,
  restoreCredsFromBackupIfNeeded as y,
};
