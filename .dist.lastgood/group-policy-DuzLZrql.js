...
import { n as parseToolsBySenderTypedKey } from "./types.tools-BLCZAu8D.js";
...
function normalizeSenderKey(value, options = {}) {
  ...
  return normalizeLowercaseStringOrEmpty(
    options.stripLeadingAt && trimmed.startsWith("@") ? trimmed.slice(1) : trimmed,
  );
}
...
function resolveChannelGroups(cfg, channel, accountId) {
  ...
  return (
    resolveAccountEntry(channelConfig.accounts, normalizedAccountId)?.groups ?? channelConfig.groups
  );
}
function resolveChannelGroupPolicyMode(cfg, channel, accountId) {
  ...
  if (!channelConfig) return;
  return (
    resolveAccountEntry(channelConfig.accounts, normalizedAccountId)?.groupPolicy ??
...
//#endregion
export {
  resolveToolsBySender as i,
  resolveChannelGroupRequireMention as n,
  resolveChannelGroupToolsPolicy as r,
  resolveChannelGroupPolicy as t,
};
