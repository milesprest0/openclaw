...
import { t as matchesSkillFilter } from "./filter-Cm-bafwa.js";
import {
  a as resolveHeartbeatAckMaxChars,
  i as resolveCronPayloadOutcome,
  t as isHeartbeatOnlyResponse,
} from "./helpers-ByPLfJuD.js";
...
import {
  i as resolveAllowedModelRef,
  r as getModelRefStatus,
  t as isCliProvider,
} from "./model-selection-cli-DuYeikT-.js";
...
import { o as resolveSessionTranscriptPath } from "./paths-CfeECf6Z.js";
import { c as retireSessionMcpRuntime } from "./pi-bundle-mcp-runtime-_btntJtc.js";
import {
  n as createCronRunDiagnosticsFromError,
  r as mergeCronRunDiagnostics,
  t as createCronRunDiagnosticsFromAgentResult,
} from "./run-diagnostics-prIxGL_8.js";
import "./selection-BcCvjGlJ.js";
import {
  a as resolveCronChannelOutputPolicy,
  n as markCronSessionPreRun,
  r as persistCronSkillsSnapshotIfChanged,
  t as createPersistCronSessionEntry,
} from "./run-session-state-B_OTuf6J.js";
import { t as resolveCronSession } from "./session-BTvoPxV7.js";
import { c as normalizeAgentId, h as toAgentStoreSessionKey } from "./session-key-B4qUwRzq.js";
...
import { u as setSessionRuntimeModel } from "./types-C7lm-riv.js";
import { i as hasNonzeroUsage, r as deriveSessionTotalTokens } from "./usage-BOaLms3p.js";
...
function formatCronPayloadModelRejection(params) {
  ...
  if (error.startsWith("model not allowed:"))
    return `cron payload.model '${modelOverride}' rejected by agents.defaults.models allowlist: ${error.slice(18).trim()} is not in [${formatAllowedModelRefs({ cfg: params.cfg })}]`;
...
function extractCronAgentDefaultsOverride(agentConfigOverride) {
  ...
  return {
    ...
    definedOverrides: Object.fromEntries(
      Object.entries(agentOverrideRest).filter(([, value]) => value !== void 0),
    ),
  };
}
...
async function loadCliRunnerRuntime() {
  return await import("./cli-runner.runtime-CS0JnreQ.js");
}
async function loadUsageFormatRuntime() {
  return await import("./usage-format-DSJ_uHLh.js");
}
async function prepareCronRunContext(params) {
  ...
  const hookExternalContentSource =
    (input.job.payload.kind === "agentTurn" ? input.job.payload.externalContentSource : void 0) ??
    resolveHookExternalContentSource(baseSessionKey);
  ...
  let thinkLevel =
    normalizeThinkLevel(
      (input.job.payload.kind === "agentTurn" ? input.job.payload.thinking : void 0) ?? void 0,
    ) ?? hooksGmailThinking;
  if (!thinkLevel)
  ...
  const timeoutMs = resolveAgentTimeoutMs({
    cfg: cfgWithAgentDefaults,
    overrideSeconds:
      input.job.payload.kind === "agentTurn" ? input.job.payload.timeoutSeconds : void 0,
  });
  ...
  const isExternalHook =
    hookExternalContentSource !== void 0 || isExternalHookSession(baseSessionKey);
  ...
  const authProfileId =
    !Boolean(cronSession.sessionEntry.authProfileOverride?.trim()) &&
...
async function finalizeCronRun(params) {
  ...
  const contextTokens =
    ...
    (await loadCronContextRuntime()).lookupContextTokens(modelUsed, { allowAsyncLoad: false }) ??
