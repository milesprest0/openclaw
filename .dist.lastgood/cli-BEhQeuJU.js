...
import {
  i as resolveCreateSpaceConfig,
  r as hasCreateSpaceConfigInput,
} from "./create-BIbqna6z.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import { n as callGatewayFromCli } from "./gateway-rpc-DjTKEsJd.js";
import {
  i as exchangeGoogleMeetAuthCode,
  n as createGoogleMeetOAuthState,
  o as resolveGoogleMeetAccessToken,
  r as createGoogleMeetPkce,
  s as waitForGoogleMeetAuthCode,
  t as buildGoogleMeetAuthUrl,
} from "./oauth-CppdOhAj.js";
...
function writeDoctorStatus(status) {
  ...
  for (const session of sessions) {
    ...
    if (health?.lastCaptionText)
      writeStdoutLine(
        "last caption text: %s%s",
        health.lastCaptionSpeaker ? `${health.lastCaptionSpeaker}: ` : "",
        health.lastCaptionText,
      );
    writeStdoutLine("realtime transcript lines: %s", health?.realtimeTranscriptLines ?? 0);
    if (health?.lastRealtimeTranscriptText)
      writeStdoutLine(
        "last realtime transcript: %s%s",
        health.lastRealtimeTranscriptRole ? `${health.lastRealtimeTranscriptRole}: ` : "",
        health.lastRealtimeTranscriptText,
      );
...
function renderArtifactsMarkdown(result) {
  ...
  for (const entry of result.artifacts) {
    ...
    pushMarkdownLine(
      lines,
      `Transcript entries: ${entry.transcriptEntries.reduce((count, transcript) => count + transcript.entries.length, 0)}`,
    );
    ...
    if (warnings.length > 0) {
      ...
      for (const warning of warnings)
        pushMarkdownLine(
          lines,
          `- ${warning.resource ? `${warning.resource}: ` : ""}${warning.message}`,
        );
    }
      ...
      for (const transcriptEntry of transcriptEntries.entries) {
        const times =
          transcriptEntry.startTime || transcriptEntry.endTime
            ? ` (${formatMarkdownOptional(transcriptEntry.startTime)} -> ${formatMarkdownOptional(transcriptEntry.endTime)})`
            : "";
        pushMarkdownLine(
          lines,
          `- ${transcriptEntry.participant ? `${participantDisplayName(entry, transcriptEntry.participant)}: ` : ""}${transcriptEntry.text ?? ""}${times}`,
        );
      }
    }
...
function renderAttendanceMarkdown(result) {
  ...
  for (const row of result.attendance) {
    ...
    for (const session of row.sessions)
      pushMarkdownLine(
        lines,
        `- ${session.name}: ${formatMarkdownOptional(session.startTime)} -> ${formatMarkdownOptional(session.endTime)}`,
      );
  }
  return `${lines.join("\n")}\n`;
}
  ...
  for (const entry of result.artifacts) {
    ...
    for (const transcriptEntries of entry.transcriptEntries) {
      ...
      for (const transcriptEntry of transcriptEntries.entries)
        pushMarkdownLine(
          lines,
          `- ${transcriptEntry.participant ? participantDisplayName(entry, transcriptEntry.participant) : "unknown"}${transcriptEntry.startTime ? ` [${transcriptEntry.startTime}]` : ""}: ${transcriptEntry.text ?? ""}`,
        );
    }
...
function registerGoogleMeetCli(params) {
  ...
  root
    .command("auth")
    .description("Google Meet OAuth helpers")
    .command("login")
  ...
  root
    ...
    .action(async (options) => {
      ...
      if (!hasCreateOAuth(params.config, options)) {
        if (hasCreateSpaceConfigInput(options))
          throw new Error(
  ...
  root
    ...
    .action(async (options) => {
      const resolved = resolveArtifactTokenOptions(params.config, options);
      const token = await resolveGoogleMeetAccessToken(resolved);
      const meetingResult = resolved.conferenceRecord
        ? { meeting: resolved.meeting }
        : await resolveMeetingForToken({
            config: params.config,
            options,
  ...
  root
    ...
    .action(async (sessionId) => {
      ...
      if (!(await (await params.ensureRuntime()).leave(sessionId)).found)
  ...
  root
    ...
    .action(async (sessionId, message) => {
      const delegated = await callGoogleMeetGateway({
        callGateway,
