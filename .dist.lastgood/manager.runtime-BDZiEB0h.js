...
import { c as resolveDiscordAccountAllowFrom } from "./accounts-Ctry1Kkb.js";
...
import { Ft as ChannelType, o as ReadyListener, s as ResumedListener } from "./discord-Do-Uybag.js";
...
import { n as formatDiscordUserTag } from "./format-C7NoGYAq.js";
import { a as shouldLogVerbose, r as logVerbose } from "./globals-BdfwDi2E.js";
import "./speech-CBdKDAfK.js";
import { t as buildDiscordGroupSystemPrompt } from "./inbound-context-DbTGw6F7.js";
...
var DiscordRealtimeVoiceSession = class {
        ...
        return {
          text: await this.runAgentTurn({
            ...
            message: formatVoiceIngressPrompt(
              [question, responseStyle ? `Spoken style: ${responseStyle}` : void 0]
                .filter(Boolean)
                .join("\n\n"),
  ...
  async connect() {
    ...
    this.bridge = createRealtimeVoiceBridgeSession({
      ...
      onTranscript: (role, text, isFinal) => {
        if (!isFinal || role !== "user" || this.params.mode !== "talk-buffer") return;
      ...
      onError: (error) =>
        logger$2.warn(`discord voice: realtime error: ${formatErrorMessage(error)}`),
  ...
  sendInputAudioForTurn(turn, discordPcm48kStereo) {
    ...
    if (realtimePcm.length > 0) this.bridge.sendAudio(realtimePcm);
  }
  handleBargeIn() {
...
async function transcribeVoiceAudio(params) {
  return normalizeOptionalString(
    (
      await getDiscordRuntime().mediaUnderstanding.transcribeAudioFile({
...
async function synthesizeVoiceReplyAudio(params) {
  ...
  const speakText = sanitizeVoiceReplyTextForSpeech(
...
var DiscordVoiceManager$1 = class {
  ...
  async join(params) {
    ...
    logVoiceVerbose(
      `join: DAVE settings encryption=${daveEncryption === false ? "off" : "on"} tolerance=${decryptionFailureTolerance ?? "default"} connectTimeout=${connectReadyTimeoutMs}ms reconnectGrace=${reconnectGraceMs}ms`,
    );
    const voiceSdk = loadDiscordVoiceSdk();
    ...
    if (voiceMode !== "stt-tts") {
      entry.realtime = new DiscordRealtimeVoiceSession({
        ...
        runAgentTurn: ({ context, message, toolsAllow, userId }) =>
          this.runDiscordRealtimeAgentTurn({
            context,
            entry,
            message,
            toolsAllow,
  ...
  async handleSpeakingStart(entry, userId) {
    ...
    const realtime =
      entry.realtime && isDiscordRealtimeVoiceMode(voiceMode) ? entry.realtime : void 0;
    ...
    try {
      ...
      if (durationSeconds < (streamAborted ? 0.2 : 0.35)) {
        logVoiceVerbose(
