import { t as findMatrixAccountConfig } from "./account-config-dncftWpr.js";
import { n as normalizeAccountId } from "./account-id-BGKP_Par.js";
import "./setup-9P7OQJwo.js";
import "./secret-ref-runtime-C2VENGRw.js";
import { r as shouldStoreMatrixAccountAtTopLevel } from "./config-paths-CNjwUFIB.js";
...
function updateMatrixAccountConfig(cfg, accountId, patch) {
  ...
  const nextAccount = {
    ...(findMatrixAccountConfig(cfg, normalizedAccountId) ??
      (normalizedAccountId === "default" ? matrix : {})),
  };
  if (patch.name !== void 0)
    if (patch.name === null) delete nextAccount.name;
    else {
  ...
  if (patch.initialSyncLimit !== void 0)
    if (patch.initialSyncLimit === null) delete nextAccount.initialSyncLimit;
    else nextAccount.initialSyncLimit = Math.max(0, Math.floor(patch.initialSyncLimit));
  if (patch.encryption !== void 0)
    if (patch.encryption === null) delete nextAccount.encryption;
    else nextAccount.encryption = patch.encryption;
  if (patch.allowBots !== void 0)
    if (patch.allowBots === null) delete nextAccount.allowBots;
    else nextAccount.allowBots = patch.allowBots;
  if (patch.autoJoin !== void 0)
    if (patch.autoJoin === null) delete nextAccount.autoJoin;
    else nextAccount.autoJoin = patch.autoJoin;
  applyNullableArrayField(nextAccount, "autoJoinAllowlist", patch.autoJoinAllowlist);
  if (patch.dm !== void 0)
    if (patch.dm === null) delete nextAccount.dm;
    else
      nextAccount.dm = cloneMatrixDmConfig({
        ...nextAccount.dm,
        ...patch.dm,
      });
  if (patch.groupPolicy !== void 0)
    if (patch.groupPolicy === null) delete nextAccount.groupPolicy;
    else nextAccount.groupPolicy = patch.groupPolicy;
  applyNullableArrayField(nextAccount, "groupAllowFrom", patch.groupAllowFrom);
  if (patch.groups !== void 0)
    if (patch.groups === null) delete nextAccount.groups;
    else nextAccount.groups = cloneMatrixRoomMap(patch.groups);
  if (patch.rooms !== void 0)
    if (patch.rooms === null) delete nextAccount.rooms;
