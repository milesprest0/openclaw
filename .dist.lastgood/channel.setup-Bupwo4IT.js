...
import { n as resolveNormalizedAccountEntry } from "./account-lookup-28TEMylU.js";
import {
...
import { y as createAllowlistProviderRouteAllowlistWarningCollector } from "./channel-policy-y8E9wmaa.js";
import {
...
import {
  a as buildTelegramModelsListChannelData,
  c as buildTelegramModelsProviderChannelData,
  i as buildTelegramModelsAddProviderChannelData,
  n as buildTelegramCommandsListChannelData,
  r as buildTelegramModelBrowseChannelData,
  s as buildTelegramModelsMenuChannelData,
...
} from "./doctor-contract-CpoTdhA8.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import {
  a as hasTelegramBotEndpointApiRoot,
  n as resolveTelegramFetch,
  o as normalizeTelegramApiRoot,
  t as resolveTelegramApiBase,
...
function resolveTelegramChatLookupFetch(params) {
  ...
  return resolveTelegramFetch(proxyUrl ? makeProxyFetch(proxyUrl) : void 0, {
...
const telegramSetupDmPolicy = {
  ...
  resolveConfigKeys: (cfg, accountId) =>
    (accountId ?? resolveDefaultTelegramAccountId(cfg)) !== "default"
      ? {
  ...
  setPolicy: (cfg, policy, accountId) => {
    ...
    return accountId == null && resolvedAccountId !== "default"
...
const TelegramChannelConfigSchema = buildChannelConfigSchema(TelegramConfigSchema, {
  uiHints: {
    "": {
      label: "Telegram",
...
function collectTelegramApiRootWarnings(params) {
  if (params.hits.length === 0) return [];
  return [
    `- ${sanitizeForLog(params.hits[0]?.path ?? "channels.telegram.apiRoot")} points at a full Telegram bot endpoint; apiRoot must be the Bot API root only. This can make startup calls like deleteWebhook, deleteMyCommands, and setMyCommands fail with 404 even when direct curl commands work.`,
...
function scanTelegramSelectedQuoteToolProgressWarnings(cfg) {
  ...
  return listTelegramAccountIds(cfg).flatMap((accountId) => {
    ...
    if (
      ...
      !resolveChannelStreamingPreviewToolProgress(account)
    )
      return [];
    return [
      {
...
async function maybeRepairTelegramAllowFromUsernames(cfg) {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("./plugin-sdk/runtime-secret-resolution.js");
  ...
  for (const accountId of listTelegramAccountIds(resolvedConfig)) {
    ...
    if (inspected.tokenSource === "none" ? "" : (normalizeOptionalString(inspected.token) ?? ""))
...
function collectTelegramEmptyAllowlistExtraWarnings(params) {
  ...
  return params.channelName === "telegram" &&
    (account.groupPolicy ?? parent?.groupPolicy ?? void 0) === "allowlist"
...
//#endregion
export {
  telegramConfigAdapter as a,
  telegramSecurityAdapter as c,
  resolveTelegramChatLookupFetch as d,
  formatDuplicateTelegramTokenReason as i,
  fetchTelegramChatId as l,
  createTelegramPluginBase as n,
  telegramSetupWizard as o,
  findTelegramTokenOwnerAccountId as r,
  telegramSetupAdapter as s,
  telegramSetupPlugin as t,
  lookupTelegramChatId as u,
};
