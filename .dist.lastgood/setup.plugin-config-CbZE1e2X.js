import { n as getPath, r as setPathCreateStrict } from "./path-utils-zSv0Iydy.js";
//#region src/wizard/setup.plugin-config.ts
let pluginMetadataSnapshotModulePromise;
function loadPluginMetadataSnapshotModule() {
  pluginMetadataSnapshotModulePromise ??= import("./plugin-metadata-snapshot-CPptdjy-.js");
...
function discoverConfigurablePlugins(params) {
  ...
  for (const plugin of params.manifestPlugins) {
    ...
    for (const [key, hint] of Object.entries(plugin.configUiHints))
      if (!hint.advanced) promptableHints[key] = hint;
