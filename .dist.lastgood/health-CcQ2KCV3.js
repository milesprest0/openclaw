import {
  a as resolveChannelAccountConfigured,
  o as resolveChannelAccountEnabled,
  t as inspectChannelAccount,
} from "./account-inspection-Dxm3WRmV.js";
import { c as resolveDefaultAgentId } from "./agent-scope-config-CXZGyKMl.js";
import {
  a as resolvePreferredAccountId,
  t as buildChannelAccountBindings,
} from "./bindings-CnvFlBa9.js";
import { i as callGateway, r as buildGatewayConnectionDetails } from "./call-hIeQLRKk.js";
import {
...
import { n as info } from "./globals-BdfwDi2E.js";
import { t as formatHealthChannelLines } from "./health-format-rRmRwTQ0.js";
import "./config-CMOdcWpc.js";
import { t as styleHealthChannelLine } from "./health-style-B6YeYS_2.js";
...
import { n as buildChannelAccountSnapshotFromAccount } from "./status-B3gyOjOn.js";
import { t as logGatewayConnectionDetails } from "./status.gateway-connection-DwyaYBo3.js";
import { n as isRich } from "./theme-CiH_wF8x.js";
...
const buildSessionSummary = async (storePath) => {
  const { loadSessionStore } = await import("./store-GG1XjrR-.js");
...
async function getHealthSnapshot(params) {
  ...
  const heartbeatSeconds = defaultAgent?.heartbeat.everyMs
    ? Math.round(defaultAgent.heartbeat.everyMs / 1e3)
    : 0;
  const sessions =
  ...
  for (const plugin of plugins) {
    ...
    for (const accountId of accountIdsToProbe) {
      ...
      if (diagnostics.length > 0)
        debugHealth("account.diagnostics", {
          channel: plugin.id,
          accountId,
          diagnostics,
        });
      ...
      const snapshot = await buildChannelAccountSnapshotFromAccount({
        ...
        runtime:
          ...
          (accountId === defaultAccountId ? params?.runtimeSnapshot?.channels[plugin.id] : void 0),
    ...
    if (debugEnabled) {
      ...
      for (const plugin of displayPlugins) {
        ...
        for (const accountId of accountIds) {
          ...
          const tokenSource =
            record && typeof record.tokenSource === "string" ? record.tokenSource : void 0;
          runtime.log(
    ...
    if (displayAgents.length === 0) {
      ...
      if (summary.sessions.recent.length > 0)
        for (const r of summary.sessions.recent)
          runtime.log(
            `- ${r.key} (${r.updatedAt ? `${Math.round((Date.now() - r.updatedAt) / 6e4)}m ago` : "no activity"})`,
          );
    } else
