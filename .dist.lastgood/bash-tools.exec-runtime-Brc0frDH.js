import "./host-env-security-W4c-l4Zm.js";
import path from "node:path";
import {
  c as readEnvInt,
  i as clampWithDefault,
  r as chunkString,
  t as buildDockerExecArgs,
...
import "./bash-tools.schemas-bZ_QTpiO.js";
import { n as findPathKey, r as mergePathPrepend } from "./path-prepend-C3srv1EO.js";
import { r as generateSecureInt } from "./secure-random-Ck_jpq4r.js";
import { p as scopedHeartbeatWakeOptions } from "./session-key-B4qUwRzq.js";
import { n as getShellConfig, r as sanitizeBinaryOutput } from "./shell-utils-BaRdb5e3.js";
...
function createFallbackSuffix(length) {
  ...
  for (let i = 0; i < length; i += 1)
    suffix += SLUG_FALLBACK_ALPHABET[generateSecureInt(36)] ?? "x";
  return suffix;
}
...
function renderExecUpdateText(params) {
  return (
    (params.warnings.length ? `${params.warnings.join("\n")}\n\n` : "") +
...
function resolveExecTarget(params) {
  ...
  if (
    ...
    throw new Error(
      `exec host not allowed (requested ${renderExecTargetLabel(requestedTarget)}; configured host is ${renderExecTargetLabel(configuredTarget)}; set tools.exec.host=${allowedConfig} to allow this override).`,
    );
  }
  ...
  const onSupervisorStdout = (chunk) => {
    if (usingPty) {
      ...
      if (requests > 0 && managedRun?.stdin)
        for (let i = 0; i < requests; i += 1) managedRun.stdin.write(cursorResponse);
...
export {
  markExited as C,
  markBackgrounded as S,
  tail as T,
  drainSession as _,
  DEFAULT_PENDING_MAX_OUTPUT as a,
  listFinishedSessions as b,
  createApprovalSlug as c,
  renderExecTargetLabel as d,
  resolveApprovalRunningNoticeMs as f,
  deleteSession as g,
  renderExecOutputText as h,
  DEFAULT_PATH as i,
  isRequestedExecTargetAllowed as l,
  runExecProcess as m,
  DEFAULT_APPROVAL_TIMEOUT_MS as n,
  applyShellPath as o,
  resolveExecTarget as p,
  DEFAULT_MAX_OUTPUT as r,
  buildApprovalPendingMessage as s,
  DEFAULT_APPROVAL_REQUEST_TIMEOUT_MS as t,
  normalizeNotifyOutput as u,
  getFinishedSession as v,
  setJobTtlMs as w,
  listRunningSessions as x,
  getSession as y,
};
