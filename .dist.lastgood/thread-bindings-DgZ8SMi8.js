...
function toSessionBindingRecord(record, defaults) {
  const idleExpiresAt =
    defaults.idleTimeoutMs > 0 ? record.lastActivityAt + defaults.idleTimeoutMs : void 0;
  ...
  const manager = {
    ...
    stop: () => {
      for (const key of getState().bindingsByAccountConversation.keys())
        if (key.startsWith(`${accountId}:`)) getState().bindingsByAccountConversation.delete(key);
...
//#endregion
export {
  createFeishuThreadBindingManager as n,
  getFeishuThreadBindingManager as r,
  __testing as t,
};
