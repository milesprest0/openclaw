import {
  ...
  S as ordinal,
  T as plugins,
  _ as createSimpleStackTrace,
  b as isObject,
  d as delay,
  f as getSafeTimers,
  g as assertTypes,
  h as printDiffOrStringify,
  l as parseErrorStacktrace,
  m as diff,
  o as getCurrentTest,
  p as processError,
  s as getNames,
  u as parseSingleStack,
  v as getCallLastIndex,
  w as format,
  x as noop,
  y as getType,
      ...
      if (canSetPrototype) {
        ...
        Object.getOwnPropertyNames(ctx).forEach(function (asserterName) {
          ...
          Object.defineProperty(
            chainableMethodWrapper,
            asserterName,
            Object.getOwnPropertyDescriptor(ctx, asserterName),
          );
        });
...
function assert$1(express, errmsg) {
  new Assertion(null, null, assert$1, true).assert(
    express,
    errmsg,
...
assert$1.notDeepOwnPropertyVal = function (obj, prop, value, msg) {
  new Assertion(obj, msg, assert$1.notDeepOwnPropertyVal, true).to.not.have.deep.own.property(
    prop,
    value,
  );
};
...
assert$1.notNestedPropertyVal = function (obj, prop, val, msg) {
  new Assertion(obj, msg, assert$1.notNestedPropertyVal, true).to.not.have.nested.property(
    prop,
    val,
  );
};
assert$1.deepNestedPropertyVal = function (obj, prop, val, msg) {
  new Assertion(obj, msg, assert$1.deepNestedPropertyVal, true).to.have.deep.nested.property(
    prop,
    val,
  );
};
assert$1.notDeepNestedPropertyVal = function (obj, prop, val, msg) {
  new Assertion(obj, msg, assert$1.notDeepNestedPropertyVal, true).to.not.have.deep.nested.property(
    prop,
    val,
  );
};
...
assert$1.throws = function (fn, errorLike, errMsgMatcher, msg) {
  ...
  return flag(
    new Assertion(fn, msg, assert$1.throws, true).to.throw(errorLike, errMsgMatcher),
    "object",
  );
};
...
assert$1.notSameOrderedMembers = function (set1, set2, msg) {
  new Assertion(set1, msg, assert$1.notSameOrderedMembers, true).to.not.have.same.ordered.members(
    set2,
  );
};
assert$1.sameDeepOrderedMembers = function (set1, set2, msg) {
  new Assertion(set1, msg, assert$1.sameDeepOrderedMembers, true).to.have.same.deep.ordered.members(
    set2,
  );
};
...
assert$1.notIncludeDeepMembers = function (superset, subset, msg) {
  new Assertion(superset, msg, assert$1.notIncludeDeepMembers, true).to.not.include.deep.members(
    subset,
  );
};
assert$1.includeOrderedMembers = function (superset, subset, msg) {
  new Assertion(superset, msg, assert$1.includeOrderedMembers, true).to.include.ordered.members(
    subset,
  );
};
...
assert$1.doesNotDecreaseBy = function (fn, obj, prop, delta, msg) {
  ...
  return new Assertion(fn, msg, assert$1.doesNotDecreaseBy, true).to.not
    .decrease(obj, prop)
    .by(delta);
};
...
function printWithType(name, value, print) {
  ...
  return (
    (type !== "null" && type !== "undefined" ? `${name} has type:  ${type}\n` : "") +
      ...
      for (const aValue of a)
        if (!b.has(aValue)) {
          ...
          for (const bValue of b)
            if (equals(aValue, bValue, filteredCustomTesters) === true) has = true;
          if (has === false) {
            allFound = false;
            break;
          }
...
function recordAsyncExpect(_test, promise, assertion, error, isSoft) {
  ...
  if (test && promise instanceof Promise) {
    ...
    test.onFinished.push(() => {
      if (!resolved) {
        const stack = (globalThis.__vitest_worker__?.onFilterStackTrace || ((s) => s || ""))(
          error.stack,
        );
        console.warn(
          [
...
const JestChaiExpect = (chai, utils) => {
  ...
  def("toBe", function (expected) {
    ...
    if (!pass) {
      ...
      else if (equals(actual, expected, [...customTesters, iterableEquality]))
  ...
  def("toContain", function (item) {
    ...
    if (typeof DOMTokenList !== "undefined" && actual instanceof DOMTokenList) {
      ...
      const expectedClassList = utils.flag(this, "negate")
  ...
  def("toHaveProperty", function (...args) {
    ...
    const getValue = () => {
      if (Object.hasOwn(actual, propertyName))
        return {
  ...
  function equalsArgumentArray(a, b) {
    return (
      a.length === b.length &&
      a.every((aItem, i) => equals(aItem, b[i], [...customTesters, iterableEquality]))
    );
  }
  ...
  def("toHaveBeenCalledExactlyOnceWith", function (...args) {
    ...
    const callCount = spy.mock.calls.length;
    const pass =
      spy.mock.calls.some((callArg) => equalsArgumentArray(callArg, args)) && callCount === 1;
    const isNot = utils.flag(this, "negate");
    const msg = utils.getMessage(this, [
      pass,
  ...
  def("toHaveBeenNthCalledWith", function (times, ...args) {
    ...
    this.assert(
      ...
      `expected ${ordinal(times)} "${spyName}" call to have been called with #{exp}${isCalled ? `` : `, but called only ${callCount} times`}`,
  ...
  [
    ...
    def(name, function (nthCall, value) {
      const spy = getSpy(this);
      const spyName = spy.getMockName();
      const result = (action === "return" ? spy.mock.results : spy.mock.settledResults)[
        nthCall - 1
      ];
...
async function saveSnapshotFile(environment, snapshotData, snapshotPath) {
  const snapshots = Object.keys(snapshotData)
    ...
    .map(
      (key) =>
        `exports[${printBacktickString(key)}] = ${printBacktickString(normalizeNewlines(snapshotData[key]))};`,
    );
...
async function saveInlineSnapshots(environment, snapshots) {
  const MagicString = (await import("./magic-string.es-DQ4zNiQK.js")).default;
    ...
    Array.from(files).map(async (file) => {
      ...
      for (const snap of snaps)
        replaceInlineSnap(
          code,
          s,
          positionToOffset(code, snap.line, snap.column),
          snap.snapshot,
          snap.assertionName,
        );
...
function replaceObjectSnap(code, s, index, newSnap, assertionName) {
  ...
  const startMatch = (
    assertionName ? buildStartObjectRegex(assertionName) : defaultStartObjectRegex
  ).exec(_code);
...
function replaceInlineSnap(code, s, currentIndex, newSnap, assertionName) {
  const { code: codeStartingAtIndex, index } = getCodeStartingAtIndex(
    code,
    currentIndex,
    assertionName ? [assertionName] : defaultMethodNames,
  );
  const startMatch = (assertionName ? buildStartRegex(assertionName) : defaultStartRegex).exec(
    codeStartingAtIndex,
  );
...
var SnapshotState = class SnapshotState {
  ...
  static async create(testFilePath, options) {
    ...
    return new SnapshotState(
      testFilePath,
      snapshotPath,
      await options.snapshotEnvironment.readSnapshotFile(snapshotPath),
      options,
    );
  }
  ...
  processDomainSnapshot({
    ...
    return this._reconcile({
      ...
      expectedDisplay:
        expectedResolved !== void 0 ? removeExtraLineBreaks(expectedResolved) : void 0,
      stack,
      assertionName,
    });
  }
...
var SnapshotClient = class {
  ...
  async pollMatchDomain(options) {
    ...
    const reference =
      expectedSnapshot.data && snapshotState.snapshotUpdateState !== "all"
  ...
  async assertRaw(options) {
    ...
    if (rawSnapshot.content == null) {
      ...
      rawSnapshot.content =
        (await snapshotState.environment.readSnapshotFile(rawSnapshot.file)) ?? void 0;
    }
  ...
  (function (module, exports$1) {
    (function (global, factory) {
      ...
      var setIteratorPrototype =
        setEntriesExists && Object.getPrototypeOf(/* @__PURE__ */ new Set().entries());
      var mapIteratorPrototype =
        mapEntriesExists && Object.getPrototypeOf(/* @__PURE__ */ new Map().entries());
...
function requireFakeTimersSrc() {
  ...
  function withGlobal(_global) {
    ...
    const utilPromisify =
      _global.process && _global.__vitest_required__ && _global.__vitest_required__.util.promisify;
    ...
    const NativeIntl = isPresent.Intl
      ? Object.defineProperties(Object.create(null), Object.getOwnPropertyDescriptors(_global.Intl))
      : void 0;
    ...
    function getInfiniteLoopError(clock, job) {
      const infiniteLoopError = /* @__PURE__ */ new Error(
...
var FakeTimers = class {
  ...
  setTimerTickMode(mode, interval) {
    if (this._checkFakeTimers())
      if (mode === "manual") this._clock.setTickMode({ mode: "manual" });
      else if (mode === "nextTimerAsync") this._clock.setTickMode({ mode: "nextAsync" });
      else if (mode === "interval")
        this._clock.setTickMode({
          mode: "interval",
          delta: interval,
        });
...
function waitFor(callback, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const handleTimeout = () => {
      ...
      let error = lastError;
      if (!error)
        error = copyStackTrace$1(
          /* @__PURE__ */ new Error("Timed out in waitFor!"),
          STACK_TRACE_ERROR,
        );
      reject(error);
    };
...
function waitUntil(callback, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const onReject = (error) => {
      ...
      if (!error)
        error = copyStackTrace$1(
          /* @__PURE__ */ new Error("Timed out in waitUntil!"),
          STACK_TRACE_ERROR,
        );
      reject(error);
    };
...
function getImporter(name) {
  ...
  return (
    parseSingleStack(
      stackArray[
        stackArray.findLastIndex((stack) => {
          return (
      ...
      get(target, key, receiver) {
        ...
        return function __VITEST_POLL_CHAIN__(...args) {
          ...
          const promise = async () => {
            ...
            if (
              Object.getOwnPropertyDescriptor(assertionFunction, "__vitest_poll_takeover__")?.value
            )
              try {
              ...
              while (true) {
                ...
                try {
                  ...
                  if (
                    isLastAttempt ||
                    (executionPhase === "assertion" &&
                      utils_exports.flag(assertion, "_poll.assert_once"))
                  ) {
                    await onSettled?.({
                      assertion,
                      status: "fail",
...
function validateAssertion(assertion) {
  if (utils_exports.flag(assertion, "negate"))
...
const SnapshotPlugin = (chai, utils) => {
  for (const key of ["matchSnapshot", "toMatchSnapshot"])
    utils.addMethod(
      ...
      key,
      wrapAssertion(utils, key, function (propertiesOrHint, hint) {
        return assertMatchResult(
          toMatchSnapshotImpl({
            assertion: this,
  ...
  utils.addMethod(
    ...
    wrapAssertion(
      ...
      function __INLINE_SNAPSHOT_OFFSET_3__(
        propertiesOrInlineSnapshot,
        inlineSnapshotOrHint,
        hint,
      ) {
        return assertMatchResult(
          toMatchSnapshotImpl({
  ...
  utils.addMethod(
    chai.Assertion.prototype,
    "toThrowErrorMatchingSnapshot",
    wrapAssertion(utils, "toThrowErrorMatchingSnapshot", function (propertiesOrHint, hint) {
      validateAssertion(this);
  ...
  utils.addMethod(
    ...
    wrapAssertion(
      utils,
      "toThrowErrorMatchingInlineSnapshot",
      function __INLINE_SNAPSHOT_OFFSET_3__(inlineSnapshotOrHint, hint) {
        validateAssertion(this);
        const received = utils.flag(this, "object");
        const promise = utils.flag(this, "promise");
        return assertMatchResult(
