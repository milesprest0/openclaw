...
import { l as onAgentEvent } from "./agent-events-c6-2eZvI.js";
import {
...
import { n as assertExplicitGatewayAuthModeWhenBothConfigured } from "./auth-mode-policy-B5zSq0ZS.js";
import { t as resolveGatewayInteractiveSurfaceAuth } from "./auth-surface-resolution-oMpr2b1p.js";
import {
  c as ensureExplicitGatewayAuth,
  d as resolveExplicitGatewayAuth,
  r as buildGatewayConnectionDetails,
} from "./call-hIeQLRKk.js";
import {
  a as replaceOversizedChatHistoryMessages,
  i as enforceChatHistoryFinalBudget,
  n as augmentChatHistoryWithCanvasBlocks,
  t as CHAT_HISTORY_MAX_SINGLE_MESSAGE_BYTES,
...
import { r as extractAssistantVisibleText } from "./chat-message-content-BLockgKs.js";
import "./message-channel-GRZIhFYD.js";
import { t as augmentChatHistoryWithCliSessionImports } from "./cli-session-history-Dv56DP5l.js";
import { n as GatewayClient, r as GatewayClientRequestError } from "./client-BYqFLh-J.js";
import "./protocol-CUn5aLrV.js";
import "./version-B4HnZN6H.js";
import {
...
import { t as loadCombinedSessionStoreForGateway } from "./combined-store-gateway-CtdiSumZ.js";
import {
  n as listChatCommands,
  r as listChatCommandsForConfig,
} from "./commands-registry-list-COR74Rkl.js";
import "./sessions-Bdy1wToU.js";
import { s as setConsoleSubsystemFilter } from "./console-D1AMcIC4.js";
...
import { t as isAuthErrorMessage } from "./failover-matches-CDN8j5cn.js";
import { n as formatTimeAgo, t as formatRelativeTimestamp } from "./format-relative-Dc5mB_gw.js";
...
import {
  i as shouldSuppressAssistantEventForLiveChat,
  n as projectLiveAssistantBufferedText,
  r as resolveMergedAssistantText,
  t as normalizeLiveAssistantEventText,
} from "./live-chat-projector-_RwQhWRr.js";
import { g as loggingState } from "./logger-BU4ooQvN.js";
import { r as INTERNAL_MESSAGE_CHANNEL } from "./message-channel-core-VUC-himU.js";
import { n as modelKey } from "./model-ref-shared-C2I7TGOl.js";
import {
...
import { x as setEmbeddedMode } from "./openclaw-tools-6wQ_ahnn.js";
...
import { n as loadGatewayModelCatalog } from "./server-model-catalog-CPOc7rIW.js";
import {
...
} from "./setup-launch-env-DyFzf2TX.js";
...
import { r as stripLeadingInboundMetadata } from "./strip-inbound-meta-D_kvNQoY.js";
import { o as resolveSessionInfoModelSelection } from "./subagent-list-CDDt_IaF.js";
import {
  g as resolveResponseUsageMode,
  m as normalizeUsageDisplay,
  r as listThinkingLevelLabels,
  t as formatThinkingLevels,
} from "./thinking-C9_Z62px.js";
import { r as resolveToolDisplay, t as formatToolDetail } from "./tool-display-qyfb5lzi.js";
import { u as registerUncaughtExceptionHandler } from "./unhandled-rejections-ChRE0RCv.js";
import { r as formatTokenCount } from "./usage-format-DQx7gWeg.js";
...
function formatAssistantErrorFromRecord(record) {
  ...
  return formatRawAssistantErrorForUi(
...
function extractTextBlocks(content, opts) {
  ...
  return composeThinkingAndContent({
    thinkingText: (opts?.includeThinking === true
...
function extractText(result) {
  ...
  for (const entry of result.content)
    if (entry.type === "text" && entry.text) lines.push(sanitizeRenderableText(entry.text));
...
var ChatLog = class extends Container {
  ...
  dropComponentReferences(component) {
    ...
    for (const [runId, entry] of this.pendingUsers.entries())
      if (entry.component === component) this.pendingUsers.delete(runId);
...
var EmbeddedTuiBackend = class {
  ...
  abortSessionRuns(sessionKey) {
    for (const run of this.runs.values())
      if (run.sessionKey === sessionKey && !run.isBtw) run.controller.abort();
  }
  nextSeq() {
    this.seq += 1;
    return this.seq;
...
function findWordBoundaryIndex(text, query) {
  ...
  for (let i = 0; i <= maxIndex; i++)
    if (textLower.startsWith(queryLower, i) && isWordBoundary(textLower, i)) return i;
  return null;
}
/**
    ...
    if (description) {
      ...
      if (descriptionLayout) {
        ...
        const descriptionWidth =
          descriptionLayout.availableWidth - usedByValue - descriptionLayout.spacingWidth;
...
function createEventHandlers(context) {
  ...
  const handleChatEvent = (payload) => {
    ...
    if (evt.state === "final") {
      ...
      if (finalText === "(no output)" && !isLocalRunId?.(evt.runId))
        chatLog.dropAssistant(evt.runId);
  ...
  const handleAgentEvent = (payload) => {
    ...
    if (evt.stream === "tool") {
      ...
      if (phase === "start") chatLog.startTool(toolCallId, toolName, data.args);
        ...
        if (allowToolOutput)
          chatLog.updateToolResult(toolCallId, data.result, { isError: Boolean(data.isError) });
        else
...
async function readTuiLastSessionKey(params) {
  const value = (await readStore(resolveTuiLastSessionStatePath(params.stateDir)))[params.scopeKey]
    ?.sessionKey;
...
function createSessionActions(context) {
  ...
  const applySessionInfo = (params) => {
    ...
    if (entry?.thinkingLevels !== void 0 || defaults?.thinkingLevels !== void 0)
          ...
          chatLog.startTool(toolCallId, toolName, {}).setResult(
            {
              ...
              details:
                typeof message.details === "object" && message.details ? message.details : void 0,
            },
...
function resolveLocalAuthCliInvocation(params) {
  ...
  return hasDistEntry || !hasRunNodeScript
    ? {
        command,
        args: [wrapperPath, "models", "auth", "login"],
      }
    : {
        command,
        args: [runNodePath, "models", "auth", "login"],
      };
}
function resolveLocalAuthSpawnOptions(params) {
  return (params.platform ?? process.platform) === "win32" &&
    /\.(cmd|bat)$/iu.test(params.command.trim())
    ? { shell: true }
    : {};
}
...
function installTuiTerminalLossExitHandler(
  requestExit,
  targets = {
    stdin: process.stdin,
...
async function runTui(opts) {
  ...
  const startWaitingTimer = () => {
    ...
    if (!waitingPhrase)
      waitingPhrase =
        defaultWaitingPhrases[Math.floor(Math.random() * defaultWaitingPhrases.length)] ??
        defaultWaitingPhrases[0] ??
        "waiting";
    waitingTick = 0;
...
//#endregion
export {
  createEditorSubmitHandler as _,
  isIgnorableTuiStopError as a,
  resolveFinalAssistantText as b,
  resolveCtrlCAction as c,
  resolveLocalAuthCliInvocation as d,
  resolveLocalAuthSpawnCwd as f,
  stopTuiSafely as g,
  runTui as h,
  installTuiTerminalLossExitHandler as i,
  resolveGatewayDisconnectState as l,
  resolveTuiSessionKey as m,
  createDeferredTuiFinish as n,
  isTuiTerminalLossError as o,
  resolveLocalAuthSpawnOptions as p,
  drainAndStopTuiSafely as r,
  resolveCodexCliBin as s,
  createBackspaceDeduper as t,
  resolveInitialTuiAgentId as u,
  createSubmitBurstCoalescer as v,
  shouldEnableWindowsGitBashPasteFallback as y,
};
