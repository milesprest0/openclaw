...
import { i as truncate, r as pad } from "./list.format-BeMcDQgH.js";
...
import { i as withProgressTotals } from "./progress-DRZHxXMU.js";
import {
...
import { i as formatTokenK, r as formatMs, u as updateConfig } from "./shared-BbHHdmwP.js";
import {
...
function parseModality(modality) {
  ...
  return normalizeLowercaseStringOrEmpty(modality)
    .split(/[^a-z]+/)
    .filter(Boolean)
    .includes("image")
    ? ["text", "image"]
...
async function modelsScanCommand(opts, runtime) {
  ...
  if (minParams !== void 0 && (!Number.isFinite(minParams) || minParams < 0))
    throw new Error("--min-params must be >= 0");
  const maxAgeDays = opts.maxAgeDays ? Number(opts.maxAgeDays) : void 0;
  if (maxAgeDays !== void 0 && (!Number.isFinite(maxAgeDays) || maxAgeDays < 0))
  ...
  if (concurrency !== void 0 && (!Number.isFinite(concurrency) || concurrency <= 0))
  ...
  const preselected = (imagePreferred.length > 0 ? imagePreferred : toolSorted)
  ...
  if (canPrompt) {
    ...
    if (imageSorted.length > 0)
      selectedImages = guardPromptCancel(
        await multiselect$1({
