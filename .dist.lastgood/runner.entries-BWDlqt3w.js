...
import {
  c as DEFAULT_VIDEO_MAX_BASE64_BYTES,
  d as getMediaUnderstandingProvider,
  l as MIN_AUDIO_FILE_BYTES,
  s as DEFAULT_TIMEOUT_SECONDS,
  t as CLI_OUTPUT_MAX_BUFFER,
} from "./defaults.constants-u25YkDfl.js";
import { i as runExec } from "./exec-Csn_09g3.js";
import { T as pathExists, r as writeExternalFileWithinRoot } from "./fs-safe-CgBWiL92.js";
import { a as shouldLogVerbose, r as logVerbose } from "./globals-BdfwDi2E.js";
import { t as describeImageWithModel } from "./image-runtime-DLf9RXbB.js";
import {
  a as mergeModelProviderRequestOverrides,
  d as sanitizeConfiguredProviderRequest,
  u as sanitizeConfiguredModelProviderRequest,
} from "./provider-request-config-B-kAAqy2.js";
import { i as resolveProxyFetchFromEnv } from "./proxy-fetch-Dt1coO5G.js";
import {
  a as resolvePrompt,
  n as resolveMaxBytes,
  r as resolveMaxChars,
  s as resolveTimeoutMs,
} from "./resolve-DB4enOPl.js";
import { t as resolveSystemBin } from "./resolve-system-bin-V2-g2eqK.js";
...
function requireSystemBin(name) {
  ...
  if (!resolved) {
    ...
    throw new Error(
      `${name} not found in trusted system directories. Install it via your system package manager (${hint}).`,
    );
  }
  return resolved;
}
...
async function loadModelAuth() {
  cachedModelAuth ??= await import("./model-auth-w3SveVAp.js");
...
async function resolveCliMediaPath(params) {
  ...
  if (normalizeLowercaseStringOrEmpty(path.extname(params.mediaPath)) === ".wav")
...
function formatDecisionSummary(decision) {
  ...
  const shortReason = summarizeDecisionReason(
    findDecisionReason(decision, decision.outcome === "failed" ? "failed" : void 0),
  );
...
async function runProviderEntry(params) {
  ...
  if (capability === "audio") {
    ...
    const model =
      entry.model?.trim() ||
      (await import("./defaults-DaV-V90B.js")).resolveDefaultMediaModel({
        cfg,
        providerId,
        capability: "audio",
      }) ||
...
export {
  fileExists as _,
  runCliEntry as a,
  parseFfprobeCodecAndSampleRate as c,
  runFfprobe as d,
  MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS as f,
  extractGeminiResponse as g,
  MEDIA_FFPROBE_TIMEOUT_MS as h,
  normalizeDecisionReason as i,
  parseFfprobeCsvFields as l,
  MEDIA_FFMPEG_TIMEOUT_MS as m,
  findDecisionReason as n,
  runProviderEntry as o,
  MEDIA_FFMPEG_MAX_BUFFER_BYTES as p,
  formatDecisionSummary as r,
  summarizeDecisionReason as s,
  buildModelDecision as t,
  runFfmpeg as u,
  MediaUnderstandingSkipError as v,
  isMediaUnderstandingSkipError as y,
};
