...
import { r as normalizeAllowListLower } from "./allow-list-CBwLGSwi.js";
import {
  o as createAccountScopedAllowlistNameResolver,
  r as buildLegacyDmAccountAllowlistAdapter,
  s as createFlatAllowlistOverrideResolver,
...
import {
  ...
  r as resolveSlackInteractiveBlockOffsets,
  t as buildSlackInteractiveBlocks,
} from "./blocks-render-B8p7B_zB.js";
import { t as adaptScopedAccountAccessor } from "./channel-config-helpers-DVht29vx.js";
...
import {
  h as stringifyRouteThreadId,
  o as channelRouteTargetsMatchExact,
} from "./channel-route-B9olp3tt.js";
import "./approval-delivery-runtime-DFh6q7-3.js";
import { i as createAttachedChannelResultAdapter } from "./channel-send-result-CsgaqKTY.js";
import { r as createSlackWebClient } from "./client-CysiXrBq.js";
...
import {
  a as normalizeSlackApproverId,
  c as shouldSuppressLocalSlackExecApprovalPrompt,
  i as isSlackExecApprovalClientEnabled,
  o as resolveSlackExecApprovalTarget,
  r as isSlackExecApprovalAuthorizedSender,
  s as shouldHandleSlackExecApprovalRequest,
  t as getSlackExecApprovalApprovers,
...
import { r as createLazyRuntimeModule } from "./lazy-runtime-BKxgiyqW.js";
import "./param-readers-ElL64-re.js";
import {
  n as extractSlackToolSend,
  t as describeSlackMessageTool,
...
import {
  ...
  s as normalizeInteractiveReply,
} from "./payload-BcGg6iOl.js";
import { t as resolveSlackReplyBlocks } from "./reply-blocks-D0M9Adoy.js";
import { n as isSingleUseReplyToMode } from "./reply-reference-BWcPXkI5.js";
import { t as createRuntimeDirectoryLiveAdapter } from "./runtime-forwarders-zg2wxLcy.js";
import { t as getOptionalSlackRuntime } from "./runtime-jN5ydXqv.js";
import "./channel-api-FxVwgfc-.js";
import { n as resolveOutboundSendDep } from "./send-deps-CtmJEFTh.js";
import {
  i as SLACK_CHANNEL,
  n as createSlackSetupWizardProxy,
  r as slackSetupAdapter,
} from "./setup-core-CvfMokqA.js";
import {
  i as slackSecurityAdapter,
  n as isSlackPluginAccountConfigured,
  r as slackConfigAdapter,
  t as createSlackPluginBase,
} from "./shared-8TT9cfLA.js";
import {
...
import {
  n as resolveSlackThreadTsValue,
  r as SLACK_TEXT_LIMIT,
  t as normalizeSlackThreadTsCandidate,
} from "./thread-ts-CAy_-OyD.js";
...
function extractSlackSessionKind(sessionKey) {
  ...
  const kind = normalizeLowercaseStringOrEmpty(
    sessionKey.match(/slack:(direct|channel|group):/i)?.[1],
  );
...
function resolveTurnSourceSlackOriginTarget(request) {
  ...
  const parsed = parseSlackTarget(turnSourceTo, {
    defaultKind:
      extractSlackSessionKind(request.request.sessionKey ?? void 0) === "direct"
        ? "user"
        : "channel",
  });
...
const slackApprovalCapability = createApproverRestrictedNativeApprovalCapability({
  ...
  nativeRuntime: createLazyChannelApprovalNativeRuntimeAdapter({
    ...
    load: async () =>
      (await import("./approval-handler.runtime-Dd2sGTPw.js")).slackApprovalNativeRuntime,
  }),
});
      ...
      return await handleSlackMessageAction({
        ...
        invoke: async (action, cfg, toolContext) => {
          ...
          return await (options?.invoke
            ...
            : (await loadSlackActionRuntime$1()).handleSlackAction(action, cfg, actionContext));
        },
      });
    },
  };
...
function buildSlackThreadingToolContext(params) {
  const configuredReplyToMode = resolveSlackReplyToMode(
    resolveSlackAccount({
      cfg: params.cfg,
      accountId: params.accountId,
    }),
  ...
  const effectiveReplyToMode =
    params.context.MessageThreadId != null ? "all" : configuredReplyToMode;
  const threadId = params.context.MessageThreadId ?? params.context.ReplyToId;
  return {
    currentChannelId: params.context.To?.startsWith("channel:")
      ? params.context.To.slice(8)
...
const loadTargetResolverRuntimeSdk = createLazyRuntimeModule(
  () => import(TARGET_RESOLVER_RUNTIME_MODULE_ID),
);
const loadSlackSetupSurfaceModule = createLazyRuntimeModule(
  () => import("./setup-surface-D1hkk2Ji.js"),
);
const loadSlackScopesModule = createLazyRuntimeModule(() => import("./scopes-Cf30XXyJ.js"));
const loadSlackOutboundAdapterModule = createLazyRuntimeModule(
  () => import("./outbound-adapter-BKigiesu.js"),
);
...
const loadSlackDirectoryConfigModule = createLazyRuntimeModule(
  () => import("./directory-config-BieIYf-w.js"),
);
const loadSlackResolveChannelsModule = createLazyRuntimeModule(
  () => import("./resolve-channels-q4_CfTF1.js"),
);
const loadSlackResolveUsersModule = createLazyRuntimeModule(
  () => import("./resolve-users-D8E_BEVq.js"),
);
...
async function loadSlackSendRuntime() {
  slackSendRuntimePromise ??= import("./send.runtime-BmcwSouz.js");
  return await slackSendRuntimePromise;
}
async function loadSlackProbeModule() {
  slackProbeModulePromise ??= import("./probe-DvaPwfOu.js");
  return await slackProbeModulePromise;
}
async function loadSlackMonitorModule() {
  slackMonitorModulePromise ??= import("./monitor-BIhOo1qP.js");
  return await slackMonitorModulePromise;
}
async function loadSlackDirectoryLiveModule() {
  slackDirectoryLiveModulePromise ??= import("./directory-live-DVmZpeFQ.js");
  return await slackDirectoryLiveModulePromise;
}
async function resolveSlackSendContext(params) {
  const send =
    resolveOutboundSendDep(params.deps, "slack") ?? (await loadSlackSendRuntime()).sendMessageSlack;
  const account = resolveSlackAccount({
    cfg: params.cfg,
    accountId: params.accountId,
  });
...
//#endregion
export {
  createSlackActions as a,
  resolveSlackChannelType as i,
  buildSlackThreadingToolContext as n,
  resolveSlackAutoThreadId as o,
  __resetSlackChannelTypeCacheForTest as r,
  slackPlugin as t,
};
