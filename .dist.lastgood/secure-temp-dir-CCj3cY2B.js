...
import {
  r as getFsSafePythonConfig,
  t as canFallbackFromPythonError,
} from "./fs-safe-defaults-DPw2RCP0.js";
import {
  i as isPathInside,
  n as hasNodeErrorCode,
  o as isSymlinkOpenError,
  p as FsSafeError,
  r as isNotFoundPathError,
  t as assertNoNulPathInput,
} from "./path-1liOXr_N.js";
import { r as resolveRootPath, t as ROOT_PATH_ALIAS_POLICIES } from "./root-path-BwPOxXJq.js";
import { n as normalizeOptionalString } from "./string-coerce-FEYRxS0z.js";
import {
  c as assertAsyncDirectoryGuard,
  d as createNearestExistingDirectoryGuard,
...
async function mkdirPathComponentsWithGuards(params) {
  ...
  for (const part of relative.split(path.sep).filter(Boolean)) {
    ...
    if (!isSameOrChildPath(path.resolve(await fs$1.realpath(next)), root))
...
function assertPinnedPythonOperationAvailable() {
  if (typeof getWorker().child.stdin?.write !== "function")
    throw new FsSafeError("helper-unavailable", "Python helper stdin is unavailable");
}
...
function assertWithinMaxBytes(bytes, maxBytes) {
  if (maxBytes !== void 0 && bytes > maxBytes)
    throw new FsSafeError(
      "too-large",
      `file exceeds limit of ${maxBytes} bytes (got at least ${bytes})`,
    );
...
async function resolveRootContext(rootDir) {
  ...
  try {
    ...
    if (!(await fs$1.stat(rootReal)).isDirectory())
...
const OPEN_WRITE_CREATE_FLAGS =
  ...
  constants.O_CREAT |
  constants.O_EXCL |
  (SUPPORTS_NOFOLLOW ? constants.O_NOFOLLOW : 0);
...
async function openVerifiedLocalFile(filePath, options) {
  ...
  try {
    ...
    if (options?.symlinks === "follow-within-root") {
      if (!sameFileIdentity(stat, await fs$1.stat(filePath)))
...
async function readOpenedFileSafely(params) {
  ...
  if (params.maxBytes !== void 0 && buffer.byteLength > params.maxBytes)
    throw new FsSafeError(
      "too-large",
  ...
  for (const entry of entries.toSorted()) {
    ...
    try {
      ...
      if (candidateStat.isFile() && sameFileIdentity(handleStat, candidateStat))
        return await fs$1.realpath(candidatePath);
    } catch (err) {
      if (!isNotFoundPathError(err)) throw err;
    }
  }
...
async function openWritableFileInRoot(root, params) {
  ...
  if (params.mkdir !== false)
    await withAsyncDirectoryGuards(
      [await createNearestExistingDirectoryGuard(rootReal, path.dirname(resolved))],
      async () => {
...
async function removePathFallback(resolved) {
  ...
  await ((await fs$1.lstat(resolved.resolved)).isDirectory()
    ? fs$1.rmdir(resolved.resolved)
    : fs$1.rm(resolved.resolved));
...
export {
  ...
  resolveHomeRelativePath as d,
  resolveUserPath as f,
  readLocalFileSafely as i,
  assertNoPathAliasEscape as l,
  DEFAULT_ROOT_MAX_BYTES as n,
  root as o,
  openLocalFileSafely as r,
  getFsSafeTestHooks as s,
  resolveSecureTempRoot as t,
  expandHomePrefix as u,
};
