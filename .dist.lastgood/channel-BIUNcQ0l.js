...
import {
  a as resolveDefaultFeishuAccountId,
  i as listFeishuAccountIds,
  l as resolveFeishuRuntimeAccount,
  m as isRecord,
  n as inspectFeishuCredentials,
  r as listEnabledFeishuAccounts,
  s as resolveFeishuAccount,
} from "./accounts-CFTFwStL.js";
import { t as formatAllowFromLowercase } from "./allow-from-BNON_Q4N.js";
...
import { i as createPairingPrefixStripper } from "./channel-pairing-Tw2QazlD.js";
import {
...
import {
  a as parseFeishuTargetId,
  i as parseFeishuDirectConversationId,
  n as buildFeishuModelOverrideParentCandidates,
  r as parseFeishuConversationId,
  t as buildFeishuConversationId,
} from "./conversation-id-CH88Qli9.js";
import {
  i as createChatChannelPlugin,
  t as buildChannelOutboundSessionRoute,
  u as stripChannelTargetPrefix,
} from "./core-BCeD7oMO.js";
import { t as createChannelDirectoryAdapter } from "./directory-runtime-B764p6cP.js";
import {
  n as listFeishuDirectoryPeers,
  t as listFeishuDirectoryGroups,
} from "./directory.static-Mv_JkTQS.js";
import { i as createLazyRuntimeNamedExport } from "./lazy-runtime-BKxgiyqW.js";
...
import {
  c as normalizeMessagePresentation,
  u as renderMessagePresentationFallbackText,
} from "./payload-BcGg6iOl.js";
import "./outbound-runtime-DIb149kM.js";
import "./conversation-runtime-CDBIORy7.js";
import { a as resolveFeishuGroupToolPolicy } from "./policy-82XAxEdx.js";
import "./account-resolution-C8X1oPma.js";
import {
  n as createRuntimeOutboundDelegates,
  t as createRuntimeDirectoryLiveAdapter,
} from "./runtime-forwarders-zg2wxLcy.js";
import {
  n as collectRuntimeConfigAssignments,
  r as secretTargetRegistryEntries,
} from "./secret-contract-TxJLRYjI.js";
...
import { t as messageActionTargetAliases } from "./security-audit-CFAVKIRt.js";
import { t as collectFeishuSecurityAuditFindings } from "./security-audit-shared-DSEpwHO3.js";
import { n as createFeishuSendReceipt } from "./send-result-DckBEKva.js";
import { r as getSessionBindingService } from "./session-binding-service-DjzRXKiv.js";
import { t as resolveFeishuSessionConversation } from "./session-conversation-7YX3q51h.js";
import {
  D as patchTopLevelChannelConfigSection,
...
import { n as looksLikeFeishuId, r as normalizeFeishuTarget } from "./targets-DqST8ng9.js";
import { t as chunkTextForOutbound } from "./text-chunking--YCa2npP.js";
import { s as hasConfiguredSecretInput } from "./types.secrets-CaNC1eIn.js";
...
const ChannelActionsSchema = zod_exports.z
  .object({ reactions: zod_exports.z.boolean().optional() })
  .strict()
  .optional();
const DmPolicySchema = zod_exports.z.enum(["open", "pairing", "allowlist"]);
const GroupPolicySchema = zod_exports.z.union([
  zod_exports.z.enum(["open", "allowlist", "disabled"]),
  zod_exports.z.literal("allowall").transform(() => "open"),
]);
const FeishuDomainSchema = zod_exports.z.union([
  zod_exports.z.enum(["feishu", "lark"]),
  zod_exports.z.string().url().startsWith("https://"),
]);
const FeishuConnectionModeSchema = zod_exports.z.enum(["websocket", "webhook"]);
const TtsOverrideSchema = zod_exports.z
  .object({
    auto: zod_exports.z.enum(["off", "always", "inbound", "tagged"]).optional(),
    ...
    personas: zod_exports.z
      .record(
        zod_exports.z.string(),
        zod_exports.z.record(zod_exports.z.string(), zod_exports.z.unknown()),
      )
      .optional(),
    summaryModel: zod_exports.z.string().optional(),
    modelOverrides: zod_exports.z
      .record(zod_exports.z.string(), zod_exports.z.unknown())
      .optional(),
    providers: zod_exports.z
      .record(
        zod_exports.z.string(),
        zod_exports.z.record(zod_exports.z.string(), zod_exports.z.unknown()),
      )
  ...
  .strict()
  .optional();
const ToolPolicySchema = zod_exports.z
  .object({
  ...
  .strict()
  .optional();
const DmConfigSchema = zod_exports.z
  .object({
    enabled: zod_exports.z.boolean().optional(),
  ...
  .strict()
  .optional();
const MarkdownConfigSchema = zod_exports.z
  .object({
    mode: zod_exports.z.enum(["native", "escape", "strip"]).optional(),
    tableMode: zod_exports.z.enum(["native", "ascii", "simple"]).optional(),
  })
  .strict()
  .optional();
const RenderModeSchema = zod_exports.z.enum(["auto", "raw", "card"]).optional();
const StreamingModeSchema = zod_exports.z.boolean().optional();
const BlockStreamingSchema = zod_exports.z.boolean().optional();
const BlockStreamingCoalesceSchema = zod_exports.z
  .object({
    enabled: zod_exports.z.boolean().optional(),
  ...
  .strict()
  .optional();
const ChannelHeartbeatVisibilitySchema = zod_exports.z
  .object({
 ...
 */
const DynamicAgentCreationSchema = zod_exports.z
  .object({
    enabled: zod_exports.z.boolean().optional(),
 ...
 */
const FeishuToolsConfigSchema = zod_exports.z
  .object({
 ...
 */
const GroupSessionScopeSchema = zod_exports.z
...
const ReactionNotificationModeSchema = zod_exports.z.enum(["off", "own", "all"]).optional();
/**
...
const FeishuGroupSchema = zod_exports.z
  .object({
    requireMention: zod_exports.z.boolean().optional(),
    tools: ToolPolicySchema,
    skills: zod_exports.z.array(zod_exports.z.string()).optional(),
    enabled: zod_exports.z.boolean().optional(),
    allowFrom: zod_exports.z
      .array(zod_exports.z.union([zod_exports.z.string(), zod_exports.z.number()]))
      .optional(),
    systemPrompt: zod_exports.z.string().optional(),
...
const FeishuSharedConfigShape = {
  ...
  groupAllowFrom: zod_exports.z
    .array(zod_exports.z.union([zod_exports.z.string(), zod_exports.z.number()]))
    .optional(),
  groupSenderAllowFrom: zod_exports.z
    .array(zod_exports.z.union([zod_exports.z.string(), zod_exports.z.number()]))
    .optional(),
  requireMention: zod_exports.z.boolean().optional(),
 ...
 */
const FeishuAccountConfigSchema = zod_exports.z
  .object({
    enabled: zod_exports.z.boolean().optional(),
  ...
  .strict();
const FeishuConfigSchema = zod_exports.z
  .object({
    enabled: zod_exports.z.boolean().optional(),
    ...
    accounts: zod_exports.z
      .record(zod_exports.z.string(), FeishuAccountConfigSchema.optional())
      .optional(),
  })
  .strict()
...
function isFeishuConfigured(cfg) {
  ...
  const accountConfigured = Object.values(feishuCfg?.accounts ?? {}).some((account) => {
    ...
    const accountAppIdConfigured = hasOwnAppId
      ? isAppIdConfigured(account.appId)
...
async function promptFeishuAllowFrom(params) {
  ...
  const existingAllowFrom =
    (resolvedAccountId !== "default" ? feishuCfg?.accounts?.[resolvedAccountId] : void 0)
      ?.allowFrom ??
    feishuCfg?.allowFrom ??
    [];
  ...
  const mergedAllowFrom = mergeAllowFromEntries(
    ...
    splitSetupEntries(
      await params.prompter.text({
        ...
        initialValue:
          existingAllowFrom.length > 0 ? existingAllowFrom.map(String).join(", ") : void 0,
      }),
    ),
  );
...
function applyNewAppSecurityPolicy(cfg, accountId, openId, groupPolicy) {
  let next = cfg;
  if (openId)
    next = patchFeishuConfig(next, accountId, {
      dmPolicy: "allowlist",
      allowFrom: [openId],
    });
...
async function runScanToCreate(prompter) {
  const { beginAppRegistration, initAppRegistration, pollAppRegistration, printQrCode } =
    await import("./app-registration-Ctd9GIy9.js");
  try {
...
async function runNewAppFlow(params) {
  ...
  if (scanResult) {
    ...
    if (appId && appSecretProbeValue) {
      const { getAppOwnerOpenId } = await import("./app-registration-Ctd9GIy9.js");
  ...
  const resolveAppIdLabel = (value) => {
    ...
    if (value && typeof value === "object") {
      ...
      if (normalizeString(rec.source) && normalizeString(rec.id))
        return normalizeString(process.env[rec.id]) ?? `env:${String(rec.id)}`;
      if (hasConfiguredSecretInput(value)) return "(configured)";
    }
  };
  const existingAppId =
    resolveAppIdLabel(feishuCfg?.appId) ??
    Object.values(feishuCfg?.accounts ?? {}).reduce(
      (found, account) => {
  ...
  status: {
    ...
    resolveStatusLines: async ({ cfg, accountId, configured }) => {
      ...
      if (configured && account.configured)
        try {
          const { probeFeishu } = await import("./probe-BLdWHzYT.js");
...
const loadFeishuChannelRuntime = createLazyRuntimeNamedExport(
  () => import("./channel.runtime-BHbJ1DH6.js"),
...
async function createFeishuActionClient(account) {
  const { createFeishuClient } = await import("./client-CKBTEuMp.js");
...
function resolveFeishuCommandConversation(params) {
  if (params.threadId) {
    ...
    return {
      conversationId:
        resolveFeishuSenderScopedCommandConversation({
          accountId: params.accountId,
          parentConversationId,
...
const feishuPlugin = createChatChannelPlugin({
  base: {
    ...
    gateway: {
      startAccount: async (ctx) => {
        const { monitorFeishuProvider } = await import("./monitor-B23BjMaQ.js");
  ...
  outbound: {
    ...
    renderPresentation: async (ctx) => {
      const renderPresentation = (await loadFeishuChannelRuntime()).feishuOutbound
        .renderPresentation;
...
//#endregion
export {
  setFeishuNamedAccountEnabled$1 as a,
  feishuSetupAdapter as i,
  feishuSetupWizard as n,
  runFeishuLogin as r,
  feishuPlugin as t,
};
