...
import {
  ...
  n as isCompactionCheckpointTranscriptFileName,
  r as isPrimarySessionTranscriptFileName,
  s as isTrajectorySessionArtifactName,
...
import { t as writeTextAtomic } from "./json-files-DifBk3kt.js";
import {
  l as listDeliverableMessageChannels,
...
import {
  C as getFileStatSnapshot,
  _ as getSerializedSessionStore,
  a as getActiveSessionMaintenanceWarning,
  b as takeMutableSessionStoreCache,
  c as pruneStaleEntries,
  g as dropSessionStoreObjectCache,
  i as capEntryCount,
  m as clearSessionStoreCaches,
  n as normalizeSessionStore,
  o as isProtectedSessionMaintenanceEntry,
  r as resolveMaintenanceConfig,
  s as pruneQuotaSuspensions,
  t as loadSessionStore,
  u as shouldRunSessionEntryMaintenance,
  v as isSessionStoreCacheEnabled,
  x as writeSessionStoreCache,
  y as setSerializedSessionStore,
...
import { i as normalizeHyphenSlug } from "./string-normalization-B6JUy5-Y.js";
import { t as createSubsystemLogger } from "./subsystem-Bjz8a2fE.js";
import {
  n as mergeSessionEntry,
  r as mergeSessionEntryPreserveActivity,
...
function resolveGroupSessionKey(ctx) {
  ...
  const normalizedChatType =
    chatType === "channel" ? "channel" : chatType === "group" ? "group" : void 0;
  ...
  const kind = secondIsKind
    ...
    : from.includes(":channel:") || normalizedChatType === "channel"
      ? "channel"
      : "group";
  const finalId = normalizeLowercaseStringOrEmpty(
...
function resolveSessionStoreEntry(params) {
  ...
  let existing =
    params.store[normalizedKey] ?? (legacyKeySet.size > 0 ? params.store[trimmedKey] : void 0);
...
function clearSessionStoreCacheForTest() {
  ...
  for (const queue of WRITER_QUEUES.values())
    for (const task of queue.pending)
      task.reject(/* @__PURE__ */ new Error("session store queue cleared for test"));
  WRITER_QUEUES.clear();
}
...
async function runExclusiveSessionStoreWrite(storePath, fn) {
  if (!storePath || typeof storePath !== "string")
    throw new Error(
      `runExclusiveSessionStoreWrite: storePath must be a non-empty string, got ${JSON.stringify(storePath)}`,
    );
...
function loadTrajectoryCleanupRuntime() {
  trajectoryCleanupRuntimePromise ??= import("./cleanup-QQLZN7HR.js");
...
function collectAcpMetadataSnapshot(store) {
  ...
  for (const [sessionKey, entry] of Object.entries(store))
    if (entry?.acp) snapshot.set(sessionKey, entry.acp);
  return snapshot;
}
...
async function updateLastRoute(params) {
  ...
  return await runExclusiveSessionStoreWrite(storePath, async () => {
    ...
    return await persistResolvedSessionEntry({
      ...
      store,
      resolved,
      next: mergeSessionEntryPreserveActivity(
        existing,
        metaPatch
          ? {
              ...basePatch,
...
//#endregion
export {
  deriveSessionMetaPatch as _,
  saveSessionStore as a,
  buildGroupDisplayName as b,
  updateSessionStoreEntry as c,
  normalizeStoreSessionKey as d,
  resolveSessionStoreEntry as f,
  deriveGroupSessionPatch as g,
  resolveSessionArtifactCanonicalPathsForEntry as h,
  runQuotaSuspensionMaintenance as i,
  clearSessionStoreCacheForTest as l,
  pruneUnreferencedSessionArtifacts as m,
  readSessionUpdatedAt as n,
  updateLastRoute as o,
  enforceSessionDiskBudget as p,
  recordSessionMetaFromInbound as r,
  updateSessionStore as s,
  archiveRemovedSessionTranscripts as t,
  drainSessionStoreWriterQueuesForTest as u,
  deriveSessionOrigin as v,
  resolveGroupSessionKey as x,
  snapshotSessionOrigin as y,
};
