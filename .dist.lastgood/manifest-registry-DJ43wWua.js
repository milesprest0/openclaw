...
import {
  c as resolvePluginActivationDecisionShared,
  i as normalizePluginsConfigWithResolver$1,
  l as toPluginActivationState,
  n as identityNormalizePluginId,
  r as isBundledChannelEnabledByChannelConfig$1,
  s as resolveMemorySlotDecisionShared,
  t as hasExplicitPluginConfig$1,
} from "./config-normalization-shared-DVyDp1ed.js";
import {
...
import { a as normalizeOptionalTrimmedStringList } from "./string-normalization-B6JUy5-Y.js";
...
function normalizePackageChannelCommands(commands) {
  ...
  const nativeSkillsAutoEnabled =
    typeof record.nativeSkillsAutoEnabled === "boolean" ? record.nativeSkillsAutoEnabled : void 0;
...
function mergePackageChannelMetaIntoChannelConfigs(params) {
  ...
  for (const [key, value] of Object.entries(params.channelConfigs))
    if (!isBlockedObjectKey(key)) merged[key] = value;
  merged[channelId] = {
    ...existing,
    ...(label ? { label } : {}),
...
function pushProviderAuthEnvVarsCompatDiagnostic(params) {
  ...
  const setupProviderEnvVars = new Map(
    (params.record.setup?.providers ?? []).map((provider) => [
      provider.id,
      new Set(provider.envVars ?? []),
    ]),
  );
...
//#endregion
export {
  resolveMemorySlotDecision as a,
  resolveEffectivePluginActivationState as i,
  hasExplicitPluginConfig as n,
  normalizePluginsConfigWithResolver as r,
  loadPluginManifestRegistry as t,
};
