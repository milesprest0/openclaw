...
import { p as FsSafeError, r as isNotFoundPathError } from "./path-1liOXr_N.js";
...
async function assertAsyncDirectoryGuard(guard) {
  ...
  if (stat.isSymbolicLink() || !stat.isDirectory())
    throw new FsSafeError("not-file", "directory component must be a directory");
  if (!sameFileIdentity(stat, guard.stat) || (await fs$1.realpath(guard.dir)) !== guard.realPath)
...
function assertSyncDirectoryGuard(guard) {
  ...
  if (stat.isSymbolicLink() || !stat.isDirectory())
    throw new FsSafeError("not-file", "directory component must be a directory");
  if (!sameFileIdentity(stat, guard.stat) || fs.realpathSync(guard.dir) !== guard.realPath)
    throw new FsSafeError("path-mismatch", "directory changed during operation");
}
...
async function guardedRename(params) {
  await withAsyncDirectoryGuards(
    [
      await createAsyncDirectoryGuard(path.dirname(params.from)),
      params.targetRoot
...
function guardedRenameSync(params) {
  withSyncDirectoryGuards(
    [
      createSyncDirectoryGuard(path.dirname(params.from)),
      params.targetRoot
...
async function guardedRm(params) {
  await withAsyncDirectoryGuards(
    [await createAsyncDirectoryGuard(path.dirname(params.target))],
    async () => {
...
function guardedRmSync(params) {
  withSyncDirectoryGuards(
    [createSyncDirectoryGuard(path.dirname(params.target))],
    () =>
      fs.rmSync(params.target, {
        ...(params.recursive !== void 0 ? { recursive: params.recursive } : {}),
        ...(params.force !== void 0 ? { force: params.force } : {}),
      }),
    { verifyAfter: params.verifyAfter },
...
export {
  guardedRm as a,
  assertAsyncDirectoryGuard as c,
  createNearestExistingDirectoryGuard as d,
  createSyncDirectoryGuard as f,
  guardedRenameSync as i,
  assertSyncDirectoryGuard as l,
  registerTempPathForExit as n,
  guardedRmSync as o,
  guardedRename as r,
  withAsyncDirectoryGuards as s,
  serializePathWrite as t,
  createAsyncDirectoryGuard as u,
};
