...
import { t as attachChannelToResult } from "./channel-send-result-CsgaqKTY.js";
...
import { n as NostrProfileSchema, t as NostrConfigSchema } from "./config-schema-CPBAvgsl.js";
...
import {
  a as resolveNostrAccount,
  c as SimplePool,
  d as verifyEvent,
  f as cbc,
  g as randomBytes,
  h as hexToBytes,
  i as resolveDefaultNostrAccountId,
  l as finalizeEvent,
  m as secp256k1,
  n as nostrSetupWizard,
  o as normalizePubkey,
  p as base64,
  r as listNostrAccountIds,
  s as validatePrivateKey,
  t as nostrSetupAdapter,
  u as getPublicKey,
} from "./setup-surface-CrSqr7YZ.js";
import {
...
const NullableFiniteNumberSchema = zod_exports.z.number().finite().nullable().catch(null);
const NostrBusStateV1Schema = zod_exports.z.object({
  version: zod_exports.z.literal(1),
  lastProcessedAt: NullableFiniteNumberSchema,
  gatewayStartedAt: NullableFiniteNumberSchema,
});
const NostrBusStateSchema = zod_exports.z.object({
  version: zod_exports.z.literal(2),
  lastProcessedAt: NullableFiniteNumberSchema,
  gatewayStartedAt: NullableFiniteNumberSchema,
  recentEventIds: zod_exports.z
    .array(zod_exports.z.unknown())
    .catch([])
    .transform((ids) => ids.filter((id) => typeof id === "string")),
});
const NostrProfileStateSchema = zod_exports.z.object({
  version: zod_exports.z.literal(1),
  lastPublishedAt: NullableFiniteNumberSchema,
  lastPublishedEventId: zod_exports.z.string().nullable().catch(null),
  lastPublishResults: zod_exports.z
    .record(zod_exports.z.string(), zod_exports.z.enum(["ok", "failed", "timeout"]))
    .nullable()
    .catch(null),
});
...
async function startNostrBus(options) {
  ...
  const sub = pool.subscribeMany(
    ...
    ],
    {
      ...
      onclose: (reason) => {
        ...
        onError?.(
          /* @__PURE__ */ new Error(`Subscription closed: ${reason.join(", ")}`),
          "subscription",
        );
      },
    },
  ...
  const publishProfile$1 = async (profile) => {
    const result = await publishProfile(
      ...
      relays,
      profile,
      (await readNostrProfileState({ accountId }))?.lastPublishedAt ?? void 0,
    );
...
const startNostrGatewayAccount = async (ctx) => {
  ...
  const resolveInboundAccess = async (senderPubkey, rawBody) =>
  ...
  const bus = await startNostrBus({
    ...
    onMessage: async (senderPubkey, text, reply, meta) => {
      ...
      const { dispatchInboundDirectDmWithRuntime } =
        await import("./inbound-direct-dm-runtime-Dr8oqvNJ.js");
    ...
    status: {
      ...createComputedAccountStatusAdapter({
        ...
        buildChannelSummary: ({ snapshot }) =>
          buildPassiveChannelStatusSummary(snapshot, { publicKey: snapshot.publicKey ?? null }),
...
//#endregion
export {
  setNostrRuntime as a,
  getNostrRuntime as i,
  nostrPlugin as n,
  contentToProfile as o,
  publishNostrProfile as r,
  getNostrProfileState as t,
};
