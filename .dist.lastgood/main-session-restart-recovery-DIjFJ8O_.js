...
import {
  ...
  s as resolveSessionTranscriptPathInDir,
} from "./paths-CfeECf6Z.js";
import { v as resolveStateDir } from "./paths-Cnwfh6dH.js";
import { t as resolveAgentSessionDirs } from "./session-dirs-B9NgFY0D.js";
import {
...
async function resumeMainSession(params) {
  try {
    ...
    log.info(
      `resumed interrupted main session: ${params.sessionKey}${params.pendingFinalDeliveryText ? " (with pending payload)" : ""}`,
    );
    return true;
  } catch (err) {
...
async function recoverRestartAbortedMainSessions(params = {}) {
  ...
  const sessionDirs = await resolveAgentSessionDirs(
    params.stateDir ?? resolveStateDir(process.env),
  );
