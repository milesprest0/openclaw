...
import { n as saveJsonFile, t as loadJsonFile } from "./json-file-DY7oy2KR.js";
...
import { C as resolveExternalAuthProfilesWithPlugins } from "./provider-runtime-DueHRTen.js";
import {
  a as replaceRuntimeAuthProfileStoreSnapshots$1,
  d as resolveLegacyAuthStorePath,
  i as hasRuntimeAuthProfileStoreSnapshot,
  l as resolveAuthStorePath,
  n as clearRuntimeAuthProfileStoreSnapshots$1,
  o as setRuntimeAuthProfileStoreSnapshot,
  p as cloneAuthProfileStore,
  r as getRuntimeAuthProfileStoreSnapshot,
  s as resolveAuthStatePath,
} from "./source-check-DUt28S7k.js";
...
import { d as normalizeSecretInputString, o as coerceSecretRef } from "./types.secrets-CaNC1eIn.js";
...
function hasUsableOAuthCredential$1(credential, opts) {
  ...
  return (
    resolveTokenExpiryState(credential.expires, now, { expiringWithinMs: refreshMarginMs }) ===
    "valid"
  );
}
...
function hasOAuthIdentity(credential) {
  return (
    normalizeAuthIdentityToken$1(credential.accountId) !== void 0 ||
    normalizeAuthEmailToken$1(credential.email) !== void 0
  );
}
function hasMatchingOAuthIdentity(existing, incoming) {
  ...
  if (existingAccountId !== void 0 && incomingAccountId !== void 0)
...
function isSafeToUseExternalCliCredential(existing, imported) {
  ...
  if (existingAccountId !== void 0 && importedAccountId !== void 0)
...
function resolveExternalCliAuthProfiles(store, options) {
  ...
  for (const providerConfig of EXTERNAL_CLI_SYNC_PROVIDERS) {
    ...
    if (
      ...
      if (existingOAuth)
        log$1.debug("kept usable local oauth over external cli bootstrap", {
          profileId: providerConfig.profileId,
          provider: providerConfig.provider,
...
function resolveExternalAuthProfileMap(params) {
  ...
  const cliProfiles =
    resolveExternalCliAuthProfiles?.(params.store, {
...
function overlayExternalAuthProfiles(store, params) {
  return overlayRuntimeExternalOAuthProfiles(
    store,
...
function hasComparableOAuthIdentityConflict(existing, candidate) {
  ...
  if (
    existingAccountId !== void 0 &&
    candidateAccountId !== void 0 &&
...
const AUTH_INVALIDATION_REASONS = new Set(["auth", "auth_permanent", "session_expired"]);
function hasAuthInvalidationSignal(stats) {
  ...
  return Object.entries(stats.failureCounts ?? {}).some(
    ([reason, count]) =>
      AUTH_INVALIDATION_REASONS.has(reason) && typeof count === "number" && count > 0,
  );
}
...
function resolveRuntimeAuthProfileStore(agentDir) {
  ...
  if (requestedStore)
    return mergeAuthProfileStores(
      loadAuthProfileStoreForAgent(void 0, {
        readOnly: true,
...
function resolveExternalCliOverlayOptions(options) {
  ...
  if (!discovery)
    return {
      ...(options?.allowKeychainPrompt !== void 0
        ? { allowKeychainPrompt: options.allowKeychainPrompt }
        : {}),
...
function loadAuthProfileStoreForRuntime(agentDir, options) {
  ...
  return overlayExternalAuthProfiles(
    mergeAuthProfileStores(loadAuthProfileStoreForAgent(void 0, options), store),
    {
      agentDir,
      ...externalCli,
    },
...
function findPersistedAuthProfileCredential(params) {
  const requestedProfile = loadPersistedAuthProfileStore(params.agentDir)?.profiles[
    params.profileId
  ];
...
//#endregion
export {
  readGeminiCliCredentialsCached as A,
  shouldReplaceStoredOAuthCredential as C,
  resolveTokenExpiryState as D,
  hasUsableOAuthCredential$1 as E,
  OAUTH_REFRESH_LOCK_OPTIONS as F,
  log$1 as I,
  CLAUDE_CLI_PROFILE_ID as M,
  CODEX_CLI_PROFILE_ID as N,
  readClaudeCliCredentialsCached as O,
  OAUTH_REFRESH_CALL_TIMEOUT_MS as P,
  shouldBootstrapFromExternalCliCredential as S,
  evaluateStoredCredentialEligibility as T,
  readManagedExternalCliCredential as _,
  findPersistedAuthProfileCredential as a,
  isSafeToAdoptBootstrapOAuthIdentity as b,
  loadAuthProfileStoreForSecretsRuntime as c,
  resolvePersistedAuthProfileOwnerAgentDir as d,
  saveAuthProfileStore as f,
  ensureAuthStoreFile as g,
  loadPersistedAuthProfileStore as h,
  ensureAuthProfileStoreWithoutExternalProfiles as i,
  AUTH_STORE_LOCK_OPTIONS as j,
  readCodexCliCredentialsCached as k,
  loadAuthProfileStoreWithoutExternalProfiles as l,
  coercePersistedAuthProfileStore as m,
  ensureAuthProfileStore as n,
  loadAuthProfileStore as o,
  updateAuthProfileStoreWithLock as p,
  ensureAuthProfileStoreForLocalUpdate as r,
  loadAuthProfileStoreForRuntime as s,
  clearRuntimeAuthProfileStoreSnapshots as t,
  replaceRuntimeAuthProfileStoreSnapshots as u,
  areOAuthCredentialsEquivalent as v,
  DEFAULT_OAUTH_REFRESH_MARGIN_MS as w,
  isSafeToAdoptMainStoreOAuthIdentity as x,
  hasUsableOAuthCredential as y,
};
