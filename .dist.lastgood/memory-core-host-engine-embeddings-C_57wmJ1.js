...
import {
  ...
  h as splitTextToUtf8ByteLimit,
  m as estimateUtf8Bytes,
} from "./internal-BUtLJk16.js";
import { l as resolveApiKeyForProvider } from "./model-auth-Do8Bfz1M.js";
...
import { n as resolveMemorySecretInputString } from "./secret-input-DQ0B1VAn.js";
import { _ as ssrfPolicyFromHttpBaseUrlAllowedHostname } from "./ssrf-DO8eIXaD.js";
import {
  r as normalizeOptionalString,
...
async function createLocalEmbeddingProvider(options) {
  const modelPath =
    normalizeOptionalString(options.local?.modelPath) ||
    "hf:ggml-org/embeddinggemma-300m-qat-q8_0-GGUF/embeddinggemma-300m-qat-Q8_0.gguf";
...
async function postJsonWithRetry(params) {
  return await (params.retryImpl ?? retryAsync)(
    async () => {
...
function applyEmbeddingBatchOutputLine(params) {
  ...
  if ((response?.status_code ?? 0) >= 400) {
    const messageFromObject =
      response?.body && typeof response.body === "object" ? response.body.error?.message : void 0;
  ...
  const embedding =
    (response?.body && typeof response.body === "object" ? (response.body.data ?? []) : [])[0]
      ?.embedding ?? [];
...
//#endregion
export {
  withRemoteHttpResponse as C,
  sanitizeAndNormalizeEmbedding as D,
  createLocalEmbeddingProvider as E,
  DEFAULT_LOCAL_MODEL as O,
  buildRemoteBaseUrlPolicy as S,
  formatUnavailableBatchError as T,
  buildBatchHeaders as _,
  normalizeEmbeddingModelWithPrefixes as a,
  applyEmbeddingBatchOutputLine as b,
  mapBatchEmbeddingsByIndex as c,
  uploadBatchJsonlFile as d,
  resolveBatchCompletionFromStatus as f,
  runEmbeddingBatchGroups as g,
  buildEmbeddingBatchGroupOptions as h,
  resolveRemoteEmbeddingBearerClient as i,
  sanitizeEmbeddingCacheHeaders as l,
  throwIfBatchTerminalFailure as m,
  resolveRemoteEmbeddingClient as n,
  debugEmbeddingsLog as o,
  resolveCompletedBatchResult as p,
  fetchRemoteEmbeddingVectors as r,
  isMissingEmbeddingApiKeyError as s,
  createRemoteEmbeddingProvider as t,
  enforceEmbeddingMaxInputTokens as u,
  normalizeBatchBaseUrl as v,
  extractBatchErrorMessage as w,
  postJsonWithRetry as x,
  EMBEDDING_BATCH_ENDPOINT as y,
};
