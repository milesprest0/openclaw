...
import { i as resolveSignalAccount } from "./accounts-D_lQP4AN.js";
import { n as sleepWithAbort, t as computeBackoff } from "./backoff-CW9s2Y7t.js";
import { n as estimateBase64DecodedBytes } from "./base64-DEg6fWrh.js";
import {
...
import {
  n as signalRpcRequest,
  r as streamSignalEvents,
  t as signalCheck,
} from "./client-DlcnnzTH.js";
...
import {
  a as isSignalSenderAllowed,
  c as resolveSignalRecipient,
  f as normalizeSignalMessagingTarget,
  l as resolveSignalSender,
  n as formatSignalSenderDisplay,
  o as normalizeSignalAllowRecipient,
  r as formatSignalSenderId,
  s as resolveSignalPeerId,
  t as formatSignalPairingIdLine,
...
import { c as kindFromMime, n as detectMime } from "./mime-CSQ-Gv-M.js";
...
import {
  n as sendReadReceiptSignal,
  r as sendTypingSignal,
  t as sendMessageSignal,
} from "./send-C89dSUdS.js";
...
function resolveSignalQuoteContext(params) {
  ...
  return {
    ...
    visibleQuoteSender:
      decision.include && quoteSender ? formatSignalSenderDisplay(quoteSender) : void 0,
  };
}
//#endregion
              ...
              return await dispatchInboundMessage({
                ...
                replyOptions: {
                  ...
                  disableBlockStreaming:
                    typeof deps.blockStreaming === "boolean" ? !deps.blockStreaming : void 0,
                  onModelSelected,
                },
              });
            } finally {
  ...
  return async (event) => {
    ...
    const reaction = deps.isSignalReactionMessage(envelope.reactionMessage)
      ...
      : deps.isSignalReactionMessage(dataMessage?.reaction)
        ...
        : null;
    const messageText = renderSignalMentions(
      dataMessage?.message ?? "",
    ...
    if (!isGroup) {
      if (
        !(await handleSignalDirectMessageAccess({
          dmPolicy: deps.dmPolicy,
    ...
    if (isGroup && requireMention && canDetectMention && mentionDecision.shouldSkip) {
      ...
      const pendingPlaceholder = (() => {
        ...
        const attachmentTypes = (dataMessage.attachments ?? []).map((attachment) =>
          typeof attachment?.contentType === "string" ? attachment.contentType : void 0,
        );
          ...
          triggerInternalHook(
            createInternalHookEvent(
              ...
              toInternalMessageReceivedContext({
                ...
                messageId:
                  typeof envelope.timestamp === "number" ? String(envelope.timestamp) : void 0,
...
function deriveSignalAttachmentRpcMaxResponseBytes(maxBytes) {
  ...
  return (
    Math.ceil((maxBytes * SIGNAL_BASE64_OVERHEAD_NUMERATOR) / SIGNAL_BASE64_OVERHEAD_DENOMINATOR) +
    SIGNAL_ATTACHMENT_RPC_RESPONSE_HEADROOM_BYTES
  );
}
async function fetchAttachment(params) {
  ...
  const saved = await saveMediaBuffer(
    buffer,
    normalizeOptionalString(attachment.contentType ?? void 0) ??
      (await detectMime({
        buffer,
...
async function monitorSignalProvider(opts = {}) {
  ...
  const historyLimit = Math.max(
    0,
    accountInfo.config.historyLimit ?? cfg.messages?.groupChat?.historyLimit ?? 50,
  );
  ...
  const startupTimeoutMs = Math.min(
    12e4,
    Math.max(1e3, opts.startupTimeoutMs ?? accountInfo.config.startupTimeoutMs ?? 3e4),
  );
