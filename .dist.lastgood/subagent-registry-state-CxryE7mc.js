...
function getSubagentSessionRuntimeMs(entry, now = Date.now()) {
  ...
  const accumulatedRuntimeMs =
    typeof entry.accumulatedRuntimeMs === "number" && Number.isFinite(entry.accumulatedRuntimeMs)
      ...
      : 0;
  if (typeof entry.startedAt !== "number" || !Number.isFinite(entry.startedAt))
    return entry.accumulatedRuntimeMs != null ? accumulatedRuntimeMs : void 0;
...
function resolveStaleCutoffMs(entry) {
  ...
  if (typeof timeoutSeconds === "number" && Number.isFinite(timeoutSeconds) && timeoutSeconds > 0)
    return Math.max(
      ...
      Math.floor(timeoutSeconds) * 1e3 + EXPLICIT_TIMEOUT_STALE_GRACE_MS,
    );
...
function listRunsForRequesterFromRuns(runs, requesterSessionKey, options) {
  ...
  const requesterRunMatchesScope =
    requesterRun && requesterRun.childSessionKey === key ? requesterRun : void 0;
...
function countPendingDescendantRunsInternal(runs, rootSessionKey, excludeRunId) {
  ...
  if (
    !forEachDescendantRun(runs, rootSessionKey, (runId, entry) => {
      ...
      if (
        (runEnded ? !cleanupCompleted : isLiveUnendedSubagentRun(entry)) &&
        runId !== excludedRunId
      )
        count += 1;
    })
...
//#endregion
export {
  SUBAGENT_TARGET_KIND_SUBAGENT as A,
  resolveSubagentSessionStatus as C,
  SUBAGENT_ENDED_REASON_COMPLETE as D,
  SUBAGENT_ENDED_OUTCOME_TIMEOUT as E,
  SUBAGENT_ENDED_REASON_ERROR as O,
  getSubagentSessionStartedAt as S,
  SUBAGENT_ENDED_OUTCOME_KILLED as T,
  hasSubagentRunEnded as _,
  countActiveDescendantRunsFromRuns as a,
  shouldKeepSubagentRunChildLink as b,
  countPendingDescendantRunsFromRuns as c,
  listDescendantRunsForRequesterFromRuns as d,
  listRunsForControllerFromRuns as f,
  RECENT_ENDED_SUBAGENT_CHILD_SESSION_MS as g,
  shouldIgnorePostCompletionAnnounceForSessionFromRuns as h,
  buildSubagentRunReadIndexFromRuns as i,
  subagentRuns as j,
  SUBAGENT_ENDED_REASON_KILLED as k,
  getSubagentRunByChildSessionKeyFromRuns as l,
  resolveRequesterForChildSessionFromRuns as m,
  persistSubagentRunsToDisk as n,
  countActiveRunsForSessionFromRuns as o,
  listRunsForRequesterFromRuns as p,
  restoreSubagentRunsFromDisk as r,
  countPendingDescendantRunsExcludingRunFromRuns as s,
  getSubagentRunsSnapshotForRead as t,
  isSubagentSessionRunActiveFromRuns as u,
  isLiveUnendedSubagentRun as v,
  SUBAGENT_ENDED_OUTCOME_ERROR as w,
  getSubagentSessionRuntimeMs as x,
  isStaleUnendedSubagentRun as y,
};
