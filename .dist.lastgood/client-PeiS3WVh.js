...
import { s as resolveCodexAppServerRuntimeOptions } from "./config-CNEUyjZv.js";
import { t as log } from "./logger-Ce0PtXjG.js";
import { n as isRpcResponse } from "./protocol-DayKrDMy.js";
...
function resolveCodexAppServerSpawnEnv(
  options,
  baseEnv = process.env,
  platform = process.platform,
) {
  ...
  if (platform === "win32") {
    ...
    for (const candidate of Object.keys(env))
      if (lowerCaseKeysToClear.has(candidate.toLowerCase())) delete env[candidate];
...
function hasCodexAppServerTransportExited(child) {
  return child.exitCode !== null && child.exitCode !== void 0
    ? true
    : child.signalCode !== null && child.signalCode !== void 0;
}
  ...
  request(method, params, options) {
    ...
    if (this.closed)
      return Promise.reject(
        this.closeError ?? /* @__PURE__ */ new Error("codex app-server client is closed"),
      );
    if (options.signal?.aborted)
      return Promise.reject(/* @__PURE__ */ new Error(`${method} aborted`));
    ...
    return new Promise((resolve, reject) => {
      ...
      if (options.timeoutMs && Number.isFinite(options.timeoutMs) && options.timeoutMs > 0) {
        timeout = setTimeout(
          () => rejectPending(/* @__PURE__ */ new Error(`${method} timed out`)),
...
function assertSupportedCodexAppServerVersion(response) {
  ...
  if (compareVersions(detectedVersion, "0.125.0") < 0)
    throw new Error(
...
function parseVersionForComparison(version) {
  ...
  return {
    parts: (prereleaseIndex >= 0 ? withoutBuild.slice(0, prereleaseIndex) : withoutBuild)
      .split(".")
...
function buildCodexAppServerExitError(code, signal, stderrTail) {
  ...
  return /* @__PURE__ */ new Error(
    `codex app-server exited: code=${formatExitValue(code)} signal=${formatExitValue(signal)}${suffix}`,
  );
}
...
//#endregion
export {
  MANAGED_CODEX_APP_SERVER_PACKAGE as a,
  isCodexAppServerConnectionClosedError as i,
  CodexAppServerRpcError as n,
  resolveCodexAppServerSpawnEnv as o,
  isCodexAppServerApprovalRequest as r,
  CodexAppServerClient as t,
};
