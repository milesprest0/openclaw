...
import { i as listRouteBindings } from "./bindings-CWpQqxjp.js";
import {
  n as getBundledChannelPlugin,
  o as hasBundledChannelPackageSetupFeature,
} from "./bundled-Ca60V8y0.js";
...
import {
  ...
  t as formatChannelAccountsDefaultPath,
} from "./default-account-warnings-C3ZhVtNz.js";
import {
  a as stripUnknownConfigKeys,
  r as noteOpencodeProviderOverrides,
} from "./doctor-config-analysis-DMF48Itz.js";
import { t as runDoctorConfigPreflight } from "./doctor-config-preflight-Bw52_p0G.js";
import { t as DEFAULT_GOOGLE_API_BASE_URL } from "./google-api-base-url-C4iUAxlI.js";
import { r as normalizeChatChannelId } from "./ids-D_WQYNuU.js";
import { L as resolveNormalizedProviderModelMaxTokens } from "./io-CEQSCTGy.js";
import { n as formatConfigIssueLines } from "./issue-format-DZoENRCj.js";
import {
  n as applyChannelDoctorCompatibilityMigrations,
  t as migrateLegacyConfig,
} from "./legacy-config-migrate-_1JrcyqD.js";
import {
  a as mergeMissing,
  i as migrateLegacyXSearchConfig,
  r as migrateLegacyWebSearchConfig,
} from "./legacy-config-migrations-Dt9zuMUJ.js";
import {
  n as ensureRecord,
  r as hasOwnKey,
  t as cloneRecord,
} from "./legacy-config-record-shared-C2deO9Ew.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-BHEbqf7R.js";
import { n as collectConfiguredModelRefs } from "./model-refs-42YDfhLg.js";
import { r as migrateLegacyRuntimeModelRef } from "./model-runtime-aliases-BTwCXkkK.js";
import { t as note } from "./note-DvJ3bfgI.js";
import { t as asObjectRecord } from "./object-CFkMKtRh.js";
import { i as setPathExistingStrict } from "./path-utils-zSv0Iydy.js";
import { t as CONFIG_PATH } from "./paths-Cnwfh6dH.js";
import { r as normalizeProviderId$1 } from "./provider-id-CG9pXYPs.js";
import { n as getLoadedChannelPlugin } from "./registry-D3zb3mnd.js";
import { o as runPluginSetupConfigMigrations } from "./setup-registry-CnJdT1HM.js";
import {
...
import { r as normalizeTalkSection } from "./talk-CZDwRYpg.js";
import { n as discoverConfigSecretTargets } from "./target-registry-BECxoPqR.js";
import {
  i as LEGACY_SECRETREF_ENV_MARKER_PREFIX,
  p as parseLegacySecretRefEnvMarker,
} from "./types.secrets-CaNC1eIn.js";
import { c as isRecord$1 } from "./utils-BGRcpLKt.js";
...
function resolveSingleAccountKeysToMove(params) {
  ...
  const entries = Object.entries(params.channel)
    .filter(
      ([key, value]) =>
        key !== "accounts" && key !== "defaultAccount" && key !== "enabled" && value !== void 0,
    )
    .map(([key]) => key);
...
function isLegacyModelsAddCodexMetadataModel(params) {
  ...
  return (
    ...
    model.contextWindow === 4e5 &&
    model.contextTokens === 272e3 &&
    model.maxTokens === 128e3
  );
}
//#endregion
...
function buildLegacyRealtimeTalkCompat(talk, normalizedTalk) {
  ...
  const compat = {};
  for (const key of ["model", "voice", "mode", "transport", "brain"])
    if (talk[key] !== void 0) compat[key] = talk[key];
...
function normalizeMissingGroupVisibleRepliesDefault(cfg, changes) {
  ...
  if (
    ...
    messages?.visibleReplies !== void 0 ||
    messages?.groupChat?.visibleReplies !== void 0
  )
    return cfg;
...
function modelEntryWithRuntimePolicy(entry, runtime) {
  ...
  const currentRuntime = isRecord$1(base.agentRuntime)
...
function normalizeLegacyRuntimeAllowlistModels(rawModels, selectedRuntime) {
  if (!selectedRuntime || !isRecord$1(rawModels))
    return {
      value: rawModels,
      changed: false,
    };
  ...
  if (Array.isArray(rawAgents.list)) {
    const nextList = rawAgents.list.map((entry, index) => {
      ...
      const agent = normalizeLegacyRuntimeAgentContainer(
        entry,
        agentId ? `agents.list.${sanitizeForLog(agentId)}` : `agents.list[${index}]`,
        changes,
      );
      if (agent.changed) {
        changed = true;
...
function normalizeLegacyNanoBananaSkill(cfg, changes) {
  ...
  const legacyEnvApiKey =
    normalizeOptionalString(
      (isRecord$1(rawLegacyEntry.env) ? rawLegacyEntry.env : void 0)?.GEMINI_API_KEY,
    ) ?? "";
  const legacyApiKey =
    legacyEnvApiKey ||
    (typeof rawLegacyEntry.apiKey === "string"
      ? normalizeOptionalString(rawLegacyEntry.apiKey)
      : rawLegacyEntry.apiKey && isRecord$1(rawLegacyEntry.apiKey)
...
function normalizeLegacyCrossContextMessageConfig(cfg, changes) {
  ...
  if (legacyAllowCrossContextSend) {
    const rawCrossContext = isRecord$1(nextMessage.crossContext)
...
function migrateLegacyDeepgramCompat(params) {
  const rawCompat = isRecord$1(params.owner.deepgram)
  ...
  const currentProviderOptions = isRecord$1(params.owner.providerOptions)
    ? structuredClone(params.owner.providerOptions)
    : {};
  const currentDeepgram = isRecord$1(currentProviderOptions.deepgram)
...
function normalizeLegacyMediaProviderOptions(cfg, changes) {
  ...
  for (const capability of ["audio", "image", "video"]) {
    const config = isRecord$1(nextMedia[capability])
...
function migratePluginWebFetchConfig(params) {
  const entry = ensureRecord(
    ensureRecord(ensureRecord(params.root, "plugins"), "entries"),
    "firecrawl",
  );
...
function migrateLegacyWebFetchConfig(raw) {
  if (!isRecord$1(raw) || !hasMappedLegacyWebFetchConfig(raw))
    return {
      config: raw,
      changes: [],
    };
...
async function loadAndMaybeMigrateDoctorConfig(params) {
  ...
  const sourceLastTouchedVersion =
    typeof sourceMeta?.lastTouchedVersion === "string" ? sourceMeta.lastTouchedVersion : void 0;
  ...
  const { applyPluginAutoEnable } = await import("./plugin-auto-enable-BFJ4hbTO.js");
  ...
  const { collectBundledProviderAllowlistPolicyWarnings, collectPluginToolAllowlistWarnings } =
    await import("./plugin-tool-allowlist-warnings-28r9yf4f.js");
  ...
  if (hasConfiguredChannels) {
    const channelDoctor = await import("./channel-doctor-Cpw9O1hU.js");
  ...
  if (shouldRepair) {
    const { runDoctorRepairSequence } = await import("./repair-sequencing-C7CXapev.js");
    ...
    const { collectDoctorPreviewWarnings } = await import("./preview-warnings-CkcQBccI.js");
    emitDoctorNotes({
      note,
