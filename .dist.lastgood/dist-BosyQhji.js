...
function B() {
  let e = typeof process != "undefined" ? process : void 0,
    r = (e == null ? void 0 : e.env) || {},
    a = r.FORCE_TTY !== "false",
    l = (e == null ? void 0 : e.argv) || [];
  return (
    (!("NO_COLOR" in r || l.includes("--no-color")) &&
      ("FORCE_COLOR" in r ||
        l.includes("--color") ||
...
function printImmutableEntries(val, config, indentation, depth, refs, printer, type) {
  return ++depth > config.maxDepth
    ...
    : `${getImmutableName(type) + SPACE}{${printIteratorEntries(val.entries(), config, indentation, depth, refs, printer)}}`;
}
...
function printImmutableRecord(val, config, indentation, depth, refs, printer) {
  ...
  return ++depth > config.maxDepth
    ? printAsLeaf(name)
    : `${name + SPACE}{${printIteratorEntries(getRecordEntries(val), config, indentation, depth, refs, printer)}}`;
}
...
function printImmutableValues(val, config, indentation, depth, refs, printer, type) {
  return ++depth > config.maxDepth
    ? printAsLeaf(getImmutableName(type))
    : `${getImmutableName(type) + SPACE}[${printIteratorValues(val.values(), config, indentation, depth, refs, printer)}]`;
}
...
const serialize$1 = (element, config, indentation, depth, refs, printer) =>
  ++depth > config.maxDepth
    ? printElementAsLeaf(getType$2(element), config)
    : printElement(
        getType$2(element),
        printProps(
...
const testSymbol =
  typeof Symbol === "function" && Symbol.for ? Symbol.for("react.test.json") : 245830487;
...
function printComplexValue(val, config, indentation, depth, refs, hasCalledToJSON) {
  ...
  if (isToStringedArrayType(toStringed))
    return hitMaxDepth
      ...
      : `${min ? "" : !config.printBasicPrototype && val.constructor.name === "Array" ? "" : `${val.constructor.name} `}[${printListItems(val, config, indentation, depth, refs, printer)}]`;
  ...
  return hitMaxDepth || isWindow(val)
    ...
    : `${min ? "" : !config.printBasicPrototype && getConstructorName(val) === "Object" ? "" : `${getConstructorName(val)} `}{${printObjectProperties(val, config, indentation, depth, refs, printer)}}`;
}
const ErrorPlugin = {
  ...
  serialize(val, config, indentation, depth, refs, printer) {
    ...
    return hitMaxDepth
      ? `[${name}]`
      : `${name} {${printIteratorEntries(Object.entries(entries).values(), config, indentation, depth, refs, printer)}}`;
  },
};
...
const stringEscapeChars = /* @__PURE__ */ new RegExp(
  "['\\u0000-\\u001f\\u007f-\\u009f\\u00ad\\u0600-\\u0604\\u070f\\u17b4\\u17b5\\u200c-\\u200f\\u2028-\\u202f\\u2060-\\u206f\\ufeff\\ufff0-\\uffff]",
  "g",
);
...
const toStringTag =
  typeof Symbol !== "undefined" && Symbol.toStringTag ? Symbol.toStringTag : false;
...
function baseFormat(args, options = {}) {
  ...
  for (let x = args[i]; i < len; x = args[++i])
    if (x === null || typeof x !== "object") str += ` ${typeof x === "symbol" ? x.toString() : x}`;
...
function filterOutComments(s) {
  ...
  for (let i = 0; i < s.length; ++i)
    if (commentState === "singleline") {
      ...
      if (s[i] === "/" && s[i + 1] === "/") commentState = "singleline";
      else if (s[i] === "/" && s[i + 1] === "*") {
...
function requireBuild() {
  ...
  const extendPathsF = (d, aEnd, bEnd, bF, isCommon, aIndexesF, iMaxF) => {
    ...
    for (iF += 1, kF += 2; iF <= nF; iF += 1, kF += 2) {
      ...
      aIndexesF[iF] =
        aFirst + countCommonItemsF(aFirst + 1, aEnd, bF + aFirst - kF + 1, bEnd, isCommon);
    }
    return iMaxF;
  };
  const extendPathsR = (d, aStart, bStart, bR, isCommon, aIndexesR, iMaxR) => {
    ...
    for (iR += 1, kR -= 2; iR <= nR; iR += 1, kR -= 2) {
      ...
      aIndexesR[iR] =
        aFirst - countCommonItemsR(aStart, aFirst - 1, bStart, bR + aFirst - kR - 1, isCommon);
    }
    return iMaxR;
  };
  ...
  if (a !== b && a.length !== 0 && b.length !== 0) {
    ...
    if (hasCommonDiff(diffs, isMultiline)) {
      ...
      return printDiffLines(
        getAlignedDiffs(diffs, optionsNormalized.changeColor),
        truncated,
        optionsNormalized,
      );
    }
...
function printDiffOrStringify(received, expected, options) {
  ...
  const { replacedExpected, replacedActual } = replaceAsymmetricMatcher(
    deepClone(received, { forceWritable: true }),
    clonedExpected,
  );
...
var TestFixtures = class TestFixtures {
  ...
  static _fixtureOptionKeys = ["auto", "injected", "scope"];
  static _fixtureScopes = ["test", "file", "worker"];
  ...
  static isFixtureOptions(obj) {
    return (
      isObject(obj) && Object.keys(obj).some((key) => TestFixtures._fixtureOptionKeys.includes(key))
    );
  }
  ...
  parseUserFixtures(
    ...
    userFixtures,
    supportNonTest,
    registrations = new Map(this._registrations),
  ) {
    const errors = [];
    Object.entries(userFixtures).forEach(([name, fn]) => {
      ...
      if (isFixtureFunction(value))
        Object.assign(value, {
          [FIXTURE_STACK_TRACE_KEY]: /* @__PURE__ */ new Error("STACK_TRACE_ERROR"),
        });
    ...
    if (options?.suiteHook) {
      ...
      if (testScopedFixtures.length > 0) {
        ...
        const error = new FixtureDependencyError(
          `Test-scoped fixtures cannot be used inside ${options.suiteHook} hook. The following fixtures are test-scoped: ${fixtureNames}. Use { scope: 'file' } or { scope: 'worker' } fixtures instead, or move the logic to ${
            {
...
function resolveDeps(
  ...
  usedFixtures.forEach((fixture) => {
    ...
    if (depSet.has(fixture))
      if (fixture.parent) fixture = fixture.parent;
      else
        throw new Error(
    ...
    resolveDeps(
      [...fixture.deps]
        .map((n) => (n === fixture.name ? fixture.parent : registrations.get(n)))
        .filter((n) => !!n),
      registrations,
      depSet,
      pendingFixtures,
...
function getUsedProps(fn, { sourceError, suiteHook } = {}) {
  ...
  const { index: fixturesIndex = 0, original: implementation = fn } =
    kPropsSymbol in fn ? fn[kPropsSymbol] : {};
  ...
  if (!(fixturesArgument[0] === "{" && fixturesArgument.endsWith("}"))) {
    ...
    const error = new FixtureParseError(
      `The ${ordinalArgument} argument inside a fixture must use object destructuring pattern, e.g. ({ task } => {}). Instead, received "${fixturesArgument}".${suiteHook ? ` If you used internal "suite" task as the ${ordinalArgument} argument previously, access it in the ${ordinal(fixturesIndex + 2)} argument instead.` : ""}`,
    );
...
function afterEach(fn, timeout) {
  ...
  return getCurrentSuite().on(
    ...
    withTimeout(
      ...
      timeout ?? getDefaultHookTimeout(),
      true,
      /* @__PURE__ */ new Error("STACK_TRACE_ERROR"),
      abortIfTimeout,
    ),
  );
}
...
function aroundAll(fn, timeout) {
  ...
  return getCurrentSuite().on(
    "aroundAll",
    Object.assign(withSuiteFixtures("aroundAll", fn, context, stackTraceError, 1), {
...
function createSuiteCollector(name, factory = () => {}, mode, each, suiteOptions) {
  ...
  const task = function (name = "", options = {}) {
    ...
    const testTags = unique([
      ...((currentSuite ?? collectorContext.currentSuite?.file)?.tags || []),
...
function withAwaitAsyncAssertions(fn, task) {
  return async (...args) => {
    ...
    if (task.promises) {
      const errors = (await Promise.allSettled(task.promises))
        .map((r) => (r.status === "rejected" ? r.reason : void 0))
        .filter(Boolean);
        ...
        if (fnFirst)
          if (arrayOnlyCases)
            suite(
              ...
              handler ? () => handler(...items) : void 0,
              options.timeout,
            );
          else
            suite(
              formatTitle(_name, items, idx),
              handler ? () => handler(i) : void 0,
              options.timeout,
            );
        else if (arrayOnlyCases)
          suite(
            formatTitle(_name, items, idx),
            options,
            handler ? () => handler(...items) : void 0,
...
function createTaskCollector(fn) {
  ...
  return createChainable(["concurrent", "sequential", "skip", "only", "todo", "fails"], taskFn, {
...
function formatTitle(template, items, idx) {
  ...
  if (template.includes("%f"))
    (template.match(/%f/g) || []).forEach((_, i) => {
  ...
  function formatAttribute(s) {
    return s.replace(/\$([$\w.]+)/g, (_, key) => {
      ...
      return objDisplay(isObjectItem ? objectAttr(items[0], key, arrayElement) : arrayElement, {
...
async function recordArtifact(task, artifact) {
  ...
  const stack = findTestFileStackTrace(
    task.file.filepath,
    /* @__PURE__ */ new Error("STACK_TRACE").stack,
  );
  if (stack) {
    artifact.location = {
      file: stack.file,
  ...
  if (Array.isArray(artifact.attachments))
    for (const attachment of artifact.attachments) manageArtifactAttachment(attachment);
...
export {
  ...
  getSafeTimers as f,
  assertTypes as g,
  printDiffOrStringify as h,
  beforeEach as i,
  parseErrorStacktrace as l,
  diff as m,
  afterEach as n,
  getCurrentTest as o,
  processError as p,
  ...
  getNames as s,
  afterAll as t,
  parseSingleStack as u,
  getCallLastIndex as v,
  format$1 as w,
  noop as x,
  getType$1 as y,
};
