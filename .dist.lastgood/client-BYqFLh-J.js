...
import {
  l as readConnectErrorDetailCode,
  s as formatConnectErrorMessage,
  t as ConnectErrorDetailCodes,
  u as readConnectErrorRecoveryAdvice,
} from "./connect-error-details-BXtaJDEs.js";
import {
  n as normalizeDeviceAuthScopes,
  t as normalizeDeviceAuthRole,
} from "./device-auth-BTSFehem.js";
import {
  a as publicKeyRawBase64UrlFromPem,
  o as signDevicePayload,
...
import { t as normalizeFingerprint } from "./fingerprint-LTrbVi4e.js";
import { t as resolveConnectChallengeTimeoutMs } from "./handshake-timeouts-BWfTnhrv.js";
import { n as logError, t as logDebug } from "./logger-BWdMCYqK.js";
import { i as isLoopbackHost, s as isSecureWebSocketUrl } from "./net-BQYp2xgJ.js";
...
import {
  ...
  J as validateEventFrame,
  Ot as validateResponseFrame,
} from "./protocol-CUn5aLrV.js";
import {
  a as withManagedProxyGatewayLoopbackRouting,
  t as ensureInheritedManagedProxyRoutingActive,
} from "./proxy-lifecycle-CBIKx2Il.js";
...
function isRetryableGatewayStartupUnavailableError(error) {
  ...
  const shaped = error;
  return (
    (shaped.gatewayCode ?? shaped.code) === "UNAVAILABLE" &&
...
function resolveGatewayStartupRetryAfterMs(error) {
  ...
  return Math.min(
    Math.max(
      Math.floor(
        typeof retryAfterMs === "number" && Number.isFinite(retryAfterMs) ? retryAfterMs : 500,
      ),
      GATEWAY_STARTUP_RETRY_MIN_MS,
    ),
...
var GatewayClient = class {
  ...
  start() {
    ...
    if (this.opts.tlsFingerprint && !url.startsWith("wss://")) {
      this.opts.onConnectError?.(
        /* @__PURE__ */ new Error("gateway tls fingerprint requires wss:// gateway url"),
      );
      return;
    }
    ...
    if (url.startsWith("wss://") && this.opts.tlsFingerprint) {
      ...
      wsOptions.checkServerIdentity = (_host, cert) => {
        ...
        if (fingerprint !== expected)
          return /* @__PURE__ */ new Error("Server TLS fingerprint mismatch");
      };
    }
  ...
  sendConnect() {
    ...
    if (!nonce) {
      this.opts.onConnectError?.(
        /* @__PURE__ */ new Error("gateway connect challenge missing nonce"),
      );
    ...
    this.request("connect", params)
      .then((helloOk) => {
        ...
        this.backoffMs = 1e3;
        this.tickIntervalMs =
          typeof helloOk.policy?.tickIntervalMs === "number" ? helloOk.policy.tickIntervalMs : 3e4;
  ...
  startTickWatch() {
    ...
    const minInterval =
      typeof rawMinInterval === "number" && Number.isFinite(rawMinInterval)
        ? Math.max(1, Math.min(3e4, rawMinInterval))
        : 1e3;
  ...
  validateTlsFingerprint() {
    ...
    if (!socket || typeof socket.getPeerCertificate !== "function")
      return /* @__PURE__ */ new Error("gateway tls fingerprint unavailable");
    ...
    if (fingerprint !== expected)
      return /* @__PURE__ */ new Error("gateway tls fingerprint mismatch");
    return null;
  }
...
//#endregion
export {
  resolveGatewayClientConnectChallengeTimeoutMs as a,
  buildDeviceAuthPayloadV3 as c,
  describeGatewayCloseCode as i,
  loadDeviceAuthToken as l,
  GatewayClient as n,
  gatewayStartupUnavailableDetails as o,
  GatewayClientRequestError as r,
  buildDeviceAuthPayload as s,
  GATEWAY_CLOSE_CODE_HINTS as t,
};
