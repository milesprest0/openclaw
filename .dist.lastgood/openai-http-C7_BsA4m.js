...
import {
  ...
  r as DEFAULT_INPUT_TIMEOUT_MS,
  t as DEFAULT_INPUT_IMAGE_MAX_BYTES,
...
import {
  ...
  l as toOpenAiChatCompletionsUsage,
  o as normalizeUsage,
...
function parseImageUrlToSource(url) {
  ...
  if (dataUriMatch) {
    ...
    const metadataParts = metadata
      ...
      .filter(Boolean);
    if (!metadataParts.some((part) => normalizeLowercaseStringOrEmpty(part) === "base64"))
...
function resolveAgentResponseText(result) {
  ...
  if (!Array.isArray(payloads) || payloads.length === 0) return "No response received.";
  return (
    payloads
