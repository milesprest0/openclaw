...
import { i as EndOfStreamError, r as fromBuffer } from "./lib-Dy7Qkhjs.js";
...
var AIFFParser = class extends BasicParser {
  ...
  async parse() {
    if ((await this.tokenizer.readToken(Header)).chunkID !== "FORM")
    ...
    switch (header.chunkID) {
      ...
      case "ID3 ": {
        const rst = fromBuffer(
          await this.tokenizer.readToken(new Uint8ArrayType(header.chunkSize)),
        );
  ...
  async readTextChunk(header) {
    const values = (await this.tokenizer.readToken(new StringType(header.chunkSize, "ascii")))
      .split("\0")
      .map((v) => v.trim())
      .filter((v) => v?.length);
