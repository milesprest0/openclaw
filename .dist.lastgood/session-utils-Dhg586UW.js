...
import {
  d as resolveAvatarMime,
  i as isAvatarHttpUrl,
  l as isWorkspaceRelativeAvatarPath,
  o as isPathWithinRoot,
  r as isAvatarDataUrl,
  t as AVATAR_MAX_BYTES,
} from "./avatar-policy-DZepcfzt.js";
import {
  a as resolveSessionStoreKey,
  i as resolveSessionStoreAgentId,
  o as resolveStoredSessionKeyForAgentStore,
} from "./combined-store-gateway-CtdiSumZ.js";
import { a as resolveContextTokensForModel, i as lookupContextTokens } from "./context-BNAuEVrI.js";
import "./boundary-file-read-CkGowAmx.js";
import { n as DEFAULT_MODEL, r as DEFAULT_PROVIDER } from "./defaults-4m7JJmD2.js";
import { a as normalizeSessionDeliveryFields } from "./delivery-context.shared-mj-12Z6_.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import { _ as projectPluginSessionExtensionsSync } from "./loader-BLowS1kl.js";
...
import { c as normalizeAgentId, l as normalizeMainKey } from "./session-key-B4qUwRzq.js";
import {
  o as parseAgentSessionKey,
  r as isCronRunSessionKey,
} from "./session-key-utils-B3KPN8Ee.js";
import {
  d as readSessionTitleFieldsFromTranscript,
  f as readSessionTitleFieldsFromTranscriptAsync,
  s as readRecentSessionUsageFromTranscript,
} from "./session-utils.fs-z-7Kw0Lg.js";
import { b as buildGroupDisplayName } from "./store-BpWdoYPF.js";
import "./model-catalog-CJYpfYgw.js";
...
import {
  a as isSubagentRunLive,
  i as getSessionDisplaySubagentRunByChildSessionKey,
  n as countActiveDescendantRuns,
  s as listSubagentRunsForController,
  t as buildSubagentRunReadIndex,
} from "./subagent-registry-read-Cr9HxsBK.js";
import {
  C as resolveSubagentSessionStatus,
  S as getSubagentSessionStartedAt,
  b as shouldKeepSubagentRunChildLink,
  x as getSubagentSessionRuntimeMs,
} from "./subagent-registry-state-CxryE7mc.js";
import { r as resolveAllAgentSessionStoreTargetsSync } from "./targets-CSZl-lN0.js";
import { i as listThinkingLevelOptions } from "./thinking-C9_Z62px.js";
import { o as resolveFreshSessionTotalTokens } from "./types-C7lm-riv.js";
import { a as resolveModelCostConfig, n as estimateUsageCost } from "./usage-format-DQx7gWeg.js";
...
function formatSessionIdPrefix(sessionId, updatedAt) {
  ...
  if (updatedAt && updatedAt > 0)
    return `${prefix} (${new Date(updatedAt).toISOString().slice(0, 10)})`;
  return prefix;
}
...
function deriveSessionTitle(entry, firstUserMessage) {
  ...
  if (firstUserMessage?.trim())
    return truncateTitle(firstUserMessage.replace(/\s+/g, " ").trim(), DERIVED_TITLE_MAX_LEN);
  ...
  for (const entry of runs) {
    ...
    if (
      (normalizeOptionalString(latest?.controllerSessionKey) ||
        normalizeOptionalString(latest?.requesterSessionKey)) !== controllerSessionKey
    )
      continue;
    if (
...
function mergeChildSessionKeys(runtimeChildSessions, storeChildSessions) {
  if (!runtimeChildSessions?.length)
    return storeChildSessions?.length ? storeChildSessions : void 0;
...
function resolveChildSessionKeys(controllerSessionKey, store, now = Date.now(), subagentRuns) {
  return mergeChildSessionKeys(
    resolveRuntimeChildSessionKeys(controllerSessionKey, now, subagentRuns),
    buildStoreChildSessionIndex(store, now, subagentRuns).get(controllerSessionKey),
  );
}
...
function pruneLegacyStoreKeys(params) {
  ...
  for (const candidate of params.candidates) {
    ...
    for (const match of findStoreKeysIgnoreCase(params.store, trimmed))
      if (match !== params.canonicalKey) keysToDelete.add(match);
  }
...
function resolveGatewaySessionStoreCandidates(cfg, agentId) {
  ...
  for (const target of resolveAllAgentSessionStoreTargetsSync(cfg))
    if (target.agentId === agentId) targets.set(target.storePath, target);
...
function resolveExplicitDeletedLegacyMainStoreTarget(params) {
  ...
  if (!parsed || legacyAgentId !== "main" || listAgentIds(params.cfg).includes(legacyAgentId))
    return null;
...
function resolveGatewaySessionStoreTarget(params) {
  ...
  if (params.scanLegacyKeys !== false) {
    ...
    for (const seed of scanTargets)
      for (const legacyKey of findStoreKeysIgnoreCase(store, seed)) storeKeys.add(legacyKey);
  }
  return {
    agentId,
    storePath,
...
function resolveGatewaySessionThinkingDefault(params) {
  return (
    (params.agentId ? resolveAgentConfig(params.cfg, params.agentId)?.thinkingDefault : void 0) ??
...
async function resolveGatewayModelSupportsImages(params) {
  ...
  try {
    const modelEntry = findModelCatalogEntry(
      await params.loadGatewayModelCatalog({ readOnly: false }),
      {
        provider: params.provider,
...
function buildGatewaySessionRow(params) {
  ...
  const sessionAgentId = normalizeAgentId(
    parseAgentSessionKey(key)?.agentId ?? resolveDefaultAgentId(cfg),
  );
  ...
  const needsTranscriptTotalTokens =
    resolvePositiveNumber(resolveFreshSessionTotalTokens(entry)) === void 0;
...
function sortAndLimitSessionEntries(entries, limit) {
  if (limit !== void 0 && limit <= SESSIONS_LIST_TOP_N_LIMIT)
...
function listSessionsFromStore(params) {
  ...
  const { entries, totalCount, limitApplied } = selectSessionEntries({
    ...
    now,
    rowContext:
      typeof opts.spawnedBy === "string" && opts.spawnedBy.length > 0 ? getRowContext() : void 0,
...
//#endregion
export {
  resolveSessionModelIdentityRef as _,
  listSessionsFromStore as a,
  loadSessionEntry as c,
  resolveDeletedAgentIdFromSessionKey as d,
  resolveFreshestSessionEntryFromStoreKeys as f,
  resolveSessionDisplayModelIdentityRef as g,
  resolveGatewaySessionThinkingDefault as h,
  listAgentsForGateway as i,
  migrateAndPruneGatewaySessionStoreKey as l,
  resolveGatewaySessionStoreTarget as m,
  deriveSessionTitle as n,
  listSessionsFromStoreAsync as o,
  resolveGatewayModelSupportsImages as p,
  filterAndSortSessionEntries as r,
  loadGatewaySessionRow as s,
  buildGatewaySessionRow as t,
  pruneLegacyStoreKeys as u,
  resolveSessionModelRef as v,
};
