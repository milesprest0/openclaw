...
import { o as success, t as danger } from "./globals-BdfwDi2E.js";
import "./cli-runtime-CnA0DtN4.js";
import "./fetch-runtime-D5GYCOX7.js";
import { d as toPinoLikeLogger, i as getChildLogger } from "./logger-BU4ooQvN.js";
import {
...
import { n as getStatusCode } from "./session-errors-Cqfz8ejf.js";
import {
  d as DisconnectReason,
  l as fetchLatestBaileysVersion,
  n as useMultiFileAuthState,
  r as makeCacheableSignalKeyStore,
  t as makeWASocket,
} from "./session.runtime-CiA-rRy6.js";
import { p as resolveUserPath, s as ensureDir } from "./utils-BGRcpLKt.js";
...
async function createWaSocket(printQr, verbose, opts = {}) {
  const logger = toPinoLikeLogger(
    getChildLogger({ module: "baileys" }, { level: verbose ? "info" : "silent" }),
    verbose ? "info" : "silent",
  );
  const authDir = resolveUserPath(opts.authDir ?? resolveDefaultWebAuthDir());
  await ensureDir(authDir);
  const sessionLogger = getChildLogger({ module: "web-session" });
  if ((await waitForCredsSaveQueueWithTimeout(authDir)) === "timed_out")
  ...
  sock.ev.on("connection.update", async (update) => {
    try {
      ...
      if (connection === "close") {
        if (getStatusCode(lastDisconnect?.error) === LOGGED_OUT_STATUS)
          console.error(
            danger(
...
function resolveEnvHttpsProxyUrl(env = process.env) {
  ...
  const httpsProxy =
    lowerHttpsProxy !== void 0 ? lowerHttpsProxy : normalizeEnvProxyValue(env.HTTPS_PROXY);
  const httpProxy =
    lowerHttpProxy !== void 0 ? lowerHttpProxy : normalizeEnvProxyValue(env.HTTP_PROXY);
...
//#endregion
export {
  resolveWhatsAppSocketTiming as i,
  newConnectionId as n,
  waitForWaConnection as r,
  createWaSocket as t,
};
