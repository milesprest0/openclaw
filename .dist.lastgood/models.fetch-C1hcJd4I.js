...
import { a as resolveApiKeyForProvider } from "./provider-auth-runtime-CXnsTkLQ.js";
import {
  c as SELF_HOSTED_DEFAULT_COST,
  l as SELF_HOSTED_DEFAULT_MAX_TOKENS,
} from "./provider-self-hosted-setup-5iusi84x.js";
...
import "./secret-input-runtime-ChwUh-m1.js";
...
function normalizeReasoningOptions(value) {
  ...
  return [
    ...new Set(
      value.map((option) => normalizeReasoningOption(option)).filter((option) => option !== null),
    ),
  ];
}
...
function normalizeLmstudioTransportReasoningCompat(compat) {
  ...
  const hasBinaryMapValue =
    map !== void 0 && Object.values(map).some((value) => value === "on" || value === "off");
  if (!hasBinarySupported && !hasBinaryMapValue) return compat;
  const normalizedSupportedReasoningEfforts =
...
function normalizeLmstudioConfiguredCatalogEntry(entry) {
  ...
  const input = Array.isArray(record.input)
    ? record.input.filter((item) => item === "text" || item === "image" || item === "document")
    : void 0;
...
function mapLmstudioWireEntry(entry) {
  ...
  const contextWindow =
    (entry.max_context_length !== void 0 &&
...
function buildLmstudioAuthHeaders(params) {
  ...
  if (apiKey && !(apiKey === "lmstudio-local") && !isNonSecretApiKeyMarker(apiKey)) {
    for (const headerName of Object.keys(headers))
      if (headerName.toLowerCase() === "authorization") delete headers[headerName];
...
async function resolveLmstudioRequestContext(params) {
  const providerHeaders =
    params.providerHeaders ?? params.config?.models?.providers?.["lmstudio"]?.headers;
...
async function resolveLmstudioRuntimeApiKey(params) {
  ...
  if (!config) return;
  const hasAuthorizationHeader = hasLmstudioAuthorizationHeader(
    params.headers ?? config.models?.providers?.["lmstudio"]?.headers,
  );
...
async function ensureLmstudioModelLoaded(params) {
  ...
  if (preflight.status !== void 0 && preflight.status >= 400)
  ...
  const advertisedContextLimit =
    matchingModel?.max_context_length !== void 0 &&
  ...
  const requestedContextLength =
    params.requestedContextLength !== void 0 &&
  ...
  const contextLengthForLoad =
    advertisedContextLimit === null
      ? (requestedContextLength ?? 64e3)
      : Math.min(requestedContextLength ?? 64e3, advertisedContextLimit);
...
//#endregion
export {
  LMSTUDIO_DOCKER_HOST_BASE_URL as A,
  resolveLoadedContextWindow as C,
  LMSTUDIO_DEFAULT_INFERENCE_BASE_URL as D,
  LMSTUDIO_DEFAULT_EMBEDDING_MODEL as E,
  LMSTUDIO_PROVIDER_LABEL as F,
  LMSTUDIO_LOCAL_API_KEY_PLACEHOLDER as M,
  LMSTUDIO_MODEL_PLACEHOLDER as N,
  LMSTUDIO_DEFAULT_LOAD_CONTEXT_LENGTH as O,
  LMSTUDIO_PROVIDER_ID as P,
  resolveLmstudioServerBase as S,
  LMSTUDIO_DEFAULT_BASE_URL as T,
  normalizeLmstudioConfiguredCatalogEntry as _,
  resolveLmstudioConfiguredApiKey as a,
  resolveLmstudioReasoningCapability as b,
  resolveLmstudioRuntimeApiKey as c,
  shouldUseLmstudioApiKeyPlaceholder as d,
  shouldUseLmstudioSyntheticAuth as f,
  normalizeLmstudioConfiguredCatalogEntries as g,
  mapLmstudioWireModelsToConfig as h,
  buildLmstudioAuthHeaders as i,
  LMSTUDIO_DOCKER_HOST_INFERENCE_BASE_URL as j,
  LMSTUDIO_DEFAULT_MODEL_ID as k,
  hasLmstudioAuthorizationHeader as l,
  mapLmstudioWireEntry as m,
  ensureLmstudioModelLoaded as n,
  resolveLmstudioProviderHeaders as o,
  buildLmstudioModelName as p,
  fetchLmstudioModels as r,
  resolveLmstudioRequestContext as s,
  discoverLmstudioModels as t,
  resolveLmstudioProviderAuthMode as u,
  normalizeLmstudioProviderConfig as v,
  LMSTUDIO_DEFAULT_API_KEY_ENV_VAR as w,
  resolveLmstudioReasoningCompat as x,
  resolveLmstudioInferenceBase as y,
};
