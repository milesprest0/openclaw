...
import {
  ...
  c as normalizeQuery,
  d as postGraphJson,
  f as resolveGraphToken,
  i as fetchGraphAbsoluteUrl,
  l as patchGraphJson,
  n as deleteGraphRequest,
  o as listChannelsForTeam,
  r as escapeOData,
  s as listTeamsByName,
  t as searchGraphUsers,
  u as postGraphBetaJson,
} from "./graph-users-CtrIdNVg.js";
import "./text-runtime-lKuAtsoz.js";
import {
  S as createMSTeamsConversationStoreFs,
  a as sendMessageMSTeams,
  b as createMSTeamsPollStoreFs,
  i as sendAdaptiveCardMSTeams,
  n as deleteMessageMSTeams,
  o as sendPollMSTeams,
  r as editMessageMSTeams,
  t as probeMSTeams,
...
async function pinMessageMSTeams(params) {
  ...
  if (conv.kind === "channel")
    throw new Error(
      "Pin/unpin is not supported for channel messages on Graph v1.0. Only chat conversations support pinned messages.",
    );
  const body = {
...
async function listPinsMSTeams(params) {
  ...
  if (conv.kind === "channel")
    throw new Error(
      "Listing pinned messages is not supported for channels on Graph v1.0. Only chat conversations support pinned messages.",
    );
...
const TEAMS_REACTION_TYPES = ["like", "heart", "laugh", "surprised", "sad", "angry"];
...
async function listReactionsMSTeams(params) {
  ...
  return {
    reactions: Array.from(grouped.entries()).map(([type, group]) => ({
      reactionType: type,
      name: type,
  ...
  msteamsOutbound: {
    ...
    ...createAttachedChannelResultAdapter({
      ...
      sendText: async ({ cfg, to, text, deps }) => {
        return await (
          resolveOutboundSendDep(deps, "msteams") ??
          ((to, text) =>
            sendMessageMSTeams({
              cfg,
              to,
