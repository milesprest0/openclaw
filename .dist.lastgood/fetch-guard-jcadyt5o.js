...
import {
  n as normalizeRequestInitHeadersForFetch,
  t as normalizeHeadersInitForFetch,
} from "./fetch-headers-CorjpjhG.js";
...
import { t as retainSafeHeadersForCrossOriginRedirect$1 } from "./redirect-headers-CPb_881Q.js";
import { r as isMockedFetch, t as fetchWithRuntimeDispatcher } from "./runtime-fetch-DhU7fIOQ.js";
import {
...
import { n as _globalUndiciStreamTimeoutMs } from "./undici-global-dispatcher-B1L_za-h.js";
import {
  n as createHttp1EnvHttpProxyAgent,
  r as createHttp1ProxyAgent,
  t as createHttp1Agent,
} from "./undici-runtime-BpIrSBt2.js";
//#region src/infra/net/socket-adapter.ts
...
function isLikelySocketDrop(err) {
...
function attach(requestOrStream, responseStarted) {
  if (!isPromiseLike(requestOrStream)) return requestOrStream;
  return requestOrStream.then(void 0, (err) => {
    throw normalizeError(err, responseStarted);
  });
}
function normalizeError(err, responseStarted) {
  if (err instanceof SocketDropException) return err;
  if (!isLikelySocketDrop(err)) return err;
  return new SocketDropException(
    `Socket dropped ${responseStarted() ? "mid-stream" : "before response"}: ${formatErrorMessage(err)}`,
    !responseStarted(),
    { cause: err },
  );
}
...
async function captureGuardedFetchExchange(params) {
  if (params.capture === false || !isTruthyEnvValue(process.env[OPENCLAW_DEBUG_PROXY_ENABLED]))
    return;
  const { captureHttpExchange } = await import("./runtime-BbneBknh.js");
  captureHttpExchange({
    url: params.url,
  ...
  while (true) {
    ...
    try {
      ...
      if (err instanceof SsrFBlockedError)
        logWarn(
          `security: blocked URL fetch (${params.auditContext ?? "url-fetch"}) targetOrigin=${parsedUrl.origin} reason=${err.message}`,
        );
      await release(dispatcher);
      throw err;
    }
...
//#endregion
export {
  withTrustedEnvProxyGuardedFetchMode as a,
  withStrictGuardedFetchMode as i,
  fetchWithSsrFGuard as n,
  withTrustedExplicitProxyGuardedFetchMode as o,
  retainSafeHeadersForCrossOriginRedirectHeaders as r,
  GUARDED_FETCH_MODE as t,
};
