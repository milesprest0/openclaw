...
import { n as getBootstrapChannelSecrets } from "./bootstrap-registry-BWOCdZvF.js";
import { r as resolvePluginCapabilityProviders } from "./capability-provider-runtime-BYdI2qTA.js";
import { r as loadInstalledPluginIndexInstallRecordsSync } from "./channel-catalog-registry-uu7_tUkL.js";
import { t as loadChannelSecretContractApi } from "./channel-contract-api-Deoqesoz.js";
import "./agent-scope-9AmhTwki.js";
import {
  n as resolvePluginConfigContractsById,
  t as collectPluginConfigContractMatches,
} from "./config-contracts-Cl3ljnWr.js";
import {
  n as normalizeMediaProviderId,
  t as resolveImageCapableConfigProviderIds,
} from "./config-provider-models-DWlpS7W-.js";
import "./shared-Dp_cRzjX.js";
import { s as normalizePluginsConfig, u as resolveEnableState } from "./config-state-BF1r9Z0v.js";
import {
  n as resolveConfiguredMediaEntryCapabilities,
  r as resolveEffectiveMediaEntryCapabilities,
} from "./entry-capabilities-BnctHCEh.js";
import "./installed-plugin-index-records-BD2sKB0z.js";
import {
  a as createLazyRuntimeSurface,
  i as createLazyRuntimeNamedExport,
} from "./lazy-runtime-BKxgiyqW.js";
import { n as normalizeSecretInput } from "./normalize-secret-input-B8M4cov-.js";
import { u as secretRefKey } from "./ref-contract-PE7UucMZ.js";
import { o as resolveSecretRefValues } from "./resolve-BZt0Bv7Z.js";
import { t as collectTtsApiKeyAssignments } from "./runtime-config-collectors-tts-Cm7Y9zuR.js";
import { n as evaluateGatewayAuthSurfaceStates } from "./runtime-gateway-auth-surfaces-Cn7J2KV2.js";
import {
...
import { m as resolveSecretInputRef } from "./types.secrets-CaNC1eIn.js";
...
import {
  o as sortWebFetchProvidersForAutoDetect,
  r as sortWebSearchProvidersForAutoDetect,
} from "./web-search-providers.shared-B9RVxTUL.js";
...
function collectMediaRequestAssignments(params) {
  ...
  const capabilityKeys = ["audio", "image", "video"];
  const isCapabilityEnabled = (capability) =>
    (isRecord(media[capability]) ? media[capability] : void 0)?.enabled !== false;
...
function collectSandboxSshAssignments(params) {
  ...
  const defaultsBackend =
    typeof defaultsSandbox?.backend === "string" ? defaultsSandbox.backend : void 0;
  ...
  (Array.isArray(agents.list) ? agents.list : []).forEach((rawAgent, index) => {
    ...
    const effectiveBackend =
      (typeof sandbox?.backend === "string" ? sandbox.backend : void 0) ??
      defaultsBackend ??
      "docker";
    const effectiveMode =
      (typeof sandbox?.mode === "string" ? sandbox.mode : void 0) ?? defaultsMode ?? "off";
    const active =
      normalizeOptionalLowercaseString(effectiveBackend) === "ssh" && effectiveMode !== "off";
    for (const key of ["identityData", "certificateData", "knownHostsData"])
...
function collectPluginConfigAssignments(params) {
  ...
  for (const [pluginId, entry] of Object.entries(entries)) {
    ...
    const enableState = resolveEnableState(
      ...
      resolvedOrigin,
      normalizedConfig,
      resolvedOrigin === "bundled" ? secretInputs.bundledDefaultEnabled : void 0,
    );
    collectConfiguredPluginSecretAssignments({
      pluginId,
      pluginConfig,
...
async function resolveRuntimeWebProviderSurface(params) {
  ...
  let allProviders = params.sortProviders(
    await params.resolveProviders({ configuredBundledPluginId }),
  );
  if (
    params.rawProvider &&
    ...
    configuredBundledPluginId = void 0;
  if (
    params.rawProvider &&
    !configuredBundledPluginId &&
    !allProviders.some((provider) => provider.id === params.rawProvider)
    ...
    if (params.configuredProvider) {
      ...
      if (selectedProvider) {
        const selectedDetails =
          params.providers.find((entry) => entry.id === selectedProvider)?.requiresCredential ===
          false
...
function inferExactBundledPluginScopedWebToolConfigOwner(params) {
  ...
  return isRecord((isRecord(entry.config) ? entry.config : void 0)?.[params.key])
    ? params.pluginId
    : void 0;
}
...
async function resolveSecretInputWithEnvFallback(params) {
  ...
  if (
    ...
    try {
      const resolvedValue = (
        await resolveSecretRefValues([ref], {
          config: params.sourceConfig,
          env: params.context.env,
...
function setResolvedWebSearchApiKey(params) {
  ...
  const search = ensureObject(
    ensureObject(ensureObject(params.resolvedConfig, "tools"), "web"),
    "search",
  );
...
async function resolveBundledWebSearchProviders(params) {
  ...
  const onlyPluginIds =
    params.configuredBundledPluginId !== void 0
