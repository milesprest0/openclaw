...
import {
  i as require_type,
  n as require_get_intrinsic,
  r as require_call_bind_apply_helpers,
  t as require_form_data,
} from "./form_data-qX1yviXX.js";
import { t as require_src } from "./src-Dce2BfHT.js";
...
var require_axios = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  const freezeMethods = (obj) => {
    reduceDescriptors(obj, (descriptor, name) => {
      if (isFunction$1(obj) && ["arguments", "caller", "callee"].includes(name)) return false;
    ...
    set(header, valueOrRewrite, rewrite) {
      ...
      function setHeader(_value, _header, _rewrite) {
        ...
        if (
          ...
          self[key] === void 0 ||
          _rewrite === true ||
          (_rewrite === void 0 && self[key] !== false)
        )
      ...
      function transformResponse(data) {
        ...
        if (
          ...
          const strictJSONParsing =
            !(transitional && transitional.silentJSONParsing) && JSONRequested;
          try {
  ...
  const textEncoder =
    typeof TextEncoder === "function" ? new TextEncoder() : new util.TextEncoder();
  const CRLF = "\r\n";
  ...
  const IPV4_MAPPED_DOTTED_RE =
    /^(?:::|(?:0{1,4}:){1,4}:|(?:0{1,4}:){5})ffff:(\d+\.\d+\.\d+\.\d+)$/i;
  ...
  function setProxy(options, configProxy, location, isRedirect) {
    ...
    if (isRedirect && options.headers) {
      for (const name of Object.keys(options.headers))
        if (name.toLowerCase() === "proxy-authorization") delete options.headers[name];
    }
    if (proxy) {
      const isProxyURL = proxy instanceof URL;
      const readProxyField = (key) =>
        isProxyURL || utils$1.hasOwnProp(proxy, key) ? proxy[key] : void 0;
      ...
      if (proxyAuth) {
        ...
        const authUsername =
          authIsObject && utils$1.hasOwnProp(proxyAuth, "username") ? proxyAuth.username : void 0;
        const authPassword =
          authIsObject && utils$1.hasOwnProp(proxyAuth, "password") ? proxyAuth.password : void 0;
        if (Boolean(authUsername || authPassword))
      ...
      return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
        ...
        if (protocol === "data:") {
          if (config.maxContentLength > -1) {
            if (
              estimateDataURLDecodedBytes(String(config.url || fullPath || "")) >
        ...
        if (data && (onUploadProgress || maxUploadRate)) {
          ...
          data = stream.pipeline(
            [data, new AxiosTransformStream({ maxRate: utils$1.toFiniteNumber(maxUploadRate) })],
            utils$1.noop,
          );
          onUploadProgress &&
            data.on(
        ...
        if (config.socketPath) {
          ...
          if (config.allowedSocketPaths != null) {
            ...
            if (
              !allowed.some(
                (entry) => typeof entry === "string" && path.resolve(entry) === resolvedSocket,
              )
            )
              return reject(
                new AxiosError(
        ...
        req.once("close", function clearCurrentReq() {
          ...
          for (const socket of boundSockets)
            if (socket[kAxiosCurrentReq] === req) socket[kAxiosCurrentReq] = null;
          boundSockets.clear();
        });
        if (config.timeout) {
    ...
    utils$1.forEach(
      ...
      function computeConfigValue(prop) {
        ...
        const configValue = merge(
          utils$1.hasOwnProp(config1, prop) ? config1[prop] : void 0,
          utils$1.hasOwnProp(config2, prop) ? config2[prop] : void 0,
          prop,
        );
  ...
  const factory = (env) => {
    var _utils$global;
    const globalObject =
      (_utils$global = utils$1.global) !== null && _utils$global !== void 0
        ? _utils$global
        : globalThis;
    ...
    return async (config) => {
      ...
      try {
        if (hasMaxContentLength && typeof url === "string" && url.startsWith("data:")) {
          if (estimateDataURLDecodedBytes(url) > maxContentLength)
            throw new AxiosError(
  ...
  var Axios = class {
    ...
    _request(configOrUrl, config) {
      ...
      this.interceptors.request.forEach(function unshiftRequestInterceptors(interceptor) {
        ...
        const transitional = config.transitional || transitionalDefaults;
        if (transitional && transitional.legacyInterceptorReqResOrdering)
    ...
    getUri(config) {
      config = mergeConfig(this.defaults, config);
      return buildURL(
        buildFullPath(config.baseURL, config.url, config.allowAbsoluteUrls),
        config.params,
        config.paramsSerializer,
      );
    }
...
var require_object_inspect = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  var weakMapHas =
    typeof WeakMap === "function" && WeakMap.prototype ? WeakMap.prototype.has : null;
  var weakSetHas =
    typeof WeakSet === "function" && WeakSet.prototype ? WeakSet.prototype.has : null;
  var weakRefDeref =
    typeof WeakRef === "function" && WeakRef.prototype ? WeakRef.prototype.deref : null;
  ...
  function canTrustToString(obj) {
    return (
      !toStringTag ||
      !(typeof obj === "object" && (toStringTag in obj || typeof obj[toStringTag] !== "undefined"))
    );
  }
  ...
  function inspectString(str, opts) {
    ...
    quoteRE.lastIndex = 0;
    return wrapQuotes(
      $replace.call($replace.call(str, quoteRE, "\\$1"), /[\x00-\x1f]/g, lowbyte),
      "single",
      opts,
    );
  }
    ...
    while (queue.length > 1) {
      ...
      if (isArray(obj)) {
        ...
        for (var j = 0; j < obj.length; ++j)
          if (typeof obj[j] !== "undefined") compacted[compacted.length] = obj[j];
  ...
  var stringify = function stringify(
    ...
    if (isNonNullishPrimitive(obj) || utils.isBuffer(obj)) {
      if (encoder)
        return [
          formatter(
            encodeValuesOnly ? prefix : encoder(prefix, defaults.encoder, charset, "key", format),
          ) +
            "=" +
  ...
  var parseValues = function parseQueryStringValues(str, options) {
    ...
    for (i = 0; i < parts.length; ++i) {
      ...
      if (pos === -1) {
        ...
        if (key !== null)
          val = utils.maybeMap(
            parseArrayValue(part.slice(pos + 1), options, isArray(obj[key]) ? obj[key].length : 0),
            function (encodedVal) {
  ...
  var normalizeParseOptions = function normalizeParseOptions(opts) {
    ...
    return {
      ...
      throwOnLimitExceeded:
        typeof opts.throwOnLimitExceeded === "boolean" ? opts.throwOnLimitExceeded : false,
    };
  };
...
var require_sign_in$3 = /* @__PURE__ */ __commonJSMin((exports) => {
  ...
  var BotSignInClient = class {
    ...
    async getUrl(params) {
      ...
      return (
        await this.http.get(`${this._apiClientSettings.oauthUrl}/${BOT_SIGNIN_ENDPOINTS.URL}?${q}`)
      ).data;
    }
...
var require_member = /* @__PURE__ */ __commonJSMin((exports) => {
  ...
  var ConversationMemberClient = class {
    ...
    async get(conversationId) {
      return (
        await this.http.get(`${this.serviceUrl}/v3/conversations/${conversationId}/members`)
      ).data.map(models.resolveAadObjectId);
    }
...
var require_conversation$1 = /* @__PURE__ */ __commonJSMin((exports) => {
  ...
  var ConversationClient = class {
    ...
    constructor(serviceUrl, options, apiClientSettings$1) {
      ...
      this._apiClientSettings = apiClientSettings.mergeApiClientSettings(apiClientSettings$1);
      this._activities = new activity.ConversationActivityClient(
        serviceUrl,
        this.http,
        this._apiClientSettings,
      );
      this._members = new member.ConversationMemberClient(
        serviceUrl,
        this.http,
        this._apiClientSettings,
      );
    ...
    members(conversationId) {
      return {
        ...
        getPaged: (pageSize, continuationToken) =>
...
var require_clients = /* @__PURE__ */ __commonJSMin((exports) => {
  ...
  var Client = class {
    ...
    constructor(serviceUrl, options, apiClientSettings$1, cloud) {
      ...
      this._apiClientSettings = apiClientSettings.mergeApiClientSettings(
        apiClientSettings$1,
        cloud,
      );
      ...
      this.conversations = new conversation.ConversationClient(
        serviceUrl,
        this.http,
        this._apiClientSettings,
      );
