...
function sanitizeTempExtension(extension) {
  if (!extension) return "";
  const token = (
    (extension.startsWith(".") ? extension : `.${extension}`).match(/[a-zA-Z0-9._-]+$/)?.[0] ?? ""
  ).replace(/^[._-]+/, "");
  return token ? `.${token}` : "";
}
...
//#endregion
export {
  withTempDownloadPath as i,
  createTempDownloadTarget as n,
  sanitizeTempFileName as r,
  buildRandomTempFilePath as t,
};
