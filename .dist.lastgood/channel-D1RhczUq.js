...
import { t as getDiscordApprovalCapability } from "./approval-native-C2QVOTRl.js";
import { a as shouldSuppressLocalDiscordExecApprovalPrompt } from "./approval-shared-vFvzyV_H.js";
import { r as resolveRequiredDiscordChannelPermissions } from "./audit-core-BMqaZo97.js";
import { n as sleepWithAbort } from "./backoff-CW9s2Y7t.js";
import "./text-runtime-lKuAtsoz.js";
import { t as discordMessageActions$1 } from "./channel-actions-D7C5gIx0.js";
...
import { n as resolveDiscordCurrentConversationIdentity } from "./conversation-identity-Cgp5ICBe.js";
...
import { n as discordOutbound } from "./outbound-adapter-DH8NSfGw.js";
import "./channel-api-CvNQ3mt5.js";
...
import { t as getDiscordRuntime } from "./runtime-BuudSymP.js";
...
import {
  i as discordSecurityAdapter,
  n as discordConfigAdapter,
  r as discordSetupAdapter,
  t as createDiscordPluginBase,
} from "./shared-CbrlsgsR.js";
import {
...
import { n as normalizeAtHashSlug } from "./string-normalization-B6JUy5-Y.js";
import { n as resolveTargetsWithOptionalToken } from "./target-resolvers-CW9z4oFc.js";
import {
...
const loadDiscordDirectoryConfigModule = createLazyRuntimeModule(
  () => import("./directory-config-CbXUC7Aj.js"),
);
const loadDiscordResolveChannelsModule = createLazyRuntimeModule(
  () => import("./resolve-channels-BcLtN9qC.js"),
);
const loadDiscordResolveUsersModule = createLazyRuntimeModule(
  () => import("./resolve-users-DvdJH5l_.js"),
);
const loadDiscordThreadBindingsManagerModule = createLazyRuntimeModule(
  () => import("./thread-bindings.manager-N9KKMMM1.js"),
);
...
async function loadDiscordProbeRuntime() {
  discordProbeRuntimePromise ??= import("./probe.runtime-BkQ5PLNe.js");
  return await discordProbeRuntimePromise;
}
async function loadDiscordAuditModule() {
  discordAuditModulePromise ??= import("./audit-BKJ6SmD5.js");
  return await discordAuditModulePromise;
}
async function loadDiscordSendModule() {
  discordSendModulePromise ??= import("./send-BJzVgxwX.js");
  return await discordSendModulePromise;
}
async function loadDiscordDirectoryLiveModule() {
  discordDirectoryLiveModulePromise ??= import("./directory-live-CKbzhFsV.js");
...
function resolveDiscordGuildEntry(guilds, groupSpace) {
  ...
  if (normalized) {
    const match = Object.values(guilds).find(
      (entry) => normalizeDiscordSlug(entry?.slug ?? void 0) === normalized,
    );
...
function resolveDiscordChannelEntry(channelEntries, params) {
  ...
  return (
    (params.groupId ? channelEntries[params.groupId] : void 0) ??
    (channelSlug ? (channelEntries[channelSlug] ?? channelEntries[`#${channelSlug}`]) : void 0) ??
    (groupChannel ? channelEntries[normalizeDiscordSlug(groupChannel)] : void 0)
  );
}
...
function resolveDiscordPolicyContext(params) {
  const guildEntry = resolveDiscordGuildEntry(
    (params.accountId
      ...
      : void 0) ?? params.cfg.channels?.discord?.guilds,
    params.groupSpace,
  );
...
function collectDiscordStatusIssues(accounts) {
  ...
  for (const entry of accounts) {
    ...
    if (readDiscordApplicationSummary(account.application).intents?.messageContent === "disabled")
      issues.push({
        channel: "discord",
        accountId,
        kind: "intent",
...
function startDiscordStartupProbe(params) {
  (async () => {
    try {
      ...
      ).probeDiscord(params.token, 2500, { includeApplication: true });
...
const discordMessageActions = {
  resolveExecutionMode: (ctx) =>
    ...
    discordMessageActions$1.resolveExecutionMode?.(ctx) ??
    "local",
  describeMessageTool: (ctx) =>
    resolveRuntimeDiscordMessageActions()?.describeMessageTool?.(ctx) ??
    discordMessageActions$1.describeMessageTool?.(ctx) ??
    null,
  extractToolSend: (ctx) =>
    resolveRuntimeDiscordMessageActions()?.extractToolSend?.(ctx) ??
    discordMessageActions$1.extractToolSend?.(ctx) ??
    null,
  prepareSendPayload: (ctx) =>
    resolveRuntimeDiscordMessageActions()?.prepareSendPayload?.(ctx) ??
    discordMessageActions$1.prepareSendPayload?.(ctx) ??
    null,
  handleAction: async (ctx) => {
    ...
    if (!discordMessageActions$1.handleAction)
...
function resolveDiscordStartupDelayMs(cfg, accountId) {
  const startupIndex = listDiscordAccountIds(cfg)
    .filter((candidateId) => {
  ...
  base: {
    ...
    heartbeat: {
      sendTyping: async ({ cfg, to, accountId, threadId }) => {
        const target = parseDiscordTarget(
    ...
    status: createComputedAccountStatusAdapter({
      ...
      buildCapabilitiesDiagnostics: async ({ account, target }) => {
        ...
        try {
          ...
          const missingRequired = resolveRequiredDiscordChannelPermissions(
            perms.channelType,
          ).filter((permission) => !perms.permissions.includes(permission));
          details.permissions = {
    ...
    gateway: {
      startAccount: async (ctx) => {
        ...
        if (startupDelayMs > 0) {
          ctx.log?.info(
            `[${account.accountId}] delaying provider startup ${Math.round(startupDelayMs / 1e3)}s to reduce Discord startup rate limits`,
          );
          try {
...
//#endregion
export {
  resolveDiscordGroupToolPolicy as i,
  collectDiscordStatusIssues as n,
  resolveDiscordGroupRequireMention as r,
  discordPlugin as t,
};
