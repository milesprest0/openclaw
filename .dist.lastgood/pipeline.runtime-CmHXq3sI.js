import { n as removeAckReactionAfterReply } from "./ack-reactions-Chyt3s4c.js";
import {
  f as removeSlackReaction,
  h as buildSlackEditTextPayload,
  l as reactSlackMessage,
  r as editSlackMessage,
  t as deleteSlackMessage,
} from "./actions-BivXGvX4.js";
import { i as normalizeSlackAllowOwnerEntry } from "./allow-list-CBwLGSwi.js";
import {
...
import {
  ...
  b as resolveChannelStreamingNativeTransport,
  c as formatChannelProgressDraftText,
  g as resolveChannelProgressDraftRender,
  h as resolveChannelProgressDraftMaxLines,
  i as buildChannelProgressDraftLineForEntry,
  p as resolveChannelProgressDraftLabel,
...
import { c as clearHistoryEntriesIfEnabled } from "./history-CiK0qvRj.js";
...
import { n as resolveSlackThreadTargets, t as prepareSlackMessage } from "./prepare-DfRHy14n.js";
import {
  a as resolveDeliveredSlackReplyThreadTs,
  i as readSlackReplyBlocks,
  n as deliverReplies,
  o as resolveSlackThreadTs,
  t as createSlackReplyDeliveryPlan,
} from "./replies-n5QyGjge.js";
...
import {
  ...
  s as normalizeSlackOutboundText,
  t as sendMessageSlack,
} from "./send-JE0jcFcj.js";
import "./blocks-input-B3PJKopi.js";
import { t as recordInboundSession } from "./session-CUCa4TmV.js";
import { o as updateLastRoute } from "./store-BpWdoYPF.js";
import {
  n as resolveSlackNativeStreaming,
  r as resolveSlackStreamingMode,
  t as mapStreamingModeToSlackLegacyDraftStreamMode,
} from "./streaming-compat-VwA4lRjP.js";
import "./room-context-Di4DZ-0f.js";
...
function lineDetail(line) {
  const parts = [
    line.detail,
    line.status && !line.detail?.includes(line.status) ? line.status : void 0,
  ]
    .map((part) => part?.trim())
...
var SlackStreamNotDeliveredError = class extends Error {
  constructor(pendingText, slackCode) {
    super(
      `slack-stream: finalize failed with ${slackCode} before any text reached Slack (${pendingText.length} chars pending)`,
    );
...
function extractSlackErrorCode(err) {
  ...
  return (typeof record.message === "string" ? record.message : "").match(
    /An API error occurred:\s*([a-z_][a-z0-9_]*)/i,
  )?.[1];
}
...
async function readSlackMessageAfterEditError(params) {
  if (params.threadTs)
    return (
      ...
      ).find((message) => message?.ts === params.messageId) ?? null
    );
  const message = (
...
async function finalizeSlackPreviewEdit(params) {
  try {
    ...
    try {
      if (
        await didSlackPreviewEditApplyAfterError({
          client: params.client,
          token: params.token,
...
async function dispatchPreparedSlackMessage(prepared) {
  ...
  const previewStreamingEnabled =
    !sourceRepliesAreToolOnly && shouldEnableSlackPreviewStreaming({ mode: slackStreaming.mode });
  ...
  const resolveDeliveryThreadTs = (params) => {
    ...
    return (
      ...
      plannedThreadTs ??
      (params.kind === "block" ? usedBlockReplyThreadTs : void 0)
    );
  };
  ...
  const deliverWithStreaming = async (params) => {
    ...
    try {
      ...
      if (err instanceof SlackStreamNotDeliveredError) {
        ...
        if (streamSession) {
          if (
            await deliverBufferedStreamFallback({
              session: streamSession,
              err,
              payload: params.payload,
              kind: params.kind,
  ...
  const pushPreviewToolProgress = async (line, options) => {
    ...
    if (previewToolProgressEnabled && !previewToolProgressSuppressed) {
      if (previewToolProgressLines.at(-1)?.text !== normalized)
        previewToolProgressLines = [...previewToolProgressLines, line].slice(
          -resolveChannelProgressDraftMaxLines(account.config),
        );
    }
