...
import { n as asObject, r as assertOkOrThrowHttpError } from "./provider-http-errors-Gz46brs9.js";
...
import "./provider-registry-DRK75MvY.js";
import "./directives-GtrZOD7p.js";
...
function normalizeBaseUrl(params) {
  ...
  ).has(normalized) || !params.policy.allowCustom
    ? canonical
    : normalized;
}
function resolveProviderConfigRecord(rawConfig, providerConfigKey) {
  return (
    asObject(asObject(rawConfig.providers)?.[providerConfigKey]) ??
    asObject(rawConfig[providerConfigKey])
  );
}
...
function createOpenAiCompatibleSpeechProvider(options) {
  ...
  const normalizeModel =
    options.normalizeModel ?? ((value, fallback) => normalizeOptionalString(value) ?? fallback);
  ...
  function readProviderConfig(config) {
    ...
    return {
      ...
      baseUrl:
        normalizeOptionalString(config.baseUrl) == null
          ? normalized.baseUrl
          : normalizeBaseUrl({
      ...
      model: normalizeModel(
        normalizeOptionalString(config.model ?? config.modelId),
        normalized.model,
      ),
  ...
  function resolveBaseUrl(params) {
    return normalizeBaseUrl({
      value:
        ...
        normalizeOptionalString(readModelProviderConfig(params.cfg, providerConfigKey)?.baseUrl),
      fallback: options.defaultBaseUrl,
      policy: options.baseUrlPolicy,
    });
  }
  return {
    ...
    resolveTalkConfig: ({ baseTtsConfig, talkProviderConfig }) => {
      ...
      const next = { ...base };
      if (talkProviderConfig.apiKey !== void 0)
