...
const execSchema = Type.Object({
  ...
  timeout: Type.Optional(
    Type.Number({ description: "Timeout in seconds (optional, kills process on expiry)" }),
  ),
  pty: Type.Optional(
    Type.Boolean({
      description:
  ...
  elevated: Type.Optional(
    Type.Boolean({ description: "Run on the host with elevated permissions (if allowed)" }),
  ),
  host: optionalStringEnum(["auto", "sandbox", "gateway", "node"], {
