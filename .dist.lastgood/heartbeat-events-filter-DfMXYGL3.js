import { r as HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS } from "./heartbeat-Cn4100FJ.js";
...
function buildExecEventPrompt(pendingEvents, opts) {
  ...
  if (!eventText) {
    if (useHeartbeatResponseTool)
      return `An async command completion event was triggered, but no command output was found. ${HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS} Do not mention, summarize, or reuse output from any earlier run.`;
  ...
  if (!deliverToUser) {
    if (useHeartbeatResponseTool)
      return `An async command completion event was triggered, but user delivery is disabled for this run. Handle the result internally. ${HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS} Do not mention, summarize, or reuse command output.`;
...
//#endregion
export {
  isRelayableExecCompletionEvent as a,
  isExecCompletionEvent as i,
  buildExecEventPrompt as n,
  isCronSystemEvent as r,
  buildCronEventPrompt as t,
};
