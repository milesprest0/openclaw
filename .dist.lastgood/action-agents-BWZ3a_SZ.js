...
import { a as sortSubagentRuns, t as formatRunLabel } from "./subagents-utils-BysrAM37.js";
...
function handleSubagentsAgentsAction(ctx) {
  ...
  const indexByChildSessionKey = new Map(
    numericOrder.map((entry, idx) => [entry.childSessionKey, idx + 1]),
  );
  const visibleRuns = [];
  ...
  if (visibleRuns.length === 0) lines.push("(none)");
    ...
    for (const entry of visibleRuns) {
      ...
      const bindingText = binding
        ? formatConversationBindingText({ conversationId: binding.conversation.conversationId })
  ...
  if (requesterBindings.length > 0) {
    ...
    for (const binding of requesterBindings) {
      ...
      lines.push(
        `- ${label} (${formatConversationBindingText({ conversationId: binding.conversation.conversationId })}, session:${binding.targetSessionKey})`,
      );
    }
  }
