...
function requestSignalHttpText(url, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const deadline = setTimeout(() => {
      request?.destroy(
        /* @__PURE__ */ new Error(`Signal HTTP exceeded deadline after ${options.timeoutMs}ms`),
      );
    }, options.timeoutMs);
    deadline.unref?.();
    const cleanup = () => {
    ...
    request.setTimeout(options.timeoutMs, () => {
      request?.destroy(
        /* @__PURE__ */ new Error(`Signal HTTP timed out after ${options.timeoutMs}ms`),
      );
    });
    ...
    request = client.request(
      ...
      (res) => {
        ...
        if (status < 200 || status >= 300) {
          ...
          rejectOnce(
            /* @__PURE__ */ new Error(
              `Signal SSE failed (${status} ${res.statusMessage || "error"})`,
            ),
          );
          return;
        }
