...
function buildNamedProgressLine(kind, name, metas, options, fields) {
  ...
  const text = formatToolAggregate(normalizedName, compactMetas.length ? compactMetas : void 0, {
...
function isCommandProgressItem(input) {
  return (
    normalizeOptionalLowercaseString(input.itemKind) === "command" || isCommandToolName(input.name)
  );
}
    ...
    case "tool":
      return buildNamedProgressLine(
        ...
        [
          ...
          input.phase && !input.name ? input.phase : void 0,
        ],
        options,
      );
    case "item": {
      ...
      if (name)
        return buildNamedProgressLine(input.event, name, [meta], options, { status: input.status });
...
function resolveChannelStreamingNativeTransport(entry) {
  return (
    asBoolean(getChannelStreamingConfigObject(entry)?.nativeTransport) ??
    asBoolean(entry?.nativeStreaming)
  );
}
...
function formatChannelProgressDraftText(params) {
  ...
  return (label ? [{ draftLabel: label }, ...params.lines] : params.lines)
    .map((line) => {
...
//#endregion
export {
  resolveChannelStreamingPreviewToolProgress as C,
  resolveChannelStreamingPreviewCommandText as S,
  resolveChannelStreamingBlockCoalesce as _,
  createChannelProgressDraftGate as a,
  resolveChannelStreamingNativeTransport as b,
  formatChannelProgressDraftText as c,
  resolveChannelPreviewStreamMode as d,
  resolveChannelProgressDraftConfig as f,
  resolveChannelProgressDraftRender as g,
  resolveChannelProgressDraftMaxLines as h,
  buildChannelProgressDraftLineForEntry as i,
  getChannelStreamingConfigObject as l,
  resolveChannelProgressDraftLineOptions as m,
  DEFAULT_PROGRESS_DRAFT_LABELS as n,
  formatChannelProgressDraftLine as o,
  resolveChannelProgressDraftLabel as p,
  buildChannelProgressDraftLine as r,
  formatChannelProgressDraftLineForEntry as s,
  DEFAULT_PROGRESS_DRAFT_INITIAL_DELAY_MS as t,
  isChannelProgressDraftWorkToolName as u,
  resolveChannelStreamingBlockEnabled as v,
  resolveChannelStreamingSuppressDefaultToolProgressMessages as w,
  resolveChannelStreamingPreviewChunk as x,
  resolveChannelStreamingChunkMode as y,
};
