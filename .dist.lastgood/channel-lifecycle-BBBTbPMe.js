...
import { i as deliverFinalizableLivePreview } from "./live-CawdYLv5.js";
...
function createArmableStallWatchdog(params) {
  ...
  const checkIntervalMs = Math.max(
    100,
    Math.floor(params.checkIntervalMs ?? Math.min(5e3, Math.max(250, timeoutMs / 6))),
  );
  let armed = false;
  let stopped = false;
  ...
  const check = () => {
    ...
    params.runtime?.error?.(
      `[${params.label}] transport watchdog timeout: idle ${Math.round(idleMs / 1e3)}s (limit ${Math.round(timeoutMs / 1e3)}s)`,
    );
    params.onTimeout({
      idleMs,
      timeoutMs,
...
//#endregion
export {
  createFinalizableDraftStreamControlsForState as a,
  deliverFinalizableDraftPreview as c,
  createFinalizableDraftStreamControls as i,
  clearFinalizableDraftMessage as n,
  takeMessageIdAfterStop as o,
  createFinalizableDraftLifecycle as r,
  createDraftStreamLoop as s,
  createArmableStallWatchdog as t,
};
