...
function hasPendingActiveTasks(taskIds) {
  ...
  for (const state of queueState.lanes.values())
    for (const taskId of state.activeTaskIds) if (taskIds.has(taskId)) return true;
  return false;
}
...
function setCommandLaneConcurrency(lane, maxConcurrent) {
  ...
  const minConcurrent =
    cleaned.startsWith("auth-probe:") || cleaned.startsWith("session:probe-") ? 1 : 0;
...
function waitForActiveTasks(timeoutMs) {
  ...
  for (const state of queueState.lanes.values())
    for (const taskId of state.activeTaskIds) activeAtStart.add(taskId);
...
//#endregion
export {
  getActiveTaskCount as a,
  getQueueSize as c,
  resetAllLanes as d,
  resetCommandLane as f,
  enqueueCommandInLane as i,
  getTotalQueueSize as l,
  waitForActiveTasks as m,
  GatewayDrainingError as n,
  getCommandLaneSnapshot as o,
  setCommandLaneConcurrency as p,
  clearCommandLane as r,
  getCommandLaneSnapshots as s,
  CommandLaneClearedError as t,
  markGatewayDraining as u,
};
