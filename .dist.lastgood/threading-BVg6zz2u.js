import {
  a as resolveDiscordChannelParentSafe,
...
import {
  ...
  n as resolveDiscordForwardedMessagesTextFromSnapshots,
  t as resolveDiscordEmbedText,
...
import "./simple-completion-runtime-BRCGnia-.js";
import {
...
import { s as withAbortTimeout } from "./timeouts-B4h4bmv9.js";
...
async function completeThreadTitle(params) {
  return await withAbortTimeout({
    ...
    createTimeoutError: () =>
      /* @__PURE__ */ new Error(`thread-title timed out after ${params.timeoutMs}ms`),
    run: async (signal) =>
...
async function resolveDiscordThreadParentInfo(params) {
  ...
  if (!parentId && threadChannel.id)
    parentId = (await resolveDiscordChannelInfo(client, threadChannel.id))?.parentId ?? void 0;
  if (!parentId) return {};
...
function resolveTrimmedDiscordMessageChannelId(params) {
  return (
    params.messageChannelId || resolveDiscordMessageChannelId({ message: params.message })
  ).trim();
}
  ...
  try {
    ...
    try {
      const existingThreadId =
        (await getChannelMessage(params.client.rest, messageChannelId, params.message.id))?.thread
          ?.id || "";
      if (existingThreadId) {
        logVerbose(
...
//#endregion
export {
  resolveDiscordReplyTarget as a,
  resolveDiscordThreadStarter as c,
  resolveDiscordReplyDeliveryPlan as i,
  sanitizeDiscordThreadName as l,
  resolveDiscordAutoThreadContext as n,
  resolveDiscordThreadChannel as o,
  resolveDiscordAutoThreadReplyPlan as r,
  resolveDiscordThreadParentInfo as s,
  maybeCreateDiscordAutoThread as t,
  __resetDiscordThreadStarterCacheForTest as u,
};
