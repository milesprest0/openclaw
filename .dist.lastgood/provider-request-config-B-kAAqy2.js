...
import { a as assertSecretInputResolved } from "./types.secrets-CaNC1eIn.js";
...
function shouldAutoAllowLoopbackModelRequest(params) {
  return (
    ...
    params.allowPrivateNetwork === void 0 &&
    params.request?.allowPrivateNetwork === void 0 &&
...
function mergeModelProviderRequestOverrides(...overrides) {
  ...
  for (const current of overrides)
    if (current?.allowPrivateNetwork !== void 0) {
      merged ??= {};
...
function resolveTlsOverride(tls) {
  ...
  if (!ca && !cert && !key && !passphrase && !serverName && rejectUnauthorized === void 0)
...
function applyResolvedAuthHeader(headers, auth) {
  ...
  for (const key of Object.keys(next))
    if (keysToDelete.has(normalizeLowercaseStringOrEmpty(key))) delete next[key];
...
//#endregion
export {
  mergeModelProviderRequestOverrides as a,
  resolveProviderRequestHeaders as c,
  sanitizeConfiguredProviderRequest as d,
  sanitizeRuntimeProviderRequestOverrides as f,
  getModelProviderRequestTransport as i,
  resolveProviderRequestPolicyConfig as l,
  buildProviderRequestDispatcherPolicy as n,
  normalizeBaseUrl as o,
  buildProviderRequestTlsClientOptions as r,
  resolveProviderRequestConfig as s,
  attachModelProviderRequestTransport as t,
  sanitizeConfiguredModelProviderRequest as u,
};
