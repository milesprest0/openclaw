...
import { r as readModelListResult } from "./models-Bk93b7WA.js";
import { t as isJsonObject } from "./protocol-DayKrDMy.js";
import {
  a as readCodexErrorNotification,
  c as readCodexTurnCompletedNotification,
  n as assertCodexThreadStartResponse,
  r as assertCodexTurnStartResponse,
} from "./protocol-validators-BXWxEgv_.js";
import { i as FALLBACK_CODEX_MODELS, r as CODEX_PROVIDER_ID } from "./provider-catalog-C3m-8sWK.js";
...
async function describeCodexImages(req, options) {
  ...
  const client = options.clientFactory
    ...
    : await import("./shared-client-zkaLNTUk.js").then(({ createIsolatedCodexAppServerClient }) =>
...
async function assertCodexModelSupportsImage(params) {
  const match = readModelListResult(
  ...
  try {
    await Promise.race([
      ...
      new Promise((_, reject) => {
        timeout = setTimeout(
          () => reject(/* @__PURE__ */ new Error("codex app-server image turn timed out")),
          params.timeoutMs,
        );
        timeout.unref?.();
        const abortListener = () =>
          reject(/* @__PURE__ */ new Error("codex app-server image turn aborted"));
