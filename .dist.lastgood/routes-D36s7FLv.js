...
import {
  ...
  E as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS,
  N as DEFAULT_BROWSER_SCREENSHOT_TIMEOUT_MS,
  d as parseBrowserHttpUrl,
...
import {
  ...
  _ as INTERACTIVE_ROLES,
  a as resolveOpenClawUserDataDir,
  b as resolveBrowserNavigationProxyMode,
  g as CONTENT_ROLES,
  h as snapshotRoleViaCdp,
  m as snapshotAria,
  t as getChromeWebSocketUrl,
  u as captureScreenshot,
  v as STRUCTURAL_ROLES,
  w as assertBrowserNavigationResultAllowed,
  y as ensureOutputDirectory,
} from "./chrome-B-sV0ork.js";
import {
  E as uploadChromeMcpFile,
  T as takeChromeMcpSnapshot,
  _ as navigateChromeMcpPage,
  a as closeChromeMcpTab,
  c as evaluateChromeMcpScript,
  f as getChromeMcpPid,
  l as fillChromeMcpElement,
  m as hoverChromeMcpElement,
  n as clickChromeMcpCoords,
  o as dragChromeMcpElement,
  r as clickChromeMcpElement,
  u as fillChromeMcpForm,
  w as takeChromeMcpScreenshot,
  x as resizeChromeMcpPage,
  y as pressChromeMcpKey,
} from "./chrome-mcp-M0WGdDqO.js";
import { r as resolveBrowserExecutableForPlatform } from "./chrome.executables-BinpIko4.js";
import {
  a as DEFAULT_DOWNLOAD_DIR,
  i as resolveProfile,
  o as DEFAULT_TRACE_DIR,
  r as resolveManagedBrowserHeadlessMode,
...
import {
  ...
  n as BrowserConflictError,
  r as BrowserError,
  s as BrowserResourceExhaustedError,
  u as BrowserValidationError,
} from "./errors-BxBS-H8t.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import { d as pathScope } from "./fs-safe-CgBWiL92.js";
import {
  a as getImageMetadata,
  l as resizeToJpeg,
  r as buildImageResizeSideGrid,
  t as IMAGE_REDUCE_QUALITY_STEPS,
} from "./image-ops-B3yygUDw.js";
...
import "./sdk-setup-tools-BVFP1IkU.js";
import {
  c as ACT_MAX_CLICK_DELAY_MS,
  l as ACT_MAX_WAIT_TIME_MS,
  o as normalizeBrowserFormField,
  p as matchBrowserUrlPattern,
  r as getRoleSnapshotStats,
  u as normalizeActBoundedNonNegativeMs,
} from "./pw-role-snapshot-DkK4EEVr.js";
import { n as normalizeString } from "./record-shared-DTpcMPFX.js";
import { n as deriveDefaultBrowserCdpPortRange } from "./sdk-config-CZ5LC61g.js";
import { a as ensureMediaDir, u as saveMediaBuffer } from "./store-N2etmi9e.js";
import {
  a as normalizeLowercaseStringOrEmpty,
  c as normalizeOptionalString$4,
} from "./string-coerce-BdEutqX5.js";
import {
  a as shouldUsePlaywrightForAriaSnapshot,
  i as resolveDefaultSnapshotFormat,
  n as getPwAiModule$1,
  o as shouldUsePlaywrightForScreenshot,
...
function getProfileContext(req, ctx) {
  ...
  if (typeof req.query.profile === "string")
    profileName = normalizeOptionalString$3(req.query.profile);
...
function toStringOrEmpty(value) {
  if (typeof value === "string" || typeof value === "number" || typeof value === "boolean")
    return normalizeOptionalString$3(String(value)) ?? "";
  return "";
}
...
async function resolveSafeRouteTabUrl(params) {
  const candidateUrl =
    (await params.profileCtx.listTabs().catch(() => [])).find(
      (tab) => tab.targetId === params.targetId,
...
function parseClickModifiers(raw) {
  if (raw.filter((m) => !ALLOWED_CLICK_MODIFIERS.has(m)).length)
...
const EXISTING_SESSION_INTERACTION_NAVIGATION_RECHECK_DELAYS_MS = [0, 250, 500];
  ...
  switch (action.kind) {
    ...
    case "wait":
      return action.loadState === "networkidle"
        ...
        : null;
    case "evaluate":
      return action.timeoutMs !== void 0 ? EXISTING_SESSION_LIMITS.act.evaluateTimeout : null;
    case "batch":
        ...
        run: async ({ profileCtx, cdpUrl, tab, resolveTabUrl }) => {
          ...
          if (isExistingSession) {
            ...
            switch (action.kind) {
              ...
              case "evaluate":
                return await jsonOk({
                  result: await runExistingSessionActionWithNavigationGuard({
                    execute: () =>
                      evaluateChromeMcpScript({
                        profileName,
                        profile: profileCtx.profile,
...
async function normalizeBrowserScreenshot(buffer, opts) {
  ...
  if (buffer.byteLength <= maxBytes && (maxDim === 0 || (width <= maxSide && height <= maxSide)))
    return { buffer };
  const sideGrid = buildImageResizeSideGrid(
    maxSide,
...
function resolveSnapshotPlan(params) {
  ...
  const explicitFormat =
    params.query.format === "aria" ? "aria" : params.query.format === "ai" ? "ai" : void 0;
...
async function renderChromeMcpLabels(params) {
  ...
  return {
    labels:
      result && typeof result === "object" && typeof result.labels === "number" ? result.labels : 0,
    skipped:
      result && typeof result === "object" && typeof result.skipped === "number"
        ? result.skipped
        : 0,
  };
}
      ...
      await withRouteTabContext({
        ...
        run: async ({ profileCtx, tab, cdpUrl }) => {
          if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
            ...
            if (labels) {
              const built = buildAiSnapshotFromChromeMcpSnapshot({
                root: await takeChromeMcpSnapshot({
                  profileName: profileCtx.profile.name,
                  profile: profileCtx.profile,
                  targetId: tab.targetId,
                }),
          ...
          if (
            ...
            buffer = (
              await pw.takeScreenshotViaPlaywright({
                cdpUrl,
                targetId: tab.targetId,
                ref,
                element,
      ...
      try {
        ...
        if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
          ...
          if (plan.labels) {
            ...
            try {
              const normalized = await normalizeBrowserScreenshot(
                await takeChromeMcpScreenshot({
                  profileName: profileCtx.profile.name,
                  profile: profileCtx.profile,
                  targetId: tab.targetId,
                  format: "png",
...
function createBrowserProfilesService(ctx) {
  ...
  const deleteProfile = async (nameRaw) => {
    ...
    if (name === (cfg.browser?.defaultProfile ?? state.resolved.defaultProfile))
      throw new BrowserValidationError(
...
async function runBrowserLiveProbe(req, ctx) {
  ...
  try {
    ...
    if (capabilities.usesChromeMcp) {
      const { takeChromeMcpSnapshot } = await import("./chrome-mcp-B4Glo31r.js");
...
async function grantPermissions(params) {
  ...
  if (
    playwrightRequiredPermissions.every((value) => Boolean(value)) &&
    ...
    if (pw)
      try {
        await (
          ...
          .context()
          .grantPermissions(playwrightRequiredPermissions, { origin: params.origin });
        return {
