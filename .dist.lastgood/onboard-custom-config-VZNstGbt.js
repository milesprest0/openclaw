import { t as normalizeAlias } from "./alias-name-bvPwko2z.js";
import { t as CONTEXT_WINDOW_HARD_MIN_TOKENS } from "./context-window-guard-Bprmj3Gv.js";
import { r as DEFAULT_PROVIDER } from "./defaults-4m7JJmD2.js";
import { _ as modelKey, i as buildModelAliasIndex } from "./model-selection-shared-D9oQINZ2.js";
...
function isAzureFoundryUrl(baseUrl) {
  try {
    return normalizeLowercaseStringOrEmpty(new URL(baseUrl).hostname).endsWith(
...
function buildEndpointIdFromUrl(baseUrl) {
  try {
    ...
    return (
      normalizeEndpointId(
        `custom-${normalizeLowercaseStringOrEmpty(url.hostname.replace(/[^a-z0-9]+/gi, "-"))}${url.port ? `-${url.port}` : ""}`,
      ) || "custom"
    );
  } catch {
    return "custom";
...
function buildOpenAiVerificationProbeRequest(params) {
  ...
  if (isAzureOpenAiUrl(params.baseUrl))
    return {
      endpoint: new URL("responses", transformAzureConfigUrl(params.baseUrl).replace(/\/?$/, "/"))
        .href,
      headers,
      body: {
...
function buildAnthropicVerificationProbeRequest(params) {
  return {
    endpoint: resolveVerificationEndpoint({
      baseUrl: /\/v1\/?$/.test(params.baseUrl.trim())
        ? params.baseUrl.trim()
...
function parseNonInteractiveCustomApiFlags(params) {
  ...
  return {
    ...
    ...(params.supportsImageInput === void 0
      ? {}
...
function applyCustomApiConfig(params) {
  ...
  const explicitInput =
    params.supportsImageInput === void 0 ? void 0 : customModelInputs(params.supportsImageInput);
  ...
  if (isAzure && isLikelyReasoningModel) {
    if (!config.agents?.defaults?.models?.[modelRef]?.params?.thinking)
      config = {
        ...config,
        agents: {
          ...config.agents,
...
//#endregion
export {
  buildOpenAiVerificationProbeRequest as a,
  parseNonInteractiveCustomApiFlags as c,
  resolveCustomProviderId as d,
  buildEndpointIdFromUrl as i,
  resolveCustomModelAliasError as l,
  applyCustomApiConfig as n,
  normalizeEndpointId as o,
  buildAnthropicVerificationProbeRequest as r,
  normalizeOptionalProviderApiKey as s,
  CustomApiError as t,
  resolveCustomModelImageInputInference as u,
};
