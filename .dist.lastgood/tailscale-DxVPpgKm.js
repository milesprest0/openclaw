...
import "./prompt-CiwliLzu.js";
import { r as logVerbose } from "./globals-BdfwDi2E.js";
import "./binaries-C07OXa79.js";
import {
...
async function findTailscaleBinary() {
  const checkBinary = async (path) => {
    ...
    try {
      await Promise.race([
        runExec(path, ["--version"], { timeoutMs: 3e3 }),
...
async function hasTailscaleFunnelRouteForPort(port, exec = runExec) {
  try {
    const { stdout } = await exec(await getTailscaleBinary(), ["funnel", "status", "--json"], {
      maxBuffer: 2e5,
      timeoutMs: 5e3,
    });
    return tailscaleFunnelStatusCoversPort(
      stdout ? parsePossiblyNoisyJsonObject(stdout) : {},
      port,
    );
  } catch {
    return false;
...
function tailscaleFunnelStatusCoversPort(status, port) {
  for (const proxy of funnelStatusBackendsForPort(status))
    if (tailscaleProxyMatchesLoopbackPort(proxy, port)) return true;
  return false;
}
...
async function readTailscaleWhoisIdentity(ip, exec = runExec, opts) {
  ...
  try {
    const { stdout } = await exec(await getTailscaleBinary(), ["whois", "--json", normalized], {
...
//#endregion
export {
  findTailscaleBinary as a,
  readTailscaleWhoisIdentity as c,
  enableTailscaleServe as i,
  disableTailscaleServe as n,
  getTailnetHostname as o,
  enableTailscaleFunnel as r,
  hasTailscaleFunnelRouteForPort as s,
  disableTailscaleFunnel as t,
};
