import { i as resolveSignalAccount, t as listEnabledSignalAccounts } from "./accounts-D_lQP4AN.js";
import { r as resolveReactionMessageId } from "./channel-actions-Ds3BuQCc.js";
import { a as createActionGate, g as readStringParam, l as jsonResult } from "./common-BPZLgNoA.js";
import {
...
function normalizeSignalReactionRecipient(raw) {
  ...
  if (normalizeLowercaseStringOrEmpty(withoutSignal).startsWith("uuid:"))
    return withoutSignal.slice(5).trim();
...
const signalMessageActions = {
  describeMessageTool: ({ cfg, accountId }) => {
    ...
    if (configuredAccounts.some((account) => createActionGate(account.config.actions)("reactions")))
      actions.add("react");
  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    if (action === "react") {
      ...
      if (!reactionLevelInfo.agentReactionsEnabled)
        throw new Error(
          `Signal agent reactions disabled (reactionLevel="${reactionLevelInfo.level}"). Set channels.signal.reactionLevel to "minimal" or "extensive" to enable.`,
        );
      const actionConfig = resolveSignalAccount({
        cfg,
        accountId,
      }).config.actions;
      if (!createActionGate(actionConfig)("reactions"))
        throw new Error("Signal reactions are disabled via actions.reactions.");
      const target = resolveSignalReactionTarget(
