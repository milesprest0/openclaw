...
function buildImageResizeSideGrid(maxSide, sideStart) {
  return [sideStart, 1800, 1600, 1400, 1200, 1e3, 800]
...
const mediaAttachmentImageOpsLoader = createLazyPromiseLoader(async () => {
  const { loadBundledPluginPublicArtifactModuleSync } =
    await import("./public-surface-loader-BZmQQE0W.js");
...
async function sipsMetadataFromBuffer(buffer) {
  return await withImageTemp(async (workspace) => {
    const { stdout } = await runExec(
      "/usr/bin/sips",
      ["-g", "pixelWidth", "-g", "pixelHeight", await workspace.write("in.img", buffer)],
      {
        timeoutMs: 1e4,
        maxBuffer: 512 * 1024,
      },
...
//#endregion
export {
  getImageMetadata as a,
  optimizeImageToPng as c,
  convertHeicToJpeg as i,
  resizeToJpeg as l,
  MAX_IMAGE_INPUT_PIXELS as n,
  hasAlphaChannel as o,
  buildImageResizeSideGrid as r,
  normalizeExifOrientation as s,
  IMAGE_REDUCE_QUALITY_STEPS as t,
  resizeToPng as u,
};
