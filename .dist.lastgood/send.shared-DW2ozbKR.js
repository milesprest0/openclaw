...
import { t as rememberDiscordDirectoryUser } from "./directory-cache-CuRnkbOV.js";
import { n as listDiscordDirectoryPeersLive } from "./directory-live-BeAeN92S.js";
import {
  C as serializePayload,
  Ft as ChannelType$1,
  J as getCurrentUser,
  Kt as require_v10,
  Q as createChannelMessage,
  Ut as PermissionFlagsBits$1,
  bt as getGuild,
  f as RequestClient,
  it as getChannel,
  l as Embed,
  m as RateLimitError,
  q as createUserDmChannel,
...
import {
  ...
  t as allowFromContainsDiscordUserId,
...
import { n as normalizePollInput, t as normalizePollDurationHours } from "./polls-CNbHLDT1.js";
import "./fetch-runtime-D5GYCOX7.js";
import "./retry-runtime-Bpwqs9Bf.js";
import { r as makeProxyFetch } from "./proxy-fetch-Dt1coO5G.js";
import { h as resolveTextChunksWithFallback } from "./reply-payload-D_-IJF8p.js";
import { r as createRateLimitRetryRunner } from "./retry-policy-CBO_PUda.js";
import {
  a as normalizeLowercaseStringOrEmpty,
  c as normalizeOptionalString,
} from "./string-coerce-BdEutqX5.js";
import { t as buildMessagingTarget } from "./targets-B1OkQZHu.js";
...
function readDiscordErrorStatus(err) {
  ...
  const raw =
    "status" in err && err.status !== void 0
      ? err.status
      : "statusCode" in err && err.statusCode !== void 0
        ? err.statusCode
        : void 0;
...
function isRetryableDiscordTransientError(err) {
  ...
  for (const candidate of collectErrorGraphCandidates(err, (current) => [
    ...
    if (status !== void 0 && (DISCORD_RETRYABLE_STATUS_CODES.has(status) || status >= 500))
      return true;
...
function resolveRest(token, account, cfg, rest, proxyFetch) {
  ...
  return createDiscordRequestClient(
    token,
    resolvedProxyFetch ? { fetch: resolvedProxyFetch } : void 0,
  );
}
...
async function resolveDiscordTarget(raw, options, parseOptions = {}) {
  ...
  try {
    const match = (
      await listDiscordDirectoryPeersLive({
        ...options,
        query: trimmed,
        limit: 1,
      })
...
function isConfiguredAllowedDiscordDmUser(input, options) {
  return allowFromContainsDiscordUserId(
...
const PERMISSION_ENTRIES = Object.entries(PermissionFlagsBits$1).filter(
...
function isThreadChannelType(channelType) {
  return (
    channelType === ChannelType$1.GuildNewsThread ||
    channelType === ChannelType$1.GuildPublicThread ||
    channelType === ChannelType$1.GuildPrivateThread
  );
}
...
async function buildDiscordSendError(err, ctx) {
  ...
  const apiDetails = [`code=${code}`, status != null ? `status=${status}` : void 0]
    .filter(Boolean)
    .join(" ");
...
async function resolveDiscordTargetChannelId(raw, opts) {
  const recipient = await parseAndResolveRecipient(
    raw,
    requireRuntimeConfig(opts.cfg, "Discord target channel resolution"),
    opts.accountId,
    { defaultKind: "channel" },
  );
...
//#endregion
export {
  resolveDiscordTarget as A,
  DiscordSendError as C,
  hasAllGuildPermissionsDiscord as D,
  fetchMemberGuildPermissionsDiscord as E,
  resolveDiscordClientAccountContext as F,
  resolveDiscordRest as I,
  DISCORD_REST_TIMEOUT_MS as L,
  createDiscordClient as M,
  createDiscordRestClient as N,
  hasAnyGuildPermissionDiscord as O,
  createDiscordRuntimeAccountContext as P,
  validateDiscordProxyUrl as R,
  DISCORD_MAX_STICKER_BYTES as S,
  fetchChannelPermissionsDiscord as T,
  resolveDiscordSendComponents as _,
  normalizeDiscordPollInput as a,
  DISCORD_MAX_EMOJI_BYTES as b,
  normalizeStickerIds as c,
  resolveDiscordTargetChannelId as d,
  sendDiscordMedia as f,
  buildDiscordMessageRequest as g,
  SUPPRESS_NOTIFICATIONS_FLAG as h,
  formatReactionEmoji as i,
  parseDiscordSendTarget as j,
  parseAndResolveRecipient as k,
  resolveChannelId as l,
  toDiscordFileBlob as m,
  buildDiscordTextChunks as n,
  normalizeEmojiName as o,
  sendDiscordText as p,
  buildReactionIdentifier as r,
  normalizeReactionEmoji as s,
  buildDiscordSendError as t,
  resolveDiscordChannelType as u,
  resolveDiscordSendEmbeds as v,
  canViewDiscordGuildChannel as w,
  DISCORD_MAX_EVENT_COVER_BYTES as x,
  stripUndefinedFields as y,
  withValidatedDiscordProxy as z,
};
