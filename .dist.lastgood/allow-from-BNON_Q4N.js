...
import { n as isAllowedParsedChatSender$1 } from "./chat-target-prefixes-BxhM8xE_.js";
import {
...
function summarizeMapping(label, mapping, unresolved, runtime) {
  ...
  if (mapping.length > 0)
    lines.push(
      `${label} resolved: ${summarizeStringEntries({
        entries: mapping,
        limit: 6,
      })}`,
    );
  if (unresolved.length > 0)
    lines.push(
      `${label} unresolved: ${summarizeStringEntries({
        entries: unresolved,
        limit: 6,
      })}`,
    );
...
//#endregion
export {
  mapAllowlistResolutionInputs as a,
  buildAllowlistResolutionSummary as c,
  patchAllowlistUsersInConfigEntries as d,
  summarizeMapping as f,
  isNormalizedSenderAllowed as i,
  canonicalizeAllowlistWithResolvedIds as l,
  formatNormalizedAllowFromEntries as n,
  mapBasicAllowlistResolutionEntries as o,
  isAllowedParsedChatSender as r,
  addAllowlistUserEntriesFromConfigEntry as s,
  formatAllowFromLowercase as t,
  mergeAllowlist as u,
};
