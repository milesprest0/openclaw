...
import { n as signalRpcRequest } from "./client-DlcnnzTH.js";
import "./text-runtime-lKuAtsoz.js";
import { t as markdownToSignalText } from "./format-BATED3-0.js";
...
import { t as resolveSignalRpcContext } from "./rpc-context-ZlbzjP5N.js";
...
async function sendMessageSignal(to, text, opts) {
  ...
  if (message.trim() && !messageFromPlaceholder)
    if (textMode === "plain") textStyles = opts.textStyles ?? [];
    else {
...
async function sendTypingSignal(to, opts) {
  const { baseUrl, account } = resolveSignalRpcContext(
    opts,
    await resolveSignalRpcAccountInfo(opts),
  );
