...
import { n as refreshMSTeamsDelegatedTokens } from "./oauth.token-DaHsWaEa.js";
import { n as privateFileStoreSync } from "./private-file-store-Bi7hPs_Y.js";
...
function inferPlaceholder(params) {
  ...
  return mime.startsWith("image/") || IMAGE_EXT_RE.test(name) || IMAGE_EXT_RE.test(`x.${fileType}`)
    ? "<media:image>"
    : "<media:document>";
}
...
async function safeFetch(params) {
  ...
  const hasDispatcher = Boolean(
    ...
    "dispatcher" in params.requestInit,
  );
...
function resolveTeamsSdkVersion() {
  try {
    return (
      createRequire(import.meta.url)("@microsoft/teams.apps/package.json").version ?? "unknown"
    );
  } catch {
    return "unknown";
  }
...
async function loadMSTeamsSdk() {
  msTeamsSdkPromise ??= Promise.all([
    import("./dist-C7RIA3uE.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)),
    import("./dist-CyDAy5BJ.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)),
  ...
  return {
    async sendActivity(textOrActivity) {
      ...
      const existingChannelData =
        msg.channelData && typeof msg.channelData === "object" ? msg.channelData : void 0;
...
async function updateActivityViaRest(params) {
  ...
  try {
    if (!response.ok) {
      ...
      throw Object.assign(
        /* @__PURE__ */ new Error(`updateActivity failed: HTTP ${response.status} ${body}`),
...
async function deleteActivityViaRest(params) {
  ...
  try {
    if (!response.ok) {
      const body = await response.text().catch(() => "");
      throw Object.assign(
        /* @__PURE__ */ new Error(`deleteActivity failed: HTTP ${response.status} ${body}`),
        { statusCode: response.status },
      );
    }
  } finally {
...
async function loadBotFrameworkJwtDeps() {
  botFrameworkJwtDepsPromise ??= Promise.all([
    import("./jsonwebtoken-LrgPyxqO.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)),
    import("./src-BIkX6Rkx.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)),
  ]).then(([jwtModule, jwksModule]) => {
    return {
...
async function createBotFrameworkJwtValidator(creds) {
  ...
  const allowedAudiences = [creds.appId, `api://${creds.appId}`, BOT_FRAMEWORK_GLOBAL_AUDIENCE];
  ...
  return {
    async validate(authHeader, _serviceUrl) {
      ...
      try {
        ...
        if (
          getAudienceClaims(verifiedPayload).includes(BOT_FRAMEWORK_GLOBAL_AUDIENCE) &&
...
function resolveMSTeamsCredentials(cfg) {
  ...
  if (authType === "federated") {
    ...
    const certificateThumbprint =
      cfg?.certificateThumbprint || process.env.MSTEAMS_CERTIFICATE_THUMBPRINT || void 0;
    const useManagedIdentity =
      cfg?.useManagedIdentity ?? process.env.MSTEAMS_USE_MANAGED_IDENTITY === "true";
    const managedIdentityClientId =
      cfg?.managedIdentityClientId || process.env.MSTEAMS_MANAGED_IDENTITY_CLIENT_ID || void 0;
...
async function fetchGraphJson(params) {
  return await readOptionalGraphJson(
    await requestGraph({
      token: params.token,
      path: params.path,
      method: params.method,
...
async function resolveGraphToken(cfg, options) {
  ...
  const accessToken = readAccessToken(
    await createMSTeamsTokenProvider(app).getAccessToken("https://graph.microsoft.com"),
  );
...
export {
  ...
  isUrlAllowed as B,
  buildUserAgent as C,
  formatUnknownError as D,
  formatMSTeamsSendErrorHint as E,
  extractHtmlFromAttachment as F,
  resolveRequestUrl as G,
  readNestedString as H,
  extractInlineImageCandidates as I,
  tryBuildGraphSharesUrlForSharedLink as J,
  safeFetchWithPolicy as K,
  inferPlaceholder as L,
  applyAuthorizationHeaderForUrl as M,
  encodeGraphShareId as N,
  isRevokedProxyError as O,
  estimateBase64DecodedBytes as P,
  isDownloadableAttachment as R,
  loadMSTeamsSdkWithAuth as S,
  classifyMSTeamsSendError as T,
  resolveAttachmentFetchPolicy as U,
  normalizeContentType as V,
  resolveMediaSsrfPolicy as W,
  resolveMSTeamsStorePath as _,
  fetchGraphJson as a,
  createMSTeamsAdapter as b,
  normalizeQuery as c,
  postGraphJson as d,
  resolveGraphToken as f,
  saveDelegatedTokens as g,
  resolveMSTeamsCredentials as h,
  fetchGraphAbsoluteUrl as i,
  IMG_SRC_RE as j,
  ATTACHMENT_TAG_RE as k,
  patchGraphJson as l,
  loadDelegatedTokens as m,
  deleteGraphRequest as n,
  listChannelsForTeam as o,
  hasConfiguredMSTeamsCredentials as p,
  safeHostForUrl as q,
  escapeOData as r,
  listTeamsByName as s,
  searchGraphUsers as t,
  postGraphBetaJson as u,
  readAccessToken as v,
  ensureUserAgentHeader as w,
  createMSTeamsTokenProvider as x,
  createBotFrameworkJwtValidator as y,
  isLikelyImageAttachment as z,
};
