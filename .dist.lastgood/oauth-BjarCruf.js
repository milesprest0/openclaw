import {
  a as MSTEAMS_OAUTH_CALLBACK_PORT,
  i as MSTEAMS_OAUTH_CALLBACK_PATH,
  o as MSTEAMS_OAUTH_REDIRECT_URI,
  r as MSTEAMS_DEFAULT_DELEGATED_SCOPES,
  s as buildMSTeamsAuthEndpoint,
  t as exchangeMSTeamsCodeForTokens,
} from "./oauth.token-DaHsWaEa.js";
...
async function manualFlow(ctx, authUrl, state, verifier, params, cause) {
  ...
  if (parsed.state !== state)
    throw new Error("OAuth state mismatch - please try again", cause ? { cause } : void 0);
  ctx.progress.update("Exchanging authorization code for tokens...");
