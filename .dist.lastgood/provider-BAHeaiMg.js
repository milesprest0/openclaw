...
import {
  ...
  d as resolveSlackBotToken,
  o as resolveSlackAccountAllowFrom,
  s as resolveSlackAccountDmPolicy,
  u as resolveSlackAppToken,
...
import { c as resolveTextChunkLimit } from "./chunk-aBEwc7QQ.js";
import { c as resolveSlackWebClientOptions } from "./client-CysiXrBq.js";
...
import { i as formatCommandArgMenuTitle } from "./commands-registry-8mCZwy9x.js";
import {
...
import {
  i as isSlackExecApprovalClientEnabled,
  n as isSlackExecApprovalApprover,
  r as isSlackExecApprovalAuthorizedSender,
} from "./exec-approvals-D2PcQ1IY.js";
import {
...
import { i as installRequestBodyLimitGuard } from "./http-body-D8UGuStR.js";
import "./history-CiK0qvRj.js";
...
import { t as escapeSlackMrkdwn } from "./mrkdwn-apwRwsss.js";
...
import { t as resolveSlackChannelAllowlist } from "./resolve-channels-CTyZkorJ.js";
import "./reply-chunking-D9l7MwoT.js";
import "./system-event-runtime-DLw8xTmR.js";
import { t as resolveSlackUserAllowlist } from "./resolve-users-cAjmPNeb.js";
import "./runtime-config-snapshot-nxqZdkUb.js";
import "./agent-runtime-CG9i2LmY.js";
import {
  a as resolveSlackEffectiveAllowFrom,
  c as normalizeSlackChannelType,
  d as resolveSlackChannelLabel,
  f as buildSlackSlashCommandMatcher,
  i as authorizeSlackSystemEventSender,
  l as resolveSlackChatType,
  m as stripSlackMentionsForCommandDetection,
  n as authorizeSlackDirectMessage,
  o as createSlackMonitorContext,
  p as resolveSlackSlashCommandConfig,
  s as isSlackChannelAllowedByPolicy,
  t as resolveSlackRoomContextHints,
  u as resolveSlackChannelConfig,
} from "./room-context-Di4DZ-0f.js";
...
import { l as normalizeMainKey } from "./session-key-B4qUwRzq.js";
import "./blocks-render-B8p7B_zB.js";
...
import "./blocks-input-B3PJKopi.js";
...
import { i as truncateSlackText, r as SLACK_TEXT_LIMIT } from "./thread-ts-CAy_-OyD.js";
import { u as normalizeResolvedSecretInputString } from "./types.secrets-CaNC1eIn.js";
  ...
  ctx.app.event("channel_id_changed", async ({ event, body }) => {
    try {
      ...
      if (migration.migrated) {
        ...
        ctx.runtime.log?.(
          warn(`[slack] No config found for old channel ID ${oldChannelId}; migration logged only`),
        );
    } catch (err) {
      ctx.runtime.error?.(
...
function collectRichTextFragments(value, out) {
  ...
  if (Array.isArray(typed.elements))
    for (const child of typed.elements) collectRichTextFragments(child, out);
}
...
function summarizeAction(action) {
  ...
  const inputNumber =
    actionType === "number_input" && inputValue != null ? Number.parseFloat(inputValue) : void 0;
  const parsedNumber = Number.isFinite(inputNumber) ? inputNumber : void 0;
  const inputEmail =
    actionType === "email_text_input" && inputValue?.includes("@") ? inputValue : void 0;
  let inputUrl;
  ...
  return {
    ...
    selectedDateTime:
      typeof typed.selected_date_time === "number" ? typed.selected_date_time : void 0,
    inputValue,
...
function formatInteractionSelectionLabel(params) {
  ...
  if (typeof params.summary.selectedDateTime === "number")
    return /* @__PURE__ */ new Date(params.summary.selectedDateTime * 1e3).toISOString();
...
function parseSlackBlockAction(params) {
  ...
  if (!typedAction) {
    params.log?.(
      `slack:interaction malformed action payload channel=${typedBody.channel?.id ?? typedBody.container?.channel_id ?? "unknown"} user=${typedBody.user?.id ?? "unknown"}`,
    );
    return null;
  }
...
async function handleSlackExecApprovalInteraction(params) {
  ...
  if (
    !(approval.approvalId.startsWith("plugin:")
...
async function resolveSlackBlockActionCommandAuthorized(params) {
  ...
  const senderName = (await params.ctx.resolveUserName(params.parsed.userId).catch(() => void 0))
    ?.name;
...
async function handleSlackBlockAction(params) {
  ...
  if (pluginInteractionData && isSlackReplyActionId(parsed.actionId)) {
    if (
      await handleSlackExecApprovalInteraction({
        ctx: params.ctx,
        parsed,
  ...
  if (pluginInteractionData && isSlackReplyActionId(parsed.actionId)) {
    ...
    if (
      await dispatchSlackPluginInteraction({
        ctx: params.ctx,
        parsed,
        pluginInteractionData,
...
function formatSlackInteractionSystemEvent(payload) {
  ...
  eventText = toEventText(
    sanitizeSlackInteractionPayloadValue(buildCompactSlackInteractionPayload(sanitizedPayload)),
  );
  if (eventText.length <= SLACK_INTERACTION_EVENT_MAX_CHARS) return eventText;
  return toEventText({
...
function registerSlackMemberEvents(params) {
  ...
  const handleMemberChannelEvent = async (params) => {
    try {
      ...
      enqueueSystemEvent(
        `Slack: ${(payload.user ? await ctx.resolveUserName(payload.user) : {})?.name ?? payload.user ?? "someone"} ${params.verb} ${ingressContext.channelLabel}.`,
        {
    ...
    Promise.resolve()
      .then(async () => {
        try {
          ...
          ctx.runtime.error?.(
            danger(`slack mention handler failed (detached): ${formatErrorMessage(err)}`),
          );
        }
      })
      .catch((err) =>
        ctx.runtime.error?.(
          danger(`slack mention handler crashed (detached): ${formatErrorMessage(err)}`),
        ),
      );
  });
}
...
async function handleSlackPinEvent(params) {
  ...
  try {
    ...
    if (!ingressContext) return;
    const userLabel =
      (payload.user ? await ctx.resolveUserName(payload.user) : {})?.name ??
      payload.user ??
      "someone";
...
const DEFAULT_APP_MENTION_RETRY_TTL_MS = 9e5;
const MIN_APP_MENTION_RETRY_TTL_MS = 6e4;
function resolveAppMentionRetryTtlMs(cfg, accountId) {
  ...
  if (candidate < MIN_APP_MENTION_RETRY_TTL_MS) return MIN_APP_MENTION_RETRY_TTL_MS;
  return candidate;
}
...
function createSlackMessageHandler(params) {
  ...
  return async (message, opts) => {
    ...
    if (canDebounce && debounceKey && conversationKey) {
      const pendingKeys =
        pendingTopLevelDebounceKeys.get(conversationKey) ?? /* @__PURE__ */ new Set();
...
function addSlackResponseMetadata(details, value) {
  ...
  if (Array.isArray(messages))
    for (const message of messages) addStringDetail(details, "slack message", message);
  const warnings = value.warnings;
  if (Array.isArray(warnings))
    for (const warning of warnings) addStringDetail(details, "slack warning", warning);
}
...
function resolveSlackBoltModule(value) {
  ...
  if (
    !isConstructorFunction(app) ||
    !isConstructorFunction(httpReceiver) ||
    !isConstructorFunction(socketModeReceiver)
  )
    return null;
  return {
    App: app,
...
function resolveSlackBoltInterop(params) {
  ...
  if (
    isConstructorFunction(defaultImport) &&
    isConstructorFunction(namespaceReceiver) &&
    isConstructorFunction(namespaceSocketModeReceiver)
  )
    return {
      App: defaultImport,
...
function shouldSkipOpenClawSlackSelfEvent(args) {
  ...
  if (event?.type === "message" && event.subtype === "message_changed" && event.user === botUserId)
    return false;
...
function createSlackBoltApp(params) {
  ...
  if (params.socketMode?.serverPingTimeout !== void 0)
    socketModeReceiverOptions.serverPingTimeout = params.socketMode.serverPingTimeout;
  if (params.socketMode?.pingPongLoggingEnabled !== void 0)
...
async function startSlackSocketAndWaitForDisconnect(params) {
  ...
  try {
    ...
    disconnectWaiter.cancel();
    if ((err === void 0 || err === null || err === "") && disconnect?.error !== void 0)
...
//#endregion
//#region extensions/slack/src/monitor/reconcile.ts
const DEFAULT_PERIOD_MS = 12e4;
const MIN_PERIOD_MS = 15e3;
const LOOKBACK_SEC = 600;
const MAX_THREAD_SCANS_PER_CHANNEL = 8;
const HISTORY_LIMIT = 50;
const DEFAULT_MAX_DEFER_MS = 30 * 6e4;
/**
 ...
 */
const PENDING_WORK_PROBE_SYMBOL = Symbol.for("prest0n.pendingThreadWork");
/** Exported for tests. "0" disables; blank/invalid -> default; floor 15s. */
function resolveReconcilePeriodMs(raw) {
  if (raw === void 0 || raw.trim() === "") return DEFAULT_PERIOD_MS;
  const n = Number(raw);
  if (!Number.isFinite(n)) return DEFAULT_PERIOD_MS;
  if (n === 0) return null;
  return Math.max(MIN_PERIOD_MS, Math.floor(n));
}
function createSlackReconciler(params) {
  const { client, runtime, handleSlackMessage, getBotUserId, getChannelIds } = params;
  const lastTsByChannel = /* @__PURE__ */ new Map();
  let sweepInFlight = false;
  let timer = null;
  const maxDeferMs = params.maxDeferMs ?? DEFAULT_MAX_DEFER_MS;
  const pendingProbe = (q) => {
    try {
      if (params.isThreadWorkPending) return Boolean(params.isThreadWorkPending(q));
      const probe = globalThis[PENDING_WORK_PROBE_SYMBOL];
      return typeof probe === "function" ? Boolean(probe(q)) : false;
    } catch {
      return false;
    }
  };
  const deferred = /* @__PURE__ */ new Map();
  const botRepliedAfter = (mentionTs, msgs, botUserId) =>
    Array.isArray(msgs) &&
    msgs.some((r) => Boolean(r?.ts) && r.user === botUserId && String(r.ts) > String(mentionTs));
  const replayMention = async (m, channelId, threadTs) => {
    const synth = {
      ...m,
      type: "message",
      channel: channelId,
    ...
    try {
      await handleSlackMessage(synth, {
        source: "app_mention",
        wasMentioned: true,
      });
      runtime.log?.(
   ...
   */
  const maybeReplayMention = async (m, channelId, threadTs, threadMsgs, botUserId) => {
   ...
   */
  const processDeferred = async (botUserId) => {
    let replayed = 0;
      ...
      try {
        ...
        try {
          const rp = await client.conversations.replies({
            channel: d.channelId,
            ts: parentTs,
            oldest: String(Math.floor(d.firstDeferredAtMs / 1e3) - LOOKBACK_SEC),
            limit: HISTORY_LIMIT,
          });
          threadMsgs = Array.isArray(rp?.messages) ? rp.messages : void 0;
        } catch {
          threadMsgs = void 0;
        }
        deferred.delete(key);
        ...
        if (await replayMention(d.m, d.channelId, d.threadTs)) replayed += 1;
      } catch (err) {
        runtime.error?.(`[slack:reconcile] deferred re-check failed key=${key}: ${String(err)}`);
      }
    return replayed;
  };
  const runSlackReconcile = async (reason) => {
    if (sweepInFlight) return { replayed: 0 };
    sweepInFlight = true;
    let replayed = 0;
    try {
      ...
      if (!mentionToken || channels.length === 0) return { replayed };
      replayed += await processDeferred(botUserId);
      const floorSec = Math.floor(Date.now() / 1e3) - LOOKBACK_SEC;
      for (const channelId of channels)
        try {
          ...
          for (const m of msgs) {
            if (!m?.ts) continue;
            if (String(m.ts) > maxTs) maxTs = String(m.ts);
            const text = typeof m.text === "string" ? m.text : "";
            if (
              !Boolean(m.user && m.user === botUserId) &&
              text.includes(mentionToken) &&
              (!lastTs || String(m.ts) > String(lastTs))
            ) {
              if (await maybeReplayMention(m, channelId, void 0, void 0, botUserId)) replayed += 1;
            }
            ...
            if (
              ...
              try {
                ...
                for (const r of Array.isArray(rp?.messages) ? rp.messages : []) {
                  ...
                  if (String(r.ts) > maxTs) maxTs = String(r.ts);
                  const rText = typeof r.text === "string" ? r.text : "";
                  if (r.user && r.user === botUserId) continue;
                  if (!rText.includes(mentionToken)) continue;
                  if (lastTs && String(r.ts) <= String(lastTs)) continue;
                  if (await maybeReplayMention(r, channelId, String(m.ts), rp?.messages, botUserId))
                    replayed += 1;
                }
              } catch (err) {
                runtime.error?.(
          ...
          if (maxTs) lastTsByChannel.set(channelId, maxTs);
          if (replayed > 0)
            runtime.log?.(
  ...
  const scheduleReconcile = (reason) => {
    runSlackReconcile(reason).catch(() => {});
  };
...
function createSlackExternalArgMenuToken(store) {
  let token = "";
  do token = generateSecureToken(SLACK_EXTERNAL_ARG_MENU_TOKEN_BYTES);
  ...
  return {
    ...
    text: {
      ...
      text: truncateSlackText(
        `Run */${escapeSlackMrkdwn(params.command)}* with *${escapeSlackMrkdwn(params.arg)}* set to this value?`,
...
async function registerSlackMonitorSlashCommands(params) {
  ...
  const handleSlashCommand = async (p) => {
    ...
    try {
      ...
      const channelType = normalizeSlackChannelType(
        channelInfo?.type ?? (command.channel_name === "directmessage" ? "im" : void 0),
        command.channel_id,
      );
      ...
      if (isDirectMessage) {
        if (
          !(await authorizeSlackDirectMessage({
            ctx,
            accountId: ctx.accountId,
      ...
      const senderName =
        (await ctx.resolveUserName(command.user_id))?.name ?? command.user_name ?? command.user_id;
  ...
  if (nativeCommands.length > 0) {
    ...
    for (const command of nativeCommands)
      ctx.app.command(`/${command.name}`, async ({ command: cmd, ack, respond, body }) => {
  ...
  const registerArgOptions = () => {
    ...
    appWithOptions.options(SLACK_COMMAND_ARG_ACTION_ID, async ({ ack, body }) => {
      ...
      const token = readSlackExternalArgMenuToken(
        typedBody.actions?.[0]?.block_id ?? typedBody.block_id,
      );
      if (!token) {
        await ack({ options: [] });
        return;
...
async function monitorSlackProvider(opts = {}) {
  ...
  const historyLimit = Math.max(
    0,
    account.config.historyLimit ?? cfg.messages?.groupChat?.historyLimit ?? 50,
  );
