...
async function isCompletionInstalled(shell, binName = "openclaw") {
  ...
  return (await fs.readFile(profilePath, "utf-8"))
    ...
    .some(
      (line) =>
        isCompletionProfileHeader(line) || isCompletionProfileLine(line, binName, cachedPath),
    );
}
/**
...
async function usesSlowDynamicCompletion(shell, binName = "openclaw") {
  ...
  for (const line of lines)
    if (isSlowDynamicCompletionLine(line, binName) && !line.includes(cachePath)) return true;
  return false;
}
...
//#endregion
export {
  isCompletionInstalled as a,
  resolveShellFromEnv as c,
  installCompletion as i,
  usesSlowDynamicCompletion as l,
  COMPLETION_SKIP_PLUGIN_COMMANDS_ENV as n,
  isCompletionShell as o,
  completionCacheExists as r,
  resolveCompletionCachePath as s,
  COMPLETION_SHELLS as t,
};
