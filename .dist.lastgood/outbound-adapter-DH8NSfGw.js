...
import { p as DiscordError } from "./discord-Do-Uybag.js";
...
import { t as createDiscordSendReceipt } from "./send.receipt-CkuBBjk4.js";
import {
...
async function sendDiscordComponentMessageLazy(...args) {
  discordComponentSendPromise ??= import("./send.components-CUddnZ5s.js").then(
...
function loadDiscordSharedInteractive() {
  discordSharedInteractivePromise ??= import("./shared-interactive-CaQZW1RG.js");
  ...
  if (!componentSpec) {
    ...
    if (nativeComponents || embeds?.length || filename)
      return attachChannelToResult(
        "discord",
        await sendPayloadMediaSequenceOrFallback({
          text: payload.text ?? "",
          mediaUrls,
          fallbackResult: {
            messageId: "",
...
function loadDiscordThreadBindings() {
  discordThreadBindingsPromise ??= import("./thread-bindings-DMwGFmF1.js");
  ...
  ...createAttachedChannelResultAdapter({
    ...
    sendMedia: async ({
      ...
      if (audioAsVoice && mediaUrl) {
        const sendVoice =
          resolveOutboundSendDep(deps, "discordVoice") ??
