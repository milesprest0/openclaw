...
import {
  i as stopActiveManagedProxyRegistration,
  n as getActiveManagedProxyUrl,
  r as registerActiveManagedProxyUrl,
  t as getActiveManagedProxyLoopbackMode,
} from "./active-proxy-state-BS8ghEz6.js";
import { u as isLoopbackIpAddress } from "./ip-skS0UDUc.js";
import { a as logWarn, r as logInfo } from "./logger-BWdMCYqK.js";
import { o as forceResetGlobalDispatcher } from "./undici-global-dispatcher-B1L_za-h.js";
...
const NO_PROXY_ENV_KEYS = ["no_proxy", "NO_PROXY", "GLOBAL_AGENT_NO_PROXY"];
...
function withTlsTargetHost(configuration) {
  ...
  if (tlsOptions["servername"] === void 0 && isIP(configuration.host) === 0)
...
function resolveProxyUrl(config) {
  ...
  if (!candidate)
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl or OPENCLAW_PROXY_URL to an http:// forward proxy.",
    );
  if (!isSupportedProxyUrl(candidate))
    throw new Error(
      "proxy: enabled but proxy URL is invalid; set proxy.proxyUrl or OPENCLAW_PROXY_URL to an http:// forward proxy.",
    );
  return candidate;
}
...
function isGatewayControlPlaneLoopbackHost(hostname) {
  return (
    hostname.trim().toLowerCase().replace(/\.+$/, "") === "localhost" ||
    isLoopbackIpAddress(hostname)
  );
}
//#endregion
export {
  withManagedProxyGatewayLoopbackRouting as a,
  stopProxy as i,
  registerManagedProxyGatewayLoopbackNoProxy as n,
  startProxy as r,
  ensureInheritedManagedProxyRoutingActive as t,
};
