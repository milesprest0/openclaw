...
function normalizeModelInput(input) {
  const normalized = Array.isArray(input)
    ? input.filter((item) => item === "text" || item === "image")
    : [];
...
function resolveFoundryApi(modelId, modelNameHint, configuredApi) {
  ...
  return usesFoundryResponsesByDefault(resolveConfiguredModelNameHint(modelId, modelNameHint))
    ? DEFAULT_GPT5_API
    : DEFAULT_API;
}
...
function buildFoundryModelCompat(modelId, modelNameHint, configuredApi) {
  ...
  const needsMaxCompletionTokens = requiresFoundryMaxCompletionTokens(
    resolveConfiguredModelNameHint(modelId, modelNameHint),
  );
...
function resolveFoundryTargetProfileId(config) {
  ...
  return (
    config.auth?.order?.["microsoft-foundry"]?.find((profileId) =>
      normalizeOptionalString(profileId),
    ) ?? (configuredProfileEntries.length === 1 ? configuredProfileEntries[0]?.[0] : void 0)
  );
}
//#endregion
export {
  resolveFoundryModelCapabilities as _,
  TOKEN_REFRESH_MARGIN_MS as a,
  usesFoundryResponsesByDefault as b,
  buildFoundryAuthResult as c,
  isFoundryProviderApi as d,
  listConfiguredFoundryProfileIds as f,
  resolveFoundryApi as g,
  resolveConfiguredModelNameHint as h,
  PROVIDER_ID as i,
  buildFoundryProviderBaseUrl as l,
  requiresFoundryMaxCompletionTokens as m,
  DEFAULT_API as n,
  applyFoundryProfileBinding as o,
  normalizeFoundryEndpoint as p,
  DEFAULT_GPT5_API as r,
  applyFoundryProviderConfig as s,
  COGNITIVE_SERVICES_RESOURCE as t,
  extractFoundryEndpoint as u,
  resolveFoundryTargetProfileId as v,
  supportsFoundryImageInput as y,
};
