...
const COMMAND_CARRIER_EXECUTABLES = new Set(["sudo", "doas", "env", "command", "builtin"]);
...
function parseCarrierOptionToken(
  ...
  standaloneOptions,
  optionsWithValue,
  nonExecutingOptions = /* @__PURE__ */ new Set(),
) {
  if (token.startsWith("--")) {
...
function envInvocationUsesModifiers(argv) {
  return (
    parseEnvInvocationPrelude(argv)?.usesModifiers ??
    normalizeExecutableToken(argv[0] ?? "") === "env"
  );
}
...
function resolveSudoLikeCarriedArgv(argv) {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    const option = parseCarrierOptionToken(
      ...
      standaloneOptions,
      optionsWithValue,
      executable === "sudo" ? SUDO_NON_EXEC_OPTIONS : void 0,
    );
    if (!option) return null;
...
//#endregion
export {
  parseEnvInvocationPrelude as C,
  splitShellArgs as E,
  isEnvAssignmentToken as S,
  normalizeExecutableToken as T,
  resolveDispatchWrapperTrustPlan as _,
  extractShellWrapperInlineCommand as a,
  COMMAND_CARRIER_EXECUTABLES as b,
  isShellWrapperExecutable as c,
  unwrapKnownShellMultiplexerInvocation as d,
  POSIX_INLINE_COMMAND_FLAGS as f,
  isDispatchWrapperExecutable as g,
  extractEnvAssignmentKeysFromDispatchWrappers as h,
  extractShellWrapperCommand as i,
  isShellWrapperInvocation as l,
  resolveInlineCommandMatch as m,
  POWERSHELL_WRAPPERS as n,
  hasEnvManipulationBeforeShellWrapper as o,
  POWERSHELL_INLINE_COMMAND_FLAGS as p,
  extractBindableShellWrapperInlineCommand as r,
  isBlockedShellWrapperCommand as s,
  POSIX_SHELL_WRAPPERS as t,
  resolveShellWrapperTransportArgv as u,
  unwrapDispatchWrappersForResolution as v,
  resolveCarrierCommandArgv as w,
  SOURCE_EXECUTABLES as x,
  unwrapKnownDispatchWrapperInvocation as y,
};
