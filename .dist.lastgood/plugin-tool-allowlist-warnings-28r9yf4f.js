...
function collectToolPolicySources(policy, label, out) {
  ...
  collectToolPolicySources(
    hasRecord(policy.sandbox) ? policy.sandbox.tools : void 0,
    `${label}.sandbox.tools`,
    out,
  );
  collectToolPolicySources(
    hasRecord(policy.subagents) ? policy.subagents.tools : void 0,
    `${label}.subagents.tools`,
    out,
  );
}
...
function collectPluginToolAllowlistWarnings(params) {
  ...
  for (const { source, entry } of exactEntries) {
    ...
    if (owners.length > 0 && owners.length === (toolOwners.get(entry) ?? []).length)
      addIssue(missingToolOwnerIssues, `${entry}\u0000${owners.join("\0")}`, source);
  }
