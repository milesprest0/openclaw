...
async function registerSlashCommands(params) {
  ...
  for (const spec of commands) {
    ...
    const existingNeedsUpdate = existingCmd
      ? existingCmd.url !== callbackUrl || existingCmd.method !== "P"
      : false;
...
function normalizeMessage(text, mediaUrl) {
  return [normalizeOptionalString(text) ?? "", normalizeOptionalString(mediaUrl)]
    .filter(Boolean)
    .join("\n");
}
...
async function resolveMattermostSendContext(to, opts) {
  ...
  const dmRetryOptions = mergeDmRetryOptions(
    ...
    .replace(/https?:\/\/[^\s)\]}]+/giu, (urlText) => {
      try {
        ...
        for (const key of url.searchParams.keys())
          if (SECRET_LOG_KEYS.has(key.toLowerCase())) url.searchParams.set(key, "redacted");
        return url.toString();
      } catch {
        return urlText;
      }
...
function clearMattermostSlashCommandValidationCacheForAccount(accountId) {
  for (const [key, entry] of commandValidationFailureCache)
    if (entry.accountId === accountId) commandValidationFailureCache.delete(key);
  for (const [key, entry] of commandLookupInflight)
    if (entry.accountId === accountId) commandLookupInflight.delete(key);
  for (const [key, entry] of commandValidationLookupRateLimit)
    if (entry.accountId === accountId) commandValidationLookupRateLimit.delete(key);
}
function sweepCommandValidationFailureCache(now = Date.now()) {
  for (const [key, entry] of commandValidationFailureCache)
    if (entry.expiresAt <= now) commandValidationFailureCache.delete(key);
...
function sweepCommandValidationLookupRateLimit(now = Date.now()) {
  ...
  for (const [key, entry] of commandValidationLookupRateLimit)
    if (now - entry.updatedAt > staleAfterMs) commandValidationLookupRateLimit.delete(key);
...
function createSlashCommandHttpHandler(params) {
  ...
  return async (req, res) => {
    ...
    if (
      !(await validateMattermostSlashCommandToken({
        accountId: account.accountId,
        client,
        registeredCommand,
        payload,
...
//#endregion
export {
  resolveCallbackUrl as C,
  registerSlashCommands as S,
  renderMattermostProviderPickerView as _,
  sendMessageMattermost as a,
  cleanupSlashCommands as b,
  computeInteractionCallbackUrl as c,
  setInteractionCallbackUrl as d,
  setInteractionSecret as f,
  renderMattermostModelsPickerView as g,
  parseMattermostModelPickerContext as h,
  registerSlashCommandRoute as i,
  createMattermostInteractionHandler as l,
  buildMattermostAllowedModelRefs as m,
  deactivateSlashCommands as n,
  resolveMattermostOpaqueTarget as o,
  deliverMattermostReplyPayload as p,
  getSlashCommandState as r,
  buildButtonProps as s,
  activateSlashCommands as t,
  resolveInteractionCallbackPath as u,
  resolveMattermostModelPickerCurrentModel as v,
  resolveSlashCommandConfig as w,
  isSlashCommandsEnabled as x,
  DEFAULT_COMMAND_SPECS as y,
};
