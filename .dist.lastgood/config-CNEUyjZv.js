...
const codexAppServerTransportSchema = zod_exports.z.enum(["stdio", "websocket"]);
const codexAppServerPolicyModeSchema = zod_exports.z.enum(["yolo", "guardian"]);
const codexAppServerApprovalPolicySchema = zod_exports.z.enum([
  "never",
  "on-request",
  "on-failure",
  "untrusted",
]);
const codexAppServerSandboxSchema = zod_exports.z.enum([
  "read-only",
  "workspace-write",
  "danger-full-access",
]);
const codexAppServerApprovalsReviewerSchema = zod_exports.z.enum([
  "user",
  "auto_review",
  "guardian_subagent",
]);
const codexDynamicToolsProfileSchema = zod_exports.z.enum(["native-first", "openclaw-compat"]);
const codexDynamicToolsLoadingSchema = zod_exports.z.enum(["searchable", "direct"]);
const codexAppServerServiceTierSchema = zod_exports.z
  .preprocess(
    (value) => (value === null ? null : normalizeCodexServiceTier(value)),
    zod_exports.z.string().trim().min(1).nullable().optional(),
  )
  .optional();
const codexPluginEntryConfigSchema = zod_exports.z
  .object({
    enabled: zod_exports.z.boolean().optional(),
    marketplaceName: zod_exports.z.literal(CODEX_PLUGINS_MARKETPLACE_NAME).optional(),
    pluginName: zod_exports.z.string().trim().min(1).optional(),
    allow_destructive_actions: zod_exports.z.boolean().optional(),
  })
  .strict();
const codexPluginsConfigSchema = zod_exports.z
  .object({
    enabled: zod_exports.z.boolean().optional(),
    allow_destructive_actions: zod_exports.z.boolean().optional(),
    plugins: zod_exports.z.record(zod_exports.z.string(), codexPluginEntryConfigSchema).optional(),
  })
  .strict();
const codexPluginConfigSchema = zod_exports.z
  .object({
    ...
    codexDynamicToolsExclude: zod_exports.z.array(zod_exports.z.string()).optional(),
    discovery: zod_exports.z
      .object({
        enabled: zod_exports.z.boolean().optional(),
        timeoutMs: zod_exports.z.number().positive().optional(),
      })
      .strict()
      .optional(),
    computerUse: zod_exports.z
      .object({
        enabled: zod_exports.z.boolean().optional(),
        autoInstall: zod_exports.z.boolean().optional(),
        marketplaceDiscoveryTimeoutMs: zod_exports.z.number().positive().optional(),
        marketplaceSource: zod_exports.z.string().optional(),
        marketplacePath: zod_exports.z.string().optional(),
        marketplaceName: zod_exports.z.string().optional(),
        pluginName: zod_exports.z.string().optional(),
        mcpServerName: zod_exports.z.string().optional(),
      })
      .strict()
      .optional(),
    codexPlugins: zod_exports.z.unknown().optional(),
    appServer: zod_exports.z
      .object({
        mode: codexAppServerPolicyModeSchema.optional(),
        transport: codexAppServerTransportSchema.optional(),
        command: zod_exports.z.string().optional(),
        args: zod_exports.z
          .union([zod_exports.z.array(zod_exports.z.string()), zod_exports.z.string()])
          .optional(),
        url: zod_exports.z.string().optional(),
        authToken: zod_exports.z.string().optional(),
        headers: zod_exports.z.record(zod_exports.z.string(), zod_exports.z.string()).optional(),
        clearEnv: zod_exports.z.array(zod_exports.z.string()).optional(),
        requestTimeoutMs: zod_exports.z.number().positive().optional(),
        turnCompletionIdleTimeoutMs: zod_exports.z.number().positive().optional(),
        approvalPolicy: codexAppServerApprovalPolicySchema.optional(),
        sandbox: codexAppServerSandboxSchema.optional(),
        approvalsReviewer: codexAppServerApprovalsReviewerSchema.optional(),
        serviceTier: codexAppServerServiceTierSchema,
        defaultWorkspaceDir: zod_exports.z.string().optional(),
      })
      .strict()
      .optional(),
  })
...
function resolveCodexPluginsPolicy(pluginConfig) {
  ...
  return {
    ...
    pluginPolicies: Object.entries(config?.plugins ?? {})
...
function normalizeHeaders(value) {
  ...
  return Object.fromEntries(
    Object.entries(value)
      .map(([key, child]) => [key.trim(), readNonEmptyString(child)])
...
//#endregion
export {
  normalizeCodexServiceTier as a,
  resolveCodexComputerUseConfig as c,
  isCodexFastServiceTier as i,
  resolveCodexPluginsPolicy as l,
  codexAppServerStartOptionsKey as n,
  readCodexPluginConfig as o,
  codexSandboxPolicyForTurn as r,
  resolveCodexAppServerRuntimeOptions as s,
  CODEX_PLUGINS_MARKETPLACE_NAME as t,
  withMcpElicitationsApprovalPolicy as u,
};
