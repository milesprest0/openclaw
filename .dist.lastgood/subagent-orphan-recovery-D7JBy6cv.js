...
import {
  ...
  n as isInternalAnnounceRequesterSession,
...
import {
  a as markSubagentRecoveryAttempt,
  n as evaluateSubagentRecoveryGate,
  o as markSubagentRecoveryWedged,
} from "./subagent-recovery-state-B5uZX9-x.js";
import {
  n as finalizeInterruptedSubagentRun,
  r as replaceSubagentRunAfterSteer,
} from "./subagent-registry-steer-runtime-TWfB-7kq.js";
...
async function recoverOrphanedSubagentSessions(params) {
  ...
  const notifiedRecoverySessionKeys =
    params.notifiedRecoverySessionKeys ?? /* @__PURE__ */ new Set();
      ...
      try {
        ...
        if (!entry) {
          ...
          const __p21bSiblings = (() => {
            ...
            if (typeof rec.endedAt === "number" && rec.endedAt > 0) return false;
            return __p21bBaseOf(String(rec.childSessionKey ?? "")) === __p21bBase;
          });
          const __p21bIsLive = ([, rec]) => {
            const k = String(rec.childSessionKey ?? "");
            if (k && store[k]) return true;
            const ts = Math.max(
              Number(rec.lastCheckpointAt ?? 0),
              Number(rec.lastActivityAt ?? 0),
              Number(rec.heartbeatAt ?? 0),
              Number(rec.updatedAt ?? 0),
            );
            return ts > 0 && __p21bNow - ts < 6e5;
          };
          if (
            ...
            (__p21bKey !== __p21bBase &&
              __p21bSiblings.some(([, rec]) => String(rec.childSessionKey ?? "") === __p21bBase)) ||
            __p21bSiblings.some(
              ([otherId, rec]) =>
                String(rec.childSessionKey ?? "") === __p21bKey && String(otherId) < String(runId),
            )
          ) {
            try {
              runRecord.supersededAt = __p21bNow;
            } catch {}
            log.warn(
          ...
          for (const [, __p21bRec] of __p21bSiblings)
            try {
              __p21bRec.supersededAt = __p21bNow;
            } catch {}
          try {
            runRecord.restartInitiatedAt = __p21bNow;
          } catch {}
          log.warn(
        ...
        if (attemptNumber > 1 && !notifiedRecoverySessionKeys.has(childSessionKey)) {
          if (
            await announceRecoveryInProgress({
              runRecord,
              attemptNumber,
              maxAttempts,
            })
