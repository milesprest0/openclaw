...
import { v as normalizeExecTarget } from "./exec-approvals-VkyGhwX9.js";
import { t as normalizeGroupActivation } from "./group-activation-DbPqof9Q.js";
import "./agent-scope-9AmhTwki.js";
import {
  i as parseVerboseOverride,
  n as applyVerboseOverride,
  r as parseTraceOverride,
  t as applyTraceOverride,
} from "./level-overrides-Dz8iJ1QO.js";
...
import {
  $a as parseSessionLabel,
  gi as errorShape,
  hi as ErrorCodes,
} from "./protocol-CUn5aLrV.js";
import { t as normalizeSendPolicy } from "./send-policy-BQXzC8_d.js";
...
async function applySessionsPatchToStore(params) {
  ...
  const sessionAgentId = normalizeAgentId(
    parseAgentSessionKey(storeKey)?.agentId ?? resolveDefaultAgentId(cfg),
  );
  ...
  if ("thinkingLevel" in patch) {
    ...
    if (raw === null) delete next.thinkingLevel;
      ...
      if (!normalized)
        return invalid(
          `invalid thinkingLevel (use ${formatThinkingLevels(normalizeOptionalString(existing?.providerOverride) || resolvedDefault.provider, normalizeOptionalString(existing?.modelOverride) || resolvedDefault.model, "|", await loadModelCatalogForPatch())})`,
        );
