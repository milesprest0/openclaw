...
import "./runtime-fetch-D3xbEpOu.js";
import {
...
function evictThreadStarterCache() {
  ...
  for (const [cacheKey, entry] of THREAD_STARTER_CACHE.entries())
    if (now - entry.cachedAt > THREAD_STARTER_CACHE_TTL_MS) THREAD_STARTER_CACHE.delete(cacheKey);
...
function isSlackHostname(hostname) {
  ...
  return ["slack.com", "slack-edge.com", "slack-files.com"].some(
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      timeoutHandle = setTimeout(() => {
        ...
        reject(
          /* @__PURE__ */ new Error(
            `slack media download timed out after ${params.totalTimeoutMs}ms`,
          ),
        );
...
async function resolveSlackMedia(params) {
  ...
  const results = (
    await mapLimit(
      files.length > 8 ? files.slice(0, 8) : files,
      ...
      async (file) => {
        ...
        try {
          ...
          if (
            ...
            if (
              normalizeOptionalLowercaseString(fetched.contentType?.split(";")[0]) ===
                "text/html" ||
...
//#endregion
export {
  resolveSlackMedia as a,
  resolveSlackThreadStarter as c,
  resolveSlackAttachmentContent as i,
  MAX_SLACK_MEDIA_FILES as l,
  SLACK_MEDIA_TOTAL_TIMEOUT_MS as n,
  resetSlackThreadStarterCacheForTest as o,
  fetchWithSlackAuth as r,
  resolveSlackThreadHistory as s,
  SLACK_MEDIA_READ_IDLE_TIMEOUT_MS as t,
  formatSlackFileReference as u,
};
