...
import {
  n as describeProcessTool,
  t as describeExecTool,
} from "./bash-tools.descriptions-Bf6WsFyS.js";
import {
  C as markExited,
  S as markBackgrounded,
  T as tail,
  _ as drainSession,
  a as DEFAULT_PENDING_MAX_OUTPUT,
  b as listFinishedSessions,
  c as createApprovalSlug,
  f as resolveApprovalRunningNoticeMs,
  g as deleteSession,
  h as renderExecOutputText,
  i as DEFAULT_PATH,
  m as runExecProcess,
  n as DEFAULT_APPROVAL_TIMEOUT_MS,
  o as applyShellPath,
  p as resolveExecTarget,
  r as DEFAULT_MAX_OUTPUT,
  s as buildApprovalPendingMessage,
  t as DEFAULT_APPROVAL_REQUEST_TIMEOUT_MS,
  u as normalizeNotifyOutput,
  v as getFinishedSession,
  w as setJobTtlMs,
  x as listRunningSessions,
  y as getSession,
} from "./bash-tools.exec-runtime-Brc0frDH.js";
import { n as processSchema, t as execSchema } from "./bash-tools.schemas-bZ_QTpiO.js";
import {
  a as coerceEnv,
  c as readEnvInt,
  d as sliceLogLines,
  f as truncateMiddle,
  i as clampWithDefault,
  l as resolveSandboxWorkdir,
  n as buildSandboxEnv,
  o as deriveSessionName,
  s as pad,
  u as resolveWorkdir,
} from "./bash-tools.shared-xCJhgAkw.js";
import { t as resolveExternalBestEffortDeliveryTarget } from "./best-effort-delivery-C4gtMi_a.js";
import {
  n as recordCommandPoll,
  r as resetCommandPollCount,
} from "./command-poll-backoff-WB2V_P20.js";
import { o as failedTextResult, v as textResult } from "./common-BPZLgNoA.js";
import { i as normalizeDeliveryContext } from "./delivery-context.shared-mj-12Z6_.js";
...
import { s as buildExecApprovalUnavailableReplyPayload } from "./exec-approval-reply-BPlmHS3G.js";
import { r as resolveExecApprovalInitiatingSurfaceState } from "./exec-approval-surface-DdO0jEpr.js";
import {
...
import { r as resolveExecSafeBinRuntimePolicy } from "./exec-safe-bin-runtime-policy-DBRGTZJu.js";
import { d as resolveApprovalAuditCandidatePath } from "./exec-safe-bin-trust-D3CIZbMH.js";
import { t as formatDurationCompact } from "./format-duration-DV-29w1K.js";
import { t as callGatewayTool } from "./gateway-VO6GibHc.js";
import { s as sanitizeHostExecEnvWithDiagnostics } from "./host-env-security-W4c-l4Zm.js";
import {
  J as resolveShellEnvFallbackTimeoutMs,
  K as getShellPathFromLoginShell,
} from "./io-CEQSCTGy.js";
import { t as killProcessTree } from "./kill-tree-BBBZkjzg.js";
import { t as createLazyImportLoader } from "./lazy-promise-B1nDYoKn.js";
import { a as logWarn, r as logInfo } from "./logger-BWdMCYqK.js";
import {
...
import { t as sendMessage } from "./message-Drg-ekN8.js";
import { i as resolveNodeIdFromList, t as listNodes } from "./nodes-utils-B1_2Nbxw.js";
import { n as APPROVALS_SCOPE, o as WRITE_SCOPE } from "./operator-scopes-Codapd9g.js";
import { i as normalizePathPrepend, t as applyPathPrepend } from "./path-prepend-C3srv1EO.js";
import { n as detectPolicyInlineEval } from "./policy-DoWoODSF.js";
import {
  c as describeInterpreterInlineEval,
  t as buildCommandPayloadCandidates,
} from "./risks-C_v3AkAH.js";
import {
  d as sanitizeUserFacingText,
  h as parseExecApprovalResultText,
  m as isExecDeniedResultText,
  p as formatExecDeniedUserMessage,
...
import { t as getProcessSupervisor } from "./supervisor-CDJIP9MZ.js";
import { t as parsePreparedSystemRunPayload } from "./system-run-approval-context-Cja4Yzq-.js";
import {
  n as resolveSystemRunCommandRequest,
  t as formatExecCommand,
} from "./system-run-command-C5E4go0-.js";
import {
  d as EXEC_TOOL_DISPLAY_SUMMARY,
  f as PROCESS_TOOL_DISPLAY_SUMMARY,
...
async function waitForExecApprovalDecision(id) {
  try {
    ...
    if (normalizeLowercaseStringOrEmpty(String(err)).includes("approval expired or not found"))
      return null;
    throw err;
  }
}
...
function formatDirectExecApprovalFollowupText(resultText, opts = {}) {
  ...
  if (parsed.kind === "completed")
    return (
      sanitizeUserFacingText(parsed.body, { errorContext: true }).trim() ||
      "Background command finished."
    );
...
async function createAndRegisterDefaultExecApprovalRequest(params) {
  ...
  return {
    ...
    preResolvedDecision:
      registration.finalDecision === void 0
...
function formatDiagnosticsPrivacy(manifest) {
  ...
  const notes = Array.isArray(privacy.notes)
    ? privacy.notes.filter((note) => typeof note === "string")
    : [];
  ...
  if (params.trigger === "diagnostics") {
    const body = [
      params.outcome.status === "completed" && params.outcome.exitCode === 0
        ...
        : formatDiagnosticsExportFailure({
            outcome: params.outcome,
            exitLabel,
          }),
      params.approvalFollowupText?.trim(),
    ]
      .filter(Boolean)
      .join("\n\n");
...
async function processGatewayAllowlist(params) {
  ...
  if (inlineEvalHit)
    params.warnings.push(
      `Warning: strict inline-eval mode requires explicit approval for ${describeInterpreterInlineEval(inlineEvalHit)}.`,
    );
  let enforcedCommand;
  ...
  if (requiresAsk) {
    ...
    const registerGatewayApproval = async (approvalId) =>
    ...
    (async () => {
      ...
      if (baseDecision.timedOut && askFallback === "allowlist")
        if (!analysisOk || !allowlistSatisfied) deniedReason = "approval-timeout (allowlist-miss)";
...
function buildNodeShellCommand(command, platform) {
  if (normalizeLowercaseStringOrEmpty((platform ?? "").trim()).startsWith("win"))
...
async function resolveNodeExecutionTarget(params) {
  ...
  if (!declaredCommands.includes("system.run"))
    throw new Error(
...
function buildNodeSystemRunInvoke(params) {
  const timeoutMs =
    params.target.runTimeoutSec > 0 ? Math.floor(params.target.runTimeoutSec * 1e3) : 0;
  return {
    ...
    params: {
      ...
      suppressNotifyOnExit:
        params.suppressNotifyOnExit === true || params.notifyOnExit === false ? true : void 0,
    },
...
async function prepareNodeSystemRun(params) {
  ...
  const prepared = parsePreparedSystemRunPayload(
    (
      await callGatewayTool(
        "node.invoke",
        { timeoutMs: 15e3 },
...
async function executeNodeHostCommand(params) {
  ...
  const { analysisOk, allowlistSatisfied, durableApprovalSatisfied, inlineEvalHit } =
    await analyzeNodeApprovalRequirement({
      request: params,
      target,
      prepared,
      hostSecurity,
  ...
  if (requiresAsk) {
    ...
    const registerNodeApproval = async (approvalId) =>
      await registerExecApprovalRequestForHostOrThrow({
        approvalId,
      ...
      (async () => {
        ...
        try {
          ...
          const output = normalizeNotifyOutput(
            [payload.stdout, payload.stderr, payload.error].filter(Boolean).join("\n").slice(-400),
          );
...
const fsSafeModuleLoader = createLazyImportLoader(() => import("./fs-safe-DADAKTWR.js"));
...
function stripPreflightEnvPrefix(argv) {
  ...
  while (idx < argv.length) {
    ...
    if (PREFLIGHT_ENV_OPTIONS_WITH_VALUES.has(option) && !token.includes("=") && idx < argv.length)
      idx += 1;
  }
  ...
  return (
    extractInterpreterScriptTargetFromArgv(
      stripPreflightEnvPrefix(
        /^(?:if|then|do|elif|else|while|until|time)$/i.test(argv[0] ?? "") ? argv.slice(1) : argv,
      ),
    )?.relOrAbsPaths ?? []
  );
}
...
function shouldFailClosedInterpreterPreflight(command) {
  ...
  const isDirectInterpreterCommand =
    Boolean(directExecutable && /^python(?:3(?:\.\d+)?)?$/i.test(directExecutable)) ||
    directExecutable === "node";
  ...
  const hasInterpreterInvocationInSegment = (rawSegment) =>
  ...
  const hasScriptHintInSegment = (segment) =>
  ...
  const hasInterpreterPipelineScriptHintInSameSegment = (rawText) => {
    return splitShellSegmentsOutsideQuotes(rawText, { splitPipes: false }).some((segment) => {
      if (
        !splitShellSegmentsOutsideQuotes(segment, { splitPipes: true })
          .slice(1)
  ...
  return {
    ...
    decision:
      normalizeLowercaseStringOrEmpty(match[2]) === "always"
        ? "allow-always"
        : normalizeLowercaseStringOrEmpty(match[2]),
  };
}
function normalizeCommandBaseName(token) {
  if (!token) return "";
  return normalizeLowercaseStringOrEmpty(token.split(/[\\/]/u).at(-1)).replace(
    /\.(?:cmd|exe)$/u,
    "",
  );
}
...
function createExecTool(defaults) {
  ...
  const agentId =
    defaults?.agentId ??
    (parsedAgentSession ? resolveAgentIdFromSessionKey(defaults?.sessionKey) : void 0);
  return {
    name: "exec",
    label: "exec",
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      const yieldWindow = allowBackground
        ? backgroundRequested
          ? 0
          : clampWithDefault(params.yieldMs ?? defaultBackgroundMs, defaultBackgroundMs, 10, 12e4)
        : null;
      ...
      let security = minSecurity(
        configuredSecurity,
        normalizeExecSecurity(params.security) ?? configuredSecurity,
      );
      ...
      const effectiveTimeout =
        (typeof params.timeout === "number" ? params.timeout : null) ?? defaultTimeoutSec;
...
function encodeKeySequence(request, cursorKeyMode) {
  ...
  if (request.keys?.length)
    for (const token of request.keys) data += encodeKeyToken(token, warnings, cursorKeyMode);
  return {
    data,
    warnings,
  };
...
async function handleProcessSendKeys(params) {
  ...
  const { data, warnings } = encodeKeySequence(
    request,
    params.session.cursorKeyMode === "unknown" ? void 0 : params.session.cursorKeyMode,
  );
      ...
      switch (params.action) {
        ...
        case "write": {
          ...
          return runningSessionResult(
            resolved.session,
            `Wrote ${(params.data ?? "").length} bytes to session ${params.sessionId}${params.eof ? " (stdin closed)" : ""}.`,
          );
        }
        case "send-keys": {
          const resolved = resolveBackgroundedWritableStdin();
        ...
        case "kill": {
          ...
          if (!canceled) {
            if (!terminateSessionFallback(scopedSession))
              return failText(
