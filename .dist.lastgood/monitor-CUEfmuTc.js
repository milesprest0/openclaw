...
import { f as findCodeRegions, p as isInsideCode } from "./assistant-visible-text-DnuPT-bL.js";
import {
...
import { t as createIMessageRpcClient } from "./client-DjZEGIaL.js";
...
import { r as stripInlineDirectiveTagsForDelivery } from "./directive-tags-91V02E1x.js";
import {
...
import { t as isInboundPathAllowed } from "./inbound-path-policy-UV1fIghZ.js";
...
import { t as extractMarkdownFormatRuns } from "./markdown-format-D31FkyTv.js";
...
} from "./media-contract-CI90DJm2.js";
import { n as resolveInboundMentionDecision } from "./mention-gating-DJ8J7HbK.js";
import { n as buildMentionRegexes, r as matchesMentionPatterns } from "./mentions-CX9MvOD0.js";
import { c as kindFromMime } from "./mime-CSQ-Gv-M.js";
import "./channel-policy-y8E9wmaa.js";
import { n as rememberIMessageReplyCache } from "./monitor-reply-cache-CJIXRvqb.js";
import "./reply-history-BYCu-BJA.js";
import "./runtime-env-B60JdRoI.js";
import { t as resolveOutboundAttachmentFromUrl } from "./outbound-attachment-DFY24CXr.js";
import {
...
import "./transport-ready-runtime-BD3mvVf8.js";
import "./runtime-config-snapshot-nxqZdkUb.js";
import "./media-runtime-Cj72foO4.js";
import { t as requireRuntimeConfig } from "./plugin-config-runtime-OXf9zy-H.js";
import {
  n as imessageRpcSupportsMethod,
  t as getCachedIMessagePrivateApiStatus,
} from "./private-api-status-t5b-NfMr.js";
...
import {
  ...
  t as sanitizeOutboundText,
} from "./sanitize-outbound-CHQ-Gh9b.js";
import { r as normalizeScpRemoteHost } from "./scp-host-4o-2UHZG.js";
...
import { c as isRecord, y as truncateUtf16Safe } from "./utils-BGRcpLKt.js";
...
function buildChatTargetParams(to, opts) {
  ...
  return {
    ...
    service:
      opts.service ??
      (target.kind === "handle" ? target.service : void 0) ??
...
const PERSISTED_ECHO_TTL_MS = 120 * 1e3;
...
function loadMirrorIfStale() {
  ...
  mirror = {
    entries: raw
      .split(/\n+/)
      .map(parseEntry)
      .filter((entry) => Boolean(entry && entry.timestamp >= cutoff))
...
async function sendMessageIMessage(to, text, opts) {
  ...
  const target = parseIMessageTarget(opts.chatId ? formatIMessageChatTarget(opts.chatId) : to);
  const service =
    opts.service ?? (target.kind === "handle" ? target.service : void 0) ?? account.config.service;
  ...
  if (opts.mediaUrl?.trim()) {
    const resolved = await (opts.resolveAttachmentImpl ?? resolveOutboundAttachmentFromUrl)(
      opts.mediaUrl.trim(),
      maxBytes,
      {
...
function attachIMessageMonitorAbortHandler(params) {
  ...
  const onAbort = () => {
    ...
    if (subscriptionId)
      params.client.request("watch.unsubscribe", { subscription: subscriptionId }).catch(() => {});
  ...
  for (const payload of replies)
    if (
      (await deliverTextOrMediaReply({
        payload,
        text: resolveSendableOutboundReplyParts(payload, {
          text: convertMarkdownTables(sanitizeOutboundText(payload.text ?? ""), tableMode),
        }).text,
...
var DefaultSentMessageCache = class {
  ...
  cleanup() {
    ...
    for (const [key, timestamp] of this.textCache.entries())
      if (now - timestamp > SENT_MESSAGE_TEXT_TTL_MS) this.textCache.delete(key);
    for (const [key, timestamp] of this.textBackedByIdCache.entries())
      if (now - timestamp > SENT_MESSAGE_TEXT_TTL_MS) this.textBackedByIdCache.delete(key);
    for (const [key, timestamp] of this.messageIdCache.entries())
      if (now - timestamp > SENT_MESSAGE_ID_TTL_MS) this.messageIdCache.delete(key);
  }
};
...
function warnGroupAllowlistMisconfigOnce(params) {
  ...
  params.log(
    `imessage: groupPolicy="allowlist" but channels.imessage.groups is empty for account "${params.accountId}". Every inbound group message will be dropped. Add channels.imessage.groups["*"] = { requireMention: true } to allow all groups, or explicit per-chat_id entries to allow specific groups.`,
  );
  return true;
}
/**
...
function warnGroupAllowlistDropPerChatOnce(params) {
  ...
  params.log(
    `imessage: dropping group message from chat_id=${chat} (account "${params.accountId}") — not in channels.imessage.groups allowlist. Add channels.imessage.groups["${chat}"] or channels.imessage.groups["*"] to allow it.`,
  );
  return true;
}
//#endregion
...
function detectReflectedContent(text) {
  ...
  for (const { re, label } of REFLECTION_PATTERNS)
    if (hasMatchOutsideCode(text, re)) matchedLabels.push(label);
  return {
...
function resolveInboundEchoMessageIds(message) {
  const values = [
    message.id != null ? String(message.id) : void 0,
...
function hasIMessageEchoMatch(params) {
  ...
  for (const scope of scopes) {
    ...
    for (const messageId of params.messageIds)
      if (params.echoCache.has(scope, { messageId })) return true;
...
function resolveIMessageInboundDecision(params) {
  ...
  const destinationCallerIdNormalized =
    normalizeIMessageHandle(destinationCallerId ?? "") || void 0;
  const isSelfChat =
    !isGroup &&
...
function buildIMessageInboundContext(params) {
  ...
  return {
    ctxPayload: finalizeInboundContext({
      ...
      MediaPaths:
        params.media?.paths && params.media.paths.length > 0 ? params.media.paths : void 0,
      MediaTypes:
        params.media?.types && params.media.types.length > 0 ? params.media.types : void 0,
...
function isOptionalStringOrNumber(value) {
  return (
    value === void 0 || value === null || typeof value === "string" || typeof value === "number"
  );
}
...
var DefaultSelfChatCache = class {
  ...
  maybeCleanup() {
    ...
    for (const [key, timestamp] of this.cache.entries())
      if (now - timestamp > SELF_CHAT_TTL_MS) this.cache.delete(key);
...
const warnIfImsgUpgradeNeeded = (() => {
  ...
  return {
    fireOnce: (rpcMethods, runtime) => {
      ...
      runtime.log?.(
        warn(
          `imessage: typing indicators / read receipts gated off (${detail}). Upgrade imsg (current bridge needs typing+read in rpc_methods).`,
        ),
      );
    },
  };
...
async function monitorIMessageProvider(opts = {}) {
  ...
  const historyLimit = Math.max(
    0,
    imessageCfg.historyLimit ?? cfg.messages?.groupChat?.historyLimit ?? 50,
  );
  ...
  async function handleMessageNow(message) {
    ...
    if (shouldLogVerbose()) {
      ...
      logVerbose(
        `imessage inbound: chatId=${chatId ?? "unknown"} from=${ctxPayload.From} len=${(ctxPayload.Body ?? "").length} preview="${preview}"`,
      );
    }
    ...
    await createIMessageRpcClient({
      ...
      onNotification: (msg) => {
        if (msg.method === "message")
          handleMessage(msg.params).catch((err) => {
