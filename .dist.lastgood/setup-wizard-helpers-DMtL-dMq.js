...
import {
  a as resolveChannelDmAllowFrom,
  o as resolveChannelDmPolicy,
} from "./dm-access-CkqmJvoT.js";
import { t as resolveSecretInputModeForEnvSelection } from "./provider-auth-mode-DkMt7nlo.js";
import {
  c as moveSingleAccountChannelSectionToDefaultAccount,
...
function loadProviderAuthInput() {
  providerAuthInputPromise ??= import("./provider-auth-ref-ZpsVsann.js");
  ...
  if (params.includeStatusLine || params.resolveExtraStatusLines)
    status.resolveStatusLines = async ({ cfg, accountId, configured }) => {
      const lines = params.includeStatusLine
        ...
        : [];
      const extraLines =
        (await params.resolveExtraStatusLines?.({
          cfg,
          accountId,
          configured,
        })) ?? [];
...
function setChannelDmPolicyWithAllowFrom(params) {
  ...
  const allowFrom =
    dmPolicy === "open" ? addWildcardAllowFrom(cfg.channels?.[channel]?.allowFrom) : void 0;
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
function setCompatChannelDmPolicyWithAllowFrom(params) {
  const existingAllowFrom = resolveChannelDmAllowFrom({
    account: params.cfg.channels?.[params.channel] ?? {
      allowFrom: void 0,
      dm: void 0,
    },
  });
  ...
  return {
    ...
    getCurrent: (cfg, accountId) => {
      ...
      return resolveChannelDmPolicy({
        account:
          accountId && accountId !== "default" ? channelConfig.accounts?.[accountId] : void 0,
...
function patchConfigForScopedAccount(params) {
  ...
  return patchScopedAccountConfig({
    cfg:
      accountId === "default" || hasExistingAccounts
        ? cfg
...
function createTopLevelChannelParsedAllowFromPrompt(params) {
  ...
  const sharedParams = {
    ...
    getExistingAllowFrom: ({ cfg }) =>
      params.getExistingAllowFrom?.(cfg) ?? cfg.channels?.[params.channel]?.allowFrom ?? [],
    ...(params.mergeEntries ? { mergeEntries: params.mergeEntries } : {}),
...
function createNestedChannelParsedAllowFromPrompt(params) {
  ...
  const sharedParams = {
    ...
    getExistingAllowFrom: ({ cfg }) =>
      ...
      cfg.channels?.[params.channel]?.[params.section]?.allowFrom ??
      [],
    ...(params.mergeEntries ? { mergeEntries: params.mergeEntries } : {}),
    applyAllowFrom: ({ cfg, allowFrom }) => setAllowFrom(cfg, allowFrom),
  };
...
//#endregion
export {
  promptLegacyChannelAllowFromForAccount as A,
  runSingleChannelSecretStep as B,
  parseSetupEntriesAllowingWildcard as C,
  patchTopLevelChannelConfigSection as D,
  patchNestedChannelConfigSection as E,
  resolveAccountIdForConfigure as F,
  setLegacyChannelDmPolicyWithAllowFrom as G,
  setAccountDmAllowFromForChannel as H,
  resolveEntriesWithOptionalToken as I,
  setSetupChannelEnabled as J,
  setNestedChannelAllowFrom as K,
  resolveGroupAllowlistWithLookupNotes as L,
  promptParsedAllowFromForScopedChannel as M,
  promptResolvedAllowFrom as N,
  promptAccountId as O,
  promptSingleChannelSecretInput as P,
  splitSetupEntries as Q,
  resolveParsedAllowFromEntries as R,
  parseMentionOrPrefixedId as S,
  patchChannelConfigForAccount as T,
  setAccountGroupPolicyForChannel as U,
  setAccountAllowFromForChannel as V,
  setChannelDmPolicyWithAllowFrom as W,
  setTopLevelChannelDmPolicyWithAllowFrom as X,
  setTopLevelChannelAllowFrom as Y,
  setTopLevelChannelGroupPolicy as Z,
  createTopLevelChannelParsedAllowFromPrompt as _,
  createAllowFromSection as a,
  noteChannelLookupFailure as b,
  createNestedChannelDmPolicy as c,
  createPromptParsedAllowFromForAccount as d,
  createStandardChannelSetupStatus as f,
  createTopLevelChannelGroupPolicySetter as g,
  createTopLevelChannelDmPolicySetter as h,
  createAccountScopedGroupAccessSection as i,
  promptParsedAllowFromForAccount as j,
  promptLegacyChannelAllowFrom as k,
  createNestedChannelDmPolicySetter as l,
  createTopLevelChannelDmPolicy as m,
  buildSingleChannelSecretPromptState as n,
  createLegacyCompatChannelDmPolicy as o,
  createTopLevelChannelAllowFromSetter as p,
  setNestedChannelDmPolicyWithAllowFrom as q,
  createAccountScopedAllowFromSection as r,
  createNestedChannelAllowFromSetter as s,
  addWildcardAllowFrom as t,
  createNestedChannelParsedAllowFromPrompt as u,
  mergeAllowFromEntries as v,
  parseSetupEntriesWithParser as w,
  noteChannelLookupSummary as x,
  normalizeAllowFromEntries as y,
  resolveSetupAccountId as z,
};
