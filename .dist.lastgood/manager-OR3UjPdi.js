...
import {
  c as isUsageCountedSessionTranscriptFileName,
  i as isSessionArchiveArtifactName,
} from "./artifacts-D7iplKs9.js";
import {
  c as buildSessionEntry,
  l as listSessionFilesForAgent,
  p as sessionPathForFile,
} from "./engine-qmd-xo7d3H-3.js";
import {
...
import { u as enforceEmbeddingMaxInputTokens } from "./memory-core-host-engine-embeddings-C_57wmJ1.js";
import {
  n as listMemoryEmbeddingProviders,
  t as getMemoryEmbeddingProvider,
} from "./memory-embedding-provider-runtime-CXU-sHWt.js";
...
import "./memory-core-host-engine-qmd-BpiWCOTr.js";
import { r as classifyMemoryMultimodalPath } from "./multimodal-91P0oi7L.js";
...
import "./dreaming-shared-IEVm2jng.js";
import { t as canAutoSelectLocal } from "./provider-adapters-woS7vNTR.js";
import { t as extractKeywords } from "./query-expansion-C10uLmwi.js";
import { n as readMemoryFile } from "./read-file--JelGp9n.js";
...
const MEMORY_EMBEDDING_DIMENSION_MISMATCH_ERROR_NAME = "MemoryEmbeddingDimensionMismatchError";
...
function resolveEmbeddingDimensionMismatch(params) {
...
function createMemoryEmbeddingDimensionMismatchError(params) {
...
function isMemoryEmbeddingDimensionMismatchError(err) {
  return (
    err instanceof Error &&
    (err.name === "MemoryEmbeddingDimensionMismatchError" ||
...
function resolveSingletonManagedCache(cacheKey) {
  ...
  if (
    ...
    resolved !== null &&
    resolved.cache instanceof Map &&
    resolved.pending instanceof Map
  )
    return resolved;
  const repaired = {
    cache: /* @__PURE__ */ new Map(),
...
function loadMemoryEmbeddingCache(params) {
  ...
  const baseParams = [provider.id, provider.model, params.providerKey];
  const batchSize = 400;
...
function estimateStructuredEmbeddingInputBytes(input) {
  ...
  for (const part of input.parts)
    if (part.type === "text") total += estimateUtf8Bytes(part.text);
    else {
...
async function removeMemoryIndexFiles(basePath, fileOps = defaultFileOps) {
  await Promise.all(
    ["", "-wal", "-shm"].map((suffix) => fileOps.rm(`${basePath}${suffix}`, { force: true })),
  );
}
...
function resolveMemoryFallbackProviderRequest(params) {
  ...
  if (!fallback || fallback === "none" || !params.currentProviderId) return null;
  const fallbackModel = params.settings.fallbackModel?.trim() || void 0;
  if (fallback === params.currentProviderId && !fallbackModel) return null;
  return {
    provider: fallback,
    model:
      fallbackModel ??
...
function resolveConfiguredSourcesForMeta(sources) {
  const normalized = Array.from(sources)
    .filter((source) => source === "memory" || source === "sessions")
...
function normalizeMetaSources(meta) {
  ...
  const normalized = Array.from(
    new Set(meta.sources.filter((source) => source === "memory" || source === "sessions")),
  ).toSorted((left, right) => left.localeCompare(right));
  return normalized.length > 0 ? normalized : ["memory"];
}
...
async function runMemoryTargetedSessionSync(params) {
  ...
  try {
    ...
    if (!(params.shouldFallbackOnError(reason) && (await params.activateFallbackProvider(reason))))
      throw err;
    const reindexParams = {
...
function shouldIgnoreMemoryWatchPath(watchPath, stats, multimodalSettings) {
  ...
  if (
    normalized
      ...
      .some((segment) => IGNORED_MEMORY_WATCH_DIR_NAMES.has(segment))
  )
    return true;
...
var MemoryManagerSyncOps = class {
  ...
  ensureVectorTable(dimensions) {
    ...
    this.db.exec(
      `CREATE VIRTUAL TABLE IF NOT EXISTS ${VECTOR_TABLE$2} USING vec0(\n  id TEXT PRIMARY KEY,\n  embedding FLOAT[${dimensions}]\n)`,
    );
  ...
  openDatabase() {
    return openMemoryDatabaseAtPath(
  ...
  async seedEmbeddingCache(sourceDb) {
    ...
    try {
      const rows = sourceDb
        .prepare(
          `SELECT provider, model, provider_key, hash, embedding, dims, updated_at FROM ${EMBEDDING_CACHE_TABLE$2}`,
        )
        .iterate();
      ...
      for (const row of rows) {
        if (!insert) {
          insert = this.db
            .prepare(`INSERT INTO ${EMBEDDING_CACHE_TABLE$2} (provider, model, provider_key, hash, embedding, dims, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?)
             ON CONFLICT(provider, model, provider_key, hash) DO UPDATE SET
               embedding=excluded.embedding,
               dims=excluded.dims,
  ...
  async syncMemoryFiles(params) {
    ...
    const deleteVectorRowsByPathAndSource =
      this.vector.enabled && this.vector.available
        ? this.db.prepare(
            `DELETE FROM ${VECTOR_TABLE$2} WHERE id IN (SELECT id FROM chunks WHERE path = ? AND source = ?)`,
          )
        : null;
    const deleteFtsRowsByPathAndSource =
      this.fts.enabled && this.fts.available
        ? this.db.prepare(`DELETE FROM ${FTS_TABLE$2} WHERE path = ? AND source = ?`)
        : null;
    const fileEntries = (
  ...
  async syncSessionFiles(params) {
    ...
    const deleteFtsRowsByPathSourceAndModel =
      this.fts.enabled && this.fts.available
        ? this.db.prepare(`DELETE FROM ${FTS_TABLE$2} WHERE path = ? AND source = ? AND model = ?`)
        : null;
  ...
  async activateFallbackProvider(reason) {
    ...
    log$3.warn(`memory embeddings: switched to fallback provider (${fallbackRequest.provider})`, {
      reason,
    });
    return true;
  }
...
function replaceMemoryVectorRow(params) {
  ...
  params.db
    ...
    .run(params.id, vectorToBlob$1(params.embedding));
}
//#endregion
...
function resolveEmbeddingTimeoutMs(params) {
  ...
  if (typeof configuredTimeoutSeconds === "number" && configuredTimeoutSeconds > 0)
    return configuredTimeoutSeconds * 1e3;
...
var MemoryManagerEmbeddingOps = class extends MemoryManagerSyncOps {
  pruneEmbeddingCacheIfNeeded() {
    ...
    if (!max || max <= 0) return;
    const count =
      this.db.prepare(`SELECT COUNT(*) as c FROM ${EMBEDDING_CACHE_TABLE$1}`).get()?.c ?? 0;
    if (count <= max) return;
    const excess = count - max;
    this.db
      .prepare(
        `DELETE FROM ${EMBEDDING_CACHE_TABLE$1}\n WHERE rowid IN (\n   SELECT rowid FROM ${EMBEDDING_CACHE_TABLE$1}\n   ORDER BY updated_at ASC\n   LIMIT ?\n )`,
      )
      .run(excess);
  }
  ...
  async resetBatchFailureCount() {
    await this.withBatchFailureLock(async () => {
      if (this.batchFailureCount > 0)
        log$2.debug("memory embeddings: batch recovered; resetting failure count");
  ...
  async runBatchWithFallback(params) {
    ...
    try {
      ...
      log$2.warn(
        `memory embeddings: ${params.provider} batch failed (${failure.count}/2); ${suffix}; falling back to non-batch embeddings: ${message}`,
      );
  ...
  clearIndexedFileData(pathname, source) {
    if (this.vector.enabled)
      try {
        this.db
          .prepare(
            `DELETE FROM ${VECTOR_TABLE$1} WHERE id IN (SELECT id FROM chunks WHERE path = ? AND source = ?)`,
          )
          .run(pathname, source);
      } catch {}
    if (this.fts.enabled && this.fts.available)
  ...
  writeChunks(entry, source, model, chunks, embeddings, vectorReady) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (this.fts.enabled && this.fts.available)
        this.db
          .prepare(
            `INSERT INTO ${FTS_TABLE$1} (text, id, path, source, model, start_line, end_line)\n VALUES (?, ?, ?, ?, ?, ?, ?)`,
          )
  ...
  async indexFile(entry, options) {
    if (!this.provider) {
      ...
      const chunks = filterNonEmptyMemoryChunks(
        chunkMarkdown(
          options.content ?? (await fs$1.readFile(entry.absPath, "utf-8")),
      ...
      if (
        readCount(
          params.db
            ...
            .get(params.providerModel, ...params.sourceFilterVec.params),
        ) > rows.length
      ) {
...
function searchChunksByEmbedding(params) {
 ...
 WHERE model = ?${params.sourceFilter.sql}`)
    .iterate(params.providerModel, ...params.sourceFilter.params);
  const topResults = [];
  for (const row of rows) {
...
async function searchKeyword(params) {
  ...
  FROM ${params.ftsTable}\n WHERE 1=1${fallbackLikeClause}${modelClause}${params.sourceFilter.sql}\n LIMIT ?`)
        .all(...fallbackLikeParams, ...modelParams, ...params.sourceFilter.params, params.limit);
    }
  else
    rows = params.db
      .prepare(`SELECT id, path, source, start_line, end_line, text,
  ...
  FROM ${params.ftsTable}\n WHERE 1=1${substringClause}${modelClause}${params.sourceFilter.sql}\n LIMIT ?`)
      .all(...substringParams, ...modelParams, ...params.sourceFilter.params, params.limit);
  return rows.map((row) => {
...
const { cache: INDEX_CACHE, pending: INDEX_CACHE_PENDING } = resolveSingletonManagedCache(
        ...
        fullQueryResults.length > 0
          ...
          : await Promise.all(
              (() => {
                ...
                return (keywords.length > 0 ? keywords : [cleaned]).map((term) =>
                  this.searchKeyword(
                    term,
                    candidates,
                    { boostFallbackRanking: true },
