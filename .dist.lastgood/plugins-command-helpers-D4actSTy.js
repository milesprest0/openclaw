...
import { t as applyExclusiveSlotSelection } from "./slots-DwcRLyGF.js";
...
function mergeRuntimeKinds(report, runtimeReport) {
  const runtimeKinds = new Map(
    runtimeReport.plugins.filter((plugin) => plugin.kind).map((plugin) => [plugin.id, plugin.kind]),
  );
  return {
...
//#endregion
export {
  formatPluginInstallWithHookFallbackError as a,
  parseNpmPackPrefixPath as c,
  resolveFileNpmSpecToLocalPath as d,
  enableInternalHookEntries as i,
  parseNpmPrefixSpec as l,
  createHookPackInstallLogger as n,
  logHookPackRestartHint as o,
  createPluginInstallLogger as r,
  logSlotWarnings as s,
  applySlotSelectionForPlugin as t,
  quietPluginJsonLogger as u,
};
