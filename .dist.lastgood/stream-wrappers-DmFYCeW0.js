...
function resolveAnthropicBetas(extraParams, modelId) {
  ...
  if (typeof configured === "string" && configured.trim()) betas.add(configured.trim());
    ...
    for (const beta of configured)
      if (typeof beta === "string" && beta.trim()) betas.add(beta.trim());
  }
  if (extraParams?.context1m === true)
    if (isAnthropic1MModel(modelId)) betas.add(ANTHROPIC_CONTEXT_1M_BETA);
...
function createAnthropicBetaHeadersWrapper(baseStreamFn, betas) {
  ...
  return (model, context, options) => {
    ...
    const allBetas = [
      ...new Set([
        ...(isOauth ? PI_AI_OAUTH_ANTHROPIC_BETAS : PI_AI_DEFAULT_ANTHROPIC_BETAS),
        ...effectiveBetas,
      ]),
    ];
...
function wrapAnthropicProviderStream(ctx) {
  ...
  return composeProviderStreamWrappers(
    ...
    anthropicBetas?.length
      ...
      : void 0,
    serviceTier ? (streamFn) => createAnthropicServiceTierWrapper(streamFn, serviceTier) : void 0,
    fastMode !== void 0 ? (streamFn) => createAnthropicFastModeWrapper(streamFn, fastMode) : void 0,
...
//#endregion
export {
  createAnthropicThinkingPrefillWrapper as a,
  resolveAnthropicServiceTier as c,
  createAnthropicServiceTierWrapper as i,
  wrapAnthropicProviderStream as l,
  createAnthropicBetaHeadersWrapper as n,
  resolveAnthropicBetas as o,
  createAnthropicFastModeWrapper as r,
  resolveAnthropicFastMode as s,
  __testing as t,
};
