import { n as CodexAppServerRpcError } from "./client-PeiS3WVh.js";
import { t as isJsonObject } from "./protocol-DayKrDMy.js";
import { a as getSharedCodexAppServerClient, o as withTimeout } from "./shared-client-NipDnwuu.js";
...
function selectNextRateLimitReset(value, nowMs) {
  const futureWindows = collectCodexRateLimitSnapshots(value)
    .flatMap((snapshot) =>
  ...
  const exhaustedWindows = futureWindows.filter(
    (window) => window.usedPercent !== void 0 && window.usedPercent >= 100,
  );
...
function isRateLimitSnapshot(value) {
  return (
    ...
    value.rateLimitReachedType !== void 0 ||
    value.limitId !== void 0 ||
    value.limitName !== void 0
  );
}
...
function formatCodexStatus(probes) {
  const lines = [
    `Codex app-server: ${probes.models.ok || probes.account.ok || probes.limits.ok || probes.mcps.ok || probes.skills.ok ? "connected" : "unavailable"}`,
  ];
  if (probes.models.ok)
    lines.push(
      `Models: ${
  ...
  lines.push(
    `Account: ${probes.account.ok ? formatCodexAccountSummary(probes.account.value) : formatCodexDisplayText(probes.account.error)}`,
  );
  lines.push(
    `Rate limits: ${probes.limits.ok ? formatCodexRateLimitSummary(probes.limits.value) : formatCodexDisplayText(probes.limits.error)}`,
  );
  lines.push(
    `MCP servers: ${probes.mcps.ok ? summarizeArrayLike(probes.mcps.value) : formatCodexDisplayText(probes.mcps.error)}`,
  );
  lines.push(
    `Skills: ${probes.skills.ok ? summarizeArrayLike(probes.skills.value) : formatCodexDisplayText(probes.skills.error)}`,
  );
  return lines.join("\n");
}
...
function formatAccount(account, limits) {
  return [
    ...
    `Rate limits: ${limits.ok ? formatCodexRateLimitSummary(limits.value) : formatCodexDisplayText(limits.error)}`,
  ].join("\n");
}
function formatComputerUseStatus(status) {
  ...
  lines.push(
    `MCP server: ${formatCodexDisplayText(status.mcpServerName)}${status.mcpServerAvailable ? ` (${status.tools.length} tools)` : " (unavailable)"}`,
  );
...
//#endregion
export {
  formatCodexStatus as a,
  formatModels as c,
  formatCodexUsageLimitErrorMessage as d,
  CODEX_CONTROL_METHODS as f,
  formatCodexDisplayText as i,
  formatThreads as l,
  buildHelp as n,
  formatComputerUseStatus as o,
  describeControlFailure as p,
  formatAccount as r,
  formatList as s,
  requestCodexAppServerJson as t,
  readString as u,
};
