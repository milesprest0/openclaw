...
import { G as getShellEnvAppliedKeys } from "./io-CEQSCTGy.js";
...
import {
  N as resolveProviderSyntheticAuthWithPlugin,
  W as shouldDeferProviderSyntheticProfileAuthWithPlugin,
  c as buildProviderMissingAuthMessageWithPlugin,
...
function hasExplicitProviderApiKeyConfig(providerConfig) {
  return (
    normalizeOptionalSecretInput(providerConfig.apiKey) !== void 0 ||
...
function resolveModelAuthMode(provider, cfg, store, options) {
  ...
  if (profiles.length > 0) {
    const modes = new Set(
      profiles.map((id) => authStore.profiles[id]?.type).filter((mode) => Boolean(mode)),
    );
    if (["oauth", "token", "api_key"].filter((k) => modes.has(k)).length >= 2) return "mixed";
    if (modes.has("oauth")) return "oauth";
    if (modes.has("token")) return "token";
    if (modes.has("api_key")) return "api-key";
  }
  if (authOverride === void 0 && normalizeProviderId(resolved) === "amazon-bedrock")
    return "aws-sdk";
...
function applyAuthHeaderOverride(model, auth, cfg) {
  ...
  if (model.headers) {
    for (const [key, value] of Object.entries(model.headers))
      if (normalizeOptionalLowercaseString(key) !== "authorization") headers[key] = value;
  }
...
//#endregion
export {
  hasAvailableAuthForProvider as a,
  hasUsableCustomProviderApiKey as c,
  resolveUsableCustomProviderApiKey as d,
  shouldPreferExplicitConfigApiKeyAuth as f,
  getCustomProviderApiKey as i,
  resolveApiKeyForProvider as l,
  applyLocalNoAuthHeaderOverride as n,
  hasRuntimeAvailableProviderAuth as o,
  getApiKeyForModel as r,
  hasSyntheticLocalProviderAuthConfig as s,
  applyAuthHeaderOverride as t,
  resolveModelAuthMode as u,
};
