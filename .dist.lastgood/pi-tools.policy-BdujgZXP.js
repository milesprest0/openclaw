...
import { r as resolveChannelGroupToolsPolicy } from "./group-policy-DuzLZrql.js";
...
import { t as resolveSessionConversation } from "./session-conversation-YLqp8hUs.js";
import {
...
import {
  ...
  n as resolveStoredSubagentCapabilities,
...
function resolveEffectiveToolPolicy(params) {
  const agentId =
    (typeof params.agentId === "string" && params.agentId.trim()
      ...
      : void 0) ?? (params.sessionKey ? resolveAgentIdFromSessionKey(params.sessionKey) : void 0);
  const agentTools = (
    params.config && agentId ? resolveAgentConfig(params.config, agentId) : void 0
  )?.tools;
  ...
  if (profile) {
    ...
    if (implicitGrants) {
      ...
      if (uncovered.length > 0)
        logWarn(
          `tools policy: profile "${profile}"${agentId ? ` (agent "${agentId}")` : ""} has configured tool sections (${formatImplicitToolSections(uncoveredEntries.map((entry) => entry.section))}) that no longer implicitly widen the profile. Add alsoAllow: [${formatToolListForWarning(uncovered)}] explicitly if these tools should be available. See #47487.`,
        );
    }
  }
...
function resolveGroupToolPolicy(params) {
  ...
  const channel = normalizeMessageChannel(
    sessionContext.channel ?? spawnedContext.channel ?? params.messageProvider,
  );
  if (!channel) return;
  let plugin;
  try {
...
//#endregion
export {
  resolveTrustedGroupId as a,
  resolveSubagentToolPolicyForSession as i,
  resolveEffectiveToolPolicy as n,
  resolveGroupToolPolicy as r,
  filterToolsByPolicy as t,
};
