...
async function runGatewayConfigHealth(ctx) {
  const { formatCliCommand } = await import("./command-format-DAIzgsCb.js");
  const { hasAmbiguousGatewayAuthModeConfig } = await import("./auth-mode-policy-BEDazvuX.js");
  const { note } = await import("./note-CROv1ngR.js");
...
async function runAuthProfileHealth(ctx) {
  const { maybeRepairLegacyFlatAuthProfileStores } =
    await import("./doctor-auth-flat-profiles-CEUxLMue.js");
  const { maybeRepairLegacyOAuthProfileIds } =
    await import("./doctor-auth-legacy-oauth-3GrAIW5e.js");
  const { noteAuthProfileHealth, noteLegacyCodexProviderOverride } =
    await import("./doctor-auth-BSOhdb-k.js");
  const { buildGatewayConnectionDetails } = await import("./call-DryNwhyi.js");
  const { note } = await import("./note-CROv1ngR.js");
...
async function runGatewayAuthHealth(ctx) {
  const { resolveSecretInputRef } = await import("./types.secrets-CaSQtz8i.js");
  const { resolveGatewayAuth } = await import("./auth-z8QAjTu_.js");
  const { note } = await import("./note-CROv1ngR.js");
  const { randomToken } = await import("./onboard-helpers-BTj6NKFG.js");
...
async function runCommandOwnerHealth(ctx) {
  const { noteCommandOwnerHealth } = await import("./doctor-command-owner-DHnGFL7S.js");
  noteCommandOwnerHealth(ctx.cfg);
}
async function runClaudeCliHealth(ctx) {
  const { noteClaudeCliHealth } = await import("./doctor-claude-cli-BFE7IpHo.js");
  noteClaudeCliHealth(ctx.cfg);
}
async function runLegacyStateHealth(ctx) {
  const { detectLegacyStateMigrations, runLegacyStateMigrations } =
    await import("./doctor-state-migrations-DFzJOGJ1.js");
  const { note } = await import("./note-CROv1ngR.js");
...
async function runLegacyPluginManifestHealth(ctx) {
  const { maybeRepairLegacyPluginManifestContracts } =
    await import("./doctor-plugin-manifests-BRL5u5wO.js");
...
async function runPluginRegistryHealth(ctx) {
  const { maybeRepairPluginRegistryState } = await import("./doctor-plugin-registry-BugIPSZh.js");
...
async function runReleaseConfiguredPluginInstallsHealth(ctx) {
  ...
  const { maybeRunConfiguredPluginInstallReleaseStep } =
    await import("./release-configured-plugin-installs-DKSY3a6K.js");
  const { note } = await import("./note-CROv1ngR.js");
  const { VERSION } = await import("./version-CPHvR5h9.js");
...
async function runStateIntegrityHealth(ctx) {
  const { noteStateIntegrity } = await import("./doctor-state-integrity-DoRXXnDi.js");
  await noteStateIntegrity(ctx.cfg, ctx.prompter, ctx.configPath);
}
async function runCodexSessionRouteHealth(ctx) {
  const { maybeRepairCodexSessionRoutes } = await import("./codex-route-warnings-Au8YQc_l.js");
  const { note } = await import("./note-CROv1ngR.js");
...
async function runSessionLocksHealth(ctx) {
  const { noteSessionLockHealth } = await import("./doctor-session-locks-cc93eul_.js");
  await noteSessionLockHealth({ shouldRepair: ctx.prompter.shouldRepair });
}
async function runSessionTranscriptsHealth(ctx) {
  const { noteSessionTranscriptHealth } = await import("./doctor-session-transcripts-K83fGcZQ.js");
...
async function runLegacyCronHealth(ctx) {
  const { maybeRepairLegacyCronStore, noteLegacyWhatsAppCrontabHealthCheck } =
    await import("./doctor-cron-DvC_pWpr.js");
...
async function runSandboxHealth(ctx) {
  const { maybeRepairSandboxImages, maybeRepairSandboxRegistryFiles, noteSandboxScopeWarnings } =
    await import("./doctor-sandbox-57HUK1Pf.js");
...
async function runGatewayServicesHealth(ctx) {
  const { maybeRepairGatewayServiceConfig, maybeScanExtraGatewayServices } =
    await import("./doctor-gateway-services-DL1nfsXI.js");
  const { noteMacLaunchAgentOverrides, noteMacLaunchctlGatewayEnvOverrides } =
    await import("./doctor-platform-notes-C9jDUq0f.js");
...
async function runStartupChannelMaintenanceHealth(ctx) {
  const { maybeRunDoctorStartupChannelMaintenance } =
    await import("./doctor-startup-channel-maintenance-CqSR2sa4.js");
...
async function runSecurityHealth(ctx) {
  const { noteSecurityWarnings } = await import("./doctor-security-FKu68eBy.js");
  await noteSecurityWarnings(ctx.cfg);
}
async function runBrowserHealth(ctx) {
  const { noteChromeMcpBrowserReadiness } = await import("./doctor-browser-D-UMSwLn.js");
  await noteChromeMcpBrowserReadiness(ctx.cfg);
}
async function runOpenAIOAuthTlsHealth(ctx) {
  const { noteOpenAIOAuthTlsPrerequisites } = await import("./oauth-tls-preflight-DZ-LlJfN.js");
...
async function runHooksModelHealth(ctx) {
  ...
  const { loadModelCatalog } = await import("./model-catalog-iNiq3Mxw.js");
  const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } =
    await import("./model-selection-BlSYWnGK.js");
  const { note } = await import("./note-CROv1ngR.js");
...
async function runSystemdLingerHealth(ctx) {
  if (
    ...
    return;
  const { resolveGatewayService } = await import("./service-Cmo96PKF.js");
  const { ensureSystemdUserLingerInteractive } = await import("./systemd-linger-BiTzOG4G.js");
  const { note } = await import("./note-CROv1ngR.js");
...
async function runWorkspaceStatusHealth(ctx) {
  const { noteWorkspaceStatus } = await import("./doctor-workspace-status-DZVsLQHT.js");
  noteWorkspaceStatus(ctx.cfg);
}
async function runSkillsHealth(ctx) {
  const { maybeRepairSkillReadiness } = await import("./doctor-skills-CJT-NPng.js");
...
async function runBootstrapSizeHealth(ctx) {
  const { noteBootstrapFileSize } = await import("./doctor-bootstrap-size-C5YSBvXm.js");
  await noteBootstrapFileSize(ctx.cfg);
}
async function runShellCompletionHealth(ctx) {
  const { doctorShellCompletion } = await import("./doctor-completion-BvNCrfw4.js");
...
async function runGatewayHealthChecks(ctx) {
  const { checkGatewayHealth, probeGatewayMemoryStatus } =
    await import("./doctor-gateway-health-1pAjnZ3G.js");
...
async function runWhatsappResponsivenessHealth(ctx) {
  const { noteWhatsappResponsivenessHealth } =
    await import("./doctor-whatsapp-responsiveness-9KhAqQXI.js");
...
async function runMemorySearchHealthContribution(ctx) {
  const { maybeRepairMemoryRecallHealth, noteMemoryRecallHealth, noteMemorySearchHealth } =
    await import("./doctor-memory-search-CXT42PQh.js");
...
async function runDevicePairingHealth(ctx) {
  const { noteDevicePairingHealth } = await import("./doctor-device-pairing-BCiF6UhE.js");
...
async function runGatewayDaemonHealth(ctx) {
  const { maybeRepairGatewayDaemon } = await import("./doctor-gateway-daemon-flow-DmdUpywt.js");
...
async function runWriteConfigHealth(ctx) {
  const { formatCliCommand } = await import("./command-format-DAIzgsCb.js");
  const { applyWizardMetadata } = await import("./onboard-helpers-BTj6NKFG.js");
  const { CONFIG_PATH, replaceConfigFile } = await import("./config/config.js");
  const { logConfigUpdated } = await import("./logging-CDeNTkKE.js");
  const { shortenHomePath } = await import("./utils-CG1GL-dA.js");
  if (
...
async function runWorkspaceSuggestionsHealth(ctx) {
  ...
  const { resolveAgentWorkspaceDir, resolveDefaultAgentId } =
    await import("./agent-scope-Bm8ZjMdO.js");
  const { noteWorkspaceBackupTip } = await import("./doctor-state-integrity-DoRXXnDi.js");
  const { MEMORY_SYSTEM_PROMPT, shouldSuggestMemorySystem } =
    await import("./doctor-workspace-pE-S5kqC.js");
  const { note } = await import("./note-CROv1ngR.js");
