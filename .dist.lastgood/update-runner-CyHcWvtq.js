...
import {
  a as resolveControlUiDistIndexPathForRoot,
...
import { i as readJsonIfExists } from "./json-files-DifBk3kt.js";
import {
  i as resolvePosixNpmScriptShell,
  r as hasNpmScriptShellSetting,
  t as applyPosixNpmScriptShellEnv,
} from "./npm-install-env-B8v3zYUa.js";
import { n as readPackageName, r as readPackageVersion } from "./package-json-BqSklvpu.js";
import { t as applyPathPrepend } from "./path-prepend-C3srv1EO.js";
import { t as movePathWithCopyFallback } from "./replace-file-8cLr_oo0.js";
import { u as trimLogTail } from "./restart-sentinel-CXl2NwhJ.js";
import { a as parseSemver } from "./runtime-guard-vJnP437q.js";
import { t as BUNDLED_RUNTIME_SIDECAR_PATHS } from "./runtime-sidecar-paths-BZ15zlXV.js";
import { t as resolveStableNodePath } from "./stable-node-path-DBLC3fs0.js";
import {
...
import { n as compareSemverStrings, s as detectPackageManager } from "./update-check-hxRluXkb.js";
import { u as pathExists$1 } from "./utils-BGRcpLKt.js";
...
const LOCAL_BUILD_METADATA_DIST_PATHS = Object.freeze([
  `dist/.buildstamp`,
...
async function collectExternalizedBundledExtensionIds(packageRoot) {
  return collectExcludedPackagedExtensionDirs(
    await readJsonIfExists(path.join(packageRoot, "package.json")),
  );
}
...
async function collectSourceCheckoutInstallErrors(packageRoot) {
  ...
  return ((await pathExists$1(path.join(realPackageRoot, ".git"))) ||
    (await pathExists$1(path.join(realPackageRoot, "pnpm-workspace.yaml")))) &&
    (await pathExists$1(path.join(realPackageRoot, "src"))) &&
    (await pathExists$1(path.join(realPackageRoot, "extensions")))
...
async function collectCriticalInstalledPackageDistPaths(packageRoot) {
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        (await pathExists$1(path.join(packageRoot, pluginRoot, "package.json"))) ||
        (await pathExists$1(path.join(packageRoot, pluginRoot, "openclaw.plugin.json")))
      )
  ...
  for (const relativePath of params.expectedFiles)
    if (
      !(actualSet !== null
        ...
        : await pathExists$1(path.join(params.packageRoot, relativePath)))
    )
...
async function isPnpmGlobalPackageRoot(pkgRoot) {
  ...
  return (
    (await pathExists$1(path.join(layoutDir, "pnpm-lock.yaml"))) ||
    (await pathExists$1(path.join(layoutDir, "package.json")))
  );
}
...
async function detectGlobalInstallManagerByPresence(runCommand, timeoutMs) {
  for (const manager of ["npm", "pnpm"]) {
    ...
    if (!root) continue;
    for (const name of ALL_PACKAGE_NAMES)
      if (await pathExists$1(path.join(root, name))) return manager;
  }
  const bunRoot = resolveBunGlobalRoot();
  for (const name of ALL_PACKAGE_NAMES)
    if (await pathExists$1(path.join(bunRoot, name))) return "bun";
  return null;
}
function globalInstallArgs(managerOrCommand, spec, pkgRoot, installPrefix) {
  ...
  if (resolved.manager === "bun") return [resolved.command, "add", "-g", spec];
  return [
    resolved.command,
    "i",
    "-g",
...
function resolveStagedNpmTargetLayout(installTarget) {
  ...
  if (installTarget.manager === "npm" || isUnambiguousNpmPrefixGlobalRoot(installTarget.globalRoot))
    return targetLayout;
  return null;
}
...
async function isManagerAvailable(runCommand, manager, timeoutMs, env) {
  try {
    return (
      (
        await runCommand(managerVersionArgs(manager), {
          timeoutMs,
          env,
        })
      ).code === 0
...
async function bootstrapPnpmViaNpm(params) {
  ...
  try {
    if (
      (
        await params.runCommand(["npm", "install", "--prefix", tempRoot, "pnpm@10"], {
          timeoutMs: params.timeoutMs,
          env: params.baseEnv,
        })
      ).code !== 0
...
async function resolveUpdateBuildManager(
  runCommand,
  root,
  timeoutMs,
  baseEnv,
...
function managerInstallIgnoreScriptsArgs(manager) {
  ...
  if (manager === "bun") return ["bun", "install", "--ignore-scripts"];
...
//#endregion
export {
  cleanupGlobalRenameDirs as a,
  detectGlobalInstallManagerForRoot as c,
  resolveGlobalInstallTarget as d,
  resolvePnpmGlobalDirFromGlobalRoot as f,
  canResolveRegistryVersionForPackageTarget as i,
  globalInstallArgs as l,
  runGatewayUpdate as n,
  createGlobalInstallEnv as o,
  normalizePackageTagInput as p,
  runGlobalPackageUpdateSteps as r,
  detectGlobalInstallManagerByPresence as s,
  resolveUpdateInstallSurface as t,
  resolveGlobalInstallSpec as u,
};
