...
import { c as normalizeAgentId, r as buildAgentMainSessionKey } from "./session-key-B4qUwRzq.js";
...
function parseCrestodianOperation(input) {
  ...
  if (["help", "?", "overview", "system"].includes(lower)) return { kind: "overview" };
...
async function runGatewayLifecycle(operation) {
  const lifecycle = await import("./lifecycle-igwaHqS9.js");
  if (operation === "start") {
...
async function loadOverviewForOperation(deps) {
  ...
  const { loadCrestodianOverview } = await import("./overview-BKCiW_L3.js");
...
async function formatOverviewForOperation(overview, deps) {
  ...
  const { formatCrestodianOverview } = await import("./overview-BKCiW_L3.js");
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
  ...
  if (operation.kind === "plugin-list") {
    ...
    await (
      ...
      (async (pluginRuntime) => {
        const { runPluginsListCommand } = await import("./plugins-list-command-CH6lev0y.js");
  ...
  if (operation.kind === "plugin-search") {
    ...
    await (
      ...
      (async (query, pluginRuntime) => {
        const { runPluginsSearchCommand } = await import("./plugins-search-command-hGdnZE7L.js");
  ...
  if (operation.kind === "setup") {
    ...
    const applyDefaultModelPrimaryUpdate = setupModel.model
      ? (await import("./shared-Dqw7BZoQ.js")).applyDefaultModelPrimaryUpdate
      : void 0;
  ...
  if (operation.kind === "config-set") {
    ...
    await (
      ...
      (async (setOpts) => {
        const { runConfigSet: importedRunConfigSet } = await import("./config-cli-DnDenb6t.js");
  ...
  if (operation.kind === "plugin-install") {
    ...
    await (
      ...
      (async (spec, pluginRuntime) => {
        const { runPluginInstallCommand } = await import("./plugins-install-command-ByrRatm0.js");
  ...
  if (operation.kind === "plugin-uninstall") {
    ...
    await (
      ...
      (async (pluginId, pluginRuntime) => {
        const { runPluginUninstallCommand } =
          await import("./plugins-uninstall-command-ClI9LFhk.js");
  ...
  if (operation.kind === "create-agent") {
    ...
    await (
      ...
      (await import("./agents.commands.add-CrdqpdKm.js")).agentsAddCommand
    )(
      {
  ...
  if (operation.kind === "doctor") {
    ...
    await (opts.deps?.runDoctor ?? (await import("./doctor-CU6nGteq.js")).doctorCommand)(runtime, {
      nonInteractive: true,
    });
  ...
  if (operation.kind === "status") {
    ...
    const { statusCommand } = await import("./status.command-_c7jRzYk.js");
  ...
  if (operation.kind === "open-tui") {
    ...
    const result = await (opts.deps?.runTui ?? (await import("./tui-CCBUdNqB.js")).runTui)({
      local: true,
      session,
      deliver: false,
      historyLimit: 200,
  ...
  if (operation.kind === "set-default-model") {
    ...
    const { mutateConfigFile, readConfigFileSnapshot } = await loadConfigFileMutationHelpers();
    const before = await readConfigFileSnapshot();
    const { applyDefaultModelPrimaryUpdate } = await import("./shared-Dqw7BZoQ.js");
    const result = await mutateConfigFile({
      base: "source",
      mutate: (cfg) => {
...
//#endregion
export {
  parseCrestodianOperation as a,
  isPersistentCrestodianOperation as i,
  executeCrestodianOperation as n,
  formatCrestodianPersistentPlan as r,
  describeCrestodianPersistentOperation as t,
};
