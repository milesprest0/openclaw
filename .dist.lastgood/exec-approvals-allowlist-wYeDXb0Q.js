...
function splitShellPipeline(command) {
  ...
  if (inHeredocBody && pendingHeredocs.length > 0) {
    ...
    if (!current.quoted && unquotedHeredocLogicalChunks.length > 0) {
      const continued = stripUnquotedHeredocLineContinuation(line);
      if (
        hasUnquotedHeredocExpansionToken([...unquotedHeredocLogicalChunks, continued.line].join(""))
      )
        return {
          ok: false,
          reason: "shell expansion in unquoted heredoc",
...
function rebuildShellCommandFromSource(params) {
  if (isWindowsPlatform(params.platform ?? null))
...
function buildSafeShellCommand(params) {
  ...
  return finalizeRebuiltShellCommand(
    rebuildShellCommandFromSource({
...
function isSafeBinUsage(params) {
  ...
  if (
    !(params.isTrustedSafeBinPathFn ?? isTrustedSafeBinPath)({
...
function isSkillPreludeReadSegment(segment, cwd) {
  if (
    normalizeLowercaseStringOrEmpty(
      resolveExecutionTargetResolution(segment.resolution)?.executableName,
    ) !== "cat"
  )
    return false;
...
function resolveAllowlistedSkillWrapperId(segment) {
  const executableName = normalizeExecutableToken(
    resolveExecutionTargetResolution(segment.resolution)?.executableName ?? segment.argv[0] ?? "",
  );
...
const SHELL_WRAPPER_DISQUALIFYING_SCRIPT_OPTIONS = ["--rcfile", "--init-file", "--startup-file"];
...
function resolveShellWrapperPositionalArgvCandidatePath(params) {
  ...
  if (!["ash", "bash", "dash", "fish", "ksh", "sh", "zsh"].includes(wrapper)) return;
  ...
  return resolveExecutionTargetCandidatePath(
    resolveCommandResolutionFromArgv([carriedExecutable], params.cwd, params.env),
    params.cwd,
  );
}
...
function buildScriptArgPatternFromArgv(argv, scriptPath, cwd, platform) {
  ...
  const normalized = (scriptIdx !== -1 ? argv.slice(scriptIdx + 1) : []).map((a) =>
    a.replace(/\//g, "\\"),
  );
...
function addAllowAlwaysPattern(out, pattern, argPattern) {
  if (
    !out.some((p) => p.pattern === pattern && (p.argPattern ?? void 0) === (argPattern ?? void 0))
  )
    out.push({
      pattern,
      argPattern,
...
function evaluateShellAllowlist(params) {
  ...
  for (const [index, { analysis, evaluation }] of finalizedEvaluations.entries()) {
    const effectiveSegmentSatisfiedBy = allowSkillPreludeAtIndex.has(index)
      ? analysis.segments.map(() => "skillPrelude")
...
//#endregion
export {
  windowsEscapeArg as _,
  resolveAllowAlwaysPatternEntries as a,
  analyzeArgvCommand as c,
  buildSafeBinsShellCommand as d,
  buildSafeShellCommand as f,
  splitCommandChainWithOperators as g,
  splitCommandChain as h,
  normalizeSafeBins as i,
  analyzeShellCommand as l,
  resolvePlannedSegmentArgv as m,
  evaluateShellAllowlist as n,
  resolveAllowAlwaysPatterns as o,
  isWindowsPlatform as p,
  isSafeBinUsage as r,
  resolveSafeBins as s,
  evaluateExecAllowlist as t,
  buildEnforcedShellCommand as u,
};
