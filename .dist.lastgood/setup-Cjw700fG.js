...
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-BeU7cwXO.js";
...
} from "./setup.migration-import-BhWxn4JO.js";
...
import { d as normalizeSecretInputString } from "./types.secrets-CaNC1eIn.js";
...
function loadAuthChoiceModule() {
  authChoiceModulePromise ??= import("./auth-choice-Ctxg08Js.js");
  return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
  configLoggingModulePromise ??= import("./logging-CDeNTkKE.js");
  return configLoggingModulePromise;
}
function loadModelPickerModule() {
  modelPickerModulePromise ??= import("./model-picker-DLXTcVxo.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
  ...
  const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all(
    [import("./provider-auth-choices-CRUBIcxd.js"), import("./setup-registry-hG-zz9lb.js")],
  );
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
  const onboardHelpers = await import("./onboard-helpers-BTj6NKFG.js");
  ...
  if (mode === "remote") {
    const { promptRemoteGatewayConfig } = await import("./onboard-remote-B6d6HfYY.js");
    const { applySkipBootstrapConfig } = await import("./onboard-config-Cr3laJNS.js");
  ...
  const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } =
    await import("./onboard-config-Cr3laJNS.js");
  ...
  if (authChoiceFromPrompt) {
    ...
    ({ promptAuthChoiceGrouped } = await import("./auth-choice-prompt-B6UZkGjw.js"));
  ...
  while (true) {
    ...
    if (authChoice === "custom-api-key") {
      const { promptCustomApiConfig } = await import("./onboard-custom-BsbDQZCE.js");
      nextConfig = (
  ...
  const { configureGatewayForSetup } = await import("./setup.gateway-config-Bdg2iVd1.js");
  ...
  if (opts.skipChannels ?? opts.skipProviders)
    ...
    const { listChannelPlugins } = await import("./plugins-ChHcCIiB.js");
    const { setupChannels } = await import("./onboard-channels-S9iBipvG.js");
  ...
  if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
    ...
    const { setupSearch } = await import("./onboard-search-BK25Pl0q.js");
  ...
  if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
    ...
    const { setupSkills } = await import("./onboard-skills-MbeUSTYr.js");
    nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
  }
  if (flow !== "quickstart") {
    const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-MiINiXeI.js");
    ...
    const { setupPluginConfig } = await import("./setup.plugin-config-CbZE1e2X.js");
  ...
  const { setupInternalHooks } = await import("./onboard-hooks-CV9b6E-D.js");
  ...
  nextConfig = await writeWizardConfigFile(nextConfig);
  const { finalizeSetupWizard } = await import("./setup.finalize-BQJrY_kQ.js");
