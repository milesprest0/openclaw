...
import {
  n as loadAgentIdentityFromWorkspace,
  t as identityHasValues,
} from "./identity-file-Csvx_B8P.js";
...
function resolveAgentModel(cfg, agentId) {
  const entryPrimary = resolvePrimaryStringValue(
    listAgentEntries(cfg).find((agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId))
      ?.model,
  );
...
//#endregion
export {
  pruneAgentConfig as a,
  loadAgentIdentity as i,
  buildAgentSummaries as n,
  findAgentEntryIndex as r,
  applyAgentConfig as t,
};
