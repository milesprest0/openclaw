...
import {
  a as writeSupportBundleDirectory,
  i as textSupportBundleFile,
  n as jsonlSupportBundleFile,
  r as supportBundleContents,
  t as jsonSupportBundleFile,
} from "./diagnostic-support-bundle-CS76YpdB.js";
import "./path-guards-DyZnivcw.js";
import { n as redactSupportString } from "./diagnostic-support-redaction-DSwEKsc9.js";
...
import {
  ...
  r as TRAJECTORY_RUNTIME_FILE_MAX_BYTES,
  s as safeTrajectorySessionFileName,
...
function isRuntimeTrajectoryEventForSession(value, sessionId) {
  ...
  return (
    ...
    (!("data" in value) || value.data === void 0 || isRecord(value.data))
  );
}
...
async function readRuntimePointerFile(sessionFile, sessionId) {
  ...
  try {
    ...
    if (
      ...
      path.basename(runtimeFile) !== safeRuntimeFileName
    )
      return;
    return runtimeFile;
  } catch {
...
async function resolveTrajectoryRuntimeFile(params) {
  ...
  for (const candidate of candidates)
    if (await isRegularNonSymlinkFile(candidate)) return candidate;
}
...
function trajectoryJsonlFile(pathName, events) {
  return jsonlSupportBundleFile(
    pathName,
    events.map((event) => safeJsonStringify(event)).filter((line) => Boolean(line)),
  );
}
...
function resolveRuntimeContext(runtimeEvents) {
  const runtimeData = runtimeEvents
    .slice()
    .toReversed()
    .find((event) => event.type === "context.compiled")?.data;
...
function resolveLatestRuntimeEventData(runtimeEvents, type) {
  return runtimeEvents
    .slice()
    .toReversed()
    .find((candidate) => candidate.type === type)?.data;
}
...
function markInvokedSkills(params) {
  ...
  const entries = skillsRecord.entries.map((entry) => {
    ...
    const skillDirName =
      rawPath?.replaceAll("\\", "/").split("/").slice(-2, -1)[0]?.toLowerCase() ?? void 0;
...
function buildPromptsCapture(params) {
  ...
  const systemPrompt =
    (typeof latestCompiled?.systemPrompt === "string" ? latestCompiled.systemPrompt : void 0) ??
...
function resolveDefaultTrajectoryExportDir(params) {
  const timestamp = (params.now ?? /* @__PURE__ */ new Date())
    .toISOString()
    .replace(/[:.]/g, "-")
    .slice(0, 19);
  ...
  if (bundleRuntimeContext.systemPrompt)
    files.push(
      textSupportBundleFile(
        "system-prompt.txt",
        redactLocalPathValues(bundleRuntimeContext.systemPrompt, redaction),
      ),
    );
...
async function resolveTrajectoryCommandOutputDir(params) {
  ...
  if (!isPathInside(realBase, await fs.realpath(existingParent)))
