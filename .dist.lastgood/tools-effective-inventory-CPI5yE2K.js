...
import { r as normalizeStaticProviderModelId } from "./model-ref-shared-C2I7TGOl.js";
import { t as createOpenClawCodingTools } from "./pi-tools-Xm5nmxui.js";
import { d as getChannelAgentToolMeta } from "./pi-tools.before-tool-call-KxT3QmrR.js";
import { n as resolveEffectiveToolPolicy } from "./pi-tools.policy-BdujgZXP.js";
import {
...
import { r as resolveToolDisplay } from "./tool-display-qyfb5lzi.js";
import { i as normalizeToolName } from "./tool-policy-shared-DsbREhoR.js";
import { i as getPluginToolMeta, t as buildPluginToolMetadataKey } from "./tools-DcY_52BH.js";
...
function buildToolInventoryNotices(params) {
  if (
    params.entries.some((entry) => normalizeToolName(entry.id) === "browser") ||
    !hasExplicitBrowserIntent(params.cfg)
  )
    return;
  if (
    [
...
function resolveEffectiveModelCompat(params) {
  ...
  return extractModelCompat(
    models.find((model) => {
      const key = normalizeLowercaseStringOrEmpty(
        normalizeStaticProviderModelId(provider, model.id),
      );
