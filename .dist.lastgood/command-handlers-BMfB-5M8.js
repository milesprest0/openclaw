...
import {
  i as isCodexFastServiceTier,
  s as resolveCodexAppServerRuntimeOptions,
} from "./config-CNEUyjZv.js";
import {
  a as parseCodexFastModeArg,
  c as setCodexConversationFastMode,
  d as steerCodexConversationTurn,
  f as stopCodexConversationTurn,
  i as formatPermissionsMode,
  l as setCodexConversationModel,
  m as resolveCodexDefaultWorkspaceDir,
  o as parseCodexPermissionsModeArg,
  p as readCodexConversationBindingData,
  r as startCodexConversationThread,
  s as readCodexConversationActiveTurn,
  u as setCodexConversationPermissions,
} from "./conversation-binding-DsMhaYSM.js";
import {
  n as listCodexAppServerModels,
  t as listAllCodexAppServerModels,
} from "./models-Bk93b7WA.js";
import { t as isJsonObject } from "./protocol-DayKrDMy.js";
import {
  a as readCodexComputerUseStatus,
  i as installCodexComputerUse,
  n as rememberCodexRateLimits,
} from "./rate-limit-cache-BbpHSs2n.js";
import {
  a as formatCodexStatus,
  c as formatModels,
  f as CODEX_CONTROL_METHODS,
  i as formatCodexDisplayText,
  l as formatThreads,
  n as buildHelp,
  o as formatComputerUseStatus,
  p as describeControlFailure,
  r as formatAccount,
  s as formatList,
  t as requestCodexAppServerJson,
  u as readString,
} from "./request-BC8expe9.js";
import {
  i as readCodexAppServerBinding,
  o as writeCodexAppServerBinding,
  t as clearCodexAppServerBinding,
} from "./session-binding-o-m5DVaT.js";
...
async function safeCodexControlRequest(pluginConfig, method, requestParams, options = {}) {
  return await safeValue(
    async () => await codexControlRequest(pluginConfig, method, requestParams, options),
  );
}
...
async function handleCodexSubcommand(ctx, options) {
  ...
  if (normalized === "account") {
    ...
    const [account, limits] = await Promise.all([
      ...
      deps.safeCodexControlRequest(options.pluginConfig, CODEX_CONTROL_METHODS.rateLimits, void 0),
    ]);
...
async function bindConversation(deps, ctx, pluginConfig, args) {
  ...
  const threadId =
    (await deps.readCodexAppServerBinding(ctx.sessionFile))?.threadId ??
    parsed.threadId ??
    "new thread";
  ...
  if (request.status === "bound")
    return {
      text: `Bound this conversation to Codex thread ${formatCodexDisplayText(threadId)} in ${formatCodexDisplayText(workspaceDir)}.`,
    };
...
async function buildThreads(deps, pluginConfig, filter) {
  return formatThreads(
    await deps.codexControlRequest(pluginConfig, CODEX_CONTROL_METHODS.listThreads, {
      limit: 10,
...
async function resolveControlSessionFile(ctx) {
  return (
    readCodexConversationBindingData(await ctx.getCurrentConversationBinding())?.sessionFile ??
    ctx.sessionFile
  );
}
async function handleCodexDiagnosticsFeedback(deps, ctx, pluginConfig, args, commandPrefix) {
  ...
  if (parsed.action === "confirm")
    return { text: await confirmCodexDiagnosticsFeedback(deps, ctx, pluginConfig, parsed.token) };
  ...
  if (ctx.diagnosticsPreviewOnly === true)
    return { text: await previewCodexDiagnosticsFeedbackApproval(deps, ctx, parsed.note) };
...
function readCodexDiagnosticsTargetsCooldownMessage(targets, ctx, now, options = {}) {
  for (const target of targets) {
    ...
    if (cooldownMs > 0) {
      if (options.includeThreadId === false)
        return `Codex diagnostics were already sent for one of these Codex threads recently. Try again in ${Math.ceil(cooldownMs / 1e3)}s.`;
  ...
  if (scopeCooldownMs > 0)
    return `Codex diagnostics were already sent for this account or channel recently. Try again in ${Math.ceil(scopeCooldownMs / 1e3)}s.`;
}
...
function pruneCodexDiagnosticsCooldownMap(map, now) {
  for (const [key, lastSentAt] of map)
    if (now - lastSentAt >= CODEX_DIAGNOSTICS_COOLDOWN_MS) map.delete(key);
}
function formatCodexErrorForDisplay(error) {
  return (
    escapeCodexChatText(
      formatCodexTextForDisplay(error).slice(0, CODEX_DIAGNOSTICS_ERROR_MAX_CHARS),
    ) || "unknown error"
  );
}
...
function isUnsafeDisplayCodePoint(codePoint) {
  return (
    ...
    (codePoint >= 917504 && codePoint <= 917631)
  );
}
