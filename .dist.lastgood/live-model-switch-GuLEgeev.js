...
async function ensureSelectedAgentHarnessPlugin(params) {
  if (
    ...
    return;
  const { ensurePluginRegistryLoaded } = await import("./runtime-registry-loader-D7StEUXN.js");
...
function hasDifferentLiveSessionModelSelection(current, next) {
  ...
  return (
    ...
    (normalizeOptionalString(current.authProfileId) ? current.authProfileIdSource : void 0) !==
...
function shouldSwitchToLiveModel(params) {
  ...
  if (
    !loadSessionStore(resolveStorePath(cfg.session?.store, { agentId: params.agentId?.trim() }), {
      skipCache: true,
    })[sessionKey]?.liveModelSwitchPending
  )
    return;
...
//#endregion
export {
  shouldSwitchToLiveModel as n,
  ensureSelectedAgentHarnessPlugin as r,
  clearLiveModelSwitchPending as t,
};
