...
import { c as writeSiblingTempFile, u as withTimeout } from "./fs-safe-CgBWiL92.js";
import {
  i as isPathInside,
  l as resolveSafeBaseDir,
  p as FsSafeError,
...
async function resolvePackedRootDir(extractDir, options) {
  ...
  const dirs = (await fs$1.readdir(extractDir, { withFileTypes: true }))
...
async function chmodInsideDestinationBestEffort(params) {
  ...
  const handle = await fs$1
    .open(params.destinationPath, fs.constants.O_RDONLY | noFollowFlag)
    .catch(() => null);
  if (!handle) {
    if ((await fs$1.lstat(params.destinationPath).catch(() => null))?.isSymbolicLink())
      throw symlinkTraversalError(params.originalPath);
    return;
  }
  try {
...
async function withStagedArchiveDestination(params) {
  ...
  const stagingDir = await fs$1.mkdtemp(
...
async function extractZip(params) {
  ...
  if ((await fs$1.stat(params.archivePath)).size > limits.maxArchiveBytes)
    throw new ArchiveLimitError(ARCHIVE_LIMIT_ERROR_CODE.ARCHIVE_SIZE_EXCEEDS_LIMIT);
...
//#endregion
export {
  prepareArchiveDestinationDir as a,
  resolvePackedRootDir as c,
  DEFAULT_MAX_ARCHIVE_BYTES_ZIP as d,
  DEFAULT_MAX_ENTRIES as f,
  isWindowsDrivePath as h,
  mergeExtractedTreeIntoDestination as i,
  ARCHIVE_LIMIT_ERROR_CODE as l,
  DEFAULT_MAX_EXTRACTED_BYTES as m,
  loadZipArchiveWithPreflight as n,
  withStagedArchiveDestination as o,
  DEFAULT_MAX_ENTRY_BYTES as p,
  createTarEntryPreflightChecker as r,
  resolveArchiveKind as s,
  extractArchive as t,
  ArchiveLimitError as u,
};
