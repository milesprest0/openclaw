import { r as getRegisteredDetachedTaskLifecycleRuntime } from "./detached-task-runtime-state-CRuVyWwI.js";
...
import {
  I as listTaskFlowsForOwnerKey,
  L as requestFlowCancel,
  N as getTaskFlowById,
  O as createTaskFlowForTask,
  T as setTaskRunDeliveryStatusByRunId,
  V as updateFlowRecordByIdExpectedRevision,
  a as finalizeTaskRunByRunId$1,
  j as findLatestTaskFlowForOwnerKey,
  k as deleteTaskFlowRecordById,
  l as isParentFlowLinkError,
  n as createTaskRecord,
  p as listTasksForFlowId,
  s as getTaskById,
  t as cancelTaskById,
  u as linkTaskToFlowById,
  v as markTaskRunningByRunId,
  x as recordTaskProgressByRunId,
...
async function cancelFlowById(params) {
  ...
  const activeTasks = listTasksForFlowId(flow.flowId).filter((task) =>
  ...
  if (refreshedTasks.filter((task) => isActiveTaskStatus(task.status)).length > 0)
    return {
      found: true,
      cancelled: false,
  ...
  if (isTerminalFlowStatus(refreshedFlow.status))
    return {
      ...
      reason:
        refreshedFlow.status === "cancelled" ? void 0 : `Flow is already ${refreshedFlow.status}.`,
      flow: refreshedFlow,
      tasks: refreshedTasks,
    };
...
//#endregion
export {
  resolveTaskFlowForLookupTokenForOwner as _,
  createQueuedTaskRun as a,
  finalizeTaskRunByRunId as c,
  runTaskInFlowForOwner as d,
  setDetachedTaskDeliveryStatusByRunId as f,
  listTaskFlowsForOwner as g,
  getTaskFlowByIdForOwner as h,
  completeTaskRunByRunId as i,
  getFlowTaskSummary as l,
  findLatestTaskFlowForOwner as m,
  cancelFlowById as n,
  createRunningTaskRun as o,
  startTaskRunByRunId as p,
  cancelFlowByIdForOwner as r,
  failTaskRunByRunId as s,
  cancelDetachedTaskRunById as t,
  recordTaskRunProgressByRunId as u,
};
