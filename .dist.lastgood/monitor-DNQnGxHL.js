import {
  i as resolveWhatsAppInboundPolicy,
  n as checkInboundAccessControl,
  r as resolveWhatsAppCommandAuthorized,
} from "./access-control-Di00ltjl.js";
...
import {
  a as resolveWhatsAppAccount,
  s as resolveWhatsAppMediaMaxBytes,
} from "./accounts-MM0YxrKm.js";
import {
  a as shouldAckReactionForWhatsApp,
...
import { t as resolveInboundSessionEnvelopeContext } from "./channel-inbound-3vUekbSs.js";
import { a as deliverInboundReplyWithMessageSendContext } from "./channel-message-Do1b_D-M.js";
import {
...
import {
  c as resolveHeartbeatSeconds,
  l as resolveReconnectPolicy,
  n as WhatsAppConnectionController,
  o as DEFAULT_RECONNECT_POLICY,
  s as newConnectionId,
} from "./connection-controller-CAWmSVnG.js";
...
import { r as hasVisibleChannelTurnDispatch } from "./dispatch-result-Cyoa53q0.js";
import { l as resolvePinnedMainDmOwnerFromAllowlist } from "./dm-policy-shared-COpbJNwP.js";
import { r as formatInboundEnvelope } from "./envelope-DWEYsyXr.js";
import { r as formatDurationPrecise } from "./format-duration-DV-29w1K.js";
import {
  n as createTransportActivityStatusPatch,
  t as createConnectedChannelStatusPatch,
} from "./gateway-runtime-BGDxySPN.js";
import { a as shouldLogVerbose, r as logVerbose } from "./globals-BdfwDi2E.js";
import {
...
import { a as resolveIdentityNamePrefix$1 } from "./identity-BfuOtf2o.js";
import {
  a as getSelfIdentity,
  c as resolveComparableIdentity,
  i as getReplyContext,
  n as getMentionIdentities,
  o as getSenderIdentity,
  r as getPrimaryIdentityId,
  s as identitiesOverlap,
  t as getComparableIdentityValues,
} from "./identity-CG3p7vp1.js";
...
import "./account-core-BYoyzRSP.js";
...
import { t as logAckFailure } from "./logging-ChWKeerl.js";
...
import { a as normalizeMentionText, n as buildMentionRegexes } from "./mentions-CX9MvOD0.js";
import {
...
import { t as isWhatsAppGroupJid } from "./normalize-target-B5_fp7Jp.js";
import {
  i as prepareWhatsAppOutboundMedia,
  o as sendWhatsAppOutboundWithRetry,
  r as normalizeWhatsAppPayloadTextPreservingIndentation,
  t as normalizeWhatsAppOutboundPayload,
} from "./outbound-media-contract-NP9fM72I.js";
import { u as resolveStorePath } from "./paths-CfeECf6Z.js";
import { t as createClaimableDedupe } from "./persistent-dedupe-De2sIEJ2.js";
import { t as dispatchReplyWithBufferedBlockDispatcher } from "./provider-dispatcher-BPV_KXNV.js";
import {
  n as cacheInboundMessageMeta,
  r as lookupInboundMessageMeta,
  t as buildQuotedMessageOptions,
} from "./quoted-message-BvLCUGwM.js";
import { t as resolveWhatsAppReactionLevel } from "./reaction-level-BiwGRet3.js";
...
import "./reply-reference-SeorAP6Z.js";
...
import "./cli-runtime-CnA0DtN4.js";
import "./hook-runtime-H-x6MkQd.js";
import "./channel-mention-gating-DvGYRW-A.js";
...
import { s as getRuntimeConfigSourceSnapshot } from "./runtime-snapshot-B0aZX-OG.js";
import {
  a as mayContainWhatsAppOutboundMention,
  c as extractContactContext,
  d as extractMediaPlaceholder,
  f as extractMentionedJids,
  i as addWhatsAppOutboundMentionsToContent,
  l as extractContextInfo,
  m as hasInboundUserContent,
  n as listWhatsAppSendResultMessageIds,
  o as resolveWhatsAppOutboundMentions,
  p as extractText,
  r as normalizeWhatsAppSendResult,
  s as describeReplyContext,
  t as createWebSendApi,
  u as extractLocationData,
} from "./send-api-C0IWeF4B.js";
import { r as sendReactionWhatsApp } from "./send-CtyIKV69.js";
import "./session-store-runtime-CGNlDXYc.js";
import { t as recordInboundSession } from "./session-CUCa4TmV.js";
import {
  i as resolveWhatsAppSocketTiming,
...
import {
  ...
  c as normalizeAgentId$1,
  d as resolveThreadSessionKeys,
  n as DEFAULT_MAIN_KEY,
  r as buildAgentMainSessionKey,
} from "./session-key-B4qUwRzq.js";
import {
  d as DisconnectReason,
  i as downloadMediaMessage,
  s as normalizeMessageContent,
  u as isJidGroup,
} from "./session.runtime-CiA-rRy6.js";
import { o as updateLastRoute, s as updateSessionStore } from "./store-BpWdoYPF.js";
...
import {
  a as resolveJidToE164,
  i as markdownToWhatsApp,
  n as isSelfChatMode,
  r as jidToE164,
} from "./text-runtime-BMQLJDUy.js";
import { d as registerUnhandledRejectionHandler } from "./unhandled-rejections-ChRE0RCv.js";
...
async function attachWebInboxToSocket(options) {
    ...
    onFlush: async (entries) => {
      ...
      try {
        ...
        for (const entry of entries)
          for (const jid of entry.mentions ?? entry.mentionedJids ?? []) mentioned.add(jid);
  ...
  const rememberOutboundMessage = (remoteJid, result) => {
    const messageId =
      typeof result === "object" && result && "key" in result ? (result.key?.id ?? "") : "";
    if (!messageId) return;
  ...
  const enrichInboundMessage = async (msg) => {
    ...
    try {
      ...
      if (!mediaPath && replyContext)
        await saveInboundMedia(await downloadQuotedInboundMedia(msg, sock));
    } catch (err) {
  ...
  const handleMessagesUpsert = async (upsert) => {
    ...
    for (const msg of upsert.messages ?? []) {
      ...
      if (upsert.type === "append") {
        ...
        if (
          (Number.isFinite(msgTsNum) ? msgTsNum * 1e3 : 0) <
...
function isBotMentionedFromTargets(msg, mentionCfg, targets) {
  ...
  if (hasMentions && !isSelfChat) {
    for (const mention of targets.normalizedMentions)
      if (identitiesOverlap(targets.self, mention)) return true;
    return false;
...
function resolveOwnerList(mentionCfg, selfE164) {
  ...
  return (Array.isArray(allowFrom) && allowFrom.length > 0 ? allowFrom : selfE164 ? [selfE164] : [])
...
async function resolveGroupActivationFor(params) {
  ...
  const activation =
    ...
    isActivationOnlyEntry(scopedEntry)
      ? void 0
      : scopedEntry?.groupActivation) ?? legacyEntry?.groupActivation;
...
async function maybeBroadcastMessage(params) {
  ...
  const processForAgent = async (agentId) => {
    ...
    try {
      ...
      if (params.preflightAudioTranscript !== void 0)
  ...
  const mentionDecision = resolveInboundMentionDecision({
    facts: {
      ...
      implicitMentionKinds: implicitMentionKindWhen(
        "quoted_bot",
        !(params.selfChatMode === true && identitiesOverlap(self, sender)) &&
          identitiesOverlap(self, replyContext?.sender),
      ),
    },
    policy: {
      isGroup: true,
...
function resolveWhatsAppGroupSystemPrompt(params) {
  ...
  if (specific != null && specific.systemPrompt != null)
    return specific.systemPrompt.trim() || void 0;
...
async function deliverWebReply(params) {
  ...
  const textChunks = chunkMarkdownTextWithMode(
    markdownToWhatsApp(convertMarkdownTables(normalizedReply.text ?? "", tableMode)),
    textLimit,
    chunkMode,
  );
  ...
  await sendMediaWithLeadingCaption({
    ...
    onError: async ({ error, mediaUrl, caption, isFirst }) => {
      ...
      if (!isFirst) return;
      const fallbackText = [remainingText.shift() ?? caption ?? "", "⚠️ Media failed."]
        .filter(Boolean)
        .join("\n");
...
function resolveWhatsAppResponsePrefix(params) {
  ...
  return (
    ...
    (configuredResponsePrefix === void 0 && params.isSelfChat
      ? resolveIdentityNamePrefix$1(params.cfg, params.agentId)
      : void 0)
  );
}
...
async function dispatchWhatsAppBufferedReply(params) {
  ...
  const deliverNormalizedPayload = async (normalizedDeliveryPayload, info) => {
    ...
    if (shouldLogVerbose())
      logVerbose(
        `Reply body: ${normalizedDeliveryPayload.text != null ? reply.text : "<media>"}${reply.hasMedia ? " (media)" : ""} -> ${fromDisplay}`,
      );
  };
  ...
  const { queuedFinal, counts } = await dispatchReplyWithBufferedBlockDispatcher({
    ...
    dispatcherOptions: {
      ...
      deliver: async (payload, info) => {
        ...
        if (
          shouldDeferWhatsAppMediaOnlyPayload({
            info,
            mediaUrls,
            reply,
          })
...
function shouldEmitWhatsAppMessageReceivedHooks(params) {
  ...
  return (
    readWhatsAppMessageReceivedHookOptIn(
...
async function processMessage(params) {
  ...
  if (params.preflightAudioTranscript === void 0 && hasAudioBody && params.msg.mediaPath)
    try {
      const { transcribeFirstAudio } = await import("./audio-preflight.runtime-D2b8alYd.js");
...
function createWebOnMessageHandler(params) {
  const processForRoute = async (msg, route, groupHistoryKey, opts) => {
    ...
    if (opts?.suppressGroupHistoryClear !== void 0)
      processParams.suppressGroupHistoryClear = opts.suppressGroupHistoryClear;
    if (opts?.preflightAudioTranscript !== void 0)
...
function isRetryableAuthUnstableError(error) {
  return (
    ...
    (typeof error === "object" &&
      error !== null &&
      "code" in error &&
      error.code === "whatsapp-auth-unstable")
  );
}
...
async function monitorWebChannel(
  verbose,
  listenerFactory = attachWebInboxToSocket,
  keepAlive = true,
  replyResolver,
        ...
        connection = await controller.openConnection({
          ...
          onHeartbeat: (snapshot) => {
            ...
            const minutesSinceLastMessage = snapshot.lastInboundAt
              ? Math.floor((Date.now() - snapshot.lastInboundAt) / 6e4)
              : null;
            const logData = {
