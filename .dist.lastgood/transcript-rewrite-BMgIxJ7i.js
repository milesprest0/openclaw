...
async function readTranscriptFileState(sessionFile) {
  ...
  const migrated =
    sessionHeaderVersion(fileEntries.find((entry) => entry.type === "session") ?? null) <
    CURRENT_SESSION_VERSION;
...
async function rewriteTranscriptEntriesInSessionFile(params) {
  ...
  try {
    ...
    if (result.changed) {
      ...
      log.info(
        `[transcript-rewrite] rewrote ${result.rewrittenEntries} entr${result.rewrittenEntries === 1 ? "y" : "ies"} bytesFreed=${result.bytesFreed} sessionKey=${params.sessionKey ?? params.sessionId ?? "unknown"}`,
      );
    }
    return result;
  } catch (err) {
...
//#endregion
export {
  setRawSessionAppendMessage as a,
  readTranscriptFileState as c,
  getRawSessionAppendMessage as i,
  writeTranscriptFileAtomic as l,
  rewriteTranscriptEntriesInSessionManager as n,
  TranscriptFileState as o,
  rewriteTranscriptEntriesInState as r,
  persistTranscriptStateMutation as s,
  rewriteTranscriptEntriesInSessionFile as t,
};
