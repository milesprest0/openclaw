...
function isCliProvider(provider, cfg) {
  ...
  if (resolveRuntimeCliBackends().some((backend) => normalizeProviderId(backend.id) === normalized))
    return true;
  if (
    resolvePluginSetupCliBackendRuntime({
      backend: normalized,
      config: cfg,
    })
  )
...
//#endregion
export {
  resolveAllowedModelRef as i,
  resolveRuntimeCliBackends as n,
