  ...
  (function (global, factory) {
    ...
    function ReadableStreamDefaultReaderRelease(reader) {
      ...
      ReadableStreamDefaultReaderErrorReadRequests(
        reader,
        /* @__PURE__ */ new TypeError("Reader was released"),
      );
    }
    ...
    function ReadableByteStreamControllerPullInto(controller, view, min, readIntoRequest) {
      ...
      if (controller._queueTotalSize > 0) {
        ...
        if (controller._closeRequested) {
          const e = /* @__PURE__ */ new TypeError(
    ...
    function SetUpReadableByteStreamController(
      ...
      uponPromise(
        promiseResolvedWith(startAlgorithm()),
        () => {
    ...
    function convertByobReadOptions(options, context) {
      ...
      return {
        min: convertUnsignedLongLongWithEnforceRange(
          (_a = options === null || options === void 0 ? void 0 : options.min) !== null &&
            _a !== void 0
            ? _a
            : 1,
      ...
      read(view, rawOptions = {}) {
        ...
        if (!ArrayBuffer.isView(view))
          return promiseRejectedWith(
            /* @__PURE__ */ new TypeError("view must be an array buffer view"),
          );
        if (view.byteLength === 0)
          return promiseRejectedWith(
            /* @__PURE__ */ new TypeError("view must have non-zero byteLength"),
          );
        if (view.buffer.byteLength === 0)
          return promiseRejectedWith(
            /* @__PURE__ */ new TypeError(`view's buffer must have non-zero byteLength`),
          );
        if (IsDetachedBuffer(view.buffer))
          return promiseRejectedWith(
        ...
        if (min === 0)
          return promiseRejectedWith(
            /* @__PURE__ */ new TypeError("options.min must be greater than 0"),
          );
        if (!isDataView(view)) {
          if (min > view.length)
            return promiseRejectedWith(
    ...
    class WritableStream {
      ...
      abort(reason = void 0) {
        ...
        if (IsWritableStreamLocked(this))
          return promiseRejectedWith(
            /* @__PURE__ */ new TypeError("Cannot abort a stream that already has a writer"),
          );
      ...
      close() {
        ...
        if (IsWritableStreamLocked(this))
          return promiseRejectedWith(
            /* @__PURE__ */ new TypeError("Cannot close a stream that already has a writer"),
          );
        if (WritableStreamCloseQueuedOrInFlight(this))
          return promiseRejectedWith(
            /* @__PURE__ */ new TypeError("Cannot close an already-closing stream"),
          );
    ...
    function WritableStreamFinishErroring(stream) {
      ...
      uponPromise(
        stream._writableStreamController[AbortSteps](abortRequest._reason),
        () => {
    ...
    function WritableStreamDefaultWriterRelease(writer) {
      const stream = writer._ownerWritableStream;
      const releasedError = /* @__PURE__ */ new TypeError(
    ...
    function WritableStreamDefaultWriterWrite(writer, chunk) {
      ...
      if (WritableStreamCloseQueuedOrInFlight(stream) || state === "closed")
        return promiseRejectedWith(
          /* @__PURE__ */ new TypeError("The stream is closing or closed and cannot be written to"),
        );
    ...
    function WritableStreamDefaultControllerProcessWrite(controller, chunk) {
      ...
      uponPromise(
        controller._writeAlgorithm(chunk),
        () => {
    ...
    function ReadableStreamPipeTo(source, dest, preventClose, preventAbort, preventCancel, signal) {
      ...
      return newPromise((resolve, reject) => {
        ...
        if (signal !== void 0) {
          abortAlgorithm = () => {
            const error =
              signal.reason !== void 0 ? signal.reason : new DOMException("Aborted", "AbortError");
            const actions = [];
            if (!preventAbort)
              actions.push(() => {
        ...
        if (WritableStreamCloseQueuedOrInFlight(dest) || dest._state === "closed") {
          const destClosed = /* @__PURE__ */ new TypeError(
    ...
    class ReadableStream {
      ...
      cancel(reason = void 0) {
        ...
        if (IsReadableStreamLocked(this))
          return promiseRejectedWith(
            /* @__PURE__ */ new TypeError("Cannot cancel a stream that already has a reader"),
          );
        return ReadableStreamCancel(this, reason);
      }
      getReader(rawOptions = void 0) {
        if (!IsReadableStream(this)) throw streamBrandCheckException$1("getReader");
        if (convertReaderOptions(rawOptions, "First parameter").mode === void 0)
    ...
    function TransformStreamDefaultControllerEnqueue(controller, chunk) {
      ...
      if (
        ReadableStreamDefaultControllerHasBackpressure(readableController) !== stream._backpressure
      )
        TransformStreamSetBackpressure(stream, true);
    }
    ...
    function TransformStreamDefaultControllerTerminate(controller) {
      ...
      TransformStreamErrorWritableAndUnblockWrite(
        stream,
        /* @__PURE__ */ new TypeError("TransformStream terminated"),
      );
    }
        ...
        return new ReadableStream({
          ...
          async pull(ctrl) {
            const buffer = await blob
              .slice(position, Math.min(blob.size, position + POOL_SIZE))
              .arrayBuffer();
    ...
    if ("stream" in part) yield* part.stream();
      ...
      while (position !== b.size) {
        const buffer = await b
          .slice(position, Math.min(b.size, position + POOL_SIZE))
          .arrayBuffer();
        position += buffer.byteLength;
