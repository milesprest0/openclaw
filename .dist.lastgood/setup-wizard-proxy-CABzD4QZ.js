...
import "./setup-binary-IJx9Zfwz.js";
...
function createDetectedBinaryStatus(params) {
  ...
  return {
    ...
    async resolveSelectionHint({ cfg, accountId }) {
      return (await detectBinary$1(
        params.resolveBinaryPath({
          cfg,
          accountId,
        }),
      ))
...
function createDelegatedTextInputShouldPrompt(params) {
  return async (inputParams) => {
    return (
      (await (
        await params.loadWizard()
      ).textInputs?.find((entry) => entry.inputKey === params.inputKey)?.shouldPrompt?.(
        inputParams,
      )) ?? false
    );
  };
...
//#endregion
export {
  createDelegatedSetupWizardProxy as a,
  createDelegatedTextInputShouldPrompt as c,
  createDelegatedResolveConfigured as i,
  createDetectedBinaryStatus as l,
  createDelegatedFinalize as n,
  createCliPathTextInput as o,
  createDelegatedPrepare as r,
  createDelegatedSetupWizardStatusResolvers as s,
  createAllowlistSetupWizardProxy as t,
};
