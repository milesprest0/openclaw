...
import { _ as updatePairedDeviceMetadata } from "./device-pairing-CFABg3cc.js";
...
import { n as sanitizeInboundSystemTags } from "./inbound-text-Uelx5Y74.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import { c as updatePairedNodeMetadata } from "./node-pairing-BFA5m3xB.js";
import { t as createOutboundSendDeps } from "./outbound-send-deps-BGJk55i-.js";
import { i as registerApnsRegistration } from "./push-apns-pGEfqt6M.js";
...
import { l as normalizeMainKey, p as scopedHeartbeatWakeOptions } from "./session-key-B4qUwRzq.js";
import {
...
import { i as deleteMediaBuffer } from "./store-N2etmi9e.js";
  ...
  switch (evt.event) {
    ...
    case "agent.request": {
      ...
      if (deliverRequested && (!channel || !to)) {
        const entryChannel =
          typeof entry?.lastChannel === "string" ? normalizeChannelId(entry.lastChannel) : void 0;
      ...
      agentCommandFromIngress(
        {
          ...
          channel: deliveryChannel,
          timeout:
            typeof link?.timeoutSeconds === "number" ? link.timeoutSeconds.toString() : void 0,
          messageChannel: "node",
          senderIsOwner: false,
          allowModelOverride: false,
        },
    ...
    case "notifications.changed": {
      ...
      const { canonicalKey: sessionKey } = loadSessionEntry(
        normalizeOptionalString(obj.sessionKey) ?? `node-${nodeId}`,
      );
    ...
    case "exec.denied": {
      ...
      const exitCode =
        typeof obj.exitCode === "number" && Number.isFinite(obj.exitCode) ? obj.exitCode : void 0;
    ...
    case NODE_PRESENCE_ALIVE_EVENT: {
      ...
      const now = Date.now();
      if (
        now - (recentNodePresencePersistAt.get(deviceId) ?? 0) <
        NODE_PRESENCE_PERSIST_MIN_INTERVAL_MS
      )
        return {
          ok: true,
          event: evt.event,
