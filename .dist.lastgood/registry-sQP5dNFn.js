...
function clearContextEnginesForOwner(owner) {
  ...
  for (const [id, entry] of registry.entries())
    if (entry.owner === normalizedOwner) registry.delete(id);
}
/**
...
async function resolveContextEngine(config, options) {
  ...
  if (!entry) {
    if (isDefaultEngine)
      throw new Error(
        `Context engine "${engineId}" is not registered. Available engines: ${listContextEngineIds().join(", ") || "(none)"}`,
      );
    console.error(
      `[context-engine] Context engine "${sanitizeForLog(engineId)}" is not registered; falling back to default engine "${defaultEngineId}".`,
    );
  ...
  try {
    ...
    console.error(
      `[context-engine] Context engine "${sanitizeForLog(engineId)}" factory threw during resolution: ${sanitizeForLog(factoryError instanceof Error ? factoryError.message : String(factoryError))}; falling back to default engine "${defaultEngineId}".`,
    );
    return resolveDefaultContextEngine(defaultEngineId, factoryCtx);
  }
  let contractError;
  try {
    ...
    console.error(
      `[context-engine] Context engine "${sanitizeForLog(engineId)}" contract validation threw: ${sanitizeForLog(validationError instanceof Error ? validationError.message : String(validationError))}; falling back to default engine "${defaultEngineId}".`,
    );
    return resolveDefaultContextEngine(defaultEngineId, factoryCtx);
  }
  if (contractError) {
...
async function resolveDefaultContextEngine(defaultEngineId, factoryCtx) {
  ...
  if (!defaultEntry)
    throw new Error(
      `[context-engine] fallback failed: default engine "${defaultEngineId}" is not registered. Available engines: ${listContextEngineIds().join(", ") || "(none)"}`,
    );
...
//#endregion
export {
  resolveContextEngineOwnerPluginId as a,
  resolveContextEngine as i,
  registerContextEngine as n,
  registerContextEngineForOwner as r,
  clearContextEnginesForOwner as t,
};
