...
import { o as installPluginFromPath } from "./install-Djc3uDTp.js";
import { l as tryReadJson } from "./json-files-DifBk3kt.js";
...
async function loadMarketplace(params) {
  const loadMarketplaceFromManifestFile = async (params) => {
    const parsed = parseMarketplaceManifest(
      await fs.readFile(params.manifestPath, "utf-8"),
      params.manifestPath,
    );
    if (!parsed.ok) {
...
async function ensureInsideMarketplaceRoot(rootDir, candidate, options) {
  ...
  if (options?.canonicalRootDir)
    try {
      if (!(await fs.lstat(options.canonicalRootDir)).isDirectory())
      ...
      if (!isPathInside(rootRealPath, await fs.realpath(existingPath)))
...
async function resolveMarketplaceEntryInstallPath(params) {
  if (params.source.kind === "path") {
    ...
    const canonicalRootDir =
      params.marketplaceOrigin === "remote" ? await fs.realpath(params.marketplaceRootDir) : void 0;
...
//#endregion
export {
  listMarketplacePlugins as n,
  resolveMarketplaceInstallShortcut as r,
  installPluginFromMarketplace as t,
};
