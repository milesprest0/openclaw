...
const EXA_SEARCH_TYPES = ["auto", "neural", "fast", "deep", "deep-reasoning", "instant"];
const EXA_FRESHNESS_VALUES = ["day", "week", "month", "year"];
...
function parseExaContents(rawContents) {
  ...
  const parseText = (value) => {
    ...
    if ("maxCharacters" in obj && parsePositiveInteger(obj.maxCharacters) === void 0)
  ...
  const parseHighlights = (value) => {
    ...
    if ("numSentences" in obj && parsePositiveInteger(obj.numSentences) === void 0)
      return invalidContentsPayload("contents.highlights.numSentences must be a positive integer.");
    if ("highlightsPerUrl" in obj && parsePositiveInteger(obj.highlightsPerUrl) === void 0)
      return invalidContentsPayload(
...
async function executeExaWebSearchProviderTool(ctx, args) {
  ...
  const count =
    readNumberParam(params, "count", { integer: true }) ?? searchConfig?.maxResults ?? void 0;
