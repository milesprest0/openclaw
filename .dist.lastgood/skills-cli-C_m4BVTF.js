import { a as resolveAgentIdByWorkspacePath } from "./agent-scope-9AmhTwki.js";
import {
...
import { t as resolveOptionFromCommand } from "./cli-utils-Ce4lxCq8.js";
...
import {
  ...
  n as readTrackedClawHubSkillSlugs,
  ...
  if (
    ...
    if (skill.requirements.bins.length > 0) {
      const binsStatus = skill.requirements.bins.map((bin) => {
        return skill.missing.bins.includes(bin)
          ? theme.error(`✗ ${bin}`)
          : theme.success(`✓ ${bin}`);
      });
    ...
    if (skill.requirements.env.length > 0) {
      const envStatus = skill.requirements.env.map((env) => {
        return skill.missing.env.includes(env)
          ? theme.error(`✗ ${env}`)
          : theme.success(`✓ ${env}`);
      });
    ...
    if (skill.requirements.config.length > 0) {
      const configStatus = skill.requirements.config.map((cfg) => {
        return skill.missing.config.includes(cfg)
          ? theme.error(`✗ ${cfg}`)
          : theme.success(`✓ ${cfg}`);
      });
    ...
    if (skill.requirements.os.length > 0) {
      const osStatus = skill.requirements.os.map((osName) => {
        return skill.missing.os.includes(osName)
          ? theme.error(`✗ ${osName}`)
...
async function loadSkillsStatusReport(options) {
  ...
  const { buildWorkspaceSkillStatus } = await import("./skills-status-BjtbbENL.js");
