...
function resolveApiMaxCompletionTokens(params) {
  ...
  return Math.min(
    raw,
    contextWindow ?? knownMaxTokens ?? VENICE_DEFAULT_CONTEXT_WINDOW,
    knownMaxTokens ?? VENICE_DISCOVERY_HARD_MAX_TOKENS,
  );
}
  ...
  try {
    const { response, release } = await retryAsync(
      ...
      },
      {
        ...
        jitter: options.retryDelayMs === void 0 ? 0.2 : 0,
...
//#endregion
export {
  discoverVeniceModels as a,
  buildVeniceModelDefinition as i,
  VENICE_DEFAULT_MODEL_REF as n,
  VENICE_MODEL_CATALOG as r,
  VENICE_BASE_URL as t,
};
