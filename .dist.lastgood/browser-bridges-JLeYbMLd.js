...
import { t as buildDockerExecArgs } from "./bash-tools.shared-xCJhgAkw.js";
...
import { n as loadActivatedBundledPluginPublicSurfaceModuleSync } from "./facade-runtime-06m_K_31.js";
import "./sandbox-paths-BVIaOwJU.js";
import { i as isPathInside } from "./path-1liOXr_N.js";
import { r as resolveRootPath } from "./root-path-BwPOxXJq.js";
import { t as sanitizeEnvVars } from "./sanitize-env-vars-BhxumuQr.js";
import { c as PATH_ALIAS_POLICIES } from "./secure-temp-dir-CCj3cY2B.js";
import { t as parseSshTarget } from "./ssh-tunnel-BggovJ_Q.js";
import {
...
var RemoteShellSandboxFsBridge = class {
  constructor(sandbox, runtime) {
    ...
    this.resolveRenameTargets = createWritableRenameTargetResolver(
  ...
  async stat(params) {
    ...
    const [kindRaw = "", sizeRaw = "0", mtimeRaw = "0"] = (
  ...
  resolveMountByContainerPath(mounts, containerPath) {
    ...
    for (const mount of ordered)
      if (isPathInsideContainerRoot(mount.containerRoot, containerPath)) return mount;
    return null;
  }
  ...
  async resolveCanonicalPath(params) {
    ...
    const canonical = normalizeContainerPath(
      (
        await this.runRemoteScript({
          script,
...
function normalizeInlineSshMaterial(contents, filename) {
  const normalizedEscapedNewlines = contents
    .replace(/^\uFEFF/, "")
    .replace(/\r\n?/g, "\n")
    .replace(/\\r\\n/g, "\\n")
    .replace(/\\r/g, "\\n");
...
function buildSshFailureMessage(stderr, exitCode) {
  ...
  return (
    trimmed ||
    (exitCode !== void 0 ? `ssh exited with code ${exitCode}` : "ssh exited with a non-zero status")
  );
}
...
//#endregion
export {
  buildPinnedMkdirpPlan as C,
  dockerSandboxBackendManager as D,
  buildPinnedWritePlan as E,
  runDockerSandboxShellCommand as O,
  resolveWritableRenameTargetsForBridge as S,
  buildPinnedRenamePlan as T,
  createRemoteShellSandboxFsBridge as _,
  getSandboxBackendManager as a,
  createWritableRenameTargetResolver as b,
  buildExecRemoteCommand as c,
  createSshSandboxSessionFromConfigText as d,
  createSshSandboxSessionFromSettings as f,
  uploadDirectoryToSshTarget as g,
  shellEscape as h,
  getSandboxBackendFactory as i,
  buildRemoteCommand as l,
  runSshSandboxCommand as m,
  startBrowserBridgeServer as n,
  registerSandboxBackend as o,
  disposeSshSandboxSession as p,
  stopBrowserBridgeServer as r,
  requireSandboxBackendFactory as s,
  BROWSER_BRIDGES as t,
  buildSshSandboxArgv as u,
  isPathInsideContainerRoot as v,
  buildPinnedRemovePlan as w,
  resolveWritableRenameTargets as x,
  normalizeContainerPath$1 as y,
};
