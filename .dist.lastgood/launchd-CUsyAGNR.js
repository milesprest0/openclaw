...
import { i as normalizeEnvVarKey, o as sanitizeHostExecEnv } from "./host-env-security-W4c-l4Zm.js";
import {
...
import {
  ...
  h as resolveLegacyGatewayLaunchAgentLabels,
  n as resolveHomeDir,
  r as GATEWAY_LAUNCH_AGENT_LABEL,
  t as resolveGatewayStateDir,
} from "./paths-D-SPS6K1.js";
import {
  n as renderPosixRestartLogSetup,
  r as resolveGatewayLogPaths,
} from "./restart-logs-DvjnXu8L.js";
import { t as cleanStaleGatewayProcessesSync } from "./restart-stale-pids-KOGEujAp.js";
import {
...
import { _ as execFileUtf8 } from "./systemd-DKgtnjvB.js";
...
function buildLaunchAgentPlist$1({
  label,
  comment,
  programArguments,
  workingDirectory,
...
async function prepareLaunchAgentProgramArguments(params) {
  ...
  return { programArguments: [wrapperPath, envFilePath, ...params.programArguments] };
}
...
async function execLaunchctl(args) {
  ...
  return await execFileUtf8(
    isWindows ? (process.env.ComSpec ?? "cmd.exe") : "launchctl",
    isWindows ? ["/d", "/s", "/c", "launchctl", ...args] : args,
...
async function resolveLaunchAgentGatewayPort(env) {
  const fromArgs = parseGatewayPortFromProgramArguments(
    (await readLaunchAgentProgramArguments(env).catch(() => null))?.programArguments,
  );
...
async function isLaunchAgentLoaded(args) {
  return (
    (
      await execLaunchctl([
        "print",
        `${resolveGuiDomain()}/${resolveLaunchAgentLabel({ env: args.env })}`,
      ])
    ).code === 0
  );
}
...
async function readLaunchAgentRuntime(env) {
  const res = await execLaunchctl([
    "print",
    `${resolveGuiDomain()}/${resolveLaunchAgentLabel({ env })}`,
  ]);
  if (res.code !== 0) {
...
async function repairLaunchAgentBootstrap(args) {
  ...
  if ((await readLaunchAgentRuntime(env)).status === "running")
    return {
      ok: true,
      status: repairStatus,
    };
...
//#endregion
export {
  readLaunchAgentRuntime as a,
  restartLaunchAgent as c,
  uninstallLaunchAgent as d,
  readLaunchAgentProgramArguments as i,
  stageLaunchAgent as l,
  isLaunchAgentLoaded as n,
  repairLaunchAgentBootstrap as o,
  launchAgentPlistExists as r,
  resolveLaunchAgentPlistPath as s,
  installLaunchAgent as t,
  stopLaunchAgent as u,
};
