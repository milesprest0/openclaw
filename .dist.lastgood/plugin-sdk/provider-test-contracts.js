...
import {
  ...
  r as beforeAll,
  t as afterAll,
} from "../dist-BosyQhji.js";
import { r as listSupportedVideoGenerationModes } from "../duration-support-DAFa2fE_.js";
import { i as makeResponse, r as createProviderUsageFetch } from "../frozen-time-BM3blIF1.js";
import {
  n as resolveAgentModelFallbackValues,
  r as resolveAgentModelPrimaryValue,
} from "../model-input-C-vWoAxX.js";
import { i as listSupportedMusicGenerationModes } from "../openclaw-tools-6wQ_ahnn.js";
import "../provider-onboard-kfCYjVQt.js";
import "../runtime-DM1BYRxu.js";
import "../agent-runtime-CG9i2LmY.js";
import {
  b as registerSingleProviderPlugin,
  v as registerProviderPlugin,
  x as requireRegisteredProvider,
  y as registerProviderPlugins,
} from "../plugin-setup-wizard-CMErCcys.js";
import { r as resolveProviderPluginChoice } from "../provider-auth-choice.runtime-DscmBbh5.js";
import { i as runProviderCatalog } from "../provider-discovery-CtL4i5sV.js";
import {
  i as resolveProviderWizardOptions,
  n as resolveProviderModelPickerEntries,
  o as setProviderWizardProvidersResolverForTest,
  t as buildProviderPluginMethodChoice,
} from "../provider-wizard-BnefwuK0.js";
import {
  n as loadBundledPluginPublicSurfaceSync,
  t as loadBundledPluginPublicSurface,
} from "../public-surface-loader-EeN7zXlH.js";
import {
  a as resolveWebFetchProviderContractEntriesForPluginId,
  i as resolveProviderContractProvidersForPluginIds,
  n as providerContractLoadError,
  o as resolveWebSearchProviderContractEntriesForPluginId,
  s as resolveBundledExplicitProviderContractsFromPublicArtifacts,
  t as pluginRegistrationContractRegistry,
} from "../registry-_MhICdyY.js";
import { t as createNonExitingRuntime } from "../runtime-kqN0Yohi.js";
import { t as clearRuntimeAuthProfileStoreSnapshots } from "../store-9hg03xJO.js";
import "../plugin-test-runtime-DcCkrGah.js";
import "../test-env-CsZ4AqPU.js";
import { n as vi, t as globalExpect } from "../test.DNmyFkvJ-DuAhK9jz.js";
import {
  a as expectedAugmentedOpenaiCodexCatalogEntriesWithGpt55,
  i as expectCodexMissingAuthHint,
  o as expectedOpenaiPluginCodexCatalogEntriesWithGpt55,
  r as expectAugmentedCodexCatalog,
} from "../testing-BOOYNCIh.js";
import {
  i as resolveBundledExplicitWebSearchProvidersFromPublicArtifacts,
  r as resolveBundledExplicitWebFetchProvidersFromPublicArtifacts,
} from "../web-provider-public-artifacts.explicit-CjQ-BUNt.js";
...
function installSharedAuthProfileStoreHooks(state) {
  beforeEach(() => {
    ...
    listProfilesForProviderMock$1.mockImplementation((store, providerId) =>
  ...
  describe("openai-codex provider auth contract", () => {
    ...
    async function expectStableFallbackProfile(params) {
      ...
      const provider = requireRegisteredProvider(
        await registerProviderPlugins(openAIPlugin),
        "openai-codex",
      );
      ...
      globalExpect(await provider.auth[0]?.run(buildAuthContext())).toEqual(
    ...
    it("uses a stable fallback id when JWT email is missing", async () => {
      await expectStableFallbackProfile({
        access: createJwt({
          "https://api.openai.com/auth": { chatgpt_account_user_id: "user-123__acct-456" },
        }),
    ...
    it("keeps OAuth failures non-fatal at the provider layer", async () => {
      ...
      await globalExpect(provider.auth[0]?.run(buildAuthContext())).resolves.toEqual({
        profiles: [],
      });
    });
  });
...
function describeGithubCopilotProviderAuthContract(load) {
  ...
  describe("github-copilot provider auth contract", () => {
    ...
    async function getProvider() {
      ...
      return requireRegisteredProvider(
        await registerProviderPlugins(githubCopilotPlugin),
        "github-copilot",
      );
    }
    ...
    it("uses the wizard prompter and openUrl hooks for the device code (no stdin/stdout)", async () => {
      ...
      const codeNote = ctx.prompter.note.mock.calls.find(
...
function installProviderPluginContractSuite(params) {
  it("satisfies the base provider plugin contract", () => {
    ...
    for (const method of provider.auth) {
      ...
      if (method.wizard) {
        ...
        if (method.wizard.modelAllowlist?.allowedKeys)
          globalExpect(method.wizard.modelAllowlist.allowedKeys).toEqual([
            ...new Set(method.wizard.modelAllowlist.allowedKeys),
          ]);
        if (method.wizard.modelAllowlist?.initialSelections)
          globalExpect(method.wizard.modelAllowlist.initialSelections).toEqual([
    ...
    if (provider.wizard?.setup || provider.wizard?.modelPicker)
      globalExpect(provider.auth.length).toBeGreaterThan(0);
    if (provider.wizard?.setup) {
      ...
      if (provider.wizard.setup.methodId)
        globalExpect(authIds).toContain(provider.wizard.setup.methodId);
      if (provider.wizard.setup.modelAllowlist?.allowedKeys)
        globalExpect(provider.wizard.setup.modelAllowlist.allowedKeys).toEqual([
          ...new Set(provider.wizard.setup.modelAllowlist.allowedKeys),
        ]);
      if (provider.wizard.setup.modelAllowlist?.initialSelections)
        globalExpect(provider.wizard.setup.modelAllowlist.initialSelections).toEqual([
          ...new Set(provider.wizard.setup.modelAllowlist.initialSelections),
        ]);
    }
    if (provider.wizard?.modelPicker?.methodId)
      globalExpect(authIds).toContain(provider.wizard.modelPicker.methodId);
  });
}
...
function installWebFetchProviderContractSuite(params) {
  it("satisfies the base web fetch provider contract", () => {
    ...
    if (provider.inactiveSecretPaths) {
      globalExpect(provider.inactiveSecretPaths).toEqual([
        ...new Set(provider.inactiveSecretPaths),
      ]);
    ...
    if (provider.applySelectionConfig && params.pluginId)
      globalExpect(
        provider.applySelectionConfig({}).plugins?.entries?.[params.pluginId]?.enabled,
      ).toBe(true);
  });
}
//#endregion
...
function setRuntimeAuthStore(store) {
  ...
  listProfilesForProviderMock.mockImplementation((authStore, providerId) =>
  ...
  beforeAll(async () => {
    ...
    vi.doMock("openclaw/plugin-sdk/provider-auth", () => {
      return {
        ...
        normalizeOptionalSecretInput: (value) =>
          typeof value === "string" && value.trim() ? value.trim() : void 0,
    ...
    if (options.providerIds.includes("github-copilot")) {
      ...
      state.githubCopilotProvider = requireRegisteredProvider(
        await registerProviderPlugins(githubCopilotPlugin),
        "github-copilot",
      );
    }
    if (options.providerIds.includes("vllm")) {
      ...
      state.vllmProvider = requireRegisteredProvider(
        await registerProviderPlugins(vllmPlugin),
        "vllm",
      );
    }
    if (options.providerIds.includes("sglang")) {
      const { default: sglangPlugin } = await options.loadSglang();
      state.sglangProvider = requireRegisteredProvider(
        await registerProviderPlugins(sglangPlugin),
        "sglang",
      );
    }
    if (options.providerIds.includes("minimax")) {
      ...
      state.minimaxPortalProvider = requireRegisteredProvider(
        registeredProviders,
        "minimax-portal",
      );
    }
    if (options.providerIds.includes("modelstudio")) {
      ...
      state.modelStudioProvider = requireRegisteredProvider(
        await registerProviderPlugins(qwenPlugin),
        "qwen",
      );
    }
    if (options.providerIds.includes("cloudflare-ai-gateway")) {
      ...
      state.cloudflareAiGatewayProvider = requireRegisteredProvider(
        await registerProviderPlugins(cloudflareAiGatewayPlugin),
      ...
      await globalExpect(
        ...
        provider: {
          ...
          models: globalExpect.arrayContaining([
            globalExpect.objectContaining({ id: "MiniMax-M2.7" }),
            globalExpect.objectContaining({ id: "MiniMax-M2.7-highspeed" }),
          ]),
        },
      });
    });
...
function expectDashscopeVideoTaskPoll(fetchWithTimeoutMock, params = {}) {
  ...
  globalExpect(fetchWithTimeoutMock).toHaveBeenNthCalledWith(
    1,
    `${baseUrl}/api/v1/tasks/${taskId}`,
    globalExpect.objectContaining({ method: "GET" }),
    timeoutMs,
    fetch,
  );
}
function expectSuccessfulDashscopeVideoResult(result, params = {}) {
  ...
  globalExpect(result.metadata).toEqual(
...
function expectExplicitMusicGenerationCapabilities(provider) {
  ...
  globalExpect(
    ...
    `${provider.id} missing edit capabilities`,
  ).toBeDefined();
...
function expectProviderOnboardAllowlistAlias(params) {
  ...
  globalExpect(
    params.applyProviderConfig({
      agents: { defaults: { models: { [params.modelRef]: { alias: params.alias } } } },
    }).agents?.defaults?.models?.[params.modelRef]?.alias,
  ).toBe(params.alias);
}
function expectProviderOnboardPrimaryAndFallbacks(params) {
  ...
  globalExpect(
    resolveAgentModelFallbackValues(
      params.applyConfig(createConfigWithFallbacks()).agents?.defaults?.model,
    ),
  ).toEqual([...EXPECTED_FALLBACKS]);
}
function expectProviderOnboardPrimaryModel(params) {
  globalExpect(resolveAgentModelPrimaryValue(params.applyConfig({}).agents?.defaults?.model)).toBe(
    params.modelRef,
  );
}
function expectProviderOnboardPreservesPrimary(params) {
  ...
  ).toBe(params.primaryModelRef);
}
function expectProviderOnboardMergedLegacyConfig(params) {
  const provider = params.applyProviderConfig(
...
function describeAnthropicProviderRuntimeContract(load) {
  describe("anthropic provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    ...
    it("owns usage auth resolution", async () => {
      await globalExpect(
        requireProviderContractProvider("anthropic").resolveUsageAuth?.({
          config: {},
          env: {},
          provider: "anthropic",
...
function describeGoogleProviderRuntimeContract(load) {
  describe("google provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    ...
    it("owns usage-token parsing", async () => {
      await globalExpect(
        requireProviderContractProvider("google-gemini-cli").resolveUsageAuth?.({
          config: {},
          env: {},
          provider: "google-gemini-cli",
          resolveApiKeyFromConfigAndStore: () => void 0,
    ...
    it("owns OAuth auth-profile formatting", () => {
      globalExpect(
        requireProviderContractProvider("google-gemini-cli").formatApiKey?.({
          type: "oauth",
          provider: "google-gemini-cli",
...
function describeOpenAIProviderRuntimeContract(load) {
  describe("openai provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    ...
    it("owns direct openai transport normalization", () => {
      globalExpect(
        requireProviderContractProvider("openai").normalizeResolvedModel?.({
          provider: "openai",
          modelId: "gpt-5.4",
          model: createModel({
            id: "gpt-5.4",
    ...
    it("owns refresh fallback for accountId extraction failures", async () => {
      ...
      refreshOpenAICodexTokenMock.mockRejectedValueOnce(
        /* @__PURE__ */ new Error("Failed to extract accountId from token"),
      );
    ...
    it("owns codex transport defaults", () => {
      globalExpect(
        requireProviderContractProvider("openai-codex").prepareExtraParams?.({
          provider: "openai-codex",
          modelId: "gpt-5.4",
...
function describeVeniceProviderRuntimeContract(load) {
  describe("venice provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    ...
    it("owns xai downstream compat flags for grok-backed Venice models", () => {
      globalExpect(
        requireProviderContractProvider("venice").normalizeResolvedModel?.({
          provider: "venice",
...
function describeZAIProviderRuntimeContract(load) {
  describe("zai provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
    ...
    it("owns usage auth resolution", async () => {
      await globalExpect(
        requireProviderContractProvider("zai").resolveUsageAuth?.({
          config: {},
...
function describeProviderWizardSetupOptionsContract() {
  describe("provider wizard setup options contract", () => {
    it("exposes every wizard setup choice through the shared wizard layer", () => {
      ...
      globalExpect(sortedValues(options.map((option) => option.value))).toEqual(
...
function describeProviderWizardModelPickerContract() {
  describe("provider wizard model picker contract", () => {
    it("exposes every model-picker entry through the shared wizard layer", () => {
      ...
      globalExpect(sortedValues(entries.map((entry) => entry.value))).toEqual(
