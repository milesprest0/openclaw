...
import { c as it } from "../dist-BosyQhji.js";
import { n as formatZonedTimestamp, t as formatUtcTimestamp } from "../format-datetime-Cd0s8Nxm.js";
import {
  a as resetGlobalHookRunner,
  i as initializeGlobalHookRunner,
} from "../hook-runner-global-DzzZF3B1.js";
import { t as addTestHook } from "../hooks.test-helpers-D3AKMEuJ.js";
import {
  n as resolveInboundMentionDecision,
  t as implicitMentionKindWhen,
} from "../mention-gating-DJ8J7HbK.js";
import {
  C as createOutboundTestPlugin,
  g as createRuntimeEnv,
  w as createTestRegistry,
...
async function expectDirectoryIds(listFn, cfg, expected, options) {
  ...
  globalExpect(options?.sorted ? sortDirectoryIds(ids) : ids).toEqual(
...
function installChannelActionsContractSuite(params) {
  ...
  for (const testCase of params.cases)
    it(`actions contract: ${testCase.name}`, () => {
      ...
      globalExpect(sortStrings(capabilities)).toEqual(
        sortStrings(testCase.expectedCapabilities ?? []),
      );
      if (params.plugin.actions?.supportsAction) {
        for (const action of testCase.expectedActions)
          globalExpect(params.plugin.actions.supportsAction({ action })).toBe(true);
        if (
          params.unsupportedAction &&
          !testCase.expectedActions.includes(params.unsupportedAction)
        )
          globalExpect(
            params.plugin.actions.supportsAction({ action: params.unsupportedAction }),
          ).toBe(false);
      }
    });
}
function installChannelSetupContractSuite(params) {
  ...
  for (const testCase of params.cases)
    it(`setup contract: ${testCase.name}`, () => {
      ...
      ).toBe(testCase.expectedValidation ?? null);
  ...
  for (const testCase of params.cases)
    it(`status contract: ${testCase.name}`, async () => {
      ...
      if (testCase.expectedState && params.plugin.status?.resolveAccountState)
        globalExpect(
          params.plugin.status.resolveAccountState({
            account,
            cfg: testCase.cfg,
  ...
  return mergeDeep(
    {
      ...
      channel: {
        ...
        mentions: {
          ...
          matchesMentionPatterns: vi.fn((text, regexes) =>
            regexes.some((regex) => regex.test(text)),
          ),
          matchesMentionWithExplicit: vi.fn((params) =>
        ...
        debounce: {
          ...
          resolveInboundDebounceMs: vi.fn((params) => {
            ...
            const perChannel =
              p?.channel && inbound?.byChannel ? inbound.byChannel[p.channel] : void 0;
...
function createSendCfgThreadingRuntime({
  loadConfig,
