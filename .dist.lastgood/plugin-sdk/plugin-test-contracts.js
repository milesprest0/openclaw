...
import { a as describe, c as it } from "../dist-BosyQhji.js";
import { t as loadPluginManifestRegistry } from "../manifest-registry-DJ43wWua.js";
import "../testing-BOOYNCIh.js";
import { n as parseMinHostVersionRequirement } from "../min-host-version-DI6PDdpD.js";
import {
  x as requireRegisteredProvider,
  y as registerProviderPlugins,
} from "../plugin-setup-wizard-CMErCcys.js";
import {
  a as registerTestPlugin,
  c as assertNoImportTimeSideEffects,
  i as createPluginRegistryFixture,
  n as loadBundledPluginPublicSurfaceSync,
  o as registerVirtualTestPlugin,
  r as resolveWorkspacePackagePublicModuleUrl,
  s as uniqueSortedStrings,
  t as loadBundledPluginPublicSurface,
} from "../public-surface-loader-EeN7zXlH.js";
import { t as pluginRegistrationContractRegistry } from "../registry-_MhICdyY.js";
import { a as parseSemver, n as isAtLeast } from "../runtime-guard-vJnP437q.js";
import {
  n as assertUniqueValues,
  t as BUNDLED_RUNTIME_SIDECAR_PATHS,
} from "../runtime-sidecar-paths-BZ15zlXV.js";
    ...
    if (minHostVersionBaseline)
      it("declares a parseable minHostVersion floor at or above the baseline", () => {
        ...
        const requirement = parseMinHostVersionRequirement(
          readJson(packagePath).openclaw?.install?.minHostVersion ?? null,
        );
        globalExpect(
          requirement,
  ...
  describe(`${params.pluginId} plugin registration contract`, () => {
    ...
    if (params.webSearchProviderIds)
      it("keeps bundled web search ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).webSearchProviderIds).toEqual(
          params.webSearchProviderIds,
        );
      });
    if (params.webFetchProviderIds)
      it("keeps bundled web fetch ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).webFetchProviderIds).toEqual(
          params.webFetchProviderIds,
        );
      });
    if (params.speechProviderIds)
      it("keeps bundled speech ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).speechProviderIds).toEqual(
    ...
    if (params.realtimeTranscriptionProviderIds)
      it("keeps bundled realtime-transcription ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).realtimeTranscriptionProviderIds).toEqual(
    ...
    if (params.realtimeVoiceProviderIds)
      it("keeps bundled realtime-voice ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).realtimeVoiceProviderIds).toEqual(
    ...
    if (params.mediaUnderstandingProviderIds)
      it("keeps bundled media-understanding ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).mediaUnderstandingProviderIds).toEqual(
    ...
    if (params.imageGenerationProviderIds)
      it("keeps bundled image-generation ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).imageGenerationProviderIds).toEqual(
    ...
    if (params.videoGenerationProviderIds)
      it("keeps bundled video-generation ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).videoGenerationProviderIds).toEqual(
    ...
    if (params.musicGenerationProviderIds)
      it("keeps bundled music-generation ownership explicit", () => {
        globalExpect(findRegistration(params.pluginId).musicGenerationProviderIds).toEqual(
    ...
    if (manifestAuthChoice)
      it("keeps onboarding auth grouping explicit", () => {
        globalExpect(
          loadPluginManifestRegistry({}).plugins.find(
...
const BUNDLED_RUNTIME_SIDECAR_BASENAMES = assertUniqueValues(
  [...new Set(BUNDLED_RUNTIME_SIDECAR_PATHS.map(getPublicArtifactBasename))],
  "bundled runtime sidecar basename",
);
const GUARDED_EXTENSION_PUBLIC_SURFACE_BASENAMES = assertUniqueValues(
