import { i as markCronJobActive, t as clearCronJobActive } from "./active-jobs-qoCzunMt.js";
import { c as resolveDefaultAgentId } from "./agent-scope-config-CXZGyKMl.js";
import { t as cleanupBrowserSessionsForLifecycleEnd } from "./browser-lifecycle-cleanup-4oZuhl4Y.js";
...
import {
  n as resolveFailureDestination,
  t as resolveCronDeliveryPlan,
} from "./delivery-plan-BkyfCXvN.js";
...
import { o as resolveFailoverReasonFromError } from "./failover-error-Dx3vQAkt.js";
import { n as fetchWithSsrFGuard } from "./fetch-guard-jcadyt5o.js";
import { n as runHeartbeatOnce } from "./heartbeat-runner-CPuhMMK_.js";
import {
...
import { t as runCronIsolatedAgentTurn } from "./isolated-agent-z4lme3jv.js";
import {
  a as computeJobPreviousRunAtMs,
  c as findJobOrThrow,
  d as isJobEnabled,
  f as nextWakeAtMs,
  g as resolveJobPayloadTextForMain,
  h as recordScheduleComputeError,
  i as computeJobNextRunAtMs,
  l as hasScheduledNextRunAtMs,
  m as recomputeNextRunsForMaintenance,
  n as applyJobPatch,
  o as createJob,
  p as recomputeNextRuns,
  r as assertSupportedJobSpec,
  s as errorBackoffMs,
  t as DEFAULT_ERROR_BACKOFF_SCHEDULE_MS,
  u as isJobDue,
} from "./jobs-SDHRSbuQ.js";
...
import { n as normalizeOptionalAgentId } from "./normalize-y9XtuoNT.js";
import { t as createOutboundSendDeps } from "./outbound-send-deps-BGJk55i-.js";
import { t as parseDurationMs } from "./parse-duration-DB19jQGd.js";
import { u as resolveStorePath } from "./paths-CfeECf6Z.js";
import {
  a as summarizeCronRunDiagnostics,
  i as normalizeCronRunDiagnostics,
  n as createCronRunDiagnosticsFromError,
} from "./run-diagnostics-prIxGL_8.js";
import { t as createCronExecutionId } from "./run-id-O8DiVdNC.js";
import {
  a as resolveCronRunLogPath,
  o as resolveCronRunLogPruneOptions,
  t as appendCronRunLog,
} from "./run-log-C0aatsE-.js";
import { t as abortAndDrainEmbeddedPiRun } from "./runs-CztEUxOL.js";
...
import "./session-utils.fs-z-7Kw0Lg.js";
import {
  c as normalizeAgentId,
  h as toAgentStoreSessionKey,
  u as resolveAgentIdFromSessionKey,
} from "./session-key-B4qUwRzq.js";
import "./pi-embedded-D_bdRDPs.js";
import { r as isCronRunSessionKey } from "./session-key-utils-B3KPN8Ee.js";
import { i as cleanupArchivedSessionTranscripts } from "./session-transcript-files.fs-D-z-Dnd7.js";
import { t as SsrFBlockedError } from "./ssrf-DO8eIXaD.js";
import { n as persist, r as warnIfDisabled, t as ensureLoaded } from "./store-BEz0x4av.js";
import {
...
import {
  a as resolveCronNotificationSessionKey,
  i as resolveCronDeliverySessionKey,
  o as resolveCronSessionTargetSessionKey,
  t as normalizeHttpWebhookUrl,
...
function isTransientCronError(error, retryOn) {
  ...
  return (retryOn?.length ? retryOn : Object.keys(TRANSIENT_PATTERNS)).some((k) =>
    TRANSIENT_PATTERNS[k]?.test(error),
  );
}
...
function maybeEmitFailureAlert(state, params) {
  ...
  if (typeof lastAlert === "number" && now - lastAlert < Math.max(0, params.alertConfig.cooldownMs))
    return;
...
function applyJobResult(state, job, result, opts) {
  ...
  job.state.lastErrorReason =
    result.status === "error" && typeof result.error === "string"
      ? (resolveFailoverReasonFromError(result.error) ?? void 0)
      : void 0;
  ...
  job.state.lastDeliveryError =
    deliveryState.status === "not-delivered" && result.error ? result.error : void 0;
...
async function start(state) {
  ...
  await locked(state, async () => {
    ...
    if (markedAnyInterruptedRun || jobs.length > 0)
      await persist(state, markedAnyInterruptedRun ? void 0 : { stateOnly: true });
  });
  ...
  try {
    const { startCronStoreWatcher } = await import("./file-watcher-2iOSQWwQ.js");
...
async function list(state, opts) {
  return await locked(state, async () => {
    ...
    return (state.store?.jobs ?? [])
      .filter((j) => includeDisabled || isJobEnabled(j))
...
function resolveEffectiveJobAgentId(job, defaultAgentId) {
  return (
    normalizeOptionalAgentId(job.agentId) ?? normalizeOptionalAgentId(defaultAgentId) ?? "main"
  );
}
...
async function inspectManualRunPreflight(state, id, mode) {
  return await locked(state, async () => {
    ...
    const now = state.deps.nowMs();
    if (!isJobDue(job, now, { forced: mode === "force" }))
      return {
        ok: true,
        ran: false,
        reason: "not-due",
...
function buildGatewayCronService(params) {
  ...
  const findAgentEntry = (cfg, agentId) =>
  ...
  const resolveCronAgent = (requested) => {
    ...
    const normalized =
      typeof requested === "string" && requested.trim() ? normalizeAgentId(requested) : void 0;
    const effectiveConfig =
      normalized !== void 0 ? mergeRuntimeAgentConfig(runtimeConfig, normalized) : runtimeConfig;
    return {
      agentId:
        normalized !== void 0 && hasConfiguredAgent(effectiveConfig, normalized)
          ? normalized
  ...
  const resolveCronSessionKey = (params) => {
    ...
    if (canonical !== "global") {
      if (
        normalizeAgentId(resolveAgentIdFromSessionKey(canonical)) !==
  ...
  const resolveCronWakeTarget = (opts) => {
    const derivedAgentId =
      (typeof opts?.agentId === "string" && opts.agentId.trim()
        ...
        : void 0) ??
      (opts?.sessionKey ? normalizeAgentId(resolveAgentIdFromSessionKey(opts.sessionKey)) : void 0);
    const runtimeConfigBase = getRuntimeConfig();
    const runtimeConfig =
      derivedAgentId !== void 0
  ...
  const resolveSessionStorePath = (agentId) =>
    resolveStorePath(params.cfg.session?.store, { agentId: agentId ?? defaultAgentId });
  ...
  const cron = new CronService({
    ...
    runHeartbeatOnce: async (opts) => {
      ...
      const agentHeartbeat =
        agentEntry && typeof agentEntry === "object" ? agentEntry.heartbeat : void 0;
      const baseHeartbeat = {
