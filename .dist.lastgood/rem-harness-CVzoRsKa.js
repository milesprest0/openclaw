...
import {
  n as previewRemDreaming,
  t as filterRecallEntriesWithinLookback,
...
function sectionToSnippets(section) {
  ...
  for (const entry of section.lines) {
    ...
    const text = normalizeWhitespace(
      stripMarkdown(trimmed.match(/^(?:[-*+]|\d+\.)\s+(?:\[[ xX]\]\s*)?(.*)$/)?.[1] ?? trimmed),
    );
...
function scoreSnippet(text, title) {
  ...
  if (REM_LOGISTICS_SIGNAL_RE.test(text)) score += 0.9;
  if (REM_ROUTING_SIGNAL_RE.test(text)) score += 1.4;
  if (REM_EXTERNALIZATION_SIGNAL_RE.test(text)) score += 1.1;
  if (REM_RETRY_SIGNAL_RE.test(text)) score += 0.9;
  if (REM_TASK_SIGNAL_RE.test(text) && !REM_BUILD_SIGNAL_RE.test(text)) score -= 0.8;
  if (title && !REM_GENERIC_SECTION_RE.test(title)) score += 0.25;
  return score;
}
...
function scoreCandidateSnippet(text, title) {
  ...
  if (REM_BUILD_SIGNAL_RE.test(text) && !REM_MEMORY_SIGNAL_RE.test(text)) score -= 0.8;
  if (REM_SECTION_TRANSIENT_TITLE_RE.test(title) && !REM_SECTION_PERSISTENCE_TITLE_RE.test(title))
    score -= 1.2;
  if (/[`/]/.test(text) || /https?:\/\//i.test(text)) score -= 0.8;
  return score;
}
