...
import { i as emitAgentEvent } from "./agent-events-c6-2eZvI.js";
import {
...
import { n as defaultCodexAppServerClientFactory } from "./client-factory-BDiPwgZz.js";
import {
  i as isCodexAppServerConnectionClosedError,
  r as isCodexAppServerApprovalRequest,
} from "./client-PeiS3WVh.js";
import {
  l as resolveCodexPluginsPolicy,
  o as readCodexPluginConfig,
  s as resolveCodexAppServerRuntimeOptions,
  u as withMcpElicitationsApprovalPolicy,
} from "./config-CNEUyjZv.js";
import { i as emitTrustedDiagnosticEvent } from "./diagnostic-events-9isI4hMq.js";
import { a as formatErrorMessage$1 } from "./errors-DZMrVkYL.js";
...
import {
  i as defaultCodexAppInventoryCache,
  r as buildCodexAppInventoryCacheKey,
} from "./plugin-activation-CPsfPsCt.js";
import { t as isJsonObject } from "./protocol-DayKrDMy.js";
import {
  i as readCodexDynamicToolCallParams,
  r as assertCodexTurnStartResponse,
  s as readCodexTurn,
} from "./protocol-validators-BXWxEgv_.js";
import {
  n as rememberCodexRateLimits,
  r as ensureCodexComputerUse,
  t as readRecentCodexRateLimits,
} from "./rate-limit-cache-BbpHSs2n.js";
import { n as redactSensitiveFieldValue, o as redactToolPayloadText } from "./redact-Mxj55RzQ.js";
import { t as appendRegularFile } from "./regular-file-CoVHB-2u.js";
import {
  d as formatCodexUsageLimitErrorMessage,
  i as formatCodexDisplayText,
} from "./request-BC8expe9.js";
import { f as setActiveEmbeddedRun, r as clearActiveEmbeddedRun } from "./runs-CztEUxOL.js";
import { o as resolveSandboxContext } from "./sandbox-Cz0C2Qt-.js";
import { i as readCodexAppServerBinding } from "./session-binding-o-m5DVaT.js";
...
import {
  c as resolveCodexAppServerAuthAccountCacheKey,
  d as resolveCodexAppServerEnvApiKeyCacheKey,
  f as resolveCodexAppServerHomeDir,
  l as resolveCodexAppServerAuthProfileId,
  r as clearSharedCodexAppServerClientIfCurrent,
  s as refreshCodexAppServerAuthTokens,
  u as resolveCodexAppServerAuthProfileIdForAgent,
} from "./shared-client-NipDnwuu.js";
import {
  a as buildTurnStartParams,
  c as buildCodexPluginThreadConfig,
  d as createCodexDynamicToolBridge,
  f as applyCodexDynamicToolProfile,
  l as buildCodexPluginThreadConfigInputFingerprint,
  n as buildDeveloperInstructions,
  o as codexDynamicToolsFingerprint,
  p as normalizeCodexDynamicToolName,
  s as startOrResumeThread,
  t as areCodexDynamicToolFingerprintsCompatible,
  u as shouldBuildCodexPluginThreadConfig,
} from "./thread-lifecycle-d7y0Zs8j.js";
...
async function waitForPluginApprovalDecision(params) {
  const waitPromise = callGatewayTool(
    ...
    { timeoutMs: DEFAULT_CODEX_APPROVAL_TIMEOUT_MS + 1e4 },
...
const ANSI_OSC_SEQUENCE_RE$1 = new RegExp(
  String.raw`(?:\u001b]|\u009d)[^\u001b\u009c\u0007]*(?:\u0007|\u001b\\|\u009c)`,
  "g",
);
const ANSI_CONTROL_SEQUENCE_RE$1 = new RegExp(
  String.raw`(?:\u001b\[[0-?]*[ -/]*[@-~]|\u009b[0-?]*[ -/]*[@-~]|\u001b[@-Z\\-_])`,
  "g",
);
const CONTROL_CHARACTER_RE$1 = new RegExp(String.raw`[\u0000-\u001f\u007f-\u009f]+`, "g");
const INVISIBLE_FORMATTING_CONTROL_RE$1 = new RegExp(
  String.raw`[\u00ad\u034f\u061c\u200b-\u200f\u202a-\u202e\u2060-\u206f\ufeff\ufe00-\ufe0f\u{e0100}-\u{e01ef}]`,
  "gu",
);
const DANGLING_TERMINAL_SEQUENCE_SUFFIX_RE$1 = new RegExp(
  String.raw`(?:\u001b\][^\u001b\u009c\u0007]*|\u009d[^\u001b\u009c\u0007]*|\u001b\[[0-?]*[ -/]*|\u009b[0-?]*[ -/]*|\u001b)$`,
);
async function handleCodexAppServerApprovalRequest(params) {
  ...
  try {
    ...
    const outcome = mapExecDecisionToOutcome(
      approvalRequestExplicitlyUnavailable(requestResult)
        ? null
    ...
    emitApprovalEvent(params.paramsForRun, {
      ...
      message: cancelled
        ...
        : `Codex app-server approval route failed: ${formatCodexDisplayText(formatErrorMessage(error))}`,
    });
...
function matchesCurrentTurn(requestParams, threadId, turnId) {
  ...
  const requestThreadId =
    readString$4(requestParams, "threadId") ?? readString$4(requestParams, "conversationId");
...
function buildApprovalContext(params) {
  const itemId =
    readString$4(params.requestParams, "itemId") ??
    readString$4(params.requestParams, "callId") ??
    readString$4(params.requestParams, "approvalId");
...
function sanitizePermissionHostValue(value) {
  const withoutScheme = sanitizePermissionScalar(value)
    .toLowerCase()
    .replace(/^[a-z][a-z0-9+.-]*:\/\//, "");
...
function sanitizeApprovalPreview(source, maxLength) {
  ...
  const sanitized = sanitizeVisibleScalar(
    source.value.replace(DANGLING_TERMINAL_SEQUENCE_SUFFIX_RE$1, ""),
  );
...
function sanitizeVisibleScalar(value) {
  return value
    .replace(ANSI_OSC_SEQUENCE_RE$1, "")
    .replace(ANSI_CONTROL_SEQUENCE_RE$1, "")
    .replace(INVISIBLE_FORMATTING_CONTROL_RE$1, " ")
    .replace(CONTROL_CHARACTER_RE$1, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function projectContextEngineAssemblyForCodex(params) {
  ...
  const renderedContext = renderMessagesForCodexContext(
    dropDuplicateTrailingPrompt(params.assembledMessages, prompt),
  );
...
function renderMessagePart(part) {
  ...
  if (type === "toolResult" || type === "tool_result")
    return `${typeof record.toolUseId === "string" ? `tool result: ${record.toolUseId}` : "tool result"} [content omitted]`;
...
function buildPluginPolicyElicitationResponse(entry, requestParams) {
  ...
  if (
    readString$3(requestParams, "mode") !== "form" ||
...
function readBridgeableApprovalElicitation(requestParams) {
  ...
  if (
    readString$3(requestedSchema, "type") !== "object" ||
    !isJsonObject(requestedSchema.properties)
  )
    return;
  const title =
    sanitizeDisplayText(readString$3(requestParams, "message") ?? "") || "Codex MCP tool approval";
  return {
    title,
...
function buildApprovalDescription(params) {
  const connectorName = sanitizeOptionalDisplayText(
    readString$3(params.meta, MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY),
  );
  const toolTitle = sanitizeOptionalDisplayText(
    readString$3(params.meta, MCP_TOOL_APPROVAL_TOOL_TITLE_KEY),
  );
  const toolDescription = sanitizeOptionalDisplayText(
    readString$3(params.meta, MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY),
  );
  const summaryLines = [
...
function readPropertyDescriptionLines(requestedSchema) {
  ...
  return Object.entries(properties)
    .map(([name, value]) => {
      ...
      if (!schema) return;
      const propTitle =
        sanitizeDisplayText(readString$3(schema, "title") ?? "") ||
...
function readDisplayParamLines(meta) {
  ...
  const lines = displayParams
    ...
    .map((entry) => {
      ...
      if (!param) return;
      const name =
        sanitizeOptionalDisplayText(readString$3(param, "display_name")) ??
        sanitizeOptionalDisplayText(readString$3(param, "name"));
      if (!name) return;
...
function formatDisplayParamValue(value) {
  return truncateDisplayText(
    sanitizeDisplayText(typeof value === "string" ? value : formatDisplayJsonValue(value ?? null)),
    MAX_DISPLAY_PARAM_VALUE_LENGTH,
  );
}
...
function buildAcceptedContent(requestedSchema, meta, outcome) {
  ...
  const required = Array.isArray(requestedSchema.required)
    ? new Set(requestedSchema.required.filter((entry) => typeof entry === "string"))
    : /* @__PURE__ */ new Set();
  const content = {};
...
function readPersistFieldValue(property, meta, outcome) {
  ...
  if (preferred)
    return options.find((option) => option.value === preferred || option.label === preferred)
      ?.value;
}
...
function readEnumOptions(schema) {
  if (Array.isArray(schema.enum)) {
    ...
    const labels = Array.isArray(schema.enumNames)
      ? schema.enumNames.filter((entry) => typeof entry === "string")
      : [];
...
var CodexAppServerEventProjector = class {
  ...
  buildResult(toolTelemetry, options) {
    ...
    const lastAssistant =
      assistantTexts.length > 0 ? this.createAssistantMessage(assistantTexts.join("\n\n")) : void 0;
  ...
  handleTokenUsage(params) {
    ...
    const current =
      (tokenUsage ? readFirstJsonObject(tokenUsage, CURRENT_TOKEN_USAGE_KEYS) : void 0) ??
  ...
  handleRawResponseItemCompleted(params) {
    ...
    if (!text) return;
    const itemId =
      readString$2(item, "id") ?? `raw-assistant-${this.assistantItemOrder.length + 1}`;
    this.rememberAssistantItem(itemId);
    this.assistantTextByItem.set(itemId, text);
  }
...
function normalizeCodexTokenUsage(record) {
  ...
  return normalizeUsage({
    input:
      promptTotalInput !== void 0 && cacheRead !== void 0
...
function itemOutputText(item) {
  ...
  if (item.type === "dynamicToolCall")
    return collectDynamicToolContentText(item.contentItems).trim() || void 0;
  if (item.type === "mcpToolCall") {
...
function resolveTrajectoryPointerFilePath(sessionFile) {
  return sessionFile.endsWith(".jsonl")
    ? `${sessionFile.slice(0, -6)}.trajectory-path.json`
...
function resolveTrajectoryFilePath(params) {
  ...
  return params.sessionFile.endsWith(".jsonl")
    ? `${params.sessionFile.slice(0, -6)}.trajectory.jsonl`
...
function createCodexUserInputBridge(params) {
  ...
  return {
    ...
    handleNotification(notification) {
      ...
      if (
        notificationParams &&
        readString$1(notificationParams, "threadId") === pending.threadId &&
        requestId !== void 0 &&
...
function formatUserInputPrompt(questions) {
  ...
  questions.forEach((question, index) => {
    ...
    question.options?.forEach((option, optionIndex) => {
      lines.push(
        `${optionIndex + 1}. ${formatCodexDisplayText(option.label)}${option.description ? ` - ${formatCodexDisplayText(option.description)}` : ""}`,
      );
    });
...
function formatDynamicToolTimeoutDetails(params) {
  ...
  const requestedPart =
    requestedTimeoutMs === void 0 ? "" : ` requestedWaitMs=${requestedTimeoutMs}`;
  const retryHint =
    action === "poll"
...
async function runCodexAppServerAttempt(params, options = {}) {
  ...
  const startupEnvApiKeyCacheKey = startupAuthProfileId
    ? void 0
    : resolveCodexAppServerEnvApiKeyCacheKey({ startOptions: appServer.start });
  ...
  if (activeContextEngine)
    try {
      ...
      log.warn("context engine assemble failed; using Codex baseline prompt", {
        error: formatErrorMessage$1(assembleErr),
      });
    }
  else if (
  ...
  try {
    ...
    const threadConfig = nativeHookRelay
...
async function handleDynamicToolCallWithTimeout(params) {
  ...
  try {
    ...
    if (!timedOut && !controller.signal.aborted)
      controller.abort(/* @__PURE__ */ new Error("OpenClaw dynamic tool call finished."));
  }
}
...
async function buildDynamicTools(input) {
  ...
  const createOpenClawCodingTools =
    ...
    (await import("./plugin-sdk/agent-harness.js")).createOpenClawCodingTools;
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        ...
        const abortListener = () =>
          rejectOnce(/* @__PURE__ */ new Error("codex app-server startup aborted"));
        params.signal.addEventListener("abort", abortListener, { once: true });
...
async function readMirroredSessionHistoryMessages(sessionFile) {
  ...
  if (!messages)
    log.warn("failed to read mirrored session history for codex harness hooks", { sessionFile });
  return messages;
}
