...
async function listChannelSummaryPlugins(params) {
  const { listReadOnlyChannelPluginsForConfig } = await import("./read-only-D50h7rfC.js");
...
async function buildChannelSummary(cfg, options) {
  ...
  const plugins =
    options?.plugins ??
    (await listChannelSummaryPlugins({
      cfg: effective,
      sourceConfig,
    }));
  for (const plugin of plugins) {
    ...
    const summaryRecord = plugin.status?.buildChannelSummary
