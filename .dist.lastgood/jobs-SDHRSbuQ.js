...
import {
  i as normalizeRequiredName,
  n as normalizeOptionalAgentId,
  r as normalizePayloadToSystemText,
} from "./normalize-y9XtuoNT.js";
import {
  n as computeNextRunAtMs,
  r as computePreviousRunAtMs,
  t as coerceFiniteScheduleNumber,
} from "./schedule-BabEAu9_.js";
import { c as normalizeAgentId } from "./session-key-B4qUwRzq.js";
import {
  i as parseAbsoluteTimeMs,
  n as resolveCronStaggerMs,
  r as resolveDefaultCronStaggerMs,
  t as normalizeCronStaggerMs,
} from "./stagger-D59LMhh_.js";
import {
  c as normalizeOptionalString,
  u as normalizeOptionalThreadValue,
} from "./string-coerce-BdEutqX5.js";
import {
  n as assertSafeCronSessionTargetId,
...
const DEFAULT_ERROR_BACKOFF_SCHEDULE_MS = [3e4, 6e4, 5 * 6e4, 15 * 6e4, 60 * 6e4];
...
function assertMainSessionAgentId(job, defaultAgentId) {
  ...
  if (normalizeAgentId(job.agentId) !== normalizeAgentId(defaultAgentId))
    throw new Error(
...
function computeJobNextRunAtMs(job, nowMs) {
  ...
  if (next === void 0 && job.schedule.kind === "cron")
    return computeStaggeredCronNextRunAtMs(job, Math.floor(nowMs / 1e3) * 1e3 + 1e3);
  return isFiniteTimestamp(next) ? next : void 0;
}
...
function nextWakeAtMs(state) {
  const enabled = (state.store?.jobs ?? []).filter(
    (j) => j.enabled && hasScheduledNextRunAtMs(j.state.nextRunAtMs),
  );
...
function mergeCronDelivery(existing, patch) {
  ...
  if ("failureDestination" in patch)
    if (patch.failureDestination === void 0) next.failureDestination = void 0;
    else {
...
function mergeCronFailureAlert(existing, patch) {
  ...
  if ("includeSkipped" in patch)
    next.includeSkipped = typeof patch.includeSkipped === "boolean" ? patch.includeSkipped : void 0;
  if ("mode" in patch) {
...
//#endregion
export {
  computeJobPreviousRunAtMs as a,
  findJobOrThrow as c,
  isJobEnabled as d,
  nextWakeAtMs as f,
  resolveJobPayloadTextForMain as g,
  recordScheduleComputeError as h,
  computeJobNextRunAtMs as i,
  hasScheduledNextRunAtMs as l,
  recomputeNextRunsForMaintenance as m,
  applyJobPatch as n,
  createJob as o,
  recomputeNextRuns as p,
  assertSupportedJobSpec as r,
  errorBackoffMs as s,
  DEFAULT_ERROR_BACKOFF_SCHEDULE_MS as t,
  isJobDue as u,
};
