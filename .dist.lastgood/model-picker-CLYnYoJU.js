...
import {
  n as createProviderAuthChecker,
  t as resolveVisibleModelCatalog,
} from "./model-catalog-visibility-BHnTVP9l.js";
import {
  n as resolveAgentModelFallbackValues,
  r as resolveAgentModelPrimaryValue,
  t as normalizeAgentModelRefForConfig,
} from "./model-input-C-vWoAxX.js";
import {
  n as isModelPickerVisibleProvider,
  t as isModelPickerVisibleModelRef,
} from "./model-picker-visibility-DsRoNWpN.js";
import "./model-selection-CyVXHdEG.js";
import { t as formatLiteralProviderPrefixedModelRef } from "./model-ref-shared-C2I7TGOl.js";
import {
  _ as modelKey,
...
import { i as formatTokenK } from "./shared-BbHHdmwP.js";
  ...
  const model = (
    (await params.prompter.text({
      ...
      validate: params.allowBlank
        ? void 0
        : (value) => (normalizeOptionalString(value) ? void 0 : "Required"),
    })) ?? ""
  ).trim();
  if (!model) return {};
...
function buildModelProviderFilterOptions(models) {
  return Array.from(new Set(models.map((entry) => entry.provider)))
...
async function promptDefaultModel(params) {
  ...
  const resolveConfiguredDisplayLabel = async () => {
    ...
    return (await resolveCachedLiteralPrefixProviders()).has(providerId)
    ...
    const options = [
      {
        ...
        hint:
          configuredRaw && configuredRaw !== resolvedKey ? `resolves to ${resolvedKey}` : void 0,
      },
    ];
    if (includeManual)
      options.push({
...
function applyModelFallbacksFromSelection(cfg, selection, opts = {}) {
  ...
  const normalizedExistingPrimary =
    existingPrimary != null ? normalizeAgentModelRefForConfig(existingPrimary) : void 0;
  ...
  const fallbacks = mergeFallbackSelection({
    ...
    preserveExistingFallback: scopeKeySet
      ? (fallback) => !scopeKeySet.has(fallback)
      : (fallback) => !isModelPickerVisibleModelRef(fallback),
  });
  const nextModel = {
...
function mergeFallbackSelection(params) {
  ...
  for (const fallback of params.selectedFallbacks)
    if (selected.has(fallback)) fallbacks.push(fallback);
  return fallbacks;
}
//#endregion
export {
  promptModelAllowlist as i,
  applyModelFallbacksFromSelection as n,
  promptDefaultModel as r,
  applyModelAllowlist as t,
};
