...
import {
  A as ChannelSelectMenu,
  B as StringSelectMenu,
  D as RadioGroup,
  E as Modal,
  F as MentionableSelectMenu,
  G as parseCustomId,
  Gt as TextInputStyle,
  H as Thumbnail,
  I as RoleSelectMenu,
  L as Row,
  M as File,
...
function parseSelectSpec(raw, label) {
  ...
  const allowedTypes = ["string", "user", "role", "mentionable", "channel"];
...
function parseModalField(raw, label, index) {
  ...
  const supported = ["text", "checkbox", "radio", "select", "role-select", "user-select"];
...
function parseComponentBlock(raw, label) {
  const obj = requireObject(raw, label);
  switch (
    normalizeBlockType(normalizeLowercaseStringOrEmpty(readString(obj.type, `${label}.type`)))
  ) {
    case "text":
      return {
        type: "text",
...
//#endregion
export {
  buildDiscordComponentMessageFlags as a,
  resolveDiscordComponentAttachmentName as c,
  buildDiscordComponentCustomId as d,
  buildDiscordModalCustomId as f,
  parseDiscordModalCustomIdForInteraction as g,
  parseDiscordModalCustomId as h,
  buildDiscordComponentMessage as i,
  DISCORD_COMPONENT_CUSTOM_ID_KEY as l,
  parseDiscordComponentCustomIdForInteraction as m,
  DiscordFormModal as n,
  DISCORD_COMPONENT_ATTACHMENT_PREFIX as o,
  parseDiscordComponentCustomId as p,
  createDiscordFormModal as r,
  readDiscordComponentSpec as s,
  formatDiscordComponentEventText as t,
  DISCORD_MODAL_CUSTOM_ID_KEY as u,
};
