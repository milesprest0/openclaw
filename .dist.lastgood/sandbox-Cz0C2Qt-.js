...
import {
  C as buildPinnedMkdirpPlan,
  D as dockerSandboxBackendManager,
  E as buildPinnedWritePlan,
  O as runDockerSandboxShellCommand,
  T as buildPinnedRenamePlan,
  a as getSandboxBackendManager,
  n as startBrowserBridgeServer,
  r as stopBrowserBridgeServer,
  s as requireSandboxBackendFactory,
  t as BROWSER_BRIDGES,
  v as isPathInsideContainerRoot,
  w as buildPinnedRemovePlan,
  y as normalizeContainerPath,
} from "./browser-bridges-JLeYbMLd.js";
import "./boundary-file-read-CkGowAmx.js";
import {
  a as DEFAULT_OPENCLAW_BROWSER_COLOR,
  d as ensureBrowserControlAuth,
  f as resolveBrowserControlAuth,
  l as resolveBrowserConfig,
  n as DEFAULT_BROWSER_ACTION_TIMEOUT_MS,
  s as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME,
  u as resolveProfile,
} from "./browser-profiles-mCnw8ifV.js";
import {
  i as resolveSandboxConfigForAgent,
  r as resolveSandboxBrowserDockerCreateConfig,
} from "./config-aLdl9MiY.js";
import {
  _ as SANDBOX_AGENT_WORKSPACE_MOUNT,
  b as SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
} from "./constants-DcbmEUTo.js";
import "./config-CMOdcWpc.js";
import {
  C as readRegistry,
  D as updateRegistry,
  E as updateBrowserRegistry,
  O as computeSandboxBrowserConfigHash,
  S as readBrowserRegistry,
  T as removeRegistryEntry,
  _ as resolveSandboxScopeKey,
  a as execDocker,
  c as isDockerDaemonUnavailable,
  d as readDockerNetworkDriver,
  f as readDockerNetworkGateway,
  g as resolveSandboxAgentId,
  h as appendWorkspaceMountArgs,
  ...
  v as resolveSandboxWorkspaceDir,
  w as removeBrowserRegistryEntry,
  y as slugifySessionKey,
} from "./docker-ug7lsjE-.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import {
  c as splitSandboxBindSpec,
  s as resolveSandboxHostPathViaExistingAncestor,
} from "./network-mode-CAACLKz5.js";
...
import { a as resolveSandboxPath, i as resolveSandboxInputPath } from "./sandbox-paths-BVIaOwJU.js";
import { f as isSameSsrFPolicy } from "./ssrf-DO8eIXaD.js";
import {
  c as normalizeOptionalString,
  s as normalizeOptionalLowercaseString,
} from "./string-coerce-BdEutqX5.js";
import { n as isToolAllowed } from "./tool-policy-DpDV9E2l.js";
import { p as resolveUserPath } from "./utils-BGRcpLKt.js";
import { n as validateNetworkMode } from "./validate-sandbox-security-B91sDgd8.js";
...
function pruneExpiredNoVncObserverTokens(now) {
  for (const [token, entry] of NO_VNC_OBSERVER_TOKENS)
    if (entry.expiresAt <= now) NO_VNC_OBSERVER_TOKENS.delete(token);
}
...
function generateNoVncPassword() {
  ...
  for (let i = 0; i < NOVNC_PASSWORD_LENGTH; i += 1)
    out += NOVNC_PASSWORD_ALPHABET[crypto.randomInt(0, 62)];
  return out;
}
...
async function ensureDockerNetwork(network, opts) {
  ...
  if ((await execDocker(["network", "inspect", network], { allowFailure: true })).code === 0)
    return;
...
async function ensureSandboxBrowser(params) {
  ...
  if (hasContainer) {
    if (noVncEnabled)
      noVncPassword =
        (await readDockerContainerEnvVar(containerName, "OPENCLAW_BROWSER_NOVNC_PASSWORD")) ??
        void 0;
    const registryEntry = (await readBrowserRegistry()).entries.find(
      ...
      if (
        ...
        const hint = (() => {
          ...
          if (params.cfg.scope === "agent")
            return `openclaw sandbox recreate --browser --agent ${resolveSandboxAgentId(params.scopeKey) ?? "main"}`;
  ...
  if (!hasContainer) {
    ...
    if (!effectiveCdpSourceRange)
      throw new Error(
        `Cannot derive CDP source range for sandbox browser on network "${browserDockerCfg.network}". Set agents.defaults.sandbox.browser.cdpSourceRange explicitly.`,
      );
    ...
    if (browserDockerCfg.binds?.length)
      for (const bind of browserDockerCfg.binds) args.push("-v", bind);
  ...
  if (noVncEnabled && !noVncPassword)
    noVncPassword =
      (await readDockerContainerEnvVar(containerName, "OPENCLAW_BROWSER_NOVNC_PASSWORD")) ?? void 0;
  ...
  async assertGuardedPathSafety(target, options, guarded) {
    if (!guarded.ok) {
      if (guarded.reason !== "path") {
        if (!(options.allowedType === "directory" && this.pathIsExistingDirectory(target.hostPath)))
...
function findMountByContainerPath(mounts, target) {
  for (const mount of mounts)
    if (isPathInsideContainerRoot(mount.containerRoot, target)) return mount;
  return null;
}
...
function isPathInsideHost(root, target) {
  ...
  return isPathInside(
    canonicalRoot,
    path.resolve(canonicalTargetParent, path.basename(resolvedTarget)),
  );
}
...
async function resolveSandboxContext(params) {
  ...
  if (cfg.prune.idleHours !== 0 || cfg.prune.maxAgeDays !== 0)
    await (await import("./prune-DWQ7Ldut.js")).maybePruneSandboxes(cfg);
...
//#endregion
export {
  ensureSandboxWorkspaceForSession as a,
  removeSandboxContainer as i,
  listSandboxContainers as n,
  resolveSandboxContext as o,
