...
function migrateMemoryWikiLegacyConfig(config) {
  const rawBridge = asRecord(
    asRecord(asRecord(config.plugins?.entries?.["memory-wiki"])?.config)?.bridge,
  );
...
function normalizeCompatibilityConfig({ cfg }) {
  return (
    migrateMemoryWikiLegacyConfig(cfg) ?? {
      config: cfg,
      changes: [],
    }
  );
...
//#endregion
export {
  migrateMemoryWikiLegacyConfig as n,
  normalizeCompatibilityConfig as r,
  legacyConfigRules as t,
};
