...
async function collectMcpSources(source) {
  ...
  if (source.projectDir) {
    const projectScopedServers = mapMcpServers(
      childRecord(childRecord(claudeJson, "projects"), source.projectDir).mcpServers,
    );
...
async function buildConfigItems(params) {
  ...
  for (const settingsPath of [
    ...
    if (settingsPath && settings.hooks !== void 0)
      items.push(
    ...
    if (settingsPath && settings.permissions !== void 0)
      items.push(
        createMigrationManualItem({
    ...
    if (settingsPath && settings.env !== void 0)
      items.push(
        createMigrationManualItem({
