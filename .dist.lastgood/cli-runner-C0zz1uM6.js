...
import { p as isFailoverErrorMessage, t as classifyFailoverReason } from "./errors-D66M8xLa.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import "./tokens-DWz8lWRf.js";
import {
...
import {
  i as buildAgentHookConversationMessages,
  ...
  try {
    ...
    if (params.cleanupCliLiveSessionOnRunEnd === true) {
      const { closeClaudeLiveSessionForContext } =
        await import("./claude-live-session-CeRPd_bL.js");
...
async function runPreparedCliAgent(context) {
  ...
  const buildAgentEndMessages = (lastAssistant) => [
  ...
  const persistBlockedBeforeAgentRun = async (block) => {
    try {
      ...
      log.warn(
        `before_agent_run block: failed to persist redacted CLI user message: ${formatErrorMessage(err)}`,
      );
    }
  };
