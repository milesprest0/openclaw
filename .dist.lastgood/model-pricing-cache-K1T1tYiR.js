...
import { w as planManifestModelCatalogRows } from "./discovery-bnS95tO3.js";
import { d as isInstalledPluginEnabled } from "./installed-plugin-index-store-YgBbP4mp.js";
...
import { t as resolvePluginWebSearchConfig } from "./plugin-web-search-config-DS7x0yMy.js";
...
function getGatewayModelPricingCacheFingerprint() {
  return stablePricingValue(
    Array.from(cachedPricing.entries()).toSorted(([a], [b]) => a.localeCompare(b)),
  );
}
//#endregion
...
function normalizeExternalPricingPolicy(value, options) {
  ...
  return {
    ...
    ...(normalizeExternalPricingSource(value.openRouter, options) !== void 0
      ? { openRouter: normalizeExternalPricingSource(value.openRouter, options) }
      : {}),
    ...(normalizeExternalPricingSource(value.liteLLM, options) !== void 0
...
function hasPrivateOrLoopbackConfiguredEndpoint(
  ...
  return (
    isPrivateOrLoopbackBaseUrl(findConfiguredProviderModel(config, ref, options)?.baseUrl) ||
...
async function refreshGatewayModelPricingCache(params) {
  ...
  inFlightRefresh = (async () => {
    ...
    const [catalogById, litellmCatalog] = await Promise.all([
      fetchOpenRouterPricingCatalog(fetchImpl, params.signal).catch((error) => {
...
//#endregion
export {
  getGatewayModelPricingCacheFingerprint as a,
  getCachedGatewayModelPricing as i,
  refreshGatewayModelPricingCache as n,
  startGatewayModelPricingRefresh as r,
  collectConfiguredModelPricingRefs as t,
};
