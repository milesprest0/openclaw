import { p as shouldMigrateStateFromPath } from "./argv-DQxa_OA3.js";
...
async function ensureConfigReady(params) {
  ...
  if (!didRunDoctorConfigFlow && shouldMigrateStateFromPath(commandPath)) {
    ...
    const runDoctorConfigPreflight = async () =>
      (await import("./doctor-config-preflight-Cqyd5jFt.js")).runDoctorConfigPreflight({
        migrateState: false,
  ...
  const isBareGatewayForegroundRun =
    commandName === "gateway" && (subcommandName === void 0 || subcommandName.trim() === "");
  const allowInvalid = commandName
    ...
    : false;
  const { formatConfigIssueLines } = await import("./issue-format-DFdBAuDX.js");
  const issues =
  ...
  const [{ colorize, isRich, theme }, { shortenHomePath }, { formatCliCommand }] =
    await Promise.all([
      import("./theme-CFs3QDvj.js"),
      import("./utils-CG1GL-dA.js"),
      import("./command-format-DAIzgsCb.js"),
    ]);
  const rich = isRich();
