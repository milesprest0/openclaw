...
import { t as splitArgsPreservingQuotes } from "./arg-split-BoGZh07Y.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import { i as runExec, r as runCommandWithTimeout } from "./exec-Csn_09g3.js";
import { i as normalizeEnvVarKey } from "./host-env-security-W4c-l4Zm.js";
import {
...
import {
  f as resolveGatewayServiceDescription,
  n as resolveHomeDir,
  o as LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES,
  p as resolveGatewaySystemdServiceName,
} from "./paths-D-SPS6K1.js";
import {
  i as writeFormattedLines,
  n as formatLine,
  r as toPosixPath,
  t as parseKeyValueOutput,
} from "./runtime-parse-B8z31bXs.js";
import { r as readStateDirDotEnvVarsFromStateDir } from "./state-dir-dotenv-DCxom2SY.js";
import {
...
import {
  n as parseSystemdEnvAssignment,
  r as parseSystemdExecStart,
  t as buildSystemdUnit,
} from "./systemd-unit-Bm5agCGa.js";
...
function readEnvironmentValueSource(command, normalizedKey) {
  for (const [rawKey, source] of Object.entries(command?.environmentValueSources ?? {}))
    if (normalizeServiceEnvKey(rawKey) === normalizedKey) return source;
}
...
function readSystemdEnvironmentValueSource(params) {
  ...
  for (const [rawKey, source] of Object.entries(params.environmentValueSources ?? {}))
    if (normalizeSystemdEnvironmentKey(rawKey) === normalizedKey) return source;
}
...
function resolveSystemctlUserScope(env) {
  ...
  const isSudoToRoot =
    (effectiveUid === null ? effectiveUser === "root" : effectiveUid === 0) &&
    isNonRootUser(sudoUser);
  return {
    machineUser: isSudoToRoot
      ? sudoUser
...
async function isSystemdUnitActive(env, unitName, scope = "user") {
  ...
  return (
    (scope === "system" ? await execSystemctl(args) : await execSystemctlUser(env, args)).code === 0
  );
}
...
async function uninstallSystemdService({ env, stdout }) {
  await assertSystemdAvailable(env);
  await execSystemctlUser(env, ["disable", "--now", `${resolveSystemdServiceName(env)}.service`]);
  const unitPath = resolveSystemdUnitPath(env);
  try {
...
async function findLegacySystemdUnits(env) {
  ...
  for (const name of LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES) {
    ...
    if (systemctlAvailable)
      enabled = (await execSystemctlUser(env, ["is-enabled", `${name}.service`])).code === 0;
    if (exists || enabled)
      results.push({
        name,
        unitPath,
...
//#endregion
export {
  writeManagedServiceEnvKeysToEnvironment as C,
  readManagedServiceEnvKeysFromEnvironment as S,
  execFileUtf8 as _,
  isSystemdUserServiceAvailable as a,
  hasInlineEnvironmentSource as b,
  resolveSystemdUserUnitPath as c,
  stopSystemdService as d,
  uninstallLegacySystemdUnits as f,
  readSystemdUserLingerStatus as g,
  enableSystemdUserLinger as h,
  isSystemdUnitActive as i,
  restartSystemdService as l,
  classifySystemdUnavailableDetail as m,
  isNonFatalSystemdInstallProbeError as n,
  readSystemdServiceExecStart as o,
  uninstallSystemdService as p,
  isSystemdServiceEnabled as r,
  readSystemdServiceRuntime as s,
  installSystemdService as t,
  stageSystemdService as u,
  collectInlineManagedServiceEnvKeys as v,
  isEnvironmentFileOnlySource as x,
  formatManagedServiceEnvKeys as y,
};
