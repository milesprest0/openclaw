...
import {
  ...
  s as init_tslib_es6,
  t as __awaiter,
} from "./tslib.es6-DPnFEwiE.js";
import { i as normalizeToken, o as resolveTwitchToken } from "./twitch-CB__p3Ax.js";
...
function splitWithLimit(str, delim, count) {
  ...
  return __spreadArray(
    __spreadArray([], __read(parts.slice(0, count - 1)), false),
    [parts.slice(count - 1).join(delim)],
    false,
  );
}
//#endregion
...
function getExpiryMillis(token) {
  return mapNullable(
    token.expiresIn,
    (_) => token.obtainmentTimestamp + _ * 1e3 - EXPIRY_GRACE_PERIOD,
  );
}
/**
 ...
 */
async function callTwitchApiRaw(
  options,
  clientId,
  accessToken,
  authorizationType,
  ...
  if (accessToken)
    headers.append(
      "Authorization",
      `${type === "helix" ? (authorizationType ?? "Bearer") : "OAuth"} ${accessToken}`,
    );
...
async function callTwitchApi(options, clientId, accessToken, authorizationType, fetchOptions = {}) {
  const response = await callTwitchApiRaw(
    options,
    clientId,
    accessToken,
    authorizationType,
...
let TokenInfo = class TokenInfo extends DataObject {
  ...
  get expiryDate() {
    return mapNullable(
      this[rawDataSymbol].expires_in,
      (v) => new Date(this._obtainmentDate.getTime() + v * 1e3),
    );
  }
};
...
async function loadAndCompareTokenInfo(clientId, token, userId, loadedScopes, requestedScopeSets) {
  if (requestedScopeSets?.length || !userId) {
    ...
    if (requestedScopeSets)
      compareScopeSets(
        scopesToCompare,
        requestedScopeSets.filter((val) => Boolean(val)),
      );
...
var EventEmitter = class {
  ...
  emit(event, ...args) {
    if (this._eventListeners.has(event))
      for (const listener of this._eventListeners.get(event)) listener(...args);
    if (this._internalEventListeners.has(event))
      for (const listener of this._internalEventListeners.get(event)) listener(...args);
  }
...
let RefreshingAuthProvider = class RefreshingAuthProvider extends EventEmitter {
  ...
  async addUserForCode(code, intents) {
    if (!this._redirectUri)
      throw new Error(
        "This method requires you to pass a `redirectUri` as a configuration property",
      );
  ...
  async refreshAccessTokenForUser(user) {
    ...
    const tokenData = await this._refreshUserTokenWithCallback(
      userId,
      previousTokenData.refreshToken,
    );
      ...
      for (const scopes of scopeSets)
        if (this._appImpliedScopes.length) {
          if (scopes.every((scope) => !this._appImpliedScopes.includes(scope)))
            throw new Error(
              `One of the scopes ${scopes.join(", ")} requested but only the scope ${this._appImpliedScopes.join(", ")} is implied`,
            );
        } else
          throw new Error(
            `One of the scopes ${scopes.join(", ")} requested but the client credentials flow does not support scopes`,
          );
...
var debugFunction = import_detect_node.isNode
  ? console.log.bind(console)
  : console.debug.bind(console);
var LogLevelToConsoleFunction =
  ((_a$2 = {}),
  (_a$2[LogLevel.CRITICAL] = console.error.bind(console)),
  (_a$2[LogLevel.ERROR] = console.error.bind(console)),
  (_a$2[LogLevel.WARNING] = console.warn.bind(console)),
  (_a$2[LogLevel.INFO] = console.info.bind(console)),
  (_a$2[LogLevel.DEBUG] = debugFunction.bind(console)),
  (_a$2[LogLevel.TRACE] = console.trace.bind(console)),
  _a$2);
//#endregion
...
var data =
  typeof process === "undefined"
    ? []
    : (_b$1 =
          (_a$1 = process.env.LOGGING) === null || _a$1 === void 0
            ? void 0
            : _a$1
                .split(";")
                .map(function (part) {
                  ...
                  if (strLevel)
                    return [
                      namespace === "default" ? void 0 : namespace.split(":"),
                      resolveLogLevel(strLevel),
                    ];
                  return null;
                })
                .filter(function (v) {
                ...
                .sort(function (_a, _b) {
                  ...
                  var b = _b[0];
                  return (
                    ((_c = b === null || b === void 0 ? void 0 : b.length) !== null && _c !== void 0
                      ? _c
                      : 0) -
                    ((_d = a === null || a === void 0 ? void 0 : a.length) !== null && _d !== void 0
                      ? _d
                      : 0)
                  );
                })) !== null && _b$1 !== void 0
...
var BaseLogger = (function () {
  function BaseLogger(_a) {
    var name = _a.name,
      minLevel = _a.minLevel,
      _b = _a.emoji,
      emoji = _b === void 0 ? false : _b,
      colors = _a.colors,
      _c = _a.timestamps,
      timestamps = _c === void 0 ? import_detect_node.isNode : _c;
    var _d, _e;
    this._name = name;
    this._minLevel =
      (_e =
        (_d = mapOptional(minLevel, function (lv) {
          return resolveLogLevel(lv);
        })) !== null && _d !== void 0
          ? _d
          : getMinLogLevelFromEnv(name)) !== null && _e !== void 0
        ? _e
        : LogLevel.WARNING;
    this._emoji = emoji;
    this._colors = colors;
...
var BrowserLogger = (function (_super) {
  ...
  BrowserLogger.prototype.log = function (level, message) {
    ...
    if (this._timestamps)
      formattedMessage = "[".concat(/* @__PURE__ */ new Date().toISOString(), "] ").concat(message);
...
var CustomLoggerWrapper = (function () {
  function CustomLoggerWrapper(_a) {
    ...
    var _b;
    this._minLevel =
      (_b = mapOptional(minLevel, function (lv) {
        return resolveLogLevel(lv);
      })) !== null && _b !== void 0
        ? _b
        : getMinLogLevelFromEnv(name);
...
function createGenericWrapper(color, ending, inner) {
  return function (str) {
    return "\x1B["
      .concat(color, "m")
      .concat(inner ? inner(str) : str, "\x1B[")
      .concat(ending, "m");
  };
}
...
var NodeLogger = (function (_super) {
  ...
  NodeLogger.prototype.log = function (level, message) {
    ...
    var builtMessage = "";
    if (this._timestamps)
      builtMessage += "[".concat(/* @__PURE__ */ new Date().toISOString(), "] ");
    if (this._emoji) {
    ...
    if (
      (_c =
        (_a = this._colors) !== null && _a !== void 0
          ? _a
          : (_b = process.stdout) === null || _b === void 0
            ? void 0
            : _b.isTTY) !== null && _c !== void 0
        ? _c
        : true
    )
      builtMessage += ""
        .concat(LogLevelToBackgroundColor[level](this._name), " ")
        .concat(LogLevelToBackgroundColor[level](LogLevel[level]), " ")
        .concat(LogLevelToColor[level](message));
    else
      builtMessage += "["
        .concat(this._name, ":")
        .concat(LogLevel[level].toLowerCase(), "] ")
        .concat(message);
    logFn(builtMessage);
  };
  return NodeLogger;
...
var CustomError = class extends Error {
  constructor(...params) {
    ...
    (_a = Error.captureStackTrace) === null ||
      _a === void 0 ||
      _a.call(Error, this, new.target.constructor);
  }
  get name() {
    return this.constructor.name;
  }
...
var PartitionedTimeBasedRateLimiter = class {
    ...
    return await new Promise((resolve, reject) => {
      ...
      const reqSpec = {
        ...
        limitReachedBehavior:
          (_a = options === null || options === void 0 ? void 0 : options.limitReachedBehavior) !==
            null && _a !== void 0
            ? _a
            : "enqueue",
      };
      const partitionKey = this._partitionKeyCallback(req);
      const usedFromBucket =
        (_b = this._usedFromBucket.get(partitionKey)) !== null && _b !== void 0 ? _b : 0;
        ...
        switch (reqSpec.limitReachedBehavior) {
          case "enqueue": {
            ...
            if (usedFromBucket + queue.length >= this._bucketSize)
              this._logger.warn(
                `Rate limit of ${this._bucketSize} for ${partitionKey ? `partition ${partitionKey}` : "default partition"} was reached, waiting for ${this._paused ? "the limiter to be unpaused" : "a free bucket entry"}; queue size is ${queue.length}`,
              );
            else
              this._logger.info(
                `Enqueueing request for ${partitionKey ? `partition ${partitionKey}` : "default partition"} because the rate limiter is paused; queue size is ${queue.length}`,
              );
            break;
          }
          case "null":
            ...
            if (this._paused)
              this._logger.info(
                `Returning null for request for ${partitionKey ? `partition ${partitionKey}` : "default partition"} because the rate limiter is paused`,
              );
            else
              this._logger.warn(
                `Rate limit of ${this._bucketSize} for ${partitionKey ? `partition ${partitionKey}` : "default partition"} was reached, dropping request and returning null`,
              );
            break;
          case "throw":
            reqSpec.reject(
              new RateLimitReachedError(
                `Request dropped because ${this._paused ? "the rate limiter is paused" : `the rate limit for ${partitionKey ? `partition ${partitionKey}` : "default partition"} was reached`}`,
              ),
            );
            break;
          default:
  ...
  async _runRequest(reqSpec, partitionKey) {
    ...
    const queue = this._getPartitionedQueue(partitionKey);
    this._logger.debug(
      `doing a request for ${partitionKey ? `partition ${partitionKey}` : "default partition"}, new queue length is ${queue.length}`,
    );
    this._usedFromBucket.set(
      partitionKey,
      ((_a = this._usedFromBucket.get(partitionKey)) !== null && _a !== void 0 ? _a : 0) + 1,
    );
        ...
        switch (reqSpec.limitReachedBehavior) {
          case "enqueue":
            ...
            if (this._usedFromBucket + this._queue.length >= this._bucketSize)
              this._logger.warn(
                `Rate limit of ${this._bucketSize} was reached, waiting for ${this._paused ? "the limiter to be unpaused" : "a free bucket entry"}; queue size is ${this._queue.length}`,
              );
            else
              this._logger.info(
          ...
          case "throw":
            reqSpec.reject(
              new RateLimitReachedError(
                `Request dropped because ${this._paused ? "the rate limiter is paused" : "the rate limit was reached"}`,
              ),
            );
            break;
          default:
...
var AbstractConnection = class extends EventEmitter {
  ...
  assumeExternalDisconnect() {
    var _a;
    (_a = this._logger) === null ||
      _a === void 0 ||
      _a.trace("AbstractConnection assumeExternalDisconnect");
    this._connected = false;
    this._connecting = false;
    this.clearSocket();
...
var DirectConnection = class extends AbstractConnection {
  ...
  connect() {
    ...
    this._socket.on("error", (err) => {
      ...
      (_a = this._logger) === null ||
        _a === void 0 ||
        _a.trace(`DirectConnection onError message:${err.message}`);
      this._connected = false;
      this._connecting = false;
...
var PersistentConnection = class extends EventEmitter {
  ...
  get isConnected() {
    var _a, _b;
    return (_b =
      (_a = this._currentConnection) === null || _a === void 0 ? void 0 : _a.isConnected) !==
      null && _b !== void 0
      ? _b
      : false;
  }
  get isConnecting() {
    var _a, _b;
    return (_b =
      (_a = this._currentConnection) === null || _a === void 0 ? void 0 : _a.isConnecting) !==
      null && _b !== void 0
      ? _b
      : this._connecting;
  }
  get hasSocket() {
    var _a, _b;
    return (_b =
      (_a = this._currentConnection) === null || _a === void 0 ? void 0 : _a.hasSocket) !== null &&
      _b !== void 0
      ? _b
      : false;
  }
  ...
  disconnect() {
    ...
    (_a = this._logger) === null ||
      _a === void 0 ||
      _a.trace(
        `PersistentConnection disconnect currentConnectionExists:${Boolean(this._currentConnection).toString()} connecting:${this._connecting.toString()}`,
      );
    this._connecting = false;
  ...
  assumeExternalDisconnect() {
    ...
    (_a = this._logger) === null ||
      _a === void 0 ||
      _a.trace("PersistentConnection assumeExternalDisconnect");
  ...
  _tryConnect(userGenerated = false) {
    ...
    (_a = this._logger) === null ||
      _a === void 0 ||
      _a.trace(
        `PersistentConnection tryConnect currentConnectionExists:${Boolean(this._currentConnection).toString()} connecting:${this._connecting.toString()}`,
      );
    const retryLimit = userGenerated ? this._initialRetryLimit : this._retryLimit;
    ((_b = this._retryTimerGenerator) !== null && _b !== void 0) ||
    ...
    newConnection.onDisconnect((manually, reason) => {
      ...
      if (manually) {
        ...
        (_a = this._logger) === null ||
          _a === void 0 ||
          _a.debug(
            `Connection error caught: ${(_b = reason === null || reason === void 0 ? void 0 : reason.message) !== null && _b !== void 0 ? _b : "unknown error"}`,
          );
        if (this._connectionRetryCount >= retryLimit) return;
        this._connectionRetryCount++;
        const secs = this._retryTimerGenerator.next().value;
        if (secs !== 0)
          (_c = this._logger) === null || _c === void 0 || _c.info(`Retrying in ${secs} seconds`);
        setTimeout(() => {
          var _a;
          if (!this._connecting) return;
          (_a = this._logger) === null ||
            _a === void 0 ||
            _a.info(userGenerated ? "Retrying connection" : "Trying to reconnect");
          this._tryConnect();
        }, secs * 1e3);
...
var WebSocketConnection = class extends AbstractConnection {
  ...
  connect() {
    ...
    this._connecting = true;
    this._socket = new WebSocket$2(
      this._url,
      (_b = this._additionalOptions) === null || _b === void 0 ? void 0 : _b.wsOptions,
    );
    this._socket.onopen = () => {
      var _a;
    ...
    this._socket.onerror = (e) => {
      ...
      (_a = this._logger) === null ||
        _a === void 0 ||
        _a.trace(`WebSocketConnection onError message:${e.message}`);
    };
...
var NotEnoughParametersError = class NotEnoughParametersError extends Error {
  constructor(_command, _expectedParams, _actualParams) {
    super(
      `command "${_command}" expected ${_expectedParams} or more parameters, got ${_actualParams}`,
    );
...
var ParameterRequirementMismatchError = class ParameterRequirementMismatchError extends Error {
  constructor(_command, _paramName, _paramSpec, _givenValue) {
    var _a;
    super(
      `required parameter "${_paramName}" did not validate against ${(_a = _paramSpec.type) !== null && _a !== void 0 ? _a : "regex"} validation: "${_givenValue}"`,
    );
    this._command = _command;
...
var import_escape_string_regexp = /* @__PURE__ */ __toESM(
...
function isChannel(str, validTypes = "#&") {
  return new RegExp(
    `^[${(0, import_escape_string_regexp.default)(validTypes)}][^ \b\0\n\r,]+$`,
  ).test(str);
}
const ctcpEscapeMap = {
  0: "\0",
      ...
      if (paramName in params) {
        ...
        if (param !== void 0) {
          if (type.checkParam(param, paramSpec, serverProperties))
            ...
            throw new Error(
              `required parameter "${paramName}" did not suit requirements: "${param}"`,
            );
        }
      }
  ...
  if (message._paramSpec)
    for (const key of Object.keys(message._paramSpec))
      Object.defineProperty(message, key, {
        get() {
          var _a, _b;
          return (_b = (_a = this._parsedParams) === null || _a === void 0 ? void 0 : _a[key]) ===
            null || _b === void 0
            ? void 0
            : _b.value;
        },
...
var Message = class Message {
  static checkParam(param, spec, serverProperties = defaultServerProperties) {
    ...
    if (spec.type === "channelList") {
      if (!param.split(",").every((chan) => isChannel(chan, serverProperties.channelTypes)))
        return false;
    }
    if (spec.match) {
  ...
  constructor(
    command,
    { params, tags, prefix, rawLine } = {},
    { serverProperties = defaultServerProperties, isServer = false, shouldParseParams = true } = {},
    paramSpec,
  ) {
    this._params = [];
  ...
  get paramCount() {
    var _a, _b;
    return (_b = (_a = this._params) === null || _a === void 0 ? void 0 : _a.length) !== null &&
      _b !== void 0
      ? _b
      : 0;
  }
  ...
  tagsToString() {
    return [...this._tags.entries()]
      .map(([key, value]) => (value ? `${key}=${escapeTag(value)}` : key))
      .join(";");
  }
  toString(includePrefix = false, fromRawParams = false) {
    const parts = [
      fromRawParams ? this._buildCommandFromRawParams() : this._buildCommandFromNamedParams(),
    ];
    if (includePrefix) {
    ...
    if (this._params) {
      ...
      for (const [paramName, paramSpec] of Object.entries(paramSpecList)) {
        ...
        if (Message.checkParam(param.value, paramSpec)) {
          ...
          throw new ParameterRequirementMismatchError(
            this._command,
            paramName,
            paramSpec,
            param.value,
        ...
        for (const key of Object.keys(this._paramSpec))
          Object.defineProperty(this, key, {
            get() {
              var _a, _b;
              return (_b =
                (_a = this._parsedParams) === null || _a === void 0 ? void 0 : _a[key]) === null ||
                _b === void 0
                ? void 0
                : _b.value;
            },
  ...
  get rawParamValues() {
    var _a, _b;
    return (_b =
      (_a = this._params) === null || _a === void 0 ? void 0 : _a.map((param) => param.value)) !==
      null && _b !== void 0
      ? _b
      : [];
  }
    ...
    return [
      ...
      ...specKeys
        .map((paramName) => {
          var _a;
          const param =
            (_a = this._parsedParams) === null || _a === void 0 ? void 0 : _a[paramName];
  ...
  _buildCommandFromRawParams() {
    ...
    return [
      this._command,
      ...((_b =
        (_a = this._params) === null || _a === void 0
          ? void 0
          : _a.map((param) => `${param.trailing ? ":" : ""}${param.value}`)) !== null &&
      _b !== void 0
        ? _b
        : []),
    ].join(" ");
...
var CapabilityNegotiation = class CapabilityNegotiation extends Message {
  ...
  isResponseTo(originalMessage) {
    ...
    switch (this.subCommand) {
      ...
      case "NAK":
        return (
          originalMessage.subCommand === "REQ" &&
          originalMessage.capabilities === this.capabilities.trim()
        );
      case "LS":
      case "LIST":
...
const all = new Map(
  [...Object.values(Commands_exports), ...Object.values(Numerics_exports)].map((cmd) => [
    cmd.COMMAND,
    cmd,
  ]),
);
    ...
    tags.set(
      ...
      tagValue
        ? tagValue.replace(/\\(.?)/g, (_, match) =>
            Object.prototype.hasOwnProperty.call(tagUnescapeMap, match)
              ? tagUnescapeMap[match]
              : match,
          )
        : "",
    );
...
var IrcClient = class extends EventEmitter {
  constructor(options) {
    ...
    if (reconnect)
      ...
      this._connection = new ConnectionType(
        connectionOptions,
        this._logger,
  ...
  pingCheck() {
    ...
    this._pingTimeoutTimer = setTimeout(() => {
      ...
      if (this._options.connection.reconnect === false)
        this._logger.error(
          `Disconnecting because the last ping took over ${this._pingTimeout} seconds`,
        );
      else
        this._logger.warn(
          `Reconnecting because the last ping took over ${this._pingTimeout} seconds`,
        );
  ...
  onNamedMessage(commandName, handler, handlerName) {
    ...
    if (!handlerName)
      do handlerName = `${commandName}:${padLeft(Math.random() * 1e4, 4, "0")}`;
  ...
  createMessage(type, params, tags) {
    return createMessage(
      ...
      params,
      void 0,
      tags ? new Map(Object.entries(tags)) : void 0,
  ...
  async _negotiateCapabilities(capList) {
    ...
    const capReply = (
      await this.sendMessageAndCaptureReply(CapabilityNegotiation, {
        subCommand: "REQ",
    ...
    if (!(capReply instanceof CapabilityNegotiation))
      throw new Error(
        `capability negotiation failed unexpectedly with "${capReply.command}" command`,
      );
  ...
  _setupConnection() {
    ...
    this._connection.onConnect(async () => {
      var _a, _b;
      this._logger.info(
        `Connection to server ${this._options.connection.hostName}:${this.port} established`,
      );
      this.emit(this.onConnect);
          ...
          this.sendMessageAndCaptureReply(CapabilityNegotiation, {
            ...
            .then(async (capReply) => {
              ...
              const capLists = capReply.map((line) =>
                arrayToObject(line.capabilities.split(" "), (part) => {
                  if (!part) return {};
                  const [cap, param] = splitWithLimit(part, "=", 2);
                  return {
                    [cap]: {
    ...
    this._connection.onDisconnect((manually, reason) => {
      ...
      if (manually) this._logger.info("Disconnected");
        ...
        const willReconnect =
          (_a = this._options.connection.reconnect) !== null && _a !== void 0 ? _a : true;
        const message = reason
          ? `Disconnected unexpectedly: ${reason.message}`
  ...
  _handleReceivedClientNick(me) {
    if (this._currentNick !== me) {
      if (this._currentNick !== "")
        this._logger.warn(
          `Mismatching nicks: passed ${this._currentNick}, but you're actually ${me}`,
        );
  ...
  _startPingCheckTimer() {
    if (this._pingCheckTimer) clearTimeout(this._pingCheckTimer);
    if (this._connection.isConnected)
      this._pingCheckTimer = setTimeout(() => this.pingCheck(), this._pingOnInactivity * 1e3);
...
function splitOnSpaces(text, maxMsgLength) {
  ...
  while (startIndex < text.length) {
    ...
    if (
      ...
      text.length - startIndex + 1 <= maxMsgLength
    )
...
let ChatClient = (ChatClient_1 = class ChatClient extends EventEmitter {
  ...
  constructor(config = {}) {
    ...
    this._ircClient = new IrcClient({
      connection: {
        hostName:
          config.hostName ??
          ((config.webSocket ?? true) ? "irc-ws.chat.twitch.tv" : "irc.chat.twitch.tv"),
          ...
          resolvedChannels.map(async (channel) => {
            try {
              ...
              this._chatLogger.warn(
                `Failed to join configured channel ${channel}; original message: ${e?.message ?? e}`,
              );
            }
          }),
        );
  ...
  static _generateJustinfanNick() {
    return `justinfan${Math.floor(Math.random() * 1e5)
      .toString()
      .padStart(5, "0")}`;
  }
  _initializeRateLimiters() {
    ...
    const executeJoinRequest = async (channel) => {
      ...
      timer = setTimeout(() => {
        ...
        reject(
          /* @__PURE__ */ new Error(
            `Did not receive a reply to join ${channel} in time; assuming that the join failed`,
          ),
        );
      }, 1e4);
...
export {
  ...
  DataObject as O,
  arrayToObject as P,
  HttpStatusCodeError as S,
  HellFreezesOverError as T,
  InvalidTokenError as _,
  removeClientManager as a,
  handleTwitchApiResponseError as b,
  Cacheable as c,
  CustomError as d,
  createLogger as f,
  TokenInfo as g,
  EventEmitter as h,
  getOrCreateClientManager as i,
  promiseWithResolvers as j,
  rawDataSymbol as k,
  PartitionedTimeBasedRateLimiter as l,
  StaticAuthProvider as m,
  stripMarkdownForTwitch as n,
  ChatClient as o,
  require_detect_node as p,
  getClientManager as r,
  CachedGetter as s,
  chunkTextForTwitch as t,
  RateLimitReachedError as u,
  callTwitchApi as v,
  extractUserName as w,
  transformTwitchApiResponse as x,
  callTwitchApiRaw as y,
};
