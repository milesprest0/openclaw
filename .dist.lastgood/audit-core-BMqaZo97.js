import { Ft as ChannelType } from "./discord-Do-Uybag.js";
...
async function auditDiscordChannelPermissionsWithFetcher(params) {
  ...
  for (const channelId of params.channelIds)
    try {
      ...
      const missing = resolveRequiredDiscordChannelPermissions(perms.channelType).filter(
        (p) => !perms.permissions.includes(p),
      );
      channels.push({
        channelId,
        ok: missing.length === 0,
...
//#endregion
export {
  collectDiscordAuditChannelIdsForAccount as n,
  resolveRequiredDiscordChannelPermissions as r,
  auditDiscordChannelPermissionsWithFetcher as t,
};
