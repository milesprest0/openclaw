...
import { r as isVoiceCompatibleAudio } from "./audio-D-EQxAcn.js";
import { n as recordChannelActivity } from "./channel-activity-C42T8R8o.js";
import { t as isDiagnosticFlagEnabled } from "./diagnostic-flags-CQ-HLl8y.js";
import { a as formatErrorMessage, o as formatUncaughtError } from "./errors-DZMrVkYL.js";
import { n as resolveTelegramFetch, o as normalizeTelegramApiRoot } from "./fetch-BsuRcQog.js";
import {
  Q as normalizeTelegramReplyToMessageId,
  a as splitTelegramHtmlChunks,
  b as buildTypingThreadParams,
  c as isSafeToRetrySendError,
  d as isTelegramRateLimitError,
  f as isTelegramServerError,
  i as renderTelegramHtmlText,
  it as parseTelegramTarget,
  nt as normalizeTelegramChatId,
  rt as normalizeTelegramLookupTarget,
  s as isRecoverableTelegramNetworkError,
...
import { d as readConfigFileSnapshotForWrite } from "./io-CEQSCTGy.js";
...
import { s as probeVideoDimensions } from "./media-runtime-Cj72foO4.js";
import { c as kindFromMime, s as isGifMedia } from "./mime-CSQ-Gv-M.js";
...
import { n as createChannelApiRetryRunner } from "./retry-policy-CBO_PUda.js";
import {
  i as saveCronStore,
...
function buildTelegramThreadReplyParams(opts) {
  ...
  const replyQuoteTextRaw =
    normalizeTelegramReplyToMessageId(opts?.replyQuoteMessageId ?? defaultQuoteMessageId) ===
    replyToMessageId
      ? opts?.replyQuoteText
      : void 0;
...
async function maybePersistResolvedTelegramTarget(params) {
  ...
  try {
    ...
    if (
      replaceTelegramDefaultToTargets({
        cfg: nextConfig,
        matchKey,
        resolvedTarget,
      })
...
function resolveTelegramClientOptions(account) {
  const timeoutSeconds =
    ...
    Number.isFinite(account.config.timeoutSeconds)
      ...
      : void 0;
  const cacheKey = shouldUseTelegramClientOptionsCache()
...
async function sendMessageTelegram(to, text, opts) {
  ...
  const renderHtmlText = (value) =>
    renderTelegramHtmlText(value, {
      textMode,
      tableMode,
    });
  ...
  const buildTextParams = (isLastChunk) =>
  ...
  if (mediaUrl) {
    ...
    const videoDimensions =
      kind === "video" && !isVideoNote ? await probeVideoDimensions(media.buffer) : void 0;
    const mediaParams = {
      ...(htmlCaption
        ? {
            caption: htmlCaption,
...
async function pinMessageTelegram(chatIdInput, messageIdInput, opts) {
  ...
  await createTelegramRequestWithDiag({
    ...
    () => api.pinChatMessage(chatId, messageId, { disable_notification: opts.notify !== true }),
    "pinChatMessage",
  );
...
async function editMessageTelegram(chatIdInput, messageIdInput, text, opts) {
  ...
  const requestWithEditShouldLog = (fn, label, shouldLog) =>
    requestWithDiag(fn, label, shouldLog ? { shouldLog } : void 0);
...
async function sendStickerTelegram(to, fileId, opts) {
  ...
  const result = await withTelegramThreadFallback(
    hasThreadParams ? threadParams : void 0,
    "sticker",
    opts.verbose,
    async (effectiveParams, label) =>
...
//#endregion
export {
  withTelegramApiErrorLogging as C,
  splitTelegramCaption as S,
  wasSentByBot as _,
  editMessageTelegram as a,
  removeTelegramNativeQuoteParam as b,
  renameForumTopicTelegram as c,
  sendPollTelegram as d,
  sendStickerTelegram as f,
  recordSentMessage as g,
  resolveTelegramVoiceSend as h,
  editMessageReplyMarkupTelegram as i,
  resetTelegramClientOptionsCacheForTests as l,
  unpinMessageTelegram as m,
  deleteMessageTelegram as n,
  pinMessageTelegram as o,
  sendTypingTelegram as p,
  editForumTopicTelegram as r,
  reactMessageTelegram as s,
  createForumTopicTelegram as t,
  sendMessageTelegram as u,
  buildTelegramSendParams as v,
  buildInlineKeyboard as x,
  getTelegramNativeQuoteReplyMessageId as y,
};
