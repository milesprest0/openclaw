import {
  o as resolveMatrixBaseConfig,
  r as listNormalizedMatrixAccountIds,
  t as findMatrixAccountConfig,
...
import {
  ...
  n as resolveGlobalMatrixEnvConfig,
...
import { r as repairCurrentTokenStorageMetaDeviceId } from "./storage-B9keKclB.js";
import {
  o as coerceSecretRef,
  u as normalizeResolvedSecretInputString,
} from "./types.secrets-CaNC1eIn.js";
import { t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-CQyhdjpI.js";
...
async function loadMatrixAuthClientDeps() {
  matrixAuthClientDepsPromise ??= Promise.all([
    import("./sdk-D4xopPdP.js"),
    import("./logging-fhpMOMYQ.js"),
...
async function loadMatrixCredentialsReadDeps() {
  matrixCredentialsReadDepsPromise ??= import("./credentials-read-0dODMojj.js").then(
...
function clean(value, path, opts) {
  ...
  return (
    normalizeResolvedSecretInputString({
      value: opts?.allowEnvSecretRefFallback
        ? ref?.source === "env"
...
function hasConfiguredMatrixAccessTokenSource(params) {
  ...
  return (
    hasConfiguredSecretInputValue(resolveMatrixBaseConfig(params.cfg).accessToken, params.cfg) ||
    clean(params.env.MATRIX_ACCESS_TOKEN, "MATRIX_ACCESS_TOKEN").length > 0
  );
}
function resolveConfiguredMatrixAuthInput(params) {
  const normalizedAccountId = normalizeAccountId(params.accountId);
  const accountValue = (findMatrixAccountConfig(params.cfg, normalizedAccountId) ?? {})[
    params.field
  ];
...
function resolveMatrixConfigForAccount(cfg, accountId, env = process.env) {
  ...
  const initialSyncLimit =
    clampMatrixInitialSyncLimit(account.initialSyncLimit) ??
    clampMatrixInitialSyncLimit(matrix.initialSyncLimit);
  const encryption =
    typeof account.encryption === "boolean" ? account.encryption : (matrix.encryption ?? false);
  const allowPrivateNetwork =
    isPrivateNetworkOptInEnabled(account) || isPrivateNetworkOptInEnabled(matrix) ? true : void 0;
  return {
...
async function resolveMatrixAuth(params) {
  ...
  if (accessToken) {
    ...
    if (
      ...
      (cachedCredentials.deviceId || void 0) !== knownDeviceId
    ) {
...
async function backfillMatrixAuthDeviceIdAfterStartup(params) {
  ...
  const deviceId = (
    await fetchMatrixWhoamiIdentity({
...
//#endregion
export {
  resolveMatrixConfigForAccount as i,
  resolveMatrixAuth as n,
  resolveMatrixAuthContext as r,
  backfillMatrixAuthDeviceIdAfterStartup as t,
};
