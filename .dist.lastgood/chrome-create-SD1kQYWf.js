...
function appendQuery$1(url, query) {
  ...
  for (const [key, value] of Object.entries(query))
    if (value !== void 0) parsed.searchParams.set(key, value);
...
function appendQuery(url, query) {
  ...
  const parsed = new URL(url);
  for (const [key, value] of Object.entries(query))
    if (value !== void 0) parsed.searchParams.set(key, String(value));
  return parsed.toString();
}
...
async function listGoogleMeetCollection(params) {
  ...
  do {
    ...
    const pageItems = assertResourceArray(
      payload[params.collectionKey],
      params.collectionKey,
      params.errorPrefix,
    );
    const remaining =
      typeof params.maxItems === "number" ? Math.max(params.maxItems - items.length, 0) : void 0;
...
function sumSessionDurationMs(sessions, fallbackStart, fallbackEnd) {
  const sessionTotal = sessions.reduce((total, session) => {
    ...
    return startMs !== void 0 && endMs !== void 0 && endMs > startMs
      ? total + (endMs - startMs)
      : total;
  }, 0);
...
function decorateAttendanceRow(row, conferenceRecord, params) {
  ...
  const lateByMs =
    conferenceStartMs !== void 0 && firstJoinMs !== void 0
      ? Math.max(firstJoinMs - conferenceStartMs, 0)
      : void 0;
  const earlyLeaveByMs =
    conferenceEndMs !== void 0 && lastLeaveMs !== void 0
...
async function resolveChromeNodeInfo(params) {
  ...
  if (requested) {
    const matches = (await listGoogleMeetNodes(params.runtime)).nodes.filter((node) =>
      matchesRequestedNode(node, requested),
    );
    if (matches.length === 1) {
      const [node] = matches;
  ...
  const nodes = (await listGoogleMeetNodes(params.runtime, { connected: true })).nodes.filter(
    isGoogleMeetNode,
  );
  if (nodes.length === 0)
    throw new Error(
...
async function callBrowserProxyOnNode(params) {
  return parseBrowserProxyResult(
...
function readBrowserCreateResult(result) {
  ...
  return {
    ...
    manualActionReason:
      typeof nested.manualActionReason === "string" ? nested.manualActionReason : void 0,
...
async function createMeetWithBrowserProxyOnNode(params) {
  ...
  while (Date.now() <= deadline)
    try {
      const result = readBrowserCreateResult(
        await callBrowserProxyOnNode({
          runtime: params.runtime,
          nodeId,
          method: "POST",
  ...
  throw new Error(
    lastResult?.manualAction ??
      `Google Meet did not return a meeting URL from the browser create flow before timeout.${lastError ? ` Last browser automation error: ${formatBrowserAutomationError(lastError)}` : ""}`,
  );
}
//#endregion
export {
  googleApiError as _,
  isSameMeetUrlForReuse as a,
  resolveChromeNode as c,
  createGoogleMeetSpace as d,
  endGoogleMeetActiveConference as f,
  fetchLatestGoogleMeetConferenceRecord as g,
  fetchGoogleMeetSpace as h,
  callBrowserProxyOnNode as i,
  resolveChromeNodeInfo as l,
  fetchGoogleMeetAttendance as m,
  isGoogleMeetBrowserManualActionError as n,
  normalizeMeetUrlForReuse as o,
  fetchGoogleMeetArtifacts as p,
  asBrowserTabs as r,
  readBrowserTab as s,
  createMeetWithBrowserProxyOnNode as t,
  buildGoogleMeetPreflightReport as u,
};
