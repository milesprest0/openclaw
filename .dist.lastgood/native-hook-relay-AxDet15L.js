...
import { n as runAgentHarnessBeforeAgentFinalizeHook } from "./lifecycle-hook-helpers-HMjMszrP.js";
...
async function runAgentHarnessAfterToolCallHook(params) {
  ...
  const eventArgs =
    adjustedArgs && typeof adjustedArgs === "object" ? adjustedArgs : params.startArgs;
  try {
...
const nativeHookRelayProviderAdapters = {
  codex: {
...
function removeNativeHookRelayInvocations(relayId) {
  for (let index = invocations.length - 1; index >= 0; index -= 1)
    if (invocations[index]?.relayId === relayId) invocations.splice(index, 1);
}
...
function readNativeHookRelayBridgeRecordIfExists(relayId) {
  ...
  try {
    ...
    if (error.code !== "ENOENT")
      log.debug("failed to read native hook relay bridge registry", {
        error,
        relayId,
      });
  }
...
function permissionRequestToolInputKeyFingerprint(toolInput) {
  ...
  if (truncated && fingerprint.length < MAX_PERMISSION_FALLBACK_KEY_CHARS)
    fingerprint += `${fingerprint ? "," : ""}...`.slice(
      0,
      MAX_PERMISSION_FALLBACK_KEY_CHARS - fingerprint.length,
    );
...
function pruneNativeHookRelayPermissionAllowAlways(now = Date.now()) {
  for (const [key, entry] of permissionAllowAlwaysApprovals)
    if (entry.expiresAtMs <= now) permissionAllowAlwaysApprovals.delete(key);
}
function removeNativeHookRelayPermissionState(relayId) {
  ...
  for (const key of pendingPermissionApprovals.keys())
    if (key.startsWith(`${relayId}:`)) pendingPermissionApprovals.delete(key);
}
...
async function requestNativeHookRelayPermissionApproval(request) {
  ...
  if (Object.prototype.hasOwnProperty.call(requestResult ?? {}, "decision"))
    ...
    decision = (
      await waitForNativeHookRelayApprovalDecision({
        approvalId,
...
//#endregion
export {
  registerNativeHookRelay as a,
  runAgentHarnessBeforeMessageWriteHook as c,
  invokeNativeHookRelayBridge as i,
  buildNativeHookRelayCommand as n,
  renderNativeHookRelayUnavailableResponse as o,
  invokeNativeHookRelay as r,
  runAgentHarnessAfterToolCallHook as s,
  __testing as t,
};
