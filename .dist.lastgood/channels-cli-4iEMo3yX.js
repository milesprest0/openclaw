...
import { t as formatCliChannelOptions } from "./channel-options-BYvssXYN.js";
...
import { t as hasExplicitOptions } from "./command-options-BWuq_8aU.js";
...
import { a as normalizeChannelId, i as listChannelPlugins } from "./registry-D3zb3mnd.js";
...
function resolveConfiguredAuthChannelInput(cfg, mode) {
  const configured = listChannelPlugins()
    .filter((plugin) => supportsChannelAuthMode(plugin, mode))
...
const channelsCommandsLoader = createLazyImportLoader(() => import("./channels-C0Gnie8l.js"));
const bundledPackageChannelMetadataLoader = createLazyImportLoader(
  () => import("./bundled-package-channel-metadata-BkQW_GKA.js"),
);
...
async function registerChannelsCli(program, argv = process.argv, options = {}) {
  ...
  channels
    ...
    .action(async (opts) => {
      await runChannelsCommand(async () => {
        const { channelsListCommand } = await import("./list-GOIREtd8.js");
  ...
  channels
    ...
    .action(async (opts) => {
      await runChannelsCommand(async () => {
        const { channelsStatusCommand } = await import("./status-4T6ylTXk.js");
  ...
  addCommand.action(async (opts, command) => {
    await runChannelsCommand(async () => {
      ...
      await channelsAddCommand(opts, defaultRuntime, {
        hasFlags: hasExplicitOptions(command, getOptionNames(command)),
      });
    });
  });
  channels
    ...
    .action(async (opts, command) => {
      await runChannelsCommand(async () => {
        ...
        await channelsRemoveCommand(opts, defaultRuntime, {
          hasFlags: hasExplicitOptions(command, optionNamesRemove),
        });
      });
    });
  channels
