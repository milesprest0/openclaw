...
import { r as parseSystemdExecStart } from "./systemd-unit-Bm5agCGa.js";
...
function renderGatewayServiceCleanupHints(env = process.env) {
  ...
  switch (process.platform) {
    ...
    case "win32":
      return [`schtasks /Delete /TN "${resolveGatewayWindowsTaskName(profile)}" /F`];
    default:
      return [];
  }
}
...
function hasGatewayServiceMarker(content) {
  ...
  return (
    ...
    hasKindKey &&
    hasMarkerValue &&
    lower.includes(normalizeLowercaseStringOrEmpty("gateway"))
  );
}
...
function isOpenClawGatewayTaskName(name) {
  ...
  return (
    normalized === normalizeLowercaseStringOrEmpty(resolveGatewayWindowsTaskName()) ||
...
//#endregion
export {
  renderGatewayServiceCleanupHints as i,
  findExtraGatewayServices as n,
  findSystemGatewayServices as r,
  detectMarkerLineWithGateway as t,
};
