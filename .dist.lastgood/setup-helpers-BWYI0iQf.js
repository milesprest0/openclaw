...
function applyAccountNameToChannelSection(params) {
  ...
  const baseWithoutName =
    accountId === "default" ? (({ name: _ignored, ...rest }) => rest)(base ?? {}) : (base ?? {});
  return {
    ...params.cfg,
    channels: {
      ...params.cfg.channels,
...
function prepareScopedSetupConfig(params) {
  ...
  if (!params.migrateBaseName || normalizeAccountId(params.accountId) === "default")
    return namedConfig;
...
function createEnvPatchedAccountSetupAdapter(params) {
  return createPatchedAccountSetupAdapter({
    ...
    validateInput: (inputParams) => {
      if (inputParams.input.useEnv && inputParams.accountId !== "default")
        return params.defaultAccountOnlyEnvError;
...
function patchScopedAccountConfig(params) {
  ...
  if (accountId === "default" && !params.scopeDefaultToAccounts)
    return {
      ...params.cfg,
      channels: {
        ...params.cfg.channels,
...
function resolveSingleAccountKeysToMove(params) {
  ...
  const keysToMove = Object.entries(params.channel)
    ...
    .map(([key]) => key)
    .filter((key) => COMMON_SINGLE_ACCOUNT_KEYS_TO_MOVE.has(key));
...
//#endregion
export {
  createSetupInputPresenceValidator as a,
  moveSingleAccountChannelSectionToDefaultAccount as c,
  createPatchedAccountSetupAdapter as i,
  patchScopedAccountConfig as l,
  applySetupAccountConfigPatch as n,
  createZodSetupInputValidator as o,
  createEnvPatchedAccountSetupAdapter as r,
  migrateBaseNameToDefaultAccount as s,
  applyAccountNameToChannelSection as t,
  prepareScopedSetupConfig as u,
};
