...
import { r as withStartedActionClient, t as withResolvedActionClient } from "./client-zsajgNip.js";
import "./string-coerce-runtime-Xx8DhtgQ.js";
import { n as formatMatrixEncryptionUnavailableError } from "./encryption-guidance-DZjNpv05.js";
...
async function ensureMatrixVerificationDmTracked(crypto, opts) {
  ...
  if (
    !(await crypto.ensureVerificationDmTracked({
      roomId,
      userId,
    }))
  )
...
async function waitForMatrixVerificationSummary(params) {
  ...
  while (Date.now() - startedAt < params.timeoutMs) {
    const found = (await params.crypto.listVerifications()).find((summary) =>
      isSameMatrixVerification(summary, params.request),
    );
    if (found) {
      last = found;
      ...
      if (isMatrixVerificationCancelled(found))
        throw new Error(
          `Matrix self-verification was cancelled${found.error ? `: ${found.error}` : ` while waiting to ${params.label}`}`,
        );
  ...
  throw new Error(
    `Timed out waiting for Matrix self-verification to ${params.label}${last ? ` (last phase: ${last.phaseName})` : ""}`,
  );
}
...
async function restoreMatrixRoomKeyBackup(opts = {}) {
  return await withResolvedActionClient(
    opts,
    async (client) =>
      await client.restoreRoomKeyBackup({ recoveryKey: normalizeOptionalString(opts.recoveryKey) }),
  );
}
async function resetMatrixRoomKeyBackup(opts = {}) {
  return await withStartedActionClient(
    opts,
    async (client) =>
      await client.resetRoomKeyBackup({ rotateRecoveryKey: opts.rotateRecoveryKey }),
  );
}
...
//#endregion
export {
  scanMatrixVerificationQr as _,
  confirmMatrixVerificationSas as a,
  getMatrixRoomKeyBackupStatus as c,
  listMatrixVerifications as d,
  mismatchMatrixVerificationSas as f,
  runMatrixSelfVerification as g,
  restoreMatrixRoomKeyBackup as h,
  confirmMatrixVerificationReciprocateQr as i,
  getMatrixVerificationSas as l,
  resetMatrixRoomKeyBackup as m,
  bootstrapMatrixVerification as n,
  generateMatrixVerificationQr as o,
  requestMatrixVerification as p,
  cancelMatrixVerification as r,
  getMatrixEncryptionStatus as s,
  acceptMatrixVerification as t,
  getMatrixVerificationStatus as u,
  startMatrixVerification as v,
  verifyMatrixRecoveryKey as y,
};
