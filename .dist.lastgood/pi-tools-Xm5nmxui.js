...
import { r as resolveAgentConfig } from "./agent-scope-config-CXZGyKMl.js";
import {
...
import { o as shouldSuppressManagedWebSearchTool } from "./codex-native-web-search-core-C0P5WyR_.js";
import { i as resolveMergedSafeBinProfileFixtures } from "./exec-safe-bin-runtime-policy-DBRGTZJu.js";
import { t as bindAbortRelay } from "./fetch-timeout-Df3EC-Xg.js";
import { t as HEARTBEAT_RESPONSE_TOOL_NAME } from "./heartbeat-tool-response-Dx8N2J-j.js";
import { t as createLazyImportLoader } from "./lazy-promise-B1nDYoKn.js";
import { a as logWarn } from "./logger-BWdMCYqK.js";
import { d as resolveGatewayMessageChannel } from "./message-channel-GRZIhFYD.js";
import {
  a as resolveToolLoopDetectionConfig,
  c as createOpenClawReadTool,
  d as createSandboxedWriteTool,
  f as wrapToolMemoryFlushAppendOnlyWrite,
  g as resolveOpenClawPluginToolsForOptions,
  l as createSandboxedEditTool,
  m as wrapToolWorkspaceRootGuardWithOptions,
  o as createHostWorkspaceEditTool,
  p as wrapToolWorkspaceRootGuard,
  s as createHostWorkspaceWriteTool,
  t as createOpenClawTools,
  u as createSandboxedReadTool,
...
import {
  ...
  p as listChannelAgentTools,
  u as copyChannelAgentToolMeta,
...
import {
  a as toRelativeSandboxPath,
  i as resolvePathFromInput,
...
import "./codex-native-web-search-Dsj3cgYT.js";
import { c as PATH_ALIAS_POLICIES, o as root } from "./secure-temp-dir-CCj3cY2B.js";
import {
...
import { i as resolveToolFsConfig, t as createToolFsPolicy } from "./tool-fs-policy-BBWoCrOZ.js";
import { i as resolveImageSanitizationLimits } from "./tool-images-D8ZmIpdW.js";
import {
  a as collectExplicitAllowlist,
  o as collectExplicitDenylist,
...
import {
  a as resolveToolProfilePolicy,
  d as EXEC_TOOL_DISPLAY_SUMMARY,
  f as PROCESS_TOOL_DISPLAY_SUMMARY,
  i as normalizeToolName,
} from "./tool-policy-shared-DsbREhoR.js";
import { i as getPluginToolMeta, n as copyPluginToolMeta } from "./tools-DcY_52BH.js";
...
async function applyUpdateHunk(filePath, chunks, options) {
  const originalLines = (
    await (options?.readFile ?? defaultReadFile)(filePath).catch((err) => {
      throw new Error(`Failed to read file to update ${filePath}: ${err}`);
    })
  ).split("\n");
  if (originalLines.length > 0 && originalLines[originalLines.length - 1] === "")
    originalLines.pop();
  let newLines = applyReplacements(
    originalLines,
    computeReplacements(originalLines, filePath, chunks),
  );
...
const applyPatchSchema = Type.Object({
  input: Type.String({ description: "Patch content using the *** Begin Patch/End Patch format." }),
});
...
function isObjectSchemaWithNoRequiredParams(schema) {
  ...
  if (!(type === "object" || (Array.isArray(type) && type.some((entry) => entry === "object"))))
    return false;
    ...
    if (
      variants.some(
        (variant) =>
          ...
          schemaHasRequiredParams(variant),
      )
    )
      return true;
  }
...
const bashToolsModuleLoader = createLazyImportLoader(() => import("./bash-tools-DnmPgv5H.js"));
...
function createOpenClawCodingTools(options) {
  ...
  const scopeKey =
    options?.exec?.scopeKey ?? options?.sessionKey ?? (agentId ? `agent:${agentId}` : void 0);
  ...
  if (includeBaseCodingTools)
    for (const tool of createCodingTools(workspaceRoot)) {
      if (tool.name === "read") {
        if (sandboxRoot) {
