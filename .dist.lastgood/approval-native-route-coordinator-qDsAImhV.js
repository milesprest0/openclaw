...
function resolveApprovalDeliveryFailedNoticeText(params) {
  return [
    ...
    `Reply with: /approve ${params.approvalKind === "exec" && params.approvalId.length > 8 ? params.approvalId.slice(0, 8) : params.approvalId} ${(params
...
function didReportDeliverToOrigin(report, originAccountId) {
  ...
  if (
    originAccountId !== void 0 &&
    reportAccountId !== void 0 &&
...
function readAllowedDecisionStrings(request) {
  const allowedDecisions =
    "allowedDecisions" in request.request ? request.request.allowedDecisions : void 0;
...
function resolveApprovalRouteNotice(params) {
  ...
  if (
    !params.reports.some((report) => report.deliveredTargets.length > 0) &&
