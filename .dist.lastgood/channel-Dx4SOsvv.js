...
import { t as collectSynologyChatSecurityAuditFindings } from "./security-audit-Ct0aUtQA.js";
import {
  i as resolveAccount,
...
import { g as resolvePinnedHostnameWithPolicy } from "./ssrf-DO8eIXaD.js";
...
let lastSendTime = 0;
const ChatUserSchema = zod_exports.z
  .object({
...
async function deliverSynologyChatReply(params) {
  ...
  return {
    visibleReplySent: await sendMessage(
...
async function sendSynologyChatText(ctx) {
  ...
  if (!(await sendMessage(requireIncomingUrl(account), ctx.text, ctx.to, account.allowInsecureSsl)))
