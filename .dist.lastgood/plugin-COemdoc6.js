import { n as describeAccountSnapshot } from "./account-helpers-DgSuSC4_.js";
import { r as defineChannelMessageAdapter } from "./channel-message-Do1b_D-M.js";
import {
  i as createPairingPrefixStripper,
  r as createLoggedPairingApprovalNotifier,
} from "./channel-pairing-Tw2QazlD.js";
...
import { i as createChatChannelPlugin } from "./core-BCeD7oMO.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import "./channel-core-CYgk_8J6.js";
import "./error-runtime-CdKGkPCx.js";
import { n as buildPassiveProbedChannelStatusSummary } from "./extension-shared-B_zNy4Rm.js";
import "./channel-config-schema-Bi-kEWa4.js";
import {
  A as accessTokenIsExpired,
  C as extractUserId,
  D as CustomError,
  E as rtfm,
  F as Enumerable,
  ...
  O as DataObject,
  P as arrayToObject,
  S as HttpStatusCodeError,
  T as HellFreezesOverError,
  _ as InvalidTokenError,
  a as removeClientManager,
  b as handleTwitchApiResponseError,
  c as Cacheable,
  d as CustomError$1,
  f as createLogger$1,
  g as TokenInfo,
  ...
  k as rawDataSymbol,
  l as PartitionedTimeBasedRateLimiter,
  m as StaticAuthProvider,
  n as stripMarkdownForTwitch,
  o as ChatClient,
  p as require_detect_node,
  r as getClientManager,
  s as CachedGetter,
  t as chunkTextForTwitch,
  u as RateLimitReachedError,
  v as callTwitchApi,
  w as extractUserName,
  x as transformTwitchApiResponse,
  y as callTwitchApiRaw,
} from "./markdown-BbSDjQCW.js";
import { t as createMessageReceiptFromOutboundResults } from "./receipt-DxFe-lje.js";
import "./channel-config-primitives-BZaUZoEw.js";
import {
  a as getAccountConfig,
  c as resolveTwitchAccountContext,
  i as DEFAULT_ACCOUNT_ID,
  l as resolveTwitchSnapshotAccountId,
  o as listAccountIds,
  r as twitchSetupWizard,
  s as resolveDefaultTwitchAccountId,
  t as twitchSetupAdapter,
} from "./setup-surface-DEK1eEtv.js";
import {
...
import {
  a as normalizeTwitchChannel,
  i as normalizeToken,
  n as isAccountConfigured,
  o as resolveTwitchToken,
  r as missingTargetError,
  t as generateMessageId,
} from "./twitch-CB__p3Ax.js";
import { t as zod_exports } from "./zod-Ct0CvXmO.js";
import { h as MarkdownConfigSchema } from "./zod-schema.core-CkFaNmbg.js";
...
var RelationAssertionError = class extends CustomError {
  constructor() {
    super(
      "Relation returned null - this may be a library bug or a race condition in your own code",
    );
  }
};
//#endregion
...
var ResponseBasedRateLimiter = class {
  ...
  async request(req, options) {
    ...
    return await new Promise((resolve, reject) => {
      ...
      if (this._batchRunning || !!this._nextBatchTimer || this._paused) {
        this._logger.trace(
          `request queued batchRunning:${this._batchRunning.toString()} hasNextBatchTimer:${(!!this._nextBatchTimer).toString()} paused:${this._paused.toString()}`,
        );
  ...
  get stats() {
    ...
    return {
      lastKnownLimit:
        (_b = (_a = this._parameters) === null || _a === void 0 ? void 0 : _a.limit) !== null &&
        _b !== void 0
          ? _b
          : null,
      lastKnownRemainingRequests:
        (_d = (_c = this._parameters) === null || _c === void 0 ? void 0 : _c.remaining) !== null &&
        _d !== void 0
          ? _d
          : null,
      lastKnownResetDate: mapNullable(
        (_e = this._parameters) === null || _e === void 0 ? void 0 : _e.resetsAt,
        (v) => new Date(v),
      ),
    };
  }
  async _runRequestBatch(reqSpecs) {
    ...
    this._logger.debug(
      `Doing ${reqSpecs.length} requests, new queue length is ${this._queue.length}`,
    );
    ...
    if (rejectedPromises.length) {
      ...
      const params = settledPromises
        .filter((p) => p.status === "fulfilled" && p.value !== void 0)
        .map((p) => p.value)
        .reduce(
          (carry, v) => {
            if (!carry) return v;
  ...
  _runNextBatch() {
    ...
    const amount = this._parameters
      ? Math.min(this._parameters.remaining, this._parameters.limit / 10)
      : 1;
...
function createTwitchSendReceipt(params) {
  ...
  return createMessageReceiptFromOutboundResults({
    results:
      params.visible === true && messageId && messageId !== "skipped"
        ? [
            {
              channel: "twitch",
              messageId,
 ...
 * );
 */
async function sendMessageTwitchInternal(
  channel,
  text,
  cfg,
  accountId,
...
const twitchOutbound = {
  ...
  sendText: async (params) => {
    ...
    if (!account)
      throw new Error(
        `Twitch account not found: ${resolvedAccountId}. Available accounts: ${availableAccountIds.join(", ") || "none"}`,
      );
 ...
 */
const TwitchRoleSchema = zod_exports.z.enum(["moderator", "owner", "vip", "subscriber", "all"]);
 ...
 */
const MultiAccountSchema = zod_exports.z
  .object({
    ...TwitchConfigBaseShape,
...
async function probeTwitch(account, timeoutMs) {
  ...
  try {
    client = new ChatClient({
      authProvider: new StaticAuthProvider(account.clientId ?? "", rawToken),
    });
  ...
  needsToRetryAfter(res) {
    if (
      ...
      (!res.headers.has("ratelimit-remaining") ||
        Number(res.headers.get("ratelimit-remaining")) === 0)
    )
      return +res.headers.get("ratelimit-reset") * 1e3 - Date.now();
    return null;
  }
...
var import_retry = /* @__PURE__ */ __toESM(
...
let HelixBitsLeaderboardEntry = class HelixBitsLeaderboardEntry extends DataObject {
  ...
  async getUser() {
    return checkRelationAssertion(
      await this._client.users.getUserById(this[rawDataSymbol].user_id),
    );
  }
};
...
let HelixBitsLeaderboard = class HelixBitsLeaderboard extends DataObject {
   ...
   */
  get entries() {
    return this[rawDataSymbol].data.map(
      (entry) => new HelixBitsLeaderboardEntry(entry, this._client),
    );
  }
  /**
...
HelixBitsLeaderboard = __decorate(
  [Cacheable, rtfm("api", "HelixBitsLeaderboard")],
...
let HelixCheermoteList = class HelixCheermoteList extends DataObject {
  ...
  getCheermoteDisplayInfo(name, bits, format) {
    ...
    const correctTier = tiers
      ...
      .find((tier) => tier.min_bits <= bits);
    if (!correctTier)
      throw new HellFreezesOverError(
        `Cheermote "${name}" does not have an applicable tier for ${bits} bits`,
      );
    return {
...
let HelixBitsApi = class HelixBitsApi extends BaseApi {
  ...
  async getLeaderboard(broadcaster, params = {}) {
    return new HelixBitsLeaderboard(
...
var HelixRequestBatcher = class {
  ...
  async request(id) {
    ...
    if (this._requestResolversById.has(id))
      this._requestResolversById.get(id).push({
        resolve,
        reject,
      });
    else
      this._requestResolversById.set(id, [
        {
          resolve,
          reject,
        },
...
let HelixPaginatedRequest = class HelixPaginatedRequest {
  ...
  _processResult(result) {
    this._currentCursor =
      typeof result.pagination === "string" ? result.pagination : result.pagination?.cursor;
...
/** @internal */ function createPaginatedResult(response, type, client) {
  ...
  return {
    ...
    cursor:
      typeof response.pagination === "string" ? response.pagination : response.pagination?.cursor,
  };
}
...
let HelixChannel = class HelixChannel extends DataObject {
  ...
  async getBroadcaster() {
    return checkRelationAssertion(
      await this._client.users.getUserById(this[rawDataSymbol].broadcaster_id),
    );
  }
  /**
...
let HelixAdSchedule = class HelixAdSchedule extends DataObject {
  ...
  get snoozeRefreshDate() {
    return this[rawDataSymbol].snooze_refresh_at
      ? /* @__PURE__ */ new Date(this[rawDataSymbol].snooze_refresh_at * 1e3)
      : null;
  }
  /**
  ...
  get nextAdDate() {
    return this[rawDataSymbol].next_ad_at
      ? /* @__PURE__ */ new Date(this[rawDataSymbol].next_ad_at * 1e3)
      : null;
  }
  /**
  ...
  get lastAdDate() {
    return this[rawDataSymbol].last_ad_at
      ? /* @__PURE__ */ new Date(this[rawDataSymbol].last_ad_at * 1e3)
      : null;
  }
  /**
...
let HelixChannelApi = class HelixChannelApi extends BaseApi {
  ...
  _getChannelByIdBatcher = new HelixRequestBatcher(
    ...
    "broadcaster_id",
    this._client,
    (data) => new HelixChannel(data, this._client),
  );
  /**
  ...
  async getChannelFollowers(broadcaster, user, pagination) {
    return createPaginatedResultWithTotal(
      await this._client.callApi({
        type: "helix",
        url: "channels/followers",
        method: "GET",
  ...
  async snoozeNextAd(broadcaster) {
    return new HelixSnoozeNextAdResult(
      (
        await this._client.callApi({
          type: "helix",
...
let HelixCustomReward = class HelixCustomReward extends DataObject {
  ...
  get cooldownExpiryDate() {
    return this[rawDataSymbol].cooldown_expires_at
      ? new Date(this[rawDataSymbol].cooldown_expires_at)
      : null;
  }
};
...
let HelixCustomRewardRedemption = class HelixCustomRewardRedemption extends DataObject {
  ...
  async updateStatus(newStatus) {
    return (
      await this._client.channelPoints.updateRedemptionStatusByIds(
        this[rawDataSymbol].broadcaster_id,
        this[rawDataSymbol].reward.id,
...
let HelixCharityApi = class HelixCharityApi extends BaseApi {
  ...
  async getCharityCampaign(broadcaster) {
    return new HelixCharityCampaign(
      (
        await this._client.callApi({
          type: "helix",
...
HelixChatBadgeSet = __decorate(
  [Cacheable, rtfm("api", "HelixChatBadgeSet", "id")],
  HelixChatBadgeSet,
);
//#endregion
...
let HelixSharedChatSession = class HelixSharedChatSession extends DataObject {
  ...
  async getHostBroadcaster() {
    return checkRelationAssertion(
      await this._client.users.getUserById(this[rawDataSymbol].host_broadcaster_id),
    );
  }
  /**
  ...
  get participants() {
    return this[rawDataSymbol].participants.map(
      (data) => new HelixSharedChatSessionParticipant(data, this._client),
    );
  }
  /**
...
let HelixChatApi = class HelixChatApi extends BaseApi {
      ...
      await this._client.callApi({
        ...
        query: {
          ...
          ...createSingleKeyQuery(
            "broadcasterId",
            filter?.broadcaster ? extractUserId(filter.broadcaster) : void 0,
          ),
          ...createPaginationQuery(filter),
        },
      }),
    ...
    return new HelixPaginatedRequest(
      {
        ...
        query: {
          ...
          ...createSingleKeyQuery(
            "broadcasterId",
            broadcaster ? extractUserId(broadcaster) : void 0,
          ),
        },
      },
      this._client,
      (data) => new HelixUserEmote(data, this._client),
    );
  }
  /**
  ...
  async getSettingsPrivileged(broadcaster) {
    const broadcasterId = extractUserId(broadcaster);
    return new HelixPrivilegedChatSettings(
      (
        await this._client.callApi({
          type: "helix",
          url: "chat/settings",
  ...
  async sendChatMessage(broadcaster, message, params) {
    const broadcasterId = extractUserId(broadcaster);
    const msg = new HelixSentChatMessage(
      (
        await this._client.callApi({
          type: "helix",
          url: "chat/messages",
          ...
          query: createSendChatMessageQuery(
            broadcasterId,
  ...
  _createModeratorActionQuery(broadcasterId) {
    return createModeratorActionQuery(
      broadcasterId,
      this._getUserContextIdWithDefault(broadcasterId),
    );
  }
...
let HelixClip = class HelixClip extends DataObject {
  ...
  async getCreator() {
    return checkRelationAssertion(
      await this._client.users.getUserById(this[rawDataSymbol].creator_id),
    );
  }
  /**
  ...
  async getVideo() {
    return checkRelationAssertion(
      await this._client.videos.getVideoById(this[rawDataSymbol].video_id),
    );
  }
  /**
  ...
  async createClipFromVod(params) {
    ...
    return (
      await this._client.callApi({
        ...
        canOverrideScopedUserContext: true,
        query: createClipCreateFromVodQuery(
          params,
          this._getUserContextIdWithDefault(broadcasterId),
        ),
      })
...
let HelixDropsEntitlement = class HelixDropsEntitlement extends DataObject {
  ...
  async getGame() {
    return checkRelationAssertion(
      await this._client.games.getGameById(this[rawDataSymbol].game_id),
    );
  }
  /**
...
let HelixPaginatedEventSubSubscriptionsRequest = class HelixPaginatedEventSubSubscriptionsRequest extends HelixPaginatedRequestWithTotal {
  ...
  async getMaxTotalCost() {
    return (this._currentData ?? (await this._fetchData({ query: { after: void 0 } })))
      .max_total_cost;
  }
};
...
let HelixEventSubApi = class HelixEventSubApi extends BaseApi {
  ...
  async createSubscription(
    ...
    if (!usesAppAuth && !user)
      throw new Error(
        `Transport ${transport.method} can only handle subscriptions with user context`,
      );
    const jsonBody = {
      type,
      version,
  ...
  async subscribeToUserUpdateEvents(user, transport, withEmail) {
    return await this.createSubscription(
      ...
      transport,
      user,
      withEmail ? ["user:read:email"] : void 0,
    );
  }
  /**
  ...
  async createConduit(shardCount) {
    return new HelixEventSubConduit(
      (
        await this._client.callApi({
          type: "helix",
          url: "eventsub/conduits",
...
let HelixHypeTrain = class HelixHypeTrain extends DataObject {
  ...
  async getBroadcaster() {
    return checkRelationAssertion(
      await this._client.users.getUserById(this[rawDataSymbol].broadcaster_user_id),
    );
  }
  /**
...
let HelixHypeTrainStatus = class HelixHypeTrainStatus extends DataObject {
  ...
  get current() {
    return mapNullable(
      this[rawDataSymbol].current,
      (data) => new HelixHypeTrain(data, this._client),
    );
  }
  /**
  ...
  get allTimeHigh() {
    return mapNullable(
      this[rawDataSymbol].all_time_high,
      (data) => new HelixHypeTrainAllTimeHigh(data),
    );
  }
  /**
  ...
  get sharedAllTimeHigh() {
    return mapNullable(
      this[rawDataSymbol].shared_all_time_high,
      (data) => new HelixHypeTrainAllTimeHigh(data),
    );
  }
};
...
var HelixHypeTrainApi = class extends BaseApi {
  ...
  async getHypeTrainStatusForBroadcaster(broadcaster) {
    return new HelixHypeTrainStatus(
      (
        await this._client.callApi({
          type: "helix",
...
/** @internal */
function createResolveUnbanRequestQuery(
  broadcaster,
  moderator,
  unbanRequestId,
  approved,
...
let HelixBanUser = class HelixBanUser extends DataObject {
  ...
  async getModerator() {
    return checkRelationAssertion(
      await this._client.users.getUserById(this[rawDataSymbol].moderator_id),
    );
  }
  /**
...
let HelixShieldModeStatus = class HelixShieldModeStatus extends DataObject {
  ...
  get lastActivationDate() {
    return this[rawDataSymbol].last_activated_at === ""
      ? null
      : new Date(this[rawDataSymbol].last_activated_at);
  }
};
...
let HelixModerationApi = class HelixModerationApi extends BaseApi {
  ...
  async checkUserBan(channel, user) {
    const userId = extractUserId(user);
    return (await this.getBannedUsers(channel, { userId })).data.some(
  ...
  async checkUserMod(channel, user) {
    const userId = extractUserId(user);
    return (await this.getModerators(channel, { userId })).data.some(
  ...
  async getShieldModeStatus(broadcaster) {
    const broadcasterId = extractUserId(broadcaster);
    return new HelixShieldModeStatus(
      (
        await this._client.callApi({
          type: "helix",
...
let HelixPredictionOutcome = class HelixPredictionOutcome extends DataObject {
  ...
  get topPredictors() {
    return (
      this[rawDataSymbol].top_predictors?.map((data) => new HelixPredictor(data, this._client)) ??
      []
    );
  }
};
...
let HelixPrediction = class HelixPrediction extends DataObject {
   ...
   */
  get outcomes() {
    return this[rawDataSymbol].outcomes.map(
      (data) => new HelixPredictionOutcome(data, this._client),
    );
  }
  /**
  ...
  get winningOutcome() {
    ...
    const found = this[rawDataSymbol].outcomes.find(
      (o) => o.id === this[rawDataSymbol].winning_outcome_id,
    );
...
let HelixSchedule = class HelixSchedule extends DataObject {
  ...
  get segments() {
    return (
      this[rawDataSymbol].segments?.map((data) => new HelixScheduleSegment(data, this._client)) ??
      []
    );
  }
  /**
...
var HelixScheduleApi = class extends BaseApi {
  ...
  async createScheduleSegment(broadcaster, data) {
    return new HelixScheduleSegment(
      (
        await this._client.callApi({
          type: "helix",
...
let HelixStreamApi = (HelixStreamApi_1 = class HelixStreamApi extends BaseApi {
  ...
  async getStreamMarkersForUser(user, pagination) {
    ...
    return {
      data: flatten(
        result.data.map((data) => HelixStreamApi_1._mapGetStreamMarkersResult(data, this._client)),
      ),
      cursor: result.pagination?.cursor,
    };
  }
  ...
  getStreamMarkersForUserPaginated(user) {
    return new HelixPaginatedRequest(
      ...
      this._client,
      (data) => HelixStreamApi_1._mapGetStreamMarkersResult(data, this._client),
    );
  }
  /**
  ...
  static _mapGetStreamMarkersResult(data, client) {
    return data.videos.reduce(
      (result, video) => [
        ...result,
        ...video.markers.map(
          (marker) => new HelixStreamMarkerWithVideo(marker, video.video_id, client),
        ),
      ],
      [],
    );
...
let HelixSubscriptionApi = class HelixSubscriptionApi extends BaseApi {
  ...
  async checkUserSubscription(user, broadcaster) {
    try {
      return new HelixUserSubscription(
        (
          await this._client.callApi({
            type: "helix",
...
let HelixInstalledExtensionList = class HelixInstalledExtensionList extends DataObject {
  ...
  getAllExtensions() {
    return [...Object.entries(this[rawDataSymbol])].flatMap(([type, typeEntries]) =>
...
let HelixUserApi = class HelixUserApi extends BaseApi {
  ...
  async updateAuthenticatedUser(user, data) {
    return new HelixPrivilegedUser(
      (
        await this._client.callApi({
          type: "helix",
          url: "users",
    ...
    return (
      await this._client.callApi({
        ...
        scopes: withInactive
          ...
          : ["user:read:broadcast", "channel:manage:extensions"],
      })
  ...
  async getActiveExtensions(user, withDev = false) {
    const userId = extractUserId(user);
    return new HelixInstalledExtensionList(
      (
        await this._client.callApi({
          type: "helix",
...
let HelixVideo = class HelixVideo extends DataObject {
  ...
  get durationInSeconds() {
    ...
    if (!parts)
      throw new HellFreezesOverError(
        `Could not parse duration string: ${this[rawDataSymbol].duration}`,
      );
    return parts
      .map((part) => {
...
let BaseApiClient = class BaseApiClient extends EventEmitter {
  ...
  async callApi(options) {
    ...
    if (
      ...
      accessToken.userId &&
      authProvider.refreshAccessTokenForUser
    ) {
  ...
  async _callApiUsingInitialToken(options, accessToken, wasRefreshed = false) {
    ...
    this.emit(
      this.onRequest,
      new ApiReportedRequest(options, response.status, accessToken.userId ?? null),
    );
  ...
  async _callApiInternal(options, clientId, accessToken, authorizationType) {
    ...
    this._logger.debug(
      `Called ${type} API: ${options.method ?? "GET"} ${options.url} - result: ${result.status}`,
    );
    return result;
  }
};
...
let ApiClient = class ApiClient extends BaseApiClient {
  ...
  async asUser(user, runner) {
    return await runner(
      new UserContextApiClient(this._config, this._logger, this._rateLimiter, extractUserId(user)),
    );
  }
  /**
  ...
  async asIntent(intents, runner) {
    ...
    for (const intent of intents) {
      ...
      if (user)
        return await runner(
          new UserContextApiClient(this._config, this._logger, this._rateLimiter, user.userId),
        );
    }
  ...
  base: {
    ...
    gateway: {
      startAccount: async (ctx) => {
        ...
        const { monitorTwitchProvider } = await import("./monitor-CTS-J52X.js");
