...
import { n as hasActiveApprovalNativeRouteRuntime } from "./approval-native-route-coordinator-qDsAImhV.js";
import { t as normalizeChatType } from "./chat-type-DsuCa6up.js";
import {
  a as buildPluginBindingUnavailableText,
  b as touchConversationBindingRecord,
  c as hasShownPluginBindingFallbackNotice,
  d as markPluginBindingFallbackNoticeShown,
  h as toPluginConversationBinding,
  n as buildPluginBindingDeclinedText,
  r as buildPluginBindingErrorText,
  u as isPluginOwnedSessionBindingRecord,
  y as resolveConversationBindingRecord,
...
import { a as isDiagnosticsEnabled } from "./diagnostic-events-9isI4hMq.js";
import {
  f as markDiagnosticSessionProgress,
  i as logMessageQueued,
  r as logMessageProcessed,
  s as logSessionStateChange,
} from "./diagnostic-TRyCbvnu.js";
import { t as createTtsDirectiveTextStreamCleaner } from "./directives-GtrZOD7p.js";
import { n as registerDispatcher } from "./dispatcher-registry-C7_fCkMY.js";
...
import {
  ...
  u as fireAndForgetHook,
} from "./hook-runner-global-DzzZF3B1.js";
import { t as finalizeInboundContext } from "./inbound-context-C2mdUiyz.js";
import {
  n as commitInboundDedupe,
  r as releaseInboundDedupe,
  t as claimInboundDedupe,
} from "./inbound-dedupe-Cwfeb1XW.js";
import {
...
import {
  c as toPluginInboundClaimEvent,
  i as toInternalMessageReceivedContext,
  l as toPluginMessageContext,
  n as deriveInboundMessageHookContext,
  s as toPluginInboundClaimContext,
  u as toPluginMessageReceivedEvent,
} from "./message-hook-mappers-NjOeo4xK.js";
import { t as normalizeReplyPayload } from "./normalize-reply-CochhAjR.js";
import { S as isParentOwnedBackgroundAcpSession } from "./openclaw-tools-6wQ_ahnn.js";
...
import { n as getReplyPayloadMetadata } from "./reply-payload-D38mxo21.js";
import { m as resolveSendableOutboundReplyParts } from "./reply-payload-D_-IJF8p.js";
import { r as generateSecureInt } from "./secure-random-Ck_jpq4r.js";
import { r as selectAgentHarness } from "./selection-BcCvjGlJ.js";
...
import { n as isAcpSessionKey } from "./session-key-utils-B3KPN8Ee.js";
import {
  n as resolveSilentReplySettings,
  r as resolveSilentReplyRewriteText,
} from "./silent-reply-B9HyBycr.js";
import {
  r as resolveSourceReplyVisibilityPolicy,
  t as isExplicitSourceReplyCommand,
} from "./source-reply-delivery-mode-BejFwJbn.js";
import {
...
import { h as normalizeVerboseLevel } from "./thinking-C9_Z62px.js";
import { a as isSilentReplyText, n as SILENT_REPLY_TOKEN } from "./tokens-DWz8lWRf.js";
import { u as mergeAlsoAllowPolicy } from "./tool-policy-_3QUoIEC.js";
import { t as isToolAllowedByPolicies } from "./tool-policy-match-c6IjbWPy.js";
import { a as resolveToolProfilePolicy } from "./tool-policy-shared-DsbREhoR.js";
import { n as normalizeTtsAutoMode } from "./tts-auto-mode-CknSJrjF.js";
import {
  i as shouldCleanTtsDirectiveText,
  r as shouldAttemptTtsPayload,
  t as resolveConfiguredTtsMode,
} from "./tts-config-DMOCN249.js";
import {
  d as resolveEffectiveReplyRoute,
  l as withFullRuntimeReplyConfig,
...
function resolveReplyRoutingDecision(params) {
  ...
  const isInternalWebchatTurn =
    ...
    (surfaceChannel === "webchat" || !surfaceChannel) &&
...
const ttsRuntimeLoader = createLazyImportLoader(() => import("./tts.runtime-DJn4URty.js"));
...
const replyMediaPathsRuntimeLoader = createLazyImportLoader(
  () => import("./reply-media-paths.runtime-CRd-tcy3.js"),
);
...
const isInboundAudioContext = (ctx) => {
  if (
    [
      ...
      .filter(Boolean)
      .map((type) => normalizeMediaType(type))
      .some((type) => type === "audio" || type.startsWith("audio/"))
  )
    return true;
  const trimmed = (
...
async function dispatchReplyFromConfig(params) {
  ...
  const traceReplyPhase = (name, run) =>
  ...
  const silentTurnFlagEnabled = (name) => {
    const raw = (process.env[name] ?? "").trim().toLowerCase();
    return raw !== "0" && raw !== "false" && raw !== "off";
  };
  const silentTurnPositiveInt = (name, fallback) => {
  ...
  const receiptAckDelayMs = silentTurnPositiveInt("OPENCLAW_SILENT_TURN_ACK_DELAY_MS", 4e3);
  ...
  let receiptAckSent = false;
  let heartbeatCount = 0;
  let receiptAckTimer;
  let heartbeatTimer;
  const cancelReceiptAckTimer = () => {
    if (receiptAckTimer !== void 0) {
      clearTimeout(receiptAckTimer);
      receiptAckTimer = void 0;
    }
  };
  const stopHeartbeat = () => {
  ...
  const deliverSilentTurnText = async (text) => {
    const payload = { text };
    const routed = await routeReplyToOriginating(payload);
    if (routed) return routed.ok;
    markInboundDedupeReplayUnsafe();
    return dispatcher.sendFinalReply(payload);
  };
  const deliverGuaranteedFinalText = async (text) => {
    if (!silentTurnGuaranteeEnabled || userVisibleFinalDelivered) return false;
    try {
      ...
      logVerbose(
        `dispatch-from-config: silent-turn guarantee delivery failed (non-fatal): ${formatErrorMessage(guaranteeErr)}`,
      );
      return false;
    }
  };
  const buildReceiptAckText = () => {
    const firstLine = (
      (
  ...
  const fireReceiptAck = async () => {
    ...
    try {
      ...
      logVerbose(
        `dispatch-from-config: silent-turn receipt ack delivered (session=${sessionKey ?? "unknown"})`,
      );
    } catch (ackErr) {
      logVerbose(
        `dispatch-from-config: silent-turn receipt ack failed (non-fatal): ${formatErrorMessage(ackErr)}`,
      );
    }
  };
  const fireHeartbeat = async () => {
    if (!heartbeatEnabled || suppressDelivery || userVisibleFinalDelivered) return;
    try {
      ...
      logVerbose(
        `dispatch-from-config: silent-turn heartbeat failed (non-fatal): ${formatErrorMessage(hbErr)}`,
      );
    }
  };
  const armSilentTurnSignals = () => {
    if (!silentTurnGuaranteeEnabled || suppressDelivery) return;
    ...
    if (heartbeatEnabled)
      heartbeatTimer = setInterval(() => {
        ...
        heartbeatCount += 1;
        fireHeartbeat();
        if (heartbeatCount >= heartbeatMax) stopHeartbeat();
      }, heartbeatIntervalMs);
  };
  ...
  const timestamp =
    typeof ctx.Timestamp === "number" && Number.isFinite(ctx.Timestamp) ? ctx.Timestamp : void 0;
  ...
  const isInternalWebchatTurn =
    normalizedCurrentSurface === "webchat" &&
    (normalizedSurfaceChannel === "webchat" || !normalizedSurfaceChannel) &&
    ctx.ExplicitDeliverRoute !== true;
  const routeReplyRuntime = Boolean(
  ...
  const normalizeReplyMediaPayload = async (payload) => {
    ...
    return await (
      await getNormalizeReplyMediaPaths()
    )(payload);
  };
  ...
  const sendPolicy = resolveSendPolicy({
    ...
    channel:
      (shouldRouteToOriginating ? routeReplyChannel : void 0) ??
  ...
  const harnessDefaultVisibleReplies =
    configuredVisibleReplies === void 0 && chatType !== "group" && chatType !== "channel"
  ...
  const runtimeProfileAlsoAllow =
    ...
    (params.replyOptions?.sourceReplyDeliveryMode === void 0 &&
  ...
  const messageToolAvailable = isToolAllowedByPolicies("message", [
    ...
    isSubagentEnvelopeSession(acpDispatchSessionKey, {
      ...
      ? resolveSubagentToolPolicyForSession(cfg, acpDispatchSessionKey, { store: subagentStore })
      : void 0,
  ]);
  ...
  const attachSourceReplyDeliveryMode = (result) =>
  ...
  try {
        ...
        ctx,
        {
          ...
          onBlockReply: (payload, context) => {
            ...
            const run = async () => {
              ...
              const queuedContext =
                payloadMetadata?.assistantMessageIndex !== void 0
                  ? {
                      ...context,
    ...
    const shouldDeliverDespiteSourceReplySuppression = (reply) =>
      suppressAutomaticSourceDelivery &&
      !sendPolicyDenied &&
    ...
    if (
      ...
      if (reasoningFallback) {
        ...
        logVerbose(
          `dispatch-from-config: silent-turn guarantee promoting reasoning-only payload to final (session=${sessionKey ?? "unknown"})`,
        );
        attemptedFinalDelivery = true;
    ...
    if (
      ...
      if (
        ...
        queuedFinal = true;
        logVerbose(
          `dispatch-from-config: silent-turn guarantee delivered final-delivery-failure backstop (session=${sessionKey ?? "unknown"})`,
        );
      }
    }
    ...
    if (inboundDedupeClaim.status === "claimed")
      if (inboundDedupeReplayUnsafe) commitInboundDedupe(inboundDedupeClaim.key);
...
//#endregion
export {
  createReplyDispatcherWithTyping as a,
  withReplyDispatcher as c,
  createReplyDispatcher as i,
  dispatchInboundMessageWithBufferedDispatcher as n,
  dispatchReplyFromConfig as o,
  dispatchInboundMessageWithDispatcher as r,
  settleReplyDispatcher as s,
  dispatchInboundMessage as t,
};
