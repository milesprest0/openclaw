import { l as resolveSlackReplyToMode } from "./accounts-Stz3jcMX.js";
import { i as shouldAckReaction } from "./ack-reactions-Chyt3s4c.js";
import { l as reactSlackMessage } from "./actions-BivXGvX4.js";
import {
...
import { r as resolveRuntimeConversationBindingRoute } from "./binding-routing-BUtBDbgl.js";
...
import { t as resolveConversationLabel } from "./conversation-label-CWxpBRej.js";
...
import {
  c as resolveSlackThreadStarter,
  s as resolveSlackThreadHistory,
  u as formatSlackFileReference,
} from "./media-DIWB0wJO.js";
import {
...
import { n as resolveChannelSourceReplyDeliveryMode } from "./reply-pipeline-BZPcIR_E.js";
...
import {
  ...
  r as authorizeSlackBotRoomMessage,
...
import "./logging-core-Dw6t88wN.js";
...
import {
  i as hasSlackThreadParticipationWithPersistence,
...
import { r as parseSlackTarget } from "./target-parsing-CoqR8Hoa.js";
import "./send.runtime-ChnzjYFl.js";
...
function loadSlackMediaModule$1() {
  slackMediaModulePromise$1 ??= import("./media-BqfGsb1r.js");
...
async function resolveSlackMessageContent(params) {
  ...
  const mediaPromise =
    ownFiles && ownFiles.length > 0
      ? loadSlackMediaModule$1().then(({ resolveSlackMedia }) =>
          resolveSlackMedia({
            files: ownFiles,
  ...
  const attachmentContentPromise =
    params.message.attachments && params.message.attachments.length > 0
      ? loadSlackMediaModule$1().then(({ resolveSlackAttachmentContent }) =>
...
function resolveSlackDmHistoryLimit(params) {
  const override =
    params.userId && params.account.config.dms?.[params.userId]?.historyLimit !== void 0
...
async function resolveSlackDmHistoryContext(params) {
  ...
  try {
    const messages = (
      (
        await params.ctx.app.client.conversations.history({
    ...
    const resolveUserLabel = async (userId) => {
      ...
      const label =
        normalizeOptionalString((await params.ctx.resolveUserName(userId)).name) ?? userId;
...
function resolveSlackThreadContext(params) {
  ...
  const isThreadReply =
    typeof incomingThreadTs === "string" &&
...
function loadSlackMediaModule() {
  slackMediaModulePromise ??= import("./media-BqfGsb1r.js");
  ...
  if (starter?.text && includeStarterContext) {
    ...
    if (!params.effectiveDirectMedia && starter.files && starter.files.length > 0) {
      ...
      if (threadStarterMedia)
        logVerbose(
          `slack: hydrated thread starter file ${threadStarterMedia.map((item) => item.placeholder).join(", ")} from root message`,
        );
    }
...
async function readSlackSubteamUsers(params) {
  ...
  try {
    ...
    const users = new Set(
      (response.users ?? []).map((userId) => normalizeSlackId(userId)).filter(Boolean),
    );
...
function auditSlackIngress(phase, fields) {
...
async function prepareSlackMessage(params) {
  ...
  const willImplicitlyThreadReply =
    ...
    resolveSlackReplyToMode(
      account,
      isDirectMessage ? "direct" : isGroupDm ? "group" : "channel",
    ) !== "off";
  ...
  const resolveWasMentioned = (mentionRegexes) =>
    opts.wasMentioned ??
    (!isDirectMessage &&
  ...
  const shouldSendAckReaction =
    shouldAckReaction$1() && (!sourceRepliesAreToolOnly || allowToolOnlyStatusReaction);
  ...
  const ctxPayload = finalizeInboundContext({
    ...
    MediaPaths:
      effectiveMedia && effectiveMedia.length > 0 ? effectiveMedia.map((m) => m.path) : void 0,
    MediaUrls:
      effectiveMedia && effectiveMedia.length > 0 ? effectiveMedia.map((m) => m.path) : void 0,
    MediaTypes:
