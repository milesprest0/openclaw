...
import { c as getAgentRunContext, l as onAgentEvent } from "./agent-events-c6-2eZvI.js";
...
import {
  c as setDetachedTaskDeliveryStatusByRunId,
  i as failTaskRunByRunId,
  r as createRunningTaskRun,
  t as completeTaskRunByRunId,
} from "./detached-task-runtime-CRxjrfaR.js";
import { a as formatErrorMessage, l as readErrorName } from "./errors-DZMrVkYL.js";
...
import {
  n as createLazyPromiseLoader,
  t as createLazyImportLoader,
} from "./lazy-promise-B1nDYoKn.js";
import { u as resolveStorePath } from "./paths-CfeECf6Z.js";
import { t as registerPendingSpawnedChildrenQuery } from "./pending-spawn-query-77FNakly.js";
import { l as retireSessionMcpRuntimeForSessionKey } from "./pi-bundle-mcp-runtime-_btntJtc.js";
import { s as toSafeImportPath } from "./plugin-module-loader-cache-7MZ9xfuO.js";
import { i as waitForAgentRun, t as isRecoverableAgentWaitError } from "./run-wait-BzfTmTHO.js";
...
import { t as emitSessionLifecycleEvent } from "./session-lifecycle-events-D9760v5u.js";
...
import "./subagent-announce-queue-CTKfk7y8.js";
import { a as normalizeLowercaseStringOrEmpty } from "./string-coerce-BdEutqX5.js";
import {
  A as SUBAGENT_TARGET_KIND_SUBAGENT,
  C as resolveSubagentSessionStatus,
  D as SUBAGENT_ENDED_REASON_COMPLETE,
  E as SUBAGENT_ENDED_OUTCOME_TIMEOUT,
  O as SUBAGENT_ENDED_REASON_ERROR,
  S as getSubagentSessionStartedAt,
  T as SUBAGENT_ENDED_OUTCOME_KILLED,
  a as countActiveDescendantRunsFromRuns,
  c as countPendingDescendantRunsFromRuns,
  d as listDescendantRunsForRequesterFromRuns,
  f as listRunsForControllerFromRuns,
  j as subagentRuns,
  k as SUBAGENT_ENDED_REASON_KILLED,
  l as getSubagentRunByChildSessionKeyFromRuns,
  n as persistSubagentRunsToDisk,
  o as countActiveRunsForSessionFromRuns,
  r as restoreSubagentRunsFromDisk,
  t as getSubagentRunsSnapshotForRead,
  w as SUBAGENT_ENDED_OUTCOME_ERROR,
  x as getSubagentSessionRuntimeMs,
  y as isStaleUnendedSubagentRun,
} from "./subagent-registry-state-CxryE7mc.js";
import { t as configureSubagentRegistrySteerRuntime } from "./subagent-registry-steer-runtime-TWfB-7kq.js";
import {
  d as withSubagentOutcomeTiming,
  t as deleteSubagentSessionForCleanup,
...
function resolveAnnounceRetryDelayMs(retryCount) {
  const baseDelay =
    MIN_ANNOUNCE_RETRY_DELAY_MS * 2 ** Math.max(0, Math.max(0, Math.min(retryCount, 10)) - 1);
  return Math.min(baseDelay, MAX_ANNOUNCE_RETRY_DELAY_MS);
}
function logAnnounceGiveUp(entry, reason) {
  const retryCount = entry.announceRetryCount ?? 0;
  const endedAgoMs =
    typeof entry.endedAt === "number" ? Math.max(0, Date.now() - entry.endedAt) : void 0;
...
function findSessionEntryByKey$1(store, sessionKey) {
  ...
  for (const [key, entry] of Object.entries(store))
    if (normalizeLowercaseStringOrEmpty(key) === normalized) return entry;
}
async function persistSubagentSessionTiming(entry) {
  ...
  const endedAt =
    typeof entry.endedAt === "number" && Number.isFinite(entry.endedAt) ? entry.endedAt : void 0;
  const runtimeMs =
    endedAt !== void 0
...
function resolveArchiveAfterMs(cfg) {
  const minutes =
    (cfg ?? getRuntimeConfig()).agents?.defaults?.subagents?.archiveAfterMinutes ?? 60;
...
const browserCleanupLoader$1 = createLazyImportLoader(
  () => import("./browser-lifecycle-cleanup-lLafoPqf.js"),
);
...
function createSubagentRegistryLifecycleController(params) {
  ...
  const formatAnnounceDeliveryError = (delivery) => {
    const errors = [
      ...
      ...(delivery.phases ?? []).map((phase) =>
        phase.error ? `${phase.phase}: ${phase.error}` : void 0,
      ),
    ]
  ...
  const finalizeResumedAnnounceGiveUp = async (giveUpParams) => {
    ...
    if (giveUpParams.entry.cleanup === "delete" || !giveUpParams.entry.retainAttachmentsOnKeep)
  ...
  const completeSubagentRun = async (completeParams) => {
    ...
    let mutated = false;
    if (
      completeParams.reason === "subagent-complete" &&
    ...
    await (
      ...
      (await loadCleanupBrowserSessionsForLifecycleEnd$1())
    )({
...
function createSubagentRunManager(params) {
  ...
  const markSubagentRunTerminated = (markParams) => {
    ...
    if (typeof markParams.childSessionKey === "string" && markParams.childSessionKey.trim()) {
      for (const [runId, entry] of params.runs.entries())
        if (entry.childSessionKey === markParams.childSessionKey.trim()) runIds.add(runId);
    }
...
const subagentAnnounceLoader = createLazyImportLoader(
  () => import("./subagent-announce-C-6BRdNH.js"),
);
...
function findSessionEntryByKey(store, sessionKey) {
  ...
  for (const [key, entry] of Object.entries(store))
    if (key.trim().toLowerCase() === normalized) return entry;
}
...
function scheduleSubagentOrphanRecovery(params) {
  ...
  import("./subagent-orphan-recovery-D7JBy6cv.js").then(
...
async function finalizeInterruptedSubagentRun(params) {
  ...
  if (typeof params.childSessionKey === "string" && params.childSessionKey.trim()) {
    ...
    for (const [runId, entry] of subagentRuns.entries())
      if (entry.childSessionKey === childSessionKey) runIds.add(runId);
  }
  if (runIds.size === 0) return 0;
  const endedAt =
...
//#endregion
export {
  finalizeInterruptedSubagentRun as a,
  initSubagentRegistry as c,
  markSubagentRunForSteerRestart as d,
  markSubagentRunTerminated as f,
  scheduleSubagentOrphanRecovery as h,
  countPendingDescendantRuns as i,
  listDescendantRunsForRequester as l,
  replaceSubagentRunAfterSteer as m,
  countActiveDescendantRuns as n,
  getLatestSubagentRunByChildSessionKey as o,
  registerSubagentRun as p,
  countActiveRunsForSession as r,
  getSubagentRunByChildSessionKey as s,
  clearSubagentRunSteerRestart as t,
  listSubagentRunsForController as u,
};
