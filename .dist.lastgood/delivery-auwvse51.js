...
import { i as chunkMarkdownTextWithMode } from "./chunk-aBEwc7QQ.js";
import {
...
import {
  ...
  n as markdownToTelegramHtml,
  o as wrapFileReferencesInHtml,
  t as markdownToTelegramChunks,
} from "./format-xrIurvHN.js";
import { r as logVerbose, s as warn, t as danger } from "./globals-BdfwDi2E.js";
...
import "./file-access-runtime-kQRvL0hE.js";
import {
...
import {
  ...
  S as splitTelegramCaption,
  b as removeTelegramNativeQuoteParam,
  h as resolveTelegramVoiceSend,
  v as buildTelegramSendParams,
  x as buildInlineKeyboard,
  y as getTelegramNativeQuoteReplyMessageId,
} from "./send-WFJDNput.js";
import { a as getCachedSticker, n as cacheSticker } from "./sticker-cache-B9mKZWry.js";
...
function isTelegramThreadNotFoundError(err) {
  if (GrammyErrorCtor$2 && err instanceof GrammyErrorCtor$2)
...
async function sendTelegramWithThreadFallback(params) {
  ...
  const shouldSuppressFirstErrorLog = (err) =>
  ...
  const runLoggedSend = (operation, requestParams, shouldLog) =>
...
function isVoiceMessagesForbidden(err) {
  if (GrammyErrorCtor$1 && err instanceof GrammyErrorCtor$1)
...
async function maybePinFirstDeliveredMessage(params) {
  if (
    !(params.pin === true || (typeof params.pin === "object" && params.pin.enabled)) ||
    typeof params.firstDeliveredMessageId !== "number"
  )
    return;
...
async function deliverReplies(params) {
  ...
  for (const originalReply of normalizedReplies) {
    ...
    const replyToId =
      params.replyToMode === "off" ? void 0 : resolveTelegramReplyId(reply.replyToId);
...
async function downloadAndSaveTelegramFile(params) {
  ...
  if (trustedLocalFile) {
    ...
    try {
      localFile = await (
        await root(trustedLocalFile.rootDir)
      ).read(trustedLocalFile.relativePath, { maxBytes: params.maxBytes });
    } catch (err) {
...
//#endregion
export {
  emitTelegramMessageSentHooks as i,
  deliverReplies as n,
  emitInternalMessageSentHook as r,
  resolveMedia as t,
};
