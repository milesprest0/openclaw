...
import {
  n as pickLastNonEmptyTextFromPayloads,
  r as pickSummaryFromOutput,
} from "./helpers-ByPLfJuD.js";
import { l as hasScheduledNextRunAtMs } from "./jobs-SDHRSbuQ.js";
...
import { a as hasReplyPayloadContent } from "./payload-BcGg6iOl.js";
...
import {
  n as isLikelyInterimCronMessage,
  t as expectsSubagentFollowup,
} from "./subagent-followup-hints-C0f9CDtC.js";
...
import { r as shouldAttemptTtsPayload } from "./tts-config-DMOCN249.js";
...
function normalizeSilentReplyText(text) {
  ...
  if (isSilentReplyText(text, "NO_REPLY"))
    return {
      text: void 0,
      strippedTrailingSilentToken: false,
    };
  ...
  if (!next.trim() || isSilentReplyText(next, "NO_REPLY"))
    return {
      text: void 0,
...
function matchesMessagingToolDeliveryTarget(target, delivery) {
  ...
  if (delivery.accountId && target.accountId && target.accountId !== delivery.accountId)
    return false;
  return (
    normalizeDeliveryTarget(channel, target.to.replace(/:topic:\d+$/, "")) ===
    normalizeDeliveryTarget(channel, delivery.to)
  );
}
...
function pruneCompletedDirectCronDeliveries(now) {
  ...
  for (const [key, entry] of COMPLETED_DIRECT_CRON_DELIVERIES)
    if (now - entry.ts >= ttlMs) COMPLETED_DIRECT_CRON_DELIVERIES.delete(key);
...
async function retryTransientDirectCronDelivery(params) {
  ...
  for (;;) {
    ...
    try {
      ...
      await logCronDeliveryWarn(
        `[cron:${params.jobId}] transient direct announce delivery failure, retrying ${nextAttempt}/${maxAttempts} in ${Math.round(delayMs / 1e3)}s: ${summarizeDirectCronDeliveryError(err)}`,
      );
      retryIndex += 1;
...
async function dispatchCronDelivery(params) {
  ...
  const formatDeliveryTargetError = (error) =>
  ...
  const deliverViaDirect = async (delivery, options) => {
    ...
    try {
      ...
      if (
        ...
        await logCronDeliveryWarn(
          `[cron:${params.job.id}] skipping stale delivery scheduled at ${new Date(scheduledAtMs).toISOString()}, started ${Math.round(startDelayMs / 6e4)}m late, current age ${Math.round((nowMs - scheduledAtMs) / 6e4)}m`,
        );
        return params.withRunSession({
          status: "ok",
          summary,
  ...
  const finalizeTextDelivery = async (delivery) => {
    ...
    if (
      normalizedSynthesizedText.text === void 0 ||
