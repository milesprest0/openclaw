...
import {
  ...
  S as CHROME_STOP_TIMEOUT_MS,
  _ as CHROME_BOOTSTRAP_EXIT_TIMEOUT_MS,
  c as isWebSocketUrl,
  f as redactCdpUrl,
  i as fetchJson,
  ...
  p as withCdpSocket,
  r as fetchCdpChecked,
  s as isDirectCdpWebSocketEndpoint,
  t as appendCdpPath,
  u as openCdpWebSocket,
  v as CHROME_BOOTSTRAP_PREFS_TIMEOUT_MS,
  x as CHROME_STDERR_HINT_MAX_CHARS,
  y as CHROME_LAUNCH_READY_WINDOW_MS,
...
import {
  ...
  t as getManagedBrowserMissingDisplayError,
} from "./config-DhdywqBF.js";
import {
  a as BrowserProfileUnavailableError,
  t as BrowserCdpEndpointBlockedError,
} from "./errors-BxBS-H8t.js";
import "./text-runtime-lKuAtsoz.js";
import "./process-runtime-ZYUgmnjB.js";
import { t as normalizeHostname } from "./hostname-DOneeF6p.js";
import { r as saveJsonFile, t as loadJsonFile } from "./json-store-C6iFE4qW.js";
import { t as prepareOomScoreAdjustedSpawn } from "./linux-oom-score-DPLaMcK0.js";
import { i as isLoopbackHost } from "./net-BQYp2xgJ.js";
import { r as ensurePortAvailable } from "./ports-CzR_dSOf.js";
import "./subsystem-BAFMLwLh.js";
import { i as redactSensitiveText } from "./redact-Mxj55RzQ.js";
import { t as ensureAbsoluteDirectory } from "./sdk-security-runtime-B-QBmBet.js";
import {
...
import { t as CONFIG_DIR } from "./utils-BGRcpLKt.js";
...
function isExplicitlyAllowedBrowserHostname(hostname, ssrfPolicy) {
  ...
  if (
    (ssrfPolicy?.allowedHostnames ?? []).some(
      (value) => normalizeHostname(value) === normalizedHostname,
    )
  )
    return true;
...
async function captureScreenshot(opts) {
  return await withCdpSocket(
    ...
    async (send) => {
      ...
      const quality =
        format === "jpeg" ? Math.max(0, Math.min(100, Math.round(opts.quality ?? 85))) : void 0;
      try {
        const base64 = (
  ...
  for (const candidateWsUrl of candidateWsUrls)
    try {
      ...
      return await withCdpSocket(
        ...
        async (send) => {
          const targetId =
            (await send("Target.createTarget", { url: opts.url }))?.targetId?.trim() ?? "";
...
async function findCursorInteractiveElements(send, sessionId) {
  ...
  const rootNodeId = (await send("DOM.getDocument", { depth: 0 }, sessionId).catch(() => null))
    ?.root?.nodeId;
...
async function resolveLinkUrls(send, refs, sessionId) {
  ...
  await Promise.all(
    Object.values(refs).map(async (ref) => {
      ...
      const objectId = (
        await send("DOM.resolveNode", { backendNodeId: ref.backendDOMNodeId }, sessionId).catch(
          () => null,
        )
      )?.object?.objectId;
      if (!objectId) return;
...
async function buildCdpRoleSnapshot(params) {
  const res = await params.send(
    ...
    params.frameId ? { frameId: params.frameId } : void 0,
    params.sessionId,
  );
...
async function diagnoseChromeCdp(cdpUrl, timeoutMs = 500, handshakeTimeoutMs = 800, ssrfPolicy) {
  ...
  if (!health.ok) {
    if (isWebSocketUrl(cdpUrl) && wsUrl !== cdpUrl) {
      if ((await diagnoseCdpHealthCommand(cdpUrl, handshakeTimeoutMs)).ok)
        return {
          ok: true,
          cdpUrl,
          wsUrl: cdpUrl,
...
async function isChromeReachable(cdpUrl, timeoutMs = 500, ssrfPolicy) {
  try {
    ...
    if (
      await fetchChromeVersion(
        isWebSocketUrl(cdpUrl) ? normalizeCdpHttpBaseForJsonEndpoints(cdpUrl) : cdpUrl,
        timeoutMs,
        ssrfPolicy,
      )
    )
...
async function getChromeWebSocketUrl(cdpUrl, timeoutMs = 500, ssrfPolicy) {
  ...
  const wsUrl =
    normalizeOptionalString(
      (await fetchChromeVersion(discoveryUrl, timeoutMs, ssrfPolicy))?.webSocketDebuggerUrl,
    ) ?? "";
  if (!wsUrl) {
...
async function launchOpenClawChrome(resolved, profile, launchOptions = {}) {
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? "#FF4500").toUpperCase(),
...
//#endregion
export {
  assertBrowserNavigationRedirectChainAllowed as C,
  withBrowserNavigationPolicy as E,
  assertBrowserNavigationAllowed as S,
  requiresInspectableBrowserNavigationRedirectsForUrl as T,
  INTERACTIVE_ROLES as _,
  resolveOpenClawUserDataDir as a,
  resolveBrowserNavigationProxyMode as b,
  formatChromeCdpDiagnostic as c,
  createTargetViaCdp as d,
  formatAriaSnapshot as f,
  CONTENT_ROLES as g,
  snapshotRoleViaCdp as h,
  launchOpenClawChrome as i,
  AX_REF_PATTERN as l,
  snapshotAria as m,
  isChromeCdpReady as n,
  stopOpenClawChrome as o,
  normalizeCdpWsUrl as p,
  isChromeReachable as r,
  diagnoseChromeCdp as s,
  getChromeWebSocketUrl as t,
  captureScreenshot as u,
  STRUCTURAL_ROLES as v,
  assertBrowserNavigationResultAllowed as w,
  InvalidBrowserNavigationUrlError as x,
  ensureOutputDirectory as y,
};
