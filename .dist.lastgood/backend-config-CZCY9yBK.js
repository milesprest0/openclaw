...
import {
  a as resolveAgentWorkspaceDir,
  c as splitShellArgs,
  n as normalizeAgentId,
  r as parseDurationMs,
  s as resolveUserPath,
  t as CANONICAL_ROOT_MEMORY_FILENAME,
} from "./config-utils-CfeWwoBk.js";
import "./fs-utils-Bbrc6fw7.js";
import { i as isPathInside } from "./path-1liOXr_N.js";
import { t as normalizeLowercaseStringOrEmpty } from "./string-utils-CQS7ux9I.js";
...
function resolveMemoryBackendConfig(params) {
  ...
  const mergedExtraPaths = [
    ...
    .filter(Boolean);
  const searchExtraPaths = Array.from(new Set(mergedExtraPaths)).map((pathValue) => ({
    path: pathValue,
  }));
  const mergedExtraCollections = [
    ...
    (value) => value !== null && typeof value === "object" && typeof value.path === "string",
  );
  const allQmdPaths = [...(qmdCfg?.paths ?? []), ...searchExtraPaths, ...mergedExtraCollections];
  const collections = [
