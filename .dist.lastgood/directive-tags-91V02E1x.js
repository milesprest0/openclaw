...
function normalizeDirectiveWhitespace(text) {
  ...
  return text
    ...
    .replace(/\n{3,}/g, "\n\n")
    .trimEnd()
    .replace(blockPlaceholderRe, (_, i) => blocks[Number(i)]);
}
...
function parseInlineDirectives(text, options = {}) {
  ...
  const replyToId =
    lastExplicitId ?? (sawCurrent ? normalizeOptionalString(currentMessageId) : void 0);
  return {
    text: cleaned,
    audioAsVoice,
    replyToId,
...
//#endregion
export {
  stripInlineDirectiveTagsFromMessageForDisplay as a,
  stripInlineDirectiveTagsForDisplay as i,
  sanitizeReplyDirectiveId as n,
  stripInlineDirectiveTagsForDelivery as r,
  parseInlineDirectives as t,
};
