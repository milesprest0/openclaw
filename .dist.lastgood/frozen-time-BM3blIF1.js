...
import { i as beforeEach, n as afterEach } from "./dist-BosyQhji.js";
import { t as isTruthyEnvValue } from "./env-GyTZ5xFK.js";
import "./file-lock-Deo0YuAs.js";
import { r as drainFileLockStateForTest } from "./file-lock-C5nUdr1w.js";
import { q as loadShellEnvFallback } from "./io-CEQSCTGy.js";
import { t as getProviderEnvVars } from "./provider-env-vars-DkmNxQP4.js";
import { a as drainSessionWriteLockStateForTest } from "./session-write-lock-DjdS_616.js";
import {
  g as resolvePinnedHostnameWithPolicy,
  h as resolvePinnedHostname,
  y as ssrf_exports,
} from "./ssrf-DO8eIXaD.js";
import { u as drainSessionStoreWriterQueuesForTest } from "./store-BpWdoYPF.js";
import { m as clearSessionStoreCaches } from "./store-load-BSFLPYqQ.js";
import {
...
function installPinnedHostnameTestHooks() {
  ...
  beforeEach(() => {
    ...
    resolvePinnedHostnameSpy = vi
      .spyOn(ssrf_exports, "resolvePinnedHostname")
      .mockImplementation((hostname) => resolvePinnedHostname$2(hostname, lookupMock));
    resolvePinnedHostnameWithPolicySpy = vi
      .spyOn(ssrf_exports, "resolvePinnedHostnameWithPolicy")
      .mockImplementation((hostname, params) =>
        resolvePinnedHostnameWithPolicy$2(hostname, {
          ...params,
          lookupFn: lookupMock,
        }),
      );
...
function canRunBufferBackedVideoToVideoLiveLane(params) {
  ...
  return (
    (slash <= 0 || slash === params.modelRef.length - 1
      ...
      : params.modelRef.slice(slash + 1).trim()) === "gen4_aleph"
  );
}
...
function mockPinnedHostnameResolution(addresses = ["93.184.216.34"]) {
  ...
  const pinned = vi
    .spyOn(ssrf_exports, "resolvePinnedHostname")
    .mockImplementation((hostname) => resolvePinnedHostname$1(hostname, lookupFn));
  const pinnedWithPolicy = vi
    .spyOn(ssrf_exports, "resolvePinnedHostnameWithPolicy")
    .mockImplementation((hostname, params) =>
      resolvePinnedHostnameWithPolicy$1(hostname, {
        ...params,
        lookupFn,
      }),
    );
...
const HOME_ENV_KEYS = ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OPENCLAW_STATE_DIR"];
...
//#endregion
export {
  maybeLoadShellEnvForGenerationProviders as A,
  resolveLiveVideoResolution as C,
  parseLiveCsvFilter as D,
  resolveLiveMusicAuthStore as E,
  createAuthCaptureJsonFetch as F,
  createRequestCaptureJsonFetch as I,
  installPinnedHostnameTestHooks as L,
  extractNonEmptyAssistantText as M,
  isLiveProfileKeyModeEnabled as N,
  parseProviderModelMap as O,
  isLiveTestEnabled as P,
  withFetchPreconnect as R,
  resolveLiveVideoAuthStore as S,
  resolveConfiguredLiveMusicModels as T,
  requestUrl as _,
  withTempDir as a,
  canRunBufferBackedVideoToVideoLiveLane as b,
  withStateDirEnv as c,
  withEnv as d,
  withEnvAsync as f,
  requestBodyText as g,
  jsonResponse as h,
  makeResponse as i,
  createSingleUserPromptMessage as j,
  redactLiveApiKey as k,
  cleanupSessionStateForTest as l,
  mockPinnedHostnameResolution as m,
  useRealTime as n,
  createTempHomeEnv as o,
  createWindowsCmdShimFixture as p,
  createProviderUsageFetch as r,
  createMockServerResponse as s,
  useFrozenTime as t,
  captureEnv as u,
  DEFAULT_LIVE_VIDEO_MODELS as v,
  DEFAULT_LIVE_MUSIC_MODELS as w,
  resolveConfiguredLiveVideoModels as x,
  canRunBufferBackedImageToVideoLiveLane as y,
};
