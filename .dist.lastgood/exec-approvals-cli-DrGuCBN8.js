...
import { t as collectExecPolicyScopeSnapshots } from "./exec-approvals-effective-C-UKG0jF.js";
import {
...
import { a as resolveNodeId, r as nodesCallOpts } from "./rpc-BatpuIU2.js";
...
async function loadSnapshot(opts, nodeId) {
  return await callGatewayFromCli(
    nodeId ? "exec.approvals.node.get" : "exec.approvals.get",
    opts,
    nodeId ? { nodeId } : {},
  );
}
      ...
      .action(async (opts) => {
        try {
          ...
          renderApprovalsSnapshot(
            snapshot,
            source === "local" ? "local" : nodeId ? `node:${nodeId}` : "gateway",
          );
