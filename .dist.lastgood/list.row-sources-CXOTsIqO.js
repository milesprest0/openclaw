...
import { t as toModelRow } from "./list.model-row-_b83X2MO.js";
import "./defaults-4m7JJmD2.js";
import { _ as modelKey } from "./model-selection-shared-D9oQINZ2.js";
import {
  i as shouldSuppressBuiltInModelFromManifest,
...
import { f as isLocalBaseUrl } from "./shared-BbHHdmwP.js";
//#region src/commands/models/list.rows.ts
const modelCatalogModuleLoader = createLazyImportLoader(
  () => import("./model-catalog-iNiq3Mxw.js"),
);
const modelResolverModuleLoader = createLazyImportLoader(() => import("./model-B3VXLGMf.js"));
const providerCatalogModuleLoader = createLazyImportLoader(
  () => import("./list.provider-catalog-y-G1a-ns.js"),
);
...
async function buildRow(params) {
  ...
  const shouldResolveProviderAuth =
    params.context.availableKeys === void 0 || allowProviderAvailabilityFallback;
  return toModelRow({
    model: params.model,
    key: params.key,
...
function resolveConfiguredModelInput(params) {
  const input = Array.isArray(params.model.input)
    ? params.model.input.filter((item) => item === "text" || item === "image")
    : [];
...
async function appendConfiguredProviderRows(params) {
  for (const [provider, providerConfig] of Object.entries(
    ...
    for (const configuredModel of providerConfig.models ?? []) {
      if (
        !shouldListConfiguredProviderModel({
          providerConfig,
          model: configuredModel,
        })
      )
...
async function appendConfiguredRows(params) {
  ...
  for (const entry of params.entries) {
    ...
    const shouldResolveProviderAuth =
      model && (params.context.availableKeys === void 0 || allowProviderAvailabilityFallback);
    params.rows.push(
      toModelRow({
        model,
        key: entry.key,
