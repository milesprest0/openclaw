...
import {
  c as readLatestDiagnosticStabilityBundleSync,
  s as readDiagnosticStabilityBundleFileSync,
} from "./diagnostic-stability-bundle-Dw0f3LlZ.js";
import {
  i as textSupportBundleFile,
  n as jsonlSupportBundleFile,
  o as writeSupportBundleZip,
...
import {
  ...
  i as sanitizeSupportConfigValue,
  n as redactSupportString,
  r as redactTextForSupport,
...
import { o as parseConfigJson5 } from "./io-CEQSCTGy.js";
...
function safeScalar(value) {
  ...
  if (typeof value === "string")
    return redactTextForSupport(value) === value && /^[A-Za-z0-9_.:-]{1,120}$/u.test(value)
      ? value
      : "<redacted>";
}
...
function readStabilityBundle(target, stateDir) {
  ...
  if (target === void 0 || target === "latest")
...
async function collectSupportLogTail(params) {
  try {
    return sanitizeLogTail(
      await params.readLogTail({
        limit: params.limit,
...
async function buildDiagnosticSupportExport(options = {}) {
  ...
  for (const snapshot of [statusSnapshot, healthSnapshot])
    if (snapshot.file) files.push(snapshot.file);
  if (stability.status === "found")
