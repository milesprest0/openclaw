...
import { t as resolveEffectiveAgentSkillFilter } from "./agent-filter-CzvntJTZ.js";
import {
...
import {
  _ as normalizeBundlePathList,
  d as CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH,
  g as mergeBundlePathLists,
} from "./discovery-bnS95tO3.js";
import { u as parseFrontmatterBlock } from "./frontmatter-BAgsU2iK.js";
...
import "./config-kFPSWjDy.js";
import "./env-overrides-DyQKAXcm.js";
import {
...
import {
  ...
  i as filterWorkspaceSkillEntriesWithOptions,
} from "./workspace-FSRlf91o.js";
...
function resolveClaudeCommandRootDirs(rootDir) {
  ...
  return mergeBundlePathLists(
    fs.existsSync(path.join(rootDir, "commands")) ? ["commands"] : [],
    declared,
  );
}
...
function sanitizeSkillCommandName(raw) {
  return (
    normalizeLowercaseStringOrEmpty(raw)
      ...
      .replace(/_+/g, "_")
      .replace(/^_+|_+$/g, "")
      .slice(0, SKILL_COMMAND_MAX_LENGTH) || SKILL_COMMAND_FALLBACK
  );
}
...
function buildWorkspaceSkillCommandSpecs(workspaceDir, opts) {
  ...
  for (const entry of userInvocable) {
    ...
    const dispatch = (() => {
      ...
      if (!(!argModeRaw || argModeRaw === "raw" ? "raw" : null))
        debugSkillCommandOnce(
