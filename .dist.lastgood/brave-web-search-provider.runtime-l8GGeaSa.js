import {
  a as resolveBraveMode,
  i as resolveBraveConfig,
  n as normalizeBraveCountry,
  r as normalizeBraveLanguageParams,
  t as mapBraveLlmContextResults,
} from "./brave-web-search-provider.shared-LVKflKNe.js";
...
import { t as assertHttpUrlTargetsPrivateNetwork } from "./ssrf-policy-CLYqev3x.js";
...
import {
  ...
  b as withTrustedWebSearchEndpoint,
  d as readCachedSearchPayload,
  f as readConfiguredSecretString,
  g as resolveSearchTimeoutSeconds,
  h as resolveSearchCount,
  i as buildSearchCacheKey,
  l as parseIsoDateRange,
  m as resolveSearchCacheTtlMs,
  p as readProviderEnvValue,
  s as normalizeFreshness,
  x as writeCachedSearchPayload,
  y as withSelfHostedWebSearchEndpoint,
...
function resolveBraveBaseUrl(braveConfig) {
  return (
    ...
    )?.replace(/\/+$/u, "") || DEFAULT_BRAVE_BASE_URL
  );
}
...
async function runBraveLlmContextSearch(params) {
  ...
  if (params.freshness) url.searchParams.set("freshness", params.freshness);
    ...
    url.searchParams.set(
      "freshness",
      `${params.dateAfter}to${/* @__PURE__ */ new Date().toISOString().slice(0, 10)}`,
    );
...
async function executeBraveSearch(args, searchConfig, options) {
  ...
  const count =
    readNumberParam(args, "count", { integer: true }) ?? searchConfig?.maxResults ?? void 0;
  ...
  const llmContextDateEnd =
    braveMode === "llm-context" && dateAfter
      ? (dateBefore ?? /* @__PURE__ */ new Date().toISOString().slice(0, 10))
      : dateBefore;
