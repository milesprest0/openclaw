import { t as createApprovalNativeRouteReporter } from "./approval-native-route-coordinator-qDsAImhV.js";
...
import { l as readConnectErrorDetailCode } from "./connect-error-details-BXtaJDEs.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import { t as createOperatorApprovalsGatewayClient } from "./operator-approvals-client-Bc-BK66V.js";
...
var ExecApprovalChannelRuntimeTerminalStartError = class extends Error {
  constructor(info, cause) {
    super(
      `native approval gateway client paused reconnect after startup auth failure (${info.detailCode ?? "unknown"}): gateway closed (${info.code}): ${info.reason}`,
  ...
  const handleGatewayEvent = (evt) => {
    if (evt.event === "exec.approval.requested" && eventKinds.has("exec")) {
      spawn(
        ...
        handleRequested(evt.payload, { ignoreIfInactive: true }),
      );
      return;
    }
      ...
      startPromise = (async () => {
        ...
        const client = await createOperatorApprovalsGatewayClient({
          ...
          onClose: (code, reason) => {
            ...
            settleReady(() =>
              rejectReady(
                lastConnectError ?? /* @__PURE__ */ new Error(`gateway closed: ${code} ${reason}`),
              ),
            );
          },
        });
...
function createChannelNativeApprovalRuntime(adapter) {
  ...
  const resolveApprovalKind =
    adapter.resolveApprovalKind ?? ((request) => defaultResolveApprovalKind(request));
...
//#endregion
export {
  isExecApprovalChannelRuntimeTerminalStartError as a,
  createExecApprovalChannelRuntime as i,
  deliverApprovalRequestViaChannelNativePlan as n,
  resolveChannelNativeApprovalDeliveryPlan as o,
  ExecApprovalChannelRuntimeTerminalStartError as r,
  createChannelNativeApprovalRuntime as t,
};
