...
import { l as tryReadJson, u as tryReadJsonSync } from "./json-files-DifBk3kt.js";
...
async function assertCanonicalPathWithinBase(params) {
  ...
  if (baseLstat.isSymbolicLink()) {
    if (!(await fs$1.stat(baseDir)).isDirectory())
      throw new Error(
  ...
  const validateDirectory = async (dirPath) => {
    ...
    if (dirLstat.isSymbolicLink()) {
      if (resolvedDirPath !== baseDir)
        throw new Error(`Invalid path: must stay within ${params.boundaryLabel}`);
      if (!(await fs$1.stat(dirPath)).isDirectory())
        throw new Error(`Invalid path: must stay within ${params.boundaryLabel}`);
    } else if (!dirLstat.isDirectory())
      throw new Error(`Invalid path: must stay within ${params.boundaryLabel}`);
    if (!isPathInside(baseRealPath, await fs$1.realpath(dirPath)))
      throw new Error(`Invalid path: must stay within ${params.boundaryLabel}`);
  };
  try {
...
function readPersistedInstalledPluginIndexInstallRecordsSync(options = {}) {
  return extractPluginInstallRecordsFromPersistedInstalledPluginIndex(
    tryReadJsonSync(resolveInstalledPluginIndexStorePath(options)),
  );
}
...
//#endregion
export {
  assertCanonicalPathWithinBase as _,
  readPersistedInstalledPluginIndexInstallRecordsSync as a,
  matchesExpectedPluginId as c,
  resolveDefaultPluginNpmDir as d,
  resolvePluginInstallDir as f,
  unscopedPackageName as g,
  packageNameMatchesId as h,
  readPersistedInstalledPluginIndexInstallRecords as i,
  resolveDefaultPluginExtensionsDir as l,
  validatePluginId as m,
  loadInstalledPluginIndexInstallRecords as n,
  resolveInstalledPluginIndexStorePath as o,
  safePluginInstallFileName as p,
  loadInstalledPluginIndexInstallRecordsSync as r,
  encodePluginInstallDirName as s,
  listChannelCatalogEntries as t,
  resolveDefaultPluginGitDir as u,
  resolveSafeInstallDir as v,
  safePathSegmentHashed as y,
};
