import { i as canResolveEnvSecretRefInReadOnlyPath } from "./extension-shared-B_zNy4Rm.js";
...
import { h as resolveSecretInputString } from "./types.secrets-CaNC1eIn.js";
import { a as truncateText, r as markdownToText } from "./web-fetch-utils-Bpv806jB.js";
...
import "./provider-web-fetch-B8N6Qi7Q.js";
...
function resolveFirecrawlMaxAgeMs(cfg, override) {
  ...
  const fetch = resolveFirecrawlFetchConfig(cfg);
  if (typeof fetch?.maxAgeMs === "number" && Number.isFinite(fetch.maxAgeMs) && fetch.maxAgeMs >= 0)
...
async function postFirecrawlJson(params, parse) {
  ...
  return await (
    (params.mode ?? (await validateFirecrawlBaseUrl(params.url))) === "selfHosted"
...
function resolveSearchItems(payload) {
  ...
  for (const entry of rawItems) {
    ...
    const record = entry;
    const metadata =
      record.metadata && typeof record.metadata === "object" ? record.metadata : void 0;
    const url =
...
function parseFirecrawlScrapePayload(params) {
  ...
  return {
    ...
    title:
      typeof metadata?.title === "string" && metadata.title
        ? wrapExternalContent(metadata.title, {
            source: "web_fetch",
...
//#endregion
export {
  runFirecrawlSearch as a,
  runFirecrawlScrape as i,
  assertFirecrawlScrapeTargetAllowed as n,
  parseFirecrawlScrapePayload as r,
  __testing as t,
};
