...
import {
  ...
  f as readConfigFileSnapshotWithPluginMetadata,
} from "./io-CEQSCTGy.js";
...
} from "./runtime-gateway-auth-surfaces-Cn7J2KV2.js";
import {
...
function createRuntimeSecretsActivator(params) {
  ...
  const loadSecretsRuntime = () => {
    secretsRuntimePromise ??= import("./runtime-B4KwL_dj.js");
    return secretsRuntimePromise;
  };
  const loadAuthProfiles = () => {
    authProfilesPromise ??= import("./auth-profiles-BJLtjnpj.js");
...
async function prepareGatewayStartupConfig(params) {
  ...
  const preflightConfig = hasActiveGatewayAuthSecretRef(startupPreflightConfig)
    ? (
