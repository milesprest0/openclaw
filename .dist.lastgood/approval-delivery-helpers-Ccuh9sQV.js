...
function buildApproverRestrictedNativeApprovalCapability(params) {
  ...
  const normalizePreferredSurface = (mode) =>
    mode === "channel" ? "origin" : mode === "dm" ? "approver-dm" : "both";
  const hasConfiguredApprovers = ({ cfg, accountId }) =>
    params.hasApprovers({
      cfg,
      accountId,
    });
  const isExecInitiatingSurfaceEnabled = ({ cfg, accountId }) =>
  ...
  const resolveExecInitiatingSurfaceState = ({ cfg, accountId }) =>
    availabilityState(
      isExecInitiatingSurfaceEnabled({
        cfg,
        accountId,
      }),
    );
  return createChannelApprovalCapability({
    ...
    delivery: {
      ...
      shouldSuppressForwardingFallback: (input) => {
        if (
          (normalizeMessageChannel(input.target.channel) ?? input.target.channel) !== params.channel
        )
          return false;
        if (params.requireMatchingTurnSourceChannel) {
          if (normalizeMessageChannel(input.request.request.turnSourceChannel) !== params.channel)
            return false;
        }
        const resolvedAccountId = params.resolveSuppressionAccountId?.(input);
        const accountId =
          (resolvedAccountId === void 0
            ? input.target.accountId?.trim()
            : resolvedAccountId.trim()) || void 0;
...
//#endregion
export {
  splitChannelApprovalCapability as i,
  createApproverRestrictedNativeApprovalCapability as n,
  createChannelApprovalCapability as r,
  createApproverRestrictedNativeApprovalAdapter as t,
};
