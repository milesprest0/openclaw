...
function describeSecureMobilePairingFix(source) {
  return (
    ...
    " Fix: use a private LAN address, prefer gateway.tailscale.mode=serve, or set gateway.remote.url / plugins.entries.device-pair.config.publicUrl to a wss:// URL. ws:// setup codes are only valid for localhost/loopback, private LAN addresses, .local hosts, or the Android emulator."
  );
}
...
function resolveAuthLabel(cfg) {
  ...
  const token =
    pickFirstDefined([process.env.OPENCLAW_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? void 0;
  const password =
    pickFirstDefined([process.env.OPENCLAW_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    void 0;
  if (mode === "token" || mode === "password")
    return resolveRequiredAuthLabel(mode, {
      token,
      password,
    });
...
async function sendQrPngToSupportedChannel(params) {
  ...
  const send = (await params.api.runtime.channel.outbound.loadAdapter(params.ctx.channel))
    ?.sendMedia;
  ...
  register(api) {
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        api.logger.info?.(
          `device-pair: /pair invoked channel=${ctx.channel} sender=${ctx.senderId ?? "unknown"} action=${action || "new"}`,
        );
        ...
        if (action === "qr") {
          ...
          if (channel === "telegram" && target)
            try {
              ...
              api.logger.warn?.(
                `device-pair: failed to arm one-shot pairing notify (${err?.message ?? err})`,
              );
            }
          ...
          if (target && canSendQrPngToChannel(channel)) {
            ...
            try {
              ...
              if (
                await sendQrPngToSupportedChannel({
                  api,
                  ctx,
                  target,
              ...
              api.logger.warn?.(
                `device-pair: QR image send failed channel=${channel}, falling back (${err?.message ?? err})`,
              );
          ...
          if (channel === "webchat") {
            ...
            try {
              ...
              const { revokeDeviceBootstrapToken } = await loadDevicePairApiModule();
              api.logger.warn?.(
                `device-pair: webchat QR render failed, falling back (${err?.message ?? err})`,
              );
              await revokeDeviceBootstrapToken({ token: payload.bootstrapToken }).catch(() => {});
              payload = await issueSetupPayload(urlResult.url);
              return {
        ...
        if (channel === "telegram" && target)
          try {
            ...
            api.logger.debug?.(
              `device-pair: runtime keys=${runtimeKeys.join(",") || "none"} channel keys=${channelKeys.join(",") || "none"}`,
            );
            ...
            api.logger.info?.(
              `device-pair: telegram split send ok target=${target} account=${ctx.accountId ?? "none"} thread=${ctx.messageThreadId ?? "none"}`,
            );
            return { text: encodeSetupCode(payload) };
          } catch (err) {
            api.logger.warn?.(
              `device-pair: telegram split send failed, falling back to single message (${err?.message ?? err})`,
            );
          }
