...
import {
  a as DIR_LIST_TOOL_DESCRIPTOR,
  c as FILE_FETCH_TOOL_DESCRIPTOR,
  d as FILE_WRITE_TOOL_DESCRIPTOR,
  f as appendFileTransferAudit,
  r as DIR_FETCH_TOOL_DESCRIPTOR,
} from "../../descriptors-_YUnOSqX.js";
import { i as getRuntimeConfig } from "../../io-CEQSCTGy.js";
import { n as mutateConfigFile } from "../../mutate-IzL9dVFH.js";
...
async function persistAllowAlways(input) {
  ...
  await mutateConfigFile({
    ...
    mutate: (draft) => {
      ...
      let key = [input.nodeId, input.nodeDisplayName]
        .filter((k) => typeof k === "string" && k.length > 0)
        .find((c) => Object.prototype.hasOwnProperty.call(fileTransfer, c));
      if (!key) {
...
const COMMANDS = ["file.fetch", "dir.list", "dir.fetch", "file.write"];
...
async function listDirFetchArchiveEntries(payload) {
  ...
  return await new Promise((resolve) => {
    const child = spawn(process.platform !== "win32" ? "/usr/bin/tar" : "tar", ["-tzf", "-"], {
...
async function handleFileTransferInvoke(ctx) {
  if (!COMMANDS.includes(ctx.command))
    return {
      ok: false,
  ...
  nodeHostCommands: [
    {
      ...
      handle: async (paramsJSON) => {
        const { handleFileFetch } = await import("../../file-fetch-alsxW5lm.js");
    ...
    },
    {
      ...
      dangerous: true,
      handle: async (paramsJSON) => {
        const { handleDirList } = await import("../../dir-list-yZpJBysQ.js");
    ...
    },
    {
      ...
      dangerous: true,
      handle: async (paramsJSON) => {
        const { handleDirFetch } = await import("../../dir-fetch-CMsRVJ8I.js");
    ...
    },
    {
      ...
      dangerous: true,
      handle: async (paramsJSON) => {
        const { handleFileWrite } = await import("../../file-write-Dsijb2yS.js");
  ...
  register(api) {
    ...
    api.registerTool(
      createLazyTool(FILE_FETCH_TOOL_DESCRIPTOR, async () => {
        const { createFileFetchTool } = await import("../../file-fetch-tool-CrrcOGxo.js");
        return createFileFetchTool();
      }),
    );
    api.registerTool(
      createLazyTool(DIR_LIST_TOOL_DESCRIPTOR, async () => {
        const { createDirListTool } = await import("../../dir-list-tool-BnSYcW4-.js");
        return createDirListTool();
      }),
    );
    api.registerTool(
      createLazyTool(DIR_FETCH_TOOL_DESCRIPTOR, async () => {
        const { createDirFetchTool } = await import("../../dir-fetch-tool-DyeqGkyw.js");
        return createDirFetchTool();
      }),
    );
    api.registerTool(
      createLazyTool(FILE_WRITE_TOOL_DESCRIPTOR, async () => {
        const { createFileWriteTool } = await import("../../file-write-tool-wJvxEimU.js");
