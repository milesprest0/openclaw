import {
  ...
  r as resolveBrowserExecutableForPlatform,
...
function collectChromeMcpProfiles(cfg) {
  ...
  if ((normalizeOptionalString(browser.defaultProfile) ?? "") === "user")
...
function collectManagedProfiles(cfg) {
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles))
    if (
      (normalizeOptionalString(asNullableRecord(rawProfile)?.driver) ?? "openclaw") !==
      "existing-session"
    )
