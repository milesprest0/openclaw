...
import { r as listEnabledFeishuAccounts } from "../../accounts-CFTFwStL.js";
import {
  a as setFeishuNamedAccountEnabled,
  i as feishuSetupAdapter,
  n as feishuSetupWizard,
  r as runFeishuLogin,
  t as feishuPlugin,
} from "../../channel-BIUNcQ0l.js";
...
import {
  a as jsonToolResult,
  d as registerFeishuChatTools,
  f as createFeishuToolClient,
  m as resolveFeishuToolAccount,
  n as registerFeishuDriveTools,
  o as toolExecutionErrorResult,
  p as resolveAnyEnabledFeishuToolsConfig,
  s as unknownToolActionResult,
...
import {
  n as handleFeishuSubagentEnded,
  r as handleFeishuSubagentSpawning,
  t as handleFeishuSubagentDeliveryTarget,
} from "../../subagent-hooks-DbyLwFNq.js";
import {
  n as createFeishuThreadBindingManager,
  r as getFeishuThreadBindingManager,
  t as __testing,
} from "../../thread-bindings-DgZ8SMi8.js";
...
async function insertBlocksInBatches(
  ...
  for (const firstLevelId of firstLevelBlockIds) {
    const newBlocks = collectDescendants(blockMap, firstLevelId).filter(
      (b) => b.block_id && !usedBlockIds.has(b.block_id),
    );
...
function normalizeConvertedBlockTree(blocks, firstLevelIds) {
  ...
  const inferredTopLevelIds = blocks
    ...
    .filter((blockId) => typeof blockId === "string");
  const rootIds = (
...
async function resolveUploadInput(
  ...
  if (imageInput) {
    ...
    if (unambiguousPath || (absolutePath && existsSync(candidate))) {
      ...
      return {
        buffer: (
          await getFeishuRuntime().media.loadWebMedia(resolvedPath, {
            maxBytes,
  ...
  return {
    buffer: (
      await getFeishuRuntime().media.loadWebMedia(resolvedFilePath, {
        maxBytes,
        optimizeImages: false,
        localRoots,
      })
    ...
    api.registerTool(
      (ctx) => {
        ...
        const trustedRequesterOpenId =
          ctx.messageChannel === "feishu" ? normalizeOptionalString(ctx.requesterSenderId) : void 0;
        return {
          name: "feishu_doc",
          label: "Feishu Doc",
          description:
          ...
          async execute(_toolCallId, params) {
            ...
            try {
              ...
              switch (p.action) {
                case "read":
                  return json$1(await readDoc(client, p.doc_token));
                case "write":
                  return json$1(
                    await writeDoc(
                      client,
                ...
                case "list_blocks":
                  return json$1(await listBlocks(client, p.doc_token));
                case "get_block":
                  return json$1(await getBlock(client, p.doc_token, p.block_id));
                case "update_block":
                  return json$1(await updateBlock(client, p.doc_token, p.block_id, p.content));
                case "delete_block":
                  return json$1(await deleteBlock(client, p.doc_token, p.block_id));
                case "create_table":
                  return json$1(
                    await createTable(
                      client,
                ...
                case "color_text":
                  return json$1(await updateColorText(client, p.doc_token, p.block_id, p.content));
                case "insert_table_row":
                  return json$1(await insertTableRow(client, p.doc_token, p.block_id, p.row_index));
                ...
                default:
                  return json$1({ error: "Unknown action" });
              }
            } catch (err) {
...
function isDefaultEmptyBitableFieldValue(value) {
  ...
  if (typeof value === "object") {
    ...
    if ("text" in record && keys.every((key) => key === "text" || key === "type"))
      return record.text === void 0 || record.text === null || record.text === "";
