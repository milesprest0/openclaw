...
import {
  i as parseVoiceCallPluginConfig,
  r as normalizeVoiceCallLegacyConfigInput,
  t as formatVoiceCallLegacyConfigWarnings,
} from "../../config-compat-DHLQ7F0B.js";
import { i as resolveVoiceCallConfig, s as validateProviderConfig } from "../../config-Dbj1icPs.js";
...
import {
  a as setupTailscaleExposureRoute,
  i as getTailscaleSelfInfo,
  n as resolveWebhookExposureStatus,
  r as cleanupTailscaleExposureRoute,
  s as resolveUserPath,
  t as createVoiceCallRuntime,
} from "../../runtime-entry-CaZWUKh6.js";
import {
...
async function callVoiceCallGateway(method, params, opts) {
  try {
    ...
    return {
      ok: true,
      payload: await voiceCallCliDeps.callGatewayFromCli(
        method,
        {
          json: true,
...
async function pollVoiceCallContinueGateway(params) {
  ...
  while (Date.now() <= deadlineMs) {
    ...
    if (!gateway.ok)
      throw new Error(
        `gateway unavailable while waiting for voicecall continue result: ${formatErrorMessage(gateway.error)}`,
      );
...
function registerVoiceCallCli(params) {
  ...
  root
    ...
    .action(async (options) => {
      ...
      if (gateway.ok) callId = isRecord(gateway.payload) ? gateway.payload.callId : void 0;
        ...
        const result = await (
          await ensureRuntime()
        ).manager.initiateCall(options.to, void 0, {
    ...
    .action(async (options) => {
      ...
      if (gateway.ok) {
        if (isRecord(gateway.payload) && typeof gateway.payload.operationId === "string") {
          writeStdoutJson(
            await pollVoiceCallContinueGateway({
      ...
      const result = await (
        await ensureRuntime()
      ).manager.continueCall(options.callId, options.message);
  ...
  root
    ...
    .action(async (options) => {
      const gateway = await callVoiceCallGateway(
        "voicecall.status",
        options.callId ? { callId: options.callId } : void 0,
      );
      if (gateway.ok) {
...
var voice_call_default = definePluginEntry({
  ...
  register(api) {
    ...
    const describeHistoricalCall = async (rt, callId) => {
      const call = (await rt.manager.getCallHistory(100))
        .toReversed()
      ...
      async ({ params, respond }) => {
        try {
          ...
          await initiateCallAndRespond({
            ...
            message,
            mode:
              params?.mode === "notify" || params?.mode === "conversation" ? params.mode : void 0,
