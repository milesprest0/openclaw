...
import {
  n as findGoogleMeetCalendarEvent,
  r as listGoogleMeetCalendarEvents,
  t as buildGoogleMeetCalendarDayWindow,
} from "../../calendar-B9ppM8Hw.js";
import "../../text-runtime-lKuAtsoz.js";
import {
  a as isSameMeetUrlForReuse,
  c as resolveChromeNode,
  f as endGoogleMeetActiveConference,
  g as fetchLatestGoogleMeetConferenceRecord,
  h as fetchGoogleMeetSpace,
  i as callBrowserProxyOnNode,
  l as resolveChromeNodeInfo,
  m as fetchGoogleMeetAttendance,
  n as isGoogleMeetBrowserManualActionError,
  o as normalizeMeetUrlForReuse,
  p as fetchGoogleMeetArtifacts,
  r as asBrowserTabs,
  s as readBrowserTab,
  t as createMeetWithBrowserProxyOnNode,
  u as buildGoogleMeetPreflightReport,
} from "../../chrome-create-SD1kQYWf.js";
import "../../routing-BccEMOrJ.js";
import "../../error-runtime-CdKGkPCx.js";
import "../../runtime-env-B60JdRoI.js";
import "../../ssrf-runtime-BhFjkd3c.js";
import "../../gateway-runtime-BGDxySPN.js";
...
import "../../realtime-transcription-AhUF6eZN.js";
import { a as formatErrorMessage } from "../../errors-DZMrVkYL.js";
import { n as callGatewayFromCli } from "../../gateway-rpc-DjTKEsJd.js";
...
import {
  n as getRealtimeTranscriptionProvider,
  r as listRealtimeTranscriptionProviders,
} from "../../provider-registry-DX6y2NsT.js";
import {
  c as getRealtimeVoiceTranscriptHealth,
  d as recordRealtimeVoiceTranscript,
  i as resamplePcm,
  l as isLikelyRealtimeVoiceAssistantEchoTranscript,
  m as consultRealtimeVoiceAgent,
  n as mulawToPcm,
  o as extendRealtimeVoiceOutputEchoSuppression,
  p as createRealtimeVoiceAgentTalkbackQueue,
  s as getRealtimeVoiceBridgeEventHealth,
  t as convertPcmToMulaw8k,
  u as recordRealtimeVoiceBridgeEvent,
} from "../../realtime-voice-CgtYi1ED.js";
...
import {
  ...
  _ as resolveRealtimeVoiceAgentConsultToolPolicy,
  b as createTalkSessionController,
  c as REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME,
  k as REALTIME_VOICE_AUDIO_FORMAT_G711_ULAW_8KHZ,
  n as resolveConfiguredRealtimeVoiceProvider,
  p as buildRealtimeVoiceAgentConsultWorkingResponse,
  t as createRealtimeVoiceBridgeSession,
  v as resolveRealtimeVoiceAgentConsultTools,
  y as resolveRealtimeVoiceAgentConsultToolsAllow,
} from "../../session-runtime-B8HmMzEf.js";
import { c as isBlockedHostnameOrIp } from "../../ssrf-DO8eIXaD.js";
import {
...
const DEFAULT_GOOGLE_MEET_AUDIO_INPUT_COMMAND = withSoxBuffer(
  DEFAULT_GOOGLE_MEET_AUDIO_INPUT_COMMAND_BASE,
  DEFAULT_GOOGLE_MEET_AUDIO_BUFFER_BYTES,
);
const DEFAULT_GOOGLE_MEET_AUDIO_OUTPUT_COMMAND = withSoxBuffer(
...
function resolveGoogleMeetConfigWithEnv(input, env = process.env) {
  ...
  const hasCustomAudioCommand =
    configuredAudioInputCommand !== void 0 || configuredAudioOutputCommand !== void 0;
  const audioFormat =
  ...
  return {
    ...
    defaults: {
      meeting:
        normalizeOptionalString$1(defaults.meeting) ??
    ...
    defaultTransport: resolveTransport$1(
    ...
    chrome: {
      ...
      guestName:
        normalizeOptionalString$1(chrome.guestName) ?? DEFAULT_GOOGLE_MEET_CONFIG.chrome.guestName,
    ...
    realtime: {
      ...
      transcriptionProvider:
        normalizeOptionalString$1(realtime.transcriptionProvider) ??
      ...
      instructions:
        normalizeOptionalString$1(realtime.instructions) ??
    ...
    oauth: {
      clientId:
        normalizeOptionalString$1(oauth.clientId) ??
        normalizeOptionalString$1(auth.clientId) ??
        readEnvString(env, GOOGLE_MEET_CLIENT_ID_KEYS),
      clientSecret:
        normalizeOptionalString$1(oauth.clientSecret) ??
        normalizeOptionalString$1(auth.clientSecret) ??
        readEnvString(env, GOOGLE_MEET_CLIENT_SECRET_KEYS),
      refreshToken:
        normalizeOptionalString$1(oauth.refreshToken) ??
        readEnvString(env, GOOGLE_MEET_REFRESH_TOKEN_KEYS),
      accessToken:
        normalizeOptionalString$1(oauth.accessToken) ??
...
async function consultOpenClawAgentForGoogleMeet(params) {
  ...
  const requesterSessionKey =
    normalizeOptionalString$1(params.requesterSessionKey) ?? `agent:${agentId}:main`;
...
function convertGoogleMeetTtsAudioForBridge(audio, sampleRate, config, outputFormat) {
  ...
  return config.chrome.audioFormat === "g711-ulaw-8khz"
    ...
    : resamplePcm(pcm, sampleRate, 24e3);
}
...
async function startCommandAgentAudioBridge(params) {
  ...
  const outputProcess = spawnFn(output.command, output.args, { stdio: ["pipe", "ignore", "pipe"] });
...
async function startCommandRealtimeAudioBridge(params) {
  ...
  const startHumanBargeInMonitor = () => {
    ...
    bargeInInputProcess.stdout?.on("data", (chunk) => {
      ...
      const now = Date.now();
      if (
        !(now <= Math.max(lastOutputPlayableUntilMs, suppressInputUntil)) &&
        now - lastOutputAtMs > 1e3
      )
        return;
      ...
      params.logger.debug?.(
        `[google-meet] human barge-in detected by local input (rms=${Math.round(stats.rms)}, peak=${stats.peak})`,
      );
    });
...
async function launchChromeMeet(params) {
  ...
  const audioBridge =
    isGoogleMeetTalkBackMode$2(params.mode) &&
...
function parsePermissionGrantNotes(result) {
  ...
  const unsupportedPermissions = Array.isArray(record.unsupportedPermissions)
    ? record.unsupportedPermissions.filter((value) => typeof value === "string")
    : [];
...
async function grantMeetMediaPermissions(params) {
  ...
  try {
    return parsePermissionGrantNotes(
    ...
    return [
      `Could not grant Meet media permissions automatically: ${error instanceof Error ? error.message : String(error)}`,
    ];
  }
}
...
async function openMeetWithBrowserRequest(params) {
  ...
  do {
    try {
      ...
      if (
        browser?.inCall === true &&
        (!isGoogleMeetTalkBackMode$2(params.mode) || browser.micMuted !== true)
      )
        return {
          launched: true,
          browser,
      ...
      browser = {
        ...
        notes: [
          ...permissionNotes,
          `Browser control could not inspect or auto-join Meet: ${error instanceof Error ? error.message : String(error)}`,
        ],
      };
      break;
    }
...
async function launchChromeMeetOnNode(params) {
  ...
  try {
    ...
    params.logger.debug?.(
      `[google-meet] node bridge cleanup before join ignored: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  const result = parseNodeStartResult(
    await params.runtime.nodes.invoke({
      nodeId,
      command: "googlemeet.chrome",
      params: {
...
function startOutputProcess(command) {
  return spawn(command.command, command.args, { stdio: ["pipe", "ignore", "pipe"] });
}
function startCommandPair(params) {
  ...
  const inputProcess = spawn(input.command, input.args, { stdio: ["ignore", "pipe", "pipe"] });
...
function normalizeMeetKey(value) {
  ...
  try {
    ...
    return (
      /^\/([a-z]{3}-[a-z]{4}-[a-z]{3})(?:$|[/?#])/i.exec(url.pathname)?.[1]?.toLowerCase() ?? value
    );
  } catch {
    return value;
  }
...
function getGoogleMeetSetupStatus(config, options) {
  ...
  if (
    ...
    const voiceCallEnabled =
      Object.hasOwn(pluginEntries, "voice-call") && voiceCallEntry.enabled !== false;
    checks.push({
  ...
  try {
    ...
    if (params.message) {
      ...
      if (spoken.success === false)
        params.logger?.warn?.(
          `[google-meet] Skipped intro speech because realtime bridge was not ready: ${spoken.error || "voicecall.speak failed"}`,
        );
      else {
        introSent = true;
        params.logger?.info(
...
var GoogleMeetRuntime = class {
  ...
  async setupStatus(options = {}) {
    ...
    const twilioDialInNumber =
      transport === "twilio" ? normalizeDialInNumber(options.dialInNumber) : void 0;
  ...
  async join(request) {
    ...
    if (reusable) {
      ...
      const spoken =
        isGoogleMeetTalkBackMode$1(mode) && speechInstructions
      ...
      if (transport === "chrome" || transport === "chrome-node") {
        ...
        session.notes.push(
          result.audioBridge
            ...
            : isGoogleMeetTalkBackMode$1(mode)
        ...
        const voiceCallResult = this.params.config.voiceCall.enabled
          ? await joinMeetViaVoiceCallGateway({
              ...
              message: isGoogleMeetTalkBackMode$1(mode)
                ? (request.message ??
    ...
    const startOutputBytes =
      beforeSessions.find(
        (session) =>
          ...
          isSameMeetUrlForReuse(session.url, url) &&
          session.transport === transport &&
          isGoogleMeetTalkBackMode$1(session.mode),
      )?.chrome?.health?.lastOutputBytes ?? 0;
  ...
  async testListen(request) {
    ...
    if (requestedMode && isGoogleMeetTalkBackMode$1(requestedMode))
      throw new Error(
    ...
    const beforeSessions = this.list();
    const before = new Set(beforeSessions.map((session) => session.id));
    const start = transcriptCheckpoint(
      beforeSessions.find(
        (session) =>
          session.state === "active" &&
          isSameMeetUrlForReuse(session.url, url) &&
  ...
  async #refreshTwilioVoiceCallStatus(session) {
    ...
    try {
      if (
        (
          await getMeetVoiceCallGatewayCall({
            config: this.params.config,
            callId,
          })
        ).found === false
  ...
  async #ensureChromeRealtimeBridge(session) {
    if (
      ...
      return;
    const health = session.chrome.health;
    if (health?.inCall !== true || health.micMuted === true || health.manualActionRequired === true)
      return;
...
const GoogleMeetToolSchema = Type.Object({
  ...
  dryRun: Type.Optional(
    Type.Boolean({ description: "For export, return the manifest without writing files." }),
  ),
...
async function createMeetFromParams(params) {
  return (await import("../../create-BxqDy3hU.js")).createMeetFromParams(params);
}
async function createAndJoinMeetFromParams(params) {
  return (await import("../../create-BxqDy3hU.js")).createAndJoinMeetFromParams(params);
}
async function resolveGoogleMeetTokenFromParams(config, raw) {
  const { resolveGoogleMeetAccessToken } = await import("../../oauth-BHlZBIt8.js");
...
async function resolveArtifactQueryFromParams(config, raw) {
  ...
  const token = await resolveGoogleMeetTokenFromParams(config, raw);
  const resolvedMeeting = conferenceRecord
    ? { meeting }
    : wantsCalendarLookup(raw)
      ? await resolveMeetingFromParams({
          config,
          raw,
          accessToken: token.accessToken,
        })
...
async function exportGoogleMeetBundleFromParams(config, raw) {
  ...
  const request = {
    ...
    allConferenceRecords: resolved.allConferenceRecords,
    mergeDuplicateParticipants: resolved.mergeDuplicateParticipants,
    ...(resolved.lateAfterMinutes !== void 0
      ? { lateAfterMinutes: resolved.lateAfterMinutes }
      : {}),
    ...(resolved.earlyBeforeMinutes !== void 0
      ? { earlyBeforeMinutes: resolved.earlyBeforeMinutes }
      : {}),
  };
  ...
  const outputDir =
    normalizeOptionalString$1(raw.outputDir) ?? normalizeOptionalString$1(raw.output);
  return {
  ...
  register(api) {
    ...
    api.registerGatewayMethod("googlemeet.status", async ({ params, respond }) => {
      try {
        respond(
          true,
          await (await ensureRuntime()).status(normalizeOptionalString$1(params?.sessionId)),
        );
      } catch (err) {
        sendError(respond, err);
      }
    ...
    api.registerGatewayMethod("googlemeet.leave", async ({ params, respond }) => {
      try {
        ...
        if (!sessionId) {
          sendError(
            respond,
            /* @__PURE__ */ new Error("sessionId required"),
    ...
    api.registerGatewayMethod("googlemeet.speak", async ({ params, respond }) => {
      try {
        ...
        respond(
          ...
          await (
            await ensureRuntime()
          ).speak(sessionId, normalizeOptionalString$1(params?.message)),
        );
      } catch (err) {
        sendError(respond, err);
      }
        ...
        async execute(_toolCallId, params) {
          ...
          try {
            ...
            switch (raw.action) {
              ...
              case "leave":
                if (!normalizeOptionalString$1(raw.sessionId))
    ...
    api.registerCli(
      async ({ program }) => {
        const { registerGoogleMeetCli } = await import("../../cli-BEhQeuJU.js");
