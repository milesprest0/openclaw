...
import {
  c as canvasConfigSchema,
  f as resolveCanvasHostConfig,
  l as isCanvasHostEnabled,
  m as registerNodesCanvasCommands,
  p as createDefaultCanvasCliDependencies,
  s as resolveCanvasHttpPathToLocalPath,
  t as createCanvasHostHandler,
} from "../../server-VL6-1VzK.js";
...
function createCanvasHttpRouteHandler(params) {
  ...
  return {
    async handleHttpRequest(req, res) {
      ...
      if (url.pathname === "/__openclaw__/a2ui" || url.pathname.startsWith(`/__openclaw__/a2ui/`))
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "eval": {
          const result = (
            await invoke("canvas.eval", {
              javaScript: readStringParam(params, "javaScript", { required: true }),
            })
          )?.payload?.result;
          if (result)
            return {
