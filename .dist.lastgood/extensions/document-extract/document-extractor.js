...
async function loadCanvasModule() {
  if (!canvasModulePromise)
    canvasModulePromise = import(CANVAS_MODULE).catch((err) => {
...
async function loadPdfJsModule() {
  if (!pdfJsModulePromise)
    pdfJsModulePromise = import(PDFJS_MODULE).catch((err) => {
...
async function extractPdfContent(request) {
  ...
  const effectivePages = request.pageNumbers
  ...
  for (const pageNum of effectivePages) {
    const pageText = (await (await pdf.getPage(pageNum)).getTextContent()).items
