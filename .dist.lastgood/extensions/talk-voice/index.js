...
import "../../talk-config-runtime-CwQ4zvC2.js";
import "../../api-BK-KwhiM.js";
...
- provider: ${providerId}\n- talk.providers.${providerId}.voiceId: ${currentVoiceId ? currentVoiceId : "(unset)"}\n- ${providerId}.apiKey: ${apiKey ? mask(apiKey) : "(unset)"}`,
        ...
        if (action === "list") {
          ...
          try {
            return {
              text: formatVoiceList(
                await api.runtime.tts.listVoices({
                  provider: providerId,
                  cfg,
        ...
        if (action === "set") {
          ...
          if (!chosen)
            return {
              text: `No voice found for ${isLikelyVoiceId(query) ? query : `"${query}"`}. Try: ${commandLabel} list`,
            };
          const nextConfig = {
            ...cfg,
            talk: {
          ...
          return {
            text: `✅ ${providerLabel} Talk voice set to ${(chosen.name ?? "").trim() || "(unnamed)"}\n${chosen.id}`,
          };
        }
        return {
          text: [
