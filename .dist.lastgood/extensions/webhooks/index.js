...
import "../../api-C8c9cYZ9.js";
import "../../runtime-api-CSK9uzzs.js";
//#region extensions/webhooks/src/config.ts
const secretRefSchema = zod_exports.z
  .object({
    source: zod_exports.z.enum(["env", "file", "exec"]),
    provider: zod_exports.z.string().trim().min(1),
    id: zod_exports.z.string().trim().min(1),
  })
  .strict();
const secretInputSchema = zod_exports.z.union([
  zod_exports.z.string().trim().min(1),
  secretRefSchema,
]);
const webhookRouteConfigSchema = zod_exports.z
  .object({
  ...
  .strict();
const webhooksPluginConfigSchema = zod_exports.z
  .object({
    routes: zod_exports.z
      .record(zod_exports.z.string().trim().min(1), webhookRouteConfigSchema)
      .default({}),
  })
  .strict();
...
const jsonValueSchema = zod_exports.z.lazy(() =>
  zod_exports.z.union([
    zod_exports.z.null(),
...
const createFlowRequestSchema = zod_exports.z
  .object({
    action: zod_exports.z.literal("create_flow"),
    controllerId: zod_exports.z.string().trim().min(1).optional(),
    goal: zod_exports.z.string().trim().min(1),
    status: zod_exports.z.enum(["queued", "running", "waiting", "blocked"]).optional(),
    notifyPolicy: zod_exports.z.enum(["done_only", "state_changes", "silent"]).optional(),
    currentStep: nullableStringSchema,
    stateJson: jsonValueSchema.nullable().optional(),
    waitJson: jsonValueSchema.nullable().optional(),
  })
  .strict();
const getFlowRequestSchema = zod_exports.z
  .object({
    action: zod_exports.z.literal("get_flow"),
    flowId: zod_exports.z.string().trim().min(1),
  })
  .strict();
const listFlowsRequestSchema = zod_exports.z
  .object({ action: zod_exports.z.literal("list_flows") })
  .strict();
const findLatestFlowRequestSchema = zod_exports.z
  .object({ action: zod_exports.z.literal("find_latest_flow") })
  .strict();
const resolveFlowRequestSchema = zod_exports.z
  .object({
    action: zod_exports.z.literal("resolve_flow"),
    token: zod_exports.z.string().trim().min(1),
  })
  .strict();
const getTaskSummaryRequestSchema = zod_exports.z
  .object({
    action: zod_exports.z.literal("get_task_summary"),
    flowId: zod_exports.z.string().trim().min(1),
  })
  .strict();
const setWaitingRequestSchema = zod_exports.z
  .object({
  ...
  .strict();
const resumeFlowRequestSchema = zod_exports.z
  .object({
  ...
  .strict();
const finishFlowRequestSchema = zod_exports.z
  .object({
  ...
  .strict();
const failFlowRequestSchema = zod_exports.z
  .object({
  ...
  .strict();
const requestCancelRequestSchema = zod_exports.z
  .object({
  ...
  .strict();
const cancelFlowRequestSchema = zod_exports.z
  .object({
    action: zod_exports.z.literal("cancel_flow"),
    flowId: zod_exports.z.string().trim().min(1),
  })
  .strict();
const runTaskRequestSchema = zod_exports.z
  .object({
  ...
  .superRefine((value, ctx) => {
    if (
      ...
      (value.startedAt !== void 0 ||
        value.lastEventAt !== void 0 ||
        value.progressSummary !== void 0)
    )
      ctx.addIssue({
...
function describeWebhookOutcome(params) {
  switch (params.action.action) {
    ...
    case "fail_flow":
    case "request_cancel":
      return mapMutationStatus(params.result);
    case "cancel_flow":
      return mapCancelStatus(params.result);
    case "run_task":
      return mapRunTaskStatus(params.result);
    default:
