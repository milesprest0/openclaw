...
import { t as CODEX_PLUGINS_MARKETPLACE_NAME } from "../../config-CNEUyjZv.js";
import "../../agent-runtime-CG9i2LmY.js";
import "../../security-runtime-6Bn2Pszp.js";
import {
  n as handleCodexConversationInboundClaim,
  t as handleCodexConversationBindingResolved,
} from "../../conversation-binding-DsMhaYSM.js";
import { T as pathExists } from "../../fs-safe-CgBWiL92.js";
import { t as createCodexAppServerAgentHarness } from "../../harness-Cm2krGZA.js";
import { n as readJsonFileWithFallback } from "../../json-store-C6iFE4qW.js";
import { t as buildCodexMediaUnderstandingProvider } from "../../media-understanding-provider-Bdgf5ly0.js";
import {
  c as hasMigrationConfigPatchConflict,
  d as markMigrationItemSkipped,
  i as applyMigrationManualItem,
  l as markMigrationItemConflict,
  m as readMigrationConfigPath,
  n as MIGRATION_REASON_TARGET_EXISTS,
  o as createMigrationItem,
  s as createMigrationManualItem,
  u as markMigrationItemError,
  v as summarizeMigrationItems,
  y as writeMigrationConfigPath,
} from "../../migration-DAySQSKv.js";
import {
  i as writeMigrationReport,
  n as copyMigrationFileItem,
  r as withCachedMigrationConfigRuntime,
  t as archiveMigrationItem,
} from "../../migration-runtime-eVU8BbuN.js";
import {
...
import { t as buildCodexProvider } from "../../provider-B-1EByt3.js";
import {
...
async function handleCodexCommand(ctx, options = {}) {
  const { handleCodexSubcommand } = await import("../../command-handlers-BMfB-5M8.js");
  try {
...
async function discoverPluginDirs(codexHome) {
  ...
  async function visit(dir, depth) {
    ...
    if (await exists(manifestPath)) {
      ...
      const name =
        (typeof manifest.name === "string" ? manifest.name.trim() : "") || path.basename(dir);
      discovered.set(dir, {
        name,
        source: dir,
        manifestPath,
...
function pluginNameFromSummary(summary) {
  ...
  for (const candidate of candidates) {
    ...
    if (!trimmed) continue;
    const normalized = (
      trimmed.endsWith(`@openai-curated`)
...
const CODEX_PLUGIN_CONFIG_PATH = ["plugins", "entries", "codex"];
const CODEX_PLUGIN_ENABLED_PATH = ["plugins", "entries", "codex", "enabled"];
const CODEX_PLUGIN_NATIVE_CONFIG_PATH = ["plugins", "entries", "codex", "config", "codexPlugins"];
function uniqueSkillName(skill, counts) {
  const base = sanitizeName(skill.name) || "codex-skill";
  if ((counts.get(base) ?? 0) <= 1) return base;
  return (
    sanitizeName(
      ["codex", sanitizeName(path.basename(path.dirname(skill.source))), base]
        .filter(Boolean)
        .join("-"),
    ) || base
  );
...
function buildPluginItems(plugins) {
  const baseCounts = /* @__PURE__ */ new Map();
...
function readExistingAllowDestructiveActions(config) {
  const value = readMigrationConfigPath(config, [
...
async function applyCodexMigrationPlan(params) {
  ...
  for (const item of plan.items) {
    ...
    if (item.id === "config:codex-plugins")
