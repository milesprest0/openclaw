...
function mapActivityHandling(value) {
  switch (value) {
    ...
    case "start-of-activity-interrupts":
      return "START_OF_ACTIVITY_INTERRUPTS";
    default:
      return;
  }
}
function mapTurnCoverage(value) {
  switch (value) {
    case "only-activity":
      return "TURN_INCLUDES_ONLY_ACTIVITY";
    case "all-input":
      return "TURN_INCLUDES_ALL_INPUT";
    case "audio-activity-and-all-video":
      return "TURN_INCLUDES_AUDIO_ACTIVITY_AND_ALL_VIDEO";
    default:
      return;
  }
}
...
function buildGoogleLiveConnectConfig(config) {
  ...
  return {
    ...
    speechConfig: {
      voiceConfig: {
        prebuiltVoiceConfig: { voiceName: config.voice ?? GOOGLE_REALTIME_DEFAULT_VOICE },
      },
    },
...
var GoogleRealtimeVoiceBridge = class {
    ...
    this.session = await ai.live.connect({
      ...
      callbacks: {
        ...
        onclose: (event) => {
          ...
          this.config.onError?.(
            /* @__PURE__ */ new Error(
              `Google Live session closed after reconnect attempts: ${closeDetails}`,
            ),
          );
  ...
  sendAudio(audio) {
    ...
    this.consecutiveSilenceMs += Math.round(
      (audio.length / bytesPerSample / this.audioFormat.sampleRateHz) * 1e3,
    );
    ...
    try {
      ...
      this.config.onError?.(
        error instanceof Error
          ? error
          : /* @__PURE__ */ new Error("Failed to send Google Live function response"),
      );
    }
  }
  acknowledgeMark() {}
  ...
  toGoogleInputPcm16k(audio) {
    if (
      ...
      this.audioFormat.sampleRateHz === 8e3 &&
      GOOGLE_REALTIME_INPUT_SAMPLE_RATE === 16e3
    )
  ...
  captureSessionLifecycle(message) {
    ...
    if (raw.goAway?.timeLeft)
      this.config.onError?.(
        /* @__PURE__ */ new Error(`Google Live session goAway: ${raw.goAway.timeLeft}`),
      );
  }
  handleSetupComplete() {
