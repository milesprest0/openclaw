...
import { n as resolveAgentEffectiveModelPrimary } from "../../agent-scope-9AmhTwki.js";
import {
...
import { y as parseModelRef } from "../../model-selection-shared-D9oQINZ2.js";
...
const ACTIVE_MEMORY_TOOL_ALLOWLIST = ["memory_recall", "memory_search", "memory_get"];
...
function resolveRecallRunChannelContext(params) {
  ...
  const runnableExplicitChannel =
    explicitChannel && isRunnableChannelName(explicitChannel) ? explicitChannel : void 0;
  ...
  const resolveReturnValue = (params) => {
    const trustedResolvedChannel =
      params.resolvedChannelStrength === "strong" ? params.resolvedChannel : void 0;
    return {
      messageChannel:
...
async function isSessionActiveMemoryDisabled(params) {
  ...
  try {
    return (
      (await readToggleStore(resolveToggleStatePath(params.api))).sessions?.[sessionKey]
        ?.disabled === true
    );
  } catch (error) {
...
function sweepExpiredCacheEntries(now = Date.now()) {
  for (const [cacheKey, cached] of activeRecallCache.entries())
    if (cached.expiresAt <= now) activeRecallCache.delete(cacheKey);
}
...
async function persistPluginStatusLines(params) {
  ...
  try {
    ...
    if (!params.statusLine && !debugLine) {
      ...
      if (
        !(Array.isArray(existingEntry?.pluginDebugEntries)
    ...
    await updateSessionStore(storePath, (store) => {
      ...
      if (!existing) return;
      const nextEntries = (
        Array.isArray(existing.pluginDebugEntries) ? existing.pluginDebugEntries : []
      ).filter(
        (entry) =>
          Boolean(entry) &&
          typeof entry === "object" &&
...
function extractTerminalMemorySearchResultFromSessionRecord(value) {
  ...
  const unavailable =
    ...
    Boolean(debug?.warning) ||
    Boolean(debug?.error) ||
    Boolean(details?.error);
  const debugHits =
    typeof debug?.hits === "number" && Number.isFinite(debug.hits) ? debug.hits : void 0;
...
async function buildTimeoutRecallResult(params) {
  ...
  return {
    ...
    searchDebug:
      ...
      (params.sessionFile ? await readActiveMemorySearchDebug(params.sessionFile) : void 0),
  };
}
...
function normalizeActiveSummary(rawReply) {
  ...
  if (!singleLine || normalizeNoRecallValue(singleLine) || isTimeoutBoilerplateSummary(singleLine))
    return null;
  return singleLine;
}
...
function getModelRef(api, agentId, config, ctx) {
  const currentRunModel =
    ctx?.modelProviderId && ctx?.modelId ? `${ctx.modelProviderId}/${ctx.modelId}` : void 0;
  ...
  try {
    ...
    if (params.abortSignal?.aborted) {
      ...
      attachPartialTimeoutData(
        error,
        partialReply,
        partialReply ? await readActiveMemorySearchDebug(sessionFile) : void 0,
      );
    }
