import {
  A as LMSTUDIO_DOCKER_HOST_BASE_URL,
  C as resolveLoadedContextWindow,
  D as LMSTUDIO_DEFAULT_INFERENCE_BASE_URL,
  E as LMSTUDIO_DEFAULT_EMBEDDING_MODEL,
  F as LMSTUDIO_PROVIDER_LABEL,
  M as LMSTUDIO_LOCAL_API_KEY_PLACEHOLDER,
  N as LMSTUDIO_MODEL_PLACEHOLDER,
  O as LMSTUDIO_DEFAULT_LOAD_CONTEXT_LENGTH,
  P as LMSTUDIO_PROVIDER_ID,
  S as resolveLmstudioServerBase,
  T as LMSTUDIO_DEFAULT_BASE_URL,
  _ as normalizeLmstudioConfiguredCatalogEntry,
  a as resolveLmstudioConfiguredApiKey,
  b as resolveLmstudioReasoningCapability,
  c as resolveLmstudioRuntimeApiKey,
  d as shouldUseLmstudioApiKeyPlaceholder,
  g as normalizeLmstudioConfiguredCatalogEntries,
  h as mapLmstudioWireModelsToConfig,
  i as buildLmstudioAuthHeaders,
  j as LMSTUDIO_DOCKER_HOST_INFERENCE_BASE_URL,
  k as LMSTUDIO_DEFAULT_MODEL_ID,
  l as hasLmstudioAuthorizationHeader,
  m as mapLmstudioWireEntry,
  o as resolveLmstudioProviderHeaders,
  p as buildLmstudioModelName,
  r as fetchLmstudioModels,
  s as resolveLmstudioRequestContext,
  t as discoverLmstudioModels,
  u as resolveLmstudioProviderAuthMode,
  v as normalizeLmstudioProviderConfig,
  w as LMSTUDIO_DEFAULT_API_KEY_ENV_VAR,
  x as resolveLmstudioReasoningCompat,
  y as resolveLmstudioInferenceBase,
} from "../../models.fetch-C1hcJd4I.js";
import { t as normalizeOptionalSecretInput } from "../../normalize-secret-input-B8M4cov-.js";
import { r as removeProviderAuthProfilesWithLock } from "../../profiles-B4Bcz6fY.js";
import { t as WizardCancelledError } from "../../prompts-CGjOiPh-.js";
import { n as buildApiKeyCredential } from "../../provider-auth-helpers-DWjbCNVf.js";
import "../../provider-auth-D0H8jtwm.js";
import { t as ensureApiKeyFromEnvOrPrompt } from "../../provider-auth-input-kX7gel6j.js";
import { h as withAgentModelAliases } from "../../provider-onboard-kfCYjVQt.js";
import "../../provider-setup-Da7A3Mys.js";
import {
  n as configureOpenAICompatibleSelfHostedProviderNonInteractive,
  t as applyProviderDefaultModel,
...
function stripLmstudioStoredAuthConfig(cfg) {
  ...
  const nextProfiles = Object.fromEntries(
    Object.entries(cfg.auth?.profiles ?? {}).filter(
      ([, profile]) => profile.provider !== LMSTUDIO_PROVIDER_ID,
    ),
  );
  const nextOrder = Object.fromEntries(
    Object.entries(cfg.auth?.order ?? {}).filter(
      ([providerId]) => providerId !== LMSTUDIO_PROVIDER_ID,
    ),
  );
  return {
    ...cfg,
...
function resolveLmstudioDiscoveryFailure(params) {
  ...
  if (!discovery.models.some((model) => model.type === "llm" && Boolean(model.key?.trim())))
    return {
      noteLines: [
...
function resolvePersistedLmstudioApiKey(params) {
  if (params.explicitAuth === "api-key") {
    if (params.preferFallbackApiKey && params.fallbackApiKey !== void 0)
...
function mergeDiscoveredLmstudioAllowlistEntries(params) {
  return withAgentModelAliases(
    ...
    params.discoveredModels
      ...
      .filter(Boolean)
      .map((id) => `${LMSTUDIO_PROVIDER_ID}/${id}`),
  );
}
...
async function promptAndConfigureLmstudioInteractive(params) {
  ...
  const credentialSource =
    credentialInput ?? (implicitRefMode && autoRefEnvKey ? `\${LM_API_TOKEN}` : apiKey);
  const credential = (
    params.prompter
      ? credentialMode === "ref" || hasConfiguredSecretInput(credentialSource)
      : normalizedApiKey !== void 0
  )
    ? params.prompter
  ...
  if (params.prompter) {
    const requestedContextWindow = resolvePositiveInteger(
...
async function configureLmstudioNonInteractive(ctx) {
  ...
  const normalizedCtx = customBaseUrl
    ...
    : ctx;
  const configureShared = async (configureCtx) =>
  ...
  const resolved = await normalizedCtx.resolveApiKey({
    ...
    flagName:
      normalizeOptionalSecretInput(normalizedCtx.opts.lmstudioApiKey) !== void 0
        ? "--lmstudio-api-key"
        : "--custom-api-key",
  ...
  const setupDiscoveryApiKey =
    (useHeaderOnlyAuth ? void 0 : resolved?.key) ??
    (shouldUseLmstudioApiKeyPlaceholder({
      hasModels: true,
      resolvedApiKey: void 0,
      hasAuthorizationHeader,
  ...
  const persistedApiKey = resolvePersistedLmstudioApiKey({
    ...
    fallbackApiKey: resolvedSyntheticLocalKey
      ? LMSTUDIO_LOCAL_API_KEY_PLACEHOLDER
      : (configured.models?.providers?.["lmstudio"]?.apiKey ?? "LM_API_TOKEN"),
    preferFallbackApiKey: true,
    hasModels: discoveredModels.length > 0,
