...
import { t as lmstudioMemoryEmbeddingProviderAdapter } from "../../memory-embedding-adapter-B6DJYJ0A.js";
import "../../provider-auth-D0H8jtwm.js";
import { t as CUSTOM_LOCAL_AUTH_MARKER } from "../../model-auth-markers-CtsHE1k1.js";
import {
  F as LMSTUDIO_PROVIDER_LABEL,
  P as LMSTUDIO_PROVIDER_ID,
  c as resolveLmstudioRuntimeApiKey,
  f as shouldUseLmstudioSyntheticAuth,
  g as normalizeLmstudioConfiguredCatalogEntries,
  n as ensureLmstudioModelLoaded,
...
import { n as parseStandalonePlainTextToolCallBlocks } from "../../tool-payload-CFuJ-o-I.js";
...
function wrapLmstudioInferencePreload(ctx) {
  ...
  return (model, context, options) => {
    ...
    if (!shouldPreloadLmstudioModels(providerConfig)) {
      ...
      return (async () => {
        return wrapLmstudioPlainTextToolCalls(
          stream instanceof Promise ? await stream : stream,
          context,
        );
      })();
    }
    ...
    const preloadPromise =
      preloadInFlight.get(preloadKey) ??
      (cooldownEntry
        ? void 0
        : (() => {
    ...
    return (async () => {
      if (preloadPromise)
        try {
          ...
          log.warn(
            `LM Studio inference preload failed for "${modelKey}" (${failures} consecutive failure${failures === 1 ? "" : "s"}, next preload attempt skipped for ~${cooldownSec}s); continuing without preload: ${String(cause)}`,
          );
        }
  ...
  register(api) {
    ...
    api.registerProvider({
      ...
      shouldDeferSyntheticProfileAuth: ({ resolvedApiKey }) =>
        resolvedApiKey?.trim() === "lmstudio-local" || resolvedApiKey?.trim() === "custom-local",
