...
import {
  _ as createRemoteShellSandboxFsBridge,
  b as createWritableRenameTargetResolver,
  c as buildExecRemoteCommand,
  d as createSshSandboxSessionFromConfigText,
  h as shellEscape,
  m as runSshSandboxCommand,
  o as registerSandboxBackend,
  p as disposeSshSandboxSession,
...
import { t as loadJsonFile } from "../../json-store-C6iFE4qW.js";
...
const nonEmptyTrimmedString = (message) =>
  zod_exports.z.string({ error: message }).trim().min(1, { error: message });
const OpenShellPluginConfigSchema = zod_exports.z.strictObject({
  mode: zod_exports.z
    .enum(["mirror", "remote"], { error: "mode must be one of mirror, remote" })
    .optional(),
  ...
  providers: zod_exports.z
    .array(
      zod_exports.z
        .string({ error: "providers must be an array of strings" })
        .trim()
        .min(1, { error: "providers must be an array of strings" }),
  ...
  return {
    ...
    timeoutMs:
      typeof cfg.timeoutSeconds === "number"
        ? Math.floor(cfg.timeoutSeconds * 1e3)
        : DEFAULT_TIMEOUT_MS,
  };
}
//#endregion
//#region extensions/openshell/src/mirror.ts
const DEFAULT_OPEN_SHELL_MIRROR_EXCLUDE_DIRS = ["hooks", "git-hooks", ".git"];
...
var OpenShellFsBridge = class {
  ...
  async readFile(params) {
    ...
    try {
      ...
      opened = await (
        await root(target.mountHostRoot)
      ).open(path.relative(target.mountHostRoot, hostPath), { hardlinks: "reject" });
      try {
  ...
  async writeFile(params) {
    ...
    await (
      await root(target.mountHostRoot)
    ).write(path.relative(target.mountHostRoot, hostPath), buffer, { mkdir: params.mkdir });
...
async function assertLocalPathSafety(params) {
  ...
  if (
    !isPathInside(
      await fs.realpath(params.root).catch(() => path.resolve(params.root)),
