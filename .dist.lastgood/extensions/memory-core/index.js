...
import { t as registerShortTermPromotionDreaming } from "../../dreaming-C4YEOnNj.js";
...
import { i as registerBuiltInMemoryEmbeddingProviders } from "../../provider-adapters-woS7vNTR.js";
...
function buildMemoryFlushPlan(params = {}) {
  ...
  const forceFlushTranscriptBytes =
    parseNonNegativeByteSize(defaults?.forceFlushTranscriptBytes) ?? 2097152;
  const reserveTokensFloor =
    normalizeNonNegativeInt(cfg?.agents?.defaults?.compaction?.reserveTokensFloor) ?? 2e4;
...
function loadMemoryToolsModule() {
  memoryToolsModulePromise ??= import("../../tools-9st3BoPG.js");
  return memoryToolsModulePromise;
}
function loadRuntimeProviderModule() {
  runtimeProviderModulePromise ??= import("../../runtime-provider-N25v7DiI.js");
...
var memory_core_default = definePluginEntry({
  ...
  register(api) {
    ...
    api.registerMemoryCapability({
      ...
      publicArtifacts: {
        async listArtifacts(params) {
          const { listMemoryCorePublicArtifacts } =
            await import("../../public-artifacts-DIDSYtKD.js");
    ...
    api.registerCommand({
      ...
      acceptsArgs: true,
      handler: async (ctx) => {
        const { handleDreamingCommand } = await import("../../dreaming-command-BUHucTiu.js");
        return await handleDreamingCommand(api, ctx);
      },
    });
    api.registerCli(
      async ({ program }) => {
        const { registerMemoryCli } = await import("../../cli-DxoSR3BB.js");
