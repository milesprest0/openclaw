...
import {
  A as lintMemoryWikiVault,
  B as parseWikiMarkdown,
  C as resolveMemoryWikiStatus,
  D as runObsidianDaily,
  E as runObsidianCommand,
  F as getMemoryWikiPage,
  I as readQueryableWikiPages,
  L as searchMemoryWiki,
  M as applyMemoryWikiMutation,
  N as normalizeMemoryWikiMutationInput,
  O as runObsidianOpen,
  P as WIKI_SEARCH_MODES,
  R as compileMemoryWikiVault,
  S as renderMemoryWikiStatus,
  T as probeObsidianCli,
  j as ingestMemoryWikiSource,
  k as runObsidianSearch,
  t as registerWikiCli,
  w as syncMemoryWikiImportedSources,
  x as buildMemoryWikiDoctorReport,
  z as initializeMemoryWikiVault,
} from "../../cli-BXzkpA0h.js";
import {
  d as resolveMemoryWikiConfig,
  l as memoryWikiConfigSchema,
...
async function listMemoryWikiImportInsights(config) {
  const items = (await readQueryableWikiPages(config.vault.path))
    .flatMap((page) => {
      ...
      return [
        {
          ...
          correctionSignals,
          preferenceSignals,
          ...(normalizeTimestamp$1(parsed.frontmatter.createdAt)
            ? { createdAt: normalizeTimestamp$1(parsed.frontmatter.createdAt) }
            : {}),
          ...(normalizeTimestamp$1(parsed.frontmatter.updatedAt)
            ? { updatedAt: normalizeTimestamp$1(parsed.frontmatter.updatedAt) }
            : {}),
        },
      ];
    })
...
const PALACE_KIND_ORDER = ["synthesis", "entity", "concept", "source", "report"];
const PRIMARY_PALACE_KINDS = new Set(["synthesis", "entity", "concept"]);
  ...
  api.registerGatewayMethod(
    ...
    async ({ respond }) => {
      try {
        ...
        respond(
          true,
          buildMemoryWikiDoctorReport(await resolveMemoryWikiStatus(config, { appConfig })),
        );
      } catch (error) {
        respondError(respond, error);
      }
