...
import {
  a as vectorDimsForModel,
  i as memoryConfigSchema,
  n as DEFAULT_RECALL_MAX_CHARS,
  r as MEMORY_CATEGORIES,
} from "../../config-IBxPHAzz.js";
import "../../runtime-env-B60JdRoI.js";
import { n as loadLanceDbModule } from "../../lancedb-runtime-DNpYO03c.js";
import "../../api-Di7BL9Fu.js";
...
function loadMemoryEmbeddingProviderModule() {
  memoryEmbeddingProviderModulePromise ??=
    import("../../plugin-sdk/memory-core-host-engine-embeddings.js");
      ...
      ({ program }) => {
        ...
        memory
          ...
          .action(async (query, opts) => {
            ...
            const output = (await db.search(vector, Number.parseInt(opts.limit, 10), 0.3)).map(
              (r) => ({
                id: r.entry.id,
                text: r.entry.text,
                category: r.entry.category,
        ...
        memory
          ...
          .action(async (opts) => {
            ...
            } else query = query.select(["id", "text", "importance", "category", "createdAt"]);
            if (opts.filter) {
    ...
    api.on("agent_end", async (event, ctx) => {
      ...
      try {
        ...
        const startIndex = resolveAutoCaptureStartIndex(
          event.messages,
          cursorKey ? autoCaptureCursors.get(cursorKey) : void 0,
        );
        let stored = 0;
        ...
        for (let index = startIndex; index < event.messages.length; index++) {
          ...
          try {
            for (const text of extractUserTextContent(message)) {
              ...
              const vector = await embeddings.embed(text);
              if ((await db.search(vector, 1, 0.95)).length > 0) continue;
              await db.store({
                text,
                vector,
                importance: 0.7,
