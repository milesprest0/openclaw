...
import { T as pathExists, p as resolvePathWithinRoot } from "../../fs-safe-CgBWiL92.js";
...
function createProposalFromMessages(params) {
  const instruction = extractTranscriptText(params.messages)
  ...
  return {
    ...
    review:
      parsed.review && typeof parsed.review === "object"
        ? normalizeReviewState(parsed.review)
        : void 0,
  };
}
...
var SkillWorkshopStore = class {
  ...
  async list(status) {
    ...
    return (
      status ? file.proposals.filter((proposal) => proposal.status === status) : file.proposals
    ...
    api.on("agent_end", async (event, ctx) => {
      ...
      if (
        !(
          ...
          (config.reviewMode === "llm" && heuristicProposal !== void 0)
        )
      )
        return;
...
export {
  ...
  skill_workshop_default as default,
