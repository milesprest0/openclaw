...
import { n as tempWorkspaceSync } from "../../private-temp-workspace-DKvr8oLR.js";
import {
...
import "../../api-usfCl0Iu.js";
import { n as resolveEffectiveTtsConfig } from "../../tts-config-DMOCN249.js";
import { a as scheduleCleanup } from "../../tts-provider-helpers-ZMKvUYzz.js";
import { d as resolveConfigDir, p as resolveUserPath } from "../../utils-BGRcpLKt.js";
...
function pickAfconvertRecipe(source, target) {
  if (target === "caf") return ["-f", "caff", "-d", "opus@24000", "-c", "1"];
}
...
function resolveExplicitTtsOverrides(params) {
  ...
  const selectedProvider =
    ...
    (modelId || voiceId ? getTtsProvider(config, prefsPath) : void 0);
...
function resolveTtsRequestSetup(params) {
  ...
  if (params.text.length > config.maxTextLength)
    return { error: `Text too long (${params.text.length} chars, max ${config.maxTextLength})` };
...
async function maybePreTranscodeForVoiceDelivery(params) {
  ...
  const preferred = resolveChannelTtsVoiceDelivery(params.channel)
    ?.preferAudioFileFormat?.trim()
    .toLowerCase();
  if (!preferred) return;
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      if (provider === primaryProvider)
        logVerbose(
          `TTS: primary provider ${provider} failed (${rawError})${providers.length > 1 ? "; trying fallback providers." : "; no fallback providers configured."}`,
        );
...
async function streamSpeech(params) {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      if (provider === primaryProvider)
        logVerbose(
          `TTS stream: primary provider ${provider} failed (${rawError})${providers.length > 1 ? "; trying fallback providers." : "; no fallback providers configured."}`,
        );
...
async function textToSpeechTelephony(params) {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      if (provider === primaryProvider)
        logVerbose(
          `TTS telephony: primary provider ${provider} failed (${rawError})${providers.length > 1 ? "; trying fallback providers." : "; no fallback providers configured."}`,
        );
...
async function maybeApplyTtsToPayload(params) {
  ...
  logVerbose(
    `TTS: conversion failed after ${Date.now() - ttsStart}ms (${result.error ?? "unknown"}).`,
  );
  return nextPayload;
}
const _test = {
