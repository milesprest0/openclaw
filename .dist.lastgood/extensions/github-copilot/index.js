import { t as resolveFirstGithubToken } from "../../auth-HjgBADef.js";
import { t as githubCopilotMemoryEmbeddingProviderAdapter } from "../../embeddings-C7A7Xc4h.js";
...
import { t as buildGithubCopilotReplayPolicy } from "../../replay-policy-Be_sjDgK.js";
import { n as ensureAuthProfileStore } from "../../store-9hg03xJO.js";
import { r as wrapCopilotProviderStream } from "../../stream-BLSyZB_M.js";
...
const COPILOT_XHIGH_MODEL_IDS = ["gpt-5.4", "gpt-5.3-codex", "gpt-5.2", "gpt-5.2-codex"];
...
function resolveExistingCopilotAuthResult(agentDir) {
  ...
  const credential = ensureAuthProfileStore(agentDir, { allowKeychainPrompt: false }).profiles[
    profileId
  ];
  ...
  register(api) {
    ...
    api.registerProvider({
      ...
      catalog: {
        ...
        run: async (ctx) => {
          if (
            (resolveCurrentPluginConfig(ctx.config).discovery?.enabled ??
              ctx.config?.models?.copilotDiscovery?.enabled) === false
          )
            return null;
      ...
      resolveThinkingProfile: ({ modelId }) => ({
        levels: [
          ...
          { id: "medium" },
          { id: "high" },
          ...(COPILOT_XHIGH_MODEL_IDS.includes(normalizeOptionalLowercaseString(modelId) ?? "")
            ? [{ id: "xhigh" }]
            : []),
        ],
      }),
