...
function findLobsterPackageRoot(resolvedEntryPath) {
  ...
  while (true) {
    ...
    try {
      if (JSON.parse(readFileSync(packageJsonPath, "utf8")).name === "@clawdbot/lobster")
        return dir;
    } catch {}
...
async function loadEmbeddedToolRuntimeFromPackage(options = {}) {
  ...
  const resolvePackageEntry =
    options.resolvePackageEntry ?? ((specifier) => lobsterRequire.resolve(specifier));
  let coreLoadError;
  try {
    return toEmbeddedToolRuntime(
      await importModule(["@clawdbot", "lobster", "core"].join("/")),
      "@clawdbot/lobster/core",
    );
  } catch (error) {
    coreLoadError = error;
  }
...
function parseRunFlowParams(params) {
  ...
  if (
    ...
    return null;
  if (resumeFlowId !== void 0 || resumeRevision !== void 0)
...
function parseResumeFlowParams(params) {
  ...
  if (
    ...
    return null;
  if (runControllerId !== void 0 || runGoal !== void 0 || stateJson !== void 0)
  ...
  return {
    ...
    async execute(_id, params) {
      ...
      const maxStdoutBytes =
        typeof params.maxStdoutBytes === "number" ? params.maxStdoutBytes : 512e3;
