...
import { t as NostrConfigSchema } from "../../config-schema-CPBAvgsl.js";
...
const nostrSetupWizard = createDelegatedSetupWizardProxy({
  channel,
  loadWizard: async () => (await import("../../setup-surface-D8lqyuLj.js")).nostrSetupWizard,
  status: {
    ...createStandardChannelSetupStatus({
      ...
      resolveConfigured: ({ cfg, accountId }) =>
        resolveSetupNostrAccount({
          cfg,
          accountId,
        }).configured,
      resolveExtraStatusLines: ({ cfg }) => {
        return [
          `Relays: ${resolveSetupNostrAccount({ cfg }).relays.length || DEFAULT_RELAYS.length}`,
        ];
      },
    }),
  },
