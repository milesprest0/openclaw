import {
  a as setNostrRuntime,
  i as getNostrRuntime,
  n as nostrPlugin,
  o as contentToProfile,
  r as publishNostrProfile,
  t as getNostrProfileState,
} from "../../channel-50E5Ujot.js";
import { n as NostrProfileSchema } from "../../config-schema-CPBAvgsl.js";
...
import "../../runtime-api-CxKE9mF8.js";
import {
...
import { a as createFixedWindowRateLimiter } from "../../webhook-ingress-BWEossCd.js";
...
function validateUrlSafety(urlStr) {
  try {
    ...
    if (isBlockedHostnameOrIp(url.hostname.trim().toLowerCase()))
      return {
        ok: false,
...
async function importProfileFromRelays(opts) {
  ...
  try {
    ...
    for (const item of events)
      if (!bestEvent || item.event.created_at > bestEvent.event.created_at) bestEvent = item;
    if (!bestEvent)
      return {
        ok: false,
    ...
    if (!verifyEvent(bestEvent.event))
      return {
        ok: false,
...
const nip05FormatSchema = zod_exports.z
  .string()
  .regex(/^[a-z0-9._-]+@[a-z0-9.-]+\.[a-z]{2,}$/i, "Invalid NIP-05 format (user@domain.com)")
  .optional();
const lud16FormatSchema = zod_exports.z
  .string()
...
function enforceLoopbackMutationGuards(ctx, req, res) {
  ...
  if (
    normalizeOptionalLowercaseString(firstHeaderValue(req.headers["sec-fetch-site"])) ===
    "cross-site"
  ) {
...
function enforceGatewayMutationScope(ctx, accountId, res) {
  ...
  if ((Array.isArray(runtimeScopes) ? runtimeScopes : []).includes(PROFILE_MUTATION_SCOPE))
    return true;
