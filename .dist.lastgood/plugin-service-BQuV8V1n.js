...
import { t as BrowserToolSchema } from "./browser-tool.schema-DmZk7_uT.js";
import { D as DEFAULT_AI_SNAPSHOT_MAX_CHARS } from "./cdp.helpers-DnNpBUOj.js";
import {
...
import "./sdk-config-CZ5LC61g.js";
import { n as startLazyPluginServiceModule } from "./plugin-runtime-BPKx6OFA.js";
import "./sdk-setup-tools-BVFP1IkU.js";
import { gi as errorShape, hi as ErrorCodes } from "./protocol-CUn5aLrV.js";
import { t as withTimeout } from "./sdk-node-runtime-Bn5PQapP.js";
...
function withConfiguredActTimeout(request, profileName) {
  ...
  const configuredTimeout =
    normalizePositiveTimeoutMs(browserToolActionDeps.getRuntimeConfig().browser?.actionTimeoutMs) ??
    6e4;
  return {
    ...typedRequest,
...
async function executeSnapshotAction(params) {
  ...
  const format =
    input.snapshotFormat === "ai" ? "ai" : input.snapshotFormat === "aria" ? "aria" : void 0;
  ...
  const limit =
    typeof input.limit === "number" && Number.isFinite(input.limit) ? input.limit : void 0;
  const maxChars =
  ...
  const depth =
    typeof input.depth === "number" && Number.isFinite(input.depth) ? input.depth : void 0;
  ...
  let refsFallback;
  const readSnapshot = async (query) =>
    proxyRequest
      ? await proxyRequest({
          method: "GET",
          path: "/snapshot",
...
async function executeConsoleAction(params) {
  ...
  const targetId = normalizeOptionalString(input.targetId);
  if (proxyRequest)
    return formatConsoleToolResult(
      await proxyRequest({
        method: "GET",
        path: "/console",
        profile,
  ...
  return formatConsoleToolResult(
    await browserToolActionDeps.browserConsoleMessages(baseUrl, {
      level,
      targetId,
      profile,
    }),
...
async function executeActAction(params) {
  ...
  try {
    ...
    params.onTabActivity?.(
      readStringValue(result.targetId) ?? readStringValue(effectiveRequest.targetId),
    );
    ...
    if (isChromeStaleTargetError(profile, err)) {
      ...
      if (retryRequest && canRetryChromeActWithoutTargetId(effectiveRequest) && tabs.length === 1)
        try {
          ...
          params.onTabActivity?.(
            readStringValue(retryResult.targetId) ?? readStringValue(retryRequest.targetId),
          );
...
async function resolveBrowserNodeTarget$1(params) {
  ...
  const browserNodes = (await browserToolDeps.listNodes({})).filter(
    (node) => node.connected && isBrowserNode$1(node),
  );
...
function resolveBrowserNode(nodes, query) {
  ...
  throw new Error(
    `ambiguous node: ${q} (matches: ${matches.map((node) => node.displayName || node.remoteIp || node.nodeId).join(", ")})`,
  );
}
  ...
  return {
    ...
    start: async () => {
      ...
      handle = await startLazyPluginServiceModule({
        ...
        loadDefaultModule: async () => await import("./server-BYVEF_lS.js"),
        startExportNames: [
...
//#endregion
export {
  createBrowserTool as i,
  browserHandlers as n,
  handleBrowserGatewayRequest as r,
  createBrowserPluginService as t,
};
