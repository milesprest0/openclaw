...
import {
  n as resolveNormalizedAccountEntry,
  t as resolveAccountEntry,
} from "./account-lookup-28TEMylU.js";
import { t as CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "./approval-handler-adapter-runtime-CEmfb1ag.js";
import { n as createChannelApprovalHandlerFromCapability } from "./approval-handler-runtime-Czq_v5v0.js";
import { a as isExecApprovalChannelRuntimeTerminalStartError } from "./approval-native-runtime-Jhr06_E7.js";
import { n as sleepWithAbort, t as computeBackoff } from "./backoff-CW9s2Y7t.js";
import {
  i as watchChannelRuntimeContexts,
  n as getChannelRuntimeContext,
  t as createTaskScopedChannelRuntime,
} from "./channel-runtime-context-BsruUUWt.js";
...
import { n as resolveChannelApprovalCapability } from "./plugins-Dn-pJrhh.js";
import {
  i as listChannelPlugins,
  r as getLoadedChannelPluginOrigin,
...
async function startChannelApprovalHandlerBootstrap(params) {
  ...
  if (existingContext !== void 0) {
    ...
    spawn(
      "failed to start native approval handler",
      startHandlerForRegisteredContext(existingContext, activeGeneration),
    );
  }
  return async () => {
    unsubscribe();
      ...
      tasks: accountIds.map((id) => async () => {
        ...
        try {
          ...
          scopedChannelRuntime = await measureStartup(`channels.${channelId}.runtime`, async () =>
            createTaskScopedChannelRuntime({ channelRuntime: await getChannelRuntime(channelId) }),
          );
          ...
          const trackedPromise = Promise.resolve()
            ...
            .then(async () => {
              ...
              log.info?.(
                `[${id}] auto-restart attempt ${attempt}/${MAX_RESTART_ATTEMPTS} in ${Math.round(delayMs / 1e3)}s`,
              );
              setRuntime(channelId, id, {
                accountId: id,
                restartPending: true,
