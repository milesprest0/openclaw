import { i as listCombinedAccountIds } from "./account-helpers-DgSuSC4_.js";
...
import { n as isAccountConfigured, o as resolveTwitchToken } from "./twitch-CB__p3Ax.js";
...
function getAccountConfig(coreConfig, accountId) {
  ...
  if (normalizedAccountId === "default") {
    ...
    const baseLevel = {
      ...
      requireMention:
        typeof twitchRaw?.requireMention === "boolean" ? twitchRaw.requireMention : void 0,
      ...
      obtainmentTimestamp:
        typeof twitchRaw?.obtainmentTimestamp === "number" ? twitchRaw.obtainmentTimestamp : void 0,
    };
    const merged = {
      ...accountFromAccounts,
      ...baseLevel,
...
async function configureWithEnvToken(
  ...
  forceAllowFrom,
  dmPolicy,
  accountId = resolveSetupAccountId(cfg),
) {
  const resolvedAccountId = accountId.trim()
    ? normalizeRequestedSetupAccountId(accountId)
    : resolveSetupAccountId(cfg);
...
const twitchSetupWizard = {
  ...
  status: {
    ...
    resolveStatusLines: ({ cfg, accountId }) => {
      ...
      return [
        `Twitch${resolvedAccountId !== "default" ? ` (${resolvedAccountId})` : ""}: ${configured ? "configured" : "needs username, token, and clientId"}`,
      ];
    },
  },
  credentials: [],
    ...
    return {
      cfg:
        forceAllowFrom && twitchDmPolicy.promptAllowFrom
          ? await twitchDmPolicy.promptAllowFrom({
              cfg: cfgWithAccount,
              prompter,
              accountId,
            })
...
//#endregion
export {
  getAccountConfig as a,
  resolveTwitchAccountContext as c,
  DEFAULT_ACCOUNT_ID as i,
  resolveTwitchSnapshotAccountId as l,
  twitchSetupPlugin as n,
  listAccountIds as o,
  twitchSetupWizard as r,
  resolveDefaultTwitchAccountId as s,
  twitchSetupAdapter as t,
};
