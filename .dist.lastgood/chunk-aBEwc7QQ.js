...
import { y as resolveChannelStreamingChunkMode } from "./channel-streaming-DxCQl0Kn.js";
import {
  n as isSafeFenceBreak,
  r as parseFenceSpans,
  t as findFenceSpanAt,
} from "./fences-CBcL-2k8.js";
import { t as chunkTextByBreakResolver } from "./text-chunking-CjnESV9N.js";
...
function resolveChunkModeForProvider(cfgSection, accountId) {
  ...
  const accounts = cfgSection.accounts;
  if (accounts && typeof accounts === "object") {
    const directMode = resolveChannelStreamingChunkMode(
...
function resolveChunkMode(cfg, provider, accountId) {
  ...
  return (
    resolveChunkModeForProvider(cfg?.channels?.[provider] ?? cfg?.[provider], accountId) ??
    DEFAULT_CHUNK_MODE
  );
}
/**
...
function chunkMarkdownText(text, limit) {
  ...
  while (start < text.length) {
    ...
    if (initialFence) {
      ...
      fenceToSplit =
        fenceAtBreak && fenceAtBreak.start === initialFence.start ? fenceAtBreak : void 0;
    }
...
export {
  chunkText as a,
  resolveTextChunkLimit as c,
  chunkMarkdownTextWithMode as i,
  chunkByParagraph as n,
  chunkTextWithMode as o,
  chunkMarkdownText as r,
  resolveChunkMode as s,
  chunkByNewline as t,
};
