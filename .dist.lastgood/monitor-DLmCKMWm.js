...
import { o as createDeferred } from "./extension-shared-B_zNy4Rm.js";
import {
...
import {
  c as isZalouserGroupEntryAllowed,
  i as resolveZalouserMessageSid,
  o as buildZalouserGroupCandidates,
  r as formatZalouserMessageSidFull,
...
import {
  ...
  o as sendSeenZalouser,
  s as sendTypingZalouser,
  t as sendDeliveredZalouser,
} from "./send-BoM09TMf.js";
import {
...
import {
  o as listZaloGroups,
  r as listZaloFriends,
  u as resolveZaloGroupContext,
  v as startZaloListener,
...
function isSenderAllowed(senderId, allowFrom) {
  ...
  return allowFrom.some((entry) => {
    return (
      normalizeLowercaseStringOrEmpty(entry).replace(/^(zalouser|zlu):/i, "") === normalizedSenderId
    );
  });
}
...
async function deliverZalouserReply(params) {
  ...
  await deliverTextOrMediaReply({
    ...
    onMediaError: (error) => {
      runtime.error(
        `Zalouser media send failed: ${error instanceof Error ? error.message : JSON.stringify(error)}`,
      );
    },
  });
...
async function monitorZalouserProvider(options) {
  ...
  const historyLimit = Math.max(
    0,
    account.config.historyLimit ?? config.messages?.groupChat?.historyLimit ?? 50,
  );
