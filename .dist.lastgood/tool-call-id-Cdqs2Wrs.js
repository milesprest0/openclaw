...
const SESSIONS_SPAWN_ATTACHMENT_METADATA_KEYS = ["name", "encoding", "mimeType"];
...
function hasUnredactedSessionsSpawnAttachments(block) {
  if (
    normalizeLowercaseStringOrEmpty(typeof block.name === "string" ? block.name.trim() : "") !==
    "sessions_spawn"
  )
    return false;
  for (const payload of [block.arguments, block.input]) {
    ...
    for (const attachment of attachments)
      if (!isRedactedSessionsSpawnAttachment(attachment)) return true;
  }
  return false;
}
//#endregion
...
function rewriteToolResultIds(params) {
  ...
  const sharedRawId =
    toolCallId && toolUseIdStr && toolCallId === toolUseIdStr ? toolCallId : void 0;
  ...
  const nextToolUseId =
    sharedResolvedId ?? (toolUseIdStr ? params.resolveId(toolUseIdStr) : void 0);
...
//#endregion
export {
  SESSIONS_SPAWN_ATTACHMENT_METADATA_KEYS as a,
  isRedactedSessionsSpawnAttachment as c,
  REDACTED_SESSIONS_SPAWN_ATTACHMENT_CONTENT as i,
  normalizeAllowedToolNames as l,
  extractToolResultId as n,
  hasUnredactedSessionsSpawnAttachments as o,
  sanitizeToolCallIdsForCloudCodeAssist as r,
  isAllowedToolCallName as s,
  extractToolCallsFromAssistant as t,
};
