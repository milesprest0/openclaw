...
import { i as readRegularFileSync, s as statRegularFileSync } from "./regular-file-CoVHB-2u.js";
import {
...
async function loadZcaJsRuntime() {
  zcaJsRuntimePromise ??= import("./dist-BssnpYvY.js").then((mod) => mod);
...
function resolveInboundTimestamp(rawTs) {
  if (typeof rawTs === "number" && Number.isFinite(rawTs))
    return rawTs > 0xe8d4a51000 ? rawTs : rawTs * 1e3;
  const parsed = Number.parseInt(
...
function clearCachedGroupContext(profile) {
  for (const key of groupContextCache.keys())
    if (key.startsWith(`${profile}:`)) groupContextCache.delete(key);
}
...
function toInboundMessage(message, ownUserId) {
  ...
  const quoteOwnerId =
    data.quote && typeof data.quote === "object" ? toNumberId(data.quote.ownerId) : "";
...
async function checkZaloAuthenticated(profileInput) {
  ...
  try {
    await withZaloApi(
      profile,
      async (api) =>
        await withTimeout(api.fetchAccountInfo(), 12e3, {
...
async function listZaloGroupMembers(profileInput, groupId) {
  return await withZaloApi(normalizeProfile(profileInput), async (api) => {
    ...
    const memberIds = Array.isArray(groupInfo.memberIds)
      ? groupInfo.memberIds.map((id) => toNumberId(id)).filter(Boolean)
      : [];
    const memVerIds = Array.isArray(groupInfo.memVerList)
      ? groupInfo.memVerList.map((id) => toNumberId(id)).filter(Boolean)
      : [];
    ...
    const uniqueIds = Array.from(new Set([...memberIds, ...memVerIds, ...currentById.keys()]));
      ...
      try {
        if (options.mediaUrl?.trim()) {
          ...
          if (media.kind === "audio") {
            let textMessageId;
            if (payloadText)
              textMessageId = extractSendMessageId(
            ...
            const voiceAsset = resolveUploadedVoiceAsset(
            ...
            const voiceMessageId = extractSendMessageId(
              await api.sendVoice({ voiceUrl }, trimmedThreadId, type),
            );
            return {
              ok: true,
          ...
          const messageId = extractSendMessageId(
            await api.sendMessage(
              {
                msg: payloadText,
...
async function startZaloQrLogin(params) {
  ...
  if (!params.force && (await checkZaloAuthenticated(profile))) {
    ...
    return {
      message: `Zalo is already linked${info?.displayName ? ` (${info.displayName})` : ""}.`,
    };
  }
...
async function startZaloListener(params) {
  ...
  const onClosed = (code, reason) => {
    failListener(
      /* @__PURE__ */ new Error(`Zalo listener closed (${code}): ${reason || "no reason"}`),
    );
  };
...
export {
  TextStyle as S,
  sendZaloTypingEvent as _,
  listZaloGroupMembers as a,
  waitForZaloQrLogin as b,
  logoutZaloProfile as c,
  resolveZaloGroupsByEntries as d,
  sendZaloDeliveredEvent as f,
  sendZaloTextMessage as g,
  sendZaloSeenEvent as h,
  listZaloFriendsMatching as i,
  resolveZaloAllowFromEntries as l,
  sendZaloReaction as m,
  getZaloUserInfo as n,
  listZaloGroups as o,
  sendZaloLink as p,
  listZaloFriends as r,
  listZaloGroupsMatching as s,
  checkZaloAuthenticated as t,
  resolveZaloGroupContext as u,
  startZaloListener as v,
  createZalouserSendReceipt as x,
  startZaloQrLogin as y,
};
