...
const GATEWAY_DIST_ENTRYPOINT_BASENAMES = ["index.js", "index.mjs", "entry.js", "entry.mjs"];
...
function buildGatewayDistEntrypointCandidates(...inputs) {
  ...
  for (const basename of GATEWAY_DIST_ENTRYPOINT_BASENAMES)
    for (const distDir of distDirs) candidates.push(path.join(distDir, basename));
  return candidates;
}
...
//#endregion
export {
  resolveGatewayInstallEntrypoint as i,
  findFirstAccessibleGatewayEntrypoint as n,
  isGatewayDistEntrypointPath as r,
  buildGatewayDistEntrypointCandidates as t,
};
