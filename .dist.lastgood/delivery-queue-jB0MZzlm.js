...
import { n as assertSafePathSegment } from "./safe-path-segment-Bp_Kir-q.js";
...
async function loadPendingJsonDurableQueueEntries(options) {
  ...
  for (const file of files)
    if (file.endsWith(".delivered")) await unlinkBestEffort(path.join(options.queueDir, file));
    else if (options.cleanupTmpMaxAgeMs !== void 0 && file.endsWith(".tmp"))
...
async function recoverPendingDeliveries(opts) {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      ...
      if (currentEntry.retryCount >= 5) {
        opts.log.warn(
          `Delivery ${currentEntry.id} exceeded max retries (${currentEntry.retryCount}/5) — moving to failed/`,
        );
...
//#endregion
export {
  moveJsonDurableQueueEntryToFailed as C,
  writeJsonDurableQueueEntry as E,
  loadPendingJsonDurableQueueEntries as S,
  resolveJsonDurableQueueEntryPaths as T,
  runOutboundDeliveryCommitHooks as _,
  recoverPendingDeliveries as a,
  jsonDurableQueueEntryExists as b,
  enqueueDelivery as c,
  loadPendingDeliveries as d,
  loadPendingDelivery as f,
  attachOutboundDeliveryCommitHook as g,
  moveToFailed as h,
  isPermanentDeliveryError as i,
  ensureQueueDir as l,
  markDeliveryPlatformSendAttemptStarted as m,
  drainPendingDeliveries as n,
  withActiveDeliveryClaim as o,
  markDeliveryPlatformOutcomeUnknown as p,
  isEntryEligibleForRecoveryRetry as r,
  ackDelivery as s,
  computeBackoffMs as t,
  failDelivery as u,
  ackJsonDurableQueueEntry as v,
  readJsonDurableQueueEntry as w,
  loadJsonDurableQueueEntry as x,
  ensureJsonDurableQueueDirs as y,
};
