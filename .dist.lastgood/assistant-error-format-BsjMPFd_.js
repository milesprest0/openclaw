...
const MALFORMED_STREAMING_FRAGMENT_ERROR_MESSAGE =
...
function formatRawAssistantErrorForUi(raw) {
  ...
  if (trimmed === "OpenClaw transport error: malformed_streaming_fragment")
  ...
  if (info?.message)
    return `${info.httpCode ? `HTTP ${info.httpCode}` : "LLM error"}${info.type ? ` ${info.type}` : ""}: ${info.message}`;
  return trimmed.length > 600 ? `${trimmed.slice(0, 600)}…` : trimmed;
}
//#endregion
export {
  parseApiErrorInfo as a,
  isCloudflareOrHtmlErrorPage as i,
  extractLeadingHttpStatus as n,
  parseApiErrorPayload as o,
  formatRawAssistantErrorForUi as r,
  MALFORMED_STREAMING_FRAGMENT_ERROR_MESSAGE as t,
};
