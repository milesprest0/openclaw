...
import {
  ...
  l as REALTIME_VOICE_AGENT_CONSULT_TOOL_POLICIES,
...
import "./realtime-voice-CgtYi1ED.js";
import { j as TtsConfigSchema } from "./zod-schema.core-CkFaNmbg.js";
 ...
 */
const E164Schema = zod_exports.z
  .string()
 ...
 */
const InboundPolicySchema = zod_exports.z.enum(["disabled", "allowlist", "pairing", "open"]);
const SecretInputSchema = buildSecretInputSchema();
const TelnyxConfigSchema = zod_exports.z
  .object({
    /** Telnyx API v2 key */
  ...
  .strict();
const TwilioConfigSchema = zod_exports.z
  .object({
    /** Twilio Account SID */
  ...
  .strict();
const PlivoConfigSchema = zod_exports.z
  .object({
  ...
  .strict();
const VoiceCallNumberRouteConfigSchema = zod_exports.z
  .object({
  ...
  .strict();
const VoiceCallServeConfigSchema = zod_exports.z
  .object({
    /** Port to listen on */
...
const VoiceCallTailscaleConfigSchema = zod_exports.z
  .object({
    /**
     ...
     */
    mode: zod_exports.z.enum(["off", "serve", "funnel"]).default("off"),
...
const VoiceCallTunnelConfigSchema = zod_exports.z
  .object({
    /**
     * Tunnel provider:
...
const VoiceCallWebhookSecurityConfigSchema = zod_exports.z
  .object({
    /**
...
const OutboundConfigSchema = zod_exports.z
  .object({
...
const RealtimeToolSchema = zod_exports.z
  .object({
  ...
  .strict();
const VoiceCallRealtimeProvidersConfigSchema = zod_exports.z
  .record(
    zod_exports.z.string(),
    zod_exports.z.record(zod_exports.z.string(), zod_exports.z.unknown()),
  )
  .default({});
const VoiceCallRealtimeToolPolicySchema = zod_exports.z.enum(
  REALTIME_VOICE_AGENT_CONSULT_TOOL_POLICIES,
);
const VoiceCallRealtimeConsultPolicySchema = zod_exports.z.enum(["auto", "substantive", "always"]);
const VoiceCallRealtimeFastContextSourceSchema = zod_exports.z.enum(["memory", "sessions"]);
const VoiceCallRealtimeFastContextConfigSchema = zod_exports.z
  .object({
...
const VoiceCallRealtimeAgentContextConfigSchema = zod_exports.z
  .object({
    ...
    files: zod_exports.z
      .array(zod_exports.z.string().min(1))
...
const VoiceCallStreamingProvidersConfigSchema = zod_exports.z
  .record(
    zod_exports.z.string(),
    zod_exports.z.record(zod_exports.z.string(), zod_exports.z.unknown()),
  )
  .default({});
const VoiceCallRealtimeConfigSchema = zod_exports.z
  .object({
...
const VoiceCallStreamingConfigSchema = zod_exports.z
  .object({
...
const VoiceCallConfigSchema = zod_exports.z
  .object({
    /** Enable voice call functionality */
    enabled: zod_exports.z.boolean().default(false),
    /** Active provider (telnyx, twilio, plivo, or mock) */
    provider: zod_exports.z.enum(["telnyx", "twilio", "plivo", "mock"]).optional(),
...
//#endregion
export {
  resolveVoiceCallEffectiveConfig as a,
  deepMergeDefined as c,
  resolveVoiceCallConfig as i,
  normalizeVoiceCallConfig as n,
  resolveVoiceCallSessionKey as o,
  resolveTwilioAuthToken as r,
  validateProviderConfig as s,
  VoiceCallConfigSchema as t,
};
