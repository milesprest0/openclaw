...
import {
  n as registerCoreCliByName,
  t as getCoreCliCommandNames,
} from "./command-registry-core-LM4nn8wn.js";
import {
  c as resolveShellFromEnv,
  i as installCompletion,
  o as isCompletionShell,
  s as resolveCompletionCachePath,
  t as COMPLETION_SHELLS,
...
import { t as getProgramContext } from "./program-context-qVxfT0Ao.js";
import {
  n as registerSubCliByName,
  t as getSubCliEntries,
    ...
    .action(async (options) => {
      ...
      if (options.install) {
        await installCompletion(
          options.shell ?? resolveShellFromEnv(),
          Boolean(options.yes),
          program.name(),
        );
        return;
...
function generateZshSubcmdList(cmd) {
  return `"1: :_values 'command' ${cmd.commands
    .map((c) => {
      const desc = c
        .description()
        .replace(/\\/g, "\\\\")
