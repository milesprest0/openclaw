...
import { i as normalizePluginsConfigWithResolver } from "./config-normalization-shared-DVyDp1ed.js";
import {
  _ as resolveInstalledPluginIndexPolicyHash,
  d as isInstalledPluginEnabled,
  f as loadInstalledPluginIndex,
  h as extractPluginInstallRecordsFromInstalledPluginIndex,
  i as refreshPersistedInstalledPluginIndex,
  m as hasOptionalMissingPluginManifestFile,
  r as readPersistedInstalledPluginIndexSync,
  t as inspectPersistedInstalledPluginIndex,
  u as getInstalledPluginRecord,
  v as fileSignatureMatches,
...
function listPluginContributionIds(params) {
  ...
  return sortUnique(
    listContributionManifestPlugins({
      ...params,
      index,
...
function resolvePluginContributionOwners(params) {
  ...
  if (params.lookUpTable && typeof params.matches === "string") {
    const owners = resolveContributionOwnerMap(params.lookUpTable, params.contribution)?.get(
      params.matches,
    );
    if (owners)
  ...
  const matcher =
    typeof params.matches === "string"
      ? (contributionId) => contributionId === params.matches
      : params.matches;
  return sortUnique(
    listContributionManifestPlugins({
      ...params,
...
function resolveProviderOwners(params) {
  ...
  if (params.lookUpTable) {
    ...
    for (const [contributionId, ownerIds] of params.lookUpTable.owners.providers.entries())
      if (normalizeProviderId(contributionId) === providerId) owners.push(...ownerIds);
    return filterContributionOwnerIds({
      owners,
      index,
      includeDisabled: params.includeDisabled,
...
//#endregion
export {
  resolveManifestContractPluginIds as a,
  resolveProviderOwners as c,
  inspectPluginRegistry as d,
  isPluginEnabled as f,
  createPluginRegistryIdNormalizer as g,
  refreshPluginRegistry as h,
  resolveManifestContractOwnerPluginId as i,
  DISABLE_PERSISTED_PLUGIN_REGISTRY_ENV as l,
  loadPluginRegistrySnapshotWithMetadata as m,
  loadPluginManifestRegistryForPluginRegistry as n,
  resolveManifestContractPluginIdsByCompatibilityRuntimePath as o,
  loadPluginRegistrySnapshot as p,
  normalizePluginsConfigWithRegistry as r,
  resolvePluginContributionOwners as s,
  listPluginContributionIds as t,
  getPluginRecord as u,
};
