...
import {
  n as loadNodeHostConfig,
  r as saveNodeHostConfig,
  t as ensureNodeHostConfig,
} from "./config-BxnQs22t.js";
import { t as ConnectErrorDetailCodes } from "./connect-error-details-BXtaJDEs.js";
import { t as resolveGatewayConnectionAuth } from "./connection-auth-DilLkDRI.js";
import {
  n as resolveDaemonInstallRuntimeInputs,
  o as resolveNodeProgramArguments,
  r as resolveDaemonNodeBinDir,
  t as emitDaemonInstallRuntimeWarning,
} from "./daemon-install-plan.shared-Bdw5CBKo.js";
import "./fs-safe-advanced-DQNZZtsF.js";
import {
  r as isGatewayDaemonRuntime,
  t as DEFAULT_GATEWAY_DAEMON_RUNTIME,
} from "./daemon-runtime-CkYPLluX.js";
import { r as loadOrCreateDeviceIdentity } from "./device-identity-BQp4SxLS.js";
import {
...
import {
  r as resolveExecSafeBinRuntimePolicy,
  t as isInterpreterLikeSafeBin,
} from "./exec-safe-bin-runtime-policy-DBRGTZJu.js";
import {
  b as resolveExecutableFromPathEnv,
...
import { t as sameFileIdentity } from "./file-identity-DAFLPKWD.js";
import { t as formatHelpExamples } from "./help-format-ejZTKN6e.js";
import {
  c as sanitizeSystemRunEnvOverrides,
  o as sanitizeHostExecEnv,
  t as inspectHostExecEnvOverrides,
} from "./host-env-security-W4c-l4Zm.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import {
  i as runServiceUninstall,
  n as runServiceStart,
  r as runServiceStop,
  t as runServiceRestart,
} from "./lifecycle-core-Czf4Lmjk.js";
import { t as formatDocsLink } from "./links-CNfoPWBd.js";
import { a as logWarn } from "./logger-BWdMCYqK.js";
...
import { t as resolveNodeService } from "./node-service-DJFuOaib.js";
import { t as parsePort } from "./parse-port-DOLYC3Yr.js";
import { t as ensureOpenClawCliOnPath } from "./path-env-Dn5fzabK.js";
import {
  _ as resolveNodeSystemdServiceName,
  g as resolveNodeLaunchAgentLabel,
  u as formatNodeServiceDescription,
  v as resolveNodeWindowsTaskName,
} from "./paths-D-SPS6K1.js";
import { n as detectPolicyInlineEval } from "./policy-DoWoODSF.js";
import { c as describeInterpreterInlineEval } from "./risks-C_v3AkAH.js";
import { a as getActivePluginRegistry } from "./runtime-ChXyIFtl.js";
import {
  n as buildPlatformServiceStartHints,
  r as formatRuntimeStatus,
  t as buildPlatformRuntimeLogHints,
} from "./runtime-hints-Bj-tHqaA.js";
import { n as defaultRuntime } from "./runtime-kqN0Yohi.js";
import { u as buildNodeServiceEnvironment } from "./runtime-paths-vFciNMRO.js";
import { c as normalizeAgentId } from "./session-key-B4qUwRzq.js";
import {
  d as resolveRuntimeStatusColor,
  h as installDaemonServiceAndEmit,
  n as createDaemonInstallActionContext,
  p as buildDaemonServiceSnapshot,
  r as failIfNixDaemonInstallMode,
  t as createCliStatusTextStyles,
} from "./shared-CD1EqH1S.js";
import {
  E as splitShellArgs,
  T as normalizeExecutableToken,
  d as unwrapKnownShellMultiplexerInvocation,
  f as POSIX_INLINE_COMMAND_FLAGS,
  h as extractEnvAssignmentKeysFromDispatchWrappers,
  l as isShellWrapperInvocation,
  m as resolveInlineCommandMatch,
  s as isBlockedShellWrapperCommand,
  t as POSIX_SHELL_WRAPPERS,
  u as resolveShellWrapperTransportArgv,
  y as unwrapKnownDispatchWrapperInvocation,
} from "./shell-wrapper-resolution-BH1NMVsN.js";
import {
...
import {
  ...
  s as normalizeSystemRunApprovalPlan,
  t as formatExecCommand,
} from "./system-run-command-C5E4go0-.js";
import { r as theme, t as colorize } from "./theme-CiH_wF8x.js";
import { n as VERSION } from "./version-BZr74W_5.js";
import {
  a as decodeWindowsOutputBuffer,
  o as resolveWindowsConsoleEncoding,
} from "./windows-command-BxrWbgN7.js";
...
function isKnownBinaryExecutableHeader(buffer) {
  if (buffer.length >= 4 && buffer.subarray(0, 4).equals(Buffer.from([127, 69, 76, 70])))
    return true;
  if (
    buffer.length >= 4 &&
    (buffer.subarray(0, 4).equals(Buffer.from([254, 237, 250, 206])) ||
      buffer.subarray(0, 4).equals(Buffer.from([206, 250, 237, 254])) ||
      buffer.subarray(0, 4).equals(Buffer.from([254, 237, 250, 207])) ||
      buffer.subarray(0, 4).equals(Buffer.from([207, 250, 237, 254])) ||
      buffer.subarray(0, 4).equals(Buffer.from([202, 254, 186, 190])) ||
      buffer.subarray(0, 4).equals(Buffer.from([190, 186, 254, 202])) ||
      buffer.subarray(0, 4).equals(Buffer.from([202, 254, 186, 191])) ||
      buffer.subarray(0, 4).equals(Buffer.from([191, 186, 254, 202])))
  )
    return true;
  ...
  return (
    ...
    buffer.subarray(peOffset, peOffset + 4).equals(Buffer.from([80, 69, 0, 0]))
  );
}
...
async function loadPluginRegistryLoaderModule() {
  pluginRegistryLoaderModulePromise ??= import("./runtime-registry-loader-D7StEUXN.js");
...
async function invokeRegisteredNodeHostCommand(command, paramsJSON) {
  const match = (getActivePluginRegistry()?.nodeHostCommands ?? []).find(
  ...
  if (command === "system.execApprovals.get") {
    try {
      ...
      await sendErrorResult(
        client,
        frame,
        normalizeLowercaseStringOrEmpty(message).includes("timed out")
          ? "TIMEOUT"
          : "INVALID_REQUEST",
        message,
      );
...
function handleNodeHostReconnectPaused(info, deps = {}) {
  ...
  (deps.writeLine ?? writeStderrLine)(
    formatNodeHostReconnectPausedMessage(info, { exiting: shouldExit }),
  );
...
async function runNodeHost(opts) {
  ...
  if (
    !(
      await startGatewayClientWhenEventLoopReady(client, {
