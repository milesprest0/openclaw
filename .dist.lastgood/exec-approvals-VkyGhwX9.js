...
import { a as resolveAllowAlwaysPatternEntries } from "./exec-approvals-allowlist-wYeDXb0Q.js";
import { o as resolveRequiredHomeDir, t as expandHomePrefix } from "./home-dir-iZwpu-40.js";
import { l as assertNoSymlinkParentsSync } from "./regular-file-CoVHB-2u.js";
...
const EXEC_TARGET_VALUES = ["auto", "sandbox", "gateway", "node"];
...
function assertSafeExecApprovalsOverwriteFallback(filePath) {
  ...
  try {
    if (fs.statSync(filePath).nlink > 1)
...
function restoreExecApprovalsFallbackDestination(filePath, destination) {
  if (!destination.existed) {
    try {
      if (sameFilesystemEntry(fs.statSync(filePath), fs.fstatSync(destination.fd)))
...
function copyExecApprovalsFallback(tempPath, filePath) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Failed to restore exec approvals after copy fallback failure for ${filePath}: ${String(copyErr)}`,
        { cause: restoreErr },
      );
    }
    throw copyErr;
...
function sanitizeExecApprovalPolicy(policy) {
  ...
  return {
    security:
      security === "deny" || security === "allowlist" || security === "full" ? security : void 0,
...
function normalizeExecApprovals(file) {
  ...
  for (const [key, agent] of Object.entries(agents)) {
    const allowlist = stripAllowlistCommandText(
      ensureAllowlistIds(coerceAllowlistEntries(agent.allowlist)),
    );
...
function recordAllowlistUse(approvals, agentId, entry, command, resolvedPath) {
  ...
  const nextAllowlist = (Array.isArray(existing.allowlist) ? existing.allowlist : []).map((item) =>
    item.pattern === entry.pattern && (item.argPattern ?? void 0) === (entry.argPattern ?? void 0)
      ? Object.assign({}, item, {
...
function addAllowlistEntry(approvals, agentId, pattern, options) {
  ...
  const existingEntry = allowlist.find(
    (entry) => entry.pattern === trimmed && (entry.argPattern ?? void 0) === trimmedArgPattern,
  );
...
//#endregion
export {
  resolveExecApprovalsPath as A,
  requestExecApprovalViaSocket as C,
  resolveExecApprovalRequestAllowedDecisions as D,
  resolveExecApprovalAllowedDecisions as E,
  restoreExecApprovalsSnapshot as M,
  saveExecApprovals as N,
  resolveExecApprovals as O,
  requestJsonlSocket as P,
  recordAllowlistUse as S,
  requiresExecApproval as T,
  normalizeExecSecurity as _,
  addAllowlistEntry as a,
  readExecApprovalsSnapshot as b,
  hasDurableExecApproval as c,
  maxAsk as d,
  mergeExecApprovalsSocketDefaults as f,
  normalizeExecHost as g,
  normalizeExecAsk as h,
  EXEC_TARGET_VALUES as i,
  resolveExecApprovalsSocketPath as j,
  resolveExecApprovalsFromFile as k,
  isExecApprovalDecisionAllowed as l,
  normalizeExecApprovals as m,
  DEFAULT_EXEC_APPROVAL_DECISIONS as n,
  addDurableCommandApproval as o,
  minSecurity as p,
  DEFAULT_EXEC_APPROVAL_TIMEOUT_MS as r,
  ensureExecApprovals as s,
  DEFAULT_EXEC_APPROVAL_ASK_FALLBACK as t,
  loadExecApprovals as u,
  normalizeExecTarget as v,
  requireValidExecTarget as w,
  recordAllowlistMatchesUse as x,
  persistAllowAlwaysPatterns as y,
};
