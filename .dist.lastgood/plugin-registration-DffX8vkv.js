...
const browserSecurityAuditCollectors = [
  async (ctx) => {
    const { collectBrowserSecurityAuditFindings } =
      await import("./extensions/browser/register.runtime.js");
...
function createLazyBrowserPluginService() {
  ...
  const loadService = async () => {
    if (!service) {
      const { createBrowserPluginService } =
        await import("./extensions/browser/register.runtime.js");
...
function registerBrowserPlugin(api) {
  ...
  api.registerCli(
    async ({ program }) => {
      const { registerBrowserCli } = await import("./browser-cli-CKqMGlKn.js");
...
//#endregion
export {
  registerBrowserPlugin as i,
  browserPluginReload as n,
  browserSecurityAuditCollectors as r,
  browserPluginNodeHostCommands as t,
};
