...
import { t as DEFAULT_DANGEROUS_NODE_COMMANDS } from "./node-command-policy-y9_MR0c9.js";
import {
...
async function configureGatewayForSetup(opts) {
  ...
  if (authMode === "token") {
    ...
    if (
      ...
      let tokenInput;
      if (existingToken)
        tokenInput = (await prompter.confirm({
  ...
  if (authMode === "password") {
    let password =
      flow === "quickstart" && quickstartGateway.password ? quickstartGateway.password : void 0;
    if (!password)
      if (
        (await resolveSecretInputModeForEnvSelection({
          prompter,
  ...
  if (
    flow === "quickstart" &&
    bind === "loopback" &&
    nextConfig.gateway?.controlUi?.allowInsecureAuth === void 0
  )
    nextConfig = {
      ...nextConfig,
      gateway: {
  ...
  if (
    ...
    nextConfig.gateway?.nodes?.denyCommands === void 0 &&
    nextConfig.gateway?.nodes?.allowCommands === void 0 &&
    nextConfig.gateway?.nodes?.browser === void 0
  )
    nextConfig = {
      ...nextConfig,
      gateway: {
