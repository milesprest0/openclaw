import { a as resolveSlackAccount, d as resolveSlackBotToken } from "./accounts-Stz3jcMX.js";
import { r as validateSlackBlocksArray } from "./blocks-input-B3PJKopi.js";
import {
  c as resolveTextChunkLimit,
  i as chunkMarkdownTextWithMode,
  s as resolveChunkMode,
} from "./chunk-aBEwc7QQ.js";
import { a as getSlackWriteClient, n as createSlackTokenCacheKey } from "./client-CysiXrBq.js";
import { n as resolveGlobalDedupeCache } from "./dedupe-DxEPwz5a.js";
import {
...
import "./runtime-api-DK5OiS6k.js";
...
function neutralizeStraySlackTildes(text) {
...
function escapeSlackMrkdwnCode(text) {
  if (!text) return "";
  if (!text.includes("&") && !text.includes("<") && !text.includes(">")) return text;
  return text
    .split("\n")
...
function buildSlackLink(link, text) {
  ...
  if (!(trimmedLabel.length > 0 && trimmedLabel !== href && trimmedLabel !== comparableHref))
    return null;
...
const threadParticipation = resolveGlobalDedupeCache(
  Symbol.for("openclaw.slackThreadParticipation"),
  {
    ttlMs: TTL_MS,
    maxSize: MAX_ENTRIES,
  },
...
//#endregion
//#region extensions/slack/src/voice-discipline.ts
/**
 ...
 * returned unchanged.
 */
function applyVoiceDiscipline(message, options = {}) {
  if (options.disabled) return message;
  if (!message) return message;
  const originalLength = message.length;
  let cleaned = message;
  for (const pattern of PROCESS_HEADER_PATTERNS) cleaned = cleaned.replace(pattern, "");
  cleaned = cleaned.replace(/\n{3,}/g, "\n\n");
  cleaned = cleaned.replace(/^\s+/, "");
  cleaned = cleaned.replace(/\s+$/, "");
  const minRemainingFraction = options.minRemainingFraction ?? 0.1;
  if (cleaned.length === 0) return message;
  if (cleaned.length < originalLength * minRemainingFraction) return message;
  return cleaned;
}
//#endregion
...
function isSlackCustomizeScopeError(err) {
  const data = getSlackWebApiErrorData(err);
  if (normalizeLowercaseStringOrEmpty(normalizeSlackApiString(data?.error)) !== "missing_scope")
    return false;
  if (
    normalizeLowercaseStringOrEmpty(normalizeSlackApiString(data?.needed))?.includes(
...
function createSlackSendReceipt(params) {
  return createMessageReceiptFromOutboundResults({
    results: params.platformMessageIds
...
function resolveToken(params) {
  ...
  if (!fallback) {
    logVerbose(
      `slack send: missing bot token for account=${params.accountId} explicit=${Boolean(params.explicit)} source=${params.fallbackSource ?? "unknown"}`,
    );
    throw new Error(
...
async function resolveChannelId(client, recipient, params) {
  ...
  const channelId = (
    await withSlackDnsRequestRetry("conversations.open", () =>
...
async function sendMessageSlack(to, message, opts) {
  const trimmedMessage = applyVoiceDiscipline(normalizeOptionalString(message) ?? "");
  ...
  const result = await runQueuedSlackSend(
    createSlackSendQueueKey({
      accountId: account.accountId,
      token,
      recipient,
...
async function sendMessageSlackQueuedInner(params) {
  ...
  const resolvedChunks = resolveTextChunksWithFallback(
    trimmedMessage,
    (chunkMode === "newline"
...
//#endregion
export {
  recordSlackThreadParticipation as a,
  buildSlackBlocksFallbackText as c,
  hasSlackThreadParticipationWithPersistence as i,
  clearSlackThreadParticipationCache as n,
  markdownToSlackMrkdwnChunks as o,
  hasSlackThreadParticipation as r,
  normalizeSlackOutboundText as s,
  sendMessageSlack as t,
};
