...
import { n as openUrl } from "./browser-open-DH0TmslB.js";
...
import { r as resolveManifestProviderAuthChoice } from "./provider-auth-choices-D-oJr9Ew.js";
import { t as applyAuthProfileConfig } from "./provider-auth-helpers-DWjbCNVf.js";
import { n as resolveProviderInstallCatalogEntry } from "./provider-install-catalog-CqwJizDn.js";
...
async function applyDefaultModelFromAuthChoice(params) {
  ...
  const preservesDifferentPrimary =
    params.preserveExistingDefaultModel === true &&
    previousPrimary !== void 0 &&
    previousPrimary !== params.selectedModel;
  let nextConfig = applyDefaultModel(
    params.preserveExistingDefaultModel === true
  ...
  if (!preservesDifferentPrimary) {
    const { ensureCodexRuntimePluginForModelSelection } =
      await import("./codex-runtime-plugin-install-B1HYLUb_.js");
    nextConfig = (
...
let providerAuthChoiceDeps = {
  loadPluginProviderRuntime: async () => import("./provider-auth-choice.runtime-BVKgj_-J.js"),
};
...
function withProviderPluginId(provider, pluginId) {
  return provider.pluginId === pluginId
    ? provider
    : {
        ...provider,
        pluginId,
...
async function applyAuthChoiceLoadedPluginProvider(params) {
  ...
  const resolveScopedRuntimeProviders = (config) =>
    resolvePluginProviders({
      config,
      workspaceDir,
      env: params.env,
  ...
  if (!resolved && installCatalogEntry) {
    const { ensureOnboardingPluginInstalled } =
      await import("./onboarding-plugin-install-9k1485H6.js");
