...
function openDelegatedOAuthUrl(url) {
  return Promise.reject(
    /* @__PURE__ */ new Error(
      `Automatic browser launch is not available. Open this URL manually: ${url}`,
    ),
  );
}
...
const msteamsGroupAccess = {
  ...
  resolveAllowlist: async ({ cfg, entries, prompter }) =>
    await resolveMSTeamsGroupAllowlist({
      cfg,
      entries,
      prompter,
    }),
  ...
  finalize: async (params) => {
    ...
    if (finalCreds?.type === "secret") {
      if (
        ...
        try {
          const { loginMSTeamsDelegated } = await import("./oauth-BjarCruf.js");
          const progress = params.prompter.progress("MSTeams Delegated OAuth");
          saveDelegatedTokens(
            await loginMSTeamsDelegated(
              {
                isRemote: true,
...
//#endregion
export {
  msteamsSetupAdapter as i,
  openDelegatedOAuthUrl as n,
  createMSTeamsSetupWizardBase as r,
  msteamsSetupWizard as t,
};
