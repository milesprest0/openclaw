...
import { t as collectIncludePathsRecursive } from "./includes-scan-BjedNnfy.js";
...
import { t as extensionUsesSkippedScannerPath } from "./scan-paths-BLf-vGUd.js";
...
function loadSkillsModule() {
  skillsModulePromise ??= import("./skills-D_vWcVag.js");
...
function loadAuditFsModule() {
  auditFsModulePromise ??= import("./audit-fs-DNn64bzI.js");
  return auditFsModulePromise;
}
function loadAgentScopeModule() {
  agentScopeModulePromise ??= import("./agent-scope-Bm8ZjMdO.js");
  return agentScopeModulePromise;
}
function loadAgentWorkspaceDirsModule() {
  agentWorkspaceDirsModulePromise ??= import("./workspace-dirs-B8QD0c48.js");
  return agentWorkspaceDirsModulePromise;
}
function loadSkillSourceModule() {
  skillSourceModulePromise ??= import("./source-ejQhuIrS.js");
  return skillSourceModulePromise;
}
function loadSkillScannerModule() {
  skillScannerModulePromise ??= import("./skill-scanner-CWsYAUXg.js");
  return skillScannerModulePromise;
}
async function loadExecDockerRaw() {
  sandboxDockerModulePromise ??= import("./docker-Cvg5hUPS.js");
...
async function loadSandboxBrowserSecurityHashEpoch() {
  sandboxConstantsModulePromise ??= import("./constants-DimZkJuk.js");
...
async function getCodeSafetySummary(params) {
  ...
  if (cache) {
    ...
    const pending = (await loadSkillScannerModule()).scanDirectoryWithSummary(params.dirPath, {
      includeFiles: params.includeFiles,
    });
  ...
  return await (
    await loadSkillScannerModule()
  ).scanDirectoryWithSummary(params.dirPath, { includeFiles: params.includeFiles });
}
...
async function collectStateDeepFilesystemFindings(params) {
  ...
  const agentIds = Array.isArray(params.cfg.agents?.list)
    ? params.cfg.agents?.list
        .map((a) => normalizeOptionalString(a && typeof a === "object" ? a.id : void 0) ?? "")
        .filter(Boolean)
    : [];
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (summary.critical > 0) {
      const details = formatCodeSafetyDetails(
        summary.findings.filter((f) => f.severity === "critical"),
        pluginPath,
      );
      findings.push({
      ...
      const details = formatCodeSafetyDetails(
        summary.findings.filter((f) => f.severity === "warn"),
        pluginPath,
      );
      findings.push({
        checkId: "plugins.code_safety",
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      ...
      if (summary.critical > 0) {
        ...
        const details = formatCodeSafetyDetails(
          summary.findings.filter((finding) => finding.severity === "warn"),
          skillDir,
        );
        findings.push({
          checkId: "skills.code_safety",
...
//#endregion
export {
  collectStateDeepFilesystemFindings as a,
  collectSandboxBrowserHashLabelFindings as i,
  collectInstalledSkillsCodeSafetyFindings as n,
  readConfigSnapshotForAudit as o,
  collectPluginsCodeSafetyFindings as r,
  shouldIgnoreInstalledPluginDirName as s,
  collectIncludeFilePermFindings as t,
};
