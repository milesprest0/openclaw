...
import { t as parseCmdScriptCommandLine } from "./cmd-argv-4E5k3WN1.js";
...
import { n as resolveLsofCommandSync } from "./ports-lsof-BZ5Y2Meo.js";
...
function pollPortOnce(port) {
  ...
  try {
    ...
    if (res.status === 1) {
      if (res.stdout)
        return parsePidsFromLsofOutput(res.stdout, POLL_SPAWN_TIMEOUT_MS).length === 0
          ? { free: true }
          : { free: false };
      return { free: true };
    }
  ...
  try {
    const port =
      typeof portOverride === "number" && Number.isFinite(portOverride) && portOverride > 0
        ...
        : resolveGatewayPort(void 0, process.env);
    const stalePids =
...
//#endregion
export {
  readWindowsProcessArgsSync as a,
  readWindowsListeningPidsOnPortSync as i,
  findGatewayPidsOnPortSync as n,
  getSelfAndAncestorPidsSync as r,
  cleanStaleGatewayProcessesSync as t,
};
