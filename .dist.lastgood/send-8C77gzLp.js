...
import { n as isAutoLinkedFileRef } from "./auto-linked-file-ref-8uLN_I32.js";
import {
  a as EventType,
  c as MsgType,
  l as RelationType,
  n as persistMatrixDirectRoomMapping,
  s as MSC4357_LIVE_KEY,
  t as inspectMatrixDirectRooms,
} from "./direct-management-FqagZ_db.js";
import { i as isStrictDirectRoom } from "./direct-room-DNGwJuvi.js";
import { r as parseBuffer } from "./lib-DWGa7aOi.js";
import "./channel-message-Do1b_D-M.js";
import "./string-coerce-runtime-Xx8DhtgQ.js";
import "./text-autolink-runtime-pm2gIP3r.js";
...
import { r as buildMatrixReactionContent } from "./reaction-common-Dher7fo3.js";
...
async function loadMatrixSendClientRuntime() {
  matrixSendClientRuntimePromise ??= import("./client-bootstrap-Jq3tIbYp.js");
...
async function renderMatrixFormattedContent(params) {
  ...
  if (params.includeMentions === false)
    return { html: markdownToMatrixHtml(markdown).trimEnd() || void 0 };
...
function buildMediaContent(params) {
  ...
  if (params.isVoice) {
    ...
    if (typeof params.durationMs === "number")
      base["org.matrix.msc1767.audio"] = { duration: params.durationMs };
  }
...
async function prepareImageInfo(params) {
  ...
  if (Math.max(meta.width, meta.height) > THUMBNAIL_MAX_SIDE)
    try {
      ...
      const thumbMeta = await getCore$1()
...
async function resolveMediaDurationMs(params) {
  ...
  try {
    ...
    const durationSeconds = (
      await parseBuffer(params.buffer, fileInfo, {
        duration: true,
        skipCovers: true,
      })
    ).format.duration;
...
async function resolveMatrixRoomId(client, raw) {
  ...
  if (normalizeLowercaseStringOrEmpty(target).startsWith("user:"))
    return await resolveDirectRoomId(client, target.slice(5));
  ...
  return await withResolvedMatrixSendClient(
    ...
    async (client) => {
      ...
      const newContent = withMatrixExtraContentFields(
        buildTextContent(convertedText, void 0, { msgtype: opts.msgtype }),
        opts.extraContent,
      );
      await enrichMatrixFormattedContent({
        client,
...
export {
  parsePollStart as _,
  sendMessageMatrix as a,
  sendSingleTextMessageMatrix as c,
  buildPollResponseContent as d,
  buildPollResultsSummary as f,
  isPollStartType as g,
  isPollEventType as h,
  reactMatrixMessage as i,
  sendTypingMatrix as l,
  formatPollResultsAsText as m,
  editMessageMatrix as n,
  sendPollMatrix as o,
  formatPollAsText as p,
  prepareMatrixSingleText as r,
  sendReadReceiptMatrix as s,
  chunkMatrixText as t,
  resolveMatrixRoomId as u,
  parsePollStartContent as v,
  resolvePollReferenceEventId as y,
};
