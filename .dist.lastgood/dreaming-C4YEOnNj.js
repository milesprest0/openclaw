import {
  B as MEMORY_DREAMING_SYSTEM_EVENT_TEXT,
  J as resolveMemoryDreamingWorkspaces,
  R as MANAGED_MEMORY_DREAMING_CRON_NAME,
  U as resolveMemoryCorePluginConfig,
  W as resolveMemoryDeepDreamingConfig,
  z as MANAGED_MEMORY_DREAMING_CRON_TAG,
} from "./dreaming-BDr8qIGr.js";
import {
  c as runDetachedDreamNarrative,
  o as generateAndAppendDreamNarrative,
  r as runDreamingSweepPhases,
  u as writeDeepDreamingReport,
} from "./dreaming-phases-D2W5WvGC.js";
import {
  n as normalizeTrimmedString,
  t as includesSystemEventToken,
} from "./dreaming-shared-IEVm2jng.js";
...
import { c as peekSystemEventEntries } from "./system-events-CttxBeh2.js";
...
function isManagedDreamingJob(job) {
  if (
    normalizeTrimmedString(job.description)?.includes(
  ...
  return (
    ...
    payloadToken === "__openclaw_memory_core_short_term_promotion_dream__"
  );
}
function isLegacyPhaseDreamingJob(job) {
  const description = normalizeTrimmedString(job.description);
  if (
    description?.includes("[managed-by=memory-core.dreaming.light]") ||
    description?.includes("[managed-by=memory-core.dreaming.rem]")
  )
    return true;
  const name = normalizeTrimmedString(job.name);
  const payloadText = normalizeTrimmedString(job.payload?.text);
  if (name === "Memory Light Dreaming" && payloadText === "__openclaw_memory_core_light_sleep__")
    return true;
...
function buildManagedDreamingPatch(job, desired) {
  ...
  if (
    normalizeLowercaseStringOrEmpty(normalizeTrimmedString(job.sessionTarget)) !==
    desired.sessionTarget
  )
    patch.sessionTarget = desired.sessionTarget;
  if (normalizeLowercaseStringOrEmpty(normalizeTrimmedString(job.wakeMode)) !== "now")
    patch.wakeMode = "now";
  ...
  if (normalizeLowercaseStringOrEmpty(normalizeTrimmedString(job.delivery?.mode)) !== "none")
  ...
  return resolveDreamingTriggerSessionKeys(sessionKey).some((candidateSessionKey) =>
    peekSystemEventEntries(candidateSessionKey).some(
      (event) =>
        ...
        normalizeTrimmedString(event.text) ===
...
function registerShortTermPromotionDreaming(api) {
  ...
  const scheduleStartupCronRetry = (config) => {
    ...
    startupCronRetryTimer = setTimeout(() => {
      ...
      reconcileManagedDreamingCron({ reason: "runtime" })
        .then((latestConfig) => {
