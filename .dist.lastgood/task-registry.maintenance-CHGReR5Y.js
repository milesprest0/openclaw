import { r as isCronJobActive } from "./active-jobs-qoCzunMt.js";
import { c as getAgentRunContext } from "./agent-events-c6-2eZvI.js";
import {
  o as getDetachedTaskLifecycleRuntime,
  u as tryRecoverTaskBeforeMarkLost,
...
import {
  i as sweepExpiredPluginStateEntries,
  r as isPluginStateDatabaseOpen,
} from "./plugin-state-store-CQXPZ-Hk.js";
import { a as resolveCronRunLogPath, i as readCronRunLogEntriesSync } from "./run-log-C0aatsE-.js";
...
import { n as readAcpSessionEntry, t as listAcpSessionEntries } from "./session-meta-rwd0ZmpO.js";
import { n as loadCronStoreSync, r as resolveCronStorePath } from "./store-CuwH2dda.js";
...
import {
  i as isSubagentRecoveryWedgedEntry,
  r as formatSubagentRecoveryWedgedReason,
} from "./subagent-recovery-state-B5uZX9-x.js";
import { t as createSubsystemLogger } from "./subsystem-Bjz8a2fE.js";
import {
  C as resolveTaskForLookupToken,
  _ as markTaskLostById,
  b as maybeDeliverTaskTerminalUpdate,
  c as hasActiveTaskForChildSessionKey,
  d as listTaskRecords,
  i as ensureTaskRegistryReady,
  r as deleteTaskRecordById,
  s as getTaskById,
  w as setTaskCleanupAfterById,
  y as markTaskTerminalById,
} from "./task-registry-Bkiwt9Td.js";
import {
  n as listTaskAuditFindings,
  r as summarizeTaskAuditFindings,
  t as configureTaskAuditTaskProvider,
...
function getSessionStoreLookup(storePath, context) {
  if (!context)
    return { store: taskRegistryMaintenanceRuntime.loadSessionStore(storePath, { clone: false }) };
...
function getNormalizedSessionEntries(lookup) {
  ...
  for (const [key, entry] of Object.entries(lookup.store))
    if (entry) entries.set(normalizeLowercaseStringOrEmpty(key), entry);
...
function findTaskSessionEntry(task, context) {
  ...
  return findSessionEntryByKey(
    getSessionStoreLookup(
...
function resolveCronRunLogRecovery(execution, context) {
  const entry = getCronRunLogEntries(context, execution.jobId).findLast(
    (candidate) =>
...
function isParentOwnedAcpSessionTask(task, acpEntry) {
  ...
  return getAcpSessionParentKeys({ entry }).some(
...
function shouldCloseOrphanedParentOwnedAcpSession(acpEntry) {
  ...
  if (!sessionKey || taskRegistryMaintenanceRuntime.hasActiveTaskForChildSessionKey({ sessionKey }))
    return false;
  if (acpEntry.acp.mode === "oneshot") return true;
  return !hasActiveSessionBinding(sessionKey);
}
...
function configureTaskRegistryMaintenance(options) {
  ...
  if (options.cronRuntimeAuthoritative !== void 0)
...
//#endregion
export {
  getReconciledTaskById as a,
  reconcileTaskLookupToken as c,
  runTaskRegistryMaintenance as d,
  setTaskRegistryMaintenanceRuntimeForTests as f,
  sweepTaskRegistry as g,
  stopTaskRegistryMaintenanceForTests as h,
  getInspectableTaskRegistrySummary as i,
  reconcileTaskRecordForOperatorInspection as l,
  stopTaskRegistryMaintenance as m,
  getInspectableActiveTaskRestartBlockers as n,
  previewTaskRegistryMaintenance as o,
  startTaskRegistryMaintenance as p,
  getInspectableTaskAuditSummary as r,
  reconcileInspectableTasks as s,
  configureTaskRegistryMaintenance as t,
  resetTaskRegistryMaintenanceRuntimeForTests as u,
};
