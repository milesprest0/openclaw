...
import { a as resolveWhatsAppAccount, o as resolveWhatsAppAuthDir } from "./accounts-MM0YxrKm.js";
import {
...
import { u as pathExists } from "./utils-BGRcpLKt.js";
...
function resolveDefaultWhatsAppAccountWriteKey(cfg) {
  ...
  return (
    Object.keys(accounts).find((accountId) => isDefaultWhatsAppAccountKey(accountId)) ?? "default"
  );
}
function resolveWhatsAppConfigPathPrefix(cfg, accountId) {
  if (accountId === "default" && shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg))
...
function mergeWhatsAppConfig(cfg, accountId, patch, options) {
  ...
  const targetAccountId =
    accountId === "default" ? resolveDefaultWhatsAppAccountWriteKey(cfg) : accountId;
  const lowerDefaultAccount =
    accountId === "default" && targetAccountId !== "default"
...
async function promptWhatsAppOwnerAllowFrom(params) {
  ...
  const normalized = normalizeWhatsAppAllowFromEntry(
    trimPromptText(
      await prompter.text({
  ...
  return {
    normalized,
    allowFrom: normalizeWhatsAppAllowFromEntries([
...
async function promptWhatsAppDmAccess(params) {
  ...
  const parsed = parseWhatsAppAllowFromEntries(
    trimPromptText(
...
async function finalizeWhatsAppSetup(params) {
  ...
  if (
    ...
    try {
      const { loginWeb } = await import("./login-BrgUbiVl.js");
...
const whatsappSetupWizard = {
  ...
  status: {
    ...
    resolveConfigured: async ({ cfg, accountId }) => {
      for (const resolvedAccountId of accountId ? [accountId] : listAccountIds(cfg))
        if ((await readWhatsAppSetupLinkState(cfg, resolvedAccountId)) === "linked") return true;
      return false;
    },
    resolveStatusLines: async ({ cfg, accountId, configured }) => {
      const linkedAccountId = (
        await Promise.all(
          (accountId ? [accountId] : listAccountIds(cfg)).map(async (resolvedAccountId) => ({
      ...
      return [
        `${labelAccountId ? `WhatsApp (${labelAccountId === "default" ? "default" : labelAccountId})` : "WhatsApp"}: ${configured ? formatWhatsAppWebAuthStatusState("linked") : formatWhatsAppWebAuthStatusState(linkedAccountId?.state ?? "not-linked")}`,
      ];
    },
  },
