...
import { c as it, i as beforeEach } from "./dist-BosyQhji.js";
import { a as resetGlobalHookRunner } from "./hook-runner-global-DzzZF3B1.js";
...
function expectChannelInboundContextContract(ctx) {
  ...
  if (chatType && chatType !== "direct")
    globalExpect(ctx.ConversationLabel?.trim() || resolveConversationLabel(ctx)).toBeTruthy();
}
function expectChannelTurnDispatchResultContract(result, expected) {
  ...
  if (expected.final !== void 0)
    globalExpect(hasFinalChannelTurnDispatch(result)).toBe(expected.final);
  if (expected.counts)
    globalExpect(resolveChannelTurnDispatchCounts(result)).toMatchObject(expected.counts);
}
//#endregion
...
function installChannelOutboundPayloadContractSuite(params) {
  ...
  it("single media delegates to sendMedia", async () => {
    ...
    const result = await run();
    globalExpect(sendMock).toHaveBeenCalledTimes(1);
    globalExpect(sendMock).toHaveBeenCalledWith(
      to,
      "cap",
      globalExpect.objectContaining({ mediaUrl: "https://example.com/a.jpg" }),
    );
    globalExpect(result).toMatchObject({ channel: params.channel });
  });
  it("multi-media iterates URLs with caption on first", async () => {
    ...
    globalExpect(sendMock).toHaveBeenNthCalledWith(
      1,
      to,
      "caption",
      globalExpect.objectContaining({ mediaUrl: "https://example.com/1.jpg" }),
    );
    globalExpect(sendMock).toHaveBeenNthCalledWith(
      2,
      to,
      "",
      globalExpect.objectContaining({ mediaUrl: "https://example.com/2.jpg" }),
    );
  ...
  it("chunking splits long text", async () => {
    ...
    for (const call of sendMock.mock.calls)
      globalExpect(call[1].length).toBeLessThanOrEqual(chunking.maxChunkLength);
    globalExpect(result).toMatchObject({ channel: params.channel });
  });
}
//#endregion
...
//#endregion
export {
  primeChannelOutboundSendMock as a,
  expectChannelTurnDispatchResultContract as i,
  installChannelOutboundPayloadContractSuite as n,
  expectChannelInboundContextContract as r,
  buildDispatchInboundCaptureMock as t,
};
