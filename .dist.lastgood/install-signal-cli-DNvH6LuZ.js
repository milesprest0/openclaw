...
import { t as extractArchive } from "./archive-CfKTdlm5.js";
...
function pickAsset(assets, platform, arch) {
  const archives = assets
    .filter((asset) => Boolean(asset.name && asset.browser_download_url))
    .filter((a) => looksLikeArchive(normalizeLowercaseStringOrEmpty(a.name)));
...
async function downloadToFile(
  ...
  try {
    ...
    await pipeline(
      isNodeReadableStream(body) ? body : Readable.fromWeb(body),
      new Transform({
        transform(chunk, _encoding, callback) {
          ...
          if (totalBytes > maxBytes) {
            callback(
              /* @__PURE__ */ new Error(
                `signal-cli archive exceeded the ${maxBytes}-byte download cap.`,
              ),
            );
            return;
          }
...
async function installSignalCliFromRelease(runtime) {
  ...
  const tmpDir = await fs$1.mkdtemp(
...
//#endregion
export {
  looksLikeArchive as a,
  installSignalCliFromRelease as i,
  extractSignalCliArchive as n,
  pickAsset as o,
  installSignalCli as r,
  downloadToFile as t,
};
