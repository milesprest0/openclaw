import { c as resolveHeartbeatPromptForSystemPrompt } from "./bootstrap-files-C8WUuJVf.js";
import { t as buildEmbeddedCompactionRuntimeContext } from "./compaction-runtime-context-CjeIjGez.js";
import { t as resolveContextEngineCapabilities } from "./context-engine-capabilities-Bcmv_lio.js";
import { c as joinPresentTextSegments } from "./hook-runner-global-DzzZF3B1.js";
import { m as drainPluginNextTurnInjectionContext } from "./loader-BLowS1kl.js";
import { t as log } from "./logger-Ce0PtXjG.js";
import { A as buildPluginAgentTurnPrepareContext } from "./runtime-ChXyIFtl.js";
import { a as isSubagentSessionKey, i as isCronSessionKey } from "./session-key-utils-B3KPN8Ee.js";
import { n as prependSystemPromptAdditionAfterCacheBoundary } from "./system-prompt-cache-boundary-Cq7GvFY6.js";
import { r as resolveEffectiveToolFsWorkspaceOnly } from "./tool-fs-policy-BBWoCrOZ.js";
import { n as derivePromptTokens } from "./usage-BOaLms3p.js";
import {
  n as buildActiveVideoGenerationTaskPromptContextForSession,
  s as buildActiveMusicGenerationTaskPromptContextForSession,
...
function shouldInjectHeartbeatPromptForTrigger(trigger) {
  return (
    (trigger ? EMBEDDED_RUN_TRIGGER_POLICY[trigger] : void 0)?.injectHeartbeatPrompt ??
...
function summarizeStructuredMediaRef(label, value) {
  ...
  if (dataUriMatch)
    return `[${label}] inline data URI (${dataUriMatch[1]?.trim() || "unknown"}, ${trimmed.length} chars)`;
...
function stringifyStructuredJsonFallback(part) {
  try {
    ...
    return withoutInlineData.length > 1e3
      ? `${withoutInlineData.slice(0, 1e3)}... (${withoutInlineData.length} chars)`
      : withoutInlineData;
  } catch {
    return;
  }
...
function stringifyStructuredContentPart(part) {
  ...
  const record = part;
  if (record.type === "text")
    return (typeof record.text === "string" ? record.text.trim() : "") || void 0;
  if (record.type === "image_url") {
    const imageUrl = record.image_url;
    return summarizeStructuredMediaRef(
      "image_url",
...
//#endregion
export {
  prependSystemPromptAddition as a,
  resolvePromptBuildHookResult as c,
  shouldInjectHeartbeatPrompt as d,
  shouldWarnOnOrphanedUserRepair as f,
  mergeOrphanedTrailingUserPrompt as i,
  resolvePromptModeForSession as l,
  buildAfterTurnRuntimeContextFromUsage as n,
  resolveAttemptFsWorkspaceOnly as o,
  forgetPromptBuildDrainCacheForRun as r,
  resolveAttemptPrependSystemContext as s,
  buildAfterTurnRuntimeContext as t,
  resolvePromptSubmissionSkipReason as u,
};
