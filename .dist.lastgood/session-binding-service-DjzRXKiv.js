...
import { t as loadJsonFile } from "./json-file-DY7oy2KR.js";
import { r as saveJsonFile } from "./json-store-C6iFE4qW.js";
import { v as resolveStateDir } from "./paths-Cnwfh6dH.js";
import { a as normalizeAnyChannelId } from "./registry-V0yq5bwZ.js";
import { t as getActivePluginChannelRegistryFromState } from "./runtime-channel-state-CefjemMO.js";
import {
  i as normalizeConversationText,
  n as normalizeConversationRef,
  t as buildChannelAccountKey,
...
function toPersistedFile() {
  return {
    ...
    bindings: [...bindingsByConversationKey.values()]
...
function resolveChannelSupportsCurrentConversationBinding(channel) {
  ...
  if (
    (getActivePluginChannelRegistryFromState()?.channels ?? []).find((entry) =>
...
function resolveAdapterPlacements(adapter) {
  const placements = adapter.capabilities?.placements?.map((value) =>
...
const ADAPTERS_BY_CHANNEL_ACCOUNT = resolveGlobalMap(
  Symbol.for("openclaw.sessionBinding.adapters"),
);
...
function createDefaultSessionBindingService() {
  return {
    bind: async (input) => {
      ...
      const placement =
        normalizePlacement(input.placement) ?? inferDefaultPlacement(normalizedConversation);
      if (!resolveAdapterPlacements(adapter).includes(placement))
        throw new SessionBindingError(
          "BINDING_CAPABILITY_UNSUPPORTED",
          `Session binding placement "${placement}" is not supported for ${normalizedConversation.channel}:${normalizedConversation.accountId}`,
          {
...
//#endregion
export {
  registerSessionBindingAdapter as a,
  isSessionBindingError as i,
  __testing as n,
  unregisterSessionBindingAdapter as o,
  getSessionBindingService as r,
  SessionBindingError as t,
};
