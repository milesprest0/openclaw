...
import { t as isChutesModelDiscoveryTestEnvironment } from "./model-discovery-env-BqP4kGlN.js";
...
function pruneExpiredCacheEntries(now = Date.now()) {
  for (const [key, entry] of modelCache.entries())
    if (now - entry.time >= CACHE_TTL) modelCache.delete(key);
}
  ...
  try {
    ...
    try {
      ...
      for (const entry of data) {
        ...
        const input = (entry.input_modalities || ["text"]).filter(
          (i) => i === "text" || i === "image",
        );
        models.push({
          id,
...
//#endregion
export {
  buildChutesModelDefinition as a,
  CHUTES_MODEL_CATALOG as i,
  CHUTES_DEFAULT_MODEL_ID as n,
  clearChutesModelCacheForTests as o,
  CHUTES_DEFAULT_MODEL_REF as r,
  discoverChutesModels as s,
  CHUTES_BASE_URL as t,
};
