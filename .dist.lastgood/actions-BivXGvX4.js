...
import { a as getSlackWriteClient, r as createSlackWebClient } from "./client-CysiXrBq.js";
import { r as logVerbose } from "./globals-BdfwDi2E.js";
import { a as resolveSlackMedia } from "./media-DIWB0wJO.js";
import { t as requireRuntimeConfig } from "./plugin-config-runtime-OXf9zy-H.js";
import { c as buildSlackBlocksFallbackText, t as sendMessageSlack } from "./send-JE0jcFcj.js";
...
function resolveToken(explicit, accountId, cfg) {
  ...
  if (!token) {
    logVerbose(
      `slack actions: missing bot token for account=${account.accountId} explicit=${Boolean(explicit)} source=${account.botTokenSource ?? "unknown"}`,
    );
...
function collectSlackThreadShares(file, channelId) {
  ...
  for (const shareMap of collectSlackShareMaps(file)) {
    ...
    for (const rawEntry of rawEntries) {
      ...
      const threadTs =
        typeof entry.thread_ts === "string" ? normalizeSlackScopeValue(entry.thread_ts) : void 0;
      matches.push({
        channelId,
        ts,
        threadTs,
...
//#endregion
export {
  listSlackEmojis as a,
  pinSlackMessage as c,
  removeOwnSlackReactions as d,
  removeSlackReaction as f,
  buildSlackEditTextPayload as h,
  getSlackMemberInfo as i,
  reactSlackMessage as l,
  unpinSlackMessage as m,
  downloadSlackFile as n,
  listSlackPins as o,
  sendSlackMessage as p,
  editSlackMessage as r,
  listSlackReactions as s,
  deleteSlackMessage as t,
  readSlackMessages as u,
};
