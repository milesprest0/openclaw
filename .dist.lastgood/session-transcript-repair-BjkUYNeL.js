...
import {
  a as SESSIONS_SPAWN_ATTACHMENT_METADATA_KEYS,
  c as isRedactedSessionsSpawnAttachment,
  i as REDACTED_SESSIONS_SPAWN_ATTACHMENT_CONTENT,
  l as normalizeAllowedToolNames,
  n as extractToolResultId,
  s as isAllowedToolCallName,
...
function redactSessionsSpawnAcpArgs(value) {
  ...
  const next = { ...rec };
  let changed = false;
  for (const key of ["resumeSessionId", "streamTo"])
        ...
        if (RAW_TOOL_CALL_BLOCK_TYPES.has(block.type ?? "")) {
          if (
            normalizeLowercaseStringOrEmpty(
              typeof block.name === "string" ? block.name.trim() : void 0,
            ) === "sessions_spawn"
          ) {
...
//#endregion
export {
  stripToolResultDetails as a,
  sanitizeToolUseResultPairing as i,
  repairToolUseResultPairing as n,
  sanitizeToolCallInputs as r,
  makeMissingToolResult as t,
};
