import { n as resolveModelDirectiveSelection } from "./model-selection-directive-BQBtvZad.js";
import {
  _ as modelKey,
...
import "./model-selection-oJf-VcPz.js";
...
function resolveProfileOverride(params) {
  ...
  const profile = ensureAuthProfileStore(params.agentDir, { allowKeychainPrompt: false }).profiles[
    raw
  ];
  if (!profile) return { error: `Auth profile "${raw}" not found.` };
  if (profile.provider !== params.provider)
    return { error: `Auth profile "${raw}" is for ${profile.provider}, not ${params.provider}.` };
  return { profileId: raw };
}
//#endregion
...
function resolveModelSelectionFromDirective(params) {
  ...
  const storedNumericProfile =
    params.directives.rawModelProfile === void 0
  ...
  const useStoredNumericProfile =
    ...
    resolveProviderIdForAuth(storedNumericProfileSelection?.selection?.provider ?? "", {
      ...
      resolveProviderIdForAuth(storedNumericProfile?.profileProvider ?? "", { config: params.cfg });
  const modelRaw =
  ...
  const rawProfile =
    ...
    (useStoredNumericProfile ? storedNumericProfile?.profileId : void 0);
