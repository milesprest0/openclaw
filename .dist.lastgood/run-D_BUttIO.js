...
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-B9OXSyzf.js";
import { t as createLazyImportLoader } from "./lazy-promise-B1nDYoKn.js";
import { C as setVerbose } from "./logger-BU4ooQvN.js";
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-BQYp2xgJ.js";
...
import { n as setGatewayWsLogStyle } from "./ws-logging-CNiCpyaB.js";
...
const gatewayLifecycleRuntimeLoader = createLazyImportLoader(
  () => import("./cli/gateway-lifecycle.runtime.js"),
);
...
async function runGatewayLoop(params) {
  ...
  const resolveRestartDrainTimeoutMs = async (restartIntent) => {
    ...
    if (typeof restartIntent?.waitMs === "number" && Number.isFinite(restartIntent.waitMs))
      return restartIntent.waitMs > 0 ? Math.floor(restartIntent.waitMs) : void 0;
    try {
          ...
          if (activeTasks > 0 || activeRuns > 0) {
            ...
            if (restartIntent?.force) {
              ...
              if (tasksDrain.drained && runsDrain.drained)
                gatewayLog$1.info("all active work drained");
              else {
...
const GATEWAY_AUTH_MODES = ["none", "token", "password", "trusted-proxy"];
const GATEWAY_TAILSCALE_MODES = ["off", "serve", "funnel"];
...
async function resolveGatewayPasswordOption(opts) {
  ...
  if (file) {
    const { readSecretFromFile } = await import("./secret-file-Du3OoTqU.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
  ...
  const { resolveControlUiRootSync } = await import("./control-ui-assets-zuIcWMIF.js");
  if (
...
async function runGatewayLoopWithSupervisedLockRecovery(params) {
  ...
  const sleep =
    params.sleep ?? (async (ms) => await new Promise((resolve) => setTimeout(resolve, ms)));
...
async function maybeWriteGatewayStartupFailureBundle(err) {
  const { writeDiagnosticStabilityBundleForFailureSync } =
    await import("./diagnostic-stability-bundle-CZhi4mE0.js");
...
async function runGatewayCommand(opts) {
  ...
  const { startGatewayServer } = await startupTrace.measure("cli.server-import", () =>
    withProgress(
      ...
      async () => import("./server-MRyHLbcM.js"),
    ),
  );
  setConsoleTimestampPrefix(true);
  if (devMode) {
    const { ensureDevGatewayConfig } = await import("./dev-wHpXtvmU.js");
  ...
  const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } =
    await import("./future-version-guard-B5nttD5l.js");
  ...
  const VALID_BIND_MODES = new Set(["loopback", "lan", "auto", "custom", "tailnet"]);
  ...
  if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
    const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-RQKfzDJT.js");
  ...
  if (opts.force)
    try {
      const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-JnjjMpJN.js");
  ...
  const { detectRespawnSupervisor } = await import("./supervisor-markers-D9agLp73.js");
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      try {
        const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-QTdgISD-.js");
        const diagnostics = await inspectPortUsage(port);
        if (diagnostics.status === "busy")
          for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
      } catch {}
      const { maybeExplainGatewayServiceStop } = await import("./shared-DDlKj2W4.js");
