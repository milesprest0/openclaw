...
import { n as pickBestEffortPrimaryLanIPv4 } from "./network-discovery-display-B5ny5Zpe.js";
import {
...
function initSelfPresence() {
    ...
    if (os.platform() === "darwin") {
      const out =
        normalizeOptionalString(
          spawnSync("sysctl", ["-n", "hw.model"], { encoding: "utf-8" }).stdout,
        ) ?? "";
  ...
  const macOSVersion = () => {
    const out =
      normalizeOptionalString(
        spawnSync("sw_vers", ["-productVersion"], { encoding: "utf-8" }).stdout,
      ) ?? "";
