...
import { n as resolveEffectiveOAuthCredential$1 } from "./oauth-t1ja4_ir.js";
import { r as normalizeProviderId } from "./provider-id-CG9pXYPs.js";
import {
  D as resolveTokenExpiryState,
  T as evaluateStoredCredentialEligibility,
...
function buildProfileHealth(params) {
  ...
  if (healthCredential.type === "token") {
    ...
    const expiresAt =
      resolveTokenExpiryState(healthCredential.expires, now) === "valid"
        ? healthCredential.expires
        : void 0;
    if (!expiresAt)
      return {
        profileId,
...
function buildAuthHealthSummary(params) {
  ...
  return {
    ...
    warnAfterMs,
    profiles,
    providers: Array.from(providersMap.values()).toSorted((a, b) =>
