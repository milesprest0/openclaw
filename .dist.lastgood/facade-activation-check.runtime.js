...
import { r as resolveRegistryPluginModuleLocationFromRecords } from "./facade-resolution-shared-DsUbSXyf.js";
...
import {
  i as configMayNeedPluginAutoEnable,
  t as applyPluginAutoEnable,
} from "./plugin-auto-enable-DBf3GcqW.js";
import {
...
function readFacadeBoundaryConfigSafely() {
  try {
    ...
    return {
      rawConfig: parsed && typeof parsed === "object" ? parsed : EMPTY_FACADE_BOUNDARY_CONFIG,
    };
  } catch {
...
function resolveBundledMetadataManifestRecord(params) {
  ...
  if (params.location.modulePath.startsWith(`${params.sourceExtensionsRoot}${path.sep}`)) {
    const resolvedDirName = path
      .relative(params.sourceExtensionsRoot, params.location.modulePath)
      .split(path.sep)[0];
  ...
  if (!params.location.modulePath.startsWith(`${normalizedBundledPluginsDir}${path.sep}`))
    return null;
  const resolvedDirName = path
    .relative(normalizedBundledPluginsDir, params.location.modulePath)
    .split(path.sep)[0];
  if (!resolvedDirName) return null;
  return readBundledPluginManifestRecordFromDir({
