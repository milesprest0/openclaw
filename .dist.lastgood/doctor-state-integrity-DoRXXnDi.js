...
import { s as resolveAgentModelFallbacksOverride } from "./agent-scope-9AmhTwki.js";
import {
...
import { f as listConfiguredChannelIdsForReadOnlyScope } from "./channel-plugin-ids-C9CG3lVz.js";
import { t as formatCliCommand } from "./command-format-yMavKibC.js";
import { a as listPluginDoctorSessionRouteStateOwners } from "./doctor-contract-registry-dUz0Xw7s.js";
import {
  n as isHeartbeatOkResponse,
  r as isHeartbeatUserMessage,
} from "./heartbeat-filter-B7WZaEWz.js";
...
import { _ as modelKey, y as parseModelRef } from "./model-selection-shared-D9oQINZ2.js";
...
import { t as asNullableObjectRecord } from "./record-coerce-BuYFJfzm.js";
...
import {
  ...
  t as clearWedgedSubagentRecoveryAbort,
...
async function repairHeartbeatPoisonedMainSession(params) {
  ...
  if (clearedPointers > 0)
    params.changes.push(
      `- Cleared ${countLabel$2(clearedPointers, "stale TUI last-session pointer")} for ${mainKey}.`,
    );
}
//#endregion
...
function entryMayContainPluginSessionRouteState(entry) {
  const record = entry;
  return (
    normalizeString(record.providerOverride) !== void 0 ||
    normalizeString(record.modelOverride) !== void 0 ||
    normalizeString(record.modelOverrideSource) !== void 0 ||
    record.liveModelSwitchPending !== void 0 ||
    normalizeString(record.modelProvider) !== void 0 ||
    normalizeString(record.model) !== void 0 ||
    normalizeString(record.agentHarnessId) !== void 0 ||
    record.cliSessionBindings !== void 0 ||
    record.cliSessionIds !== void 0 ||
    normalizeString(record.authProfileOverride) !== void 0 ||
    normalizeString(record.authProfileOverrideSource) !== void 0
  );
}
  ...
  return params.cliSessionKeys.some((key) => {
    ...
    return (
      (bindings !== null &&
        ...
        bindings[normalized] !== void 0) ||
      (ids !== null && typeof ids === "object" && normalized in ids && ids[normalized] !== void 0)
    );
  });
}
...
function scanEntryForOwner(params) {
  ...
  if (
    !routeAllowsOwner &&
    !(directOverrideIsOwned && directOverrideSource !== void 0 && directOverrideSource !== "auto")
  ) {
  ...
  if (scan.repairs.length > 0)
    for (const [ownerLabel, repairs] of groupRepairsByOwner(scan.repairs)) {
      ...
      if (
        ...
        if (repaired > 0)
          params.changes.push(
            `- Cleared stale ${ownerLabel} session routing state for ${countSessionLabel(repaired)}.`,
          );
      }
    }
  ...
  if (entries.length > 0) {
    ...
    if (wedgedSubagentSessions.length > 0) {
      ...
      if (
        ...
        if (repaired > 0)
          changes.push(
            `- Cleared aborted restart-recovery flags for ${countLabel(repaired, "wedged subagent session")}.`,
          );
      }
