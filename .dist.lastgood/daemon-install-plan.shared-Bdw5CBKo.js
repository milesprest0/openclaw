...
import {
  n as findFirstAccessibleGatewayEntrypoint,
  r as isGatewayDistEntrypointPath,
  t as buildGatewayDistEntrypointCandidates,
} from "./gateway-entrypoint-Dqwpv1Qt.js";
import {
  a as resolveSystemNodeInfo,
  i as resolvePreferredNodePath,
...
async function resolveCliEntrypointPathForService() {
  ...
  if (isGatewayDistEntrypointPath(resolvedPath)) {
    ...
    if (isGatewayDistEntrypointPath(normalized) && normalized !== resolvedPath)
      try {
...
async function resolveBinaryPath(binary) {
  ...
  try {
    const resolved = execFileSync(cmd, [binary], { encoding: "utf8" })
      .trim()
      .split(/\r?\n/)[0]
      ?.trim();
...
async function resolveCliProgramArguments(params) {
  ...
  if (!params.dev)
    try {
      return {
        programArguments: [execPath, await resolveCliEntrypointPathForService(), ...params.args],
      };
    } catch (error) {
  ...
  return {
    programArguments: [await resolveBunPath(), devCliPath, ...params.args],
    workingDirectory: repoRoot,
  };
}
async function resolveGatewayProgramArguments(params) {
  return resolveCliProgramArguments({
    args: ["gateway", "--port", String(params.port)],
    dev: params.dev,
...
async function emitNodeRuntimeWarning(params) {
  ...
  const warning = renderSystemNodeWarning(
    await resolveSystemNodeInfo({ env: params.env }),
    params.nodeProgram,
  );
...
//#endregion
export {
  resolveGatewayProgramArguments as a,
  OPENCLAW_WRAPPER_ENV_KEY as i,
  resolveDaemonInstallRuntimeInputs as n,
  resolveNodeProgramArguments as o,
  resolveDaemonNodeBinDir as r,
  resolveOpenClawWrapperPath as s,
  emitDaemonInstallRuntimeWarning as t,
};
