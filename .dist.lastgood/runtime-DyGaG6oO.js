...
import {
  n as getActiveRuntimeWebToolsMetadata$1,
  r as setActiveRuntimeWebToolsMetadata,
  t as clearActiveRuntimeWebToolsMetadata,
} from "./runtime-web-tools-state-BGr6b-K6.js";
...
function hasRuntimeWebToolConfigSurface(config) {
  ...
  const fetchExplicitlyDisabled =
    ...
    typeof web === "object" &&
    !Array.isArray(web) &&
    typeof web.fetch === "object" &&
    web.fetch?.enabled === false;
  if (web && typeof web === "object" && !Array.isArray(web)) {
    const webRecord = web;
    if ("search" in webRecord || "x_search" in webRecord) return true;
    if ("fetch" in webRecord && hasActiveRuntimeWebFetchProviderSurface(webRecord.fetch, defaults))
      return true;
  }
  const entries = config.plugins?.entries;
...
//#endregion
export {
  prepareSecretsRuntimeSnapshot as a,
  getActiveSecretsRuntimeSnapshot as i,
  clearSecretsRuntimeSnapshot as n,
  getActiveRuntimeWebToolsMetadata as r,
  activateSecretsRuntimeSnapshot as t,
};
