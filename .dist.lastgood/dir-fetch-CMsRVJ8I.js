...
import { j as resolveAbsolutePathForRead } from "./fs-safe-CgBWiL92.js";
...
async function handleDirFetch(params) {
  ...
  if (!(await preflightDu(canonical, maxBytes)))
    return {
      ok: false,
      code: "TREE_TOO_LARGE",
  ...
  const tarArgs = ["-czf", "-", "-C", canonical, "."];
  const TAR_HARD_TIMEOUT_MS = 6e4;
  const tarBuffer = await new Promise((resolve) => {
    const child = spawn(process.platform !== "win32" ? "/usr/bin/tar" : "tar", tarArgs, {
