...
import { t as resolveConversationIdFromTargets } from "./conversation-id-BkporI-9.js";
import "./config-CMOdcWpc.js";
import "./message-channel-core-VUC-himU.js";
import { n as resolveConversationDeliveryTarget } from "./delivery-context-D3lOjhR9.js";
import {
...
import {
  a as hasVisibleAgentPayload,
  r as hasMessagingToolDeliveryEvidence,
  t as getGatewayAgentResult,
...
import {
  ...
  r as resolveQueueSettings,
  t as isSteeringQueueMode,
...
import { n as normalizeConversationRef } from "./session-binding-normalization-C5mpgnkR.js";
...
import "./message-Drg-ekN8.js";
import {
  c as normalizeOptionalString,
  s as normalizeOptionalLowercaseString,
} from "./string-coerce-BdEutqX5.js";
import { t as enqueueAnnounce } from "./subagent-announce-queue-CTKfk7y8.js";
import { a as getSubagentDepthFromSessionStore } from "./subagent-capabilities-C7k0LjTH.js";
import { n as resolveRouteTargetForLoadedChannel } from "./target-parsing-loaded-ClasI1Xo.js";
...
function normalizeAnnounceRouteTarget(context) {
  ...
  let route = stripThreadRouteSuffix(
    (channel
...
function resolveBoundConversationOrigin(params) {
  ...
  const inferredThreadId =
    ...
    (parentConversationId && parentConversationId !== conversationId ? conversationId : void 0) ??
...
function resolveDirectAnnounceTransientRetryDelaysMs() {
  return process.env.OPENCLAW_TEST_FAST === "1" ? [8, 16, 32] : [5e3, 1e4, 2e4];
}
...
async function runAnnounceDeliveryWithRetry(params) {
  ...
  for (;;) {
    ...
    try {
      ...
      defaultRuntime.log(
        `[warn] Subagent announce ${params.operation} transient failure, retrying ${nextAttempt}/${maxAttempts} in ${Math.round(delayMs / 1e3)}s: ${summarizeDeliveryError(err)}`,
      );
      retryIndex += 1;
...
async function resolveSubagentCompletionOrigin(params) {
  ...
  try {
    const hookOrigin = normalizeDeliveryContext(
      (
...
async function sendAnnounce(item) {
  const announceTimeoutMs = resolveSubagentAnnounceTimeoutMs(
...
async function maybeQueueSubagentAnnounce(params) {
  ...
  if (isSteeringQueueMode(queueSettings.mode)) {
    if (
      subagentAnnounceDeliveryDeps.queueEmbeddedPiMessage(sessionId, params.steerMessage, {
...
function inferCompletionChatType(params) {
  const explicit = normalizeChatType(
    params.requesterEntry?.chatType ?? params.requesterEntry?.origin?.chatType ?? void 0,
  );
      ...
      if (
        requesterActivity.sessionId
          ? subagentAnnounceDeliveryDeps.queueEmbeddedPiMessage(
              ...
              {
                steeringMode: "all",
                ...(requesterQueueSettings.debounceMs !== void 0
...
//#endregion
export {
  resolveSubagentAnnounceTimeoutMs as a,
  resolveAnnounceOrigin as c,
  isNonTerminalAgentRunStatus as d,
  loadSessionEntryByKey as i,
  buildAnnounceIdFromChildRun as l,
  isInternalAnnounceRequesterSession as n,
  resolveSubagentCompletionOrigin as o,
  loadRequesterSessionEntry as r,
  runAnnounceDeliveryWithRetry as s,
  deliverSubagentAnnouncement as t,
  buildAnnounceIdempotencyKey as u,
};
