import { t as refreshAwsSharedConfigCacheForBedrock } from "./aws-credential-refresh-BWJxepay.js";
import {
  n as resolveBedrockConfigApiKey,
  t as mergeImplicitBedrockProvider,
} from "./discovery-shared-DsHAFoz3.js";
import { t as bedrockMemoryEmbeddingProviderAdapter } from "./memory-embedding-adapter-DibH99LA.js";
...
const BEDROCK_SERVICE_TIER_VALUES = ["flex", "priority", "default", "reserved"];
...
async function resolveAppProfileTraits(modelId, fallbackRegion) {
  ...
  try {
    const models =
      (
        await (
          await createBedrockControlPlane(extractRegionFromArn(modelId) ?? fallbackRegion)
        ).getInferenceProfile({ inferenceProfileIdentifier: modelId })
      ).models ?? [];
...
function registerAmazonBedrockPlugin(api) {
  ...
  api.registerProvider({
    ...
    catalog: {
      order: "simple",
      run: async (ctx) => {
        const { resolveImplicitBedrockProvider } =
          await import("./extensions/amazon-bedrock/discovery.js");
                ...
                ? {
                    onPayload: (payload, payloadModel) => {
                      if (payload && typeof payload === "object")
                        patchOpus47MaxThinkingEffort(payload);
                      return originalOnPayload?.(payload, payloadModel);
                    },
                  }
                : {}),
...
//#endregion
export {
  resetBedrockAppProfileCacheEligibilityForTest as n,
  setBedrockAppProfileControlPlaneForTest as r,
  registerAmazonBedrockPlugin as t,
};
