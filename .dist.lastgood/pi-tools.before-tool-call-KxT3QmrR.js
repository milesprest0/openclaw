...
import { a as createLazyRuntimeSurface } from "./lazy-runtime-BKxgiyqW.js";
import { h as getPluginSessionExtensionStateSync } from "./loader-BLowS1kl.js";
import {
  c as resolveMessageActionDiscoveryChannelId,
  l as resolveMessageActionDiscoveryForPlugin,
  r as createMessageActionDiscoveryContext,
  s as resolveCurrentChannelMessageToolDiscoveryAdapter,
} from "./message-action-discovery-jVpVmmf8.js";
import {
  n as channelPluginHasNativeApprovalPromptUi,
  t as NATIVE_APPROVAL_PROMPT_RUNTIME_CAPABILITY,
} from "./native-approval-prompt-OaI3z8Mv.js";
import { t as adjustedParamsByToolCallId } from "./pi-tools.before-tool-call.state-8NYFrS40.js";
import "./plugins-Dn-pJrhh.js";
import { i as listChannelPlugins, t as getChannelPlugin } from "./registry-D3zb3mnd.js";
...
import { n as copyPluginToolMeta } from "./tools-DcY_52BH.js";
import { i as PluginApprovalResolutions } from "./types-B6YlI8nn.js";
...
function resolveChannelPromptCapabilities(params) {
  ...
  const capabilities = normalizePromptCapabilities(
    plugin?.agentPrompt?.messageToolCapabilities?.({
      cfg,
      accountId: params.accountId,
    }),
  );
...
async function requestPluginToolApproval(params) {
  ...
  try {
    const requestResult = await callGatewayTool(
      ...
      { timeoutMs: (approval.timeoutMs ?? 12e4) + 1e4 },
      {
    ...
    if ((approval.timeoutBehavior ?? "deny") === "allow")
      return {
        blocked: false,
        params: mergeParamsWithApprovalOverrides(params.baseParams, params.overrideParams),
      };
...
async function recordLoopOutcome(args) {
  ...
  try {
    ...
    const record = recordToolCallOutcome(
...
async function runBeforeToolCallHook(args) {
    ...
    if (loopResult.stuck) {
      ...
      if (
        shouldEmitLoopWarning(
          sessionState,
          args.ctx.runId ? `${args.ctx.runId}:${baseWarningKey}` : baseWarningKey,
          loopResult.count,
        )
      ) {
  ...
  try {
    ...
    const trustedPolicyResult = await runTrustedToolPolicies(
      ...
      toolContext,
      args.ctx?.config ? { config: args.ctx.config } : void 0,
    );
...
//#endregion
export {
  resolveChannelReactionGuidance as _,
  isAbortSignalCancellation as a,
  runBeforeToolCallHook as c,
  getChannelAgentToolMeta as d,
  listAllChannelSupportedActions as f,
  resolveChannelPromptCapabilities as g,
  resolveChannelMessageToolHints as h,
  consumeAdjustedParamsForToolCall as i,
  wrapToolWithBeforeToolCallHook as l,
  listChannelSupportedActions as m,
  __testing as n,
  isBeforeToolCallBlockedError as o,
  listChannelAgentTools as p,
  buildBlockedToolResult as r,
  isToolWrappedWithBeforeToolCallHook as s,
  BeforeToolCallBlockedError as t,
  copyChannelAgentToolMeta as u,
};
