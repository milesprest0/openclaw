...
import { i as callGateway, u as randomIdempotencyKey } from "./call-hIeQLRKk.js";
...
import { t as buildGatewayConnectionDetailsWithResolvers } from "./connection-details-CZR2KRBD.js";
import { r as DEFAULT_PROVIDER } from "./defaults-4m7JJmD2.js";
import { u as buildMediaUnderstandingRegistry } from "./defaults.constants-u25YkDfl.js";
import { t as collectOption } from "./helpers-f4HP4BWN.js";
import { a as getImageMetadata, i as convertHeicToJpeg } from "./image-ops-B3yygUDw.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import { t as formatDocsLink } from "./links-CNfoPWBd.js";
import {
  a as registerBuiltInMemoryEmbeddingProviders,
  t as createEmbeddingProvider,
} from "./memory-core-bundled-runtime-D5Q_wGgy.js";
import {
  i as listMemoryEmbeddingProviders,
...
import { n as detectMime, r as extensionForMime, u as normalizeMimeType } from "./mime-CSQ-Gv-M.js";
...
import { n as canonicalizeCaseOnlyCatalogModelRef } from "./model-selection-CyVXHdEG.js";
...
import {
  i as runWebSearch,
  r as listWebSearchProviders,
  t as isWebSearchProviderConfigured,
} from "./runtime-CNEVbPNH.js";
import {
...
const IMAGE_BACKGROUNDS = ["transparent", "opaque", "auto"];
...
function getAuthProfileIdsForProvider(cfg, providerId) {
  return listProfilesForProvider(
    loadAuthProfileStoreForRuntime(resolveAgentDir(cfg, resolveDefaultAgentId(cfg))),
    providerId,
  );
}
...
function requireModelRunPrompt(value) {
  if (typeof value !== "string" || normalizeOptionalString(value) === void 0)
...
async function runModelAuthStatus() {
  const captured = [];
  const { modelsStatusCommand } = await import("./list.status-command-CEMXEJsJ.js");
    ...
    result.videos.map(async (video, index) => {
      ...
      if (!videoBuffer && video.url) {
        ...
        if (params.output && response.body) {
          const ext =
            extensionForMime(normalizeMimeType(video.mimeType)) ||
...
function registerCapabilityCli(program) {
  ...
  modelAuth
    ...
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        const { modelsAuthLoginCommand } = await import("./auth-5-Qacmy4.js");
  ...
  audio
    ...
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        const cfg = getRuntimeConfig();
        const providers = [...buildMediaUnderstandingRegistry(void 0, cfg).values()]
  ...
  tts
    ...
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        const result = await runTtsProviders(
          resolveTransport({
            local: Boolean(opts.local),
            gateway: Boolean(opts.gateway),
            supported: ["local", "gateway"],
  ...
  tts
    ...
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        const result = await runTtsPersonas(
          resolveTransport({
            local: Boolean(opts.local),
            gateway: Boolean(opts.gateway),
            supported: ["local", "gateway"],
    ...
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        ...
        const result = {
          ...
          description: [...buildMediaUnderstandingRegistry(void 0, cfg).values()]
