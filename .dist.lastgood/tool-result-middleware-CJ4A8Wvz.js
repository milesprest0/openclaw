import { n as createLazyPromiseLoader } from "./lazy-promise-B1nDYoKn.js";
import { t as log$1 } from "./logger-Ce0PtXjG.js";
import {
  d as isGoogleModelApi,
  u as isGemma4ModelRequiringReasoningStrip,
...
import { D as shouldPreserveThinkingBlocks } from "./provider-model-shared-CmD-CscC.js";
import {
  $ as resolveProviderRuntimePlugin,
  p as inspectProviderToolSchemasWithPlugin,
  v as normalizeProviderToolSchemasWithPlugin,
...
function supportsModelTools(model) {
  return (
    (model.compat && typeof model.compat === "object" ? model.compat : void 0)?.supportsTools !==
    false
  );
}
//#endregion
...
function resolveTranscriptPolicy(params) {
  ...
  const cacheKey = cacheConfig
    ? resolveTranscriptPolicyCacheKey({
        ...params,
        provider,
        config: cacheConfig,
      })
  ...
  const policy = buildReplayPolicy
    ? mergeTranscriptPolicy(buildReplayPolicy(context) ?? void 0)
...
function createAgentToolResultMiddlewareRunner(ctx, handlers) {
  ...
  const resolvedHandlersLoader = createLazyPromiseLoader(async () => {
    const { loadAgentToolResultMiddlewaresForRuntime } =
      await import("./agent-tool-result-middleware-loader-B_AcKK-M.js");
      ...
      for (const handler of await resolveHandlers())
        try {
          ...
          if (isValidMiddlewareToolResult(candidate)) current = candidate;
            ...
            log.warn(
              `[${ctx.runtime}] discarded invalid tool result middleware output for ${truncateUtf16Safe(event.toolName, 120)}`,
            );
            return buildMiddlewareFailureResult();
          }
        } catch {
          log.warn(
            `[${ctx.runtime}] tool result middleware failed for ${truncateUtf16Safe(event.toolName, 120)}`,
          );
          return buildMiddlewareFailureResult();
        }
      return current;
...
//#endregion
export {
  normalizeProviderToolSchemas as a,
  logProviderToolSchemaDiagnostics as i,
  resolveTranscriptPolicy as n,
  collectTextContentBlocks as o,
  shouldAllowProviderOwnedThinkingReplay as r,
  supportsModelTools as s,
  createAgentToolResultMiddlewareRunner as t,
};
