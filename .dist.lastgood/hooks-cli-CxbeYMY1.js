...
import { a as resolveHookEntries } from "./config-BBGMGbLn.js";
import { a as formatErrorMessage } from "./errors-DZMrVkYL.js";
import { t as buildWorkspaceHookStatus } from "./hooks-status-TTqab9um.js";
...
import {
  i as invokeNativeHookRelayBridge,
  o as renderNativeHookRelayUnavailableResponse,
...
import { n as runPluginInstallCommand } from "./plugins-install-command-CL4P9c99.js";
import { t as runPluginUpdateCommand } from "./plugins-update-command-D6dIqxZv.js";
import { n as defaultRuntime } from "./runtime-kqN0Yohi.js";
import { a as buildPluginDiagnosticsReport } from "./status-BpQ1MVWd.js";
...
import { t as loadWorkspaceHookEntries } from "./workspace-DWQlUHuF.js";
import "node:stream";
...
function buildHooksReport(config) {
  ...
  return buildWorkspaceHookStatus(workspaceDir, {
    config,
    entries: mergeHookEntries(
      buildPluginDiagnosticsReport({
        config,
        workspaceDir,
      }).hooks.map((hook) => hook.entry),
      workspaceEntries,
    ),
  });
}
  ...
  if (
    ...
    if (hook.requirements.bins.length > 0) {
      const binsStatus = hook.requirements.bins.map((bin) => {
        return hook.missing.bins.includes(bin)
          ? theme.error(`✗ ${bin}`)
          : theme.success(`✓ ${bin}`);
      });
...
function registerHooksCli(program) {
  ...
  hooks
    ...
    .action(async (name, opts) =>
      runHooksCliAction(async () => {
        writeHooksOutput(
          formatHookInfo(buildHooksReport(getRuntimeConfig()), name, opts),
          opts.json,
        );
      }),
    );
