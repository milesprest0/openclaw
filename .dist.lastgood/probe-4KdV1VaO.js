...
import { i as getFileExtension, n as detectMime, r as extensionForMime } from "./mime-CSQ-Gv-M.js";
...
import { t as getMSTeamsRuntime } from "./runtime-api-B6Kge591.js";
import "./reply-chunking-D9l7MwoT.js";
import "./web-media-Bwh5GBGF.js";
import { u as isPrivateIpAddress } from "./ssrf-DO8eIXaD.js";
import "./ssrf-policy-CLYqev3x.js";
...
import { _ as sleep, c as isRecord } from "./utils-BGRcpLKt.js";
...
function findPreferredDmConversationByUserId(entries, id) {
  ...
  for (const entry of entries) {
    ...
    } else unknownTypeMatches.push(entry);
  }
...
function pruneToLimit$2(conversations) {
  ...
  entries.sort((a, b) => {
    return (
      (parseStoredConversationTimestamp(a[1].lastSeenAt) ?? 0) -
      (parseStoredConversationTimestamp(b[1].lastSeenAt) ?? 0)
    );
  });
...
function pruneToLimit$1(polls) {
  ...
  entries.sort((a, b) => {
    return (
      (parseTimestamp(a[1].updatedAt ?? a[1].createdAt) ?? 0) -
      (parseTimestamp(b[1].updatedAt ?? b[1].createdAt) ?? 0)
    );
  });
...
async function validateConsentUploadUrl(url, opts) {
  ...
  if (
    !(opts?.allowlist ?? CONSENT_UPLOAD_HOST_ALLOWLIST).some(
...
function pruneExpired(uploads, nowMs, ttlMs) {
  ...
  for (const [id, record] of Object.entries(uploads))
    if (nowMs - record.createdAt <= ttlMs) kept[id] = record;
  return kept;
}
...
async function uploadToOneDrive(params) {
  ...
  const res = await fetchFn(
    `${GRAPH_ROOT}/me/drive/root:${`/OpenClawShared/${encodeURIComponent(params.filename)}`}:/content`,
    {
      method: "PUT",
      headers: {
        "User-Agent": buildUserAgent(),
...
async function resolveGraphChatId(params) {
  ...
  let path;
  if (userAadObjectId)
    path = `/me/chats?$filter=${encodeURIComponent(`chatType eq 'oneOnOne' and members/any(m:m/microsoft.graph.aadUserConversationMember/userId eq '${userAadObjectId}')`)}&$select=id`;
    ...
    if (mediaMode === "inline") {
      ...
      if (firstMedia) {
        ...
        for (let i = 1; i < reply.mediaUrls.length; i++)
          if (reply.mediaUrls[i]) out.push({ mediaUrl: reply.mediaUrls[i] });
      } else
        pushTextMessages(out, reply.text, {
          chunkText,
          chunkLimit,
...
async function sendMSTeamsMessages(params) {
      ...
      extractMessageId(
        await sendWithRetry(
          async () => {
            ...
            pendingUploadId =
              typeof activity._pendingUploadId === "string" ? activity._pendingUploadId : void 0;
  ...
  const sendProactively = async (batch, startIndex, threadActivityId) => {
    ...
    const conversationId =
      params.conversationRef.conversation?.conversationType === "channel" && threadActivityId
...
async function resolveMSTeamsSendContext(params) {
  ...
  if (recipient.type === "user") {
    ...
    if (resolvedType && resolvedType !== "personal")
      throw new Error(
        `Conversation reference for user:${recipient.id} resolved to a ${resolvedType} conversation (${conversationId}) instead of a personal DM. The bot must receive a DM from this user before it can send proactively.`,
      );
  }
  ...
  if (graphChatId === void 0 && sharePointSiteId)
    try {
      ...
      if (resolved)
        await store.upsert(conversationId, {
          ...ref,
          graphChatId: resolved,
        });
      else
...
async function probeMSTeams(cfg) {
  ...
  try {
    ...
    const tokenProvider = createMSTeamsTokenProvider(app);
    if (!(await tokenProvider.getAccessToken("https://api.botframework.com")))
      throw new Error("Failed to acquire bot token");
    let graph;
    try {
      const accessToken = readAccessToken(
        await tokenProvider.getAccessToken("https://graph.microsoft.com"),
      );
...
export {
  readJsonFile as C,
  createMSTeamsConversationStoreFs as S,
  writeJsonFile as T,
  buildFileInfoCard as _,
  sendMessageMSTeams as a,
  createMSTeamsPollStoreFs as b,
  renderReplyPayloadsToMessages as c,
  withRevokedProxyFallback as d,
  resolveGraphChatId as f,
  removePendingUploadFs as g,
  getPendingUploadFs as h,
  sendAdaptiveCardMSTeams as i,
  sendMSTeamsMessages as l,
  removePendingUpload as m,
  deleteMessageMSTeams as n,
  sendPollMSTeams as o,
  getPendingUpload as p,
  editMessageMSTeams as r,
  buildConversationReference as s,
  probeMSTeams as t,
  AI_GENERATED_ENTITY as u,
  parseFileConsentInvoke as v,
  withFileLock as w,
  extractMSTeamsPollVote as x,
  uploadToConsentUrl as y,
};
