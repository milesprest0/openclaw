...
const channelDoctorModuleLoader = createLazyImportLoader(
  () => import("./channel-doctor-Cpw9O1hU.js"),
);
...
function hasConfiguredSafeBins(cfg) {
  ...
  if (hasRecord(globalExec) && Array.isArray(globalExec.safeBins) && globalExec.safeBins.length > 0)
    return true;
...
function collectVisibleReplyToolPolicyWarnings(cfg) {
  ...
  if (
    ...
    groupPolicy.path !== "messages.visibleReplies"
  )
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(targets)}; OpenClaw falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  return warnings;
}
async function collectDoctorPreviewWarnings(params) {
  ...
  const channelPluginRuntime =
    hasChannelConfig && hasExplicitChannelPluginBlockerConfig(params.cfg)
      ? await import("./channel-plugin-blockers-C4CKGna8.js")
      : void 0;
  ...
  if (hasChannelConfig) {
    ...
    const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } =
      await import("./open-policy-allowfrom-BFFszmRm.js");
  ...
  if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
    ...
    } = await import("./stale-plugin-config-ktmzRyDs.js");
  ...
  if (hasPluginConfig) {
    const { collectCodexRouteWarnings } = await import("./codex-route-warnings-Au8YQc_l.js");
    warnings.push(
  ...
  const { collectCodexNativeAssetWarnings } = await import("./codex-native-assets-CLGgNmKp.js");
  warnings.push(
    ...(await collectCodexNativeAssetWarnings({
      cfg: params.cfg,
      env,
    })),
  );
  if (hasPluginLoadPaths(params.cfg)) {
    const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } =
      await import("./bundled-plugin-load-paths-BxBFlFMK.js");
  ...
  if (hasChannelConfig) {
    ...
    const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-C039wlLG.js");
  ...
  if (hasToolsBySenderKey(params.cfg)) {
    const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } =
      await import("./legacy-tools-by-sender-DdZHo-MG.js");
  ...
  if (hasConfiguredSafeBins(params.cfg)) {
    ...
    } = await import("./exec-safe-bins-CIFnuzhn.js");
