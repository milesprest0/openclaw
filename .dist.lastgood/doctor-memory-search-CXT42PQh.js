...
import {
  c as noteWorkspaceMemoryHealth,
  o as maybeRepairWorkspaceMemoryHealth,
} from "./doctor-workspace-DDgkJweW.js";
import { t as checkQmdBinaryAvailability } from "./engine-qmd-xo7d3H-3.js";
...
import {
  n as getActiveMemorySearchManager,
  r as resolveActiveMemoryBackendConfig,
} from "./memory-runtime-BfIZHpc8.js";
...
import { t as hasConfiguredMemorySecretInput } from "./secret-input-DQ0B1VAn.js";
import { t as hasAnyAuthProfileStoreSource } from "./source-check-DUt28S7k.js";
import "./legacy-config-record-shared-C2deO9Ew.js";
import { c as normalizeOptionalString } from "./string-coerce-BdEutqX5.js";
import { c as isRecord, p as resolveUserPath } from "./utils-BGRcpLKt.js";
//#endregion
...
function buildMemoryRecallIssueNote(audit) {
  ...
  const guidance = audit.issues.some((issue) => issue.fixable)
...
async function noteMemoryRecallHealth(cfg) {
  try {
    ...
    const message = buildMemoryRecallIssueNote(
      await auditShortTermPromotionArtifacts({
        workspaceDir,
        qmd:
          context?.backend === "qmd"
            ? {
    ...
    const dreamingMessage = buildDreamingArtifactIssueNote(
      await auditDreamingArtifacts({ workspaceDir }),
    );
...
async function maybeRepairMemoryRecallHealth(params) {
  ...
  try {
    ...
    if (!(await auditDreamingArtifacts({ workspaceDir })).issues.some((issue) => issue.fixable))
      return;
    if (
      !(await params.prompter.confirmRuntimeRepair({
...
async function hasApiKeyForProvider(provider, cfg, agentDir) {
  const authProviderId =
    resolveMemoryEmbeddingProviderDoctorMetadata(provider)?.authProviderId ?? provider;
  if (
...
function resolvePrimaryMemoryProviderEnvVar(provider) {
  return (
    resolveMemoryEmbeddingProviderDoctorMetadata(provider)?.envVars[0] ??
