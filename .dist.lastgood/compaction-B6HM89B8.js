...
import { t as loadEmbeddedPiMcpConfig } from "./embedded-pi-mcp-S06qG8M1.js";
...
import { o as readRootJsonObjectSync } from "./json-files-DifBk3kt.js";
import { t as log$2 } from "./logger-Ce0PtXjG.js";
import "./defaults-4m7JJmD2.js";
import {
  i as resolveEffectivePluginActivationState,
  r as normalizePluginsConfigWithResolver,
} from "./manifest-registry-DJ43wWua.js";
import { t as applyMergePatch } from "./merge-patch-Gj7xrHOG.js";
import { r as applyPiCompactionSettingsFromConfig } from "./pi-settings-DhiihpH7.js";
import {
  r as loadPluginMetadataSnapshot,
  t as isPluginMetadataSnapshotCompatible,
...
import {
  ...
  n as rewriteTranscriptEntriesInSessionManager,
  r as rewriteTranscriptEntriesInState,
  s as persistTranscriptStateMutation,
...
function loadEnabledBundlePiSettingsSnapshot(params) {
  ...
  for (const diagnostic of embeddedPiMcp.diagnostics)
    log$1.warn(`bundle MCP skipped for ${diagnostic.pluginId}: ${diagnostic.message}`);
  if (Object.keys(embeddedPiMcp.mcpServers).length > 0)
    snapshot = applyMergePatch(snapshot, { mcpServers: embeddedPiMcp.mcpServers });
  return snapshot;
}
...
function buildEmbeddedPiSettingsSnapshot(params) {
  ...
  return applyMergePatch(
    applyMergePatch(params.globalSettings, sanitizePiSettingsSnapshot(params.pluginSettings ?? {})),
...
function truncateToolResultText(text, maxChars, options = {}) {
  ...
  if (hasImportantTail(text) && budget > minKeepChars * 2) {
    const tailBudget = Math.min(Math.floor(budget * 0.3), 4e3);
...
function resolveLiveToolResultMaxChars(params) {
  const configuredCap =
    resolveAgentContextLimits(params.cfg, params.agentId)?.toolResultMaxChars ?? 16e3;
...
function truncateOversizedToolResultsInExistingSessionManager(params) {
  ...
  log$2.info(
    `[tool-result-truncation] Truncated ${rewriteResult.rewrittenEntries} tool result(s) in session (contextWindow=${contextWindowTokens} maxChars=${maxChars} aggregateBudgetChars=${aggregateBudgetChars} oversized=${plan.oversizedReplacementCount} aggregate=${plan.aggregateReplacementCount}) sessionKey=${params.sessionKey ?? params.sessionId ?? "unknown"}`,
  );
  return {
...
function estimateMessagesTokens(messages) {
  return stripToolResultDetails(stripRuntimeContextCustomMessages(messages)).reduce(
...
function isOversizedForSummary(msg, contextWindow) {
  return estimateCompactionMessageTokens(msg) * SAFETY_MARGIN > contextWindow * 0.5;
}
...
function pruneHistoryForContextShare(params) {
  const defaultShare = params.mode === "handoff" ? 0.2 : 0.5;
...
//#endregion
export {
  createPreparedEmbeddedPiSettingsManager as _,
  pruneHistoryForContextShare as a,
  DEFAULT_MAX_LIVE_TOOL_RESULT_CHARS as c,
  sessionLikelyHasOversizedToolResults as d,
  truncateOversizedToolResultsInSession as f,
  formatContextLimitTruncationNotice as g,
  truncateToolResultText as h,
  estimateMessagesTokens as i,
  estimateToolResultReductionPotential as l,
  truncateToolResultMessage as m,
  SUMMARIZATION_OVERHEAD_TOKENS as n,
  resolveContextWindowTokens as o,
  truncateOversizedToolResultsInSessionManager as p,
  computeAdaptiveChunkRatio as r,
  summarizeInStages as s,
  SAFETY_MARGIN as t,
  resolveLiveToolResultMaxChars as u,
};
