...
import { t as ID3v2Parser } from "./ID3v2Parser-ULHz7fC_.js";
import { r as fromBuffer } from "./lib-Dy7Qkhjs.js";
  ...
  async readData(header) {
    ...
    switch (header.chunkID.trim()) {
      case "FVER":
        ...
        break;
      case "PROP":
        if ((await this.tokenizer.readToken(FourCcToken)) !== "SND ")
        ...
        break;
      case "ID3": {
        const rst = fromBuffer(
          await this.tokenizer.readToken(new Uint8ArrayType(Number(header.chunkSize))),
        );
  ...
  async handleSoundPropertyChunks(remainingSize) {
    ...
    while (remainingSize > 0) {
      ...
      switch (sndPropHeader.chunkID.trim()) {
        ...
        case "ABSS":
          debug(
            `ABSS ${await this.tokenizer.readToken(UINT16_BE)}:${await this.tokenizer.readToken(UINT8)}:${await this.tokenizer.readToken(UINT8)}.${await this.tokenizer.readToken(UINT32_BE)}`,
          );
          break;
        case "LSCO":
