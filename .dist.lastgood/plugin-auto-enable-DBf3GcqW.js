...
import {
  i as listBundledChannelIdsWithConfiguredState,
  r as hasBundledChannelConfiguredState,
  t as isChannelConfigured,
} from "./channel-configured-6eK1b4ge.js";
import { t as getChatChannelMeta } from "./chat-meta-CUH0ZInO.js";
import { a as listPotentialConfiguredChannelPresenceSignals } from "./config-presence-DQuxzSYt.js";
...
import { _ as resolveInstalledPluginIndexPolicyHash } from "./installed-plugin-index-store-YgBbP4mp.js";
...
import "./registry-V0yq5bwZ.js";
import { t as ensurePluginAllowlisted } from "./plugins-allowlist-Bga4C6mB.js";
...
import { l as resolveOwningPluginIdsForModelRef } from "./providers-CWWim-ts.js";
import { n as resolvePluginSetupAutoEnableReasons } from "./setup-registry-CnJdT1HM.js";
import {
...
function resolveExternalCatalogPreferOver(channelId, env) {
  for (const rawPath of resolveExternalCatalogPaths(env)) {
    ...
    try {
      const channel = parseExternalCatalogChannelEntries(
...
function resolveAutoEnableProviderPluginIds(registry) {
  ...
  for (const plugin of registry.plugins)
    for (const providerId of plugin.autoEnableWhenConfiguredProviders ?? [])
      if (!entries.has(providerId)) entries.set(providerId, plugin.id);
...
function hasXaiSetupAutoEnableRelevantConfig(cfg) {
  ...
  return (
    ...
    (isRecord(cfg.tools?.web) && isRecord(cfg.tools.web.x_search))
  );
}
...
function resolveChannelAutoEnableDisplayLabel(entry, manifestRegistry) {
  ...
  return (
    (builtInChannelId ? getChatChannelMeta(builtInChannelId)?.label : void 0) ??
...
function materializePluginAutoEnableCandidatesInternal(params) {
  ...
  for (const [pluginId, reasons] of autoEnabledReasons)
    if (!isBlockedObjectKey(pluginId)) autoEnabledReasonRecord[pluginId] = [...reasons];
  return {
    config: next,
    changes,
...
//#endregion
export {
  resolvePluginAutoEnableCandidateReason as a,
  configMayNeedPluginAutoEnable as i,
  materializePluginAutoEnableCandidates as n,
  detectPluginAutoEnableCandidates as r,
  applyPluginAutoEnable as t,
};
