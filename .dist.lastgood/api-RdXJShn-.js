...
import { n as buildHostnameAllowlistPolicyFromSuffixAllowlist } from "./ssrf-policy-CLYqev3x.js";
...
function resolveGoogleAuthDispatcherPolicy(input, init) {
  ...
  const proxyBypassed = shouldBypassGoogleAuthProxy(
    requestUrl,
    Array.isArray(googleAuthInit.noProxy) ? googleAuthInit.noProxy : [],
  );
  const agent = resolveGoogleAuthAgent(googleAuthInit, requestUrl);
  const explicitProxy =
    readGoogleAuthProxyUrl(googleAuthInit.proxy) ??
    (hasProxyAgentShape(agent) ? agent.proxy.toString() : void 0);
    ...
    googleAuthRuntimePromise = (async () => {
      try {
        const [googleAuthModule, gaxiosModule] = await Promise.all([
          import("./src-BE5V75IH.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)),
          import("./src-BqmNGzab.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)),
        ]);
        return {
...
async function getGoogleAuthTransport() {
  ...
  return installGoogleAuthHeaderCompatibilityInterceptor(
    new Gaxios({ fetchImplementation: createGoogleAuthFetch() }),
  );
}
async function resolveValidatedGoogleChatCredentials(account) {
  ...
  if (account.credentialsFile)
    return validateGoogleChatServiceAccountCredentials(
      await readCredentialsFile(account.credentialsFile),
    );
  return null;
}
//#endregion
...
async function updateGoogleChatMessage(params) {
  ...
  return {
    messageName: (
      await fetchJson(account, `${CHAT_API_BASE}/${messageName}?updateMask=text`, {
        method: "PATCH",
...
async function downloadGoogleChatMedia(params) {
  ...
  return await fetchBuffer(account, `${CHAT_API_BASE}/media/${resourceName}?alt=media`, void 0, {
    maxBytes,
  });
}
...
//#endregion
export {
  findGoogleChatDirectMessage as a,
  sendGoogleChatMessage as c,
  verifyGoogleChatRequest as d,
  downloadGoogleChatMedia as i,
  updateGoogleChatMessage as l,
  deleteGoogleChatMessage as n,
  listGoogleChatReactions as o,
  deleteGoogleChatReaction as r,
  probeGoogleChat as s,
  createGoogleChatReaction as t,
  uploadGoogleChatAttachment as u,
};
