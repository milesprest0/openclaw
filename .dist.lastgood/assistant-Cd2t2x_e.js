...
async function planCrestodianCommandWithLocalRuntime(params) {
  ...
  for (const backend of backends)
    try {
      const parsed = parseCrestodianAssistantPlanText(
        await runLocalRuntimePlanner(backend, {
          prompt,
          deps: params.deps,
        }),
      );
...
async function runLocalRuntimePlanner(backend, params) {
  ...
  try {
    ...
    switch (backend.runner) {
      case "cli":
        return extractPlannerResultText(
          await (params.deps?.runCliAgent ?? (await loadRunCliAgent()))({
            sessionId,
            sessionKey,
            agentId: "crestodian",
            trigger: "manual",
      ...
      case "embedded":
        return extractPlannerResultText(
          await (params.deps?.runEmbeddedPiAgent ?? (await loadRunEmbeddedPiAgent()))({
            sessionId,
            sessionKey,
            agentId: "crestodian",
            trigger: "manual",
...
async function loadRunCliAgent() {
  return (await import("./cli-runner-B_7Q_t36.js")).runCliAgent;
}
async function loadRunEmbeddedPiAgent() {
  return (await import("./pi-embedded-6CIIAYDg.js")).runEmbeddedPiAgent;
}
