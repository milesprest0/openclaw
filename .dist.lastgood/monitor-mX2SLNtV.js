...
import { i as resolveLineAccount, r as resolveDefaultLineAccountId } from "./accounts-Cy8a_3aQ.js";
import {
  n as isSenderIdAllowed,
  r as mergeDmAllowFromSources,
  t as firstDefined,
} from "./allow-from-REahseRU.js";
import {
...
import { r as chunkMarkdownText } from "./chunk-aBEwc7QQ.js";
...
import { n as evaluateMatchedGroupAccessForPolicy } from "./group-access-CdIgeyPS.js";
import {
...
import {
  D as showLoadingAnimation,
  S as pushMessagesLine,
  T as replyMessageLine,
  c as processLineMessage,
  d as createImageMessage,
  f as createLocationMessage,
  g as getUserDisplayName,
  k as buildTemplateMessageFromPayload,
  m as createTextMessageWithQuickReplies,
  p as createQuickReplyItems,
  u as createFlexMessage,
  w as pushTextMessageWithQuickReplies,
  x as pushMessageLine,
} from "./markdown-to-line-BbiEnkin.js";
...
import {
  i as mergeHeaders,
  n as HTTPFetchClient,
  r as convertResponseToReadable,
} from "./messagingApiClient-lBgXDOrE.js";
import "./command-auth-C7cr8cwH.js";
import { t as resolvePairingIdLabel } from "./pairing-labels-4nTcW2E_.js";
import {
...
import {
  a as resolveLineGroupConfigEntry,
  n as getLineRuntime,
  t as buildLineQuickReplyFallbackText,
} from "./quick-reply-fallback-DtLI7aqg.js";
...
const normalizeDmAllowFromWithStore = (params) =>
  normalizeAllowFrom(mergeDmAllowFromSources(params));
...
var MessagingApiBlobClient = class {
  ...
  async getMessageContentWithHttpInfo(messageId) {
    const response = await this.httpClient.get(
      "/v2/bot/message/{messageId}/content".replace("{messageId}", String(messageId)),
    );
    return {
  ...
  async getMessageContentPreviewWithHttpInfo(messageId) {
    const response = await this.httpClient.get(
      "/v2/bot/message/{messageId}/content/preview".replace("{messageId}", String(messageId)),
    );
    return {
      httpResponse: response,
      body: convertResponseToReadable(response),
...
async function downloadLineMedia(messageId, channelAccessToken, maxBytes = 10 * 1024 * 1024) {
  const response = await new MessagingApiBlobClient({ channelAccessToken }).getMessageContent(
    messageId,
  );
  const chunks = [];
  let totalSize = 0;
...
async function deliverLineAutoReply(params) {
  ...
  const mediaMessages = resolveSendableOutboundReplyParts(payload)
    .mediaUrls.map((url) => url?.trim())
...
function buildPeerId(source) {
  ...
  const groupKey =
    normalizeOptionalString(source.type === "group" ? source.groupId : void 0) ??
    normalizeOptionalString(source.type === "room" ? source.roomId : void 0);
  ...
  if (shouldLogVerbose()) {
    ...
    const mediaInfo =
      params.verboseLog.kind === "inbound" && (params.verboseLog.mediaCount ?? 0) > 1
        ...
        : "";
    logVerbose(
      `${params.verboseLog.kind === "inbound" ? "line inbound" : "line postback"}: from=${ctxPayload.From} len=${body.length}${mediaInfo} preview="${preview}"`,
    );
  }
  return {
    ctxPayload,
...
const LINE_DOWNLOADABLE_MESSAGE_TYPES = new Set(["image", "video", "audio", "file"]);
...
async function shouldProcessLineEvent(event, context) {
  ...
  const effectiveGroupAllow = normalizeAllowFrom(
    firstDefined(groupAllowOverride, account.config.groupAllowFrom, fallbackGroupAllowFrom),
  );
...
async function sendLineReplyChunks(params) {
  ...
  if (params.replyToken && !replyTokenUsed)
    try {
      ...
      for (let i = 0; i < remaining.length; i += 1)
        if (i === remaining.length - 1 && hasQuickReplies)
  ...
  for (let i = 0; i < params.chunks.length; i += 1)
    if (i === params.chunks.length - 1 && hasQuickReplies)
      await params.pushTextMessageWithQuickReplies(
        params.to,
        params.chunks[i],
        params.quickReplies,
...
async function readLineWebhookRequestBody(
  ...
  timeoutMs = LINE_WEBHOOK_PREAUTH_BODY_TIMEOUT_MS$1,
) {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      const rawBody = await readBody(
        req,
        Math.min(maxBodyBytes, LINE_WEBHOOK_PREAUTH_MAX_BODY_BYTES$1),
        LINE_WEBHOOK_PREAUTH_BODY_TIMEOUT_MS$1,
      );
...
async function monitorLineProvider(opts) {
      ...
      try {
        ...
        if (
          !hasFinalChannelMessageReplyDispatch(
            turnResult.dispatched ? turnResult.dispatchResult : void 0,
          )
        )
  ...
  const createScopedLineWebhookHandler = (target) =>
...
//#endregion
export {
  validateLineSignature as a,
  isSenderAllowed as c,
  parseLineWebhookBody as i,
  normalizeAllowFrom as l,
  createLineNodeWebhookHandler as n,
  downloadLineMedia as o,
  readLineWebhookRequestBody as r,
  MessagingApiBlobClient as s,
  monitorLineProvider as t,
  normalizeDmAllowFromWithStore as u,
};
