...
import { t as formatLocationText } from "./location-C1vvCrHh.js";
import "./routing-BccEMOrJ.js";
import "./error-runtime-CdKGkPCx.js";
import { a as readChannelAllowFromStore } from "./pairing-store-CzWw0BEc.js";
import { t as resolveTelegramPreviewStreamMode } from "./preview-streaming-KdLBVJWR.js";
...
import { i as markdownToIR, n as renderMarkdownWithMarkers } from "./tables-Dey9v5zf.js";
import { i as renderMarkdownIRChunksWithinLimit } from "./text-runtime-lKuAtsoz.js";
...
async function expandTelegramAllowFromWithAccessGroups(params) {
  ...
  return expanded.some((entry) => !originalEntries.has(entry))
...
function buildSenderLabel(msg, senderId) {
  ...
  const fallbackId =
    (senderId != null ? normalizeOptionalString(String(senderId)) : void 0) ??
    (msg.from?.id != null ? String(msg.from.id) : void 0);
...
function hasBotMention(msg, botUsername) {
  ...
  for (const ent of entities) {
    ...
    if (
      normalizeLowercaseStringOrEmpty(text.slice(ent.offset, ent.offset + ent.length)) === mention
    )
      return true;
  }
  return false;
...
function normalizeForwardedUserLabel(user) {
  ...
  return {
    display:
      (name && username
        ? `${name} (@${username})`
        : name || (username ? `@${username}` : void 0)) || `user:${id}`,
    name: name || void 0,
    username,
    id,
  };
...
function describeReplyTarget(msg) {
  ...
  const quotePosition =
    kind === "quote" && typeof quote?.position === "number" && Number.isFinite(quote.position)
      ...
      : void 0;
  const quoteEntities =
    kind === "quote" && Array.isArray(quote?.entities) ? quote.entities : void 0;
...
function wrapSegmentFileRefs(text, codeDepth, preDepth, anchorDepth) {
  ...
  return text
    ...
    .replace(getOrphanedTldPattern(), (match, prefix, tld) =>
...
function markdownToTelegramChunks(markdown, limit, options = {}) {
  return renderTelegramChunksWithinHtmlLimit(
    markdownToIR(markdown ?? "", {
      linkify: true,
      enableSpoilers: true,
      headingStyle: "none",
...
//#endregion
export {
  parseTelegramReplyToMessageId as $,
  resolveTelegramReplyId as A,
  hasBotMention as B,
  isTelegramCommandsAllowFromConfigured as C,
  resolveTelegramForumFlag as D,
  resolveTelegramDirectPeerId as E,
  buildSenderLabel as F,
  expandTelegramAllowFromWithAccessGroups as G,
  normalizeForwardedContext as H,
  buildSenderName as I,
  normalizeAllowFrom as J,
  resolveTelegramDmAllow as K,
  expandTextLinks as L,
  resolveTelegramThreadSpec as M,
  shouldUseTelegramDmThreadSession as N,
  resolveTelegramForumThreadId as O,
  withResolvedTelegramForumFlag as P,
  normalizeTelegramReplyToMessageId as Q,
  extractTelegramLocation as R,
  extractTelegramForumFlag as S,
  resolveTelegramCommandAuthorization as T,
  resolveTelegramMediaPlaceholder as U,
  isBinaryContent as V,
  resolveTelegramPrimaryMedia as W,
  resolveSenderAllowMatch as X,
  normalizeDmAllowFromWithStore as Y,
  resolveTelegramEffectiveDmPolicy as Z,
  buildTelegramParentPeer as _,
  splitTelegramHtmlChunks as a,
  resolveTelegramTargetChatType as at,
  buildTypingThreadParams as b,
  isSafeToRetrySendError as c,
  isTelegramRateLimitError as d,
  parseTelegramThreadId as et,
  isTelegramServerError as f,
  buildTelegramGroupPeerId as g,
  buildTelegramGroupFrom as h,
  renderTelegramHtmlText as i,
  parseTelegramTarget as it,
  resolveTelegramStreamMode as j,
  resolveTelegramGroupAllowFromContext as k,
  isTelegramClientRejection as l,
  buildGroupLabel as m,
  markdownToTelegramHtml as n,
  normalizeTelegramChatId as nt,
  wrapFileReferencesInHtml as o,
  stripTelegramInternalPrefixes as ot,
  tagTelegramNetworkError as p,
  isSenderAllowed as q,
  markdownToTelegramHtmlChunks as r,
  normalizeTelegramLookupTarget as rt,
  isRecoverableTelegramNetworkError as s,
  markdownToTelegramChunks as t,
  isNumericTelegramChatId as tt,
  isTelegramPollingNetworkError as u,
  buildTelegramRoutingTarget as v,
  resetTelegramForumFlagCacheForTest as w,
  describeReplyTarget as x,
  buildTelegramThreadParams as y,
  getTelegramTextParts as z,
};
