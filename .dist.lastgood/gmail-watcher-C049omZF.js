...
function spawnGogServe(cfg) {
  ...
  child.on("exit", (code, signal) => {
    ...
    if (addressInUse) {
      log.warn(
        "gog serve failed to bind (address already in use); stopping restarts. Another watcher is likely running. Set OPENCLAW_SKIP_GMAIL_WATCHER=1 or stop the other process.",
      );
      watcherProcess = null;
      return;
    }
...
async function startGmailWatcher(cfg) {
  ...
  if (!(await startGmailWatch(runtimeConfig)))
