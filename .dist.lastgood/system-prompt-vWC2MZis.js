...
import { n as buildMemoryPromptSection } from "./memory-state-BfIgqH24.js";
import { l as listDeliverableMessageChannels } from "./message-channel-GRZIhFYD.js";
import {
  i as isKnownNativeApprovalPromptChannel,
  r as hasNativeApprovalPromptRuntimeCapability,
} from "./native-approval-prompt-OaI3z8Mv.js";
import {
  n as normalizeStructuredPromptSection,
  t as normalizePromptCapabilityIds,
} from "./prompt-cache-stability-CYa4MYXQ.js";
import { t as sanitizeForPromptLiteral } from "./sanitize-for-prompt-vkEfjDrI.js";
import {
  a as normalizeLowercaseStringOrEmpty,
  s as normalizeOptionalLowercaseString,
} from "./string-coerce-BdEutqX5.js";
import { t as SYSTEM_PROMPT_CACHE_BOUNDARY } from "./system-prompt-cache-boundary-Cq7GvFY6.js";
...
//#endregion
//#region src/agents/prompt-invariants.ts
const PROTECTED_MARKERS = ["HARD", "HARD RULE", "identity-level", "IDENTITY-LEVEL", "OVERRIDES"];
const IDENTITY_TRUTH_HEADING = /^(#+)\s*identity\s+truth\b/iu;
const MARKDOWN_HEADING = /^(#{1,6})\s+/u;
function normalizeLineForPresence(line) {
  return line.replace(/\s+/gu, " ").trim();
}
function hasProtectedMarker(line) {
  const upper = line.toUpperCase();
  return PROTECTED_MARKERS.some((marker) => upper.includes(marker.toUpperCase()));
}
function extractProtectedLines(sourceText) {
  const lines = sourceText.split(/\r?\n/u);
  const protectedLines = [];
  let identityTruthLevel;
  for (const line of lines) {
    const headingMatch = line.match(MARKDOWN_HEADING);
    if (headingMatch) {
      const headingLevel = headingMatch[1]?.length ?? 0;
      if (identityTruthLevel !== void 0 && headingLevel <= identityTruthLevel)
        identityTruthLevel = void 0;
      if (IDENTITY_TRUTH_HEADING.test(line)) identityTruthLevel = headingLevel;
    }
    if (
      (identityTruthLevel !== void 0 && normalizeLineForPresence(line).length > 0) ||
      hasProtectedMarker(line)
    )
      protectedLines.push(line);
  }
  return protectedLines;
}
function assertProtectedLinesPresent(assembledPrompt, protectedLines) {
...
const DEFAULT_PROJECT_CONTEXT_MAX_CHARS = 48e3;
...
function resolveProjectContextOptimization(config) {
  return {
...
function classifyContextRegions(file) {
  ...
  if (basename === "user.md" || basename === "identity.md")
    return {
      inline: regions,
      onDemand: [],
    };
...
function pointerLineForRegion(topic) {
  return `↳ ${topic} — load via memory_search when relevant`;
}
function clampProjectContextChars(lines, maxChars) {
  const rendered = lines.join("\n");
  if (rendered.length <= maxChars) return lines;
  return `${truncateUtf16Safe(rendered, Math.max(0, maxChars - 61))}

[…project context truncated — load via memory_search/read…]`.split("\n");
}
...
function buildAgentSystemPrompt(params) {
  ...
  const providerSectionOverrides = Object.fromEntries(
    Object.entries(promptContribution?.sectionOverrides ?? {})
      .map(([key, value]) => [
        key,
        normalizeProviderPromptBlock(typeof value === "string" ? value : void 0),
      ])
          ...
          params.sandboxInfo?.enabled
            ? [
                ...
                params.sandboxInfo.workspaceAccess
                  ? `Agent workspace access: ${params.sandboxInfo.workspaceAccess}${params.sandboxInfo.agentWorkspaceMount ? ` (mounted at ${sanitizeForPromptLiteral(params.sandboxInfo.agentWorkspaceMount)})` : ""}`
                  : "",
...
//#endregion
export {
  buildLimitedBootstrapPromptLines as i,
  buildAgentSystemPrompt as n,
  buildFullBootstrapPromptLines as r,
  appendAgentBootstrapSystemPromptSupplement as t,
};
