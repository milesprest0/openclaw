...
function getGlobalManagers() {
  ...
  if (!globalWithState[GLOBAL_STATE_KEY])
    globalWithState[GLOBAL_STATE_KEY] = /* @__PURE__ */ new Map();
...
function createSidecarLockManager(key) {
  ...
  async function acquire(options) {
    ...
    while (true) {
      ...
      try {
        ...
        const release = () =>
          releaseHeldLock(state, normalizedTargetPath, createdHeld).then(() => void 0);
        return {
          lockPath,
          normalizedTargetPath,
          release,
        ...
        if (handle) {
          ...
          if (state.held.get(normalizedTargetPath)?.handle === handle)
            state.held.delete(normalizedTargetPath);
          await fs$1.rm(lockPath, { force: true }).catch(() => void 0);
        ...
        if (
          await (options.shouldReclaim ?? defaultShouldReclaim)({
            lockPath,
            normalizedTargetPath,
          ...
          if ((options.staleRecovery ?? "fail-closed") === "fail-closed")
            throw Object.assign(
              /* @__PURE__ */ new Error(`file lock stale for ${normalizedTargetPath}`),
              {
        ...
        if (
          ...
          (maxRetries !== void 0 && attempt >= maxRetries)
        )
          throw Object.assign(
            /* @__PURE__ */ new Error(`file lock timeout for ${normalizedTargetPath}`),
            {
        ...
        const remaining =
          options.timeoutMs === void 0 || options.timeoutMs === Number.POSITIVE_INFINITY
  ...
  async function drain() {
    for (const [normalizedTargetPath, held] of Array.from(state.held.entries()))
      await releaseHeldLock(state, normalizedTargetPath, held, { force: true }).catch(() => void 0);
  }
  function reset() {
