...
import {
  a as resolveCanonicalRootMemoryPath,
  n as LEGACY_ROOT_MEMORY_FILENAME,
  o as resolveLegacyRootMemoryPath,
  s as resolveRootMemoryRepairDir,
  t as CANONICAL_ROOT_MEMORY_FILENAME,
} from "./root-memory-files-tg3oDJNP.js";
import { g as shortenHomePath } from "./utils-BGRcpLKt.js";
import { t as DEFAULT_AGENTS_FILENAME } from "./workspace-DvzHmVcJ.js";
...
async function shouldSuggestMemorySystem(workspaceDir) {
  if ((await listWorkspaceEntries(workspaceDir)).has("MEMORY.md"))
    try {
      if ((await fs.promises.stat(resolveCanonicalRootMemoryPath(workspaceDir))).isFile())
        return false;
    } catch {}
...
async function detectRootMemoryFiles(workspaceDir) {
  ...
  const [canonical, legacy] = await Promise.all([
    entries.has("MEMORY.md") ? statIfExists(canonicalPath) : Promise.resolve({ exists: false }),
    entries.has("memory.md") ? statIfExists(legacyPath) : Promise.resolve({ exists: false }),
  ]);
  return {
...
async function moveLegacyRootMemoryFileToArchive(params) {
  ...
  const archiveDir = path.join(
    repairDir,
    /* @__PURE__ */ new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-"),
  );
...
async function noteWorkspaceMemoryHealth(cfg) {
  try {
    const rootMemoryWarning = formatRootMemoryFilesWarning(
      await detectRootMemoryFiles(resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg))),
    );
...
//#endregion
export {
  formatRootMemoryFilesWarning as a,
  noteWorkspaceMemoryHealth as c,
  formatLegacyWorkspaceWarning as i,
  shouldSuggestMemorySystem as l,
  detectLegacyWorkspaceDirs as n,
  maybeRepairWorkspaceMemoryHealth as o,
  detectRootMemoryFiles as r,
  migrateLegacyRootMemoryFile as s,
  MEMORY_SYSTEM_PROMPT as t,
};
