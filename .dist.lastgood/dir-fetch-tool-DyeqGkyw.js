...
import {
  ...
  l as FILE_TRANSFER_SUBDIR,
  n as DIR_FETCH_HARD_MAX_BYTES,
  r as DIR_FETCH_TOOL_DESCRIPTOR,
  t as DIR_FETCH_DEFAULT_MAX_BYTES,
} from "./descriptors-_YUnOSqX.js";
import { t as callGatewayTool } from "./gateway-VO6GibHc.js";
import { i as mimeFromExtension, t as IMAGE_MIME_INLINE_SET } from "./mime-Dc_4RScI.js";
import { i as resolveNodeIdFromList, t as listNodes } from "./nodes-utils-B1_2Nbxw.js";
import {
  a as readTrimmedString,
  i as readGatewayCallOptions,
  n as readBoolean,
  o as throwFromNodePayload,
  r as readClampedInt,
  t as humanSize,
} from "./params-DoH_fTYA.js";
...
async function listTarTypeChars(tarBuffer) {
  return new Promise((resolve) => {
    const child = spawn(process.platform !== "win32" ? "/usr/bin/tar" : "tar", ["-tzvf", "-"], {
      stdio: ["pipe", "pipe", "pipe"],
    });
    let stdout = "";
    let stderr = "";
...
async function validateTarUncompressedBudget(
  ...
  return new Promise((resolve) => {
    const child = spawn(process.platform !== "win32" ? "/usr/bin/tar" : "tar", ["-xOzf", "-"], {
      stdio: ["pipe", "pipe", "pipe"],
    });
    let totalBytes = 0;
    let stderr = "";
...
function createDirFetchTool() {
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (crypto.createHash("sha256").update(tarBuffer).digest("hex") !== sha256)
