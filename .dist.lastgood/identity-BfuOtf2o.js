...
function resolveAckReaction(cfg, agentId, opts) {
  if (opts?.channel && opts?.accountId) {
    const accountReaction = getChannelConfig(cfg, opts.channel)?.accounts?.[opts.accountId]
      ?.ackReaction;
...
function resolveResponsePrefix(cfg, agentId, opts) {
  if (opts?.channel && opts?.accountId) {
    const accountPrefix = getChannelConfig(cfg, opts.channel)?.accounts?.[opts.accountId]
      ?.responsePrefix;
...
//#endregion
export {
  resolveIdentityNamePrefix as a,
  resolveHumanDelayConfig as i,
  resolveAgentIdentity as n,
  resolveMessagePrefix as o,
  resolveEffectiveMessagesConfig as r,
  resolveResponsePrefix as s,
  resolveAckReaction as t,
};
