...
import {
  a as selectDiagnosticStabilitySnapshot,
  r as normalizeDiagnosticStabilityQuery,
} from "./diagnostic-stability-B9xLFdo7.js";
import { n as buildGatewayDiscoveryTarget } from "./gateway-discovery-targets-rIaHCkS6.js";
...
import { n as parseTimeoutMsWithFallback } from "./parse-timeout-BoXPCRnA.js";
...
import { n as addGatewayRunCommand } from "./run-D_BUttIO.js";
...
function renderBeaconLines(beacon, rich) {
  ...
  const lines = [
    `- ${colorize(rich, theme.accentBright, target.title)} ${colorize(rich, theme.muted, target.domain)}`,
  ];
...
const configModuleLoader = createLazyImportLoader(
  () => import("./read-best-effort-config.runtime.js"),
);
const gatewayStatusModuleLoader = createLazyImportLoader(
  () => import("./gateway-status-DDLK_S0F.js"),
);
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-CDLeEYK5.js"));
const bonjourDiscoveryModuleLoader = createLazyImportLoader(
  () => import("./bonjour-discovery-rBnCgab3.js"),
);
const wideAreaDnsModuleLoader = createLazyImportLoader(() => import("./widearea-dns-CLXTRW6T.js"));
const healthStyleModuleLoader = createLazyImportLoader(() => import("./health-style-DrtZLX_T.js"));
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-DSJ_uHLh.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(
  () => import("./diagnostic-stability-bundle-CZhi4mE0.js"),
);
const supportExportModuleLoader = createLazyImportLoader(
  () => import("./diagnostic-support-export-DSuSL4Il.js"),
);
const daemonStatusGatherModuleLoader = createLazyImportLoader(
  () => import("./status.gather-CXBoNbsC.js"),
);
...
function renderStabilitySummary(snapshot, rich) {
  const lines = [
    ...
    `${colorize(rich, theme.muted, "Events:")} ${snapshot.count}/${snapshot.capacity}${snapshot.dropped > 0 ? ` · dropped=${snapshot.dropped}` : ""}`,
  ];
  ...
  if (memory)
    lines.push(
      `${colorize(rich, theme.muted, "Memory:")} rss=${formatBytes(memory.latest?.rssBytes)} heap=${formatBytes(memory.latest?.heapUsedBytes)} maxRss=${formatBytes(memory.maxRssBytes)} pressure=${memory.pressureCount}`,
    );
  ...
  if (payloadLarge) {
    const surfaces = Object.entries(payloadLarge.bySurface)
      ...
      .join(", ");
    lines.push(
      `${colorize(rich, theme.muted, "Large payloads:")} total=${payloadLarge.count} rejected=${payloadLarge.rejected} truncated=${payloadLarge.truncated} chunked=${payloadLarge.chunked}${surfaces ? ` · ${surfaces}` : ""}`,
    );
  }
    ...
    .action(async (opts) => {
      await runGatewayCommand(async () => {
        ...
        for (const beacon of deduped)
          for (const line of renderBeaconLines(beacon, rich)) defaultRuntime.log(line);
