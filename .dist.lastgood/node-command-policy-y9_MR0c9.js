...
import { n as normalizeDeviceMetadataForPolicy } from "./device-metadata-normalization-RrgoRUye.js";
import {
...
function resolvePlatformIdByPrefix(value) {
  for (const rule of PLATFORM_PREFIX_RULES)
    if (rule.prefixes.some((prefix) => value.startsWith(prefix))) return rule.id;
}
function resolvePlatformIdByDeviceFamily(value) {
  for (const rule of DEVICE_FAMILY_TOKEN_RULES)
    if (rule.tokens.some((token) => value.includes(token))) return rule.id;
}
function normalizePlatformId(platform, deviceFamily) {
  ...
  return (
    resolvePlatformIdByDeviceFamily(normalizeDeviceMetadataForPolicy(deviceFamily)) ?? "unknown"
  );
}
...
//#endregion
export {
  normalizeDeclaredNodeCommands as a,
  listDangerousPluginNodeCommands as i,
  isForegroundRestrictedPluginNodeCommand as n,
  resolveNodeCommandAllowlist as o,
  isNodeCommandAllowed as r,
  DEFAULT_DANGEROUS_NODE_COMMANDS as t,
};
