import { a as normalizeDiscordSlug } from "./allow-list-Jocmm0YN.js";
import { n as fetchDiscord, t as DiscordApiError } from "./api-CBB6yRJS.js";
import {
  i as listGuilds,
  n as filterDiscordGuilds,
  r as resolveDiscordAllowlistToken,
  t as buildDiscordUnresolvedResults,
} from "./resolve-allowlist-common-COCT93dz.js";
...
async function listGuildChannels(token, fetcher, guildId) {
  return (await fetchDiscord(`/guilds/${guildId}/channels`, token, fetcher))
    .map((channel) => {
  ...
  for (const input of params.entries) {
    ...
    if (parsed.channelId) {
      ...
      if (result.status === "not-found" && parsed.guildId) {
        ...
        if (guild) {
          const match = preferActiveMatch(
            (await getChannels(guild.id)).filter(
    ...
    if (match) {
      ...
      results.push({
        ...
        archived: match.archived,
        note:
          candidates.length > 1 && guild?.name ? `matched multiple; chose ${guild.name}` : void 0,
      });
      continue;
    }
    results.push({
