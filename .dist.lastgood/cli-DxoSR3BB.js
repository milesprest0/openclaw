...
import { t as DEFAULT_PROMOTION_MIN_SCORE } from "./short-term-promotion-C3R0Rj1g.js";
...
function registerMemoryCli(program) {
  ...
  memory
    ...
    .option("--agent <id>", "Agent id (default: default agent)")
    .option("--limit <n>", "Max candidates", (value) => Number(value))
    .option(
      "--min-score <n>",
      `Minimum weighted score (default: ${DEFAULT_PROMOTION_MIN_SCORE})`,
      (value) => Number(value),
    )
    .option("--min-recall-count <n>", `Minimum recall count (default: 3)`, (value) => Number(value))
    .option("--min-unique-queries <n>", `Minimum distinct query count (default: 2)`, (value) =>
      Number(value),
    )
