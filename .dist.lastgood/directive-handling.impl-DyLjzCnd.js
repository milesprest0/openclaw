...
import { r as formatRemainingShort } from "./auth-health-ChFtt_mg.js";
...
import { d as renderExecTargetLabel } from "./bash-tools.exec-runtime-Brc0frDH.js";
import { i as resolveModelsCommandReply } from "./commands-models-BgA34Ll2.js";
import { t as resolveModelSelectionFromDirective } from "./directive-handling.model-selection-Dkj0eQuC.js";
import {
  a as formatElevatedRuntimeHint,
  c as formatInternalVerboseCurrentReplyOnlyText,
  i as formatDirectiveAck,
  l as formatInternalVerbosePersistenceDeniedText,
  n as canPersistInternalVerboseDirective,
  o as formatElevatedUnavailableText,
  r as enqueueModeSwitchEvents,
  s as formatInternalExecPersistenceDeniedText,
  t as canPersistInternalExecDirective,
  u as withOptions,
} from "./directive-handling.shared-CgtfsG4G.js";
import "./plugins-Dn-pJrhh.js";
import "./sessions-Bdy1wToU.js";
import { n as resolveExecDefaults } from "./exec-defaults-C4RqVOp-.js";
import { t as resolveFastModeState } from "./fast-mode-C7KEUg_P.js";
import "./model-selection-CyVXHdEG.js";
import { n as applyVerboseOverride, t as applyTraceOverride } from "./level-overrides-Dz8iJ1QO.js";
import { t as maskApiKey } from "./mask-api-key-C4W96Oz3.js";
import "./selection-BcCvjGlJ.js";
import { d as resolveUsableCustomProviderApiKey } from "./model-auth-Do8Bfz1M.js";
...
import { n as resolveSelectedAndActiveModel } from "./model-runtime-D7UoNxI6.js";
import {
  _ as modelKey,
...
import { i as resolveQueueSettings, l as refreshQueuedFollowupSession } from "./queue-m5o6nz1B.js";
      ...
      label: order
        .map((profileId) => {
          ...
          if (
            ...
            return `${profileId}=missing${formatFlagsSuffix(flags)}`;
          if (profile.type === "api_key")
            return `${profileId}=${resolveStoredCredentialLabel({
              value: profile.key,
              refValue: profile.keyRef,
              mode,
          ...
          if (profile.type === "token") {
            ...
            const expirationFlag = formatExpirationLabel(
              profile.expires,
              now,
              formatUntil,
              "expired",
...
async function maybeHandleModelDirectiveInfo(params) {
  ...
  if (wantsSummary) {
    ...
    const activeRuntimeLine = modelRefs.activeDiffers
      ...
      : null;
    const channelData = (
      params.surface ? getChannelPlugin(params.surface) : null
    )?.commands?.buildModelBrowseChannelData?.();
    if (channelData)
      return {
        text: [
...
function isCompleteSessionEntry(entry) {
  return Boolean(
    entry && typeof entry.sessionId === "string" && typeof entry.updatedAt === "number",
  );
}
//#endregion
...
function maybeHandleQueueDirective(params) {
  ...
  if (
    ...
    directives.rawQueueMode === void 0 &&
    directives.rawDebounce === void 0 &&
    directives.rawCap === void 0 &&
    directives.rawDrop === void 0
  ) {
  ...
  const queueDebounceInvalid =
    directives.rawDebounce !== void 0 && typeof directives.debounceMs !== "number";
...
async function handleDirectiveOnly(params) {
  ...
  if (directives.hasThinkDirective && !directives.thinkLevel) {
    if (!directives.rawThinkLevel)
      return {
        text: withOptions(
          `Current thinking level: ${currentThinkLevel ?? "off"}.`,
  ...
  if (directives.hasFastDirective && directives.fastMode === void 0) {
    if (
      ...
      return {
        text: withOptions(
          `Current fast mode: ${effectiveFastMode ? "on" : "off"}${effectiveFastModeSource === "config" ? " (config)" : effectiveFastModeSource === "default" ? " (default)" : ""}.`,
          "status, on, off",
        ),
      };
    return {
  ...
  if (directives.hasElevatedDirective && !directives.elevatedLevel) {
    if (!directives.rawElevatedLevel) {
      ...
      return {
        text: [
          withOptions(
            `Current elevated level: ${currentElevatedLevel ?? "off"}.`,
            "on, off, ask, full",
          ),
  ...
  const prevElevatedLevel =
    currentElevatedLevel ?? sessionEntry.elevatedLevel ?? (elevatedAllowed ? "on" : "off");
  ...
  const shouldPersistSessionEntry =
    ...
    (directives.hasFastDirective && directives.fastMode !== void 0) ||
  ...
  if (shouldPersistSessionEntry) {
    ...
    if (modelSelection)
      modelSelectionUpdated = applyModelOverrideToSessionEntry({
        entry: sessionEntry,
