...
import { s as createDraftStreamLoop } from "./channel-lifecycle-BBBTbPMe.js";
...
import {
  ...
  d as resolveChannelPreviewStreamMode,
...
import { r as resolveDualTextControlCommandGate } from "./command-gating-BAZRoOCR.js";
...
import {
  i as resolveChannelTurnDispatchCounts,
  n as hasFinalChannelTurnDispatch,
} from "./dispatch-result-Cyoa53q0.js";
import {
...
import {
  ...
  B as isUrlAllowed,
  C as buildUserAgent,
  D as formatUnknownError,
  E as formatMSTeamsSendErrorHint,
  F as extractHtmlFromAttachment,
  G as resolveRequestUrl,
  H as readNestedString,
  I as extractInlineImageCandidates,
  J as tryBuildGraphSharesUrlForSharedLink,
  K as safeFetchWithPolicy,
  L as inferPlaceholder,
  M as applyAuthorizationHeaderForUrl,
  N as encodeGraphShareId,
  P as estimateBase64DecodedBytes,
  R as isDownloadableAttachment,
  S as loadMSTeamsSdkWithAuth,
  T as classifyMSTeamsSendError,
  U as resolveAttachmentFetchPolicy,
  V as normalizeContentType,
  W as resolveMediaSsrfPolicy,
  _ as resolveMSTeamsStorePath,
  a as fetchGraphJson,
  b as createMSTeamsAdapter,
  h as resolveMSTeamsCredentials,
  j as IMG_SRC_RE,
  k as ATTACHMENT_TAG_RE,
  q as safeHostForUrl,
  w as ensureUserAgentHeader,
  x as createMSTeamsTokenProvider,
  y as createBotFrameworkJwtValidator,
  z as isLikelyImageAttachment,
} from "./graph-users-CtrIdNVg.js";
import {
...
import {
  a as resolveMSTeamsRouteConfig,
  i as resolveMSTeamsReplyPolicy,
  n as resolveMSTeamsAllowlistMatch,
  t as isMSTeamsGroupAllowed,
} from "./policy-gyGlBeFg.js";
import {
  C as readJsonFile,
  S as createMSTeamsConversationStoreFs,
  T as writeJsonFile,
  _ as buildFileInfoCard,
  b as createMSTeamsPollStoreFs,
  c as renderReplyPayloadsToMessages,
  d as withRevokedProxyFallback,
  f as resolveGraphChatId,
  g as removePendingUploadFs,
  h as getPendingUploadFs,
  l as sendMSTeamsMessages,
  m as removePendingUpload,
  p as getPendingUpload,
  s as buildConversationReference,
  u as AI_GENERATED_ENTITY,
  v as parseFileConsentInvoke,
  w as withFileLock,
  x as extractMSTeamsPollVote,
  y as uploadToConsentUrl,
} from "./probe-4KdV1VaO.js";
import { n as readResponseWithLimit } from "./read-response-with-limit-BoeNKitZ.js";
...
} from "./resolve-allowlist-BJh7ranH.js";
import { n as getOptionalMSTeamsRuntime, t as getMSTeamsRuntime } from "./runtime-api-B6Kge591.js";
...
function pruneExpiredCooldowns(cooldownMs) {
  ...
  for (const [key, time] of lastReflectionBySession)
    if (now - time >= cooldownMs) lastReflectionBySession.delete(key);
}
...
async function storeSessionLearning(params) {
  ...
  if (!exists && legacyLearningsFile !== learningsFile)
    await fs.rm(legacyLearningsFile, { force: true }).catch(() => void 0);
}
//#endregion
...
async function runFeedbackReflection(params) {
  ...
  const storePath = getMSTeamsRuntime().channel.session.resolveStorePath(cfg.session?.store, {
    agentId: params.agentId,
  });
...
async function downloadMSTeamsBotFrameworkAttachment(params) {
  ...
  if (
    !isUrlAllowed(
      `${normalizeServiceUrl(params.serviceUrl)}/v3/attachments/${encodeURIComponent(params.attachmentId)}`,
      policy.allowHosts,
    )
  )
    return;
  ...
  const candidateView =
    views.find((view) => view?.viewId === "original") ??
    views.find((view) => typeof view?.viewId === "string");
  const viewId =
  ...
  return {
    buffer: await readResponseWithLimit(response, params.maxBytes, {
      onOverflow: ({ size, maxBytes }) =>
        /* @__PURE__ */ new Error(`payload size ${size} exceeds maxBytes ${maxBytes}`),
    }),
...
function scopeCandidatesForUrl(url) {
  try {
    ...
    return host.endsWith("graph.microsoft.com") ||
...
function buildMSTeamsAttachmentPlaceholder(attachments, limits) {
  ...
  if (list.length === 0) return "";
  const totalImages =
    list.filter(isLikelyImageAttachment).length + extractInlineImageCandidates(list, limits).length;
 ...
 */
async function fetchParentMessageCached(
  token,
  groupId,
  channelId,
  parentId,
  fetchParent = fetchChannelMessage,
) {
...
function extractId(response) {
  if (response && typeof response === "object" && "id" in response)
    return readStringValue(response.id);
}
    ...
    return (
      await deliverWithFinalizableLivePreviewAdapter({
        ...
        adapter: defineFinalizableLivePreviewAdapter({
          ...
          resolveFinalizedId: (previewId) =>
            stream.messageId ?? stream.previewStreamId ?? previewId,
...
function createMSTeamsReplyDispatcher(params) {
  ...
  const resolvedBlockStreamingEnabled =
    resolveChannelPreviewStreamMode(msteamsCfg, "partial") === "block"
      ? true
      : resolveChannelStreamingBlockEnabled(msteamsCfg);
...
function resolveMSTeamsRouteSessionKey(params) {
  const channelThreadId = params.isChannel
    ? (params.conversationMessageId ?? params.replyToId ?? void 0)
    : void 0;
...
function createMSTeamsMessageHandler(deps) {
  ...
  const historyLimit = Math.max(
    0,
    msteamsCfg?.historyLimit ?? cfg.messages?.groupChat?.historyLimit ?? 50,
  );
  ...
  const handleTeamsMessageNow = async (params) => {
    ...
    const attachmentTypes = attachments
      .map((att) => (typeof att.contentType === "string" ? att.contentType : void 0))
      .filter(Boolean)
      .slice(0, 3);
    ...
    const conversationRef = buildStoredConversationReference({
      ...
      threadId:
        conversationType === "channel"
          ? (conversationMessageId ?? activity.replyToId ?? void 0)
          : void 0,
    });
    const {
      dmPolicy,
    ...
    const mediaPayload = buildMSTeamsMediaPayload(
    ...
    if (activity.replyToId && isChannel && teamId)
      try {
        ...
        const isThreadSenderAllowed = (msg) =>
          groupPolicy === "allowlist"
            ? resolveMSTeamsAllowlistMatch({
                allowFrom: effectiveGroupAllowFrom,
    ...
    const senderTimezone =
      activity.entities?.find((e) => e.type === "clientInfo")?.timezone || conversationRef.timezone;
...
async function handleFeedbackInvoke(context, deps) {
  ...
  const feedbackThreadId = isChannel
    ? (extractMSTeamsConversationMessageId(rawConversationId) ?? activity.replyToId ?? void 0)
    : void 0;
  if (feedbackThreadId)
...
async function monitorMSTeamsProvider(opts) {
  ...
  const resolveAllowlistUsers = async (label, entries) => {
    ...
    for (const entry of resolved)
      if (entry.resolved && entry.id) additions.push(entry.id);
    ...
    jwtValidator
      ...
      .catch((err) => {
        if (
          ...
          /ECONNREFUSED|ENOTFOUND|EHOSTUNREACH|ETIMEDOUT|ECONNRESET/i.test(err.code ?? err.message)
        )
          runtime?.error(
  ...
  const handler = {
    ...
    async run(context) {
      ...
      if (activityType === "message") for (const h of messageHandlers) await h(context, noop);
        ...
        for (const h of membersAddedHandlers) await h(context, noop);
      else if (activityType === "messageReaction") {
        const activity = ctx?.activity;
        if (activity?.reactionsAdded?.length)
          for (const h of reactionsAddedHandlers) await h(context, noop);
        if (activity?.reactionsRemoved?.length)
          for (const h of reactionsRemovedHandlers) await h(context, noop);
      }
    },
  };
  return handler;
