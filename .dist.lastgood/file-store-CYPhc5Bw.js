...
import { o as writeSecretFileAtomic } from "./secret-file-CvOsk3nE.js";
import {
...
import {
  n as getFsSafeLockConfig,
  t as createSidecarLockManager,
} from "./sidecar-lock-BaqvhFfF.js";
import {
  f as createSyncDirectoryGuard,
  l as assertSyncDirectoryGuard$1,
...
async function pruneExpiredStoreEntries(params) {
  ...
  const pruneEmptyDirs =
    (recursive || maxDepth !== void 0) && (params.options.pruneEmptyDirs ?? false);
  ...
  async function assertRootGuard() {
    ...
    if (
      ...
      (await fs$1.realpath(rootGuard.dir)) !== rootGuard.realPath
    )
...
function ensureParentSync(params) {
  ...
  for (const segment of path.relative(rootDir, dir).split(path.sep).filter(Boolean)) {
    ...
    if (!isPathInside(rootReal, fs.realpathSync(current)))
...
async function readFileStoreCopySource(params) {
  ...
  if (params.maxBytes !== void 0 && sourceStat.size > params.maxBytes)
  ...
  try {
    ...
    if (params.maxBytes !== void 0 && message.includes(`exceeds ${params.maxBytes} bytes`))
      throw new FsSafeError("too-large", `file exceeds maximum size of ${params.maxBytes} bytes`, {
        cause: error instanceof Error ? error : void 0,
      });
    throw error;
...
function assertMaxBytes(size, maxBytes) {
  if (maxBytes !== void 0 && size > maxBytes)
  ...
  return matchRootFileOpenFailure(opened, {
    path: (failure) => {
      ...
      throw new FsSafeError("path-mismatch", "store target changed during read", {
        cause: failure.error instanceof Error ? failure.error : void 0,
      });
    },
    validation: (failure) => {
      ...
      return createJsonStore(
        {
          ...
          readRequired: async () =>
            await (await openRoot()).readJson(assertRelativePath(relativePath)),
...
function ensurePrivateDirectorySync(rootDir, targetDir, mode) {
  ...
  for (const segment of path.relative(root, target).split(path.sep).filter(Boolean)) {
    ...
    if (!isPathInside(fs.realpathSync(root), fs.realpathSync(current)))
