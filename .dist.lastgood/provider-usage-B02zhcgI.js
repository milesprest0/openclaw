...
import { t as resolveFetch } from "./fetch-Cwtbr1ux.js";
...
import { a as resolveProviderAuthEnvVarCandidates } from "./provider-env-vars-DkmNxQP4.js";
import { r as normalizeProviderId } from "./provider-id-CG9pXYPs.js";
import {
  R as resolveProviderUsageAuthWithPlugin,
  z as resolveProviderUsageSnapshotWithPlugin,
} from "./provider-runtime-DueHRTen.js";
import {
  a as resolveLegacyPiAgentAccessToken,
  c as withTimeout,
  i as ignoredErrors,
...
function normalizeProviderIds(providerIds) {
  return [
    ...new Set(
      [...providerIds]
        .map((providerId) => (providerId ? normalizeProviderId(providerId) : void 0))
...
function resolveUsageCredentialProviderIds(params) {
  ...
  try {
    ...
    for (const plugin of snapshot.plugins) {
      ...
      if (
        !isUsageProviderManifestEligible({
          plugin,
          state: params.state,
        })
      )
  ...
  const tasks = auths.map((auth) => {
    ...
    return withTimeout(
      ...
      return failureSnapshot(
        (error instanceof Error ? error.message : String(error)).trim() || "Fetch failed",
      );
    });
  });
  return {
...
//#endregion
export {
  formatUsageWindowSummary as i,
  formatUsageReportLines as n,
  formatUsageSummaryLine as r,
  loadProviderUsageSummary as t,
};
