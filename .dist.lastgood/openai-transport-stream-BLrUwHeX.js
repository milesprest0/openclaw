...
import { r as resolveDebugProxySettings } from "./env-C2C0rw4L.js";
import {
  a as withTrustedEnvProxyGuardedFetchMode,
  n as fetchWithSsrFGuard,
} from "./fetch-guard-jcadyt5o.js";
import { t as normalizeToolParameterSchema } from "./pi-tools-parameter-schema-Dqx6G8X9.js";
import { r as resolveProviderRequestCapabilities } from "./provider-attribution-SP5uub3O.js";
import { c as detectOpenAICompletionsCompat } from "./provider-model-compat-DIuSGYN-.js";
import {
  a as mergeModelProviderRequestOverrides,
  i as getModelProviderRequestTransport,
  l as resolveProviderRequestPolicyConfig,
  n as buildProviderRequestDispatcherPolicy,
} from "./provider-request-config-B-kAAqy2.js";
import { L as resolveProviderTransportTurnStateWithPlugin } from "./provider-runtime-DueHRTen.js";
import { c as shouldUseEnvHttpProxyForUrl } from "./proxy-env-C3VNcjQ7.js";
import { n as repairToolUseResultPairing } from "./session-transcript-repair-BjkUYNeL.js";
import { v as ssrfPolicyFromHttpBaseUrlFakeIpHostnameAllowlist } from "./ssrf-DO8eIXaD.js";
import {
  ...
  if (record.type === "object") {
    ...
    if (!required) violations.push(`${path}.required`);
      ...
      for (const key of Object.keys(properties))
        if (!requiredSet.has(key)) violations.push(`${path}.required.${key}`);
    }
  }
  if (
    record.properties &&
...
function resolveOpenAIResponsesCompactThreshold(model) {
  ...
  if (contextWindow) return Math.max(1e3, Math.floor(contextWindow * 0.7));
  return 8e4;
}
...
function applyOpenAIResponsesPayloadPolicy(payloadObj, policy) {
  ...
  if (policy.useServerCompaction && payloadObj.context_management === void 0)
...
//#endregion
//#region src/agents/provider-stream-idle.ts
const DEFAULT_HEARTBEAT_MS = 2e4;
const DEFAULT_FALLBACK_MS = 9e4;
/**
 ...
 */
function resolveIdleDetectionKnobs(model) {
  const params = model && typeof model === "object" ? (model.params ?? void 0) : void 0;
  const rawHeartbeat = readNumericKnob(params, "idleHeartbeatMs");
  const rawFallback = readNumericKnob(params, "idleFallbackMs");
  return {
    idleHeartbeatMs: rawHeartbeat === void 0 ? DEFAULT_HEARTBEAT_MS : Math.max(0, rawHeartbeat),
    idleFallbackMs: rawFallback === void 0 ? DEFAULT_FALLBACK_MS : Math.max(0, rawFallback),
  };
}
function readNumericKnob(params, key) {
  if (!params) return;
 ...
 * available).
 */
async function* wrapAsyncIterableWithIdleDetection(source, opts) {
  ...
  let idleFallbackError;
  const IDLE_SENTINEL = { idleFallbackSentinel: true };
  let resolveIdleDeadline;
  const clearTimers = () => {
  ...
  const armFallback = () => {
    ...
    fallbackTimer = setTimeout(() => {
      ...
      try {
        ...
        err = cbErr instanceof Error ? cbErr : /* @__PURE__ */ new Error("provider idle timeout");
      }
  ...
  try {
    while (true) {
      ...
      if (fallbackEnabled) {
        ...
        nextResult = await Promise.race([iterator.next(), idlePromise]);
        resolveIdleDeadline = void 0;
      } else nextResult = await iterator.next();
      if (nextResult.idleFallbackSentinel) {
        try {
          await iterator.return?.();
        } catch {}
        throw idleFallbackError ?? /* @__PURE__ */ new Error("provider idle timeout");
      }
      const step = nextResult;
      if (step.done) return;
      resetTimers();
      yield step.value;
    }
 ...
 */
function createProviderIdleTimeoutError(info) {
  ...
  err.retryable = true;
  if (info.provider) err.provider = info.provider;
  return err;
}
//#endregion
...
function resolveOpenAIStrictToolFlagWithDiagnostics(tools, strictSetting, context) {
  ...
  if (strictSetting === true && strict === false && log.isEnabled("debug", "any")) {
    ...
    log.debug(
      `OpenAI ${context.transport} tool schema strict mode downgraded to strict=false for ${context.model.provider ?? "unknown"}/${context.model.id ?? "unknown"} because ${diagnostics.length} tool schema(s) are not strict-compatible`,
      {
...
function ensureOpenAICodexResponsesInput(messages, context) {
  ...
  if (!buildOpenAICodexResponsesInstructions(context))
    throw new Error(
...
function buildOpenAIResponsesParams(model, context, options, metadata) {
  ...
  const supportsDeveloperRole =
    typeof compat.supportsDeveloperRole === "boolean" ? compat.supportsDeveloperRole : void 0;
  ...
  if (options?.serviceTier !== void 0 && payloadPolicy.allowsServiceTier)
    params.service_tier = options.serviceTier;
  if (context.tools)
    params.tools = convertResponsesTools(context.tools, model, {
      strict: resolveOpenAIStrictToolSetting(model, { transport: "stream" }),
    });
  if (model.reasoning) {
...
async function processOpenAICompletionsStream(responseStream, output, model, stream) {
  ...
  const flushPendingPostToolCallDeltas = () => {
    ...
    for (const delta of bufferedDeltas)
      if (delta.kind === "text") appendTextDeltaInternal(delta.text);
  ...
  const idleKnobs = resolveIdleDetectionKnobs(model);
  const idleAwareCompletionsStream = wrapAsyncIterableWithIdleDetection(responseStream, {
    idleHeartbeatMs: idleKnobs.idleHeartbeatMs,
    idleFallbackMs: idleKnobs.idleFallbackMs,
    onHeartbeat: ({ elapsedMs }) => {
      stream.push({
  ...
  for await (const rawChunk of idleAwareCompletionsStream) {
    ...
    if (typeof chunk.model === "string" && chunk.model) output.servedModel = chunk.model;
    ...
    if (choice.delta.content) {
      ...
      for (const contentDelta of contentDeltas)
        if (currentBlock?.type === "toolCall") queuePostToolCallDelta(contentDelta);
        else if (contentDelta.kind === "text") appendTextDelta(contentDelta.text);
        else appendThinkingDelta(contentDelta);
      if (contentDeltas.length > 0) continue;
    }
...
function isOpenRouterGeminiModel(model) {
  return model.provider === "openrouter" && model.id.toLowerCase().includes("google/gemini");
}
function resolveOpenRouterGeminiSystemPromptSplit(model, systemPrompt) {
  if (!systemPrompt || !isOpenRouterGeminiModel(model)) return null;
...
function injectOpenRouterGeminiDynamicSystemSuffix(messages, dynamicSuffix) {
...
function buildOpenAICompletionsParams(model, context, options) {
    ...
    params.prompt_cache_key = options.sessionId;
  {
    ...
    if (effectiveMaxTokens)
      if (compat.maxTokensField === "max_tokens") params.max_tokens = effectiveMaxTokens;
  ...
  if (
    ...
    params.reasoning = { effort: resolvedCompletionsReasoningEffort };
  else if (
    resolvedCompletionsReasoningEffort &&
    model.reasoning &&
    compat.supportsReasoningEffort &&
...
//#endregion
export {
  resolveOpenAIReasoningEffortForModel as C,
  normalizeOpenAIReasoningEffort as S,
  transformTransportMessages as _,
  applyOpenAIResponsesPayloadPolicy as a,
  resolveOpenAIStrictToolFlagForInventory as b,
  coerceTransportToolCallArguments as c,
  failTransportStream as d,
  finalizeTransportStream as f,
  sanitizeTransportPayloadText as g,
  sanitizeNonEmptyTransportPayloadText as h,
  createOpenAIResponsesTransportStreamFn as i,
  createEmptyTransportUsage as l,
  mergeTransportMetadata as m,
  createAzureOpenAIResponsesTransportStreamFn as n,
  resolveOpenAIResponsesPayloadPolicy as o,
  mergeTransportHeaders as p,
  createOpenAICompletionsTransportStreamFn as r,
  flattenCompletionMessagesToStringContent as s,
  buildOpenAICompletionsParams as t,
  createWritableTransportEventStream as u,
  buildGuardedModelFetch as v,
  resolveOpenAIStrictToolSetting as w,
  mapOpenAIReasoningEffortForModel as x,
  normalizeOpenAIStrictToolParameters as y,
};
