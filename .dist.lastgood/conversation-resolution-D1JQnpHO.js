...
import { r as normalizeConversationTargetRef } from "./session-binding-normalization-C5mpgnkR.js";
import {
...
import {
  n as resolveBundledChannelThreadBindingInboundConversation,
  t as resolveBundledChannelThreadBindingDefaultPlacement,
} from "./thread-binding-api-JqQauuqz.js";
...
function resolveChannelTargetId(params) {
  ...
  const parsedTarget = normalizeOptionalString(
    parseExplicitTargetForChannel(params.channel, target)?.to,
  );
  if (parsedTarget)
    return resolveConversationIdFromTargets({ targets: [parsedTarget] }) ?? parsedTarget;
  return target;
}
...
//#endregion
export {
  resolveCommandConversationResolution as n,
  resolveInboundConversationResolution as r,
  resolveChannelDefaultBindingPlacement as t,
};
