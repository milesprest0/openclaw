...
import "./operator-scopes-Codapd9g.js";
...
import { r as isAdminOnlyMethod } from "./method-scopes-DaDxLn4o.js";
import { p as resolveSubagentSpawnModelSelection } from "./model-selection-CyVXHdEG.js";
import { t as privateFileStore } from "./private-file-store-Bi7hPs_Y.js";
import "./sessions-Bdy1wToU.js";
import { i as resolveContextEngine } from "./registry-sQP5dNFn.js";
...
import { c as normalizeAgentId, s as isValidAgentId } from "./session-key-B4qUwRzq.js";
import { i as isCronSessionKey, o as parseAgentSessionKey } from "./session-key-utils-B3KPN8Ee.js";
import { t as emitSessionLifecycleEvent } from "./session-lifecycle-events-D9760v5u.js";
import {
  m as resolveGatewaySessionStoreTarget,
  u as pruneLegacyStoreKeys,
...
import {
  ...
  n as mapToolContextToSpawnedRunMetadata,
...
import {
  p as registerSubagentRun,
  r as countActiveRunsForSession,
} from "./subagent-registry-DESvhcq6.js";
import { t as buildSubagentSystemPrompt } from "./subagent-system-prompt-DPMjI8nX.js";
import { f as normalizeThinkLevel, t as formatThinkingLevels } from "./thinking-C9_Z62px.js";
import { u as resolveThreadBindingSpawnPolicy } from "./thread-bindings-policy-CD0S48SB.js";
...
async function materializeSubagentAttachments(params) {
  ...
  try {
    ...
    for (const raw of requestedAttachments) {
      ...
      const encoding =
        (normalizeOptionalString(raw?.encoding) ?? "utf8") === "base64" ? "base64" : "utf8";
...
async function prepareContextEngineSubagentSpawn(params) {
  try {
    ...
    return {
      status: "ok",
      preparation: await (
        await subagentSpawnDeps.resolveContextEngine(params.cfg)
...
async function spawnSubagentDirect(params, ctx) {
  ...
  if (spawnMode === "session" && !requestThreadBinding)
    return {
      status: "error",
      error:
        'sessions_spawn(mode="session") requires thread=true so the subagent can stay bound to a channel thread. Retry with { mode: "session", thread: true } on a channel that supports threads, use mode="run" for one-shot work, or use sessions_send(sessionKey=...) to keep talking to a persistent session without thread binding.',
    };
  const cleanup =
    spawnMode === "session"
      ? "keep"
