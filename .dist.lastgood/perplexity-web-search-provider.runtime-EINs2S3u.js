...
import {
  a as isDirectPerplexityBaseUrl,
  i as inferPerplexityBaseUrlFromApiKey,
  n as PERPLEXITY_DIRECT_BASE_URL,
  t as DEFAULT_PERPLEXITY_BASE_URL,
...
import {
  ...
  o as isoToPerplexityDate,
  p as readProviderEnvValue,
  s as normalizeFreshness,
  v as throwWebSearchApiError,
...
function extractPerplexityCitations(data) {
  ...
  for (const choice of data.choices ?? [])
    for (const annotation of choice.message?.annotations ?? []) {
      ...
      const normalizedUrl = normalizeOptionalString(
...
async function executePerplexitySearch(args, searchConfig) {
  ...
  if (!structured) {
    ...
    if (maxTokens !== void 0 || maxTokensPerPage !== void 0)
      return {
