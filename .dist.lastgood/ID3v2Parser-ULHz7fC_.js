...
import {
  ...
  l as UINT32SYNCSAFE,
  n as ExtendedHeader,
  o as TextEncodingToken,
  r as ID3v2Header,
  s as TextHeader,
  t as AttachedPictureType,
} from "./ID3v2Token-Ck_ECRa1.js";
  ...
  readData(uint8Array, type, includeCovers) {
    ...
    switch (type !== "TXXX" && type[0] === "T" ? "T*" : type) {
      ...
      case "POPM": {
        ...
        if (uint8Array.length === 0) {
          this.warningCollector.addWarning(
            `id3v2.${this.major} type=${type} POPM frame missing rating byte`,
          );
          output = {
            email,
            rating: 0,
      ...
      case "CHAP": {
        ...
        while (offset < length) {
          const subFrame = readFrameHeader(
            uint8Array.subarray(offset),
            this.major,
...
var ID3v2Parser = class ID3v2Parser {
  ...
  async parse(metadata, tokenizer, options) {
    ...
    await (id3Header.flags.isExtendedHeader
  ...
  parseMetadata(data) {
    ...
    while (true) {
      ...
      const frameHeader = readFrameHeader(
        frameHeaderBytes,
  ...
  static mapId3v2Chapters(id3Tags) {
    ...
    const topLevelToc = id3Tags
      .filter((t) => t.id === "CTOC")
      ?.find((t) => t.value.flags?.topLevel);
