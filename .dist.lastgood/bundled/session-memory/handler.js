...
import { r as resolveHookConfig } from "../../config-BBGMGbLn.js";
import { i as hasInterSessionUserProvenance } from "../../input-provenance-OI5EnRxZ.js";
import { t as generateSlugViaLLM } from "../../llm-slug-generator-DGocbz77.js";
...
async function getRecentSessionContentWithResetFallback(sessionFilePath, messageCount = 15) {
  ...
  try {
    ...
    const resetCandidates = (await fs.readdir(dir))
      .filter((name) => name.startsWith(resetPrefix))
      .toSorted();
    if (resetCandidates.length === 0) return primary;
    return (
      (await getRecentSessionContent(
        path.join(dir, resetCandidates[resetCandidates.length - 1]),
        messageCount,
      )) || primary
    );
  } catch {
