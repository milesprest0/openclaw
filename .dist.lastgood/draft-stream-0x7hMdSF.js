...
import { c as MsgType } from "./direct-management-FqagZ_db.js";
import {
  c as sendSingleTextMessageMatrix,
  n as editMessageMatrix,
  r as prepareMatrixSingleText,
...
function createMatrixDraftStream(params) {
  ...
  const sendOrEdit = async (text) => {
    ...
    try {
      if (!currentEventId) {
        currentEventId = (
          await sendSingleTextMessageMatrix(roomId, preparedText.trimmedText, {
            client,
            cfg,
            replyToId,
            threadId,
      ...
      if (
        ...
        err.message.startsWith("Matrix single-message text exceeds limit")
      )
        finalizeInPlaceBlocked = true;
      if (!currentEventId) sendFailed = true;
      stopped = true;
