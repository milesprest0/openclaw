...
import {
  a as CANVAS_HOST_PATH,
  c as isA2uiPath,
  n as normalizeUrlPath,
  r as resolveFileWithinRoot,
  s as injectCanvasLiveReload,
} from "./a2ui-BEuqaFeK.js";
...
import { D as sanitizeUntrustedFileName } from "./fs-safe-CgBWiL92.js";
...
import { t as resolveNodeFromNodeList } from "./node-resolve-DGp2jBCK.js";
...
import { a as asRecord } from "./record-coerce-BuYFJfzm.js";
...
import { g as shortenHomePath, p as resolveUserPath, s as ensureDir } from "./utils-BGRcpLKt.js";
...
function createDefaultCanvasCliDependencies() {
  const nodesCallOpts = (cmd, defaults) =>
    cmd
      .option(
        "--url <url>",
    ...
    canvas
      ...
      .action(async (opts) => {
        await deps.runNodesCommand("canvas snapshot", async () => {
          ...
          const payload = parseCanvasSnapshotPayload(
            (typeof raw === "object" && raw !== null ? raw : {}).payload,
          );
      ...
      .action(async (opts) => {
        await deps.runNodesCommand("canvas a2ui push", async () => {
          ...
          const jsonl = hasText
            ...
            : await fs$1.readFile(String(opts.jsonl), "utf8");
...
function parseCanvasHostConfig(value) {
  ...
  return {
    ...
    ...(readPositiveInteger(value.port) !== void 0
      ? { port: readPositiveInteger(value.port) }
      : {}),
    ...(readBoolean(value.liveReload) !== void 0
...
function buildCanvasDocumentEntryUrl(documentId, entrypoint) {
  const encodedEntrypoint = normalizeLogicalPath(entrypoint)
    .split("/")
...
function normalizeBasePath(rawPath) {
  const normalized = normalizeUrlPath(
    (rawPath ?? "/__openclaw__/canvas").trim() || "/__openclaw__/canvas",
  );
  ...
  const server = http.createServer((req, res) => {
    ...
    (async () => {
      if (req.url && isA2uiPath(new URL(req.url, "http://localhost").pathname)) {
        const { handleA2uiHttpRequest } = await import("./a2ui-CiMl-9F9.js");
...
//#endregion
export {
  resolveCanvasDocumentAssets as a,
  canvasConfigSchema as c,
  parseCanvasPluginConfig as d,
  resolveCanvasHostConfig as f,
  parseCanvasSnapshotPayload as g,
  canvasSnapshotTempPath as h,
  createCanvasDocument as i,
  isCanvasHostEnabled as l,
  registerNodesCanvasCommands as m,
  startCanvasHost as n,
  resolveCanvasDocumentDir as o,
  createDefaultCanvasCliDependencies as p,
  buildCanvasDocumentEntryUrl as r,
  resolveCanvasHttpPathToLocalPath as s,
  createCanvasHostHandler as t,
  isCanvasPluginEnabled as u,
};
