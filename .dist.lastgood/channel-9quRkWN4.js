...
import { t as listAccountIds } from "./account-ids-Cf3YHW18.js";
import { a as resolveWhatsAppAccount } from "./accounts-MM0YxrKm.js";
import { t as createWhatsAppLoginTool } from "./agent-tools-login-D4mr-Ujh.js";
import { n as buildDmGroupAccountAllowlistAdapter } from "./allowlist-config-edit-CiyWlmas.js";
...
import {
  ...
  n as isWhatsAppNewsletterJid,
  o as normalizeWhatsAppAllowFromEntry,
...
import { r as normalizeWhatsAppPayloadTextPreservingIndentation } from "./outbound-media-contract-NP9fM72I.js";
import "./normalize-BTcm6QzK.js";
...
import { t as getWhatsAppRuntime } from "./runtime-DbU0mohw.js";
import {
...
import {
  ...
  c as formatWhatsAppConfigAllowFromEntries,
  i as resolveWhatsAppGroupRequireMention,
  n as loadWhatsAppChannelRuntime,
  o as resolveWhatsAppGroupIntroHint,
  r as whatsappSetupWizardProxy,
  s as resolveWhatsAppMentionStripRegexes,
  ...
  send: {
    text: async (ctx) =>
      toWhatsAppMessageSendResult(
        await whatsappChannelOutbound.sendText({ ...ctx }),
        ctx.replyToId,
      ),
  },
});
...
async function checkWhatsAppHeartbeatReady(params) {
  ...
  if (
    !(params.deps?.hasActiveWebListener
  ...
  return collectIssuesForEnabledAccounts({
    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (healthState === "stale") {
        const staleSuffix =
          lastInboundAt != null
            ? ` (last inbound ${Math.max(0, Math.floor((Date.now() - lastInboundAt) / 6e4))}m ago)`
            : "";
        issues.push({
          channel: "whatsapp",
          accountId,
...
const loadWhatsAppDirectoryConfig = createLazyRuntimeModule(
  () => import("./directory-config-K1bsaCsS.js"),
);
const loadWhatsAppChannelReactAction = createLazyRuntimeModule(
  () => import("./channel-react-action-CCyAOmb8.js"),
);
  ...
  base: {
    ...createWhatsAppPluginBase({
      ...
      isConfigured: async (account) => {
        return (
          (await (await loadWhatsAppChannelRuntime()).readWebAuthState(account.authDir)) ===
          "linked"
        );
      },
    }),
    ...
    auth: {
      login: async ({ cfg, accountId, runtime, verbose }) => {
        const resolvedAccountId =
          accountId?.trim() || whatsappPlugin.config.defaultAccountId?.(cfg) || "default";
        await (
          await loadWhatsAppChannelRuntime()
        ).loginWeb(Boolean(verbose), void 0, runtime, resolvedAccountId);
      },
    },
    lifecycle: {
    ...
    heartbeat: {
      checkReady: async ({ cfg, accountId, deps }) =>
        await checkWhatsAppHeartbeatReady({
          cfg,
          accountId: accountId ?? void 0,
          deps,
        }),
    ...
    status: createAsyncComputedAccountStatusAdapter({
      ...
      resolveAccountSnapshot: async ({ account, runtime }) => {
        const authState = await (
          await loadWhatsAppChannelRuntime()
        ).readWebAuthState(account.authDir);
        return {
      ...
      logSelfId: ({ account, runtime, includeChannelPrefix }) => {
        loadWhatsAppChannelRuntime().then((runtimeExports) =>
