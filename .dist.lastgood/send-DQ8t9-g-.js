...
import {
  ...
  m as isRecord$1,
  u as createFeishuApiError,
  v as requestFeishuApi,
} from "./accounts-CFTFwStL.js";
import { r as createFeishuClient } from "./client-DiGc8k1n.js";
...
import { _ as mediaKindFromMime } from "./mime-CSQ-Gv-M.js";
import { r as withTempWorkspace } from "./private-temp-workspace-DKvr8oLR.js";
import "./text-runtime-lKuAtsoz.js";
import { r as readRegularFile } from "./regular-file-CoVHB-2u.js";
...
import {
  i as toFeishuSendResult,
  r as resolveFeishuReceiptKind,
  t as assertFeishuMessageApiSuccess,
...
import { i as resolveReceiveIdType, r as normalizeFeishuTarget } from "./targets-DqST8ng9.js";
import { i as withTempDownloadPath } from "./temp-download-CgMpGD8s.js";
...
function extractFeishuUploadKey(response, params) {
  ...
  if (wrappedResponse.code !== void 0 && wrappedResponse.code !== 0)
    throw new Error(
...
function extractFeishuDownloadMetadata(response) {
  ...
  return {
    contentType,
    fileName:
      (disposition ? decodeDispositionFileName(disposition) : void 0) ??
...
async function readFeishuResponseBuffer(params) {
  ...
  const responseWithOptionalFields = response;
  if (responseWithOptionalFields.code !== void 0 && responseWithOptionalFields.code !== 0)
    throw new Error(
...
async function uploadImageFeishu(params) {
  ...
  return {
    imageKey: extractFeishuUploadKey(
...
async function uploadFileFeishu(params) {
  ...
  return {
    fileKey: extractFeishuUploadKey(
      await requestFeishuApi(
        () =>
...
function resolveFeishuOutboundMediaKind(params) {
  ...
  if (
    [".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp", ".ico", ".tiff"].includes(ext) ||
    mimeKind === "image"
  )
...
function isFeishuBroadcastMention(mention) {
  ...
  return [mention.id?.open_id, mention.id?.user_id, mention.id?.union_id].some(
    (id) => id?.trim().toLowerCase() === "all",
  );
}
/**
...
function renderElement(element, imageKeys, mediaKeys, mentionedOpenIds) {
  ...
  switch (normalizeLowercaseStringOrEmpty(toStringOrEmpty(element.tag))) {
    ...
    case "at":
      {
        const normalizedMention = normalizeFeishuExternalKey(
...
function parsePostContent(content) {
  try {
    ...
    return {
      textContent:
        [escapeMarkdownText(payload.title.trim()), paragraphs.join("\n").trim()]
          .filter(Boolean)
          .join("\n\n")
...
function readInteractiveElementArrays(parsed) {
  ...
  for (const candidate of [parsed.elements, body?.elements])
    if (Array.isArray(candidate)) elementArrays.push(candidate);
  for (const candidate of [parsed.i18n_elements, body?.i18n_elements]) {
    if (!isRecord(candidate)) continue;
    for (const localeElements of Object.values(candidate))
      if (Array.isArray(localeElements)) elementArrays.push(localeElements);
  }
...
async function getMessageFeishu(params) {
  ...
  try {
    ...
    const item =
      rawItem && (rawItem.body !== void 0 || rawItem.message_id !== void 0) ? rawItem : null;
    if (!item) return null;
...
async function listFeishuThreadMessages(params) {
  ...
  if (!account.configured) throw new Error(`Feishu account "${account.accountId}" not configured`);
  const response = await createFeishuClient(account).im.message.list({
    params: {
...
async function editMessageFeishu(params) {
  ...
  if (!account.configured) throw new Error(`Feishu account "${account.accountId}" not configured`);
  if ((typeof text === "string" && text.trim().length > 0) === Boolean(card))
  ...
  const payload = buildFeishuPostMessagePayload({
    messageText: convertMarkdownTables(
      text,
...
//#endregion
export {
  shouldSuppressFeishuTextForVoiceMedia as _,
  sendCardFeishu as a,
  sendStructuredCardFeishu as c,
  extractMentionTargets as d,
  isFeishuBroadcastMention as f,
  sendMediaFeishu as g,
  downloadMessageResourceFeishu as h,
  resolveFeishuCardTemplate as i,
  parsePostContent as l,
  isFeishuGroupChatType as m,
  getMessageFeishu as n,
  sendMarkdownCardFeishu as o,
  isMentionForwardRequest as p,
  listFeishuThreadMessages as r,
  sendMessageFeishu as s,
  editMessageFeishu as t,
  buildMentionedCardContent as u,
  normalizeFeishuExternalKey as v,
};
