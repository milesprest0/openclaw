...
function resolveThreadBindingThreadName(params) {
  return `🤖 ${normalizeOptionalString(params.label) || normalizeOptionalString(params.agentId) || "agent"}`
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 100);
}
...
function resolveThreadBindingFarewellText(params) {
  ...
  if (params.reason === "idle-expired")
    return prefixSystemMessage(
      `Session ended automatically after ${formatThreadBindingDurationLabel(normalizeThreadBindingDurationMs(params.idleTimeoutMs))} of inactivity. Messages here will no longer be routed.`,
    );
  if (params.reason === "max-age-expired")
    return prefixSystemMessage(
      `Session ended automatically at max age of ${formatThreadBindingDurationLabel(normalizeThreadBindingDurationMs(params.maxAgeMs))}. Messages here will no longer be routed.`,
    );
  return prefixSystemMessage(DEFAULT_THREAD_BINDING_FAREWELL_TEXT);
}
//#endregion
export {
  resolveThreadBindingThreadName as i,
  resolveThreadBindingFarewellText as n,
  resolveThreadBindingIntroText as r,
  formatThreadBindingDurationLabel as t,
};
