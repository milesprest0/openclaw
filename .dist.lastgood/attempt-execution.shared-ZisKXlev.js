import {
  n as formatAgentInternalEventsForPrompt,
  t as formatAgentInternalEventsForPlainPrompt,
} from "./internal-events-BUopmNQM.js";
import {
  c as hasInternalRuntimeContext,
  u as stripInternalRuntimeContext,
...
async function persistSessionEntry(params) {
  const persisted = await updateSessionStore(params.storePath, (store) => {
    ...
    for (const field of params.clearedFields ?? [])
      if (!Object.hasOwn(params.entry, field)) Reflect.deleteProperty(merged, field);
...
function resolvePlainInternalEventBody(body, events) {
  ...
  if (!renderedEvents) return body;
  return (
    [renderedEvents, stripInternalRuntimeContext(body).trim()].filter(Boolean).join("\n\n") || body
  );
}
...
//#endregion
export {
  resolveInternalEventTranscriptBody as i,
  prependInternalEventContext as n,
  resolveAcpPromptBody as r,
  persistSessionEntry as t,
};
