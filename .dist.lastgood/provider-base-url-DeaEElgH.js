import {
  a as OLLAMA_DEFAULT_MAX_TOKENS,
  i as OLLAMA_DEFAULT_COST,
...
async function queryOllamaModelShowInfo(apiBase, modelName) {
  ...
  try {
    ...
    try {
      ...
      if (paramCtx !== void 0 && (contextWindow === void 0 || paramCtx > contextWindow))
...
async function buildOllamaProvider(configuredBaseUrl, opts) {
  ...
  return {
    ...
    api: "ollama",
    models: (
      await enrichOllamaModelsWithContext(apiBase, models.slice(0, OLLAMA_CONTEXT_ENRICH_LIMIT))
    ).map((model) =>
...
//#endregion
export {
  enrichOllamaModelsWithContext as a,
  queryOllamaContextWindow as c,
  buildOllamaProvider as i,
  queryOllamaModelShowInfo as l,
  buildOllamaBaseUrlSsrFPolicy as n,
  fetchOllamaModels as o,
  buildOllamaModelDefinition as r,
  isReasoningModelHeuristic as s,
  readProviderBaseUrl as t,
  resolveOllamaApiBase as u,
};
