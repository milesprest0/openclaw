...
import {
  i as coerceImageAssistantText,
  r as minimaxUnderstandImage,
  s as hasImageReasoningOnlyResponse,
  t as isMinimaxVlmModel,
} from "./minimax-vlm-Dobrn9Ul.js";
import { l as resolveApiKeyForProvider, r as getApiKeyForModel } from "./model-auth-Do8Bfz1M.js";
...
import { v as normalizeModelRef } from "./model-selection-shared-D9oQINZ2.js";
...
import { b as prepareProviderDynamicModel } from "./provider-runtime-DueHRTen.js";
...
function composeImageDescriptionPayloadHandlers(first, second) {
  ...
  return (payload, payloadModel) => {
    const runSecond = (firstResult) => {
      ...
      const coerceResult = (resolvedSecond) =>
        resolvedSecond === void 0 ? firstResult : resolvedSecond;
...
async function resolveImageRuntime(params) {
  ...
  if (!model.input?.includes("image")) {
    ...
    throw new Error(
      `Model does not support images: ${params.provider}/${params.model} (resolved ${model.provider}/${model.id} input: ${formatModelInputCapabilities(model.input)})`,
    );
  }
    ...
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timeout = setTimeout(() => {
          ...
          reject(
            /* @__PURE__ */ new Error(`image description timed out after ${params.timeoutMs}ms`),
          );
        }, params.timeoutMs);
      }),
    ]);
