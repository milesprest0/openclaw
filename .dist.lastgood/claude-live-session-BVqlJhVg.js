...
import { t as extractBalancedJsonFragments } from "./balanced-json-mt5Zw5l1.js";
...
function toCliUsage(raw) {
  ...
  const input =
    pick("input") ??
    ((Object.hasOwn(raw, "cached") || nestedCached !== void 0) && typeof totalInput === "number"
...
function parseCliJson(raw, backend, providerId) {
  ...
  for (const parsed of parsedRecords) {
    ...
    const nextText =
      ...
      collectCliText(parsed);
    const trimmedText = (
      shouldUnwrapNestedCliResultText({
        providerId,
        parsed,
      })
...
function resetNoOutputTimer(session) {
  ...
  turn.noOutputTimer = setTimeout(() => {
    closeLiveSession(
      ...
      createTimeoutError(
        session,
        `CLI produced no output for ${Math.round(session.noOutputTimeoutMs / 1e3)}s and was terminated.`,
      ),
    );
...
function createTurn(params) {
  ...
  turn.noOutputTimer = setTimeout(() => {
    closeLiveSession(
      ...
      createTimeoutError(
        params.session,
        `CLI produced no output for ${Math.round(params.noOutputTimeoutMs / 1e3)}s and was terminated.`,
      ),
    );
  ...
  turn.timeoutTimer = setTimeout(() => {
    closeLiveSession(
      ...
      createTimeoutError(
        params.session,
        `CLI exceeded timeout (${Math.round(params.context.params.timeoutMs / 1e3)}s) and was terminated.`,
      ),
    );
...
//#endregion
export {
  createCliJsonlStreamingParser as a,
  shouldUseClaudeLiveSession as i,
  closeClaudeLiveSessionForContext as n,
  extractCliErrorMessage as o,
  runClaudeLiveSessionTurn as r,
  parseCliOutput as s,
  buildClaudeLiveArgs as t,
};
