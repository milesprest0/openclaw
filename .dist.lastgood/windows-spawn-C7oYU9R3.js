...
function resolveBinEntry(packageName, binField) {
  ...
  if (packageName) {
    ...
    const normalizedPreferred =
      typeof preferred === "string" ? normalizeOptionalString(preferred) : void 0;
...
function resolveWindowsSpawnProgramCandidate(params) {
  ...
  if (ext === ".cmd" || ext === ".bat") {
    ...
    if (entrypoint) {
      if (normalizeLowercaseStringOrEmpty(path.extname(entrypoint)) === ".exe")
        return {
          command: entrypoint,
          leadingArgv: [],
...
//#endregion
export {
  resolveWindowsSpawnProgramCandidate as a,
  resolveWindowsSpawnProgram as i,
  materializeWindowsSpawnProgram as n,
  resolveWindowsExecutablePath as r,
  applyWindowsSpawnProgramPolicy as t,
};
