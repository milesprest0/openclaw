...
import { n as materializeGatewayAuthSecretRefs } from "./auth-config-utils-DiGXQjFc.js";
...
import { a as getQrRemoteCommandSecretTargetIds } from "./command-secret-targets-CkWxZ_Vf.js";
import { a as trimToUndefined } from "./credential-planner-CcCVplQO.js";
import { i as issueDeviceBootstrapToken } from "./device-bootstrap-CvlHJ2e9.js";
...
import {
  a as isCarrierGradeNatIpv4Address,
  c as isIpv6Address,
  f as isRfc1918Ipv4Address,
  m as parseCanonicalIpAddress,
  s as isIpv4Address,
} from "./ip-skS0UDUc.js";
...
import { t as renderQrTerminal } from "./qr-terminal-DNeh75Fq.js";
import { r as resolveRequiredConfiguredSecretRefInputString } from "./resolve-configured-secret-input-string-UIhD1m18.js";
...
import {
  i as pickMatchingExternalInterfaceAddress,
  o as safeNetworkInterfaces,
} from "./tailnet-mP5NqepX.js";
import {
...
function describeSecureMobilePairingFix(source) {
  return (
    ...
    " Fix: use a private LAN address, prefer gateway.tailscale.mode=serve, or set gateway.remote.url / plugins.entries.device-pair.config.publicUrl to a wss:// URL. ws:// is only valid for localhost, private LAN addresses, .local hosts, or the Android emulator."
  );
}
  ...
  if (
    (parsed.protocol === "https:"
      ...
      : parsed.protocol === "http:"
        ? "ws:"
        : parsed.protocol) !== "ws:" ||
    isMobilePairingCleartextAllowedHost(parsed.hostname)
  )
    return null;
...
function resolvePairingSetupAuthLabel(cfg, env) {
  ...
  const token =
    envToken || (tokenRef ? void 0 : normalizeSecretInputString(cfg.gateway?.auth?.token));
  const password =
    envPassword || (passwordRef ? void 0 : normalizeSecretInputString(cfg.gateway?.auth?.password));
  if (mode === "password") {
    if (!password)
    ...
    .action(async (opts) => {
      try {
        ...
        if (wantsRemote && !opts.url && !opts.publicUrl) {
          ...
          if (
            !Boolean(trimToUndefined(remoteUrl)) &&
          ...
          publicUrl:
            (typeof opts.url === "string" && opts.url.trim()
              ...
              : typeof opts.publicUrl === "string" && opts.publicUrl.trim()
                ? opts.publicUrl.trim()
                : void 0) ?? (wantsRemote ? void 0 : readDevicePairPublicUrlFromConfig(cfg)),
