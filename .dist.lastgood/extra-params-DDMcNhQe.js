...
import { _ as modelKey, g as legacyModelKey } from "./model-selection-shared-D9oQINZ2.js";
...
import {
  Y as prepareProviderExtraParams,
  Z as resolveProviderExtraParamsForTransport,
  tt as wrapProviderStreamFn,
} from "./provider-runtime-DueHRTen.js";
import { i as createGoogleThinkingPayloadWrapper } from "./provider-stream-shared-wxMuvsGy.js";
import {
  d as createOpenAIResponsesContextManagementWrapper,
  n as createOpenRouterSystemCacheWrapper,
  p as createOpenAIStringContentWrapper,
  s as createOpenAICompletionsToolsCompatWrapper,
  x as resolveCacheRetention,
  y as createMinimaxThinkingDisabledWrapper,
...
function resolveExtraParams(params) {
  ...
  const modelConfig =
    configuredModels?.[canonicalKey] ?? (legacyKey ? configuredModels?.[legacyKey] : void 0);
  ...
  const override =
    params.extraParamsOverride && Object.keys(params.extraParamsOverride).length > 0
      ? sanitizeExtraParamsRecord(
          Object.fromEntries(
            Object.entries(params.extraParamsOverride).filter(([, value]) => value !== void 0),
          ),
        )
      : void 0;
  const merged = {
...
function applyDefaultOpenAIGptRuntimeParams(params, merged) {
  ...
  if (!Object.hasOwn(merged, "parallel_tool_calls") && !Object.hasOwn(merged, "parallelToolCalls"))
...
function createStreamFnWithExtraParams(baseStreamFn, extraParams, provider, model) {
  ...
  const initialCacheRetention = resolveCacheRetention(
    extraParams,
    provider,
    typeof model?.api === "string" ? model.api : void 0,
    typeof model?.id === "string" ? model.id : void 0,
  );
  ...
  const wrappedStreamFn = (callModel, context, options) => {
    const cacheRetention = resolveCacheRetention(
      extraParams,
      provider,
      typeof callModel.api === "string" ? callModel.api : void 0,
      typeof callModel.id === "string" ? callModel.id : void 0,
    );
...
function sanitizeExtraBodyRecord(value) {
  return Object.fromEntries(
    Object.entries(sanitizeExtraParamsRecord(value) ?? {}).filter(([, entry]) => entry !== void 0),
  );
}
 ...
 */
function normalizeExplicitCacheRetention(value) {
...
function createAnthropicHistoryCacheBreakpointsWrapper(baseStreamFn, mode) {
  const underlying = baseStreamFn ?? streamSimple;
  return (model, context, options) =>
...
function applyPostPluginStreamWrappers(ctx) {
  ...
  const extraBody = resolveExtraBodyParam(
    resolveAliasedParamValue([ctx.effectiveExtraParams, ctx.override], "extra_body", "extraBody"),
  );
  if (extraBody)
...
function applyExtraParamsToAgent(
  ...
  const providerWrapperHandled =
    pluginWrappedStreamFn !== void 0 && pluginWrappedStreamFn !== providerStreamBase;
...
//#endregion
export {
  resolvePreparedExtraParams as a,
  resolveExtraParams as i,
  resolveAgentTransportOverride as n,
  resolveExplicitSettingsTransport as r,
  applyExtraParamsToAgent as t,
};
