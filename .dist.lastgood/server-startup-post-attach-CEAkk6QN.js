...
import { t as GATEWAY_EVENT_UPDATE_AVAILABLE } from "./events-DmGz-TZt.js";
import { i as getPluginModuleLoaderStats } from "./plugin-module-loader-cache-7MZ9xfuO.js";
...
function scheduleGatewayMemoryBackend(params) {
  ...
  const start = () => {
    import("./server-startup-memory-Cgsz4v6-.js")
...
async function refreshLatestUpdateRestartSentinelIfPresent() {
  ...
  return await (
    await import("./server-restart-sentinel-Cw4Mx9Jq.js")
...
async function hasGatewayStartupInternalHookListeners() {
  const { hasInternalHookListeners } = await import("./internal-hooks-D7RXflgE.js");
  return hasInternalHookListeners("gateway", "startup");
}
async function waitForAcpRuntimeBackendReady(params) {
  const { getAcpRuntimeBackend } = await import("./registry-0bhpOH70.js");
...
async function prewarmConfiguredPrimaryModel(params) {
  const { resolveAgentModelPrimaryValue } = await import("./model-input-BsYa1E13.js");
  const explicitPrimary = resolveAgentModelPrimaryValue(params.cfg.agents?.defaults?.model)?.trim();
  if (!explicitPrimary) return;
  const { normalizeProviderId } = await import("./provider-id-DDtEp0QB.js");
  if (
  ...
  const [
    ...
    import("./agent-scope-Bm8ZjMdO.js"),
    import("./defaults-C94FBkKL.js"),
    import("./model-selection-BlSYWnGK.js"),
    import("./runtime-Ba-zXjfm.js"),
  ]);
  ...
  const { ensureOpenClawModelsJson } = await import("./models-config-C6YT6lty.js");
...
async function prewarmConfiguredPrimaryModelWithTimeout(
  params,
  prewarm = prewarmConfiguredPrimaryModel,
) {
  let settled = false;
  ...
  const timeout = setTimeout$1(params.timeoutMs ?? PRIMARY_MODEL_PREWARM_TIMEOUT_MS, void 0, {
    ref: false,
  }).then(() => {
    if (!settled)
      params.log.warn(
...
function schedulePrimaryModelPrewarm(params, prewarm = prewarmConfiguredPrimaryModel) {
  ...
  measureStartup(params.startupTrace, "sidecars.model-prewarm", () =>
...
async function startGatewaySidecars(params) {
  await measureStartup(params.startupTrace, "sidecars.gmail-watch", async () => {
    if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) {
      const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-Bi5jo9Ue.js");
  ...
  await measureStartup(params.startupTrace, "sidecars.gmail-model", async () => {
    if (params.cfg.hooks?.gmail?.model) {
      const [
        ...
        import("./defaults-C94FBkKL.js"),
        import("./model-catalog-iNiq3Mxw.js"),
        import("./model-selection-BlSYWnGK.js"),
      ]);
    ...
    try {
      if (internalHooksConfigured) {
        const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([
          import("./internal-hooks-D7RXflgE.js"),
          import("./loader-CrsrlkUK.js"),
        ]);
  ...
  if (internalHooksConfigured || (await hasGatewayStartupInternalHookListeners()))
    setTimeout(() => {
      import("./internal-hooks-D7RXflgE.js").then(
  ...
  await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
    try {
      const { startPluginServices } = await import("./services-FX2MH79V.js");
      ...
      try {
        const [{ resolveStateDir }, { resolveAgentSessionDirs }, { cleanStaleLockFiles }] =
          await Promise.all([
            ...
            import("./session-dirs-BjAd1rRK.js"),
            import("./session-write-lock-CvduYymU.js"),
          ]);
        ...
        for (const sessionsDir of sessionDirs) {
          ...
          if (result.cleaned.length > 0) {
            const { markRestartAbortedMainSessionsFromLocks } =
              await import("./main-session-restart-recovery-DIjFJ8O_.js");
  ...
  schedulePostReadySidecarTask({
    ...
    run: async () => {
      ...
      setTimeout(() => {
        import("./server-restart-sentinel-Cw4Mx9Jq.js")
  ...
  schedulePostReadySidecarTask({
    ...
    log: params.log,
    run: async () => {
      const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-DKtulXIp.js");
...
const defaultGatewayPostAttachRuntimeDeps = {
  getGlobalHookRunner: async () =>
    (await import("./hook-runner-global-D0QzDMlK.js")).getGlobalHookRunner(),
  logGatewayStartup: async (params) =>
    (await import("./server-startup-log-OCmlp151.js")).logGatewayStartup(params),
  refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
  scheduleGatewayUpdateCheck: async (...args) =>
    (await import("./update-startup-DA3-Ykup.js")).scheduleGatewayUpdateCheck(...args),
  startGatewaySidecars,
  startGatewayTailscaleExposure: async (...args) =>
    (await import("./server-tailscale-CcNeTVb3.js")).startGatewayTailscaleExposure(...args),
};
async function startGatewayPostAttachRuntime(
  params,
  runtimeDeps = defaultGatewayPostAttachRuntimeDeps,
) {
