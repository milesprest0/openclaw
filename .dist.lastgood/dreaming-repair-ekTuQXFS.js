...
async function listSessionCorpusFiles(sessionCorpusDir) {
  return (await fs.readdir(sessionCorpusDir, { withFileTypes: true }))
...
async function auditDreamingArtifacts(params) {
  ...
  try {
    ...
    for (const corpusFile of corpusFiles) {
      const suspiciousLines = (await fs.readFile(corpusFile, "utf-8"))
        .split(/\r?\n/)
        .map((line) => line.trim())
...
async function repairDreamingArtifacts(params) {
  ...
  const ensureArchiveDir = () => {
    archiveDir ??= path.join(
      ...
      buildArchiveTimestamp(params.now ?? /* @__PURE__ */ new Date()),
    );
    return archiveDir;
  };
