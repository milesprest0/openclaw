...
function getColorForConsole() {
  ...
  return process.stdout.isTTY || process.stderr.isTTY || isRichConsoleEnv()
    ? new Chalk({ level: 1 })
    : new Chalk({ level: 0 });
}
...
function stripRedundantSubsystemPrefixForConsole(message, displaySubsystem) {
  ...
  if (message.startsWith("[")) {
    ...
    if (closeIdx > 1) {
      if (
        normalizeLowercaseStringOrEmpty(message.slice(1, closeIdx)) ===
  ...
  if (
    normalizeLowercaseStringOrEmpty(message.slice(0, displaySubsystem.length)) !==
    normalizeLowercaseStringOrEmpty(displaySubsystem)
  )
    return message;
...
function formatConsoleLine(opts) {
  ...
  const displayMessage = stripRedundantSubsystemPrefixForConsole(
    redactSensitiveText(opts.message),
    displaySubsystem,
  );
  return `${[
    (() => {
    ...
    .filter(Boolean)
    .join(" ")} ${levelColor(displayMessage)}`;
}
...
//#endregion
export {
  stripRedundantSubsystemPrefixForConsole as i,
  createSubsystemRuntime as n,
  runtimeForLogger as r,
  createSubsystemLogger as t,
};
