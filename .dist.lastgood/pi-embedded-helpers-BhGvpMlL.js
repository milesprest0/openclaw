...
import { g as isReasoningConstraintErrorMessage } from "./errors-D66M8xLa.js";
import "./sanitize-user-facing-text-Cgbm6W76.js";
import {
...
import { f as normalizeThinkLevel } from "./thinking-C9_Z62px.js";
import {
...
import { t as sanitizeContentBlocksImages } from "./tool-images-D8ZmIpdW.js";
...
function trimBootstrapContent(content, fileName, maxChars) {
  ...
  const compactMarkerTemplate = (headChars, tailChars) =>
    `[…truncated ${headChars}+${tailChars}/${trimmed.length}]`;
  const separatorCharsFor = (headCount, tailCount, markerContent) =>
  ...
  const resolveMarkerTemplate = () => {
    ...
    return maxChars - fullMarker.length - separatorCharsFor(1, 1, fullMarker) >=
      MIN_BOOTSTRAP_TRIMMED_CONTENT_CHARS
      ? markerTemplate
  ...
  for (const msg of sanitizedIds) {
    ...
    if (role === "toolResult") {
      ...
      const nextContent = await sanitizeContentBlocksImages(
        Array.isArray(toolMsg.content) ? toolMsg.content : [],
        label,
        imageSanitization,
      );
      out.push({
    ...
    if (role === "assistant") {
      ...
      if (assistantMsg.stopReason === "error") {
        ...
        if (Array.isArray(content)) {
          const finalContent = dropEmptyTextBlocks(
            await sanitizeContentBlocksImages(content, label, imageSanitization),
          );
      ...
      if (Array.isArray(content)) {
        ...
        if (!allowNonImageSanitization) {
          const nextContent = await sanitizeContentBlocksImages(
            dropEmptyTextBlocks(strippedContent),
            label,
            imageSanitization,
          );
...
//#endregion
export {
  sanitizeGoogleTurnOrdering as _,
  isMessagingToolDuplicateNormalized as a,
  downgradeOpenAIFunctionCallReasoningPairs as c,
  isGoogleModelApi as d,
  buildBootstrapContextFiles as f,
  resolveBootstrapTotalMaxChars as g,
  resolveBootstrapPromptTruncationWarningMode as h,
  isMessagingToolDuplicate as i,
  downgradeOpenAIReasoningBlocks as l,
  resolveBootstrapMaxChars as m,
  validateGeminiTurns as n,
  normalizeTextForComparison as o,
  ensureSessionHeader as p,
  pickFallbackThinkingLevel as r,
  sanitizeSessionMessagesImages as s,
  validateAnthropicTurns as t,
  isGemma4ModelRequiringReasoningStrip as u,
};
