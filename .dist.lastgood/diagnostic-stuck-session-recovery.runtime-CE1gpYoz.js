import { f as resetCommandLane, o as getCommandLaneSnapshot } from "./command-queue-BtommWHg.js";
import { t as diagnosticLogger } from "./diagnostic-runtime-Bcy49DVv.js";
import { i as isDiagnosticSessionStateCurrent } from "./diagnostic-session-state-BAjgBZHS.js";
import {
  b as resolveCronSessionDiagnosticContext,
  v as formatRecoveryOutcome,
  y as formatStoppedCronSessionDiagnosticFields,
...
import {
  ...
  s as isEmbeddedPiRunHandleActive,
  t as abortAndDrainEmbeddedPiRun,
  u as resolveActiveEmbeddedRunHandleSessionId,
...
async function recoverStuckDiagnosticSession(params) {
  ...
  try {
    ...
    const fallbackActiveSessionId =
      params.sessionId && isEmbeddedPiRunHandleActive(params.sessionId) ? params.sessionId : void 0;
    ...
    if (aborted || released > 0) {
      ...
      diagnosticLogger.warn(
        `stuck session recovery: sessionId=${params.sessionId ?? activeSessionId ?? "unknown"} sessionKey=${params.sessionKey ?? "unknown"} age=${Math.round(params.ageMs / 1e3)}s action=${action} aborted=${aborted} drained=${drained} released=${released}${stoppedFields ? ` ${stoppedFields}` : ""}`,
      );
    ...
    diagnosticLogger.warn(
      `stuck session recovery failed: sessionId=${params.sessionId ?? "unknown"} sessionKey=${params.sessionKey ?? "unknown"} err=${String(err)}`,
    );
    return outcome;
  } finally {
