...
import { u as runFfmpeg } from "./runner.entries-BWDlqt3w.js";
...
const VALID_OUTPUT_FORMATS = ["mp3", "opus", "wav"];
...
async function runCli(params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      clearTimeout(timer);
      if (timedOut)
        return reject(/* @__PURE__ */ new Error(`CLI TTS timed out after ${params.timeoutMs}ms`));
      if (code !== 0) {
      ...
      if (audioFile) {
        if (!existsSync(audioFile))
          return reject(
            /* @__PURE__ */ new Error(`CLI TTS: output file not found at ${audioFile}`),
          );
        const format = detectFormat(audioFile);
        if (!format)
          return reject(/* @__PURE__ */ new Error(`CLI TTS: unknown format for ${audioFile}`));
        return resolve({
