...
import { n as getMemoryEmbeddingProvider } from "./memory-embedding-providers-qSONi9MJ.js";
import "./agent-scope-9AmhTwki.js";
import {
  a as isMemoryMultimodalEnabled,
  o as normalizeMemoryMultimodalSettings,
} from "./multimodal-91P0oi7L.js";
...
import { i as clampNumber, p as resolveUserPath, r as clampInt } from "./utils-BGRcpLKt.js";
...
function mergeConfig(cfg, defaults, overrides, agentId) {
  ...
  const primaryAdapter =
    provider === "auto" ? void 0 : getConfiguredMemoryEmbeddingProvider(provider, cfg);
  const defaultRemote = defaults?.remote;
  const overrideRemote = overrides?.remote;
  const fallback = overrides?.fallback ?? defaults?.fallback ?? "none";
  const fallbackModel =
    overrides?.fallbackModel?.trim() || defaults?.fallbackModel?.trim() || void 0;
  const fallbackAdapter =
    fallback && fallback !== "none" ? getConfiguredMemoryEmbeddingProvider(fallback, cfg) : void 0;
  const includeRemote =
    Boolean(
  ...
  const batch = {
    ...
    pollIntervalMs:
      overrideRemote?.batch?.pollIntervalMs ?? defaultRemote?.batch?.pollIntervalMs ?? 2e3,
    timeoutMinutes:
  ...
  const queryInputType =
    overrides?.queryInputType?.trim() || defaults?.queryInputType?.trim() || void 0;
  const documentInputType =
    overrides?.documentInputType?.trim() || defaults?.documentInputType?.trim() || void 0;
