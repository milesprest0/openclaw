...
import { a as refreshChutesTokens } from "./chutes-oauth-96yUmrYJ.js";
...
import { t as assertNoOAuthSecretRefPolicyViolations } from "./policy-DxNeVGSY.js";
...
import { i as resolveSecretRefString } from "./resolve-BZt0Bv7Z.js";
import {
  f as resolveOAuthRefreshLockPath,
  l as resolveAuthStorePath,
} from "./source-check-DUt28S7k.js";
import {
  C as shouldReplaceStoredOAuthCredential,
  D as resolveTokenExpiryState,
  F as OAUTH_REFRESH_LOCK_OPTIONS,
  I as log,
  P as OAUTH_REFRESH_CALL_TIMEOUT_MS,
  S as shouldBootstrapFromExternalCliCredential,
  _ as readManagedExternalCliCredential,
  b as isSafeToAdoptBootstrapOAuthIdentity,
  c as loadAuthProfileStoreForSecretsRuntime,
  d as resolvePersistedAuthProfileOwnerAgentDir,
  f as saveAuthProfileStore,
  g as ensureAuthStoreFile,
  i as ensureAuthProfileStoreWithoutExternalProfiles,
  j as AUTH_STORE_LOCK_OPTIONS,
  l as loadAuthProfileStoreWithoutExternalProfiles,
  p as updateAuthProfileStoreWithLock,
  v as areOAuthCredentialsEquivalent,
  x as isSafeToAdoptMainStoreOAuthIdentity,
  y as hasUsableOAuthCredential,
 ...
 */
const DEPRECATED_PROVIDER_MIGRATION_HINTS = {
  "qwen-portal":
...
var OAuthManagerRefreshError = class extends Error {
  ...
  constructor(params) {
    const structuredCause =
      typeof params.cause === "object" && params.cause !== null ? params.cause : void 0;
...
async function loadFreshStoredOAuthCredential(params) {
  const reloaded = loadAuthProfileStoreWithoutExternalProfiles(params.agentDir).profiles[
    params.profileId
  ];
  if (
    reloaded?.type !== "oauth" ||
...
function createOAuthManager(adapter) {
  function adoptNewerMainOAuthCredential(params) {
    ...
    try {
      const mainCred = ensureAuthProfileStoreWithoutExternalProfiles(void 0, {
    ...
    try {
      return await new Promise((resolve, reject) => {
        timeoutHandle = setTimeout(() => {
          reject(
            /* @__PURE__ */ new Error(
              `OAuth refresh call "${label}" exceeded hard timeout (${timeoutMs}ms)`,
            ),
          );
        }, timeoutMs);
      ...
      return await withFileLock(globalRefreshLockPath, OAUTH_REFRESH_LOCK_OPTIONS, async () =>
        withFileLock(authPath, AUTH_STORE_LOCK_OPTIONS, async () => {
          ...
          if (params.agentDir)
            try {
              const mainCred =
                loadAuthProfileStoreWithoutExternalProfiles(void 0).profiles[params.profileId];
              if (
                mainCred?.type === "oauth" &&
...
async function refreshOAuthCredential(credential) {
  ...
  return (
    (await getOAuthApiKey(oauthProvider, { [credential.provider]: credential }))?.newCredentials ??
    null
  );
}
...
//#endregion
export {
  resolveEffectiveOAuthCredential as n,
  formatAuthDoctorHint as r,
  resolveApiKeyForProfile as t,
};
