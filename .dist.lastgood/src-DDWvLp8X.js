...
import {
  ...
  o as Blob,
  r as formDataToBlob,
  s as init_fetch_blob,
  t as init_from,
} from "./from-B3fIfXOT.js";
...
async function consumeBody(data) {
  ...
  try {
    ...
    throw error instanceof FetchBaseError
      ? error
      : new FetchError(
...
var init_body = __esmMin(() => {
  ...
  Body = class {
    constructor(body, { size = 0 } = {}) {
      ...
      } else if (types.isAnyArrayBuffer(body)) body = Buffer$1.from(body);
    ...
    async formData() {
      ...
      const { toFormData } = await import("./multipart-parser-sn0ioKNA.js");
...
var init_headers = __esmMin(() => {
  validateHeaderName =
    typeof http.validateHeaderName === "function"
      ...
      : (name) => {
          if (!/^[\^`\-\w!#$%&'*+.|~]+$/.test(name)) {
            const error = /* @__PURE__ */ new TypeError(
...
var init_request = __esmMin(() => {
  ...
  Request = class Request extends Body {
    constructor(input, init = {}) {
      ...
      this.follow =
        init.follow === void 0 ? (input.follow === void 0 ? 20 : input.follow) : init.follow;
      this.compress =
