...
function parseSimpleSlackOptions(raw) {
  ...
  if (
    new Set(entries.map((entry) => normalizeLowercaseStringOrEmpty(entry))).size !== entries.length
  )
    return null;
...
function isSlackInteractiveRepliesEnabled(params) {
  return resolveInteractiveRepliesFromCapabilities(
    resolveSlackAccount({
      cfg: params.cfg,
...
//#endregion
export {
  isSlackInteractiveRepliesEnabled as n,
  parseSlackOptionsLine as r,
  compileSlackInteractiveReplies as t,
};
