...
import { i as normalizeCronRunDiagnostics } from "./run-diagnostics-prIxGL_8.js";
...
async function pruneIfNeeded(filePath, opts) {
  ...
  const lines = (await fs$1.readFile(filePath, "utf-8").catch(() => ""))
    .split("\n")
    .map((l) => l.trim())
    .filter(Boolean);
...
async function appendCronRunLog(filePath, entry, opts) {
  const resolved = path.resolve(filePath);
  const next = (writesByPath.get(resolved) ?? Promise.resolve())
    .catch(() => void 0)
    .then(async () => {
...
function normalizeRunStatuses(opts) {
  if (Array.isArray(opts?.statuses) && opts.statuses.length > 0) {
    const filtered = opts.statuses.filter(
      (status) => status === "ok" || status === "error" || status === "skipped",
    );
      ...
      const entry = {
        ...
        usage: usage
          ? {
              ...
              cache_read_tokens:
                typeof usage.cache_read_tokens === "number" ? usage.cache_read_tokens : void 0,
              cache_write_tokens:
                typeof usage.cache_write_tokens === "number" ? usage.cache_write_tokens : void 0,
            }
          : void 0,
      };
...
async function readCronRunLogEntriesPageAll(opts) {
  ...
  const jsonlFiles = (await runsRoot.list(".", { withFileTypes: true }).catch(() => []))
  ...
  const filtered = filterRunLogEntries(
    (
      await Promise.all(
...
//#endregion
export {
  resolveCronRunLogPath as a,
  readCronRunLogEntriesSync as i,
  readCronRunLogEntriesPage as n,
  resolveCronRunLogPruneOptions as o,
  readCronRunLogEntriesPageAll as r,
  appendCronRunLog as t,
};
