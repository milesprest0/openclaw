...
import { n as readClaudeCliFallbackSeed } from "./cli-session-history-Dv56DP5l.js";
import {
...
async function claudeCliSessionTranscriptHasContent(params) {
  ...
  for (const entry of projectEntries) {
    ...
    if (
      await jsonlFileHasAssistantMessage(path.join(projectsDir, entry.name, `${sessionId}.jsonl`))
    )
      return true;
  }
  return false;
...
function createAcpVisibleTextAccumulator() {
  ...
  const resolveNextCandidate = (base, chunk) => {
    ...
    if (isSilentReplyText(base, "NO_REPLY") && !chunk.startsWith(base) && startsWithWordChar(chunk))
      return chunk;
  ...
  return {
    consume(chunk) {
      ...
      if (!visibleText) {
        ...
        if (
          isSilentReplyText(trimmedLeadCandidate, "NO_REPLY") ||
          isSilentReplyPrefixText(trimmedLeadCandidate, "NO_REPLY")
        ) {
...
//#endregion
export {
  sessionFileHasContent as a,
  resolveFallbackRetryPrompt as i,
  claudeCliSessionTranscriptHasContent as n,
  createAcpVisibleTextAccumulator as r,
  buildClaudeCliFallbackContextPrelude as t,
};
