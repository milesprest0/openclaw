...
const FEISHU_COMMENT_FILE_TYPES = ["doc", "docx", "file", "sheet", "slides"];
...
function formatFeishuApiError(error, options = {}) {
  ...
  const feishuLogId =
    ...
    (options.includeNestedErrorLogId
      ? readString(isRecord(responseData?.error) ? responseData.error.log_id : void 0)
      : void 0);
...
function readMentionUserId(element) {
  ...
  return (
    normalizeString((isRecord(element.person) ? element.person : void 0)?.user_id) ||
...
function parseCommentContentElements(params) {
  ...
  for (const rawElement of elements) {
    ...
    const text =
      (type === "text_run" ? readElementTextPreservingWhitespace(element) : void 0) ||
      (type === "text" ? readElementTextPreservingWhitespace(element) : void 0) ||
      (type === "docs_link" || type === "link" ? readDocsLinkUrl(element) : void 0) ||
...
function buildResolvedFeishuAccount(params) {
  ...
  const accountId = hasExplicitAccountId
    ...
    : (defaultSelection?.accountId ?? "default");
...
export {
  readString as _,
  resolveDefaultFeishuAccountId as a,
  normalizeCommentFileType as b,
  resolveFeishuCredentials as c,
  encodeQuery as d,
  extractReplyText as f,
  parseCommentContentElements as g,
  normalizeString as h,
  listFeishuAccountIds as i,
  resolveFeishuRuntimeAccount as l,
  isRecord as m,
  inspectFeishuCredentials as n,
  resolveDefaultFeishuAccountSelection as o,
  formatFeishuApiError as p,
  listEnabledFeishuAccounts as r,
  resolveFeishuAccount as s,
  FeishuSecretRefUnavailableError as t,
  createFeishuApiError as u,
  requestFeishuApi as v,
  parseFeishuCommentTarget as x,
  buildFeishuCommentTarget as y,
};
