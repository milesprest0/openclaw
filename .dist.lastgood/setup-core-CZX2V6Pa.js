...
function buildTlonAccountFields(input) {
  return {
    ...
    ...(typeof input.dangerouslyAllowPrivateNetwork === "boolean"
      ? { network: { dangerouslyAllowPrivateNetwork: input.dangerouslyAllowPrivateNetwork } }
      : {}),
...
function resolveTlonAccount(cfg, accountId) {
  ...
  if (!resolveTlonChannelConfig(cfg))
    return {
...
async function resolveTlonSetupStatusLines(cfg, accountId) {
  ...
  return [
    `${accountId && accountId !== "default" ? `Tlon (${accountId})` : "Tlon"}: ${configured ? "configured" : "needs setup"}`,
  ];
}
...
//#endregion
export {
  tlonSetupAdapter as a,
  validateUrbitBaseUrl as c,
  formatTargetHint as d,
  normalizeShip as f,
  resolveTlonOutboundTarget as h,
  resolveTlonSetupStatusLines as i,
  listTlonAccountIds as l,
  parseTlonTarget as m,
  createTlonSetupWizardBase as n,
  isBlockedUrbitHostname as o,
  parseChannelNest as p,
  resolveTlonSetupConfigured as r,
  normalizeUrbitHostname as s,
  applyTlonSetupConfig as t,
  resolveTlonAccount as u,
};
