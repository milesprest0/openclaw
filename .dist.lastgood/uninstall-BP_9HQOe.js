...
import {
  ...
  f as resolvePluginInstallDir,
  u as resolveDefaultPluginGitDir,
...
import { n as relinkOpenClawPeerDependenciesInManagedNpmRoot } from "./plugin-peer-link-lg5CcSDT.js";
import { n as createSafeNpmInstallEnv } from "./safe-package-install-BQGUJgWF.js";
...
function resolveRecordedManagedInstallPath(params) {
  ...
  try {
    return path.resolve(resolvePluginInstallDir(params.pluginId, recordedExtensionsDir)) ===
      resolvedInstallPath
      ? params.installPath
      : null;
  } catch {
...
async function applyPluginUninstallDirectoryRemoval(removal) {
  ...
  if (removal.cleanup?.kind === "npm" && npmCleanupManifestExists) {
    ...
    if (uninstall.code !== 0)
      warnings.push(
        `Failed to prune npm dependencies for plugin package ${removal.cleanup.packageName}: ${uninstall.stderr.trim() || uninstall.stdout.trim() || `npm exited with code ${uninstall.code}`}`,
      );
    try {
...
//#endregion
export {
  formatUninstallSlotResetPreview as a,
  resolveUninstallChannelConfigKeys as c,
  formatUninstallActionLabels as i,
  resolveUninstallDirectoryTarget as l,
  applyPluginUninstallDirectoryRemoval as n,
  planPluginUninstall as o,
  createEmptyUninstallActions as r,
  removePluginFromConfig as s,
  UNINSTALL_ACTION_LABELS as t,
};
