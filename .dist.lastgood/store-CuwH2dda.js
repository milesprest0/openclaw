...
function resolveSchedulePayload(job) {
  if (job.schedule && typeof job.schedule === "object" && !Array.isArray(job.schedule))
    return schedulePayloadFromRecord(job.schedule);
...
function cronSchedulingInputsEqual(previous, next) {
  ...
  return (
    previousIdentity !== void 0 && nextIdentity !== void 0 && previousIdentity === nextIdentity
  );
}
//#endregion
...
async function saveCronStore(storePath, store, opts) {
  ...
  if (!stateOnly && (configNeedsWrite || migrating)) {
    if (!(opts?.skipBackup === true || !configChanged))
      try {
...
//#endregion
export {
  cronSchedulingInputsEqual as a,
  saveCronStore as i,
  loadCronStoreSync as n,
  resolveCronStorePath as r,
  loadCronStore as t,
};
