...
async function loadSdk() {
  ...
  try {
    ...
    throw new Error(
      "No API key found for provider bedrock: @aws-sdk/client-bedrock-runtime is not installed. Install it with: npm install @aws-sdk/client-bedrock-runtime",
    );
  }
}
...
async function createBedrockEmbeddingProvider(options) {
  ...
  const embedSingle = async (text) => {
    return sanitizeAndNormalizeEmbedding(
      parseSingle(family, await invoke(buildBody(family, text, client.dimensions))),
    );
  };
  const embedCohere = async (texts, inputType) => {
    return parseCohereBatch(
      family,
      await invoke(buildCohereBody(family, texts, inputType, client.dimensions)),
    ).map((e) => sanitizeAndNormalizeEmbedding(e));
  };
...
async function hasAwsCredentials(
  env = process.env,
  loadCredentialProvider = loadCredentialProviderSdk,
) {
...
//#endregion
export {
  createBedrockEmbeddingProvider as n,
  hasAwsCredentials as r,
  DEFAULT_BEDROCK_EMBEDDING_MODEL as t,
};
