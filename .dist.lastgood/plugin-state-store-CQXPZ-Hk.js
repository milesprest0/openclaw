...
const PLUGIN_STATE_SIDECAR_SUFFIXES = ["", "-shm", "-wal"];
...
function enforcePostRegisterLimits(params) {
  const namespaceCount = countRow(
    params.store.statements.countLiveNamespace.get(params.pluginId, params.namespace, params.now),
  );
  ...
  if (
    countRow(params.store.statements.countLivePlugin.get(params.pluginId, params.now)) >
...
function pluginStateRegisterIfAbsent(params) {
  try {
    return runWriteTransaction("register", (store) => {
      ...
      store.statements.pruneExpiredNamespace.run(params.pluginId, params.namespace, now);
      if (
        store.statements.insertEntryIfAbsent.run({
          plugin_id: params.pluginId,
          namespace: params.namespace,
          entry_key: params.key,
          value_json: params.valueJson,
...
function pluginStateEntries(params) {
  try {
    ...
    return statements.selectEntries
      .all(params.pluginId, params.namespace, Date.now())
...
function assertValueSize(json) {
  if (textEncoder.encode(json).byteLength > 65536)
...
//#endregion
export {
  sweepExpiredPluginStateEntries as i,
  closePluginStateSqliteStore as n,
  isPluginStateDatabaseOpen as r,
  createPluginStateKeyedStore as t,
};
