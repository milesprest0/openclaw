...
import { n as getActiveManagedProxyUrl } from "./active-proxy-state-BS8ghEz6.js";
  ...
  constructor(params, proxy, resolve, reject) {
    ...
    this.onTargetSecureConnect = () => {
      ...
      if (socket.alpnProtocol !== "h2") {
        ...
        this.fail(
          /* @__PURE__ */ new Error(`APNs TLS tunnel negotiated ${negotiated} instead of h2`),
        );
        return;
      }
      this.succeed(socket);
  ...
  startTimeout() {
    ...
    if (timeoutMs && Number.isFinite(timeoutMs) && timeoutMs > 0)
      this.timeout = setTimeout(() => {
        this.fail(
          /* @__PURE__ */ new Error(`Proxy CONNECT timed out after ${Math.trunc(timeoutMs)}ms`),
        );
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        fail(
          /* @__PURE__ */ new Error(
            `APNs reachability probe timed out after ${Math.trunc(params.timeoutMs)}ms`,
          ),
        );
      ...
      request.once("end", () => {
        ...
        if (status === void 0 || !Number.isFinite(status)) {
          reject(
            /* @__PURE__ */ new Error("APNs reachability probe ended without an HTTP/2 status"),
          );
          return;
        }
        resolve({
...
//#endregion
export {
  connectApnsHttp2Session as n,
  probeApnsHttp2ReachabilityViaProxy as r,
  APNS_HTTP2_CANCEL_CODE as t,
};
