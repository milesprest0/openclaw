...
import { o as getTailnetHostname } from "./tailscale-DxVPpgKm.js";
...
async function maybeAddTailnetOriginToControlUiAllowedOrigins(params) {
  ...
  const tsOrigin = await getTailnetHostname(void 0, params.tailscaleBin ?? void 0)
    .then((host) => buildTailnetHttpsOrigin(host))
    .catch(() => null);
  if (!tsOrigin) return params.config;
  const updatedOrigins = appendAllowedOrigin(
    params.config.gateway?.controlUi?.allowedOrigins ?? [],
    tsOrigin,
  );
  return {
    ...params.config,
...
//#endregion
export {
  maybeAddTailnetOriginToControlUiAllowedOrigins as a,
  TAILSCALE_MISSING_BIN_NOTE_LINES as i,
  TAILSCALE_DOCS_LINES as n,
  TAILSCALE_EXPOSURE_OPTIONS as r,
  validateIPv4AddressInput as t,
};
