...
function resolvePairingCredentialsDir(env = process.env) {
  return resolveOAuthDir(
    env,
    resolveStateDir(env, () => resolveRequiredHomeDir(env, os.homedir)),
  );
}
...
function clearAllowFromFileReadCacheForNamespace(cacheNamespace) {
  for (const key of allowFromReadCache.keys())
    if (key.startsWith(`${cacheNamespace}\u0000`)) allowFromReadCache.delete(key);
}
//#endregion
export {
  resolveAllowFromAccountId as a,
  safeChannelKey as c,
  readAllowFromFileWithExists as i,
  setAllowFromFileReadCache as l,
  dedupePreserveOrder as n,
  resolveAllowFromFilePath as o,
  readAllowFromFileSyncWithExists as r,
  resolvePairingCredentialsDir as s,
  clearAllowFromFileReadCacheForNamespace as t,
  shouldIncludeLegacyAllowFromEntries as u,
};
