...
import { n as resolveBundledPluginsDir } from "./bundled-dir-CSFHC_7h.js";
...
import {
  n as resolveBundledPluginGeneratedPath,
  t as unwrapDefaultModuleExport,
} from "./module-export-CpPaSiil.js";
import { n as resolveOpenClawPackageRootSync } from "./openclaw-root-DDaGBMF_.js";
import { d as safeRealpathSync, i as isPathInside } from "./path-1liOXr_N.js";
import {
  a as isJavaScriptModulePath,
  n as getCachedPluginModuleLoader,
} from "./plugin-module-loader-cache-7MZ9xfuO.js";
import { n as loadPluginManifestRegistryForPluginRegistry } from "./plugin-registry-CWiNK0Xv.js";
 ...
 */
function moduleResolvesUnderBoundaryRoot(root, modulePath) {
...
function resolveBundledChannelBoundaryRoot(params) {
  ...
  if (moduleResolvesUnderBoundaryRoot(distRoot, params.modulePath)) return distRoot;
...
function hasBundledChannelPackageSetupFeature(id, feature) {
  return (
    resolveBundledChannelMetadata(id, resolveBundledChannelRootScope())?.packageManifest
      ?.setupFeatures?.[feature] === true
  );
}
...
function getBundledChannelAccountInspectorForRoot(id, rootScope, loadContext) {
  if (loadContext.lazyAccountInspectorsById.has(id))
    return loadContext.lazyAccountInspectorsById.get(id) ?? void 0;
  const entry = getLazyGeneratedBundledChannelEntryForRoot(id, rootScope, loadContext)?.entry;
...
function getBundledChannelSetupPluginForRoot(id, rootScope, loadContext) {
  if (loadContext.lazySetupPluginsById.has(id))
    return loadContext.lazySetupPluginsById.get(id) ?? void 0;
...
function getBundledChannelSetupSecretsForRoot(id, rootScope, loadContext) {
  if (loadContext.lazySetupSecretsById.has(id))
    return loadContext.lazySetupSecretsById.get(id) ?? void 0;
  const entry = getLazyGeneratedBundledChannelSetupEntryForRoot(id, rootScope, loadContext);
  if (!entry) return;
  try {
    const secrets =
...
//#endregion
export {
  resolveManifestOwnerBasePolicyBlock as S,
  resolveExistingPluginModulePath as _,
  getBundledChannelSetupSecrets as a,
  isBundledManifestOwner as b,
  listBundledChannelLegacyStateMigrationDetectors as c,
  listBundledChannelSetupPlugins as d,
  moduleResolvesUnderBoundaryRoot as f,
  loadChannelPluginModule as g,
  resolveBundledChannelRootScope as h,
  getBundledChannelSetupPlugin as i,
  listBundledChannelPluginIds as l,
  normalizeChannelMeta as m,
  getBundledChannelPlugin as n,
  hasBundledChannelPackageSetupFeature as o,
  setBundledChannelRuntime as p,
  getBundledChannelSecrets as r,
  listBundledChannelLegacySessionSurfaces as s,
  getBundledChannelAccountInspector as t,
  listBundledChannelPlugins as u,
  hasExplicitManifestOwnerTrust as v,
  passesManifestOwnerBasePolicy as x,
  isActivatedManifestOwner as y,
};
