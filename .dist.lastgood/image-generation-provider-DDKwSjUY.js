...
import { u as sanitizeConfiguredModelProviderRequest } from "./provider-request-config-B-kAAqy2.js";
...
import { u as resolveClosestSize } from "./runtime-shared-CdcIZV6B.js";
...
import { r as resolveConfiguredOpenAIBaseUrl } from "./shared-DsZ1ZHSm.js";
...
const OPENAI_LEGACY_IMAGE_SIZES = ["1024x1024", "1536x1024", "1024x1536"];
...
const OPENAI_BACKGROUNDS = ["transparent", "opaque", "auto"];
...
function buildAzureImageUrl(rawBaseUrl, model, action) {
  return `${rawBaseUrl
    ...
    .replace(
      /\/v1$/,
      "",
    )}/openai/deployments/${model}/images/${action}?api-version=${resolveAzureApiVersion()}`;
}
...
function appendOpenAIImageOptions(target, req) {
  ...
  const entries = {
    ...
    ...(openai?.outputCompression !== void 0
...
function inferImageUploadFileName(params) {
  ...
  const ext =
    extensionForMime(params.mimeType?.trim().toLowerCase() || DEFAULT_OUTPUT_MIME)?.slice(1) ??
    "png";
...
function logCodexImageAuthSelected(params) {
  ...
  log.info(
    `image auth selected: provider=openai-codex mode=${sanitizeLogValue(params.authMode)} transport=codex-responses requestedModel=${sanitizeLogValue(model)} responsesModel=${DEFAULT_OPENAI_CODEX_IMAGE_RESPONSES_MODEL} timeoutMs=${params.timeoutMs}`,
  );
}
async function generateOpenAICodexImage(params) {
  ...
  return {
    images: images.map((image, index) =>
      Object.assign({}, image, { fileName: `image-${index + 1}.${output.extension}` }),
    ),
    model,
    metadata: {
  ...
  return createOpenAIImageGenerationProviderBase({
    ...
    async generateImage(req) {
      ...
      try {
        ...
        const output = resolveOutputMime(req.outputFormat);
        return {
          images: parseOpenAiCompatibleImageResponse(data, {
