...
import { r as resolveMatrixEnvAuthReadiness } from "./env-auth-CkkG1Yqg.js";
...
import {
  r as moveSingleMatrixAccountConfigToNamedAccount,
  t as resolveMatrixSetupDmAllowFrom,
} from "./setup-dm-policy-DRRR5zwB.js";
import { t as promptChannelAccessConfig } from "./setup-group-access-CBMujhxx.js";
import {
  O as promptAccountId,
...
function normalizeMatrixInviteAutoJoinTargets(entries) {
  return [
    ...new Set(
      entries.map((entry) => normalizeOptionalString(entry)).filter((entry) => Boolean(entry)),
    ),
  ];
}
function resolveMatrixOnboardingAccountId(cfg, accountId) {
  return normalizeAccountId(
    normalizeOptionalString(accountId) || resolveDefaultMatrixAccountId(cfg) || "default",
  );
}
...
async function promptMatrixAllowFrom(params) {
  ...
  while (true) {
    ...
    if (pending.length > 0) {
      const { resolveMatrixTargets } = await import("./resolve-targets-BnGx-wq4.js");
...
async function configureMatrixInviteAutoJoin(params) {
  ...
  while (true) {
    const allowlist = normalizeMatrixInviteAutoJoinTargets(
      splitSetupEntries(
    ...
    if (allowlist.length === 0 || invalidEntries.length > 0) {
      await params.prompter.note(
        [
          ...
          invalidEntries.length > 0 ? `Invalid: ${invalidEntries.join(", ")}` : void 0,
        ]
          .filter(Boolean)
          .join("\n"),
        "Matrix invite auto-join",
      ...
      if (accessConfig.entries.length > 0)
        try {
          ...
          for (const entry of accessConfig.entries) {
            ...
            const { listMatrixDirectoryGroupsLive } = await import("./directory-live-1nN9GTyM.js");
...
const matrixOnboardingAdapter = {
  ...
  afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
    const { runMatrixSetupBootstrapAfterConfigWrite } =
