...
import { o as resolveTelegramAccount, r as listTelegramAccountIds } from "./accounts-u_rpwiv9.js";
import {
  c as createNestedAllowlistOverrideResolver,
  n as buildDmGroupAccountAllowlistAdapter,
} from "./allowlist-config-edit-CiyWlmas.js";
import {
...
import { t as resolveTelegramInlineButtons } from "./button-types-CtkW6u4a.js";
...
import {
  ...
  t as attachChannelToResult,
} from "./channel-send-result-CsgaqKTY.js";
import "./channel-core-CYgk_8J6.js";
import {
  a as telegramConfigAdapter,
  c as telegramSecurityAdapter,
  i as formatDuplicateTelegramTokenReason,
  n as createTelegramPluginBase,
  o as telegramSetupWizard,
  r as findTelegramTokenOwnerAccountId,
  s as telegramSetupAdapter,
...
import {
  ...
  r as markdownToTelegramHtmlChunks,
...
import { t as resolveTelegramInteractiveTextFallback } from "./interactive-fallback-DX9yssrw.js";
...
import {
  p as resolvePayloadMediaUrls,
  y as sendPayloadMediaSequenceOrFallback,
} from "./reply-payload-D_-IJF8p.js";
import { r as resolveTelegramStartupProbeTimeoutMs } from "./request-timeouts-B6nwfY1u.js";
import { t as getTelegramRuntime } from "./runtime--f86u1XL.js";
import { t as sanitizeForPlainText } from "./sanitize-text-CR0zkUDk.js";
import { n as resolveOutboundSendDep } from "./send-deps-CtmJEFTh.js";
import { t as resolveTelegramSessionConversation } from "./session-conversation-Bm0irZd4.js";
import { t as detectTelegramLegacyStateMigrations } from "./state-migrations-BdaMBGiM.js";
import {
...
function resolveTurnSourceTelegramOriginTarget(request) {
  ...
  return {
    to: turnSourceTo,
    threadId: parseTelegramThreadId(
      request.request.turnSourceThreadId ?? parsedTurnSourceTarget?.messageThreadId ?? void 0,
    ),
  };
}
...
const telegramNativeApprovalCapability = createApproverRestrictedNativeApprovalCapability({
  ...
  nativeRuntime: createLazyChannelApprovalNativeRuntimeAdapter({
    ...
    load: async () =>
      (await import("./approval-handler.runtime-BMDvvX47.js")).telegramApprovalNativeRuntime,
  }),
});
...
function resolveTelegramRequireMention(params) {
  ...
  if (!chatId) return;
  const scopedGroups =
    (accountId ? cfg.channels?.telegram?.accounts?.[accountId]?.groups : void 0) ??
  ...
  const defaultTopicConfig =
    topicId && groupDefault?.topics ? groupDefault.topics[topicId] : void 0;
...
async function loadTelegramSendModule$1() {
  telegramSendModulePromise$1 ??= import("./send-D8dh-Ptf.js");
...
async function resolveDefaultTelegramSend(deps) {
  return (
    ...
    (await loadTelegramSendModule$1()).sendMessageTelegram
  );
}
  ...
  if (account.connected === true && lastTransportActivityAt != null) {
    ...
    if (ageMs > TELEGRAM_POLLING_STALE_TRANSPORT_MS)
      issues.push({
        ...
        kind: "runtime",
        message: appendTelegramRuntimeError(
          `Telegram polling transport is stale (last successful getUpdates ${Math.max(0, Math.floor(ageMs / 6e4))}m ago)`,
          account.lastError,
        ),
        fix,
      });
...
async function loadTelegramSendModule() {
  telegramSendModulePromise ??= import("./send-D8dh-Ptf.js");
  return await telegramSendModulePromise;
}
async function loadTelegramUpdateOffsetRuntime() {
  telegramUpdateOffsetRuntimePromise ??=
    import("./extensions/telegram/update-offset-runtime-api.js");
...
const telegramMessageActions = {
  ...
  handleAction: async (ctx) => {
    ...
    if (!telegramMessageActions$1.handleAction)
  ...
  base: {
    ...
    lifecycle: {
      detectLegacyStateMigrations: ({ cfg, env }) =>
        detectTelegramLegacyStateMigrations({
          cfg,
          env,
        }),
...
//#endregion
export {
  telegramOutbound as a,
  resolveTelegramGroupRequireMention as c,
  sendTelegramPayloadMessages as i,
  resolveTelegramGroupToolPolicy as l,
  collectTelegramStatusIssues as n,
  looksLikeTelegramTargetId as o,
  TELEGRAM_TEXT_CHUNK_LIMIT as r,
  normalizeTelegramMessagingTarget as s,
  telegramPlugin as t,
  resolveTelegramAutoThreadId as u,
};
