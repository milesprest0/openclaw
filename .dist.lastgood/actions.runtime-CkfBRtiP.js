...
import { r as resolveIMessageMessageId } from "./monitor-reply-cache-CJIXRvqb.js";
  ...
  return await new Promise((resolve, reject) => {
    ...
    const timer =
      options.timeoutMs && options.timeoutMs > 0
        ? setTimeout(() => {
            ...
            reject(
              /* @__PURE__ */ new Error(`iMessage action timed out after ${options.timeoutMs}ms`),
            );
          }, options.timeoutMs)
        : null;
    ...
    child.on("close", (code) => {
      ...
      if (!parsed) {
        reject(
          /* @__PURE__ */ new Error(
            `imsg returned non-JSON output: ${stdout.trim() || stderr.trim()}`,
          ),
        );
        return;
      }
  ...
  async resolveChatGuidForTarget(params) {
    ...
    try {
      const list = asChatList(
        await client.request("chats.list", { limit: 1e3 }, { timeoutMs: params.options.timeoutMs }),
      );
