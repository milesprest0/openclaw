...
import { n as resolveBedrockConfigApiKey } from "./discovery-shared-DsHAFoz3.js";
...
function resolveKnownContextWindow(modelId) {
  ...
  for (const candidate of candidates) {
    ...
    if (
      ...
      KNOWN_CONTEXT_WINDOWS[withoutVersionSuffix] !== void 0
    )
...
function matchesProviderFilter(summary, filter) {
  ...
  const normalized = normalizeOptionalLowercaseString(
    summary.providerName ??
      (typeof summary.modelId === "string" ? summary.modelId.split(".")[0] : void 0),
  );
...
async function discoverBedrockModels(params) {
  ...
  const discoveryPromise = (async () => {
    const [rawFoundationResponse, profileSummaries] = await Promise.all([
      client.send(sdk.createListFoundationModelsCommand()),
...
//#endregion
export {
  resetBedrockDiscoveryCacheForTest as n,
  resolveImplicitBedrockProvider as r,
  discoverBedrockModels as t,
};
