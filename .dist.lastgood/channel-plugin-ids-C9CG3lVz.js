import { t as resolveManifestActivationPluginIds } from "./activation-planner-jSNtbjOx.js";
import {
...
import {
  S as resolveManifestOwnerBasePolicyBlock,
  b as isBundledManifestOwner,
  v as hasExplicitManifestOwnerTrust,
  x as passesManifestOwnerBasePolicy,
  y as isActivatedManifestOwner,
} from "./bundled-Ca60V8y0.js";
import { t as isSafeChannelEnvVarTriggerName } from "./channel-env-var-names-DQk0FiGU.js";
import { t as collectPluginConfigContractMatches } from "./config-contracts-Cl3ljnWr.js";
import {
  a as listPotentialConfiguredChannelPresenceSignals,
  i as listPotentialConfiguredChannelIds,
  r as listExplicitlyDisabledChannelIdsForConfig,
  t as hasMeaningfulChannelConfig,
} from "./config-presence-DQuxzSYt.js";
import {
...
import {
  ...
  q as resolveMemoryDreamingPluginId,
} from "./dreaming-BDr8qIGr.js";
import { t as collectConfiguredAgentHarnessRuntimes } from "./harness-runtimes-Bh9W-avI.js";
import { g as isPluginEnabledByDefaultForPlatform } from "./installed-plugin-index-store-YgBbP4mp.js";
import {
...
function listManifestEnvConfiguredChannelSignals(params) {
  ...
  for (const record of params.records) {
    ...
    for (const channelId of record.channels) {
      if (
        !(record.channelEnvVars?.[channelId] ?? []).some((envVar) =>
          hasNonEmptyEnvValue(params.env, envVar),
        )
      )
        continue;
  ...
  for (const channelId of normalizeChannelIds(entrySources.keys())) {
    ...
    const blockedReasons =
      owningRecords.length === 0
        ...
        : [
            ...new Set(
              evaluations.map((entry) => entry.blockedReason).filter((reason) => Boolean(reason)),
            ),
          ].toSorted((left, right) => left.localeCompare(right));
    entries.push({
      channelId,
...
function resolveProviderConfigActivation(ttsConfig, providerId) {
  ...
  if (isRecord$1(ttsConfig.providers)) {
    for (const [key, providerConfig] of Object.entries(ttsConfig.providers))
      if (normalizeConfiguredSpeechProviderIdForStartup(key) === providerId)
        fromProviders = isConfigActivationValueEnabled$1(providerConfig);
  }
  ...
  for (const [key, providerConfig] of Object.entries(ttsConfig)) {
    ...
    if (normalizeConfiguredSpeechProviderIdForStartup(key) === providerId)
      return isConfigActivationValueEnabled$1(providerConfig);
  }
}
...
function findActivePersona(ttsConfig) {
  ...
  for (const [id, persona] of Object.entries(ttsConfig.personas))
    if (normalizeOptionalLowercaseString(id) === personaId && isRecord$1(persona)) return persona;
}
...
function addConfiguredTtsProviderIds(target, value) {
  ...
  if (isRecord$1(value.providers)) {
    for (const [providerId, providerConfig] of Object.entries(value.providers))
      if (isConfigActivationValueEnabled$1(providerConfig))
...
function collectConfiguredSpeechProviderIds(config) {
  ...
  if (isRecord$1(channels))
    for (const [channelId, channelConfig] of Object.entries(channels)) {
      ...
      if (isRecord$1(channelConfig.voice))
        addConfiguredTtsProviderIds(configured, channelConfig.voice.tts);
      if (isRecord$1(channelConfig.accounts))
        for (const [accountId, accountConfig] of Object.entries(channelConfig.accounts)) {
          ...
          if (isRecord$1(accountConfig.voice))
  ...
  if (isRecord$1(pluginEntries)) {
    for (const entry of Object.values(pluginEntries))
      if (isRecord$1(entry) && isRecord$1(entry.config))
...
function hasExplicitHookPolicyConfig(entry) {
  return (
    ...
    entry?.hooks?.timeoutMs !== void 0 ||
    (entry?.hooks?.timeouts !== void 0 && Object.keys(entry.hooks.timeouts).length > 0)
  );
}
...
function resolveGatewayStartupPluginPlanFromRegistry(params) {
  ...
  return {
    ...
    pluginIds: params.index.plugins
      .filter((plugin) => {
...
//#endregion
export {
  resolveConfiguredDeferredChannelPluginIdsFromRegistry as a,
  resolveGatewayStartupPluginPlanFromRegistry as c,
  listConfiguredAnnounceChannelIdsForConfig as d,
  listConfiguredChannelIdsForReadOnlyScope as f,
  resolveDiscoverableScopedChannelPluginIds as g,
  resolveConfiguredChannelPresencePolicy as h,
  resolveConfiguredDeferredChannelPluginIds as i,
  hasConfiguredChannelsForReadOnlyScope as l,
  resolveConfiguredChannelPluginIds as m,
  resolveChannelPluginIds as n,
  resolveGatewayStartupPluginIds as o,
  listExplicitConfiguredChannelIdsForConfig as p,
  resolveChannelPluginIdsFromRegistry as r,
  resolveGatewayStartupPluginIdsFromRegistry as s,
  loadGatewayStartupPluginPlan as t,
  hasExplicitChannelConfig as u,
};
