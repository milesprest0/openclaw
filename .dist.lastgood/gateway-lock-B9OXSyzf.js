...
import { z } from "zod";
import {
  n as parseProcCmdline,
  r as parseWindowsCmdline,
  t as isGatewayArgv,
} from "./gateway-process-argv-CHw7D59b.js";
import {
  l as resolveGatewayLockDir,
  o as resolveConfigPath,
  v as resolveStateDir,
} from "./paths-Cnwfh6dH.js";
import { n as isPidAlive } from "./pid-alive-vlUbRgZw.js";
...
function readWindowsCmdline(pid) {
  try {
    ...
    const match = (
      buf.length >= 2 && buf[0] === 255 && buf[1] === 254
...
async function acquireGatewayLock(opts = {}) {
  ...
  const sleep =
    opts.sleep ?? (async (ms) => await new Promise((resolve) => setTimeout(resolve, ms)));
  ...
  throw new GatewayLockError(
    `gateway already running${lastPayload?.pid ? ` (pid ${lastPayload.pid})` : ""}; lock timeout after ${timeoutMs}ms`,
  );
}
//#endregion
