...
import "./async-lock-runtime--vP9Jbvx.js";
...
import "./memory-host-events-BTLLU5y1.js";
...
import {
  i as filterLiveShortTermRecallEntries,
  l as recordShortTermRecalls,
  o as readShortTermRecallEntries,
  s as recordDreamingPhaseSignals,
...
const dreamsFileLocks = resolveGlobalMap(
  Symbol.for("openclaw.memoryCore.dreamingNarrative.fileLocks"),
);
function isRequestScopedSubagentRuntimeError(err) {
  return (
    ...
    (err instanceof Error &&
      ...
      extractErrorCode(err) === "OPENCLAW_SUBAGENT_RUNTIME_REQUEST_SCOPE")
  );
}
...
async function writeBackfillDiaryEntries(params) {
  return await updateDreamsFile({
    ...
    updater: (existing, dreamsPath) => {
      ...
      const nextBlocks = [
        ...splitDiaryBlocks(
          startIdx >= 0 && endIdx > startIdx ? stripped.updated.slice(startIdx + 38, endIdx) : "",
        ),
...
function trimTrackedSessionScopes(seenMessages) {
  ...
  for (const [scope, hashes] of Object.entries(seenMessages))
    if (keep.has(scope)) next[scope] = hashes;
  return next;
}
...
function buildSessionRenderedLine(params) {
  return `[${`${params.agentId}/${params.sessionPath}#L${params.lineNumber}`}] ${params.snippet}`.slice(
    0,
...
async function collectSessionIngestionBatches(params) {
  ...
  for (const [scope, hashes] of Object.entries(trimmedSeenMessages))
    if (!areStringArraysEqual(params.state.seenMessages[scope] ?? [], hashes)) changed = true;
  for (const scope of Object.keys(params.state.seenMessages))
    if (!Object.hasOwn(trimmedSeenMessages, scope)) changed = true;
  const batches = [];
...
async function collectDailyIngestionBatches(params) {
  ...
  const cutoffMs = calculateLookbackCutoffMs(params.nowMs, params.lookbackDays);
  const files = (
    await fs.readdir(memoryDir, { withFileTypes: true }).catch((err) => {
      if (err?.code === "ENOENT") return [];
      throw err;
    })
  )
  ...
  for (const file of files) {
    ...
    if (!raw) continue;
    const chunks = buildDailySnippetChunks(
      stripManagedDailyDreamingLines(raw.split(/\r?\n/)),
      perFileCap,
    );
    const results = [];
...
//#endregion
export {
  dedupeDreamDiaryEntries as a,
  runDetachedDreamNarrative as c,
  seedHistoricalDailyMemorySignals as i,
  writeBackfillDiaryEntries as l,
  previewRemDreaming as n,
  generateAndAppendDreamNarrative as o,
  runDreamingSweepPhases as r,
  removeBackfillDiaryEntries as s,
  filterRecallEntriesWithinLookback as t,
  writeDeepDreamingReport as u,
};
