...
import {
  ...
  n as detectCarriedShellBuiltinArgv,
  r as detectCarrierInlineEvalArgv,
  s as detectShellWrapperThroughCarrierArgv,
...
function payloadBaseFromArgument(argument, payload) {
  ...
  return {
    ...
    mapOffset(offset) {
      const mappedRawOffset =
        argument.decodedSourceOffsets[payloadOffset + offset] ?? rawPayloadOffset + offset;
      return {
...
function shellWrapperPayloadForParsing(argv, argumentsList, dynamicArguments) {
  ...
  if (
    !canParseShellWrapperPayload(
      transportArgv,
      (shellCommandFlag(transportArgv, 1) ?? shellCommandFlag(argv, 1))?.flag ?? null,
    )
  )
    return null;
  return {
