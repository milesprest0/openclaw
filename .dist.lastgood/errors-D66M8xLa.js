import {
  n as extractLeadingHttpStatus,
  r as formatRawAssistantErrorForUi,
} from "./assistant-error-format-BsjMPFd_.js";
import {
  a as isPeriodicUsageLimitErrorMessage,
  c as isTimeoutErrorMessage,
  i as isOverloadedErrorMessage,
  l as matchesFormatErrorPattern,
  n as isAuthPermanentErrorMessage,
  o as isRateLimitErrorMessage,
  r as isBillingErrorMessage,
  s as isServerErrorMessage,
  t as isAuthErrorMessage,
} from "./failover-matches-CDN8j5cn.js";
import { t as isModelNotFoundErrorMessage } from "./live-model-errors-BxEU9TwQ.js";
import { n as classifyOAuthRefreshFailure } from "./oauth-refresh-failure-BP623TH9.js";
import { t as formatSandboxToolPolicyBlockedMessage } from "./runtime-status-r0KsiafK.js";
import {
  a as formatTransportErrorCopy,
  c as isLikelyHttpErrorText,
  ...
  r as formatDiskSpaceErrorCopy,
  s as isInvalidStreamingEventOrderError,
  u as isStreamingJsonParseError,
...
function matchesProviderContextOverflow(errorMessage) {
  ...
  return (
    resolveProviderRuntimeHooks()?.matchesProviderContextOverflowWithPlugin({
...
function classifyProviderSpecificError(errorMessage) {
  const pluginReason =
    resolveProviderRuntimeHooks()?.classifyProviderFailoverReasonWithPlugin({
      context: { errorMessage },
    }) ?? null;
...
function isOpenAICodexScopeContext(raw, provider) {
  return (
    normalizeLowercaseStringOrEmpty(provider) === "openai-codex" ||
...
function classifyFailoverClassificationFromMessage(raw, provider) {
  ...
  if (isTransientHttpError(raw)) {
    if (extractLeadingHttpStatus(raw.trim())?.code === 529)
...
function formatAssistantErrorText(msg, opts) {
  ...
  if (providerRuntimeFailureKind === "refresh_contention")
    return "Authentication refresh is already in progress elsewhere and this attempt timed out waiting for it. Retry in a moment.";
  if (providerRuntimeFailureKind === "refresh_timeout")
    return "Authentication refresh timed out before the provider completed. Retry in a moment; re-authenticate only if it keeps failing.";
  if (providerRuntimeFailureKind === "callback_timeout")
    return "Browser OAuth did not complete before manual fallback kicked in. Retry the login flow and paste the redirect URL if prompted.";
  if (providerRuntimeFailureKind === "callback_validation")
    return "Browser OAuth returned an invalid or incomplete callback. Retry the login flow and make sure the full redirect URL is pasted if prompted.";
  if (providerRuntimeFailureKind === "auth_scope")
    return "Authentication is missing the required OpenAI Codex scopes. Re-run OpenAI/Codex login and try again.";
  if (providerRuntimeFailureKind === "auth_html_403")
    return "Authentication failed with an HTML 403 response from the provider. Re-authenticate and verify your provider account access.";
...
function parseImageDimensionError(raw) {
  if (!raw) return null;
  if (!normalizeLowercaseStringOrEmpty(raw).includes("image dimensions exceed max allowed size"))
    return null;
...
//#endregion
export {
  isTransientHttpError as _,
  formatAssistantErrorText as a,
  parseImageSizeError as b,
  isBillingAssistantError as c,
  isContextOverflowError as d,
  isFailoverAssistantError as f,
  isReasoningConstraintErrorMessage as g,
  isRateLimitAssistantError as h,
  extractObservedOverflowTokenCount as i,
  isCloudCodeAssistFormatError as l,
  isLikelyContextOverflowError as m,
  classifyFailoverSignal as n,
  inferSignalStatus as o,
  isFailoverErrorMessage as p,
  classifyProviderRuntimeFailureKind as r,
  isAuthAssistantError as s,
  classifyFailoverReason as t,
  isCompactionFailureError as u,
  isUnclassifiedNoBodyHttpSignal as v,
  parseImageDimensionError as y,
};
