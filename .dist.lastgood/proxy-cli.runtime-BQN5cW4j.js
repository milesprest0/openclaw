...
import { t as buildDebugProxyCoverageReport } from "./coverage-Cu4lAREw.js";
import { r as resolveDebugProxySettings, t as applyDebugProxyEnv } from "./env-C2C0rw4L.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import { r as probeApnsHttp2ReachabilityViaProxy } from "./push-apns-http2-CFQaCxOd.js";
import { t as resolveSystemBin } from "./resolve-system-bin-V2-g2eqK.js";
import {
  c as closeDebugProxyCaptureStore,
  i as initializeDebugProxyCapture,
  l as getDebugProxyCaptureStore,
  r as finalizeDebugProxyCapture,
...
import { r as createHttp1ProxyAgent } from "./undici-runtime-BpIrSBt2.js";
...
async function runDeniedCheck(params) {
  ...
  try {
    ...
    if (
      params.target.expectedCanaryToken !== void 0 &&
  ...
  try {
    ...
    if (!hasApnsReachabilityProof(result))
      return {
        ...
        ok: false,
        error:
          "APNs reachability check failed: response did not include an apns-id header or APNs InvalidProviderToken body. The proxy may be intercepting the connection instead of tunneling it.",
      };
    return {
      kind: "apns",
      url: params.authority,
...
function assertDebugProxyDirectUpstreamAllowed(env = process.env) {
  ...
  throw new Error(
    `Debug proxy direct upstream forwarding is disabled while managed proxy mode is active. Set ${DEBUG_PROXY_DIRECT_CONNECT_OVERRIDE}=1 only for approved local diagnostics.`,
  );
}
...
async function startDebugProxyServer(params) {
  ...
  const server = createServer$1(async (req, res) => {
    ...
    const upstream = (target.protocol === "https:" ? request$1 : request)(
      target,
      {
        method: req.method,
        headers: req.headers,
...
//#endregion
export {
  readDebugProxyBlobCommand,
  runDebugProxyCoverageCommand,
  runDebugProxyPurgeCommand,
