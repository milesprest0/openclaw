...
function extractImagesFromPart(images, part) {
  ...
  if (!data) return;
  const mimeType =
    (typeof inlineData?.mimeType === "string" ? inlineData.mimeType : void 0) ??
    (typeof inlineData?.mime_type === "string" ? inlineData.mime_type : void 0) ??
    "image/png";
  const image = generatedImageAssetFromBase64({
    base64: data,
    index: images.length,
...
function extractOpenRouterImagesFromResponse(body) {
  ...
  for (const choice of body.choices ?? []) {
    ...
    if (typeof content === "string" && content.length > 0)
      ...
      for (const part of content) extractImagesFromPart(images, part);
  }
  return images;
}
