...
import { g as DEFAULT_TOOL_DENY, h as DEFAULT_TOOL_ALLOW } from "./constants-DcbmEUTo.js";
...
function classifyToolAgainstSandboxToolPolicy(name, policy) {
  ...
  const blockedByDeny = matchesAnyGlobPattern(
    normalized,
    compileGlobPatterns({
...
function resolveSandboxToolPolicyForAgent(cfg, agentId) {
  const agentPolicy = (cfg && agentId ? resolveAgentConfig(cfg, agentId) : void 0)?.tools?.sandbox
    ?.tools;
  ...
  const expanded = expandResolvedPolicy({
    ...
    deny: Array.isArray(denyConfig.values)
      ? [...denyConfig.values]
...
export {
  isToolAllowed as n,
  resolveSandboxToolPolicyForAgent as r,
  classifyToolAgainstSandboxToolPolicy as t,
};
