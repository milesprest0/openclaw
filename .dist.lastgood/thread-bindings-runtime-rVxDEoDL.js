...
function toSessionBindingRecord(params) {
  const idleExpiresAt =
    params.idleTimeoutMs > 0 ? params.record.lastActivityAt + params.idleTimeoutMs : void 0;
...
function createAccountScopedConversationBindingManager(params) {
  ...
  const manager = {
    ...
    getByConversationId: (conversationId) =>
      getState(params.stateKey).bindingsByAccountConversation.get(
        resolveBindingKey({
          accountId,
          conversationId,
        }),
    ...
    listBySessionKey: (targetSessionKey) =>
      [...getState(params.stateKey).bindingsByAccountConversation.values()].filter(
    ...
    bindConversation: ({ conversationId, targetKind, targetSessionKey, metadata }) => {
      ...
      const existing = getState(params.stateKey).bindingsByAccountConversation.get(
        resolveBindingKey({
          accountId,
      ...
      getState(params.stateKey).bindingsByAccountConversation.set(
        resolveBindingKey({
          accountId,
          conversationId: normalizedConversationId,
        }),
    ...
    unbindBySessionKey: (targetSessionKey) => {
      ...
      for (const record of getState(params.stateKey).bindingsByAccountConversation.values()) {
        if (record.accountId !== accountId || record.targetSessionKey !== targetSessionKey)
          continue;
        getState(params.stateKey).bindingsByAccountConversation.delete(
          resolveBindingKey({
            accountId,
    ...
    stop: () => {
      for (const key of getState(params.stateKey).bindingsByAccountConversation.keys())
...
//#endregion
export {
  resetAccountScopedConversationBindingsForTests as n,
  createAccountScopedConversationBindingManager as t,
};
