...
async function planAllModelListSources(params) {
  ...
  const { loadStaticManifestCatalogRowsForList, loadSupplementalManifestCatalogRowsForList } =
    await import("./list.manifest-catalog-ppTD3wUk.js");
  if (!params.providerFilter) {
    const { loadProviderIndexCatalogRowsForList } =
      await import("./list.provider-index-catalog-B7hgXCJd.js");
    return createSourcePlan({
      kind: "registry",
  ...
  const { hasProviderStaticCatalogForFilter } = await import("./list.provider-catalog-y-G1a-ns.js");
  if (
    await hasProviderStaticCatalogForFilter({
      cfg: params.cfg,
      providerFilter: params.providerFilter,
    })
  )
