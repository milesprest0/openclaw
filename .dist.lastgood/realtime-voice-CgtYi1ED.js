...
import {
  f as buildRealtimeVoiceAgentConsultPrompt,
  g as parseRealtimeVoiceAgentConsultArgs,
  m as collectRealtimeVoiceAgentConsultVisibleText,
} from "./session-runtime-B8HmMzEf.js";
import { f as parseSessionThreadInfoFast } from "./store-load-BSFLPYqQ.js";
...
async function consultRealtimeVoiceAgent(params) {
  ...
  const result = await params.agentRuntime.runEmbeddedPiAgent({
    ...
    currentThreadTs:
      consultDeliveryContext?.threadId != null ? String(consultDeliveryContext.threadId) : void 0,
    sessionFile,
    workspaceDir,
    config: params.cfg,
...
function hasMeaningfulEchoOverlap(userTokens, assistantTokens) {
  ...
  return (
    uniqueUserTokens.filter((token) => assistantTokenSet.has(token)).length /
      uniqueUserTokens.length >=
    0.58
  );
}
...
function sampleBandlimited(input, inputSamples, srcPos, cutoffCyclesPerSample) {
  ...
  for (let tap = -half; tap <= half; tap += 1) {
    ...
    const coeff =
      lowPass * (0.5 - 0.5 * Math.cos((2 * Math.PI * tapIndex) / (RESAMPLE_FILTER_TAPS - 1)));
...
//#endregion
export {
  resamplePcmTo8k as a,
  getRealtimeVoiceTranscriptHealth as c,
  recordRealtimeVoiceTranscript as d,
  resolveRealtimeVoiceFastContextConsult as f,
  resamplePcm as i,
  isLikelyRealtimeVoiceAssistantEchoTranscript as l,
  consultRealtimeVoiceAgent as m,
  mulawToPcm as n,
  extendRealtimeVoiceOutputEchoSuppression as o,
  createRealtimeVoiceAgentTalkbackQueue as p,
  pcmToMulaw as r,
  getRealtimeVoiceBridgeEventHealth as s,
  convertPcmToMulaw8k as t,
  recordRealtimeVoiceBridgeEvent as u,
};
