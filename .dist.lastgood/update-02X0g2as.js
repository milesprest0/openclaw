...
import { i as resolveBundledPluginSources } from "./bundled-sources-BqA2rgRL.js";
import { f as resolvePluginInstallDir } from "./channel-catalog-registry-uu7_tUkL.js";
import {
  r as installPluginFromClawHub,
  t as CLAWHUB_INSTALL_ERROR_CODE,
} from "./clawhub-Bc1Chqe7.js";
import { t as buildClawHubPluginInstallRecordFields } from "./clawhub-install-records-B98Uouzt.js";
...
import {
  ...
  D as compareOpenClawReleaseVersions,
  M as parseRegistryNpmSpec,
} from "./discovery-bnS95tO3.js";
import { t as installPluginFromGitSpec } from "./git-install-DwslkMeP.js";
import {
  n as resolveNpmInstallSpecsForUpdateChannel,
  t as resolveClawHubInstallSpecsForUpdateChannel,
} from "./install-channel-specs-BlPZbOGs.js";
import {
  a as installPluginFromNpmSpec,
  t as PLUGIN_INSTALL_ERROR_CODE,
} from "./install-Djc3uDTp.js";
import { a as resolveNpmSpecMetadata } from "./install-source-utils-DrH7L6Si.js";
import {
  n as recordPluginInstall,
  t as buildNpmResolutionInstallFields,
} from "./installs-Dt5uWKE7.js";
import { t as installPluginFromMarketplace } from "./marketplace-Bq_M0rSR.js";
import {
...
import {
  i as readInstalledPackageVersion,
  n as installedPackageNeedsOpenClawPeerLinkRepair,
  r as readInstalledPackagePeerDependencies,
  t as expectedIntegrityForUpdate,
} from "./package-update-utils-XrztudBX.js";
import { t as linkOpenClawPeerDependencies } from "./plugin-peer-link-lg5CcSDT.js";
import {
  i as parseComparableSemver,
  t as compareComparableSemver,
} from "./semver-compare-CNMZuf2e.js";
...
function shouldSkipUnchangedNpmInstall(params) {
  ...
  if (params.currentVersion !== params.metadata.version) return false;
  if (!params.record.resolvedName || !params.record.resolvedSpec || !params.record.resolvedVersion)
    return false;
...
function isBundledVersionNewer(bundledVersion, installedVersion) {
  ...
  const cmp = compareComparableSemver(
    parseComparableSemver(bundledVersion),
    parseComparableSemver(installedVersion),
  );
...
function resolveRecordedExtensionsDir(params) {
  ...
  try {
    return resolvePluginInstallDir(params.pluginId, parentDir) === params.installPath
      ? parentDir
      : void 0;
  } catch {
    return;
...
function describeBetaNpmFallback(params) {
  ...
  const reason =
    ...
    /\b(ETARGET|notarget)\b|No matching version found|dist-tag|tag .*not found/i.test(
      ...
      ? "has no beta npm release"
      : "failed beta npm update";
...
function resolveTrustedSourceLinkedOfficialClawHubSpec(params) {
  ...
  if (!officialSpec || !officialPackageName) return;
  return [params.record.clawhubPackage, resolveClawHubSpecPackageName(params.record.spec)]
    .filter((value) => Boolean(value))
    .includes(officialPackageName)
    ? officialSpec
    : void 0;
...
function isBridgeAlreadyInstalledFromPreferredSource(params) {
  return getExternalizedBundledPluginPreferredSource(params.bridge) === "clawhub"
  ...
  for (const pluginId of targets) {
    ...
    if (record.source === "clawhub" || record.source === "marketplace") {
      ...
      if (
        ...
        logger.warn?.(
          `Skipping "${pluginId}" update: bundled version ${bundledSource.version} is newer than the installed ${record.source} version ${record.version}. Uninstall the ${record.source} plugin to use the bundled version, or pin a newer version explicitly.`,
        );
        outcomes.push({
          pluginId,
          status: "skipped",
...
async function syncPluginsForUpdateChannel(params) {
  ...
  if (params.channel === "dev")
    for (const [pluginId, record] of Object.entries(installs)) {
      ...
      if (record.source === "path" && pathsEqual(record.sourcePath, bundledInfo.localPath, env))
        continue;
      next = recordPluginInstall(next, {
        pluginId,
        source: "path",
...
//#endregion
export {
  updateNpmInstalledPlugins as i,
  resolveTrustedSourceLinkedOfficialNpmSpec as n,
  syncPluginsForUpdateChannel as r,
  resolveTrustedSourceLinkedOfficialClawHubSpec as t,
};
