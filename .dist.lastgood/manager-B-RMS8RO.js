...
import { i as requireAcpRuntimeBackend, n as getAcpRuntimeBackend } from "./registry-C6ubc_kk.js";
import {
...
} from "./session-identity-QvqXBpWC.js";
import { c as normalizeAgentId, l as normalizeMainKey } from "./session-key-B4qUwRzq.js";
import { n as isAcpSessionKey, o as parseAgentSessionKey } from "./session-key-utils-B3KPN8Ee.js";
import {
  i as upsertAcpSessionMeta,
  n as readAcpSessionEntry,
  t as listAcpSessionEntries,
} from "./session-meta-rwd0ZmpO.js";
import {
...
async function reconcileManagerRuntimeSessionIdentifiers(params) {
  ...
  if (!identityEquals(currentIdentity, nextIdentity)) {
    ...
    logVerbose(
      `acp-manager: session identity updated for ${params.sessionKey} (agentSessionId ${currentAgentSessionId} -> ${nextAgentSessionId}, acpxSessionId ${currentAcpxSessionId} -> ${nextAcpxSessionId}, acpxRecordId ${currentAcpxRecordId} -> ${nextAcpxRecordId})`,
    );
  }
...
function validateRuntimeOptionPatch(patch) {
  ...
  if (Object.hasOwn(rawPatch, "timeoutSeconds"))
    if (rawPatch.timeoutSeconds === void 0) next.timeoutSeconds = void 0;
...
function normalizeRuntimeOptions(options) {
  ...
  const backendExtras =
    backendExtrasEntries.length > 0 ? Object.fromEntries(backendExtrasEntries) : void 0;
  return {
  ...
  for await (const event of params.runtime.runTurn(params.turn)) {
    ...
    if (event.type === "done") sawTerminalEvent = true;
      ...
      streamError = new AcpRuntimeError(
        ...
        normalizeOptionalString(event.message) || "ACP turn failed before completion.",
      );
...
var AcpSessionManager = class {
      ...
      try {
        if (
          await this.withSessionActor(session.sessionKey, async () => {
            ...
            return !isSessionIdentityPending(
  ...
  async getSessionStatus(params) {
    ...
    return await this.withSessionActor(
      ...
      async () => {
        this.throwIfAborted(params.signal);
        const resolvedMeta = requireReadySessionMeta(
          this.resolveSession({
            cfg: params.cfg,
            sessionKey,
          }),
  ...
  async setSessionRuntimeMode(params) {
    ...
    return await this.withSessionActor(sessionKey, async () => {
      ...
      if (
        !(
          await this.resolveRuntimeCapabilities({
            runtime,
            handle,
          })
        ).controls.includes("session/set_mode") ||
        !runtime.setMode
      )
        throw createUnsupportedControlError({
          backend: handle.backend || meta.backend,
  ...
  async updateSessionRuntimeOptions(params) {
    ...
    return await this.withSessionActor(sessionKey, async () => {
      const nextOptions = mergeRuntimeOptions({
        current: resolveRuntimeOptionsFromMeta(
  ...
  resolveTurnTimeoutMs(params) {
    ...
    if (
      ...
      return Math.max(1e3, Math.round(runtimeTimeoutSeconds * 1e3));
  ...
  async awaitTurnWithTimeout(params) {
    ...
    try {
      ...
      if (outcome === timeoutToken) {
        ...
        throw new AcpRuntimeError(
          "ACP_TURN_FAILED",
          `ACP turn timed out after ${Math.max(1, Math.round(params.timeoutLabelMs / 1e3))}s.`,
        );
      }
      if (outcome.kind === "error") throw outcome.error;
      return outcome.value;
    ...
    return await this.withSessionActor(sessionKey, async () => {
      ...
      if (shouldSkipRuntimeClose) {
        if (input.discardPersistentState) {
          ...
          try {
            await this.deps
              .getRuntimeBackend(configuredBackend || void 0)
              ?.runtime.prepareFreshSession?.({ sessionKey });
          } catch (error) {
            logVerbose(
  ...
  async ensureRuntimeHandle(params) {
    const agent =
      normalizeOptionalString(params.meta.agent) ||
    ...
    const persistedResumeSessionId =
      mode === "persistent" ? resolveRuntimeResumeSessionId(previousIdentity) : void 0;
    ...
    if (shouldPrepareFreshPersistentSession)
      await runtime.prepareFreshSession?.({ sessionKey: params.sessionKey });
  ...
  resolveBackgroundTaskContext(params) {
    ...
    const requesterSessionKey =
      normalizeOptionalString(childEntry?.spawnedBy) ??
      normalizeOptionalString(childEntry?.parentSessionKey);
    if (!requesterSessionKey) return null;
    return {
      requesterSessionKey,
      requesterOrigin:
        this.deps.readSessionEntry({
          cfg: params.cfg,
...
//#endregion
export {
  validateRuntimeConfigOptionInput as a,
  validateRuntimeModelInput as c,
  parseRuntimeTimeoutSecondsInput as i,
  validateRuntimePermissionProfileInput as l,
  getAcpSessionManager as n,
  validateRuntimeCwdInput as o,
  AcpSessionManager as r,
  validateRuntimeModeInput as s,
  __testing as t,
  resolveAcpSessionResolutionError as u,
};
