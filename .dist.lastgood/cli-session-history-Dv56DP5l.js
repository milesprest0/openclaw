...
import { t as attachOpenClawTranscriptMeta } from "./session-utils.fs-z-7Kw0Lg.js";
...
function normalizeClaudeCliContent(content, toolNameRegistry) {
  ...
  for (const item of content) {
    ...
    if (type === "tool_use") {
      ...
      if (block.input !== void 0 && block.arguments === void 0)
...
function extractComparableText(message) {
  ...
  if (!joined) return;
  return (
    (role === "user" ? stripInboundMetadata(joined) : joined).replace(/\s+/g, " ").trim() || void 0
  );
}
...
function resolveImportedExternalId(message) {
  ...
  return normalizeOptionalString(
    ("__openclaw" in message && message.__openclaw && typeof message.__openclaw === "object"
      ? (message.__openclaw ?? {})
      : void 0
    )?.externalId,
  );
...
function compareHistoryMessages(a, b) {
  ...
  if (aTimestamp !== void 0 && bTimestamp !== void 0 && aTimestamp !== bTimestamp)
