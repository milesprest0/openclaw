...
import "./runtime-doctor-DPEAm6hl.js";
...
import { t as isDiscordMutableAllowEntry } from "./security-doctor-C0vPeu_-.js";
...
function collectDiscordNumericIdWarnings(params) {
  ...
  for (const hits of hitsByListPath.values())
    if (hits.some((hit) => !hit.safe)) blockedHits.push(...hits);
...
function maybeRepairDiscordNumericIds(cfg, doctorFixCommand) {
  ...
  const repairList = (pathLabel, holder, key) => {
    ...
    if (
      raw.some((entry) => typeof entry === "number" && (!Number.isSafeInteger(entry) || entry < 0))
    )
      return;
    let converted = 0;
