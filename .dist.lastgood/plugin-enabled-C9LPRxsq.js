import { o as stopOpenClawChrome } from "./chrome-B-sV0ork.js";
...
import { t as isPwAiLoaded } from "./pw-ai-state-BlSN1YsS.js";
import "./config-CE62Wyxe.js";
import {
  n as listKnownProfileNames,
  t as createBrowserRouteContext,
} from "./server-context-Il7jEHOv.js";
import {
  a as isSubagentSessionKey,
  i as isCronSessionKey,
  n as isAcpSessionKey,
} from "./session-key-utils-B3KPN8Ee.js";
import { n as sweepTrackedBrowserTabs } from "./session-tab-registry-D-1NosPu.js";
import { n as getPwAiModule } from "./target-id-Bq4VUN4D.js";
...
function collectNestedErrorCandidates(err) {
  ...
  while (queue.length > 0) {
    ...
    for (const nested of [record.cause, record.reason, record.original, record.error, record.data])
...
function isPlaywrightDialogRaceUnhandledRejection(reason) {
  for (const candidate of collectNestedErrorCandidates(reason)) {
    ...
    for (const playwrightMethod of PLAYWRIGHT_DIALOG_METHODS)
      if (message.includes(playwrightMethod)) return true;
  }
  return false;
}
...
export {
  ...
  createBrowserRuntimeState as o,
  ensureBrowserControlRuntime as r,
  stopBrowserRuntime as s,
