...
import { t as isTailnetIPv4 } from "./tailnet-mP5NqepX.js";
...
function parseTailscaleStatusIPv4s(stdout) {
  ...
  if (peerObj && typeof peerObj === "object")
    for (const peer of Object.values(peerObj)) addIps(peer);
  return [...new Set(out)];
}
...
function parseDnsSdResolve(stdout, instanceName) {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    if (line.startsWith("txt") || line.includes("txtvers="))
      txt = parseTxtTokens(line.split(/\s+/).filter(Boolean));
  }
...
async function discoverViaDnsSd(domain, timeoutMs, run) {
  const instances = parseDnsSdBrowse(
    (await run(["dns-sd", "-B", GATEWAY_SERVICE_TYPE, domain], { timeoutMs })).stdout,
  );
  const results = [];
  for (const instance of instances) {
    const parsed = parseDnsSdResolve(
      (await run(["dns-sd", "-L", instance, GATEWAY_SERVICE_TYPE, domain], { timeoutMs })).stdout,
      instance,
    );
    if (parsed)
      results.push({
...
async function discoverWideAreaViaTailnetDns(domain, timeoutMs, run) {
  ...
  for (const candidate of tailscaleCandidates)
    try {
      ips = parseTailscaleStatusIPv4s(
        (
          await run([candidate, "status", "--json"], {
  ...
  const worker = async () => {
    while (nameserver === null) {
      ...
      if (!ip) continue;
      try {
        const lines = parseDigShortLines(
          (
            await run(["dig", "+short", "+time=1", "+tries=1", `@${ip}`, probeName, "PTR"], {
...
//#endregion
export {
  resolveGatewayDiscoveryEndpoint as i,
  pickResolvedGatewayHost as n,
  pickResolvedGatewayPort as r,
  discoverGatewayBeacons as t,
};
