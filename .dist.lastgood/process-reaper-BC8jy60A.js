...
async function readLeaseFile(filePath) {
  ...
  return {
    ...
    leases: Array.isArray(value.leases)
      ? value.leases.map(normalizeLease).filter((lease) => !!lease)
      : [],
  };
}
...
function createAcpxProcessLeaseStore(params) {
  ...
  return {
    ...
    async listOpen(gatewayInstanceId) {
      return (await readCurrent()).leases.filter(
        (lease) =>
...
function liveCommandMatchesLeaseIdentity(params) {
  ...
  return (
    commandOptionEquals(parts, "--openclaw-acpx-lease-id", params.expectedLeaseId) &&
    commandOptionEquals(parts, "--openclaw-gateway-instance-id", params.expectedGatewayInstanceId)
  );
}
...
async function reapStaleOpenClawOwnedAcpxOrphans(params) {
  ...
  return {
    ...
    terminatedPids: await terminatePids(
      uniquePids(orphanTrees.flatMap((tree) => tree.toReversed())),
      params.deps,
    ),
  };
}
//#endregion
export {
  OPENCLAW_GATEWAY_INSTANCE_ID_ARG as a,
  hashAcpxProcessCommand as c,
  OPENCLAW_ACPX_LEASE_ID_ARG as i,
  withAcpxLeaseEnvironment as l,
  isOpenClawOwnedAcpxProcessCommand as n,
  createAcpxProcessLeaseId as o,
  reapStaleOpenClawOwnedAcpxOrphans as r,
  createAcpxProcessLeaseStore as s,
  cleanupOpenClawOwnedAcpxProcessTree as t,
};
