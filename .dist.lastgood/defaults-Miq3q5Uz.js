...
function resolveDefaultRegistry(cfg, workspaceDir) {
  ...
  return cacheConfigRegistry(
    cacheKey,
    buildMediaUnderstandingManifestMetadataRegistry(cfg, workspaceDir),
  );
}
...
function resolveDefaultMediaModel(params) {
  ...
  return normalizeOptionalString(
    (params.providerRegistry ?? resolveDefaultRegistry(params.cfg, params.workspaceDir)).get(
      normalizeMediaProviderId(params.providerId),
    )?.defaultModels?.[params.capability],
  );
}
...
function providerSupportsNativePdfDocument(params) {
  return (
    (params.providerRegistry ?? resolveDefaultRegistry(params.cfg, params.workspaceDir))
      .get(normalizeMediaProviderId(params.providerId))
      ?.nativeDocumentInputs?.includes("pdf") ?? false
  );
}
//#endregion
export {
  resolveAutoMediaKeyProviders as n,
  resolveDefaultMediaModel as r,
  providerSupportsNativePdfDocument as t,
};
