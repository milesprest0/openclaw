import {
  i as resolveDeprecatedAuthChoiceReplacement,
  n as isDeprecatedAuthChoice,
  r as normalizeLegacyOnboardAuthChoice,
  t as formatDeprecatedNonInteractiveAuthChoiceError,
} from "./auth-choice-legacy-DgFglK71.js";
import { t as resolveGatewayAuthToken } from "./auth-token-resolution-Civdg1Yd.js";
...
import "./daemon-runtime-CkYPLluX.js";
import {
  i as applySkipBootstrapConfig,
  r as applyLocalSetupWorkspaceConfig,
} from "./onboard-config-CaX-AhyH.js";
import {
...
import { t as restoreTerminalState } from "./restore-DEobrepY.js";
...
import { t as runSetupWizard } from "./setup-Cjw700fG.js";
import { c as normalizeOptionalString } from "./string-coerce-BdEutqX5.js";
import {
  l as isValidEnvSecretRefId,
  m as resolveSecretInputRef,
...
function classifyGatewayHealthFailure(params) {
  ...
  if (/\b(?:unauthorized|forbidden|invalid token|invalid password|auth mismatch)\b/i.test(combined))
...
async function collectGatewayHealthFailureDiagnostics() {
  ...
  try {
    ...
    const [loaded, runtime] = await Promise.all([
      ...
      service.readRuntime(env).catch(() => void 0),
    ]);
    diagnostics.service = {
      label: service.label,
      loaded,
  ...
  try {
    const { readLastGatewayErrorLine } = await import("./diagnostics-BEffz99J.js");
...
async function runNonInteractiveLocalSetup(params) {
  ...
  const inferredAuthChoice = opts.authChoice
    ? void 0
    : (await import("./auth-choice-inference-CtISUw-F.js")).inferAuthChoiceFromFlags(opts, {
        config: nextConfig,
        workspaceDir,
        env: process.env,
      });
  ...
  if (authChoice !== "skip") {
    const { applyNonInteractiveAuthChoice } = await import("./auth-choice-BLplwZSt.js");
  ...
  if (opts.installDaemon) {
    const { installGatewayDaemonNonInteractive } = await import("./daemon-install-CKnuagUT.js");
  ...
  if (!opts.skipHealth) {
    const { healthCommand } = await import("./health-CDLeEYK5.js");
...
async function runNonInteractiveMigrationImport(params) {
  ...
  const { detectSetupMigrationSources, runSetupMigrationImport } =
    await import("./setup.migration-import-BTntt2yC.js");
...
async function setupWizardCommand(opts, runtime = defaultRuntime) {
  ...
  if (opts.nonInteractive && isDeprecatedAuthChoice(originalAuthChoice, { env: process.env })) {
    runtime.error(
      formatDeprecatedNonInteractiveAuthChoiceError(originalAuthChoice, { env: process.env }),
    );
    runtime.exit(1);
    return;
  }
