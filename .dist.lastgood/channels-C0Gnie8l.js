...
import { a as describeBinding, t as applyAgentBindings } from "./agents.bindings-BNIQ-B9L.js";
import { i as getBundledChannelSetupPlugin } from "./bundled-Ca60V8y0.js";
...
import { t as resolveCommandConfigWithSecrets } from "./command-config-resolution-B8fep8sX.js";
import "./config-CMOdcWpc.js";
import { n as getChannelsCommandSecretTargetIds } from "./command-secret-targets-CkWxZ_Vf.js";
import { t as requireValidConfigFileSnapshot } from "./config-validation-DPqhMlGD.js";
...
import {
  i as resolveChannelDefaultAccountId,
...
import { t as channelsListCommand } from "./list-BwQMAoTn.js";
import { n as resolveLogFile } from "./log-tail-C7O8b799.js";
import { o as getResolvedLoggerSettings } from "./logger-BU4ooQvN.js";
import { t as listManifestChannelContributionIds } from "./manifest-contribution-ids-09jjuga-.js";
import {
  l as resolveMessageActionDiscoveryForPlugin,
  r as createMessageActionDiscoveryContext,
} from "./message-action-discovery-jVpVmmf8.js";
import { r as replaceConfigFile } from "./mutate-IzL9dVFH.js";
import { t as parseLogLine } from "./parse-log-line-D6kfXBom.js";
...
import { n as defaultRuntime, r as writeRuntimeJson } from "./runtime-kqN0Yohi.js";
import { c as moveSingleAccountChannelSectionToDefaultAccount } from "./setup-helpers-BWYI0iQf.js";
import {
  c as shouldUseWizard,
  o as formatChannelAccountLabel,
  s as requireValidConfig,
} from "./shared-DbFZmdPe.js";
import {
  n as formatGatewayChannelsStatusLines,
  t as channelsStatusCommand,
} from "./status-DxZz578j.js";
import {
...
const channelSetupPluginInstallLoader = createLazyImportLoader(
  () => import("./plugin-install-CKKhxWAA.js"),
);
const onboardChannelsLoader = createLazyImportLoader(
  () => import("./onboard-channels-S9iBipvG.js"),
);
...
async function resolveCatalogChannelEntry(raw, cfg) {
  ...
  return (
    cfg
      ? await import("./trusted-catalog-Dpww95kT.js").then(
      ...
      : await import("./catalog-DHuEBnWJ.js").then(({ listChannelPluginCatalogEntries }) =>
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
  ...
  if (shouldUseWizard(params)) {
    const [{ buildAgentSummaries }, onboardChannels] = await Promise.all([
      import("./agents.config-CNVSQqyl.js"),
      loadOnboardChannels(),
    ]);
    ...
    if (
      ...
      for (const channel of selection) {
        ...
        const existingName =
          plugin?.config.describeAccount?.(account, nextConfig)?.name ?? account?.name;
  ...
  if (!channel && catalogEntry) {
    ...
    const { isCatalogChannelInstalled } = await import("./discovery-XMnzu6rO.js");
    if (
...
async function resolveChannelReports(params) {
  ...
  for (const accountId of accountIds) {
    ...
    const actions = Array.from(
      new Set(["send", "broadcast", ...discoveredActions.map((action) => action)]),
    );
    reports.push({
      plugin,
      channel: plugin.id,
      accountId,
      accountName:
        typeof resolvedAccount.name === "string"
          ? normalizeOptionalString(resolvedAccount.name)
          : void 0,
      configured,
      enabled,
...
async function channelsCapabilitiesCommand(opts, runtime = defaultRuntime) {
  ...
  for (const report of reports) {
    ...
    const probeLines =
      report.plugin.status?.formatCapabilitiesProbe?.({ probe: report.probe }) ??
...
async function channelsLogsCommand(opts, runtime = defaultRuntime) {
  ...
  const filtered = (await readTailLines(file, limit * 4))
    .map(parseLogLine)
    .filter((line) => Boolean(line))
    .filter((line) => matchesChannel(line, channel));
...
async function channelsRemoveCommand(opts, runtime = defaultRuntime, params) {
  ...
  const resolvedPluginState = Boolean(lookupChannel || channel)
    ? await (async () => {
        const { resolveInstallableChannelPlugin } =
          await import("./channel-plugin-resolution-D0gdG684.js");
...
async function channelsResolveCommand(opts, runtime) {
  ...
  const plugin =
    (explicitChannel ? resolvedExplicit?.plugin : void 0) ??
    (selection.channel ? getChannelPlugin(selection.channel) : void 0);
  ...
  for (const result of results)
    if (result.resolved && result.id) runtime.log(formatResolveResult(result));
    else
      runtime.error(
        danger(
