...
import { t as CHANNEL_IDS } from "./ids-D_WQYNuU.js";
import {
  D as collectPluginSchemaMetadata,
  E as collectChannelSchemaMetadata,
  O as GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA,
...
import {
  r as isSensitiveUrlConfigPath,
  t as SENSITIVE_URL_HINT_TAG,
} from "./redact-sensitive-url-Bu3cghBT.js";
import { a as isSensitiveConfigPath } from "./redact-snapshot-DiercLAY.js";
...
import { t as sensitive } from "./zod-schema.sensitive-5DB_eCSg.js";
...
const FIELD_HELP = {
  ...
  "agents.defaults.experimental.historyCacheBreakpoints":
    'Experimental Anthropic history cache breakpoints: "off" (default) keeps history markers unchanged, "shadow" computes frozen/warm breakpoint indices for diagnostics without mutating payloads, and "on" emits history breakpoints only when frozen:true tags are present in the assembled message payload.',
  ...
  "agents.defaults.memorySearch.fallbackModel":
    "Optional explicit model id for fallback-provider embedding requests. When set, this overrides the fallback adapter default model so failover can stay dimension-compatible with the primary index.",
  "agents.defaults.memorySearch.fallbackOutputDimensionality":
    "Optional fallback-provider output dimension override. When unset, fallback requests inherit memorySearch.outputDimensionality so vector writes stay consistent with the pinned store dimensions.",
  ...
  "agents.defaults.historyOptimization.compactToolCallArgs":
    'Deterministically compact frozen assistant tool-call arguments into a digest object (default: false). Only applies when freezeMode is "frozen".',
...
const FIELD_LABELS = {
  ...
  "observability.promptInstrumentation": "Prompt Instrumentation",
  "observability.promptInstrumentation.enabled": "Prompt Instrumentation Enabled",
  logging: "Logging",
  ...
  "agents.defaults.contextBudget": "Per-Turn Context Budget",
  "agents.defaults.contextBudget.enabled": "Context Budget Enabled",
  "agents.defaults.contextBudget.maxAssembledTokens": "Context Budget Max Assembled Tokens",
  "agents.defaults.contextBudget.perThreadMaxImages": "Context Budget Max Inline Images",
  "agents.defaults.contextBudget.reserveTokens": "Context Budget Reserve Tokens",
  "agents.defaults.contextBudget.targetBand": "Context Budget Target Band",
  "agents.defaults.contextBudget.targetBand.min": "Context Budget Target Min Tokens",
...
function applyDerivedTags(hints) {
  ...
  for (const [path, hint] of Object.entries(hints)) {
    ...
    const tags = [
      ...normalizeTags([...deriveTagsForPath(path, hint), ...existingTags]),
...
function collectMatchingSchemaPaths(schema, path, matchesPath, paths = /* @__PURE__ */ new Set()) {
  ...
  if (currentSchema instanceof z.ZodObject) {
    ...
    if (catchallSchema && !(catchallSchema instanceof z.ZodNever))
      collectMatchingSchemaPaths(catchallSchema, path ? `${path}.*` : "*", matchesPath, paths);
...
function mapSensitivePaths(schema, path, hints) {
  ...
  if (currentSchema instanceof z.ZodObject) {
    ...
    const catchallSchema = currentSchema._def.catchall;
    if (catchallSchema && !(catchallSchema instanceof z.ZodNever))
      next = mapSensitivePaths(catchallSchema, path ? `${path}.*` : "*", next);
  } else if (currentSchema instanceof z.ZodArray) {
    const nextPath = path ? `${path}[]` : "[]";
    ...
    for (const option of currentSchema.options) next = mapSensitivePaths(option, path, next);
  else if (currentSchema instanceof z.ZodIntersection) {
  ...
  const collectSchemaKeys = (schema, basePath) => {
    ...
    if (Array.isArray(node.items)) {
      for (const item of node.items)
        if (item && typeof item === "object") collectSchemaKeys(item, `${basePath}[]`);
      return;
    }
...
function applyPluginSchemas(schema, plugins) {
  const next = cloneSchema(schema);
  const entriesNode = asJsonSchemaObject(
    asJsonSchemaObject(asJsonSchemaObject(next)?.properties?.plugins)?.properties?.entries,
  );
  ...
  for (const plugin of plugins) {
    ...
    const entryObject = asJsonSchemaObject(
      entryBase ? cloneSchema(entryBase) : { type: "object" },
    ) ?? { type: "object" };
...
//#endregion
export {
  readBestEffortRuntimeConfigSchema as n,
  lookupConfigSchema as r,
  loadGatewayRuntimeConfigSchema as t,
};
