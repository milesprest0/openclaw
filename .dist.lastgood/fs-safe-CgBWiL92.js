...
import {
  i as isPathInside,
  o as isSymlinkOpenError,
  p as FsSafeError,
  r as isNotFoundPathError,
} from "./path-1liOXr_N.js";
import {
  a as inspectPathPermissions,
  c as isWorldReadable,
  l as isWorldWritable,
  o as isGroupReadable,
  s as isGroupWritable,
  u as modeBits,
} from "./permissions-DkQuD-Po.js";
import { t as assertSafePathPrefix } from "./safe-path-segment-Bp_Kir-q.js";
import {
  f as resolveUserPath,
  o as root,
  s as getFsSafeTestHooks,
  t as resolveSecureTempRoot,
  u as expandHomePrefix,
} from "./secure-temp-dir-CCj3cY2B.js";
import { t as normalizeLowercaseStringOrEmpty } from "./string-coerce-FEYRxS0z.js";
import {
  c as assertAsyncDirectoryGuard,
  n as registerTempPathForExit,
  s as withAsyncDirectoryGuards,
  t as serializePathWrite,
  u as createAsyncDirectoryGuard,
} from "./write-queue-CClMlwr5.js";
    ...
    if (ancestor) {
      ...
      if (canonicalAncestor !== ancestor)
        throw new FsSafeError("symlink", "path traverses a symlink", {
          cause: {
            canonicalPath: path.join(canonicalAncestor, path.relative(ancestor, normalized)),
          },
        });
    }
  }
...
async function resolveWritablePathWithinRoot(params) {
  ...
  if (
    (await validateCanonicalPathWithinRoot({
      rootRealPath,
...
async function ensureDirectoryWithinRoot(params) {
  ...
  try {
    ...
    if (!isPathInside(rootReal, await fs$1.realpath(nearestExistingPath)))
      return invalidPath(params.scopeLabel);
    ...
    if (!isPathInside(rootReal, await fs$1.realpath(targetPath)))
      return invalidPath(params.scopeLabel);
    return {
      ok: true,
      path: targetPath,
...
async function writeViaSiblingTempPath(params) {
  ...
  const tempDir = await fs$1.mkdtemp(
    path.join(
...
async function openSecureHandle(options) {
  ...
  let handle;
  try {
    handle = await fs$1.open(
      options.filePath,
      options.trust?.allowSymlink ? constants.O_RDONLY : OPEN_READ_FLAGS,
    );
  } catch (err) {
  ...
  try {
    ...
    const pathStat = options.trust?.allowSymlink
      ? await fs$1.stat(options.filePath)
      : await fs$1.lstat(options.filePath);
    ...
    if (!sameFileIdentity(await fs$1.stat(realPath), openedStat))
      throw new FsSafeError("path-mismatch", `${label(options)} real path changed during open.`);
    if (options.io?.maxBytes !== void 0 && openedStat.size > options.io.maxBytes)
      throw new FsSafeError(
        "too-large",
...
async function assertTrustedDirs(options, realPath) {
  ...
  if (
    ...
    ).some((dir) => isPathInside(dir, realPath))
  )
    throw new FsSafeError(
      "outside-workspace",
...
async function readSecureFile(options) {
  ...
  try {
    ...
    if (options.io?.maxBytes !== void 0 && buffer.byteLength > options.io.maxBytes)
      throw new FsSafeError(
        "too-large",
        `${label(options)} exceeded maxBytes (${options.io.maxBytes}).`,
      );
...
function shouldStop(result, options) {
  return (
    options.maxEntries !== void 0 && result.scannedEntryCount >= Math.max(0, options.maxEntries)
  );
}
...
function walkDirectorySync(rootDir, options = {}) {
  ...
  function visit(dir, depth) {
    ...
    for (const dirent of entries) {
      ...
      if (
        kind === "directory" &&
        (options.maxDepth === void 0 || depth < options.maxDepth) &&
...
async function writeFileWithinRoot(params) {
  await (
    await root(params.rootDir)
  ).write(params.relativePath, params.data, {
...
//#endregion
export {
  findExistingAncestor as A,
  safeFileURLToPath as C,
  sanitizeUntrustedFileName as D,
  pathExistsSync as E,
  resolveAbsolutePathForWrite as M,
  assertAbsolutePathInput as O,
  isWindowsNetworkPath as S,
  pathExists as T,
  readLocalFileFromRoots as _,
  walkDirectory as a,
  basenameFromMediaSource as b,
  writeSiblingTempFile as c,
  pathScope as d,
  resolveExistingPathsWithinRoot as f,
  resolveWritablePathWithinRoot as g,
  resolveStrictExistingPathsWithinRoot as h,
  writeFileWithinRoot as i,
  resolveAbsolutePathForRead as j,
  canonicalPathFromExistingAncestor as k,
  writeViaSiblingTempPath as l,
  resolvePathsWithinRoot as m,
  readFileWithinRoot as n,
  walkDirectorySync as o,
  resolvePathWithinRoot as p,
  writeExternalFileWithinRoot as r,
  readSecureFile as s,
  ensureAbsoluteDirectory as t,
  withTimeout as u,
  resolveLocalPathFromRootsSync as v,
  trySafeFileURLToPath as w,
  hasEncodedFileUrlSeparator as x,
  assertNoWindowsNetworkPath as y,
};
