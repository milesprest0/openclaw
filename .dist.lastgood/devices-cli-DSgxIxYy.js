...
import { r as PAIRING_SCOPE, t as ADMIN_SCOPE } from "./operator-scopes-Codapd9g.js";
...
const devicesCallOpts = (cmd, defaults) =>
  cmd
...
function resolveLocalPairingFallback(opts, error) {
  const details = readConnectPairingRequiredMessage(
    normalizeLowercaseStringOrEmpty(normalizeErrorMessage(error)),
  );
...
async function approvePairingWithFallback(opts, requestId) {
  ...
  try {
    ...
    if (isDevicePairingApprovalDenied(error) && !scopes?.includes("operator.admin"))
      return await callGatewayCli(
        "device.pair.approve",
        opts,
        { requestId },
