...
function clearWaitSessionIds(sessionKey) {
  for (const [sessionId, mappedKey] of replyRunState.waitKeysBySessionId)
    if (mappedKey === sessionKey) replyRunState.waitKeysBySessionId.delete(sessionId);
}
...
function createReplyOperation(params) {
  ...
  if (params.upstreamAbortSignal)
    if (params.upstreamAbortSignal.aborted) abortInternally(params.upstreamAbortSignal.reason);
    else
  ...
  const operation = {
    ...
    abortForRestart() {
      const phaseBeforeAbort = phase;
      abortWithReason("restart", /* @__PURE__ */ new Error("Reply operation aborted for restart"), {
...
const replyRunRegistry = {
  ...
  waitForIdle(sessionKey, timeoutMs = 15e3) {
    ...
    return new Promise((resolve) => {
      const waiters =
        replyRunState.waitersByKey.get(normalizedSessionKey) ?? /* @__PURE__ */ new Set();
      const waiter = {
        resolve,
        timer: setTimeout(
          () => {
...
const ACTIVE_EMBEDDED_RUNS =
  embeddedRunState.activeRuns ?? (embeddedRunState.activeRuns = /* @__PURE__ */ new Map());
const ACTIVE_EMBEDDED_RUN_SNAPSHOTS =
  embeddedRunState.snapshots ?? (embeddedRunState.snapshots = /* @__PURE__ */ new Map());
const ACTIVE_EMBEDDED_RUN_SESSION_IDS_BY_KEY =
  embeddedRunState.sessionIdsByKey ??
  (embeddedRunState.sessionIdsByKey = /* @__PURE__ */ new Map());
const EMBEDDED_RUN_WAITERS =
  embeddedRunState.waiters ?? (embeddedRunState.waiters = /* @__PURE__ */ new Map());
const EMBEDDED_RUN_MODEL_SWITCH_REQUESTS =
  embeddedRunState.modelSwitchRequests ??
  (embeddedRunState.modelSwitchRequests = /* @__PURE__ */ new Map());
function getActiveEmbeddedRunCount() {
  ...
  for (const sessionId of listActiveReplyRunSessionIds())
    if (!ACTIVE_EMBEDDED_RUNS.has(sessionId)) activeCount += 1;
  return Math.max(activeCount, getActiveReplyRunCount());
}
//#endregion
export {
  waitForReplyRunEndBySessionId as _,
  EMBEDDED_RUN_WAITERS as a,
  abortActiveReplyRuns as c,
  forceClearReplyRunBySessionId as d,
  isReplyRunActiveForSessionId as f,
  resolveActiveReplyRunSessionId as g,
  replyRunRegistry as h,
  EMBEDDED_RUN_MODEL_SWITCH_REQUESTS as i,
  abortReplyRunBySessionId as l,
  queueReplyRunMessage as m,
  ACTIVE_EMBEDDED_RUN_SESSION_IDS_BY_KEY as n,
  getActiveEmbeddedRunCount as o,
  isReplyRunStreamingForSessionId as p,
  ACTIVE_EMBEDDED_RUN_SNAPSHOTS as r,
  ReplyRunAlreadyActiveError as s,
  ACTIVE_EMBEDDED_RUNS as t,
  createReplyOperation as u,
};
