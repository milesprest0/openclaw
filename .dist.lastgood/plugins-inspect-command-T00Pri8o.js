...
import { u as quietPluginJsonLogger } from "./plugins-command-helpers-D4actSTy.js";
...
function formatInstallLines(install) {
  ...
  if (install.clawpackSpecVersion !== void 0)
...
async function runPluginsInspectCommand(id, opts) {
  ...
  if (inspect.policy.hasAllowedModelsConfig)
    policyLines.push(
      `allowedModels: ${inspect.policy.allowedModels.length > 0 ? inspect.policy.allowedModels.join(", ") : "(configured but empty)"}`,
    );
