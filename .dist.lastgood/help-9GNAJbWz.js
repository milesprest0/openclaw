...
import {
  c as hasFlag,
  l as hasRootVersionAlias,
  m as getSubCliCommandsWithSubcommands,
  y as getCoreCliCommandsWithSubcommands,
} from "./argv-DQxa_OA3.js";
import { i as hasEmittedCliBanner, r as formatCliBannerLine } from "./banner-BmQ3UoLQ.js";
...
import { _ as ALLOWED_LOG_LEVELS, b as tryParseLogLevel } from "./logger-BU4ooQvN.js";
...
function configureProgramHelp(program, ctx) {
  ...
  program.configureHelp({
    ...
    subcommandTerm: (cmd) => {
      const hasSubcommands =
        cmd.parent === program && ROOT_COMMANDS_WITH_SUBCOMMANDS.has(cmd.name());
  ...
  const formatHelpOutput = (str) => {
    let output = str;
    if (
      new RegExp(`^Usage:\\s+${CLI_NAME_PATTERN}\\s+\\[options\\]\\s+\\[command\\]\\s*$`, "m").test(
        output,
      ) &&
      /^Commands:/m.test(output)
    )
