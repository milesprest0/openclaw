import { r as listLoadedChannelPlugins } from "./registry-loaded-DVm3aV50.js";
import { n as getActivePluginChannelRegistryVersionFromState } from "./runtime-channel-state-CefjemMO.js";
import {
  c as normalizeOptionalString,
  s as normalizeOptionalLowercaseString,
} from "./string-coerce-BdEutqX5.js";
import { a as listThinkingLevels, l as BASE_THINKING_LEVELS } from "./thinking-C9_Z62px.js";
...
const formatMcpArgs = (values) =>
  formatActionArgs(values, {
    formatKnownAction: (action, path) => {
      if (action === "show" || action === "get") return path ? `${action} ${path}` : action;
    },
  });
const formatPluginsArgs = (values) =>
  formatActionArgs(values, {
    formatKnownAction: (action, path) => {
...
const BROWSER_SAFE_THINKING_LEVELS = [...BASE_THINKING_LEVELS, "xhigh", "adaptive", "max"];
