...
import { t as resolveAccountWithDefaultFallback } from "./account-core-BYoyzRSP.js";
import {
  r as describeWebhookAccountSnapshot,
  s as resolveMergedAccountConfig,
  t as createAccountListHelpers,
} from "./account-helpers-DgSuSC4_.js";
...
import {
  ...
  n as createChannelPairingController,
...
} from "./doctor-contract-KzGPkfm4.js";
...
import { r as deliverFormattedTextWithAttachments } from "./reply-payload-D_-IJF8p.js";
import { t as getNextcloudTalkRuntime } from "./runtime-api-Bgr5GzPX.js";
import "./ssrf-runtime-BhFjkd3c.js";
import "./secret-file-runtime-DqkhSdQj.js";
...
} from "./secret-contract-B6-4aVy3.js";
...
import { f as ssrfPolicyFromPrivateNetworkOptIn } from "./ssrf-policy-CLYqev3x.js";
import {
...
import { r as WEBHOOK_RATE_LIMIT_DEFAULTS } from "./webhook-ingress-BWEossCd.js";
...
function resolveNextcloudTalkSecret(cfg, opts) {
  ...
  if (envSecret && resolvedAccountId === "default")
    return {
      secret: envSecret,
      source: "env",
    };
...
const nextcloudTalkSecurityAdapter = {
...
const NextcloudTalkRoomSchema = zod_exports.z
  .object({
  ...
  .strict();
const NextcloudTalkNetworkSchema = zod_exports.z
  .object({
  ...
  .strict()
  .optional();
const NextcloudTalkAccountSchemaBase = zod_exports.z
  .object({
    ...
    rooms: zod_exports.z
      .record(zod_exports.z.string(), NextcloudTalkRoomSchema.optional())
      .optional(),
...
const NextcloudTalkConfigSchema = NextcloudTalkAccountSchemaBase.extend({
  accounts: zod_exports.z
    .record(zod_exports.z.string(), NextcloudTalkAccountSchema.optional())
    .optional(),
...
const NextcloudTalkWebhookPayloadSchema = zod_exports.z.object({
  type: zod_exports.z.enum(["Create", "Update", "Delete"]),
  actor: zod_exports.z.object({
    type: zod_exports.z.literal("Person"),
    id: zod_exports.z.string().min(1),
    name: zod_exports.z.string(),
  }),
  object: zod_exports.z.object({
    type: zod_exports.z.literal("Note"),
    id: zod_exports.z.string().min(1),
    name: zod_exports.z.string(),
    content: zod_exports.z.string(),
    mediaType: zod_exports.z.string(),
  }),
  target: zod_exports.z.object({
    type: zod_exports.z.literal("Collection"),
    id: zod_exports.z.string().min(1),
    name: zod_exports.z.string(),
  }),
});
...
async function processNextcloudTalkReplayGuardedMessage(params) {
  if (
    (await params.replayGuard.claimMessage({
...
function createNextcloudTalkReplayGuard(options) {
  ...
  return {
    ...
    shouldProcessMessage: async ({ accountId, roomToken, messageId }) => {
      ...
      if (!replayKey) return true;
      if ((await dedupe.claim(replayKey, { namespace: accountId })).kind !== "claimed")
        return false;
      return await dedupe.commit(replayKey, { namespace: accountId });
    },
  };
...
const nextcloudTalkDmPolicy = {
  ...
  resolveConfigKeys: (cfg, accountId) =>
    (accountId ?? resolveDefaultNextcloudTalkAccountId(cfg)) !== "default"
      ? {
