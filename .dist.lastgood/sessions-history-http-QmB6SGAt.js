...
import {
  i as projectChatDisplayMessages,
  t as DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS,
} from "./chat-display-projection-BE1OTzu2.js";
import {
  a as getHeader,
  d as resolveTrustedHttpOperatorScopes,
  n as authorizeScopedGatewayHttpRequestOrReply,
  r as checkGatewayHttpRequestAuth,
} from "./http-auth-utils-QDjCKoNp.js";
import {
  a as sendMethodNotAllowed,
  i as sendJson,
  r as sendInvalidRequest,
  s as setSseHeaders,
...
import {
  ...
  t as attachOpenClawTranscriptMeta,
...
function buildSessionHistorySnapshot(params) {
  const history = paginateSessionMessages(
    toSessionHistoryMessages(
      projectChatDisplayMessages(params.rawMessages, { maxChars: params.maxChars ?? 8e3 }),
    ),
    params.limit,
    params.cursor,
  );
...
function resolveSessionHistoryPath(req) {
  const match = new URL(req.url ?? "/", `http://${req.headers.host ?? "localhost"}`).pathname.match(
    /^\/sessions\/([^/]+)\/history$/,
  );
...
async function handleSessionHistoryHttpRequest(req, res, opts) {
  ...
  const boundedSnapshot =
    cursor === void 0 && typeof limit === "number"
  ...
  const isStreamStillAuthorized = async () => {
    ...
    return authorizeOperatorScopesForMethod(
      "chat.history",
      resolveTrustedHttpOperatorScopes(req, currentRequestAuth.requestAuth),
    ).allowed;
  };
