...
import { s as resolveChunkMode } from "./chunk-aBEwc7QQ.js";
...
import {
  ...
  _ as resolveDiscordSendComponents,
  a as normalizeDiscordPollInput,
  c as normalizeStickerIds,
  f as sendDiscordMedia,
  g as buildDiscordMessageRequest,
  h as SUPPRESS_NOTIFICATIONS_FLAG,
  k as parseAndResolveRecipient,
  l as resolveChannelId,
  n as buildDiscordTextChunks,
  p as sendDiscordText,
  t as buildDiscordSendError,
  u as resolveDiscordChannelType,
  v as resolveDiscordSendEmbeds,
...
function deriveForumThreadName(text) {
  return (
    ...
    ).slice(0, DISCORD_THREAD_NAME_LIMIT) || /* @__PURE__ */ new Date().toISOString().slice(0, 16)
  );
}
...
async function sendMessageDiscord(to, text, opts) {
  ...
  const textLimit =
    typeof opts.textLimit === "number" && Number.isFinite(opts.textLimit)
      ? Math.max(1, Math.min(Math.floor(opts.textLimit), 2e3))
      : void 0;
  const mediaMaxBytes =
...
async function sendStickerDiscord(to, stickerIds, opts) {
  ...
  return toDiscordSendResult(
    await request(
      () =>
        createChannelMessage(rest, channelId, {
          body: {
