...
import { i as stripInlineDirectiveTagsForDisplay } from "./directive-tags-91V02E1x.js";
import { i as hasInterSessionUserProvenance } from "./input-provenance-OI5EnRxZ.js";
import { u as stripInternalRuntimeContext } from "./internal-runtime-context-DfXSY94W.js";
...
import {
  n as stripInboundMetadata,
  t as extractInboundSenderLabel,
} from "./strip-inbound-meta-D_kvNQoY.js";
import {
...
function* enumerableOwnEntries(value) {
  ...
  for (const key in record)
    if (Object.prototype.propertyIsEnumerable.call(record, key)) yield [key, record[key]];
}
...
function boundedJsonUtf8Bytes(value, maxBytes) {
  ...
  const visit = (entry, inArray) => {
    ...
    if (typeof objectEntry.toJSON === "function" && !(objectEntry instanceof Date))
...
const extractToolCallNames = (message) => {
  ...
  const toolName =
    typeof toolNameRaw === "string" ? normalizeOptionalString$1(toolNameRaw) : void 0;
...
function stripEnvelopeFromMessage(message) {
  ...
  const entry = message;
  const stripUserEnvelope =
    (typeof entry.role === "string" ? normalizeLowercaseStringOrEmpty(entry.role) : "") === "user";
  let changed = false;
  const next = { ...entry };
...
async function buildSessionTranscriptIndex(filePath, stat) {
  ...
  await visitTranscriptJsonLines(filePath, (line, offset, byteLength) => {
    ...
    const parentId =
      parsed.parentId === null ? null : (normalizeOptionalString(parsed.parentId) ?? void 0);
    const rawEntry = {
      ...(id ? { id } : {}),
  ...
  const activeRawEntries = hasTreeEntries
...
async function readSessionTranscriptIndex(filePath) {
  ...
  const index = await promise.finally(() => {
    if (transcriptIndexBuilds.get(filePath)?.promise === promise)
...
function parseRecentTranscriptTailMessages(lines, maxMessages) {
  return transcriptRecordsToMessages(
    selectActiveTranscriptRecords(
      lines.flatMap((line) => {
...
async function readSessionMessagesAsync(sessionId, storePath, sessionFile, opts) {
  ...
  if (!filePath) return [];
  return (
    (await readSessionTranscriptIndex(filePath))?.entries.flatMap((entry) =>
...
async function readRecentSessionMessagesAsync(sessionId, storePath, sessionFile, opts) {
  ...
  return parseRecentTranscriptTailMessages(
    await readRecentTranscriptTailLinesAsync(filePath, stat, {
      ...opts,
      maxMessages,
    }),
    maxMessages,
...
async function readSessionTitleFieldsFromTranscriptAsync(
  sessionId,
  storePath,
  sessionFile,
  agentId,
...
function extractTextFromContent(content) {
  if (typeof content === "string")
    return stripInlineDirectiveTagsForDisplay(content).text.trim() || null;
...
function findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId) {
  return (
    resolveSessionTranscriptCandidates(sessionId, storePath, sessionFile, agentId).find((p) =>
      fs.existsSync(p),
    ) ?? null
  );
}
...
async function readLatestRecentSessionUsageFromTranscriptAsync(
  sessionId,
  storePath,
  sessionFile,
  agentId,
...
function readRecentSessionUsageFromTranscript(
  sessionId,
  storePath,
  sessionFile,
  agentId,
  ...
  return withOpenTranscriptFd(filePath, (fd) => {
    ...
    if (bytesRead <= 0) return null;
    return extractAggregateUsageFromTranscriptChunk(
      buf
        .toString("utf-8", 0, bytesRead)
        .split(/\r?\n/)
        .slice(readStart > 0 ? 1 : 0)
...
export {
  jsonUtf8Bytes as C,
  firstEnumerableOwnKeys as S,
  stripEnvelope as _,
  readRecentSessionMessagesWithStatsAsync as a,
  extractToolCallNames as b,
  readSessionMessageCountAsync as c,
  readSessionTitleFieldsFromTranscript as d,
  readSessionTitleFieldsFromTranscriptAsync as f,
  stripEnvelopeFromMessages as g,
  stripEnvelopeFromMessage as h,
  readRecentSessionMessagesAsync as i,
  readSessionMessagesAsync as l,
  readSessionTranscriptIndex as m,
  capArrayByJsonBytes as n,
  readRecentSessionTranscriptLines as o,
  visitSessionMessagesAsync as p,
  readLatestRecentSessionUsageFromTranscriptAsync as r,
  readRecentSessionUsageFromTranscript as s,
  attachOpenClawTranscriptMeta as t,
  readSessionPreviewItemsFromTranscript as u,
  stripMessageIdHints as v,
  jsonUtf8BytesOrInfinity as w,
  boundedJsonUtf8Bytes as x,
  countToolResults as y,
};
