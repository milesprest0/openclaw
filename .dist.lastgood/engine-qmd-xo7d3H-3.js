...
import {
  ...
  d as parseUsageCountedSessionIdFromFileName,
  i as isSessionArchiveArtifactName,
  n as isCompactionCheckpointTranscriptFileName,
} from "./artifacts-D7iplKs9.js";
import { t as formatErrorMessage } from "./error-utils-Da3HMwV3.js";
import { t as hashText } from "./hash-BocRyeN4.js";
import { n as HEARTBEAT_PROMPT } from "./heartbeat-Cn4100FJ.js";
import { i as isExecCompletionEvent } from "./heartbeat-events-filter-DfMXYGL3.js";
import { r as isHeartbeatUserMessage } from "./heartbeat-filter-B7WZaEWz.js";
...
import { o as statRegularFile, r as readRegularFile } from "./regular-file-CoVHB-2u.js";
...
import {
  n as normalizeOptionalLowercaseString,
  t as normalizeLowercaseStringOrEmpty,
} from "./string-utils-CQS7ux9I.js";
...
function shouldSkipTranscriptFileForDreaming(absPath) {
  ...
  if (isSessionArchiveArtifactName(fileName) && !isUsageCountedSessionTranscriptFileName(fileName))
    return true;
  return false;
}
...
function isDreamingNarrativeGeneratedRecord(record) {
  ...
  if (!record || typeof record !== "object" || Array.isArray(record)) return false;
  const candidate = record;
  if (hasDreamingNarrativeRunId(candidate.runId) || hasDreamingNarrativeRunId(candidate.sessionKey))
    return true;
...
function isDreamingNarrativeSessionStoreKey(sessionKey) {
  ...
  return (secondSeparator < 0 ? trimmed : trimmed.slice(secondSeparator + 1)).startsWith(
...
function sanitizeSessionText(text, role) {
  const normalized = normalizeSessionText(
    stripInternalRuntimeContext(stripInboundMetadataForUserRole(text, role)),
  );
...
async function buildSessionEntry(absPath, opts = {}) {
  try {
    ...
    const lines = (await readRegularFile({ filePath: absPath })).buffer
      .toString("utf-8")
      .split("\n");
    const collected = [];
    const lineMap = [];
    ...
    const sessionStoreClassification =
      opts.generatedByDreamingNarrative === void 0 || opts.generatedByCronRun === void 0
    ...
    for (let jsonlIdx = 0; jsonlIdx < lines.length; jsonlIdx++) {
      ...
      const renderedLines = renderSessionExportLines(
...
function isQmdNoResultsOutput(raw) {
  return raw
    ...
    .some((line) => isQmdNoResultsLine(line));
}
  ...
  try {
    ...
    return parsed.map((item) => {
      ...
      return {
        ...
        score:
          typeof record.score === "number" && Number.isFinite(record.score) ? record.score : void 0,
...
function normalizeQmdSessionKey(key) {
  ...
  const normalized = normalizeLowercaseStringOrEmpty(
    parseAgentSessionKey(trimmed)?.rest ?? trimmed,
  );
  ...
  return await new Promise((resolve, reject) => {
    ...
    const timer = params.timeoutMs
      ? setTimeout(() => {
          ...
          reject(
            /* @__PURE__ */ new Error(
              `${params.commandSummary} timed out after ${params.timeoutMs}ms`,
            ),
          );
        }, params.timeoutMs)
      : null;
    ...
    child.on("close", (code) => {
      ...
      if (code === 0)
        ...
        reject(
          /* @__PURE__ */ new Error(
            `${params.commandSummary} failed (code ${code}): ${stderr || stdout}`,
          ),
        );
    });
  });
...
//#endregion
export {
  deriveQmdScopeChatType as a,
  buildSessionEntry as c,
  loadSessionTranscriptClassificationForAgent as d,
  normalizeSessionTranscriptPathForComparison as f,
  deriveQmdScopeChannel as i,
  listSessionFilesForAgent as l,
  resolveCliSpawnInvocation as n,
  isQmdScopeAllowed as o,
  sessionPathForFile as p,
  runCliCommand as r,
  parseQmdQueryJson as s,
  checkQmdBinaryAvailability as t,
  loadDreamingNarrativeTranscriptPathSetForAgent as u,
};
