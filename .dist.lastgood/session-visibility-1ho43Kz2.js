...
async function listSpawnedSessionKeys(params) {
  ...
  try {
    ...
    const keys = (Array.isArray(list?.sessions) ? list.sessions : [])
      .map((entry) => normalizeOptionalString(entry?.key) ?? "")
      .filter(Boolean);
    return new Set(keys);
  } catch {
...
function resolveEffectiveSessionToolsVisibility(params) {
  ...
  if (
    (params.cfg.agents?.defaults?.sandbox?.sessionToolsVisibility ?? "spawned") === "spawned" &&
    visibility !== "tree"
  )
    return "tree";
  return visibility;
...
//#endregion
export {
  listSpawnedSessionKeys as a,
  resolveSessionToolsVisibility as c,
  createSessionVisibilityRowChecker as i,
  sessionVisibilityGatewayTesting as l,
  createSessionVisibilityChecker as n,
  resolveEffectiveSessionToolsVisibility as o,
  createSessionVisibilityGuard as r,
  resolveSandboxSessionToolsVisibility as s,
  createAgentToAgentPolicy as t,
};
