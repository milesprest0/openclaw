...
import "./config-api-C9yg7n_u.js";
import "./channel-api-CvNQ3mt5.js";
import "./outbound-session-route-CITZVYyj.js";
...
import { n as getOptionalDiscordRuntime } from "./runtime-BuudSymP.js";
import { t as sendMessageDiscord } from "./send.outbound-8Xjee9fn.js";
import { n as createDiscordSendResult } from "./send.receipt-CkuBBjk4.js";
import {
  M as createDiscordClient,
  h as SUPPRESS_NOTIFICATIONS_FLAG,
  k as parseAndResolveRecipient,
  l as resolveChannelId,
  m as toDiscordFileBlob,
  t as buildDiscordSendError,
  u as resolveDiscordChannelType,
  y as stripUndefinedFields,
...
function deletePersistentComponentConsumptionGroup(entry) {
  ...
  for (const id of resolveComponentConsumptionIds(entry))
    store.delete(id).catch(disablePersistentComponentRegistry);
}
async function resolvePersistentRegistryEntry(params) {
  const store = params.openStore();
  if (!store) return null;
  try {
    return readPersistedRegistryEntry(
...
function extractComponentAttachmentNames(spec) {
  ...
  for (const block of spec.blocks ?? [])
    if (block.type === "file") names.push(resolveDiscordComponentAttachmentName(block.file));
  return names;
}
...
async function sendDiscordComponentMessage(to, spec, opts) {
  ...
  const { channelId } = await resolveChannelId(
    rest,
    await parseAndResolveRecipient(to, cfg, opts.accountId),
    request,
  );
...
async function editDiscordComponentMessage(to, messageId, spec, opts) {
  ...
  try {
    result = await request(
      () => editChannelMessage(rest, channelId, messageId, { body }),
      "components",
    );
  } catch (err) {
    throw await buildDiscordSendError(err, {
...
//#endregion
export {
  resolveDiscordModalEntryWithPersistence as a,
  resolveDiscordComponentEntryWithPersistence as i,
