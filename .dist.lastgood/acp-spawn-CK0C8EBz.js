...
import "./regular-file-ypPhZaXF.js";
import { i as callGateway } from "./call-hIeQLRKk.js";
import { o as normalizeAssistantPhase } from "./chat-message-content-BLockgKs.js";
import {
  r as resolveInboundConversationResolution,
  t as resolveChannelDefaultBindingPlacement,
} from "./conversation-resolution-D1JQnpHO.js";
import {
  n as resolveConversationDeliveryTarget,
  t as formatConversationTarget,
} from "./delivery-context-D3lOjhR9.js";
import "./config-CMOdcWpc.js";
import { t as deliveryContextFromSession } from "./delivery-context.shared-mj-12Z6_.js";
import {
...
import { i as areHeartbeatsEnabled, o as requestHeartbeat } from "./heartbeat-wake-2F-mloyP.js";
import { i as getRuntimeConfig } from "./io-CEQSCTGy.js";
import "./store-BpWdoYPF.js";
...
import { i as resolveAcpAgentPolicyError, r as isAcpEnabledByPolicy } from "./policy-BeBi9Mtp.js";
import "./runtime-internal-BpFvipjk.js";
...
import {
  i as isSessionBindingError,
  r as getSessionBindingService,
} from "./session-binding-service-DjzRXKiv.js";
import {
...
import {
  ...
  p as scopedHeartbeatWakeOptions,
...
import { n as readAcpSessionEntry } from "./session-meta-rwd0ZmpO.js";
import {
  c as resolveMainSessionAlias,
  s as resolveInternalSessionKey,
} from "./sessions-helpers-BUCXJZwG.js";
import {
  a as resolveSpawnedWorkspaceInheritance,
  s as resolveSubagentTargetPolicy,
  t as resolveRequesterOriginForChild,
...
import {
  a as getSubagentDepthFromSessionStore,
  i as resolveSubagentCapabilityStore,
  r as resolveSubagentCapabilities,
  t as isSubagentEnvelopeSession,
} from "./subagent-capabilities-C7k0LjTH.js";
import {
  r as countActiveRunsForSession,
  s as getSubagentRunByChildSessionKey,
} from "./subagent-registry-DESvhcq6.js";
...
import { m as listTasksForOwnerKey } from "./task-registry-Bkiwt9Td.js";
import {
...
import { a as resolveSessionTranscriptFile } from "./transcript-B5N3gLUE.js";
...
function resolveAcpSpawnStreamLogPath(params) {
  ...
  try {
    return resolveAcpStreamLogPathFromSessionFile(
      resolveSessionFilePath(
        sessionId,
        storeEntry.entry,
        resolveSessionFilePathOptions({ storePath: storeEntry.storePath }),
      ),
      sessionId,
    );
  } catch {
...
function startAcpSpawnParentStreamRelay(params) {
  ...
  const maxRelayLifetimeMs =
    typeof params.maxRelayLifetimeMs === "number" && Number.isFinite(params.maxRelayLifetimeMs)
      ? Math.max(1e3, Math.floor(params.maxRelayLifetimeMs))
  ...
  const noOutputWatcherTimer = setInterval(() => {
    ...
    emit(
      `${relayLabel} has produced no output for ${Math.round(noOutputNoticeMs / 1e3)}s. It may be waiting for interactive input.`,
      `${contextPrefix}:stall`,
    );
  }, noOutputPollMs);
  ...
  relayLifetimeTimer = setTimeout(() => {
    if (disposed) return;
    emit(
      `${relayLabel} stream relay timed out after ${Math.max(1, Math.round(maxRelayLifetimeMs / 1e3))}s without completion.`,
  ...
  const unsubscribe = onAgentEvent((event) => {
    ...
    if (phase === "end") {
      ...
      const durationMs =
        startedAt != null && endedAt != null && endedAt >= startedAt ? endedAt - startedAt : void 0;
      if (durationMs != null)
        emit(
          `${relayLabel} run completed in ${Math.max(1, Math.round(durationMs / 1e3))}s.`,
          `${contextPrefix}:done`,
        );
...
function resolveAcpSpawnRuntimePolicyError(params) {
  ...
  if (params.requesterSandboxed === true || requesterRuntime.sandboxed)
...
function countUntrackedActiveAcpRunsForOwner(ownerKey) {
  ...
  const trackedChildSessionKeys = new Set(
    tasks
      ...
      .map((task) => normalizeOptionalString(task.childSessionKey)),
  );
  return new Set(
    tasks.flatMap((task) => {
...
function hasSessionLocalHeartbeatRelayRoute(params) {
  ...
  const parentEntry = loadSessionStore(
    resolveStorePath(params.cfg.session?.store, { agentId: params.requesterAgentId }),
...
function prepareAcpThreadBinding(params) {
  ...
  const placementToUse =
    ...
    resolvePlacementWithoutChannelPlugin({ capabilities });
...
function resolveAcpSpawnBootstrapDeliveryPlan(params) {
  ...
  const fallbackThreadId =
    fallbackThreadIdRaw != null ? normalizeOptionalString(String(fallbackThreadIdRaw)) : void 0;
  ...
  const bindingMatchesRequesterConversation = Boolean(
    ...
    (params.binding?.conversation.parentConversationId ?? void 0) ===
      (requesterConversationRef.parentConversationId ?? void 0),
  );
  ...
  const useInlineDelivery =
    Boolean(params.requester.origin?.channel && inferredDeliveryTo) &&
...
async function spawnAcpDirect(params, ctx) {
  ...
  if (spawnMode === "session" && !requestThreadBinding)
    return createAcpSpawnFailure({
      ...
      error:
        'sessions_spawn(runtime="acp", mode="session") requires thread=true so the ACP session can stay bound to a channel thread. Retry with { mode: "session", thread: true } on a channel that exposes threads (e.g. Discord, Slack, Telegram topics), or use mode="run" for one-shot work.',
    });
  ...
  const streamLogPath =
    effectiveStreamToParent && parentSessionKey
      ? resolveAcpSpawnStreamLogPath({ childSessionKey: sessionKey })
      : void 0;
  ...
  try {
    const responseRunId = normalizeOptionalString(
      (
        await callGateway({
          method: "agent",
          params: {
...
//#endregion
export {
  ACP_SPAWN_SESSION_ACCEPTED_NOTE as a,
  resolveAcpSpawnRuntimePolicyError as c,
  ACP_SPAWN_SANDBOX_MODES as i,
  spawnAcpDirect as l,
  ACP_SPAWN_ERROR_CODES as n,
  ACP_SPAWN_STREAM_TARGETS as o,
  ACP_SPAWN_MODES as r,
  isSpawnAcpAcceptedResult as s,
  ACP_SPAWN_ACCEPTED_NOTE as t,
  cleanupFailedAcpSpawn as u,
};
