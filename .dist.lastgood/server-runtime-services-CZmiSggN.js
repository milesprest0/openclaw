import {
  i as resolveChannelRestartReason,
  r as evaluateChannelHealth,
} from "./channel-health-policy-DbJG9FMx.js";
import { n as isVitestRuntimeEnv } from "./env-GyTZ5xFK.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-CPuhMMK_.js";
...
function startChannelHealthMonitor(deps) {
    ...
    try {
      ...
      for (const [channelId, accounts] of Object.entries(snapshot.channelAccounts)) {
        ...
        for (const [accountId, status] of Object.entries(accounts)) {
          ...
          try {
            if (status.running)
              await channelManager.stopChannel(channelId, accountId, { manual: false });
  ...
  if (abortSignal?.aborted) stopped = true;
    ...
    log.info?.(
      `started (interval: ${Math.round(checkIntervalMs / 1e3)}s, startup-grace: ${Math.round(timing.monitorStartupGraceMs / 1e3)}s, channel-connect-grace: ${Math.round(timing.channelConnectGraceMs / 1e3)}s)`,
    );
  }
  return { stop };
}
...
function startGatewayChannelHealthMonitor(params) {
  ...
  return startChannelHealthMonitor({
    ...
    ...(staleEventThresholdMinutes != null && {
      staleEventThresholdMs: staleEventThresholdMinutes * 6e4,
    }),
...
function recoverPendingOutboundDeliveries(params) {
  (async () => {
    const { recoverPendingDeliveries } = await import("./delivery-queue-CPfKkeWi.js");
    const { deliverOutboundPayloads } = await import("./deliver-NKHYtB4R.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
  ...
  let stopRefresh;
  (async () => {
    const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-Bs9GR30v.js");
    if (stopped) return;
...
export {
  ...
  startGatewayChannelHealthMonitor as i,
  runGatewayPostReadyMaintenance as n,
  startGatewayRuntimeServices as o,
  scheduleGatewayPostReadyMaintenance as r,
