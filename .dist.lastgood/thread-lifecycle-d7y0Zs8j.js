...
import { a as createCodexAppServerToolResultExtensionRunner } from "./agent-harness-runtime-CRXkNxrN.js";
import {
...
import { i as isCodexAppServerConnectionClosedError } from "./client-PeiS3WVh.js";
import {
  l as resolveCodexPluginsPolicy,
  r as codexSandboxPolicyForTurn,
...
import { s as runAgentHarnessAfterToolCallHook } from "./native-hook-relay-AxDet15L.js";
import {
...
import {
  ...
  n as readCodexPluginInventory,
  t as ensureCodexPluginActivation,
} from "./plugin-activation-CPsfPsCt.js";
import {
  n as CODEX_GPT5_HEARTBEAT_PROMPT_OVERLAY,
  r as renderCodexPromptOverlay,
} from "./prompt-overlay-DzVK11nB.js";
import { t as isJsonObject } from "./protocol-DayKrDMy.js";
import {
  n as assertCodexThreadStartResponse,
  t as assertCodexThreadResumeResponse,
} from "./protocol-validators-BXWxEgv_.js";
import { r as isModernCodexModel } from "./provider-B-1EByt3.js";
import {
  i as readCodexAppServerBinding,
  n as isCodexAppServerNativeAuthProfile,
  o as writeCodexAppServerBinding,
  t as clearCodexAppServerBinding,
} from "./session-binding-o-m5DVaT.js";
import { t as createAgentToolResultMiddlewareRunner } from "./tool-result-middleware-CJ4A8Wvz.js";
...
function applyCodexDynamicToolProfile(tools, config) {
  ...
  if ((config.codexDynamicToolsProfile ?? "native-first") === "native-first")
    for (const name of CODEX_NATIVE_FIRST_DYNAMIC_TOOL_EXCLUDES) excludes.add(name);
...
async function buildCodexPluginThreadConfig(params) {
  ...
  for (const record of inventory.records) {
    if (record.activationRequired) {
      if (
        !activationResults.find((item) => item.identity.configKey === record.policy.configKey)?.ok
      )
        continue;
    }
...
async function startOrResumeThread(params) {
  ...
  if (binding?.threadId) {
    ...
    if (pluginBindingStale) {
      log.debug("codex app-server plugin app config changed; starting a new thread", {
        threadId: binding.threadId,
      });
  ...
  if (binding?.threadId)
    if (
      ...
      if (preserveExistingBinding)
        ...
        log.debug("codex app-server dynamic tool catalog changed; starting a new thread", {
          threadId: binding.threadId,
        });
        await clearCodexAppServerBinding(params.params.sessionFile);
      }
...
//#endregion
export {
  buildTurnStartParams as a,
  buildCodexPluginThreadConfig as c,
  createCodexDynamicToolBridge as d,
  applyCodexDynamicToolProfile as f,
  buildThreadStartParams as i,
  buildCodexPluginThreadConfigInputFingerprint as l,
  buildDeveloperInstructions as n,
  codexDynamicToolsFingerprint as o,
  normalizeCodexDynamicToolName as p,
  buildThreadResumeParams as r,
  startOrResumeThread as s,
  areCodexDynamicToolFingerprintsCompatible as t,
  shouldBuildCodexPluginThreadConfig as u,
};
