...
import {
  a as createAuthRateLimiter,
  o as normalizeRateLimitClientIp,
  r as AUTH_RATE_LIMIT_SCOPE_HOOK_AUTH,
} from "./auth-rate-limit-Es6mTeJX.js";
import { c as resolveHookExternalContentSource } from "./external-content-BCW9yxs8.js";
import { o as requestHeartbeat } from "./heartbeat-wake-2F-mloyP.js";
import {
  a as isHookAgentAllowed,
  c as normalizeHookDispatchSessionKey,
  d as readJsonBody,
  f as resolveHookChannel,
  g as resolveHookTargetAgentId,
  h as resolveHookSessionKey,
  i as getHookSessionKeyPrefixError,
  l as normalizeHookHeaders,
  m as resolveHookIdempotencyKey,
  n as getHookAgentPolicyError,
  o as isSessionKeyAllowedByPrefix,
  p as resolveHookDeliver,
  r as getHookChannelError,
  s as normalizeAgentPayload,
  t as extractHookToken,
  u as normalizeWakePayload,
  v as applyHookMappings,
} from "./hooks-Cc0PqL86.js";
...
function createHooksRequestHandler(opts) {
  ...
  const resolveHookClientKey = (req) => {
    ...
    return normalizeRateLimitClientIp(
  ...
  const buildHookReplayCacheKey = (params) => {
    ...
    return `${tokenFingerprint}:${createHash("sha256")
      .update(
        JSON.stringify({
      ...
      .digest("hex")}:${idempotencyFingerprint}`;
  };
...
function resolveHookRunSummary(result) {
  return (
    (result.status !== "ok" ? normalizeOptionalString(result.diagnostics?.summary) : void 0) ||
  ...
  const dispatchAgentHook = (value) => {
    ...
    (async () => {
      try {
        ...
        const { runCronIsolatedAgentTurn } = await import("./isolated-agent-WeuAFo90.js");
