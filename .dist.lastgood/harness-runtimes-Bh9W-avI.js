import { t as resolveModelRuntimePolicy } from "./model-runtime-policy-DVwYLRbI.js";
...
function pushConfiguredModelRuntimeIds(config, runtimes) {
  ...
  const pushModelMapRuntimeIds = (models) => {
    ...
    for (const entry of Object.values(models)) {
      ...
      const runtime = normalizeRuntimeId(
        isRecord(entry.agentRuntime) ? entry.agentRuntime.id : void 0,
      );
  ...
  if (Array.isArray(config.agents?.list))
    for (const agent of config.agents.list) {
      ...
      pushCodexForOpenAIModel(
        ...
        typeof agent.id === "string" ? agent.id : void 0,
      );
    }
