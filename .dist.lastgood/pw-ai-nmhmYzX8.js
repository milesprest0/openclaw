...
import { n as markPwAiLoaded } from "./pw-ai-state-BlSN1YsS.js";
import {
  c as ACT_MAX_CLICK_DELAY_MS,
  d as resolveActInteractionTimeoutMs,
  f as resolveActWaitTimeoutMs,
  i as parseRoleRef,
  l as ACT_MAX_WAIT_TIME_MS,
  n as buildRoleSnapshotFromAriaSnapshot,
  p as matchBrowserUrlPattern,
  r as getRoleSnapshotStats,
  t as buildRoleSnapshotFromAiSnapshot,
...
function isRecoverableStalePageSelectionError(err, reusedCachedBrowser) {
  ...
  if (err instanceof Error && err.message.includes("No pages available in the connected browser."))
    return true;
  if (err instanceof BrowserTabNotFoundError) return true;
  return (err instanceof Error ? err.message : formatErrorMessage(err))
    .toLowerCase()
    .includes("tab not found");
}
...
function evictStalePlaywrightBrowserConnection(cdpUrl) {
  takeCachedPlaywrightBrowserConnection(cdpUrl)
    ?.browser.close()
    .catch(() => {});
}
...
function ensurePageState(page) {
  ...
  if (!observedPages.has(page)) {
    ...
    page.on("download", (download) => {
      ...
      const managedPath = buildManagedDownloadPath(
        sanitizeUntrustedFileName(download.suggestedFilename?.() || "download.bin", "download.bin"),
      );
  ...
  const connectWithRetry = async () => {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1)
      try {
        ...
        const endpoint =
          (await getChromeWebSocketUrl(normalized, timeout, ssrfPolicy).catch(() => null)) ??
          normalized;
  ...
  try {
    return (
      (normalizeOptionalString$1(
        (await session.send("Target.getTargetInfo"))?.targetInfo?.targetId,
      ) ??
        "") ||
      null
    );
...
async function findPageByTargetIdViaTargetList(pages, targetId, cdpUrl, ssrfPolicy) {
  ...
  return matchPageByTargetList(
    pages,
    await fetchJson(appendCdpPath(cdpHttpBase, "/json/list"), 2e3),
    targetId,
  );
}
...
function refLocator(page, ref) {
  ...
  if (/^e\d+$/.test(normalized)) {
    ...
    if (state?.roleRefsMode === "aria")
      return (
        state.roleRefsFrameSelector ? page.frameLocator(state.roleRefsFrameSelector) : page
...
async function tryTerminateExecutionViaCdp(opts) {
  ...
  const wsUrlRaw =
    normalizeOptionalString$1(
      pages.find((p) => normalizeOptionalString$1(p.id) === targetId)?.webSocketDebuggerUrl,
    ) ?? "";
  if (!wsUrlRaw) return;
  ...
  await withCdpSocket(
    ...
    async (send) => {
      ...
      try {
        if (needsAttach) {
          const attachedSessionId = normalizeOptionalString$1(
            (
...
async function snapshotRoleViaPlaywright(opts) {
  ...
  if (opts.refsMode === "aria") {
    if (normalizeOptionalString$1(opts.selector) || normalizeOptionalString$1(opts.frameSelector))
...
async function takeScreenshotViaPlaywright(opts) {
  ...
  if (opts.ref) {
    ...
    return {
      buffer: await refLocator(page, opts.ref).screenshot({
        type,
  ...
  if (opts.element) {
    if (opts.fullPage) throw new Error("fullPage is not supported for element screenshots");
    return {
      buffer: await page.locator(opts.element).first().screenshot({
        type,
        timeout: opts.timeoutMs,
      }),
    };
...
async function responseBodyViaPlaywright(opts) {
  ...
  const maxChars =
    typeof opts.maxChars === "number" && Number.isFinite(opts.maxChars)
      ? Math.max(1, Math.min(5e6, Math.floor(opts.maxChars)))
      : 2e5;
  const timeout = normalizeTimeoutMs(opts.timeoutMs, 2e4);
  const page = await getPageForTargetId(opts);
  ensurePageState(page);
...
async function setGeolocationViaPlaywright(opts) {
  ...
  const origin =
    normalizeOptionalString$1(opts.origin) ||
    (() => {
      try {
