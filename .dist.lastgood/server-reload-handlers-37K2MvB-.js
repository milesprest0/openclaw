...
import {
  n as loadInstalledPluginIndexInstallRecords,
  r as loadInstalledPluginIndexInstallRecordsSync,
...
import {
  ...
  n as listPluginInstallTimestampMetadataPaths,
  r as listPluginInstallWholeRecordPaths,
...
import { _ as resolveHooksConfig } from "./hooks-Cc0PqL86.js";
import { n as formatConfigIssueLines } from "./issue-format-DZoENRCj.js";
import "./pi-bundle-mcp-tools-D10hXSOR.js";
import { o as resetModelCatalogCache } from "./model-catalog-CJYpfYgw.js";
import { i as disposeAllSessionMcpRuntimes } from "./pi-bundle-mcp-runtime-_btntJtc.js";
import { t as bumpSkillsSnapshotVersion } from "./refresh-state-BmEneb6Z.js";
import {
  a as deferGatewayRestartUntilIdle,
  d as resolveGatewayRestartDeferralTimeoutMs,
  o as emitGatewayRestart,
...
import { m as resolveConfigWriteFollowUp } from "./runtime-snapshot-B0aZX-OG.js";
import { t as buildGatewayCronService } from "./server-cron-Bbp6BMec.js";
import { r as markGatewayModelCatalogStaleForReload } from "./server-model-catalog-CPOc7rIW.js";
import {
  a as startGatewayCronWithLogging,
  i as startGatewayChannelHealthMonitor,
} from "./server-runtime-services-CZmiSggN.js";
import {
  a as setCurrentSharedGatewaySessionGeneration,
  n as disconnectStaleSharedGatewayAuthClients,
} from "./server-shared-auth-generation-CFRckQqN.js";
import { r as resetDirectoryCache } from "./target-resolver-_EpoEgO9.js";
...
function createGatewayReloadHandlers(params) {
  ...
  const waitForActiveWorkBeforeChannelReload = async (channels, nextConfig) => {
    ...
    params.logReload.warn(
      `config change requires channel reload (${channelNames}) — deferring until ${initialDetails.join(", ")} complete`,
    );
    ...
    while (true) {
      ...
      if (timeoutMs !== void 0 && elapsedMs >= timeoutMs) {
        ...
        params.logReload.warn(
          `channel reload timeout after ${elapsedMs}ms with ${remaining.join(", ")} still active; reloading channels anyway`,
        );
        return;
      }
  ...
  const applyHotReload = async (plan, nextConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      const [{ stopGmailWatcher }, { startGmailWatcherWithLogs }] = await Promise.all([
        import("./gmail-watcher-C5OhC7U8.js"),
        import("./gmail-watcher-lifecycle-Bi5jo9Ue.js"),
      ]);
      ...
      deferGatewayRestartUntilIdle({
        ...
        hooks: {
          ...
          onStillPending: (_pending, elapsedMs) => {
            ...
            const taskBlockers = formatTaskBlockers();
            params.logReload.warn(
              `restart still deferred after ${elapsedMs}ms with ${remaining.join(", ")} active${taskBlockers ? ` (${taskBlockers})` : ""}`,
            );
          },
          onTimeout: (_pending, elapsedMs) => {
            ...
            restartPending = false;
            params.logReload.warn(
              `restart timeout after ${elapsedMs}ms with ${remaining.join(", ")} still active${taskBlockers ? ` (${taskBlockers})` : ""}; forcing restart`,
            );
          },
          onCheckError: (err) => {
            restartPending = false;
