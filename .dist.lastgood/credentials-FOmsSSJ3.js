import {
  ...
  i as trimCredentialToUndefined,
  t as createGatewayCredentialPlan,
...
function resolveLocalGatewayCredentials(params) {
  const localResolved = resolveGatewayCredentialsFromValues({
    configToken: params.plan.localToken.configured
      ? params.plan.localToken.value
      : params.plan.remoteToken.value,
    configPassword: params.plan.localPassword.configured
  ...
  if (
    ...
    params.localPasswordPrecedence === "config-first" &&
    !params.plan.localPassword.value &&
...
function resolveRemoteGatewayCredentials(params) {
  ...
  const password =
    params.remotePasswordFallback === "remote-only"
      ? params.plan.remotePassword.value
  ...
  const localTokenFallback =
    params.remoteTokenFallback === "remote-only" ? void 0 : params.plan.localToken.value;
  const localPasswordFallback =
    params.remotePasswordFallback === "remote-only" ? void 0 : params.plan.localPassword.value;
  if (
...
//#endregion
export {
  resolveGatewayProbeCredentialsFromConfig as a,
  resolveGatewayCredentialsFromValues as i,
  isGatewaySecretRefUnavailableError as n,
  resolveGatewayCredentialsFromConfig as r,
  GatewaySecretRefUnavailableError as t,
};
