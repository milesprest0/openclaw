import { t as sanitizeAssistantVisibleText } from "./assistant-visible-text-DnuPT-bL.js";
...
import { o as sendVoiceMessageDiscord } from "./send-MjDHAxf5.js";
...
function sanitizeDiscordFrontChannelReplyPayloads(payloads) {
  ...
  for (const payload of payloads) {
    ...
    if (
      !resolveSendableOutboundReplyParts(nextPayload).hasContent &&
      !hasNonTextReplyPayloadContent(nextPayload)
    )
      continue;
...
function resolveBindingIdentity(cfg, binding) {
  if (!binding) return;
  const identity = {
    name: (`🤖 ${binding.label?.trim() || binding.agentId}`.trim() || "🤖 agent").slice(0, 80),
  };
  try {
...
function createDiscordDeliveryDeps(params) {
  return {
    ...
    discordVoice: (to, audioPath, opts) =>
...
//#endregion
export {
  resolveReplyContext as i,
  buildDirectLabel as n,
  buildGuildLabel as r,
  deliverDiscordReply as t,
};
