...
import "./record-shared-dnv_gtJK.js";
import {
  Category,
  ClientEvent,
  Filter,
...
import { n as resolveMatrixRoomKeyBackupReadinessError } from "./backup-health-1pC5l6ya.js";
import {
  n as formatMatrixErrorReason,
  r as isMatrixNotFoundError,
  t as formatMatrixErrorMessage,
} from "./errors-DQ-r4gaL.js";
import {
  a as matrixEventToRaw,
  n as createMatrixGuardedFetch,
  o as parseMxc,
  t as MatrixAuthedHttpClient,
...
import { t as createMatrixJsSdkClientLogger } from "./logging-OuOcffLy.js";
import {
  n as isRepairableSecretStorageAccessError,
  r as MATRIX_IDB_PERSIST_INTERVAL_MS,
  t as MatrixRecoveryKeyStore,
} from "./recovery-key-store-B5hhZBrh.js";
import {
  i as throwIfMatrixStartupAborted,
  n as createMatrixStartupAbortError,
} from "./startup-abort-gz1WDgpo.js";
import { t as claimCurrentTokenStorageState } from "./storage-B9keKclB.js";
import { o as normalizeNullableString } from "./string-coerce-BdEutqX5.js";
import {
...
async function loadMatrixCryptoRuntime() {
  matrixCryptoRuntimePromise ??= import("./crypto-runtime-BstMQfzv.js").then((runtime) => {
...
var MatrixClient = class {
  ...
  async ensureCryptoSupportInitialized() {
    ...
    this.verificationManager ??= new runtime.MatrixVerificationManager({
      trustOwnDeviceAfterSas: async (deviceId) => {
  ...
  async waitForInitialSyncReady(params = {}) {
    ...
    await new Promise((resolve, reject) => {
      ...
      timeoutId = setTimeout(() => {
        settleReject(
          /* @__PURE__ */ new Error(
            `Matrix client did not reach a ready sync state within ${timeoutMs}ms`,
          ),
        );
      }, timeoutMs);
      timeoutId.unref?.();
  ...
  async bootstrapCryptoIfNeeded(abortSignal) {
    ...
    if (!initial.crossSigningPublished || initial.ownDeviceVerified === false)
      if ((await this.getOwnDeviceVerificationStatus()).signedByOwner)
        LogService.warn(
          "MatrixClientLite",
  ...
  async getUserId() {
    ...
    const resolved = (
      await this.doRequest("GET", "/_matrix/client/v3/account/whoami")
    ).user_id?.trim();
  ...
  async redactEvent(roomId, eventId, reason) {
    return (
      await this.client.redactEvent(roomId, eventId, void 0, reason?.trim() ? { reason } : void 0)
    ).event_id;
  }
  ...
  async uploadContent(file, contentType, filename) {
    return (
      await this.client.uploadContent(new Uint8Array(file), {
  ...
  async verifyWithRecoveryKey(rawRecoveryKey) {
    ...
    if (backupUsableBeforeStagedRecovery && storedRecoveryKeyMatches) {
      const status = await this.getOwnDeviceVerificationStatus();
      const backupUsable =
        resolveMatrixRoomKeyBackupReadinessError(status.backup, { requireServerBackup: true }) ===
        null;
  ...
  async getOwnCrossSigningPublicationStatus() {
    ...
    try {
      const response = await this.doRequest("POST", "/_matrix/client/v3/keys/query", void 0, {
        device_keys: { [userId]: [] },
      });
  ...
  async bootstrapOwnDeviceVerification(params) {
    const pendingVerifications = async () =>
    ...
    if (success)
      this.recoveryKeyStore.commitStagedRecoveryKey({
        keyId: await this.resolveDefaultSecretStorageKeyId(this.client.getCrypto()),
      });
    else this.recoveryKeyStore.discardStagedRecoveryKey();
    return {
      success,
  ...
  async resolveRoomKeyBackupVersion() {
    try {
      return normalizeOptionalString(
        (await this.doRequest("GET", "/_matrix/client/v3/room_keys/version")).version,
      );
    } catch {
      return null;
    }
  ...
  async refreshDmCache() {
    ...
    for (const value of Object.values(direct)) {
      ...
      for (const roomId of value)
        if (typeof roomId === "string" && roomId.trim()) this.dmRoomIds.add(roomId);
    }
    return true;
  }
};
