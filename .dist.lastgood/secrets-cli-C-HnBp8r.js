...
import {
  n as getPath,
  r as setPathCreateStrict,
  t as deletePathStrict,
} from "./path-utils-zSv0Iydy.js";
import { v as resolveStateDir } from "./paths-Cnwfh6dH.js";
import {
  r as listKnownSecretEnvVarNames,
  t as getProviderEnvVars,
} from "./provider-env-vars-DkmNxQP4.js";
import { r as normalizeProviderId } from "./provider-id-CG9pXYPs.js";
import {
  a as formatExecSecretRefIdValidationMessage,
  c as isValidSecretProviderAlias,
  l as resolveDefaultSecretProviderAlias,
  o as isValidExecSecretRefId,
  u as secretRefKey,
...
import { a as prepareSecretsRuntimeSnapshot } from "./runtime-DyGaG6oO.js";
import { n as defaultRuntime } from "./runtime-kqN0Yohi.js";
import "./config-CMOdcWpc.js";
import {
  n as hasConfiguredPlaintextSecretValue,
  r as isExpectedResolvedSecretValue,
  t as assertExpectedResolvedSecretValue,
} from "./secret-value-5JX0SNC9.js";
...
import { l as resolveAuthStorePath } from "./source-check-DUt28S7k.js";
import {
  c as loadAuthProfileStoreForSecretsRuntime,
  h as loadPersistedAuthProfileStore,
  m as coercePersistedAuthProfileStore,
...
import {
  a as listAuthProfileSecretTargetEntries,
  c as resolvePlanTargetAgainstRegistry,
  n as discoverConfigSecretTargets,
  t as discoverAuthProfileSecretTargets,
} from "./target-registry-BECxoPqR.js";
import { r as theme } from "./theme-CiH_wF8x.js";
import { m as resolveSecretInputRef, o as coerceSecretRef } from "./types.secrets-CaNC1eIn.js";
import { c as isRecord, d as resolveConfigDir, p as resolveUserPath } from "./utils-BGRcpLKt.js";
import { w as SecretProviderSchema } from "./zod-schema.core-CkFaNmbg.js";
...
function isSecretsApplyPlan(value) {
  ...
  for (const target of typed.targets) {
    ...
    if (
      ...
      !candidate.path.trim() ||
      (candidate.pathSegments !== void 0 && !Array.isArray(candidate.pathSegments)) ||
      !resolved ||
      !ref ||
      typeof ref !== "object" ||
    ...
    if (resolved.entry.configFile === "auth-profiles.json") {
      if (typeof candidate.agentId !== "string" || candidate.agentId.trim().length === 0)
        return false;
      if (
        candidate.authProfileProvider !== void 0 &&
...
async function projectPlanState(params) {
  const { snapshot, writeOptions } = await createSecretsConfigIO({
    env: params.env,
...
function applyConfigTargetMutations(params) {
  ...
  for (const { target, resolved } of resolvedTargets) {
    if (resolved.entry.configFile === "auth-profiles.json") {
      if (
        applyAuthProfileTargetMutation({
          target,
          resolved,
    ...
    const previous = getPath(params.nextConfig, targetPathSegments);
    if (isNonEmptyString(previous)) scrubbedValues.add(previous.trim());
    if (setPathCreateStrict(params.nextConfig, targetPathSegments, target.ref))
      configChanged = true;
...
function scrubAuthStoresForProviderTargets(params) {
  ...
  for (const authStorePath of listAuthProfileStorePaths(params.nextConfig, params.stateDir)) {
    const parsed =
      params.authStoreByPath.get(authStorePath) ?? readJsonObjectIfExists(authStorePath).value;
...
async function collectUnresolvedRefFindings(params) {
  ...
  for (const assignment of params.collector.refAssignments) {
    ...
    if (!isExpectedResolvedSecretValue(resolvedByRefKey.get(key), assignment.expected))
      addFinding(params.collector, {
        code: "REF_UNRESOLVED",
        severity: "error",
        file: assignment.file,
...
function configureCandidateSortKey(candidate) {
  if (candidate.configFile === "auth-profiles.json")
    return `auth-profiles:${candidate.agentId ?? ""}:${candidate.path}`;
...
function removeSecretProvider(config, providerAlias) {
  ...
  if (isRecord(config.secrets?.defaults)) {
    ...
    if (defaults && defaults.env === void 0 && defaults.file === void 0 && defaults.exec === void 0)
...
function validateEnvNameCsv(value) {
  ...
  for (const entry of entries)
    if (!ENV_NAME_PATTERN.test(entry)) return `Invalid env name: ${entry}`;
}
      ...
      assertNoCancel(
        await text({
          ...
          validate: (value) => {
            ...
            if (!trimmed) return;
            if (parseOptionalPositiveInt(trimmed, params.max) === void 0)
...
function loadAuthProfileStoreForConfigure(params) {
  return (
    loadPersistedAuthProfileStore(resolveAgentDir(params.config, params.agentId)) ?? {
      version: 1,
      profiles: {},
    }
  );
  ...
  const trustedDirsRaw = assertNoCancel(
    await text({
      ...
      validate: (value) => {
        const entries = parseCsv(value ?? "");
        for (const entry of entries)
          if (!isAbsolutePathValue(entry)) return `Trusted dir must be absolute: ${entry}`;
      },
    }),
    "Secrets configure cancelled.",
  );
...
async function promptProviderConfig(source, current) {
  if (source === "env")
    return await promptEnvProvider(current?.source === "env" ? current : void 0);
  if (source === "file")
    return await promptFileProvider(current?.source === "file" ? current : void 0);
...
async function configureProvidersInteractive(config) {
  while (true) {
    ...
    if (action === "edit") {
      ...
      if (!current) continue;
      const nextProviderConfig = await promptProviderConfig(
        await promptProviderSource(current.source),
        current,
      );
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          ...
          candidate.isDerived === true ? "derived" : void 0,
        ]
          .filter(Boolean)
          .join(" | "),
      }));
...
function registerSecretsCli(program) {
  ...
  addGatewayClientOptions(
    ...
    try {
      const result = await callGatewayFromCli("secrets.reload", opts, void 0, {
        expectFinal: false,
      });
      if (opts.json) {
