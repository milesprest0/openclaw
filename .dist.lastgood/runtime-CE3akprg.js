...
import "./runtime-api-l8EvCtqq.js";
...
import { n as AcpRuntimeError$1 } from "./errors-DxZLVXmo.js";
import {
  c as hashAcpxProcessCommand,
  l as withAcpxLeaseEnvironment,
  n as isOpenClawOwnedAcpxProcessCommand,
  o as createAcpxProcessLeaseId,
  t as cleanupOpenClawOwnedAcpxProcessTree,
} from "./process-reaper-BC8jy60A.js";
...
const OUTPUT_ERROR_ORIGINS = ["cli", "runtime", "queue", "acp"];
...
function isUsageLike(error) {
  ...
  return (
    ...
    asRecord$1$1(error)?.code === "commander.invalidArgument"
  );
}
...
function normalizeOutputError(error, options = {}) {
  ...
  const detailCode =
    meta.detailCode ??
    options.detailCode ??
    (error instanceof AuthPolicyError || isAcpAuthRequiredPayload(acp) ? "AUTH_REQUIRED" : void 0);
  return {
    code,
...
function findBuiltInAgentPackage(agentCommand) {
  ...
  return Object.values(BUILT_IN_AGENT_PACKAGES).find(
    (spec) =>
...
function defaultResolvePackageRoot(packageName) {
  ...
  while (true) {
    ...
    if (fs.existsSync(manifestPath))
      try {
        if (JSON.parse(fs.readFileSync(manifestPath, "utf8")).name === packageName)
...
function resolvePackageBin(spec, manifest) {
  ...
  return (
    ...
    (Object.keys(manifest.bin).length === 1 ? Object.values(manifest.bin)[0] : void 0)
  );
}
...
function isToolUse(raw) {
  ...
  return (
    ...
    hasOwn$1(record, "input") &&
    typeof record.is_input_complete === "boolean" &&
    (record.thought_signature === void 0 ||
...
function parseConversationRecord(record) {
  ...
  return {
    title:
      record.title === void 0 || record.title === null || typeof record.title === "string"
        ? record.title
        : null,
...
async function resolvePermissionRequest(params, mode, nonInteractivePolicy = "deny") {
  ...
  if (isAutoApprovedReadKind(inferToolKind(params)) && allowOption)
...
const GEMINI_ACP_FLAG_VERSION = [0, 33, 0];
...
async function detectGeminiVersion(command) {
  const versionLine = (await readCommandOutput(command, ["--version"], GEMINI_VERSION_TIMEOUT_MS))
    ?.split(/\r?\n/)
    .map((line) => line.trim())
...
var AcpClient = class {
  ...
  updateRuntimeOptions(options) {
    ...
    if (options.nonInteractivePermissions !== void 0)
    ...
    if (options.suppressSdkConsoleErrors !== void 0)
...
function ensureToolUseContent(agent, toolCallId) {
  for (const content of agent.content)
    if (isAgentToolUseContent(content) && content.ToolUse.id === toolCallId) return content.ToolUse;
  const created = {
    id: toolCallId,
    name: "tool_call",
    raw_input: "{}",
...
function applyToolCallUpdate(agent, update) {
  ...
  if (hasOwn(update, "title"))
    tool.name = normalizeAgentName$2(update.title) ?? tool.name ?? "tool_call";
...
function usageToTokenUsage(update) {
  ...
  if (
    normalized.input_tokens === void 0 &&
    normalized.output_tokens === void 0 &&
    normalized.cache_creation_input_tokens === void 0 &&
    normalized.cache_read_input_tokens === void 0
  )
    return;
  return normalized;
}
  ...
  return {
    ...
    session_options: state.session_options
      ? {
          ...
          ...(state.session_options.system_prompt !== void 0
            ? {
                system_prompt:
...
function recordPromptSubmission(conversation, prompt, timestamp = isoNow$3()) {
  const userContent = (typeof prompt === "string" ? textPrompt(prompt) : prompt)
  ...
  conversation.messages.push({
    User: {
      ...
      content: userContent.map((content) => {
        if ("Text" in content)
          return { Text: trimRuntimeText(content.Text, MAX_RUNTIME_AGENT_TEXT_CHARS) };
        return content;
      }),
    },
  });
...
function assertRequestedModelSupported(params) {
  if (!params.models) {
    ...
    throw new RequestedModelUnsupportedError(
      `Cannot ${params.context === "replay" ? "replay saved model" : "apply --model"} "${params.requestedModel}": the ACP agent did not advertise model support. Generic model selection requires ACP models plus session/set_model support, or an adapter-specific startup model flag.`,
    );
  }
  if (
    !new Set(params.models.availableModels.map((model) => model.modelId)).has(params.requestedModel)
  )
    throw new RequestedModelUnsupportedError(
      `Cannot ${params.context === "replay" ? "replay saved model" : "apply --model"} "${params.requestedModel}": the ACP agent did not advertise that model. Available models: ${formatAvailableModelIds(params.models)}.`,
    );
}
...
function sessionOptionsFromRecord(record) {
  ...
  if (typeof storedSystemPrompt === "string" && storedSystemPrompt.length > 0)
    ...
    typeof storedSystemPrompt.append === "string" &&
    storedSystemPrompt.append.length > 0
  )
    sessionOptions.systemPrompt = { append: storedSystemPrompt.append };
...
function resolveStatusTextForTag(params) {
  ...
  if (tag === "plan") {
    const content = asTrimmedString(
      (Array.isArray(payload.entries) ? payload.entries : []).find((entry) => isRecord(entry))
        ?.content,
    );
...
var AcpRuntimeManager = class {
  ...
  async retainPersistentClientAfterTurn(input) {
    ...
    if (
      !!record.acpxRecordId.includes(":oneshot:") ||
      record.closed ||
    ...
    (async () => {
      ...
      try {
        ...
        if (record && conversation) {
          ...
          if (!closed && client)
            pooled = await this.retainPersistentClientAfterTurn({
              record,
              client,
            });
        }
  ...
  async getStatus(handle) {
    ...
    return {
      ...
      details: {
        ...
        ...(record.acpx?.config_options !== void 0
  ...
  async setMode(handle, mode, sessionMode = "persistent") {
    ...
    if (controller) await controller.setSessionMode(mode);
      ...
      targetRecord = (
        await this.withRuntimeControlSession(record, sessionMode, async ({ client, sessionId }) => {
        ...
        await this.withRuntimeControlSession(
          ...
          async ({ client, sessionId, record: connectedRecord }) => {
            applyConfigOptionsToRecord(
              connectedRecord,
              (await client.setSessionConfigOption(sessionId, key, value))?.configOptions,
            );
...
var AcpxRuntime$1 = class {
  ...
  startTurn(input) {
    ...
    const turnPromise = this.getManager().then((manager) =>
      manager.startTurn({
        handle,
        text: input.text,
  ...
  async close(input) {
    ...
    await (
      await this.getManager()
    ).close(handle, { discardPersistentState: input.discardPersistentState });
  }
  async getManager() {
...
function extractGeneratedWrapperPath(command) {
  return (
    splitCommandParts(command ?? "").find(
      (part) =>
        basename$1(part) === "codex-acp-wrapper.mjs" ||
        basename$1(part) === "claude-agent-acp-wrapper.mjs",
    ) ?? ""
  );
}
function selectCurrentSessionLease(params) {
  ...
  for (const lease of candidates)
    if (!selected || lease.startedAt > selected.startedAt) selected = lease;
  return selected;
}
...
function isOpenClawBridgeCommand(command) {
  ...
  if (basename$1(parts[0] ?? "") === OPENCLAW_BRIDGE_EXECUTABLE)
...
var AcpxRuntime = class {
  ...
  async resolveCommandForHandle(handle) {
    const recordCommand = readAgentCommandFromRecord(
  ...
  async canReuseStablePersistentSession(params) {
    ...
    const existingSessionId =
      typeof existing === "object" && existing !== null ? existing.acpSessionId : void 0;
      ...
      if (
        ...
        const override =
          key === "model"
            ...
            : normalizeCodexAcpModelOverride(void 0, input.value);
